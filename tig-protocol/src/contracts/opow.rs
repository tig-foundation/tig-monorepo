...
pub(crate) async fn update(cache: &mut AddBlockCache) {
    ...
    for challenge_id in active_challenge_ids.iter() {
        ...
        let challenge_data = active_challenges_block_data.get_mut(challenge_id).unwrap();
        let mut qualifier_solutions = solutions
            .iter()
            .filter(|(settings, _, _)| {
                challenge_data
                    .qualifier_difficulties
            ...
            .map(|(settings, &num_solutions, &num_nonces)| {
                (
                    settings.player_id.clone(),
                    num_nonces,
                    num_solutions as f64 / num_nonces as f64,
                )
            })
            .collect::<Vec<_>>();
        qualifier_solutions.sort_by(|(_, _, a), (_, _, b)| a.partial_cmp(b).unwrap());
        let median_nonces = qualifier_solutions
            .iter()
            .map(|(_, num_nonces, _)| num_nonces)
            .sum::<u32>()
            / 2;

        let mut cumulative_nonces = 0;
        challenge_data.median_solution_ratio = qualifier_solutions
            .iter()
            .find_map(|(_, num_nonces, solution_ratio)| {
                cumulative_nonces += num_nonces;
                if cumulative_nonces >= median_nonces {
                    Some(*solution_ratio)
                } else {
                    None
                }
        ...
        let mut player_weighted_reliability: HashMap<String, f64> = HashMap::new();
        let mut player_nonces: HashMap<String, u32> = HashMap::new();

        for (player_id, num_nonces, solution_ratio) in qualifier_solutions.iter() {
            let reliability = if *solution_ratio >= challenge_data.median_solution_ratio {
                config.opow.max_reliability
            } else {
                config.opow.min_reliability
            };

            let weighted_reliability = reliability * (*num_nonces as f64);

            *player_weighted_reliability
                .entry(player_id.clone())
                .or_insert(0.0) += weighted_reliability;
            *player_nonces.entry(player_id.clone()).or_insert(0) += num_nonces;
        }

        for player_id in player_weighted_reliability.keys() {
            let opow_data = active_opow_block_data.get_mut(player_id).unwrap();
            opow_data.average_reliability_by_challenge.insert(
                challenge_id.clone(),
                player_weighted_reliability[player_id] / player_nonces[player_id] as f64,
            );
        }
    }
                    ...
                    .map(|player_id| {
                        (
                            ...
                            PreciseNumber::from(
                                ...
                                *active_opow_block_data[player_id]
                                    .average_reliability_by_challenge
                                    .get(challenge_id)
                                    .unwrap_or(&0.0),
                            ),
                        )
