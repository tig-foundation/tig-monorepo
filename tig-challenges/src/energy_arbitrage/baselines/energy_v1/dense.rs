...
fn most_violated_line(challenge: &Challenge, flows: &[f64]) -> Option<Violation> {
    ...
    for (l, &flow) in flows.iter().enumerate() {
        ...
        if violation > crate::energy_arbitrage::constants::EPS_FLOW * limit {
...
fn compute_battery_schedule(
    ...
    let delta_t = crate::energy_arbitrage::constants::DELTA_T;
    let kappa_deg = crate::energy_arbitrage::constants::KAPPA_DEG;
    let beta_deg = crate::energy_arbitrage::constants::BETA_DEG;
