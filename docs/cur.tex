...
m,\qquad n,
\]
a singular-value structure, and a random seed.  The generator uses fixed
design constants $\Delta=10{,}000$, $a=13$, and spectral perturbation
$\varepsilon_\sigma=0.15$.  From these parameters it constructs $S=8$
related CUR sub-instances as described in Section~\ref{sec: instancegen}.
The challenge proceeds in three steps:
\begin{itemize}
    \item \textbf{Step 1. Generation.} The generator computes shared
    orthonormal bases $U$ and $V$ once, then constructs
    \[
    (A_1,k_1),\ldots,(A_S,k_S),
    \]
    where $A_s\in\mathbb{R}^{m\times n}$ has true rank $r_s$ and $k_s$ is
    the target rank.  True ranks and target-rank ratios are sampled using the
    stratified procedure in Section~\ref{sec: instancegen}.
    \item \textbf{Step 2. Solving.} For every $(A_s,k_s)$, the innovator's
    algorithm selects exactly $k_s$ distinct columns and $k_s$ distinct rows
    and returns a finite linking matrix $U_s^{\mathrm{link}}$.  These define
    \[
    CUR_s=C_sU_s^{\mathrm{link}}R_s.
    \]
    \item \textbf{Step 3. Verification and scoring.} Each CUR approximation
    is scored by the continuous rule in Section~\ref{sec: score}; the final
    challenge score is the arithmetic mean over all eight sub-instances.
\end{itemize}
The multiple sub-instances amortise expensive generation while testing the
algorithm across a range of true-rank and target-rank regimes.
...
An instance is specified by a random seed, matrix dimensions $m,n$, and a
choice of singular-value structure.  The initial design fixes
$\Delta=10{,}000$, $a=13$, and $\varepsilon_\sigma=0.15$.
Let
\[
\tau=\min\{m,n\}.
\]
...
\end{itemize}
 ...
 The value $\Delta=10{,}000$ and spectral-decay parameter $a=13$ are fixed
 across the initial tracks. They could be varied in future designs, and
 sparsity could also be introduced.
...
\section{Pseudo Code}
This section summarises the generator, harness, and verifier.

\begin{algorithm}
\caption{\textbf{Generate} the eight CUR sub-instances.}
\label{sec:algorithm_generate}
\begin{algorithmic}[1]
\STATE \textbf{Input:} track $(m,n,\mathrm{poly})$ and random seed
\STATE \textbf{Output:} $S=8$ matrix--target-rank pairs $(A_s,k_s)$
\STATE Set $\tau\gets\min\{m,n\}$, $\Delta\gets10{,}000$, $a\gets13$,
and $\varepsilon_\sigma\gets0.15$
\STATE Generate $G_U\in\mathbb R^{m\times\tau}$ and
$G_V\in\mathbb R^{n\times\tau}$ from the seed
\STATE Compute $G_UD=UR_U$ and $G_VD=VR_V$
\FOR{$s=1,\ldots,S$}
    \STATE Sample $\alpha_s\sim\operatorname{Uniform}(\mathcal A_s)$ and set
    $r_s\gets\lfloor\alpha_s\tau\rceil$
    \STATE Sample $\widetilde\rho_s\sim\operatorname{Uniform}(\mathcal R_s)$
\ENDFOR
\STATE Randomly permute $(\widetilde\rho_1,\ldots,\widetilde\rho_S)$ to obtain
$(\rho_1,\ldots,\rho_S)$
\FOR{$s=1,\ldots,S$}
    \STATE Sample $I_s\subseteq\{1,\ldots,\tau\}$ uniformly without replacement,
    with $|I_s|=r_s$
    \FOR{$j=1,\ldots,r_s$}
        \STATE Sample $\varepsilon_j^{(s)}\sim
        \operatorname{Uniform}(-\varepsilon_\sigma,\varepsilon_\sigma)$
        \STATE $\sigma_j^{(s)}\gets
        f_{\mathrm{type}}(r_s,j,a)(1+\varepsilon_j^{(s)})$
    \ENDFOR
    \STATE Sample an independent permutation $\pi_s$ of
    $\{1,\ldots,r_s\}$
    \STATE Construct
    $A_s\gets\sum_{j=1}^{r_s}\sigma_j^{(s)}
    u_{i_j}v_{i_{\pi_s(j)}}^T$
    \STATE $k_s\gets\lfloor\rho_s r_s\rceil$
\ENDFOR
\STATE \textbf{return} $(A_s,k_s)_{s=1}^S$
\end{algorithmic}
\end{algorithm}
...
\STATE $\mathrm{Solutions}\gets()$
\FOR{$s=1,\ldots,S$}
    \STATE $(\mathcal J_s,\mathcal I_s,U_s^{\mathrm{link}})
    \gets\mathrm{Innovator\_Submission}(A_s,k_s)$
    \STATE Append $(\mathcal J_s,\mathcal I_s,U_s^{\mathrm{link}})$ to
    $\mathrm{Solutions}$
\ENDFOR
\STATE \textbf{return} $\mathrm{Solutions}$
\end{algorithmic}
\end{algorithm}

\begin{algorithm}
\caption{\textbf{Verify} and score the submitted solutions.}
\label{sec:algorithm_verify}
\begin{algorithmic}[1]
\STATE \textbf{Input:} $(A_s,k_s)_{s=1}^S$ and submitted solutions
\STATE $\mathrm{TotalScore}\gets0$
\FOR{$s=1,\ldots,S$}
    \STATE Require $k_s$ distinct in-bounds row indices, $k_s$ distinct
    in-bounds column indices, and a finite $k_s\times k_s$ linking matrix
    \STATE $C_s\gets A_s[:,\mathcal J_s]$ and
    $R_s\gets A_s[\mathcal I_s,:]$
    \STATE $CUR_s\gets C_sU_s^{\mathrm{link}}R_s$
    \STATE Sort the known singular values as
    $\sigma_{(1)}^{(s)}\geq\cdots\geq\sigma_{(r_s)}^{(s)}$
    \STATE $E_s^*\gets
    \left(\sum_{j=k_s+1}^{r_s}(\sigma_{(j)}^{(s)})^2\right)^{1/2}$
    \STATE $q_s\gets\|A_s-CUR_s\|_F/E_s^*$
    \STATE $z_s\gets\log(\max\{q_s,1\})/\log(k_s+1)$
    \STATE $\mathrm{TotalScore}\gets\mathrm{TotalScore}+1/(1+z_s)$
\ENDFOR
