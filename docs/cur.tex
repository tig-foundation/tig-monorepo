...
\begin{itemize}
    ...
    $A_s$. The submitted solution contains only these row and column indices;
    the innovator never submits a linking matrix.
    \item \textbf{Step 3. Verification and scoring.} For every sub-instance,
    the verifier constructs $C_s$ and $R_s$ from the submitted indices and
    computes $U_s^{\mathrm{link}}$ using the fixed fast QR calculation in
    Section~\ref{sec:linking-matrices}. Each resulting approximation
...
\section{Verifier-Computed Linking Matrices}\label{sec:linking-matrices}
...
\mathbb{R}^{k_s\times k_s}$. The linking matrix is not part of an innovator's
solution. For every sub-instance, the verifier computes it with one canonical
fast method using thin QR factorisations
\[
C_s=Q_C T_C,
\qquad
R_s^T=Q_R T_R,
...
R_s=A_s[\mathcal I_s,:],
\]
where $\mathcal J_s$ and $\mathcal I_s$ are the submitted column and row
indices. The implementation fixes the arithmetic precision, QR convention,
projection order, and triangular-solve order so verification is deterministic.
The verifier's result is authoritative.

An innovator may call the challenge's shared fast-link evaluation helper while
searching over candidate index sets. This is only a local preview of the
verifier's calculation and is not serialized. The submitted solution for each
sub-instance is exactly
\[
(\mathcal J_s,\mathcal I_s).
\]
Consequently, the raw solution payload contains only $2k_s$ integer indices,
or $8k_s$ bytes when indices are represented as 32-bit integers, before
serialization overhead. No $k_s^2$ linking-matrix payload is returned.
...
\FOR{$s=1,\ldots,S$}
    ...
    \mathrm{Innovator\_Select}(A_s,k_s)$
...
and $\mathcal I_s$ contains the selected row indices. The innovator may use
$\mathrm{FixedFastLink}$ locally to estimate the quality the verifier will
assign, but no linking matrix is accepted in the solution. The matrices $C_s$
and $R_s$ are also reconstructed by the verifier and are never returned
explicitly.
