...
pub struct Hyperparameters {
    ...
    /// On capstone track target+dp_seed dominate; setting false removes ~33% of the
    /// joint_optimize_step compute budget.
    pub use_zero_seed: bool,
    /// Include the DP-preferred seed in joint_optimize_step (default: true).
    /// On capstone track (Q=10M ceiling, go=3) the target warm-start alone may converge;
    /// setting false reduces seeds [target,dp_seed]→[target], saving ~50% of
    /// joint_optimize_step.
    ...
    for _ in 0..hp.coord_polish_passes {
        ...
        for b in 0..challenge.num_batteries {
            ...
            for &candidate in &candidates[..n_cands] {
                ...
                // PTDF feasibility check: O(n_lines) with early exit.
                let delta = candidate - cur;
                let feasible = (0..n_lines).all(|l| {
                    (flows[l] + st_b[l] * delta).abs() <= limits[l] * (1.0 + EPS_FLOW) + 1e-6
                });
                if !feasible {
                    continue;
                }
...
fn policy(
    ...
    // use_zero_seed=false: omit zero-vector where target+dp_seed dominate.
    // use_dp_seed=false: omit dp_seed; target warm-start alone may converge on Q=10M tracks.
    let seeds = match (hp.use_dp_seed, hp.use_zero_seed) {
        (true,  true)  => vec![target, dp_seed, zero.clone()],
        (true,  false) => vec![target, dp_seed],
        (false, true)  => vec![target, zero.clone()],
