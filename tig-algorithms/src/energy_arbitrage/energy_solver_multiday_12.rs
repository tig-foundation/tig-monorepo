// energy_solver_multiday_12: STRUCTURAL — DA-trend-aware threshold shift. If next-K DA prices trend upward, raise discharge_th (wait for higher); if downward, lower it (act now). trend_factor=0 reproduces multiday_9.
...
pub fn help() {
    ...
    println!("  trend_factor: float range=[0.0, 1.0] default=0.0  # 0=multiday_9; positive shifts thresholds by trend over lookahead");
    println!();
    println!("Adds DA-trend-aware threshold shifting to multiday_9. trend_signal = (DA[t+K-1] - DA[t+1]).");
    println!("discharge_th_eff = discharge_th + trend_factor * trend_signal  (rising DA → higher th)");
    println!("charge_th_eff    = charge_th    - trend_factor * trend_signal  (rising DA → lower charge_th)");
    println!("All other params at multiday_9 winners (ct=8, dt=1, ew=66, ls=66, sf=1.273, p=2.545).");
}

fn parse_hp(h: &Option<Map<String, Value>>) -> Hyperparameters {
    let mut out = Hyperparameters::default();
    if let Some(m) = h {
        if let Some(v) = m.get("trend_factor").and_then(|v| v.as_f64()) {
            out.trend_factor = v;
        }
    }
    out
...
fn policy_with_hp(challenge: &Challenge, state: &State, hp: &Hyperparameters) -> Result<Vec<f64>> {
    ...
    let trend_factor = hp.trend_factor.clamp(0.0, 4.0);
    ...
    for (i, battery) in challenge.batteries.iter().enumerate() {
        ...
        // DA trend signal: end - start of the future window.
        ...
        // Trend-aware effective thresholds:
        //   rising DA (trend > 0) → wait for higher (raise discharge_th, lower charge_th)
        //   falling DA (trend < 0) → act now (lower discharge_th, raise charge_th)
