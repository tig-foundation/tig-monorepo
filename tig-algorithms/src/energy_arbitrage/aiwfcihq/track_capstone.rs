...
mod helpers {
    ...
    pub struct AycdicdbCache {
        ...
        pub soc_grids: Vec<Vec<f64>>,
        pub soc_meta: Vec<SocGridMeta>,
    }

    #[derive(Clone, Debug)]
    pub struct SocGridMeta {
        s_min: f64,
        r_lo: f64,
        r_hi: f64,
        s_max: f64,
    ...
    fn build_reachable_soc_grid(
        bat: &Battery,
        current_soc: f64,
        num_steps: usize,
        soc_levels: usize,
    ) -> (Vec<f64>, SocGridMeta) {
        let s_min = bat.soc_min_mwh;
        let s_max = bat.soc_max_mwh;
        let total = soc_levels.saturating_sub(1);

        if total == 0 {
            let meta = SocGridMeta { s_min, r_lo: s_min, r_hi: s_max, s_max, m0: 0, m1: 0, m2: 0 };
            return (vec![s_min], meta);
        }

        let span = (s_max - s_min).max(0.0);
        if span <= 1e-9 {
            let meta = SocGridMeta { s_min, r_lo: s_min, r_hi: s_max, s_max, m0: 0, m1: total, m2: 0 };
            return (vec![s_min; soc_levels], meta);
        }

        let current_soc = current_soc.clamp(s_min, s_max);
        let dt = 0.25_f64;
        let focus_h = num_steps.clamp(1, 16);
        let charge_step = bat.power_charge_mw * bat.efficiency_charge * dt;
        let discharge_step = bat.power_discharge_mw * dt / bat.efficiency_discharge.max(1e-9);
        let pad = (0.02 * span + 0.75 * charge_step.max(discharge_step)).min(0.12 * span);

        let mut r_lo = (current_soc - focus_h as f64 * discharge_step - pad).clamp(s_min, s_max);
        let mut r_hi = (current_soc + focus_h as f64 * charge_step + pad).clamp(s_min, s_max);
        if r_hi < r_lo {
            let tmp = r_lo;
            r_lo = r_hi;
            r_hi = tmp;
        ...
        let band_frac = ((r_hi - r_lo) / span).clamp(0.0, 1.0);
        let left_w = (r_lo - s_min).max(0.0);
        let right_w = (s_max - r_hi).max(0.0);
        let outer_nonzero = (left_w > 1e-9) as usize + (right_w > 1e-9) as usize;

        let (m0, m1, m2, r_lo, r_hi) = if band_frac >= 0.94 || total <= outer_nonzero {
            (0usize, total, 0usize, s_min, s_max)
        } else {
            let max_mid = total.saturating_sub(outer_nonzero);
            let mid_share = (0.72 - 0.28 * band_frac).clamp(0.45, 0.78);
            let mut m1 = (((total as f64) * mid_share).round() as usize).clamp(1, max_mid);
            let remaining = total.saturating_sub(m1);
            let outer_sum = left_w + right_w;

            let mut m0 = if outer_sum > 1e-12 {
                (((remaining as f64) * left_w / outer_sum).round() as usize).min(remaining)
            } else {
                0
            };
            let mut m2 = remaining.saturating_sub(m0);

            if left_w > 1e-9 && m0 == 0 && remaining > 0 {
                m0 = 1;
                m2 = m2.saturating_sub(1);
            }
            if right_w > 1e-9 && m2 == 0 && remaining > 0 {
                if m0 > 0 {
                    m0 -= 1;
                    m2 = 1;
                } else if m1 > 1 {
        ...
        let mut grid = Vec::with_capacity(soc_levels);
        grid.push(s_min);

        if m0 > 0 {
            for i in 1..=m0 {
                grid.push(s_min + (r_lo - s_min) * (i as f64) / (m0 as f64));
            }
        }
        if m1 > 0 {
            for i in 1..=m1 {
                grid.push(r_lo + (r_hi - r_lo) * (i as f64) / (m1 as f64));
            }
        }
        if m2 > 0 {
            for i in 1..=m2 {
                grid.push(r_hi + (s_max - r_hi) * (i as f64) / (m2 as f64));
            }
        }

        if let Some(first) = grid.first_mut() { *first = s_min; }
        if let Some(last) = grid.last_mut() { *last = s_max; }
        for i in 1..grid.len() {
            if grid[i] < grid[i - 1] {
                grid[i] = grid[i - 1];
            }
        ...
        let meta = SocGridMeta { s_min, r_lo, r_hi, s_max, m0, m1, m2 };
        (grid, meta)
    }

    #[inline]
    fn soc_interp_indices(grid: &[f64], meta: &SocGridMeta, soc: f64) -> (usize, usize, f64) {
        let n = grid.len();
        if n <= 1 {
            return (0, 0, 0.0);
        }
        ...
        let (idx0, idx1) = if meta.m0 > 0 && soc < meta.r_lo {
            let step = (meta.r_lo - meta.s_min) / (meta.m0 as f64);
            if step > 1e-12 {
                let x = ((soc - meta.s_min) / step).clamp(0.0, meta.m0 as f64 - 1e-12);
                let j = x.floor() as usize;
                let i0 = j.min(meta.m0 - 1);
                (i0, i0 + 1)
            } else {
                (0, 1)
            }
        } else if meta.m2 > 0 && soc > meta.r_hi {
            let start = meta.m0 + meta.m1;
            let step = (meta.s_max - meta.r_hi) / (meta.m2 as f64);
            if step > 1e-12 {
                let x = ((soc - meta.r_hi) / step).clamp(0.0, meta.m2 as f64 - 1e-12);
                let j = x.floor() as usize;
                let i0 = start + j.min(meta.m2 - 1);
                (i0, i0 + 1)
            } else {
                (n - 2, n - 1)
            }
            ...
            let start = meta.m0;
            if meta.m1 > 0 {
                let step = (meta.r_hi - meta.r_lo) / (meta.m1 as f64);
                if step > 1e-12 {
                    let x = ((soc - meta.r_lo) / step).clamp(0.0, meta.m1 as f64 - 1e-12);
                    let j = x.floor() as usize;
                    let i0 = start + j.min(meta.m1 - 1);
                    (i0, i0 + 1)
                } else {
                    let i0 = start.saturating_sub(1).min(n - 2);
                    (i0, i0 + 1)
                }
            } else {
                let i0 = start.saturating_sub(1).min(n - 2);
        ...
        let g0 = grid[idx0];
        let g1 = grid[idx1];
        let frac = if g1 > g0 + 1e-12 {
            ((soc - g0) / (g1 - g0)).clamp(0.0, 1.0)
        } else {
            0.0
        };
        (idx0, idx1, frac)
    ...
    #[inline]
    fn interp_on_soc_grid(values: &[f64], grid: &[f64], meta: &SocGridMeta, soc: f64) -> f64 {
        if values.is_empty() {
            return 0.0;
        }
        let (i0, i1, frac) = soc_interp_indices(grid, meta, soc);
        values[i0] * (1.0 - frac) + values[i1] * frac
    }

    #[inline]
    fn dp_value_at_soc(ca: &AycdicdbCache, b: usize, t: usize, soc: f64) -> f64 {
        interp_on_soc_grid(&ca.dp[b][t], &ca.soc_grids[b], &ca.soc_meta[b], soc)
    }

    #[inline]
    fn dp_slope_at_soc(ca: &AycdicdbCache, b: usize, t: usize, soc: f64) -> f64 {
        if ca.dp[b][t].len() <= 1 {
            return 0.0;
        }
        let (i0, i1, _) = soc_interp_indices(&ca.soc_grids[b], &ca.soc_meta[b], soc);
        let ds = (ca.soc_grids[b][i1] - ca.soc_grids[b][i0]).abs().max(1e-9);
        (ca.dp[b][t][i1] - ca.dp[b][t][i0]) / ds
    }
    ...
    fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
        STATE.with(|s| -> Result<Vec<f64>> {
            ...
            let base_actions = if hp.use_kkt {
                ...
                let mut asca_flows = flows_base.to_vec();
                let dual_util_gate = hp.kkt_cong_threshold.max(0.82);
                let mut dual_stress_count = 0usize;
                for l in 0..asca_flows.len() {
                    for &(b, imp) in &cache.ptdf_sparse[l] {
                        asca_flows[l] += imp * a[b];
                    }
                    let limit = challenge.network.flow_limits[l];
                    if limit > 1e-6 && asca_flows[l].abs() / limit >= dual_util_gate {
                        dual_stress_count += 1;
                    }
                }

                if hp.dual_iters > 0 && dual_stress_count > 0 {
            ...
            if is_last_step {
                ...
                let mut near_line_count = 0usize;
                let mut severe_line_count = 0usize;
                let mut touched = vec![false; challenge.num_batteries];

                for l in 0..num_l {
                    let mut f = flows_base[l];
                    for &(bb, imp) in &cache.ptdf_sparse[l] { f += imp * baseline_actions[bb]; }

                    let limit = challenge.network.flow_limits[l];
                    if limit <= 1e-6 { continue; }
                    let util = f.abs() / limit;
                    if util > max_util { max_util = util; }

                    if util >= 0.97 {
                        severe_line_count += 1;
                        for &(bb, _) in &cache.ptdf_sparse[l] { touched[bb] = true; }
                    } else if util >= 0.90 {
                        near_line_count += 1;
                        for &(bb, _) in &cache.ptdf_sparse[l] { touched[bb] = true; }
                    }
                ...
                let touched_count = touched.iter().filter(|&&x| x).count();
                let mut regret_sum = 0.0_f64;
                let mut regret_count = 0usize;
                for b in 0..challenge.num_batteries {
                    let u = baseline_actions[b];
                    if !touched[b] && u.abs() <= 1e-6 { continue; }

                    let cur_v = eval_profit(challenge, state, cache, b, u);
                    let zero_v = if u.abs() <= 1e-12 {
                        cur_v
                    } else {
                        eval_profit(challenge, state, cache, b, 0.0)
                    };
                    let regret = (zero_v - cur_v).max(0.0);
                    if regret > 1e-8 {
                        regret_sum += regret;
                        regret_count += 1;
                    }
                ...
                let coupled = near_line_count >= 2
                    || touched_count >= (challenge.num_batteries / 12).max(4).min(challenge.num_batteries.max(1));
                let regret_rich = regret_count >= 2 || regret_sum > 0.25;
                let should_refine = severe_line_count > 0
                    || max_util >= 0.95
                    || (max_util >= 0.88 && coupled)
                    || regret_rich;
                ...
                if !should_refine {
                    ...
                    let candidate_hint = (8usize
                        + severe_line_count * 6
                        + near_line_count * 2
                        + touched_count / 4
                        + regret_count * 2)
                        .clamp(12, 28)
                        .min(challenge.num_batteries.max(1));
                    let fuel_budget = 150_000_000_u64 * candidate_hint as u64;
    ...
    fn build_dp_with_mu(
        ...
        let mut dp: Vec<Vec<Vec<f64>>> = Vec::with_capacity(num_b);
        for b in 0..num_b {
            dp.push(vec![vec![0.0_f64; soc_grids[b].len()]; num_t + 1]);
        }

        for b in 0..num_b {
            let bat = &challenge.batteries[b];
            let node = batt_nodes[b];
            let grid = &soc_grids[b];
            let meta = &soc_meta[b];
            ...
            for t in (0..num_t).rev() {
                ...
                for i in 0..grid.len() {
                    ...
                    for j in 0..=action_grid {
                        let u = if span_u > 0.0 {
                            u_min + span_u * (j as f64) / (action_grid as f64)
                        } else { u_min };
                        ...
                        let v_next = interp_on_soc_grid(&dp[b][t + 1], grid, meta, next_soc);
    ...
    fn ldd_simulate_flows(
        ...
        for t in 0..num_t {
            ...
            for b in 0..num_b {
                ...
                for j in 0..=sim_pts {
                    let u = if span_u > 0.0 { u_min + span_u * (j as f64) / (sim_pts as f64) } else { u_min };
                    let abs_u = u.abs();
                    ...
                    let v_next = interp_on_soc_grid(&dp[b][t_next], grid, meta, next_soc);
    ...
    fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> AycdicdbCache {
        ...
        let slack_bus = challenge.network.slack_bus;
        let mut ptdf_node: Vec<Vec<f64>> = vec![Vec::new(); num_n];
        for node in 0..num_n {
            if node == slack_bus { continue; }
            let mut inj_node = inj_base.clone();
            inj_node[node] += 1.0;
            let flows_node = challenge.network.compute_flows(&inj_node);
            let mut impacts = Vec::with_capacity(num_l);
            for l in 0..num_l {
                impacts.push(flows_node[l] - flows0[l]);
            }
            ptdf_node[node] = impacts;
        }
        ...
        for b in 0..num_b {
            ...
            let mut bnode = slack_bus;
            for k in 0..num_n {
                if (inj1[k] - inj_base[k]).abs() > 0.5 && k != slack_bus {
                    bnode = k;
                    break;
                }
            }
            batt_nodes[b] = bnode;
            if let Some(impacts) = ptdf_node.get(bnode) {
                if !impacts.is_empty() {
                    for l in 0..num_l {
                        let impact = impacts[l];
                        if impact.abs() > 1e-8 {
        ...
        let mut soc_grids = Vec::with_capacity(num_b);
        let mut soc_meta = Vec::with_capacity(num_b);
        let steps_remaining = num_t.saturating_sub(state.time_step);
        let effective_soc_levels = (hp.soc_levels as f64 * steps_remaining as f64 / num_t as f64).ceil().max(31.0).min(hp.soc_levels as f64) as usize;
        for b in 0..num_b {
            let (grid, meta) = build_reachable_soc_grid(&challenge.batteries[b], state.socs[b], steps_remaining, effective_soc_levels);
            soc_grids.push(grid);
            soc_meta.push(meta);
        }
        ...
        if hp.anticipate_lmp && num_l > 0 {
            ...
            let mut all_exo_flows: Vec<Vec<f64>> = Vec::with_capacity(num_t);
            for t in 0..num_t {
                all_exo_flows.push(challenge.network.compute_flows(&challenge.exogenous_injections[t]));
            }

            let mut line_freq: Vec<f64> = vec![0.0; num_l];
            let mut line_avg_exceed: Vec<f64> = vec![0.0; num_l];
            for l in 0..num_l {
                let limit = challenge.network.flow_limits[l];
                if limit <= 1e-6 { continue; }
                let mut exceed_count = 0usize;
                let mut exceed_sum = 0.0_f64;
                for t in 0..num_t {
                    let util = all_exo_flows[t][l].abs() / limit;
                    if util > hp.lmp_threshold {
                        exceed_count += 1;
                        exceed_sum += util - hp.lmp_threshold;
                    }
                }
                line_freq[l] = exceed_count as f64 / (num_t as f64).max(1.0);
                line_avg_exceed[l] = if exceed_count > 0 { exceed_sum / exceed_count as f64 } else { 0.0 };
            }

            for t in 0..num_t {
                let f_exo = &all_exo_flows[t];
                ...
                if max_ratio <= hp.lmp_threshold { continue; }

                let selective_floor = if max_ratio > 1.0 {
                    (0.90 * max_ratio).max(hp.lmp_threshold)
                } else {
                    (0.96 * max_ratio).max(hp.lmp_threshold)
                };
                let row_cap = if max_ratio > 1.0 {
                    base_premium * 1.80
                } else {
                    base_premium * 1.35
                };
                ...
                for l in 0..num_l {
                    ...
                    if limit <= 1e-6 { continue; }
                    let ratio = f_exo[l].abs() / limit;
                    if ratio < selective_floor { continue; }

                    let proba = ((ratio - hp.lmp_threshold) / (1.0 - hp.lmp_threshold).max(1e-6))
                        .clamp(0.0, 1.0);
                    let freq_scale = 1.0 + 0.5 * line_freq[l] + 0.3 * line_avg_exceed[l].min(1.0);
                    let premium = base_premium * proba * freq_scale;
                    let sign_f = f_exo[l].signum();
                    for &(b, impact) in &ptdf_sparse[l] {
                        if impact.abs() > 1e-6 {
                            let nodal_shift = -impact * sign_f * premium;
                            expected_premiums[t][b] =
                                (expected_premiums[t][b] + nodal_shift).clamp(-row_cap, row_cap);
                        }
                    }
                }
            }
        ...
        let dp = if hp.ldd_iters > 0 && num_l > 0 {
            ...
            for ldd_iter in 0..hp.ldd_iters {
                let flows_all = ldd_simulate_flows(
                    challenge,
                    state,
                    &dp,
                    &batt_nodes,
        ...
        AycdicdbCache { dp, ptdf_sparse, b_to_lines, batt_nodes, soc_grids, soc_meta }
    }
    ...
    fn eval_profit(challenge: &Challenge, state: &State, ca: &AycdicdbCache, b: usize, u: f64) -> f64 {
        ...
        profit + dp_value_at_soc(ca, b, t_next, next_soc)
    }
    ...
    fn run_asca(
        ...
        let optimize_battery = |b: usize,
            ...
            let base_v = best_v;
            let mut neg_range: Option<(f64, f64)> = None;
            let mut pos_range: Option<(f64, f64)> = None;
            let mut neg_signal = f64::NEG_INFINITY;
            let mut pos_signal = f64::NEG_INFINITY;
            let mut neg_curve = f64::NEG_INFINITY;
            let mut pos_curve = f64::NEG_INFINITY;
            let mut neg_merit = f64::NEG_INFINITY;
            let mut pos_merit = f64::NEG_INFINITY;

            if u_min < 0.0 {
                let lo = u_min;
                let hi = 0.0_f64.min(u_max);
                if lo < hi {
                    neg_range = Some((lo, hi));
                    ...
                    let side_best_v = edge_v.max(probe_v);
                    let side_best_u = if probe_v >= edge_v { probe_u } else { edge_u };
                    if side_best_v > best_v {
                        best_v = side_best_v;
                        best_u = side_best_u;
                    }

                    neg_signal = side_best_v - base_v;
                    neg_curve = probe_v - edge_v.max(base_v);
                    neg_merit = neg_signal + 1.5 * neg_curve.max(0.0);
                }
            }
            ...
            if u_max > 0.0 {
                ...
                if lo < hi {
                    ...
                    pos_signal = side_best_v - base_v;
                    pos_curve = probe_v - edge_v.max(base_v);
                    pos_merit = pos_signal + 1.5 * pos_curve.max(0.0);
                }
            }

            let search_eps = 1e-6 * (1.0 + base_v.abs() + score_hint.abs());
            let score_gate = (0.10 * score_hint).max(8.0 * search_eps);
            let curve_gate = (0.04 * score_hint).max(4.0 * search_eps);

            let search_side = match (neg_range, pos_range) {
                (Some(nr), None) => Some((nr.0, nr.1, neg_signal, neg_curve, neg_merit)),
                (None, Some(pr)) => Some((pr.0, pr.1, pos_signal, pos_curve, pos_merit)),
                (Some(nr), Some(pr)) => {
                    let choose_neg = if neg_merit > pos_merit + search_eps {
                        true
                    } else if pos_merit > neg_merit + search_eps {
                        false
                    } else if neg_curve > pos_curve + search_eps {
                        true
                    } else if pos_curve > neg_curve + search_eps {
                        false
                    } else if best_u < -1e-9 {
                        true
                    } else if best_u > 1e-9 {
                    ...
                    Some(if choose_neg {
                        (nr.0, nr.1, neg_signal, neg_curve, neg_merit)
                    } else {
                        (pr.0, pr.1, pos_signal, pos_curve, pos_merit)
                    })
                }
                (None, None) => None,
            };

            if let Some((lo, hi, side_signal, side_curve, side_merit)) = search_side {
                let frontier_ok = allow_ternary && side_signal > -2.0 * search_eps;
                let strong_side = side_signal > score_gate
                    || side_curve > curve_gate
                    || side_merit > 1.25 * score_gate;

                if frontier_ok || strong_side {
            ...
            let secondary_gate = (0.40 * score_hint).max(10.0 * search_eps);
            if let Some((nlo, nhi)) = neg_range {
                if neg_merit > secondary_gate {
                    let chosen_neg = match search_side {
                        Some((lo, _, _, _, _)) => lo < 0.0,
                        None => false,
                    };
                    if !chosen_neg {
                        let (u, v) = ternary_search(|u| eval_profit(challenge, state, ca, b, u), nlo, nhi, hp.ternary_iters);
                        if v > best_v { best_v = v; best_u = u; }
                    }
                }
            }
            if let Some((plo, phi_range)) = pos_range {
                if pos_merit > secondary_gate {
                    let chosen_pos = match search_side {
                        Some((lo, _, _, _, _)) => lo > 0.0,
                        None => false,
                    };
                    if !chosen_pos {
                        let (u, v) = ternary_search(|u| eval_profit(challenge, state, ca, b, u), plo, phi_range, hp.ternary_iters);
        ...
        let ternary_frontier = (active_list.len() / 4).clamp(4, 20);
        ...
        for _sweep in 0..hp.asca_iters {
            ...
            while processed < pop_budget {
                ...
                let delta = optimize_battery(
                    b,
                    &mut flows,
                    actions,
                    &cur_vals,
                    &zero_vals,
                    processed <= ternary_frontier,
                    score_now,
                );
    ...
    fn ternary_search<F: Fn(f64) -> f64>(f: F, mut l: f64, mut r: f64, iters: usize) -> (f64, f64) {
        ...
        let mut x1 = r - (r - l) / phi;
        let mut x2 = l + (r - l) / phi;
        let mut f1 = f(x1);
        let mut f2 = f(x2);
        let (mut best_x, mut best_f) = if f1 >= f2 { (x1, f1) } else { (x2, f2) };
        let steps = (2 * iters).saturating_sub(2);
        for _ in 0..steps {
            if f1 < f2 {
                l = x1;
                x1 = x2;
                ...
                x2 = l + (r - l) / phi;
                f2 = f(x2);
                if f2 > best_f { best_x = x2; best_f = f2; }
            } else {
                r = x2;
                x2 = x1;
                f2 = f1;
                x1 = r - (r - l) / phi;
                f1 = f(x1);
                if f1 > best_f { best_x = x1; best_f = f1; }
            }
        }
        (best_x, best_f)
    }
    ...
    fn run_dual_ascent(
        ...
        let price_scale = 0.45 * avg_price.max(10.0) * hp.kkt_price_scale;
        let base_alpha = if hp.da_step_size > 0.0 { hp.da_step_size } else { 0.01 };
        ...
        for l in 0..num_l {
            ...
            if util > hp.kkt_cong_threshold {
                ...
                nu[l] = excess_frac * excess_frac * price_scale * flows[l].signum();
            }
        }

        let mut active_lines = vec![false; num_l];
        let mut active_b = vec![false; num_b];
        let tol = hp.convergence_tol.max(1e-6);
        ...
        for k in 1..=hp.dual_iters {
            ...
            let near_util = if max_util > 1.0 {
                0.86
            } else {
                (max_util - 0.05).max(hp.kkt_cong_threshold.max(0.82))
            };

            for v in active_lines.iter_mut() { *v = false; }
            for l in 0..num_l {
                let limit = (challenge.network.flow_limits[l] - hp.flow_margin).max(0.0);
                if limit <= 1e-6 { continue; }
                let util = flows[l].abs() / limit;
                ...
                if overflow > tol
                    || util >= near_util
                    || (nu[l].abs() > 1e-7 && util >= near_util - 0.08)
                {
                    active_lines[l] = true;
                }
            }
            if !active_lines.iter().any(|&x| x) {
                if let Some(l_star) = worst_l {
                    if max_util > hp.kkt_cong_threshold.max(0.75) {
                        active_lines[l_star] = true;
                    } else {
                        break;
                    }
                } else {
            ...
            for v in active_b.iter_mut() { *v = false; }
            for l in 0..num_l {
            ...
            let active_list: Vec<usize> = (0..num_b).filter(|&b| active_b[b]).collect();
            if active_list.is_empty() { break; }

            let alpha = 0.75 * base_alpha / (k as f64);
            ...
            for &b in &active_list {
                ...
                let current = actions[b];
                let mut best_u = current;
                let mut best_v = eval_profit(challenge, state, ca, b, current) - nu_dot_ptdf * current;
                ...
                let dt = 0.25_f64;
                let t_next = (state.time_step + 1).min(ca.dp[b].len() - 1);
                let slope = dp_slope_at_soc(ca, b, t_next, soc);

                let cap = bat.capacity_mwh;
                let a_coeff = (dt / cap).powi(2);
                let eta_c = bat.efficiency_charge;
                let eta_d = bat.efficiency_discharge.max(1e-9);

                let b_pos = (rt_price - 0.25) * dt - slope * dt / eta_d - nu_dot_ptdf;
                let u_ana_pos = if a_coeff > 1e-12 {
                    (b_pos / (2.0 * a_coeff)).clamp(u_min, u_max)
                } else {
                    if b_pos > 0.0 { u_max } else { u_min }
                };

                let b_neg = (rt_price + 0.25) * dt - slope * eta_c * dt - nu_dot_ptdf;
                let u_ana_neg = if a_coeff > 1e-12 {
                    (b_neg / (2.0 * a_coeff)).clamp(u_min, u_max)
                } else {
                    if b_neg > 0.0 { u_max } else { u_min }
                };

                for &cand in &[u_ana_pos, u_ana_neg, 0.0_f64] {
                    let cand = cand.clamp(u_min, u_max);
                    if (cand - best_u).abs() <= 1e-10 { continue; }
                    let v = eval_profit(challenge, state, ca, b, cand) - nu_dot_ptdf * cand;
                    if v > best_v {
                        best_v = v;
                        best_u = cand;
                    }
                ...
                if delta.abs() > 1e-6 {
                    let refine_lo = (best_u - 0.5 * delta.abs()).max(u_min);
                    let refine_hi = (best_u + 0.5 * delta.abs()).min(u_max);
                    if refine_hi - refine_lo > 1e-8 {
                        let (ref_u, ref_v) = ternary_search(
                            |u| eval_profit(challenge, state, ca, b, u) - nu_dot_ptdf * u,
                            refine_lo, refine_hi, 2,
                        );
                        if ref_v > best_v { best_u = ref_u; }
                    }
                }
                ...
                if delta.abs() > 1e-10 {
                    actions[b] = best_u;
                    if delta.abs() > max_du { max_du = delta.abs(); }
                    for &(l, p) in &ca.b_to_lines[b] {
                        if l < num_l { flows[l] += p * delta; }
                    }
                }
            ...
            for l in 0..num_l {
                ...
                let overflow = (flows[l].abs() - limit).max(0.0);
                if overflow > max_viol { max_viol = overflow; }
            ...
            if max_viol < tol && max_du < tol { break; }
        }
    ...
    fn run_deflator(
        ...
        for _ in 0..hp.deflator_iters {
            ...
            for &(l, p) in &ca.b_to_lines[b] {
                if l >= num_l { continue; }
                let old_flow = flows[l];
                let old_overflow = (old_flow.abs() - limits[l]).max(0.0);
                flows[l] += p * delta;
                let new_overflow = (flows[l].abs() - limits[l]).max(0.0);
                total_over += new_overflow - old_overflow;
            }
        }
    ...
    fn joint_lp_dispatch(
        ...
        for b in 0..num_b {
            ...
            let dv = dp_slope_at_soc(ca, b, t_next, soc);
    ...
    fn multi_lp_dispatch(
        ...
        let t_term = t0 + h;
        for b in 0..nb {
            let grid = &ca.soc_grids[b];
            let soc_levels = grid.len();
            ...
            for i in 0..(terminal_knots - 1) {
                ...
                let soc_lo = grid[idx_lo];
                let soc_hi = grid[idx_hi];
...
mod lp {
    ...
    pub fn lp_solve_with_budget(
        ...
        let extract_primal = |basis: &[usize], tab: &[Vec<f64>]| -> Vec<f64> {
        ...
        let mut best_x = extract_primal(&basis, &tab);
        let mut best_obj = 0.0_f64;
        let mut pivots_used = 0usize;

        for _ in 0..max_pivots {
            const SHORTLIST: usize = 12;
            let mut shortlist = [(f64::NEG_INFINITY, usize::MAX); SHORTLIST];
            ...
            for j in 0..n_vars {
                ...
                if rc < -LP_EPS {
                    any_negative = true;
                    let merit = -rc;
                    let mut pos = shortlist_len.min(SHORTLIST - 1);
                    if shortlist_len < SHORTLIST {
                        shortlist_len += 1;
                    } else if merit <= shortlist[pos].0 {
                        continue;
                    }
                    while pos > 0 && merit > shortlist[pos - 1].0 {
                        shortlist[pos] = shortlist[pos - 1];
                        pos -= 1;
                    }
                    shortlist[pos] = (merit, j);
                }
            }
            ...
            let consider_column = |j: usize,
                                   tab: &[Vec<f64>],
                                   entering: &mut Option<usize>,
                                   leaving_row: &mut usize,
                                   best_score: &mut f64,
                                   best_ratio: &mut f64,
                                   best_pivot: &mut f64| {
                let rc = tab[m][j];
                ...
                let mut cand_row: Option<usize> = None;
                let mut cand_ratio = f64::INFINITY;
                let mut cand_pivot = 0.0_f64;

                for i in 0..m {
                    let aij = tab[i][j];
                    if aij > LP_EPS {
                        let ratio = tab[i][rhs_col] / aij;
                        let better = ratio + 1e-12 < cand_ratio
                            || ((ratio - cand_ratio).abs() <= 1e-12 && aij > cand_pivot);
                        if better {
                            cand_ratio = ratio;
                            cand_row = Some(i);
                            cand_pivot = aij;
                ...
                let Some(row) = cand_row else { return; };

                let immediate_gain = (-rc) * cand_ratio.max(0.0);
                let stability = cand_pivot / (1.0 + cand_pivot);
                let score = immediate_gain * (0.75 + 0.25 * stability);

                let better = score > *best_score + 1e-12
                    || ((score - *best_score).abs() <= 1e-12 && cand_pivot > *best_pivot + 1e-12)
                    || ((score - *best_score).abs() <= 1e-12
                        && (cand_pivot - *best_pivot).abs() <= 1e-12
                        && cand_ratio < *best_ratio);
                ...
                if better {
                    ...
                    *best_pivot = cand_pivot;
                }
            };

            let mut entering: Option<usize> = None;
            let mut leaving_row = 0usize;
            let mut best_score = f64::NEG_INFINITY;
            let mut best_ratio = f64::INFINITY;
            let mut best_pivot = 0.0_f64;

            for k in 0..shortlist_len {
                let j = shortlist[k].1;
                if j != usize::MAX {
                    consider_column(
                        j,
            ...
            let Some(entering) = entering else {
                return (Some(best_x), pivots_used);
            };
            ...
            let obj = tab[m][rhs_col];
            if obj.is_finite() && obj > best_obj + 1e-9 {
                best_obj = obj;
                best_x = extract_primal(&basis, &tab);
            }
        }

        (Some(best_x), pivots_used)
    }
...
pub fn solve_challenge(
    ...
    track_capstone::solve(challenge, save_solution, hyperparameters)
}