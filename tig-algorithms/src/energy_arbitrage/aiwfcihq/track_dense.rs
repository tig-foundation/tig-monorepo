...
mod helpers {
    ...
    pub struct TrackHp {
        ...
        pub screened_multistart_refine_cap: usize,
    }
    ...
    impl TrackHp {
        pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
            ...
            if let Some(v) = m.get("screened_multistart_refine_cap").and_then(|v| v.as_u64()) { self.screened_multistart_refine_cap = v as usize; }
        }
    }
    ...
    fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
        STATE.with(|s| -> Result<Vec<f64>> {
            ...
            let base_actions = if hp.screened_multistart_refine_cap > 0
                && !hp.use_kkt
                && hp.dual_iters == 0
                && hp.max_admm_iters == 0
            {
                run_screened_multistart_policy(
                    challenge,
                    state,
                    cache,
                    hp,
                    &flows_base,
                    hp.screened_multistart_refine_cap,
                )
            } else if hp.use_kkt {
    ...
    fn compute_action_flows(ca: &AycdicdbCache, flows_base: &[f64], actions: &[f64]) -> Vec<f64> {
    ...
    fn best_action_in_window(
        ...
        if lo_bias > hi_bias + 0.05 && u_min < cur - 1e-7 {
            push_unique_probe(&mut probes, (2.0 * cur + u_min) / 3.0);
        } else if hi_bias > lo_bias + 0.05 && cur + 1e-7 < u_max {
            push_unique_probe(&mut probes, (2.0 * cur + u_max) / 3.0);
        } else if max_util > 0.90 {
            push_unique_probe(&mut probes, 0.5 * (u_min + u_max));
        }
    ...
    fn build_line_relief_seed(
        ...
        let zero_actions = vec![0.0_f64; num_b];
        let mut ranked: Vec<(usize, f64)> = (0..num_b)
            .filter_map(|b| {
                let v0 = eval_profit(challenge, state, ca, b, 0.0);
    ...
    fn run_screened_multistart_policy(
        ...
        let mut best_score = refine_basic_seed(challenge, state, ca, hp, flows_base, &mut best_actions);
        ...
        for (idx, (mut seed, _)) in seed_pool.into_iter().enumerate() {
            if idx >= refine_cap {
                break;
            }
            let score = refine_basic_seed(challenge, state, ca, hp, flows_base, &mut seed);
    ...
    fn refine_solution(
        ...
        if !da.is_empty() && t_now < da.len() {
            ...
            let price_spread = (max_p - min_p).abs();
...
mod track_dense {
    ...
    fn defaults() -> TrackHp {
        TrackHp {
            ...
            lp_horizon: 0,
            screened_multistart_refine_cap: 2,
        }
    }
...
pub struct Hyperparameters {
    ...
    pub screened_multistart_refine_cap: Option<usize>,
}
...
pub fn solve_challenge(
    ...
    track_dense::solve(challenge, save_solution, hyperparameters)
}
