...
mod helpers {
    ...
    fn run_asca(
        ...
        let seed_price_scale = avg_price.max(10.0) * 0.25;
        let mut line_mu = vec![0.0_f64; num_l];
        for l in 0..num_l {
            let limit = challenge.network.flow_limits[l];
            if limit <= 1e-6 { continue; }
            let util = flows_base[l].abs() / limit;
        ...
        let potentials: Vec<f64> = (0..num_b)
            .map(|b| if active[b] { potential(challenge, state, ca, b) } else { 0.0 })
            .collect();
        let mut order: Vec<usize> = Vec::with_capacity(num_b);
        let mut scores = vec![f64::NEG_INFINITY; num_b];
        let rebuild_order = |line_mu: &[f64], order: &mut Vec<usize>, scores: &mut [f64]| {
            order.clear();
            for b in 0..num_b {
                if !active[b] { continue; }
                let shadow: f64 = ca.b_to_lines[b].iter().map(|&(l, imp)| line_mu[l] * imp).sum();
                scores[b] = potentials[b] - shadow;
                order.push(b);
            }
            order.sort_by(|&a, &b| {
        ...
        rebuild_order(&line_mu, &mut order, &mut scores);
        ...
        for _sweep in 0..hp.asca_iters {
            ...
            for l in 0..num_l {
                ...
                if util > hp.lmp_threshold {
                    let excess = ((util - hp.lmp_threshold) / (1.0 - hp.lmp_threshold).max(1e-6))
                        .clamp(0.0, 1.0);
                    line_mu[l] = excess * seed_price_scale * flows[l].signum();
                }
            }
            rebuild_order(&line_mu, &mut order, &mut scores);
        }
    ...
    fn ternary_search<F: Fn(f64) -> f64>(f: F, mut l: f64, mut r: f64, iters: usize) -> (f64, f64) {
        if l >= r { return (l, f(l)); }
        let phi: f64 = 0.5_f64 * (5.0_f64.sqrt() - 1.0);
        let inv_phi = 1.0 - phi;
        let mut m1 = l + inv_phi * (r - l);
        let mut m2 = l + phi * (r - l);
        let mut f1 = f(m1);
        let mut f2 = f(m2);
        let golden_iters = iters.saturating_mul(2).saturating_sub(2);
    ...
    fn run_deflator(
        ...
        let lp_lambda = 1e8;
        let per_call_pivots = (hp.deflator_iters.saturating_mul(4)).max(100);
        if let Some(new_actions) = joint_lp_dispatch(challenge, state, ca, flows_base, lp_lambda, per_call_pivots) {
            for b in 0..num_b {
                let (lo, hi) = state.action_bounds[b];
                actions[b] = new_actions[b].clamp(lo, hi);
            }
            return;
        }
...
pub fn solve_challenge(
    ...
    track_multiday::solve(challenge, save_solution, hyperparameters)
}