...
mod helpers {
    ...
    pub struct TrackHp {
        ...
        // Multi-period LP (MPC) dispatch
        pub lp_horizon: usize,  // MPC horizon in steps (0 = disabled, 4-12 recommended)
    }
    ...
    pub fn kkt_policy(
        ...
        // ASCA baseline for comparison (unchanged)
    ...
    fn joint_lp_dispatch(
        ...
        // Variables: [d_0..d_nb (discharge), c_0..c_nb (charge mag), viol_0..viol_nl (flow slack)]
        let n = 2 * num_b + num_l;
        // Constraints: [ub_d, ub_c, soc_min, soc_max per battery] + [flow_pos, flow_neg per line]
        ...
        // Soft line-flow constraints: total_flow = exo + Σ PTDF*(d-c); |total_flow| <= limit + viol_l
        let row_f = 4 * num_b;
        for l in 0..num_l {
            let limit = challenge.network.flow_limits[l];
            let exo = flows_base[l];
            let viol_idx = 2 * num_b + l;
            let rp = row_f + 2 * l;     // positive direction
    ...
    /// Multi-period LP dispatch with soft line constraints over `horizon` steps (MPC).
    /// Returns actions for the **first** step of the window.
    fn multi_lp_dispatch(
        ...
        let mut m_count = 0;
        m_count += 2 * nb * H; // action upper bounds
        m_count += 2 * nb * H; // SOC lower & upper
        m_count += 2 * active_lines.len() * H; // flow constraints
        m_count += (K - 1) * nb; // terminal value
        let m = m_count;
        ...
        // Flow constraints with soft slacks
        // Precompute exogenous flows for each time step in the horizon
    ...
    fn refine_solution(
        ...
        // PASS 2 — last-step coordinate descent
        ...
        // PASS 3 — pairwise marginal-guided reallocation
        ...
        // ============ PASS 4 (new – i3) ============
        // Greedy per-battery unconstrained re-optimisation with strict feasibility gate.
        // Uses a tiny fraction of fuel_budget; capped at 200 steps.
        ...
        // ============ PASS 5 (new – DeepSeek curvature-aware LP terminal refine) ============
        // Inspired by DeepSeek's build_dp_terminal_pwl: use piecewise-linear terminal
        // value approximation to re-optimize batteries whose end-of-horizon SOC lies
        // near a high-curvature region.  Without DP table access in refine(), we build
        // a synthetic value function from price data (DA prices) and identify kink
        // regions, then try targeted SOC-repositioning nudges for batteries in those
        // regions.  Strict feasibility gate ensures no regression.
        ...
        // Build synthetic terminal values from DA price curvature:
        // v[soc_idx] approximates remaining-value at end of last step,
        // using price range * capacity as proxy for arbitrage potential.
        let soc_levels_est = K;
        let mut synth_v = vec![0.0_f64; soc_levels_est];

        // Compute price-based terminal value: higher at SOC levels where you can
        // sell high later.  Use price spread as proxy for arbitrage opportunity.
        let da = &challenge.market.day_ahead_prices;
        ...
        if !da.is_empty() && t_now < da.len() {
            ...
            // Build piecewise-linear value function with K breakpoints
            // Curvature concentrated around price transition points
            for si in 0..soc_levels_est {
                let soc_frac = si as f64 / (soc_levels_est - 1) as f64;
                // Value is higher when you hold charge if future prices are high,
                // or are discharged if future prices are low
                synth_v[si] = soc_frac * price_spread;
            }
        }

        // Compute second differences (curvature) of synthetic value function
        ...
        // Select K breakpoints by curvature
        ...
        for b in 0..num_b {
            ...
            // Find which kink interval the battery sits near
            ...
            // Try targeted SOC repositioning: move toward midpoint of high-value kink interval
            if near_kink {
                let node = bat.node;
                let rt = state.rt_prices.get(node).copied().unwrap_or(0.0);
                let (lo, hi) = state.action_bounds[b];

                // Try several SOC-target actions: discharge more, charge more, stay
                let eta_d = bat.efficiency_discharge.max(1e-9);
                let eta_c = bat.efficiency_charge;

                // Candidate: shift toward mid-range SOC
        ...
        // ============ PASS 6 (new – CGSP-inspired congestion-aware re-opt) ============
        // Column-Generated Shadow Prices (CGSP) adapted for post-solve refinement:
        // compute dual prices from actual flow congestion on best_actions,
        // then re-optimize each battery with congestion-adjusted effective prices.
        // Strict feasibility gate: only accept changes that keep all lines within limits.
        ...
        for _it in 0..max_p6_iters {
            // Compute congestion duals from current flows
            ...
            // Re-optimize each battery with congestion-adjusted price
            for b in 0..num_b {
                let bat = &challenge.batteries[b];
                let node = bat.node;
                let rt = state.rt_prices.get(node).copied().unwrap_or(0.0);
                ...
                // Congestion-adjusted effective price
                let cong_adj: f64 = ptdf_b6[b].iter().map(|&(l, imp)| mu6[l] * imp).sum();
                let p_eff = rt - cong_adj;

                // Analytical optimal for each branch (charge/discharge)
...
mod lp {
    ...
    pub fn lp_solve_with_budget(
        ...
        (Some(x), max_pivots) // caller tracks actual pivots conservatively
    }
