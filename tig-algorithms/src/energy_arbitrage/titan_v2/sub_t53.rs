...
mod helpers {
    ...
    fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
        STATE.with(|s| -> Result<Vec<f64>> {
            ...
            // V3 safety-net: run refinement only on the last step
            ...
            if is_last_step {
                ...
                let refined_score: f64 = (0..challenge.num_batteries)
                    ...
                    .sum();
                // Feasibility: all line flows within limits (with 0.1% tolerance)
    ...
    /// PASS 1: Per-battery ternary perturbation with congestion awareness.
    /// PASS 2: Dual-based congestion rebalancing (DeepSeek DW-inspired):
    ///   Compute shadow prices from congested lines, re-optimize each battery
    ///   using congestion-adjusted effective prices, then enforce feasibility
    ///   via the deflator. Mirrors DW dual optimization post-hoc.
    fn refine_solution(
        challenge: &Challenge,
        state: &State,
        ca: &TitanCache,
        ...
        let num_b = challenge.num_batteries;
        let num_l = challenge.network.flow_limits.len();
        let mut fuel: u64 = fuel_budget;

        // ---- PASS 1 (existing kept body, DO NOT MODIFY) ----
        ...
        // Conservative upsizing on non-binding lines
        for &b in &order {
            let current = res[b];
        ...
        // ---- PASS 2 (DeepSeek DW-inspired: dual-based congestion rebalancing) ----
        // DeepSeek insight: independent battery optimization ignores network
        // coupling. Shadow prices from congested lines encode what the DP
        // should have seen. Re-optimize with congestion-adjusted prices.
        let pass2_fuel = fuel / 3;
        if pass2_fuel > 0 {
            // Compute shadow prices μ_l from current post-PASS1 flows
            ...
            // Compute congestion-adjusted effective prices for each battery
            let mut p_eff = vec![0.0_f64; num_b];
            for b in 0..num_b {
                let node = ca.batt_nodes[b];
                let rt = if node < state.rt_prices.len() { state.rt_prices[node] } else { 0.0 };
            ...
            // Check whether dual signal is meaningful (non-trivial congestion)
            let dual_magnitude: f64 = mu.iter().map(|x| x.abs()).sum();
            if dual_magnitude > 1e-6 {
                // Re-optimize each battery with congestion-aware price
                ...
                // Run deflator to restore feasibility
                ...
                // Accept dual actions only if they improve total profit
                let current_profit: f64 = (0..num_b)
                    .map(|b| eval_profit(challenge, state, ca, b, res[b]))
                    .sum();

                if dual_profit > current_profit + 1e-8 {
                    // Feasibility check on dual solution
