...
pub struct TitanCache {
  ...
  pub jump_score: Vec<f64>,
  pub dp_charge_shadow: Vec<Vec<f64>>,
  pub dp_discharge_shadow: Vec<Vec<f64>>,
}
...
fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> TitanCache {
  ...
  let mut jump_score = vec![0.0_f64; num_b];
  for b in 0..num_b {
  ...
  let eff_p = p_da + expected_premiums[t][b];
  if eff_p > max_p { max_p = eff_p; }
  }
  jump_score[b] = max_p;
  }
  ...
  let mut dp_charge_shadow = vec![vec![0.0_f64; soc_levels]; num_b];
  let mut dp_discharge_shadow = vec![vec![0.0_f64; soc_levels]; num_b];
  let epsilon_soc = 0.5_f64;

  for b in 0..num_b {
  let bat = &challenge.batteries[b];
  let soc_span = (bat.soc_max_mwh - bat.soc_min_mwh).max(1e-9);
  let t_next = (state.time_step + 1).min(num_t);
  for i in 0..soc_levels {
  let soc = bat.soc_min_mwh + soc_span * (i as f64) / ((soc_levels - 1) as f64);
  let soc_lo = (soc - epsilon_soc).max(bat.soc_min_mwh);
  let soc_hi = (soc + epsilon_soc).min(bat.soc_max_mwh);

  let idx_lo_f = (soc_lo - bat.soc_min_mwh) / soc_span * ((soc_levels - 1) as f64);
  let idx0_lo = (idx_lo_f.floor() as isize).max(0) as usize;
  let frac_lo = (idx_lo_f - idx0_lo as f64).clamp(0.0, 1.0);
  let idx1_lo = (idx0_lo + 1).min(soc_levels - 1);
  let v_lo = dp[b][t_next][idx0_lo] * (1.0 - frac_lo) + dp[b][t_next][idx1_lo] * frac_lo;

  let idx_hi_f = (soc_hi - bat.soc_min_mwh) / soc_span * ((soc_levels - 1) as f64);
  let idx0_hi = (idx_hi_f.floor() as isize).max(0) as usize;
  let frac_hi = (idx_hi_f - idx0_hi as f64).clamp(0.0, 1.0);
  let idx1_hi = (idx0_hi + 1).min(soc_levels - 1);
  let v_hi = dp[b][t_next][idx0_hi] * (1.0 - frac_hi) + dp[b][t_next][idx1_hi] * frac_hi;

  let span = soc_hi - soc_lo;
  if span > 1e-12 {
  dp_charge_shadow[b][i] = (v_hi - v_lo) / span;
  dp_discharge_shadow[b][i] = (v_lo - v_hi) / span;
  }
  }
  }

  TitanCache { dp, ptdf_sparse, b_to_lines, batt_nodes, jump_score, dp_charge_shadow, dp_discharge_shadow }
}
...
#[inline]
fn dual_soc_shadow(ca: &TitanCache, b: usize, soc: f64, bat: &Battery) -> (f64, f64) {
  let soc_span = (bat.soc_max_mwh - bat.soc_min_mwh).max(1e-9);
  let soc_levels = ca.dp[b][0].len();
  let idx_f = (soc - bat.soc_min_mwh) / soc_span * ((soc_levels - 1) as f64);
  let idx0 = (idx_f.floor() as isize).max(0) as usize;
  let idx0c = idx0.min(soc_levels - 1);
  (ca.dp_charge_shadow[b][idx0c], ca.dp_discharge_shadow[b][idx0c])
}

#[inline]
fn compute_unconstrained_qp(
  challenge: &Challenge, state: &State, ca: &TitanCache, b: usize
) -> f64 {
  let bat = &challenge.batteries[b];
  let node = ca.batt_nodes[b];
  let rt_price = if node < state.rt_prices.len() { state.rt_prices[node] } else { 0.0 };
  let dt = 0.25_f64;
  let (charge_shadow, discharge_shadow) = dual_soc_shadow(ca, b, state.socs[b], bat);

  let eta_c = bat.efficiency_charge;
  let eta_d = bat.efficiency_discharge;

  let marginal_charge = -rt_price * eta_c * dt + charge_shadow * eta_c * dt
  - 0.25_f64 * dt;
  let marginal_discharge = rt_price * dt / eta_d + discharge_shadow / eta_d * dt
  - 0.25_f64 * dt;

  if marginal_discharge > 0.0 && marginal_discharge >= marginal_charge.abs() {
  let u_unconstrained = marginal_discharge * bat.capacity_mwh * bat.capacity_mwh / (2.0_f64 * dt * dt);
  u_unconstrained.min(bat.power_discharge_mw).max(0.0_f64)
  } else if marginal_charge > 0.0 {
  let u_unconstrained = marginal_charge * bat.capacity_mwh * bat.capacity_mwh / (2.0_f64 * dt * dt * eta_c);
  (-u_unconstrained).max(-bat.power_charge_mw).min(0.0_f64)
  } else {
  0.0_f64
  }
}
...
fn run_asca(
  ...
  for b in 0..num_b {
  let u_init = compute_unconstrained_qp(challenge, state, ca, b);
  actions[b] = u_init;
  for &(l, p) in &ca.b_to_lines[b] {
  if l < num_l { flows[l] += p * u_init; }
  }
  }
