// energy_solver_multiday_11: STRUCTURAL — SOC-aware threshold scaling. Each battery's effective discharge_th decreases when SOC high, charge_th decreases when SOC low. SOC inferred from action_bounds ratio. soc_bias=0 reproduces multiday_9.
...
pub fn help() {
    ...
    println!("  soc_bias: float range=[0.0, 0.5] default=0.0  # SOC-driven threshold tilt; 0=multiday_9, higher=stronger SOC-cycling preference");
    println!();
    println!("Adds SOC-aware threshold scaling to multiday_9. SOC proxy from action_bounds (u_max / (u_max + |u_min|)).");
    println!("At high SOC: discharge_th *= (1 - 2*bias*(soc-0.5))  [lower th, more eager to discharge]");
    println!("At low SOC: charge_th *= (1 + 2*bias*(soc-0.5))     [lower th, more eager to charge]");
    println!("All other params held at multiday_9 winners (ct=8, dt=1, ew=66, ls=66, sf=1.273, p=2.545).");
}

fn parse_hp(h: &Option<Map<String, Value>>) -> Hyperparameters {
    let mut out = Hyperparameters::default();
    if let Some(m) = h {
        if let Some(v) = m.get("soc_bias").and_then(|v| v.as_f64()) {
            out.soc_bias = v;
        }
    }
    out
...
fn policy_with_hp(challenge: &Challenge, state: &State, hp: &Hyperparameters) -> Result<Vec<f64>> {
    ...
    let soc_bias = hp.soc_bias.clamp(0.0, 1.0);
    ...
    for (i, battery) in challenge.batteries.iter().enumerate() {
        ...
        // SOC proxy from action_bounds: u_max=room to discharge, |u_min|=room to charge.
        // soc_proxy in [0, 1]: 0 = empty (can only charge), 1 = full (can only discharge), 0.5 = mid.
        let pos = u_max.max(0.0);
        let neg = (-u_min).max(0.0);
        let soc_proxy = if pos + neg > EPS {
            pos / (pos + neg)
        } else {
            0.5
        };
        let soc_centered = soc_proxy - 0.5; // [-0.5, 0.5]
        // tilt: high SOC → easier discharge (lower threshold), harder charge (higher threshold)
        // factor_d = 1 - 2*bias*soc_centered ∈ [1-bias, 1+bias]
        // factor_c = 1 + 2*bias*soc_centered ∈ [1+bias, 1-bias] (inverted)
        let tilt = (2.0 * soc_bias * soc_centered).clamp(-0.95, 0.95);
        let discharge_th_eff = (base_discharge_th * (1.0 - tilt)).max(0.5);
        let charge_th_eff = (base_charge_th * (1.0 + tilt)).max(0.5);
