...
n if n <= 80 => near_arb_v1::solve_challenge(challenge, save_solution, hyperparameters),
n if n <= 150 => titan_v2::solve_challenge(challenge, save_solution, hyperparameters),
n => Err(anyhow!(
"ea_frontier_router: unsupported num_batteries={} (expected one of 10/20/40/60/100)",
n
)),
}
}
pub fn help() {
println!(
"ea_frontier_router: routes c008 by track: titan_v2 on baseline/congested/capstone, near_arb_v1 on multiday/dense; capstone uses titan_v2 sub_t53 with KKT enabled."
);
}
