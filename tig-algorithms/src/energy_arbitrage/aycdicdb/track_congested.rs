...
mod helpers {
    ...
    fn lns_dw_per_step(
        ...
        let line_set = select_active_lines(challenge, flows_base, ca, Some(base_actions), hp.lns_max_lines);
    ...
    fn targeted_tight_line_polish(
        ...
        let mut line_weights = vec![0.0_f64; num_l];
        for &(util, l) in &tight_lines {
            line_weights[l] = 0.25 + util * util;
        }

        let impact_on_line = |b: usize, l: usize| -> f64 {
            ca.b_to_lines[b].iter()
                .find(|&&(ll, _)| ll == l)
                .map(|&(_, p)| p)
                .unwrap_or(0.0)
        };
        ...
        ranked.truncate(if num_b <= 24 { 8.min(num_b) } else { 6.min(num_b) });
        ...
        let subset_profit_now: f64 = subset.iter()
            .map(|&b| eval_profit(challenge, state, ca, b, actions[b]))
            .sum();

        let desire: Vec<f64> = subset.iter().map(|&b| {
            let cur = actions[b];
            let (lo, hi) = state.action_bounds[b];
            let mut best_u = cur;
            let mut best_v = eval_profit(challenge, state, ca, b, cur);

            let try_u = |u: f64, best_u: &mut f64, best_v: &mut f64| {
                if !u.is_finite() { return; }
                let uu = u.clamp(lo, hi);
                let v = eval_profit(challenge, state, ca, b, uu);
                if v.is_finite() && v > *best_v + 1e-12 {
                    *best_v = v;
                    *best_u = uu;
                }
            };

            try_u(lo, &mut best_u, &mut best_v);
            try_u(hi, &mut best_u, &mut best_v);
            if lo <= 0.0 && 0.0 <= hi {
                try_u(0.0, &mut best_u, &mut best_v);
            }

            let mut anchors: Vec<(f64, usize, f64)> = ca.b_to_lines[b].iter()
                .filter_map(|&(l, p)| {
                    let w = line_weights[l];
                    if w > 0.0 { Some((w * p.abs(), l, p)) } else { None }
                })
                .collect();
            anchors.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
            anchors.truncate(2);

            for &(_, l, p) in &anchors {
                if p.abs() > 1e-12 {
                    let target = (-(flows[l] - p * cur) / p).clamp(lo, hi);
                    try_u(target, &mut best_u, &mut best_v);
                }
            }
        ...
        if desire.iter().all(|d| d.abs() < 1e-8) {
            return None;
        }

        let project_direction = |dir: &[f64], coeff: &[f64]| -> Option<Vec<f64>> {
            let dot_dc: f64 = dir.iter().zip(coeff.iter()).map(|(d, c)| d * c).sum();
            let dot_cc: f64 = coeff.iter().map(|c| c * c).sum();
            if dot_cc <= 1e-12 {
                return None;
            }

            let mut projected = Vec::with_capacity(dir.len());
            for i in 0..dir.len() {
                projected.push(dir[i] - coeff[i] * dot_dc / dot_cc);
            }

            let mass: f64 = projected.iter().map(|d| d.abs()).sum();
            let diff: f64 = projected.iter().zip(dir.iter()).map(|(a, b)| (a - b).abs()).sum();
            if mass > 1e-8 && diff > 1e-8 {
                Some(projected)
            } else {
                None
            }
        ...
        let combo_main: Vec<f64> = subset.iter().map(|&b| {
            let mut c = 0.0_f64;
            for &(_, l) in tight_lines.iter().take(2) {
                c += flows[l].signum() * line_weights[l] * impact_on_line(b, l);
            }
            c
        }).collect();

        let mut directions: Vec<Vec<f64>> = Vec::new();
        if let Some(dir) = project_direction(&desire, &combo_main) {
            directions.push(dir);
        }
        {
            let l0 = tight_lines[0].1;
            let coeff0: Vec<f64> = subset.iter().map(|&b| impact_on_line(b, l0)).collect();
            if let Some(dir) = project_direction(&desire, &coeff0) {
                directions.push(dir);
            }
        }
        if tight_lines.len() >= 2 {
            let l1 = tight_lines[1].1;
            let coeff1: Vec<f64> = subset.iter().map(|&b| impact_on_line(b, l1)).collect();
            if let Some(dir) = project_direction(&desire, &coeff1) {
                directions.push(dir);
            }
        }
        ...
        let ternary_iters = (hp.ternary_iters / 2).max(6);
        let mut best_actions: Option<Vec<f64>> = None;
        let mut best_profit = base_profit;

        for dir in directions {
            if dir.iter().all(|d| d.abs() < 1e-9) {
                continue;
            }
            ...
            for (i, &b) in subset.iter().enumerate() {
                ...
                let x2 = (hi - cur) / d;
                let (llo, hhi) = if x1 < x2 { (x1, x2) } else { (x2, x1) };
                if llo > alpha_lo { alpha_lo = llo; }
                if hhi < alpha_hi { alpha_hi = hhi; }
                for &(l, _) in &ca.b_to_lines[b] {
                    if l < num_l { touched_lines[l] = true; }
                }
            }

            if !alpha_lo.is_finite() || !alpha_hi.is_finite() || alpha_lo > alpha_hi {
                continue;
            }
            ...
            for l in 0..num_l {
                ...
                for (i, &b) in subset.iter().enumerate() {
                    let d = dir[i];
                    if d.abs() < 1e-10 { continue; }
                    c += impact_on_line(b, l) * d;
                }
                if c.abs() < 1e-12 { continue; }

                let limit = limits[l];
                let x1 = (-limit - flows[l]) / c;
                let x2 = ( limit - flows[l]) / c;
                let (llo, hhi) = if x1 < x2 { (x1, x2) } else { (x2, x1) };
                if llo > alpha_lo { alpha_lo = llo; }
                if hhi < alpha_hi { alpha_hi = hhi; }
            }
            ...
            let eval_alpha = |alpha: f64| -> f64 {
                let mut val = base_profit - subset_profit_now;
                for (i, &b) in subset.iter().enumerate() {
                    val += eval_profit(challenge, state, ca, b, actions[b] + alpha * dir[i]);
                }
                val
            };

            let mut best_alpha = 0.0_f64;
            let mut best_val = base_profit;

            {
                let try_alpha = |alpha: f64, best_alpha: &mut f64, best_val: &mut f64| {
                    if !alpha.is_finite() { return; }
                    let val = eval_alpha(alpha);
                    if val.is_finite() && val > *best_val + 1e-9 {
                        *best_val = val;
                        *best_alpha = alpha;
                    }
                };

                try_alpha(alpha_lo, &mut best_alpha, &mut best_val);
                try_alpha(alpha_hi, &mut best_alpha, &mut best_val);
                if alpha_lo <= 0.0 && 0.0 <= alpha_hi {
                    try_alpha(0.0, &mut best_alpha, &mut best_val);
                }

                if alpha_lo < 0.0 {
                    let hi_neg = 0.0_f64.min(alpha_hi);
                    if alpha_lo < hi_neg {
                        let (a, v) = ternary_search(|x| eval_alpha(x), alpha_lo, hi_neg, ternary_iters);
                        if v.is_finite() && v > best_val + 1e-9 {
                            best_val = v;
                            best_alpha = a;
                        }
                    }
                ...
                if alpha_hi > 0.0 {
                    let lo_pos = 0.0_f64.max(alpha_lo);
                    if lo_pos < alpha_hi {
                        let (a, v) = ternary_search(|x| eval_alpha(x), lo_pos, alpha_hi, ternary_iters);
                        if v.is_finite() && v > best_val + 1e-9 {
                            best_val = v;
                            best_alpha = a;
                        }
            ...
            if best_alpha.abs() <= 1e-10 || best_val <= best_profit + 1e-9 {
                continue;
            }

            let mut cand = actions.to_vec();
            let mut cand_flows = flows.clone();
            for (i, &b) in subset.iter().enumerate() {
                let new_u = (actions[b] + best_alpha * dir[i]).clamp(state.action_bounds[b].0, state.action_bounds[b].1);
                let delta = new_u - actions[b];
                if delta.abs() <= 1e-12 { continue; }
                cand[b] = new_u;
                for &(l, p) in &ca.b_to_lines[b] {
                    if l < num_l { cand_flows[l] += p * delta; }
                }
            }
            ...
            let feasible = (0..num_l).all(|l| {
                let limit = limits[l];
                if limit > 1e-6 {
                    cand_flows[l].abs() <= limit * 1.000_001 + 1e-6
                } else {
                    cand_flows[l].abs() <= 1e-6
                }
            });

            if feasible {
                best_profit = best_val;
                best_actions = Some(cand);
            }
        }
    ...
    fn refine_solution(
        ...
        let compute_flows = |act: &[f64]| -> Vec<f64> {
            challenge.network.compute_flows(&challenge.compute_total_injections(state, act))
        };

        let immediate_profit = |b: usize, u: f64| -> f64 {
            let bat = &challenge.batteries[b];
            let node = challenge.batteries[b].node;
            ...
            let revenue = u * rt * dt;
            let tx = 0.25 * abs_u * dt;
            let deg = ((abs_u * dt) / bat.capacity_mwh.max(1e-9)).powi(2);
            revenue - tx - deg
        };
        ...
        let feasible_interval = |b: usize, cur_u: f64, cur_flows: &[f64]| -> (f64, f64) {
            ...
            for &(l, p) in &ptdf_b[b] {
                ...
                let lim = limits[l];
                let f_other = cur_flows[l] - p * cur_u;
                let x1 = (-lim - f_other) / p;
                let x2 = ( lim - f_other) / p;
                let (llo, hhi) = if x1 < x2 { (x1, x2) } else { (x2, x1) };
                if llo > lo { lo = llo; }
                if hhi < hi { hi = hhi; }
            }
        ...
        let baseline_actions = actions.clone();
        let mut actions = actions;
        for b in 0..num_b {
            let (lo, hi) = state.action_bounds[b];
            actions[b] = actions[b].clamp(lo, hi);
            if !actions[b].is_finite() {
                actions[b] = 0.0_f64.clamp(lo, hi);
            }
        }

        let mut flows = compute_flows(&actions);
        let mut local_profit: f64 = (0..num_b).map(|b| immediate_profit(b, actions[b])).sum();
        let start_actions = actions.clone();

        let stage1_sweeps = if fuel_budget > 1_000_000_000 && num_b <= 24 { 3 } else { 2 };
        for _ in 0..stage1_sweeps {
            let mut order: Vec<(f64, usize)> = Vec::with_capacity(num_b);
            for b in 0..num_b {
                let cur = actions[b];
                let (lo, hi) = feasible_interval(b, cur, &flows);
                let base = immediate_profit(b, cur);
                let mut upside = 0.0_f64;

                upside = upside.max((immediate_profit(b, lo) - base).max(0.0));
                upside = upside.max((immediate_profit(b, hi) - base).max(0.0));
                if lo <= 0.0 && 0.0 <= hi {
                    upside = upside.max((immediate_profit(b, 0.0) - base).max(0.0));
                }

                let orig = start_actions[b].clamp(lo, hi);
                upside = upside.max((immediate_profit(b, orig) - base).max(0.0));

                let mut stress = 0.0_f64;
                for &(l, p) in &ptdf_b[b] {
                    let lim = limits[l].max(1e-6);
                    stress += p.abs() * (flows[l].abs() / lim).min(1.5);
                }
                order.push((upside * (1.0 + 0.5 * stress), b));
            }
            ...
            for &(_, b) in &order {
                ...
                let mut best_u = cur;
                let mut best_local = local_profit;

                {
                    let try_u = |u: f64, best_u: &mut f64, best_local: &mut f64| {
                        if !u.is_finite() || (u - cur).abs() < 1e-10 { return; }
                        let cand_local = local_profit - immediate_profit(b, cur) + immediate_profit(b, u);
                        if cand_local > *best_local + 1e-9 {
                            *best_local = cand_local;
                            *best_u = u;
                        }
                    };

                    try_u(lo, &mut best_u, &mut best_local);
                    try_u(hi, &mut best_u, &mut best_local);

                    if lo <= 0.0 && 0.0 <= hi {
                        try_u(0.0, &mut best_u, &mut best_local);
                    }

                    let orig = start_actions[b].clamp(lo, hi);
                    try_u(orig, &mut best_u, &mut best_local);

                    let mut touched: Vec<(f64, usize, f64)> = ptdf_b[b].iter()
                        .filter_map(|&(l, p)| {
                            let lim = limits[l].max(1e-6);
                            let util = flows[l].abs() / lim;
                            if util > 0.80 { Some((util, l, p)) } else { None }
                        })
                        .collect();
                    touched.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
                    touched.truncate(2);
                    for &(_, l, p) in &touched {
                        let target = (-(flows[l] - p * cur) / p).clamp(lo, hi);
                        try_u(target, &mut best_u, &mut best_local);
                    }
                    ...
                    if lo < 0.0 {
                        ...
                        if lo < hi_neg {
                            let eval = |u: f64| {
                                local_profit - immediate_profit(b, cur) + immediate_profit(b, u)
                            };
                            let (u, v) = ternary_search(eval, lo, hi_neg, 12);
                            if v > best_local + 1e-9 {
                                best_local = v;
                                best_u = u;
                            }
                        }
                    ...
                    if hi > 0.0 {
                        ...
                        if lo_pos < hi {
                            ...
                            let (u, v) = ternary_search(eval, lo_pos, hi, 12);
                            if v > best_local + 1e-9 {
                                best_local = v;
                                best_u = u;
                            }
        ...
        let max_sub_lines = num_l.min(if num_b <= 24 { 6 } else { 4 });
        let subset_cap = num_b.min(if num_b <= 24 { 10 } else { 8 });
        let pair_cap = subset_cap.min(if num_b <= 24 { 8 } else { 6 });
        let sub_rounds = if fuel_budget > 1_000_000_000 && num_b <= 20 { 3 } else { 2 };

        for _ in 0..sub_rounds {
            let mut tight_lines: Vec<(f64, usize)> = (0..num_l)
                .filter_map(|l| {
                    let lim = limits[l];
                    if lim <= 1e-6 { None } else { Some((flows[l].abs() / lim, l)) }
                })
                .collect();
            tight_lines.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
            tight_lines.truncate(max_sub_lines);
            if tight_lines.is_empty() { break; }

            let mut line_weights = vec![0.0_f64; num_l];
            for &(util, l) in &tight_lines {
                line_weights[l] = (0.25 + util).powi(2);
            }

            let mut subset_rank: Vec<(f64, usize)> = Vec::new();
            for b in 0..num_b {
                let cur = actions[b];
                let (lo, hi) = state.action_bounds[b];
                let room = (hi - cur).abs() + (cur - lo).abs();
                let mut touch = 0.0_f64;
                for &(l, p) in &ptdf_b[b] {
                    let w = line_weights[l];
                    if w > 0.0 {
                ...
                let score = touch * (1.0 + 0.2 * room) + 0.05 * (start_actions[b] - cur).abs();
                if score > 1e-9 {
                    subset_rank.push((score, b));
                }
            }
            if subset_rank.is_empty() { break; }

            subset_rank.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
            subset_rank.truncate(subset_cap);
            let subset: Vec<usize> = subset_rank.iter().map(|&(_, b)| b).collect();
            ...
            for &b in &subset {
...

                {
                    ...
                    let mut anchors: Vec<(f64, usize, f64)> = ptdf_b[b].iter()
                        .filter_map(|&(l, p)| {
                            let w = line_weights[l];
                            if w > 0.0 { Some((w, l, p)) } else { None }
                        })
                        .collect();
                    anchors.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
                    anchors.truncate(3);
                    ...
                    if lo < 0.0 {
                        ...
                        if lo < hi_neg {
                            ...
                            let (u, v) = ternary_search(eval, lo, hi_neg, 10);
                            if v > best_local + 1e-9 {
                                best_local = v;
                                best_u = u;
                            }
                    ...
                    if hi > 0.0 {
                        ...
                        if lo_pos < hi {
                            ...
                            let (u, v) = ternary_search(eval, lo_pos, hi, 10);
                            if v > best_local + 1e-9 {
                                best_local = v;
                                best_u = u;
                            }
            ...
            let pair_order: Vec<usize> = subset.iter().take(pair_cap).copied().collect();
            let mut best_pair_local = local_profit;
            let mut best_pair_move: Option<(usize, usize, f64, f64)> = None;

            for &bi in &pair_order {
                for &bj in &pair_order {
                    if bi == bj { continue; }
                    ...
                    for &(_, l) in &tight_lines {
                        let w = line_weights[l];
                        if w <= 0.0 { continue; }
                        let pi = impact_on_line(bi, l);
                        let pj = impact_on_line(bj, l);
                        cross += w * pi * pj;
                        denom += w * pj * pj;
                    }
                    ...
                    if !ratio.is_finite() || ratio.abs() < 1e-9 || ratio.abs() > 20.0 { continue; }

                    let cur_i = actions[bi];
                    let cur_j = actions[bj];

                    let mut dir_imp = vec![0.0_f64; num_l];
                    for &(ll, p) in &ptdf_b[bi] { dir_imp[ll] += p; }
                    for &(ll, p) in &ptdf_b[bj] { dir_imp[ll] += ratio * p; }
...

                    {
                        ...
                        let x2 = hi - cur_i;
                        let (llo, hhi) = if x1 < x2 { (x1, x2) } else { (x2, x1) };
                        if llo > t_lo { t_lo = llo; }
                        if hhi < t_hi { t_hi = hhi; }
                    }
                    {
                        let (lo, hi) = state.action_bounds[bj];
                        let x1 = (lo - cur_j) / ratio;
                        let x2 = (hi - cur_j) / ratio;
                        let (llo, hhi) = if x1 < x2 { (x1, x2) } else { (x2, x1) };
                        if llo > t_lo { t_lo = llo; }
                        if hhi < t_hi { t_hi = hhi; }
                    }
                    ...
                    for ll in 0..num_l {
                        ...
                        if c.abs() < 1e-12 { continue; }
                        let lim = limits[ll];
                        let x1 = (-lim - flows[ll]) / c;
                        let x2 = ( lim - flows[ll]) / c;
                        let (llo, hhi) = if x1 < x2 { (x1, x2) } else { (x2, x1) };
                        if llo > t_lo { t_lo = llo; }
                        if hhi < t_hi { t_hi = hhi; }
                    }

                    if t_lo > t_hi || t_lo > tol || t_hi < -tol { continue; }

                    let eval_t = |t: f64| {
                        local_profit
                            - immediate_profit(bi, cur_i)
                            - immediate_profit(bj, cur_j)
                            + immediate_profit(bi, cur_i + t)
                            + immediate_profit(bj, cur_j + ratio * t)
                    };

                    let mut best_t = 0.0_f64;
                    let mut candidate_local = local_profit;

                    {
                        let try_t = |t: f64, best_t: &mut f64, candidate_local: &mut f64| {
                            if !t.is_finite() { return; }
                            let val = eval_t(t);
                            if val > *candidate_local + 1e-9 {
                                *candidate_local = val;
                                *best_t = t;
                            }
                        };

                        let mut cuts = vec![t_lo, t_hi, 0.0, -cur_i, -cur_j / ratio];
                        cuts.retain(|x| x.is_finite() && *x >= t_lo - 1e-9 && *x <= t_hi + 1e-9);
                        cuts.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
                        cuts.dedup_by(|a, b| (*a - *b).abs() < 1e-9);

                        for &t in &cuts {
                            try_t(t, &mut best_t, &mut candidate_local);
                        }
                        ...
                        for w in cuts.windows(2) {
                            ...
                            let (t, v) = ternary_search(|x| eval_t(x), a, b, 10);
                            if v > candidate_local + 1e-9 {
                                candidate_local = v;
                                best_t = t;
                            }
                        }
                    ...
                    if candidate_local > best_pair_local + 1e-9 {
                        best_pair_local = candidate_local;
                        best_pair_move = Some((bi, bj, ratio, best_t));
                    }
                }
            }

            if let Some((bi, bj, ratio, best_t)) = best_pair_move {
                if best_pair_local > local_profit + 1e-9 {
                    actions[bi] += best_t;
                    actions[bj] += ratio * best_t;
                    for &(l, p) in &ptdf_b[bi] {
                        flows[l] += p * best_t;
                    }
                    for &(l, p) in &ptdf_b[bj] {
                        flows[l] += p * ratio * best_t;
                    }
                    local_profit = best_pair_local;
                    improved_round = true;
                }
            }
        ...
        let cleanup_depth = num_b.min(if num_b <= 24 { 12 } else { 8 });
        let mut cleanup_rank: Vec<(f64, usize)> = Vec::with_capacity(num_b);
        for b in 0..num_b {
            let cur = actions[b];
            let (lo, hi) = feasible_interval(b, cur, &flows);
            let mut span_score = (hi - lo).abs();
            let mut touch = 0.0_f64;
            for &(l, p) in &ptdf_b[b] {
                let lim = limits[l].max(1e-6);
                touch += p.abs() * (flows[l].abs() / lim).min(2.0);
            }
            span_score *= 1.0 + touch;
            cleanup_rank.push((span_score, b));
        }
        cleanup_rank.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
        cleanup_rank.truncate(cleanup_depth);

        for &(_, b) in &cleanup_rank {
            let cur = actions[b];
            let (lo, hi) = feasible_interval(b, cur, &flows);
            if hi - lo < 1e-10 { continue; }
        ...
        let final_flows = compute_flows(&actions);
        let feasible = (0..num_l).all(|l| final_flows[l].abs() <= limits[l] + 1e-6 * limits[l].max(1.0));
        if feasible {
            actions
        } else {
            baseline_actions
...
mod track_congested {
    ...
    fn defaults() -> TrackHp {
        TrackHp {
            ...
            use_lp: true,
            lp_soft_lambda: 1e5,
            lp_per_call_pivots: 700,
            lp_total_pivots: 12000,
            use_policy: false,
            use_warmstart: true,
            use_mpc: false,
            mpc_horizon: 2,
