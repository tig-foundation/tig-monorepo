...
mod helpers {
    ...
    fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
        STATE.with(|s| -> Result<Vec<f64>> {
            ...
            let use_aycdicdb_extras =
                hp.use_kkt || hp.dual_iters > 0 || hp.max_admm_iters > 0 || hp.use_lp || hp.lp_horizon > 0;

            if use_aycdicdb_extras {
                ...
                let seed_price_scale = avg_price.max(10.0) * 0.25;

                let mut near_binding = 0usize;
                let mut line_mu = vec![0.0_f64; flows_base.len()];
                for l in 0..flows_base.len() {
                    ...
                    if util > hp.lmp_threshold {
                        near_binding += 1;
                        let excess = ((util - hp.lmp_threshold) / (1.0 - hp.lmp_threshold).max(1e-6))
                            .clamp(0.0, 1.0);
                        line_mu[l] = excess * seed_price_scale * flows_base[l].signum();
                    }
                }
                ...
                for b in 0..num_b {
                    ...
                    let shadow: f64 = if near_binding > 0 {
                        cache.b_to_lines[b].iter().map(|&(l, impact)| line_mu[l] * impact).sum()
                    } else { 0.0 };
                    let mut best_u = 0.0_f64;
                    let mut best_v = eval_profit(challenge, state, cache, b, 0.0);
                    for &u_try in &[lo, hi, 0.5 * lo, 0.5 * hi] {
                        let v = eval_profit(challenge, state, cache, b, u_try) - shadow * u_try;
                ...
                let (mut best_actions, _) = [(actions_a, val_a), (actions_b, val_b)]
                    .into_iter()
    ...
    fn joint_pair_refine(
        ...
        let pair_budget = (num_b * 2).min(64);
        ...
        while improved {
            ...
            'outer: for i in 0..num_b {
                for j in (i + 1)..num_b {
                    ...
                    let base_val =
                        eval_profit(challenge, state, ca, i, cur_i) +
                        eval_profit(challenge, state, ca, j, cur_j);

                    let mut best_pair = (cur_i, cur_j, base_val);

                    for &alpha in &[-0.5_f64, -0.25, 0.25, 0.5] {
                        let cand_i = (cur_i + alpha * (hi_i - lo_i)).clamp(lo_i, hi_i);
                        let cand_j = (cur_j - alpha * (hi_j - lo_j)).clamp(lo_j, hi_j);
                        ...
                        for l in 0..num_l {
                            ...
                            for &(b, imp) in &ca.ptdf_sparse[l] {
                                if b == i { f += imp * delta_i; }
                                if b == j { f += imp * delta_j; }
                            }
                            let limit = (challenge.network.flow_limits[l] - hp.flow_margin).max(0.0);
                            if f.abs() > limit {
                                feasible = false;
                        ...
                        if feasible {
                            let val =
                                eval_profit(challenge, state, ca, i, cand_i) +
                                eval_profit(challenge, state, ca, j, cand_j);
                            if val > best_pair.2 {
                                best_pair = (cand_i, cand_j, val);
                            }
                        }
                    }

                    if best_pair.2 > base_val + 1e-9 {
                        let delta_i = best_pair.0 - cur_i;
                        let delta_j = best_pair.1 - cur_j;
                        actions[i] = best_pair.0;
                        actions[j] = best_pair.1;
                        for l in 0..num_l {
                            let mut add = 0.0_f64;
                            for &(b, imp) in &ca.ptdf_sparse[l] {
                                if b == i { add += imp * delta_i; }
                                if b == j { add += imp * delta_j; }
                            }
                            flows[l] += add;
                        }
                        improved = true;
    ...
    fn run_deflator(
        ...
        let mut current_vals: Vec<f64> = (0..num_b)
            .map(|b| eval_profit(challenge, state, ca, b, actions[b]))
            .collect();

        let mut is_safe = false;
        for _ in 0..hp.deflator_iters {
            let mut worst_l = num_l;
            let mut worst_overflow = 0.0_f64;
            ...
            for l in 0..num_l {
                ...
                if overflow > worst_overflow {
                    ...
                    worst_l = l;
                    worst_sign = flows[l].signum();
                }
            }

            if worst_l == num_l || worst_overflow <= 1e-9 {
                is_safe = true;
                break;
            }

            let mut best_b = num_b;
            let mut best_new_action = 0.0_f64;
            let mut best_new_val = 0.0_f64;
            let mut best_delta = 0.0_f64;
            let mut best_total_relief = 0.0_f64;
            let mut best_score = f64::INFINITY;

            for &(b, impact) in &ca.ptdf_sparse[worst_l] {
                let cur_action = actions[b];
                let contrib = impact * cur_action;
                if contrib * worst_sign <= 1e-9 { continue; }

                let max_relief = contrib.abs();
                if max_relief <= 1e-12 { continue; }

                let reduction = max_relief.min(worst_overflow);
                let ratio = (1.0 - reduction / max_relief).clamp(0.0, 1.0);
                let cand_action = cur_action * ratio;
                let delta_action = cand_action - cur_action;
                if delta_action.abs() <= 1e-12 { continue; }

                let primary_relief = (-(impact * delta_action) * worst_sign).max(0.0);
                if primary_relief <= 1e-12 { continue; }

                let cand_val = eval_profit(challenge, state, ca, b, cand_action);
                let profit_loss = (current_vals[b] - cand_val).max(0.0);

                let mut total_relief = primary_relief;
                for &(ll, pp) in &ca.b_to_lines[b] {
                    if ll == worst_l { continue; }
                    let other_limit = (challenge.network.flow_limits[ll] - hp.flow_margin).max(0.0);
                    if other_limit <= 1e-6 { continue; }
                    let other_overflow = flows[ll].abs() - other_limit;
                    if other_overflow <= 1e-9 { continue; }

                    let line_relief = (-(pp * delta_action) * flows[ll].signum()).max(0.0);
                    if line_relief > 1e-12 {
                        total_relief += line_relief.min(other_overflow);
                    }
                }

                let score = profit_loss / total_relief.max(1e-9);
                if score < best_score
                    || ((score - best_score).abs() <= 1e-12 && total_relief > best_total_relief)
                {
                    best_b = b;
                    best_new_action = cand_action;
                    best_new_val = cand_val;
                    best_delta = delta_action;
                    best_total_relief = total_relief;
                    best_score = score;
                }
            }

            if best_b == num_b || best_delta.abs() <= 1e-12 { break; }

            actions[best_b] = best_new_action;
            current_vals[best_b] = best_new_val;
            for &(ll, pp) in &ca.b_to_lines[best_b] {
                if ll < num_l { flows[ll] += pp * best_delta; }
            }
        }
