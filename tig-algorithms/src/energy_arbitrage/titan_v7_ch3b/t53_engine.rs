...
// ---- C-H3B: CONCAVE-HULL PWL DISPATCH LP replacing the PGA stage (KNOWN METHOD) ---------------------------------------
// Per step: for each battery the exact one-step objective f_b(u) = dp_action_value(...) is sampled at its kinks (bounds,
// 0, SOC-grid crossings, clamp points) and midpoints, its upper concave envelope is built, and the resulting separable
// concave PWL LP over the box and all line constraints is solved by constraint generation + bounded dual simplex
// (lpdisp.rs). The returned feasible action replaces the PGA result; titan's unchanged polish chain and final guard follow.
// Inputs are exactly those of the PGA: current state, bounds, base flows, sens, limits, titan's V tables for t+1.
const CH3B_NSUB: usize = 2;
fn ch3b_kinks(battery: &Battery, soc: f64, lo: f64, hi: f64, soc_lo: f64, soc_step_inv: f64, levels: usize) -> Vec<f64> {
    let mut k: Vec<f64> = vec![lo, hi];
    if lo < 0.0 && hi > 0.0 { k.push(0.0); }
    if soc_step_inv > 0.0 {
        let step = 1.0 / soc_step_inv;
        for j in 0..levels {
            let s = soc_lo + j as f64 * step;
            let ud = (soc - s) * battery.efficiency_discharge / DELTA_T; if ud > 0.0 && ud > lo && ud < hi { k.push(ud); }
            let uc = -(s - soc) / (battery.efficiency_charge * DELTA_T); if uc < 0.0 && uc > lo && uc < hi { k.push(uc); }
        }
    }
    for s in [battery.soc_min_mwh, battery.soc_max_mwh] {
        let ud = (soc - s) * battery.efficiency_discharge / DELTA_T; if ud > lo && ud < hi { k.push(ud); }
        let uc = -(s - soc) / (battery.efficiency_charge * DELTA_T); if uc > lo && uc < hi { k.push(uc); }
    }
    k.sort_by(|a, b| a.partial_cmp(b).unwrap()); k.dedup_by(|a, b| (*a - *b).abs() <= 1e-12); k
}
fn ch3b_dispatch(challenge: &Challenge, state: &State, dps: &[BatteryDP], sens: &[Vec<f64>], base_flows: &[f64]) -> Vec<f64> {
    let t = state.time_step;
    let hulls: Vec<super::lpdisp::Hull> = (0..challenge.num_batteries).map(|b| {
        let battery = &challenge.batteries[b]; let (lo, hi) = state.action_bounds[b]; let soc = state.socs[b]; let price = state.rt_prices[battery.node];
        let dp = &dps[b];
        if hi - lo <= 1e-12 { return super::lpdisp::Hull { x0: lo, len: vec![], slope: vec![] }; }
        let kinks = ch3b_kinks(battery, soc, lo, hi, dp.soc_lo, dp.soc_step_inv, dp.levels);
        super::lpdisp::build_hull(&kinks, CH3B_NSUB, |u| dp_action_value(dp, battery, t, soc, price, u))
    }).collect();
    let (mut u, _stats) = super::lpdisp::solve(&hulls, sens, base_flows, &challenge.network.flow_limits, &state.action_bounds);
    clamp_to_bounds(&mut u, &state.action_bounds);
    u
}
// ---- end C-H3B ------------------------------------------------------------------------------------------------------
...
fn policy(
    ...
    let mut result = if hp.use_dual_dispatch {
        ...
        // C-H3B: the joint dispatch decision comes from the concave-hull dispatch LP (same inputs as the PGA stage)
        let _unused_seeds = (target, dp_seed, zero.clone());
        let lp_result = ch3b_dispatch(challenge, state, dps, sens, &base_flows);
        let pga_val = total_step_value(challenge, state, dps, &lp_result);
        let mut r = lp_result;
