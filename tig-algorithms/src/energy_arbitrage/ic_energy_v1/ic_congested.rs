...
mod helpers {
    ...
    pub struct TrackHp {
        ...
        // KKT exogenous-LMP dispatch
        pub use_kkt: bool,
        pub kkt_cong_threshold: f64,
        pub kkt_price_scale: f64,
        // ADMM dispatch (replaces deflator when max_admm_iters > 0)
        pub max_admm_iters: usize,
        pub admm_rho: f64,
        pub admm_primal_tol: f64,
        // Soft-constrained joint LP dispatch per timestep
        pub use_lp: bool,
        pub lp_soft_lambda: f64, // penalty for soft line-flow violations
        pub lp_per_call_pivots: usize, // pivot budget per LP call (0 = unlimited, default LP_MAX_PIVOTS)
        pub lp_total_pivots: usize,    // cumulative pivot budget across all steps (0 = unlimited)
        // CMA-ES policy dispatch
        pub use_policy: bool,
        // Warm-start ASCA/KKT from DP unconstrained optimum
        pub use_warmstart: bool,
        // MPC 2-step receding-horizon LP
        pub use_mpc: bool,
        pub mpc_horizon: usize,
        pub mpc_pivot_budget: usize,
        // DW-lite dual price discovery
        pub use_dw: bool,
        pub dw_iters: usize,
        ...
        // DW prescreen: simulate with zero duals to find truly binding lines
        pub use_dw_prescreen: bool,
        // ALNS-DW per-step column-generation repair for congested timesteps
        pub use_lns: bool,
        pub lns_cg_iters: usize,
        pub lns_cg_column_limit: usize,
        pub lns_max_lines: usize,
        pub lns_lp_pivots_total: usize,
        // Pivot budget lending: dynamically reallocate LP pivots from easy steps to congested ones
        pub use_pivot_reserve: bool,
        // Top-K line filtering for joint LP: only include the K most-loaded lines as constraints
        pub lp_max_lines: usize,
        // Parallel DP construction across batteries
        pub use_parallel_dp: bool,
        // Gauss-Hermite stochastic DP: integrate over price scenarios in DP value function
        pub use_sdp: bool,
        pub sdp_k: usize,
        // Proximal-scaled subgradient with momentum for LDD dual updates
        pub use_ldd_proximal: bool,
        pub ldd_momentum: f64,
        pub ldd_clip_fraction: f64,
        // Tail-mixture quadrature: replace GH quadrature with Gaussian-Pareto mixture when use_sdp is active
        pub use_tail_quadrature: bool,
        // PTDF-aware congestion surcharge experiment: adjusts effective RT prices before dispatch.
        pub use_congestion_safety: bool,
        pub congestion_safety_threshold: f64,
        pub congestion_safety_strength: f64,
    }
    ...
    impl TrackHp {
        pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
            ...
            if let Some(v) = m.get("dual_iters").and_then(|v| v.as_u64()) {
                self.dual_iters = v as usize;
            }
            if let Some(v) = m.get("da_step_size").and_then(|v| v.as_f64()) {
                self.da_step_size = v.max(0.0);
            }
            if let Some(v) = m.get("ldd_iters").and_then(|v| v.as_u64()) {
                self.ldd_iters = v as usize;
            }
            if let Some(v) = m.get("ldd_step_size").and_then(|v| v.as_f64()) {
                self.ldd_step_size = v.max(0.0);
            }
            if let Some(v) = m.get("use_kkt").and_then(|v| v.as_bool()) {
                self.use_kkt = v;
            }
            if let Some(v) = m.get("kkt_cong_threshold").and_then(|v| v.as_f64()) {
                self.kkt_cong_threshold = v.clamp(0.0, 1.0);
            }
            if let Some(v) = m.get("kkt_price_scale").and_then(|v| v.as_f64()) {
                self.kkt_price_scale = v.max(0.1);
            }
            if let Some(v) = m.get("max_admm_iters").and_then(|v| v.as_u64()) {
                self.max_admm_iters = v as usize;
            }
            if let Some(v) = m.get("admm_rho").and_then(|v| v.as_f64()) {
                self.admm_rho = v;
            }
            if let Some(v) = m.get("admm_primal_tol").and_then(|v| v.as_f64()) {
                self.admm_primal_tol = v;
            }
            if let Some(v) = m.get("use_lp").and_then(|v| v.as_bool()) {
                self.use_lp = v;
            }
            if let Some(v) = m.get("lp_soft_lambda").and_then(|v| v.as_f64()) {
                self.lp_soft_lambda = v.max(0.0);
            }
            if let Some(v) = m.get("lp_per_call_pivots").and_then(|v| v.as_u64()) {
                self.lp_per_call_pivots = v as usize;
            }
            if let Some(v) = m.get("lp_total_pivots").and_then(|v| v.as_u64()) {
                self.lp_total_pivots = v as usize;
            }
            if let Some(v) = m.get("use_policy").and_then(|v| v.as_bool()) {
                self.use_policy = v;
            }
            if let Some(v) = m.get("use_warmstart").and_then(|v| v.as_bool()) {
                self.use_warmstart = v;
            }
            if let Some(v) = m.get("use_mpc").and_then(|v| v.as_bool()) {
                self.use_mpc = v;
            }
            if let Some(v) = m.get("mpc_horizon").and_then(|v| v.as_u64()) {
                self.mpc_horizon = v as usize;
            }
            if let Some(v) = m.get("mpc_pivot_budget").and_then(|v| v.as_u64()) {
                self.mpc_pivot_budget = v as usize;
            }
            if let Some(v) = m.get("use_dw").and_then(|v| v.as_bool()) {
                self.use_dw = v;
            }
            if let Some(v) = m.get("dw_iters").and_then(|v| v.as_u64()) {
                self.dw_iters = v as usize;
            }
            if let Some(v) = m.get("dw_max_lines").and_then(|v| v.as_u64()) {
                self.dw_max_lines = v as usize;
            }
            if let Some(v) = m.get("dw_max_cols_per_batt").and_then(|v| v.as_u64()) {
                self.dw_max_cols_per_batt = v as usize;
            }
            if let Some(v) = m.get("dw_pivot_budget_per_solve").and_then(|v| v.as_u64()) {
                self.dw_pivot_budget_per_solve = v as usize;
            }
            if let Some(v) = m.get("dw_total_pivot_budget").and_then(|v| v.as_u64()) {
                self.dw_total_pivot_budget = v as usize;
            }
            if let Some(v) = m.get("use_dw_prescreen").and_then(|v| v.as_bool()) {
                self.use_dw_prescreen = v;
            }
            if let Some(v) = m.get("use_lns").and_then(|v| v.as_bool()) {
                self.use_lns = v;
            }
            if let Some(v) = m.get("lns_cg_iters").and_then(|v| v.as_u64()) {
                self.lns_cg_iters = v as usize;
            }
            if let Some(v) = m.get("lns_cg_column_limit").and_then(|v| v.as_u64()) {
                self.lns_cg_column_limit = (v as usize).max(2);
            }
            if let Some(v) = m.get("lns_max_lines").and_then(|v| v.as_u64()) {
                self.lns_max_lines = (v as usize).max(1);
            }
            if let Some(v) = m.get("lns_lp_pivots_total").and_then(|v| v.as_u64()) {
                self.lns_lp_pivots_total = v as usize;
            }
            if let Some(v) = m.get("use_pivot_reserve").and_then(|v| v.as_bool()) {
                self.use_pivot_reserve = v;
            }
            if let Some(v) = m.get("lp_max_lines").and_then(|v| v.as_u64()) {
                self.lp_max_lines = (v as usize).max(1);
            }
            if let Some(v) = m.get("use_parallel_dp").and_then(|v| v.as_bool()) {
                self.use_parallel_dp = v;
            }
            if let Some(v) = m.get("use_sdp").and_then(|v| v.as_bool()) {
                self.use_sdp = v;
            }
            if let Some(v) = m.get("sdp_k").and_then(|v| v.as_u64()) {
                self.sdp_k = (v as usize).max(2);
            }
            if let Some(v) = m.get("use_ldd_proximal").and_then(|v| v.as_bool()) {
                self.use_ldd_proximal = v;
            }
            if let Some(v) = m.get("ldd_momentum").and_then(|v| v.as_f64()) {
                self.ldd_momentum = v.clamp(0.0, 1.0);
            }
            if let Some(v) = m.get("ldd_clip_fraction").and_then(|v| v.as_f64()) {
                self.ldd_clip_fraction = v.clamp(0.01, 1.0);
            }
            if let Some(v) = m.get("use_tail_quadrature").and_then(|v| v.as_bool()) {
                self.use_tail_quadrature = v;
            }
            if let Some(v) = m.get("use_congestion_safety").and_then(|v| v.as_bool()) {
                self.use_congestion_safety = v;
            }
            if let Some(v) = m
                .get("congestion_safety_threshold")
                .and_then(|v| v.as_f64())
            {
                self.congestion_safety_threshold = v.clamp(0.01, 0.99);
            }
            if let Some(v) = m.get("congestion_safety_strength").and_then(|v| v.as_f64()) {
                self.congestion_safety_strength = v.max(0.0);
            }
        }
    }
    ...
    fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
        STATE.with(|s| -> Result<Vec<f64>> {
            ...
            let mut congestion_surcharge = vec![0.0_f64; num_b];
            let mut cs_batt_sur_nonzero = 0usize;
            let state_eff = if hp.use_congestion_safety {
                for l in 0..flows_base.len() {
                    let limit = challenge.network.flow_limits[l];
                    if limit <= 1e-6 { continue; }
                    let util = flows_base[l].abs() / limit;
                    if util > hp.congestion_safety_threshold {
                        let denom = (1.0 - hp.congestion_safety_threshold).max(1e-9);
                        let excess = (util - hp.congestion_safety_threshold) / denom;
                        let mu_l = excess * hp.congestion_safety_strength * flows_base[l].signum();
                        for &(b, impact) in &cache.ptdf_sparse[l] {
                            if b < num_b { congestion_surcharge[b] += mu_l * impact; }
                        }
                    }
                }
                cs_batt_sur_nonzero = congestion_surcharge.iter().filter(|&&s| s.abs() > 1e-12).count();
                let mut eff = state.clone();
                for b in 0..num_b {
                    let node = cache.batt_nodes[b];
                    if node < eff.rt_prices.len() { eff.rt_prices[node] -= congestion_surcharge[b]; }
                }
                Some(eff)
            } else {
                None
            ...
            // Warm-start: compute DP unconstrained optimal actions per battery
            ...
            // Baseline dispatch: KKT (with ASCA comparison inside) or pure ASCA
            ...
            let base_actions = if hp.use_congestion_safety && cs_batt_sur_nonzero > 0 {
                // Explicit surcharge candidate: keep existing KKT/LP/MPC/Policy/LNS paths on the original
                // state to avoid double-counting, but force one local ASCA/repair candidate to see the
                // PTDF-derived price signal so the experiment cannot silently no-op.
                let eff_state = state_eff.as_ref().expect("congestion safety state missing");
                let mut cs_act: Vec<f64> = if hp.use_warmstart {
                    (0..num_b)
                        .map(|b| optimal_unconstrained_action(challenge, eff_state, cache, b))
                        .collect()
                } else {
                    vec![0.0; num_b]
                };
                run_asca(challenge, eff_state, cache, hp, &flows_base, &mut cs_act);
                if hp.dual_iters > 0 {
                    let asca_actions = cs_act.clone();
                    let dual_actions = run_dual_ascent(challenge, eff_state, cache, hp, &flows_base, &asca_actions);
                    let profit_asca: f64 = (0..num_b)
                        .map(|b| eval_profit(challenge, eff_state, cache, b, asca_actions[b]))
                        .sum();
                    let profit_dual: f64 = (0..num_b)
                        .map(|b| eval_profit(challenge, eff_state, cache, b, dual_actions[b]))
                        .sum();
                    if profit_dual >= profit_asca { cs_act = dual_actions; }
                } else if hp.max_admm_iters > 0 {
                    if !run_admm_dispatch(challenge, eff_state, cache, hp, &flows_base, &mut cs_act) {
                        run_deflator(challenge, eff_state, cache, hp, &flows_base, &mut cs_act);
                    }
                } else {
                    run_deflator(challenge, eff_state, cache, hp, &flows_base, &mut cs_act);
                }
                let cs_p: f64 = (0..num_b).map(|b| eval_profit(challenge, state, cache, b, cs_act[b])).sum();
                let base_p: f64 = (0..num_b).map(|b| eval_profit(challenge, state, cache, b, base_actions[b])).sum();
                let selected = cs_p >= base_p;
                if selected { cs_act } else { base_actions }
            } else {
                base_actions
            };

            // Soft-constrained joint LP: jointly optimizes all batteries with soft line limits
            // with congestion gate and pivot budget to keep cycle ≤ target
            let actions = if hp.use_lp {
                // Congestion gate: skip LP if no line is near its limit
                ...
                // Dynamic pivot budget or static per-call limit
                ...
                // Global pivot budget check (only for non-reserve mode; reserve mode has its own cap)
            ...
            // MPC 2-step receding-horizon LP: short-horizon joint optimisation over t and t+1.
            // Runs as an additional candidate; best is selected by total profit.
            ...
            // CMA-ES policy dispatch path: if enabled AND weights loaded, use policy
            let actions = if hp.use_policy {
                if let Some(ref weights) = inner.policy_weights {
                    ...
                    // No weights yet — train at t==0, otherwise fall back to baseline
            ...
            // ALNS-DW per-step column-generation repair: if congestion detected,
            // re-optimise actions via small column-generation master LP.
            let actions = if hp.use_lns {
                if let Some(mut lns_act) = lns_dw_per_step(challenge, state, cache, hp, &flows_base, &actions) {
                    ...
                    let base_p: f64 = (0..challenge.num_batteries)
                        .map(|b| eval_profit(challenge, state, cache, b, actions[b])).sum();
                    if lns_p > base_p { lns_act } else { actions }
                } else {
                    actions
                }
            } else {
            ...
            let mut baseline_actions = actions;
            // Final safety pass: candidate selection paths compare profit under their own objective,
            // so always restore network feasibility under the real state before returning.
            run_deflator(challenge, state, cache, hp, &flows_base, &mut baseline_actions);
            ...
            Ok(if is_last_step {
                ...
                // V3.2 — Inline feasibility check. compute_profit doesn't penalize
                // infeasibility, so the bench would clip violating actions and
                // crash Q. Verify ourselves before accepting refined.
    ...
    // Build DP tables with given dual prices mu[t][l].
    // p_eff(t,b) = DA(t,b) + expected_premiums[t][b] - Σ_l mu[t][l]*PTDF(l,b)
    fn build_dp_with_mu(
        challenge: &Challenge,
        hp: &TrackHp,
        batt_nodes: &[usize],
        ...
        for b in 0..num_b {
            ...
            for t in (0..num_t).rev() {
                ...
                // LDD adjustment: penalise flows that violate congestion constraints
                let mu_adjust: f64 = b_to_lines[b]
                    .iter()
                ...
                for i in 0..soc_levels {
                    ...
                    for j in 0..=action_grid {
                        ...
                        let idx_f =
                            (next_soc - bat.soc_min_mwh) / soc_span * ((soc_levels - 1) as f64);
                        ...
                        let v_next =
                            dp[b][t + 1][idx0c] * (1.0 - frac) + dp[b][t + 1][idx1c] * frac;
    ...
    // Simulate forward trajectories using DP greedy policy, return total flows per timestep.
    fn ldd_simulate_flows(
        ...
        for t in 0..num_t {
            ...
            for b in 0..num_b {
                ...
                for j in 0..=sim_pts {
                    let u = if span > 0.0 {
                        u_min + span * (j as f64) / (sim_pts as f64)
                    } else {
                        u_min
                    };
                    let abs_u = u.abs();
    ...
    /// Marginal DP value at (battery b, time t_next, SoC soc) via linear interpolation.
    /// Returns d(V)/d(soc) — the shadow price of one extra MWh in the battery.
    #[inline]
    fn dp_lambda(
        dp_b: &[Vec<f64>],
        t_next: usize,
    ...
    /// Compute the unconstrained optimal action for battery b using analytic
    /// quadratic branches on the profit function (revenue - tx - degradation + V_next).
    /// Ignores network congestion — only respects SOC-based bounds.
    #[inline]
    fn optimal_unconstrained_action(
        ...
        let rt = if node < state.rt_prices.len() {
        ...
        if u_min < -1e-12 {
            ...
            if u_min < hi {
                // Optimal charge (as negative u): maximize revenue + V_next - degradation
                // d²profit/du² ≈ -two_deg (concave), so analytical peak exists
                let b_c = dt * (lambda * bat.efficiency_charge - rt - 0.25);
                if two_deg > 1e-30 {
                    let raw_charge = (-b_c) / two_deg; // optimal |u| for charging
                ...
                // Boundary: minimum charge
                {
                    let v_lo = eval_profit(challenge, state, ca, b, u_min);
                    if v_lo.is_finite() && v_lo > best_val {
                        best_val = v_lo;
                        best_u = u_min;
                    }
                }
        ...
        if u_max > 1e-12 {
            ...
            if lo < u_max {
                ...
                // Boundary: maximum discharge
                {
                    let v_hi = eval_profit(challenge, state, ca, b, u_max);
                    if v_hi.is_finite() && v_hi > best_val {
                        best_val = v_hi;
                        best_u = u_max;
                    }
                }
    ...
    // ============================================================================
    // DW prescreen: identify binding lines via zero-dual greedy simulation
    // ============================================================================
    // Simulate a full trajectory using a DP built with mu=0, then collect per-
    // timestep flows. Returns the line indices that are most violated, sorted by
    // maximum violation amount descending, capped at `max_lines`.
    ...
    // DW-lite dual price discovery
    // ============================================================================
    // Small-scale Dantzig-Wolfe column generation to produce accurate congestion
    // dual prices. Uses only top-K congested lines, limited columns per battery,
    // and bounded LP pivots. Returns mu[time_step][line] for injection into DP.
    // When `preselected_lines` is Some, uses those lines instead of the default
    // exogenous-utilization ranking.
    fn build_dw_dual_prices(
        ...
        // 1. Select top congested lines: either from prescreen (binding lines in
        // zero-dual greedy simulation) or from exogenous flow utilization baseline
        let active_lines: Vec<usize> = if let Some(preselected) = preselected_lines {
            // Prescreen mode: only use lines that were flagged as binding
            let k_active = hp.dw_max_lines.min(preselected.len());
            preselected.iter().take(k_active).copied().collect()
        } else {
            // Baseline: exogenous utilization ranking
        ...
        // Precompute PTDF contribution for each battery to active lines
        let ptdf_batt: Vec<Vec<f64>> = (0..num_b)
            .map(|b| {
                (0..n_active)
                    .map(|ai| {
                        let l = active_lines[ai];
                        let val = b_to_lines[b]
                            .iter()
                            .find(|&&(ll, _)| ll == l)
                            .map(|&(_, coef)| coef)
                            .unwrap_or(0.0);
        ...
        // Precompute exogenous flows for all timesteps
        let exo_flows_all: Vec<Vec<f64>> = (0..num_t)
            .map(|t| {
                challenge
                    .network
                    .compute_flows(&challenge.exogenous_injections[t])
        ...
        // 2. Build initial columns: for each battery, generate a greedy schedule using DA prices
        ...
        for b in 0..num_b {
            ...
            for t in 0..num_t {
                ...
                // Greedy: discharge if price high, charge if price low
        ...
        // Precompute per-column profile and profit for the master LP
        let n_constraints = n_active * num_t;
        let M_penalty: f64 = 1e7; // big M penalty for slack variables
        ...
        'dw_outer: for _dw_iter in 0..hp.dw_iters {
            ...
            // n_slacks = n_constraints for soft line-flow constraints
            let n_slacks = n_constraints;
            let total_vars = n_cols + n_slacks;

            // Compute column profits (using DA prices, ignore network for profit calc)
            ...
            // Slack variables get large negative cost (penalize violation)
            for s in 0..n_slacks {
                c_lp[n_cols + s] = -M_penalty;
            }

            // Build master LP matrix: row = (active_line, timestep) pair
            ...
            for ai in 0..n_active {
                ...
                for t in 0..num_t {
                    ...
                    // For each column, add PTDF * action contribution
                    for c in 0..n_cols {
                    ...
                    // Slack variable coefficient: -1 (a*x <= b, slack added to LHS: A*x - s <= b => A*x <= b+s)
                    // Wait: A*x <= (limit - exo). With slack: A*x + (-s) <= limit - exo, s >= 0.
                    // So coefficient of slack in the constraint row is -1.
                    a_lp[row][n_cols + row] = -1.0;
                }
            }

            // Solve master LP with dual extraction
            ...
            // Extract duals and map back to full line set
            let Some(duals) = duals_opt else {
                break 'dw_outer;
            };
            let Some(_x_sol) = sol else {
                break 'dw_outer;
            };
            ...
            for ai in 0..n_active {
                ...
                for t in 0..num_t {
                    ...
                    if row < duals.len() {
                        let d = duals[row];
                        // Guard: skip duals that are unrealistically large
                        if d.abs() > 1e6 {
                            dual_valid = false;
                            continue;
                        }
            ...
            for t in 0..num_t {
                for ai in 0..n_active {
                    ...
                    if (mu[t][l] - mu_new[t][l]).abs() > 1e-3 {
                        converged = true; // not converged, need to check all
                        converged = false;
                        break;
                    }
                }
            ...
            // 3b. Column generation: for each battery, try to find improved column via DP with current duals
            for b in 0..num_b {
                let bat = &challenge.batteries[b];
                let node = batt_nodes[b];
                let eta_c = bat.efficiency_charge;
                ...
                // Compute effective price at each timestep: DA + congestion dual adjustment
                ...
                for t in 0..num_t {
                    ...
                    let cong_adj: f64 = (0..n_active)
                        .map(|ai| mu[t][active_lines[ai]] * ptdf_batt[b][ai])
                        .sum();
                    eff_prices[t] = da - cong_adj;
                }

                // Generate candidate column: greedy schedule with effective prices
                // Discharge when eff_price is high, charge when low
                ...
                // Check if this candidate is genuinely different from existing columns for this battery
                let is_duplicate = columns
                    .iter()
                    .zip(col_batt_idx.iter())
                    .filter(|(_, &idx)| idx == b)
                    .any(|(col, _)| {
                        col.iter()
                            .zip(candidate_col.iter())
                            .all(|(a, c)| (a - c).abs() < 1e-4)
                    });

                if !is_duplicate {
                    // Check columns-per-battery limit
                    columns.push(candidate_col);
                    col_batt_idx.push(b);

                    let new_count = col_batt_idx.iter().filter(|&&idx| idx == b).count();
                    if new_count > hp.dw_max_cols_per_batt {
                        // Remove oldest (worst) column for this battery
        ...
        // Safety: validate all dual values are finite and bounded
        for t in 0..num_t {
            for l in 0..num_l {
    ...
    /// Compute total profit of a schedule (Vec<f64> of actions over time) for battery b.
    fn compute_schedule_profit(challenge: &Challenge, b: usize, schedule: &[f64]) -> f64 {
        let bat = &challenge.batteries[b];
        let node = b; // We pass batt_nodes from caller; here use battery node for DA prices
        let dt = 0.25_f64;
        ...
        for t in 0..schedule.len().min(challenge.num_steps) {
            ...
            let da_price =
                if challenge.batteries[b].node < challenge.market.day_ahead_prices[t].len() {
    ...
    /// Compute dual contribution of a column: sum over active (line, time) of mu[t][l] * ptdf * action[t]
    #[allow(dead_code)]
    ...
    // ALNS-DW per-step column-generation repair for congested timesteps
    // ============================================================================
    // At each step of policy_entry, if line flows are congested, build a small
    // column-generation master LP that jointly re-optimises all battery actions
    // while respecting the most binding line limits. Replaces the uniform
    // deflator with selective, spatially-aware coordination.
    //
    // Master LP structure:
    //   Variables: w[b][j] = column weight for battery b, column j
    //   Convexity: sum_j w[b][j] = 1  (equality enforced as pair of <=)
    //   Flow:      sum_{b,j} PTDF[l][b_node] * action[b][j] * w[b][j] <= limit - exo_flow[l]
    //   Objective: max sum_{b,j} profit(b,j) * w[b][j]
    //
    // Columns seeded: current action, zero, min-bound, max-bound, and fractions.
    // New columns generated via effective price = RT - dual_price * PTDF.
    fn lns_dw_per_step(
        challenge: &Challenge,
        state: &State,
        ca: &TitanCache,
        ...
        // Check for actual congestion — skip if no line is near limit
        ...
        if max_util < hp.kkt_cong_threshold {
            return None; // Not congested enough
        }

        // 1. Select most congested lines for the restricted master
        ...
        for l in 0..num_l {
            ...
            // Compute current flow: base + battery contributions
        ...
        // Also include lines near limit even if not violated (to prevent future violations)
        for l in 0..num_l {
            let limit = challenge.network.flow_limits[l];
            if limit < 1e-6 {
                continue;
        ...
        let line_set: Vec<usize> = violations
            .into_iter()
            .take(k_lines)
            .map(|(l, _)| l)
            .collect();
        ...
        // 2. Seed columns for each battery: current action, zero, bounds, fractions
        ...
        for b in 0..num_b {
            ...
            // Limit columns per battery
            if unique.len() > hp.lns_cg_column_limit {
                // Keep first (current), zero, lo, hi, truncate rest
                ...
                for &a in &unique {
                    if keep.is_empty()
                        || (!has_zero && a.abs() < 1e-6)
                        || (!has_lo && (a - lo).abs() < 1e-6)
                        || (!has_hi && (a - hi).abs() < 1e-6)
                    {
                        if !has_zero && a.abs() < 1e-6 {
                            has_zero = true;
                        }
                        if !has_lo && (a - lo).abs() < 1e-6 {
                            has_lo = true;
                        }
                        if !has_hi && (a - hi).abs() < 1e-6 {
                            has_hi = true;
                        }
                        keep.push(a);
                    }
                    if keep.len() >= hp.lns_cg_column_limit {
                        break;
                    }
                }
                unique = keep;
        ...
        for _cg_iter in 0..hp.lns_cg_iters {
            if pivot_budget < 50 {
                break;
            } // Not enough pivots for a meaningful solve
            ...
            // Constraints: convexity (=1 enforced as two <=: sum<=1 and -sum<=-1), plus flow
            // Each convexity: 2 constraints per battery (<=1 and =>1 which is -sum<=-1)
            ...
            // Fill convexity constraints and objective
            let mut col_offset = 0usize;
            for b in 0..num_b {
                // Row 2b: sum_j w[b][j] <= 1
                for j in 0..col_pools[b].len() {
                    a_mat[2 * b][col_offset + j] = 1.0;
                }
                b_vec[2 * b] = 1.0;

                // Row 2b+1: -sum_j w[b][j] <= -1  (i.e. sum >= 1)
                for j in 0..col_pools[b].len() {
                    a_mat[2 * b + 1][col_offset + j] = -1.0;
                }
                // RHS: -1, but lp_solve requires b >= 0. Transform: multiply by -1, flip inequality.
                // -sum_j w <= -1  is equivalent to  sum_j w >= 1  which is  -sum_j w <= -1.
                // Since lp_solve requires b_i >= 0, we need a different approach.
                // Instead enforce: sum_j w[b][j] <= 1 AND we ensure a feasible solution exists
                // by seeding at least one column. We'll accept inequality convexity as a
                // relaxation; the LP will naturally push weights to 1 for high-profit columns.
                // For true equality, we'd need a two-phase LP. Let's use: sum <= 1 (enough for relaxation).
                // The dual-based column gen will fill in the best action.
                a_mat[2 * b + 1] = vec![0.0_f64; n_cols_total]; // zero out - no lower bound
                b_vec[2 * b + 1] = 0.0;

                // Objective coefficient: profit for each column
                for j in 0..col_pools[b].len() {
                    let u = col_pools[b][j];
                    // Use eval_profit_with_price for the profit at this action and current RT price
                    let node = ca.batt_nodes[b];
                    let rt = if node < state.rt_prices.len() {
                        state.rt_prices[node]
                    } else {
            ...
            // Flow constraints for congested lines (both directions)
            for (i, &l) in line_set.iter().enumerate() {
                let limit = challenge.network.flow_limits[l];
                let exo = flows_base[l];

                // Positive direction: PTDF * action <= limit - exo
                let row_p = 2 * num_b + 2 * i;
                let mut col_idx = 0usize;
                for b in 0..num_b {
                    let ptdf_coef = ca.b_to_lines[b]
                        .iter()
                        .find(|&&(ll, _)| ll == l)
                        .map(|&(_, coef)| coef)
                        .unwrap_or(0.0);
                ...
                // Negative direction: -PTDF * action <= limit + exo
            ...
            let (primal, duals_opt, pivots_used) =
                super::lp::lp_solve_with_duals(n_cols_total, m, &c_vec, &a_mat, &b_vec, per_solve);
            pivot_budget = pivot_budget.saturating_sub(pivots_used);

            let Some(primal_x) = primal else {
                break;
            };

            // Extract actions: for each battery, pick column with highest weight
            ...
            // Column generation: use duals to create new candidate columns
            let Some(duals) = duals_opt else {
                break;
            };

            // Line duals start at index 2*num_b (after convexity rows), both pos and neg rows
            let line_dual_offset = 2 * num_b;
            if line_dual_offset >= duals.len() {
                break;
            }

            let mut added_any = false;
            for b in 0..num_b {
                if col_pools[b].len() >= hp.lns_cg_column_limit {
                    continue;
                }
                ...
                // Effective price: RT - sum(lin duals * PTDF)
                // Use only the positive-direction duals (even-indexed flow rows)
                let mut cong_adj = 0.0_f64;
                for (i, &l) in line_set.iter().enumerate() {
                    let dual_row = line_dual_offset + 2 * i; // positive-direction row
                    if dual_row >= duals.len() {
                        continue;
                    }
                ...
                if !eff_price.is_finite() {
                    continue;
                }

                // Find best action at effective price using ternary search
                let (lo, hi) = state.action_bounds[b];
                if hi - lo < 1e-12 {
                    continue;
                }
                ...
                if lo < 0.0 {
                    let (u, v) =
                        ternary_search(&eval_action, lo, 0.0_f64.min(hi), hp.ternary_iters);
                    if v > best_cand_val {
                        best_cand_val = v;
                        candidate_u = u;
                    }
                ...
                if hi > 0.0 {
                    let (u, v) =
                        ternary_search(&eval_action, 0.0_f64.max(lo), hi, hp.ternary_iters);
                    if v > best_cand_val {
                        candidate_u = u;
                    }
                }

                // Check for reduced cost > tolerance
            ...
            if !added_any {
                break;
            } // No new columns, converged
        }

        // 4. Validate and return best actions
    ...
    /// Compute the dynamic LP pivot budget for the current step using pivot budget lending.
    /// Borrows unused pivot budget from earlier easy steps to give congested steps more pivots.
    /// The total pivot count never exceeds hp.lp_total_pivots.
    ...
    fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> TitanCache {
            ...
            for t in 0..num_t {
                ...
                for l in 0..num_l {
                    ...
                    if ratio > hp.lmp_threshold {
                        let proba = ((ratio - hp.lmp_threshold)
                            / (1.0 - hp.lmp_threshold).max(1e-6))
                        .clamp(0.0, 1.0);
        ...
        // KKT path: avoid double-penalization. KKT computes its own shadow prices
        // at runtime, so DP must not pre-embed any static congestion signal.
        if hp.use_kkt {
        ...
        // Lagrangian Dual Decomposition: iteratively build DP with augmented prices
        // that encode congestion costs, converging to a network-consistent policy.
        // DW-lite takes precedence when enabled: produces congestion-aware duals via
        // small-scale column generation, replacing LDD for superior DP guidance.
        // DW prescreen (use_dw_prescreen): simulate with zero-dual DP to identify
        // truly binding lines, then restrict DW master to only those lines.
        let dp = if hp.use_dw && num_l > 0 {
            let prescreen_lines: Vec<usize> = if hp.use_dw_prescreen {
                // Build temporary zero-dual DP, simulate, prescreen binding lines
                let zero_mu = vec![vec![0.0_f64; num_l]; num_t];
                let dp0 = build_dp_parallel_or_serial(
                    challenge,
                    hp,
                    &batt_nodes,
                    &expected_premiums,
                ...
                let flows_all =
                    ldd_simulate_flows(challenge, state, &dp0, &batt_nodes, &ptdf_sparse);
                prescreen_binding_lines(challenge, &flows_all, hp.dw_max_lines)
            } else {
                Vec::new()
            };
            let ps: Option<&[usize]> = if prescreen_lines.is_empty() {
                None
            } else {
                Some(&prescreen_lines)
            };
            let mu_dw = build_dw_dual_prices(challenge, state, hp, &batt_nodes, &b_to_lines, ps);
            build_dp_parallel_or_serial(
                challenge,
                hp,
                &batt_nodes,
                &expected_premiums,
            ...
            let mut mu = vec![vec![0.0_f64; num_l]; num_t];
            let mut dp = build_dp_parallel_or_serial(
                challenge,
                hp,
                &batt_nodes,
                &expected_premiums,
            ...
            if hp.use_ldd_proximal {
                // Proximal-scaled subgradient with momentum and clipping
                let mut prev_average = vec![vec![0.0_f64; num_l]; num_t];
                let mut fallback = false;

                for ldd_iter in 0..hp.ldd_iters {
                    let flows_all =
                        ldd_simulate_flows(challenge, state, &dp, &batt_nodes, &ptdf_sparse);
                    let step_base = hp.ldd_step_size;

                    // Compute per-timestep L2 violation norm for adaptive scaling
                    ...
                    // Update duals with proximal scaling, momentum, and clipping
                    for t in 0..num_t {
                        ...
                        for &l in &active_lines_ldd {
                            ...
                            if abs_f > limit {
                                ...
                                let eff_delta = if ldd_iter == 0 {
                                    ...
                                    let avg_delta = hp.ldd_momentum * prev_average[t][l]
                                        + (1.0 - hp.ldd_momentum) * clamped_delta;
                    ...
                    dp = build_dp_parallel_or_serial(
                        challenge,
                        hp,
                        &batt_nodes,
                        &expected_premiums,
                    ...
                    // Safety check: after proximal update, verify effective prices aren't all negative
                    if ldd_iter == 0 {
                        let mut any_positive = 0usize;
                        for b in 0..num_b.min(20) {
                            let node = batt_nodes[b];
                            let da = if node < challenge.market.day_ahead_prices.len()
                                && 0 < challenge.market.day_ahead_prices[node].len()
                            {
                                challenge.market.day_ahead_prices[node][0]
                            } else {
                                0.0
                            };
                            let cong_adj: f64 = b_to_lines[b]
                                .iter()
                                .map(|&(l, impact)| {
                                    if l < mu[0].len() {
                                        mu[0][l] * impact.abs()
                ...
                // If proximal fallback triggered, revert to classical LDD
                if fallback {
                ...
                // Classical fixed-step LDD (baseline)
                for ldd_iter in 0..hp.ldd_iters {
                    let flows_all =
                        ldd_simulate_flows(challenge, state, &dp, &batt_nodes, &ptdf_sparse);
            ...
            // Adaptive-Premium DP: one cheap simulation round updates premiums with
            // policy-aware congestion signals before building the full-res DP.
            // Closes the static-premium underestimation that keeps Q at deflator level.
            let mut hp_cheap = hp.clone();
            hp_cheap.soc_levels = 31;
            hp_cheap.action_grid = 15;
            let mu_zero = vec![vec![0.0_f64; num_l]; num_t];
            let dp_low = build_dp_parallel_or_serial(
                challenge,
                &hp_cheap,
                &batt_nodes,
                &expected_premiums,
            ...
            let flows_all =
                ldd_simulate_flows(challenge, state, &dp_low, &batt_nodes, &ptdf_sparse);
            for t in 0..num_t {
                for l in 0..num_l {
                    let limit = challenge.network.flow_limits[l];
                    if limit <= 1e-6 {
    ...
    fn run_dual_ascent(
        ...
        for k in 1..=hp.dual_iters {
            ...
            for b in 0..num_b {
                let nu_dot_ptdf: f64 = ca.b_to_lines[b]
                    .iter()
                ...
                let mut best_u = actions[b];
                let mut best_v =
                    eval_profit(challenge, state, ca, b, best_u) - nu_dot_ptdf * best_u;
    ...
    // ===== KKT EXOGENOUS-LMP DISPATCH (i131) =====
    //
    // Key fix vs i130: use EXOGENOUS flows (flows_base) for shadow price estimation,
    // not unconstrained-action flows.  Unconstrained actions produce 200-400% violations
    // → enormous μ_l → negative p_eff → near-zero dispatch.
    // Exogenous flows reflect structural congestion independent of batteries, giving
    // moderate, stable shadow prices that tilt effective prices toward global optimum.
    //
    // Pipeline:
    //  1. Shadow prices μ_l from exogenous flows_base (structural congestion)
    //  2. Effective nodal price p_eff[b] = RT[b_node] − Σ_l μ_l × PTDF(l,b)
    //  3. Re-optimise each battery at p_eff (ternary search, no network bounds)
    //  4. Run deflator to restore feasibility
    //  5. Compare KKT vs ASCA, return better
    ...
    // One KKT dispatch + deflator pass. mu_override provides shadow prices;
    // if empty, prices are computed from flows_base (exogenous-only, pass 1).
    // Returns (feasible actions, total profit).
    fn kkt_pass(
        challenge: &Challenge,
        state: &State,
        ca: &TitanCache,
        ...
        let mu: Vec<f64> = if mu_override.is_empty() {
            // Pass 1: exogenous-only shadow prices
            ...
            for l in 0..num_l {
                ...
                if util > hp.kkt_cong_threshold {
                    let excess_frac = ((util - hp.kkt_cong_threshold)
                        / (1.0 - hp.kkt_cong_threshold).max(1e-6))
                    .min(1.0);
    ...
    pub fn kkt_policy(
        ...
        let avg_price: f64 = if num_b > 0 {
            (0..num_b)
                .map(|b| {
                    ...
                    if n < state.rt_prices.len() {
                        state.rt_prices[n].abs()
                    } else {
                        0.0
                    }
                })
        ...
        // Pass 1: exogenous-only shadow prices → KKT actions → deflator
        let (actions1, profit1) = kkt_pass(challenge, state, ca, hp, flows_base, &[], price_scale);

        // Compute post-deflator battery flows analytically (no extra compute_flows)
        ...
        // Implied duals from actual post-deflator flows (battery-aware congestion signal)
        ...
        for l in 0..num_l {
            ...
            if util > cong_threshold2 {
                let excess_frac =
                    ((util - cong_threshold2) / (1.0 - cong_threshold2).max(1e-6)).min(1.0);
                mu2[l] = excess_frac * price_scale * total_flow1[l].signum();
            }
        }

        // Pass 2: battery-aware shadow prices → re-optimise → deflator
        let (actions2, profit2) = kkt_pass(challenge, state, ca, hp, flows_base, &mu2, price_scale);

        // Accept pass 2 only if it improves profit (guard: ≥98% of pass 1 to avoid collapse)
        ...
        // ASCA baseline for comparison — warm-start from DP unconstrained optimum if available
        let mut actions_asca = if warm_init.len() == num_b {
            warm_init.to_vec()
        } else {
            vec![0.0_f64; num_b]
        };
        ...
        if hp.dual_iters > 0 {
            ...
            let pa: f64 = (0..num_b)
                ...
                .sum();
            let pd: f64 = (0..num_b)
                .map(|b| eval_profit(challenge, state, ca, b, dual[b]))
                .sum();
            if pd > pa {
                actions_asca = dual;
            }
    ...
    // Box-constrained ADMM: find joint-feasible dispatch replacing global deflator.
    // Variables: u_b per battery, s_l per line (total flow, clamped to [-limit, limit]).
    // Returns true if converged or actual flows feasible; caller falls back to deflator if false.
    fn run_admm_dispatch(
        ...
        for l in 0..num_l {
            ...
            if (flows_base[l] + bat_f).abs() > limit {
                any_violated = true;
                break;
            }
        }
        ...
        // Dual variables y_l, auxiliary s_l (total flow, feasible by construction)
        let mut y = vec![0.0_f64; num_l];
        let mut s: Vec<f64> = (0..num_l)
            .map(|l| {
                let limit = (challenge.network.flow_limits[l] - hp.flow_margin).max(0.0);
                let mut bat_f = 0.0_f64;
                for &(b, imp) in &ca.ptdf_sparse[l] {
        ...
        for _iter in 0..hp.max_admm_iters {
            ...
            // Recompute bat_flow from current actions
            ...
            // u-update: per-battery 1D grid search (Gauss-Seidel style, updating bat_flow inline)
            for b in 0..num_b {
                let (lo, hi) = state.action_bounds[b];
                if (hi - lo).abs() < 1e-12 {
                    continue;
                ...
                // offset_l = s[l] - flows_base[l] + y[l]/rho - (bat_flow[l] - a_lb * actions[b])
                let offsets: Vec<(f64, f64)> = lines_b
                    .iter()
                    .map(|&(l, a_lb)| {
                        let off =
                            s[l] - flows_base[l] + y[l] / rho - (bat_flow[l] - a_lb * actions[b]);
                        (off, a_lb)
                    })
                    .collect();
                ...
                let delta = best_u - actions[b];
                for &(l, a_lb) in lines_b {
                    bat_flow[l] += a_lb * delta;
                }
                actions[b] = best_u;
            }

            // s-update: project onto feasible box [-limit, limit]
            for l in 0..num_l {
                let limit = (challenge.network.flow_limits[l] - hp.flow_margin).max(0.0);
                s[l] = (bat_flow[l] + flows_base[l] - y[l] / rho).clamp(-limit, limit);
            }

            // y-update + convergence check
            ...
            let max_du = (0..num_b)
                ...
                .fold(0.0_f64, f64::max);
            if max_resid < tol && max_du < tol {
                return true;
            }
        }

        // Accept if actual flows are feasible even without full convergence
        let mut bat_flow_final = vec![0.0_f64; num_l];
        for l in 0..num_l {
            for &(b, imp) in &ca.ptdf_sparse[l] {
                bat_flow_final[l] += imp * actions[b];
            }
        }
        for l in 0..num_l {
            let limit = (challenge.network.flow_limits[l] - hp.flow_margin).max(0.0);
            if (bat_flow_final[l] + flows_base[l]).abs() > limit + 1.0 {
                return false;
            }
        }
        true
    ...
    /// Select the top-K most congested lines by base-flow utilization ratio.
    /// Returns all lines when K >= num_lines (no filtering). Skips lines with
    /// near-zero limit to avoid division overflow.
    ...
    // Soft-constrained joint LP dispatch: jointly optimizes all battery actions with
    // soft line-flow constraints. Always feasible (violation slacks absorb congestion).
    // Includes DP future-value slope in objective for intertemporal optimality.
    fn joint_lp_dispatch(
        ...
        // Top-K line filtering: rank lines by base-flow utilization, keep only the K most-loaded
        let active_lines = select_active_lines(challenge, flows_base, lp_hp.lp_max_lines);
        let n_active = active_lines.len();

        // Variables: [d_0..d_nb (discharge), c_0..c_nb (charge mag), viol_0..viol_nk (flow slack)]
        let n = 2 * num_b + n_active;
        // Constraints: [ub_d, ub_c, soc_min, soc_max per battery] + [flow_pos, flow_neg per active line]
        ...
        // Violation penalty (maximize = minimize violations)
        for ai in 0..n_active {
        ...
        for b in 0..num_b {
            ...
            // DP future-value slope at current SoC
            let soc_levels = ca.dp[b][0].len();
            ...
            // Objective: (rt - kappa)*dt + future SoC change value
            c_vec[b] = (rt - 0.25) * dt - dv * dt / eta_d; // discharge
            c_vec[num_b + b] = -(rt + 0.25) * dt + dv * eta_c * dt; // charge

            let r = 4 * b;
            // d_b <= u_max (discharge upper bound, RHS >= 0)
            a_mat[r][b] = 1.0;
            b_vec[r] = u_max.max(0.0);

            // c_b <= -u_min (charge upper bound, -u_min >= 0 since u_min <= 0)
            a_mat[r + 1][num_b + b] = 1.0;
            b_vec[r + 1] = (-u_min).max(0.0);

            // SoC min: d*dt/eta_d - c*eta_c*dt <= soc - soc_min
            a_mat[r + 2][b] = dt / eta_d;
            a_mat[r + 2][num_b + b] = -eta_c * dt;
            b_vec[r + 2] = (soc - bat.soc_min_mwh).max(0.0);

            // SoC max: -d*dt/eta_d + c*eta_c*dt <= soc_max - soc
            a_mat[r + 3][b] = -dt / eta_d;
            a_mat[r + 3][num_b + b] = eta_c * dt;
            b_vec[r + 3] = (bat.soc_max_mwh - soc).max(0.0);
        }

        // Soft line-flow constraints: only for active (top-K) lines
        let row_f = 4 * num_b;
        for (ai, &l) in active_lines.iter().enumerate() {
            let limit = challenge.network.flow_limits[l];
            let exo = flows_base[l];
            let viol_idx = 2 * num_b + ai;
            let rp = row_f + 2 * ai; // positive direction
            let rn = rp + 1; // negative direction
            ...
            // Violation slack relaxes the constraint
            a_mat[rp][viol_idx] = -1.0;
            a_mat[rn][viol_idx] = -1.0;

            // Clamp RHS to >= 0 to satisfy lp_solve's non-negativity guard
    ...
    // Same as joint_lp_dispatch but returns (Option<Vec<f64>>, usize) where the
    // second element is the actual number of pivots consumed.
    fn joint_lp_dispatch_with_used(
        ...
        let (opt_x, pivots_used) =
            super::lp::lp_solve_with_budget(n, m, &c_vec, &a_mat, &b_vec, max_pivots);
        let Some(opt_x) = opt_x else {
            return (None, 0);
        };
    ...
    // MPC 2-step receding-horizon LP
    // ============================================================================
    // Solves a small LP spanning current step t and next step t+1, jointly
    // optimising all batteries. Uses DP terminal value at t+2 as linear
    // approximation of continuation value. Returns None on failure.
    fn mpc_dispatch_2step(
        ...
        let hz = hp.mpc_horizon.min(2); // cap at 2
        let remaining = challenge.num_steps.saturating_sub(t);
        if remaining < 2 || hz < 2 {
            return None; // need at least 2-step horizon; fall back to baseline
        }
        let dt = 0.25_f64;
        ...
        // Variables layout:
        //   horizon step h=0: d[b] at b, c[b] at num_b + b
        //   horizon step h=1: d[b] at 2*num_b + b, c[b] at 3*num_b + b
        //   horizon step h=0: v+[l] at 4*num_b + l,            v-[l] at 4*num_b + num_l + l
        //   horizon step h=1: v+[l] at 4*num_b + 2*num_l + l,  v-[l] at 4*num_b + 3*num_l + l
        let n = 4 * num_b + 4 * num_l;

        // Constraints:
        //   For each h in {0,1}, each b: d <= ub_d, c <= ub_c  (2 * num_b per h)
        //   For each h in {0,1}, each l: flow+ <= limit - exo + v+,  -flow <= limit + exo + v-
        //   That's 4*num_b + 4*num_l constraints
        ...
        // Compute DP shadow prices for each battery at t+2 (terminal value of horizon)
        ...
        let da_t = if t < challenge.market.day_ahead_prices.len() {
            ...
            challenge
                ...
                .day_ahead_prices
                .last()
                .unwrap_or(&challenge.market.day_ahead_prices[0])
        };
        ...
        let exo_flows_t1;
        {
            let exo_inj = if t + 1 < challenge.exogenous_injections.len() {
                ...
                challenge
                    ...
                    .last()
                    .unwrap_or(&challenge.exogenous_injections[t])
            };
        ...
        for b in 0..num_b {
            ...
            for h in 0..2 {
                ...
                // Objective: maximize
                // discharge profit: (da_price - 0.25)*dt - lambda*(dt/eta_d) [SoC drops]
                // charge cost: -(da_price + 0.25)*dt + lambda*(eta_c*dt) [SoC rises]
                c_vec[d_idx] = (da_p - 0.25) * dt - lambda_b * dt / eta_d;
                c_vec[c_idx] = (-da_p - 0.25) * dt + lambda_b * eta_c * dt;

                // Constraints row offset for this h:
                // d-ub: row 2*h*num_b + b
                // c-ub: row 2*h*num_b + num_b + b
                let r_d = 2 * h * num_b + b;
        ...
        // Flow constraints for each line, each step
        for h in 0..2 {
            ...
            for l in 0..num_l {
                ...
                // Slack variables with negative sign (a*x <= b format)
                ...
                // RHS (clamped to >= 0 for lp_solve non-negativity guard)
                b_vec[rp] = (limit - exo).max(0.0);
                b_vec[rn] = (limit + exo).max(0.0);
            }
        }
        ...
        // Extract actions for step t (h=0 only)
        let mut actions = vec![0.0_f64; num_b];
        for b in 0..num_b {
            let d0 = opt_x[b];
    ...
    fn lp_dispatch_with_dp(
        ...
        for b in 0..num_b {
            ...
            // Objective: immediate profit + marginal future value change
            // discharge u_p: revenue + future SoC decrease penalty
            // charge u_m: cost + future SoC increase gain
            c_vec[b] = (rt - 0.25) * dt - dv * dt / eta_d;
            c_vec[num_b + b] = (-rt - 0.25) * dt + dv * eta_c * dt;
        ...
        for l in 0..num_l {
            ...
            // No RHS clamping: negative RHS → lp_solve guard returns None → KKT fallback
    ...
    // ============================================================================
    // CMA-ES Policy Dispatch — parameterized dispatch policy
    // ============================================================================
    //
    // Policy structure: per-battery piecewise-linear in SOC with 4 segments.
    // Each segment has `num_features` slope weights + 1 bias.
    // Total params per battery: `num_segments * (num_features + 1)` = 4 * 7 = 28.
    //
    // Features (6): norm_soc, time_fraction, da_price_norm, rt_da_delta_norm,
    //               congestion_bin, action_range (hi-lo at this timestep)
    ...
    /// Normalize DA price to roughly [-1, 1] range (typical DA prices ~50-150)
    fn da_price_norm(p: f64) -> f64 {
        (p - 100.0) / 50.0
    }

    /// Normalize RT-DA delta to roughly [-1, 1] (typical deltas ~-30 to +30)
    fn rt_da_delta_norm(d: f64) -> f64 {
        d / 30.0
    }

    /// Evaluate policy for a single battery given weights and features
    /// weights: flat vec of POLICY_PARAMS_PER_BATT = num_segments * (num_features + 1)
    /// Layout: for seg in 0..num_segments: weights[seg*(num_features+1) + f] for f in 0..num_features, then bias
    fn eval_policy_for_battery(
        weights: &[f64],
        features: &[f64],
        norm_soc: f64,
        action_lo: f64,
        ...
        let mut action = weights[base + POLICY_NUM_FEATURES]; // bias
    ...
    /// Dispatch all batteries from their respective policies
    pub fn policy_dispatch(challenge: &Challenge, state: &State, weights: &[Vec<f64>]) -> Vec<f64> {
        let num_b = challenge.num_batteries;
        let mut actions = vec![0.0_f64; num_b];

        // Congestion indicator from exogenous flows
        let exo_inj = &challenge.exogenous_injections[state
            .time_step
            .min(challenge.exogenous_injections.len().saturating_sub(1))];
        ...
        let congestion_bin = if max_util < 0.5 {
            0.0
        } else if max_util < 0.8 {
            0.5
        } else {
            1.0
        };
        ...
        for b in 0..num_b {
            ...
            let norm_soc = if soc_range > 1e-9 {
                (soc - bat.soc_min_mwh) / soc_range
            } else {
                0.5
            };
            ...
            let features: [f64; POLICY_NUM_FEATURES] = [
                norm_soc,
                time_fraction,
                da_price_norm(da_price),
                rt_da_delta_norm(rt_da_delta),
                congestion_bin,
                action_range,
            ];
    ...
    // ============================================================================
    // CMA-ES Training — optimize policy weights
    // ============================================================================
    ...
    /// Box-Muller normal sampler (thread-local spare)
    fn policy_randn() -> f64 {
    ...
    /// Simulate full game horizon with given policy weights, return total profit.
    fn simulate_policy(
        challenge: &Challenge,
        initial_state: &State,
        cache: &TitanCache,
        ...
        for t in 0..num_t {
            let rt_prices: Vec<f64> = challenge
                .market
                .day_ahead_prices
                .iter()
                .map(|np| {
            ...
            for b in 0..num_b {
                ...
                let disch_limit =
                    (bat.soc_max_mwh - soc) * bat.efficiency_discharge.max(1e-9) / 0.25;
                let lo = (-max_ch).max(charge_limit);
                let hi = max_dis.min(disch_limit);
                let (lo, hi) = (lo, hi);

                // Build minimal state for this timestep
                let state_t = State {
                    time_step: t,
                    socs: socs.clone(),
                    rt_prices: rt_prices.clone(),
                    exogenous_injections: challenge.exogenous_injections[t].clone(),
                    action_bounds: vec![(lo, hi); num_b],
                    total_profit,
                };
                actions[b] = 0.0; // placeholder
            }

            // Dispatch all batteries from policy
            actions = policy_dispatch(
                challenge,
                &State {
                    time_step: t,
                    socs: socs.clone(),
                    rt_prices: rt_prices.clone(),
                    exogenous_injections: challenge.exogenous_injections[t].clone(),
                    action_bounds: (0..num_b)
                        .map(|b| {
                            let bat = &challenge.batteries[b];
                            let soc = socs[b];
                            let lo = (-(bat.power_charge_mw * 0.25))
                                .max(-(soc - bat.soc_min_mwh) / bat.efficiency_charge.max(1e-9));
                            let hi = (bat.power_discharge_mw * 0.25).min(
                                (bat.soc_max_mwh - soc) * bat.efficiency_discharge.max(1e-9) / 0.25,
                            );
                            (lo, hi)
                        })
            ...
            // Deflator
            let zero_action = vec![0.0_f64; num_b];
            let inj_base = challenge.compute_total_injections(
                &State {
                    time_step: t,
                    socs: socs.clone(),
                    rt_prices: rt_prices.clone(),
    ...
    /// Train policy weights using diagonal CMA-ES
    pub fn train_policy_cmaes(
        ...
        let cc = (4.0 + 2.0 / (dim as f64 + 4.0))
            / (dim as f64 + 4.0 + 2.0 * (1.0 - 2.0 / (dim as f64 + 4.0)).sqrt());
        let cs = (eff_popsize + 2.0) / (dim as f64 + eff_popsize + 2.0);
        let ca = cs / (cs + 2.0);
        let dc = (1.0 + 2.0 * ((eff_popsize - 1.0) / (dim as f64 + 1.0)).sqrt() + cs)
            / (dim as f64 + cs);
        let ds = (1.0 + 2.0 * ((eff_popsize - 1.0) / (dim as f64 + 1.0)).sqrt() + cs)
            / (dim as f64 + cs);
        ...
        for _gen in 0..max_gens {
            ...
            let chi_n = (dim as f64).sqrt()
                * (1.0 - 1.0 / dim as f64
                    + (dim as f64) * (1.0 - 2.0 / (dim as f64 + 4.0)).sqrt() / (dim as f64 - 1.0))
                    .sqrt();
            for d in 0..dim {
                pc[d] = (1.0 - dc) * pc[d]
                    + (dc * (eff_popsize / ec).sqrt()).min(1.0) * (m_new[d] - mean[d]) / sigma;
            }
    ...
    // ============================================================================
    // Parallel DP construction — embarrassingly parallel across batteries
    // ============================================================================
    // When `hp.use_parallel_dp` is true, partition batteries across threads and
    // build each battery's DP table in parallel. Results are assembled into the
    // canonical dp[b][t][i] layout. Falls back to serial `build_dp_with_mu` when
    // disabled or when thread count would be 1.
    ...
    fn build_dp_parallel_or_serial(
        ...
        // Stochastic DP override: if enabled, replace each battery's deterministic
        // DP table with a scenario-integrated version that captures jump optionality.
        if hp.use_sdp {
            for b in 0..num_b {
    ...
    /// Build DP table for a SINGLE battery. This is the core per-battery computation
    /// extracted from `build_dp_with_mu` to enable parallel execution.
    ...
    // ============================================================================
    // Gauss-Hermite Stochastic DP — quadrature-based stochastic value function
    // ============================================================================
    // For each (t, soc) cell, the DP maximizes expected profit across K Gauss-
    // Hermite price-scenarios plus an analytic jump-premium correction. The
    // continuous shock is integrated via GH nodes; the Pareto jump component is
    // approximated by its unconditional expectation (exact due to independence).
    // This produces a VF that captures tail-risk optionality, shifting optimal
    // SOC upward when jumps are probable.
    ...
    fn build_dp_stochastic_for_battery(
        ...
        // Precompute standard GH quadrature for fallback path
        ...
        for t in (0..num_t).rev() {
            ...
            // Select quadrature: tail-mixture or standard GH
            ...
            let scene_perturb: Vec<f64> = nodes_q
                .iter()
                .map(|&z| sigma * z * base_abs + base_abs * jump_prem_factor)
                .collect();
    ...
    /// Build a 3-point quadrature for the mixture distribution of the price shock:
    /// (1-rho_jump) * N(0,sigma^2)  +  rho_jump * [ N(0,sigma^2) + da_price * Pareto(alpha) ]
    ///
    /// Returns (nodes, weights) where node values are in standard-deviation units
    /// (i.e. perturbation = sigma * node * base_abs).  The weights sum to 1.
    fn build_tail_mixture_quadrature(
        ...
        // Guard: require alpha > 1 for finite Pareto mean, sigma > 0, rho in (0,1)
        if alpha <= 1.0 || sigma <= 0.0 || rho_jump <= 0.0 || rho_jump >= 1.0 {
            return gh_quadrature(3);
        }

        // Pareto conditional mean (for alpha > 1)
        ...
        // Use 2-point Gauss-Hermite for the normal part
        let (gh2_nodes, gh2_weights) = gh_quadrature(2);

        let mut nodes = Vec::with_capacity(3);
        let mut weights = Vec::with_capacity(3);

        // Append the two normal-component points, scaled by w_norm
        for (n, w) in gh2_nodes.iter().zip(gh2_weights.iter()) {
            nodes.push(*n);
            weights.push(w_norm * w);
        }

        // Append the jump-component point: deterministic shift equal to the
        // conditional mean of the jump, expressed in standard-deviation units.
        // mean_pareto/sigma = mean_pareto * da_abs / (sigma * da_abs) = mean_pareto / sigma
        let jump_node = mean_pareto / sigma;
        let jump_node = if jump_node.is_finite() && jump_node > 0.0 {
            jump_node
        } else {
            2.0
        };
        ...
        // Safety: ensure weights sum to 1 (within tolerance)
    ...
    // ============================================================
    // This block is the ONLY part of the codebase that V3 evolution
    // touches. Its body is replaced by DeepSeek's proposed heuristic on
    // each V3 iter. The wrapper around it (in policy_entry) provides a
    // safety net so the refinement can never regress below baseline.
    ...
    fn refine_solution(
        ...
        // ----- PASS 1 (preserved): coordinate-descent with feasibility checks -----
        let fuel_budget_p1 = fuel_budget * 2 / 3;
        let (limits, ptdf) = (&challenge.network.flow_limits, &challenge.network.ptdf);
        ...
        let feasible = |act: &[f64]| -> bool {
            ...
            flows
                .iter()
                .enumerate()
                .all(|(l, &f)| f.abs() <= limits[l] + 1e-6 * limits[l])
        };
        ...
        let max_steps_p1 = (fuel_budget_p1 as usize)
            .saturating_div(cost_per_eval)
            .min(200);
        let mut steps_used = 0;

        for _pass in 0..3 {
            if steps_used >= max_steps_p1 {
                break;
            }
            for b in 0..num_b {
                if steps_used >= max_steps_p1 {
        ...
        // ----- PASS 2 (new): dual-pricing penalty + last-step coordinate-ascent polish -----
        let fuel_budget_p2 = fuel_budget / 3;
        let mut flows = challenge
            .network
            .compute_flows(&challenge.compute_total_injections(state, &actions));
        ...
        let max_it1 = ((fuel_budget_p2 as usize) / (num_b.max(1) * 10))
            .min(20)
            .max(5);
        for _ in 0..max_it1 {
            for b in 0..num_b {
                ...
                if lo < 0.0 && lo < hi.min(0.0) {
                    ...
                    let c = if dk > 1e-12 {
                        (-b_c / (2.0 * dk)).clamp(lo, hi.min(0.0))
                    } else {
                        lo
                    };
                    let v = eval_fn(c);
                ...
                if hi > 0.0 && hi > lo.max(0.0) {
                    ...
                    let c = if dk > 1e-12 {
                        (b_d / (2.0 * dk)).clamp(lo.max(0.0), hi)
                    } else {
                        hi
                    };
                    let v = eval_fn(c);
            ...
            for l in 0..num_l {
                ...
                let vp = (flows[l] - lim).max(0.0);
                let vn = (-flows[l] - lim).max(0.0);
                mu_pos[l] = (mu_pos[l] + step_size * vp).min(500.0);
                mu_neg[l] = (mu_neg[l] + step_size * vn).min(500.0);
                mv = mv.max(vp).max(vn);
            }
            step_size *= 0.85;
            ...
            for b in 0..num_b {
                ...
                for delta in [step_frac * range, -step_frac * range] {
                    ...
                    for &(l, imp) in &ptdf_b[b] {
                        if (flows[l] + imp * (u - base)).abs()
                            > challenge.network.flow_limits[l] + 1e-6
                        {
                            feas = false;
                            break;
                        }
...
mod lp {
    ...
    // Assumes b_i >= 0 (holds for TIG T50 typical instances).
    ...
    /// Original lp_solve delegates to budgeted version with default cap.
    ...
    /// Solve LP and return both primal solution and dual (shadow) prices for each constraint.
    /// Dual i = negative of final reduced cost in objective row for slack variable i,
    /// which corresponds to the shadow price of the i-th constraint in standard form (A x <= b).
    /// Returns (primal_solution, dual_prices, pivots_used) or (None, None, 0) on failure.
    pub fn lp_solve_with_duals(
        ...
        for pivot_count in 0..max_pivots {
            let entering = match (0..n_vars).find(|&j| tab[m][j] < -LP_EPS) {
                Some(j) => j,
                None => {
                    // Optimal — extract duals from objective row for slack variables
                    let duals = (0..m)
                        .map(|i| {
                            let slack_col = n + i;
        ...
        // Pivot budget exhausted — still extract best available solution and duals
        let duals = (0..m)
            .map(|i| {
                let slack_col = n + i;
                let val = -tab[m][slack_col];
...
mod track_baseline {
    ...
    fn defaults() -> TrackHp {
        TrackHp {
            ...
            use_congestion_safety: false,
            congestion_safety_threshold: 0.85,
            congestion_safety_strength: 0.3,
        }
    }
