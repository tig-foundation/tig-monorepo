...
mod helpers {
    ...
    pub struct TrackHp {
        ...
        // Select the LMP premium regime from forecast network stress at runtime.
        pub use_stress_adaptive_premium: bool,
        ...
        // i24 (iter 3622): SWAP#5 tail-scenario family — V-table price-scenario model.
        // VarA: data-driven cross-node quantile scenarios (R8 transplant aiwfcihq build_adaptive_scenarios)
        pub use_emp_scenarios: bool,
        // VarB: GH5 diffusion-only + 2 explicit jump atoms weighted rho_j (models jump skew/kurtosis
        // that the fused sqrt(sigma^2+jump_var) Gaussian flattens)
        pub use_jump_mixture: bool,
        // VarC: asymmetric p_sell=(1+jump_premium)·p inside GH5 nodes (jump_premium is dead code under use_sdp)
        pub use_sdp_jump_wedge: bool,
        // i25 (iter 3623): tail amplification scale in emp_scenarios (replaces hardcoded 1.5).
        // tail_w = emp_tail_scale * rho_j for tail quantiles (q<0.2 or q>0.8).
        // Default 1.5 = bit-exact with i24 VarA. Higher = more conservative (hold SoC for bigger spikes).
        pub emp_tail_scale: f64,
        
        // w_k ∝ (u_{k+1}-u_{k-1})/2 with boundary extension to 0 and 1.
        // false=uniform 0.20 (CTRL bit-exact), true=trapezoid weights.
        pub emp_trap_weights: bool,
        // i29 VarA: 7-node center-dense grid {0.10,0.30,0.42,0.50,0.58,0.70,0.90} uniform weights.
        // Operationalizes i28 lesson: DP value concentrates at median; 2 extra center nodes improve resolution.
        pub emp_center_nodes: bool,
        // i29 VarB: empirical coeff. of variation (std(pool)/avg) as overlay amplitude.
        // Replaces parametric sigma in base_vol = sigma*(1+tail_w) with data-driven cluster spread.
        pub emp_disp_overlay: bool,
        // i30 L6 TEMPOREL: value-weighted scenario selection (NeurPRISE regret V(R)-V(Ξ)).
        // On each backward DP step, compute cross-scenario DP value variance at median SoC;
        // weight each scenario proportionally to its contribution to that variance.
        // regret_weight_mode: 1=VarA (value-std), 2=VarB (quantile-dist), 3=VarC (geometric combo)
        pub use_regret_weights: bool,
        pub regret_weight_mode: u8,
        // i31 L6-BIS: scenario grid structure. 0=5-node {0.10,0.35,0.65,0.90,0.50} (CTRL bit-exact),
        // 1=7-node extreme tails {0.05,0.10,0.35,0.65,0.90,0.95,0.50}, 2=4-node no-median {0.10,0.35,0.65,0.90}.
        pub emp_tail_nodes: u8,
    }
    ...
    impl TrackHp {
        pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
            ...
            if let Some(v) = m.get("use_stress_adaptive_premium").and_then(|v| v.as_bool()) { self.use_stress_adaptive_premium = v; }
            ...
            if let Some(v) = m.get("use_emp_scenarios").and_then(|v| v.as_bool()) { self.use_emp_scenarios = v; }
            if let Some(v) = m.get("use_jump_mixture").and_then(|v| v.as_bool()) { self.use_jump_mixture = v; }
            if let Some(v) = m.get("use_sdp_jump_wedge").and_then(|v| v.as_bool()) { self.use_sdp_jump_wedge = v; }
            if let Some(v) = m.get("emp_tail_scale").and_then(|v| v.as_f64()) { self.emp_tail_scale = v.clamp(0.0, 20.0); }
            if let Some(v) = m.get("emp_trap_weights").and_then(|v| v.as_bool()) { self.emp_trap_weights = v; }
            if let Some(v) = m.get("emp_center_nodes").and_then(|v| v.as_bool()) { self.emp_center_nodes = v; }
            if let Some(v) = m.get("emp_disp_overlay").and_then(|v| v.as_bool()) { self.emp_disp_overlay = v; }
            if let Some(v) = m.get("use_regret_weights").and_then(|v| v.as_bool()) { self.use_regret_weights = v; }
            if let Some(v) = m.get("regret_weight_mode").and_then(|v| v.as_u64()) { self.regret_weight_mode = v as u8; }
            if let Some(v) = m.get("emp_tail_nodes").and_then(|v| v.as_u64()) { self.emp_tail_nodes = v as u8; }
        }
    }
    ...
    // A single premium scale is brittle across congestion regimes.  Measure the
    // mean forecast loading of every line and use a stronger premium only when
    // congestion is sparse/localized; broadly stressed instances use the more
    // conservative scale.  This is instance-dependent behavior that cannot be
    // reproduced by one static lmp_premium_scale override.
    fn stress_adaptive_premium_scale(challenge: &Challenge, hp: &TrackHp) -> f64 {
        if !hp.use_stress_adaptive_premium {
            return hp.lmp_premium_scale;
        }

        let slack = challenge.network.slack_bus;
        let mut stress_sum = 0.0_f64;
        let mut stress_count = 0usize;
        for exogenous in &challenge.exogenous_injections {
            let mut balanced = exogenous.clone();
            if slack < balanced.len() {
                let non_slack_sum: f64 = balanced
                    .iter()
                    .enumerate()
                    .filter(|(node, _)| *node != slack)
                    .map(|(_, value)| *value)
                    .sum();
                balanced[slack] = -non_slack_sum;
            }
            let flows = challenge.network.compute_flows(&balanced);
            for (flow, limit) in flows.iter().zip(&challenge.network.flow_limits) {
                if *limit > 1e-9 {
                    stress_sum += flow.abs() / *limit;
                    stress_count += 1;
                }
            }
        }
        ...
        let mean_stress = stress_sum / stress_count as f64;
        if mean_stress <= 0.2763 { 0.54 } else { 0.47 }
    }
    ...
    fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> TitanCache {
        ...
        if hp.anticipate_lmp && num_l > 0 {
            let base_premium = 20.0 * stress_adaptive_premium_scale(challenge, hp);
        ...
        // i24 VarA (use_emp_scenarios): data-driven cross-node quantile scenarios per cluster
        // (R8 transplant aiwfcihq build_adaptive_scenarios, adapted to cluster-indexed DP).
        // Per cluster: pool day_ahead_prices[t][*] over member timesteps, empirical quantiles
        // {0.10,0.35,0.65,0.90} + median as price multipliers rel. to pool mean, tail-weighted by rho_j.
        let emp_scenarios: Vec<Vec<(f64, f64)>> = if hp.use_sdp && hp.use_emp_scenarios {
            let sigma = challenge.market.params.volatility;
            let rho_j = challenge.market.params.jump_probability;
            let mut scen: Vec<Vec<(f64, f64)>> = vec![Vec::new(); k_actual];
            let mut pools: Vec<Vec<f64>> = vec![Vec::new(); k_actual];
            for t in 0..num_t {
                let ck = cluster_map[t].min(k_actual.saturating_sub(1));
                for &p in &challenge.market.day_ahead_prices[t] { pools[ck].push(p); }
            }
            for ck in 0..k_actual {
                let pool = &mut pools[ck];
                if pool.is_empty() { scen[ck].push((1.0, 1.0)); continue; }
                pool.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
                let avg: f64 = pool.iter().sum::<f64>() / pool.len() as f64;
                if avg <= 1e-6 { scen[ck].push((1.0, 1.0)); continue; }
                // VarB: replace parametric sigma with empirical coeff. of variation
                let amp_scale = if hp.emp_disp_overlay {
                    let sum2: f64 = pool.iter().map(|&x| x * x).sum::<f64>();
                    let mean2 = sum2 / pool.len() as f64;
                    let variance = (mean2 - avg * avg).max(0.0);
                    (variance.sqrt() / avg.max(1e-6)).max(0.01)
                } else {
                    sigma
                };
                let mut step: Vec<(f64, f64)> = Vec::with_capacity(7);
                if hp.emp_center_nodes {
                    // VarA: 7-node center-dense grid {0.10,0.30,0.42,0.50,0.58,0.70,0.90}, uniform w=1/7
                    for &q in &[0.10_f64, 0.30, 0.42, 0.50, 0.58, 0.70, 0.90] {
                        let idx = (((pool.len() - 1) as f64) * q).round() as usize;
                        let rel = pool[idx] / avg;
                        let tail_w = if q < 0.2 || q > 0.8 { hp.emp_tail_scale * rho_j } else { 0.5 * rho_j };
                        let base_vol = amp_scale * (1.0 + tail_w);
                        let dev = if q == 0.50 {
                            rel  // center node: pure empirical median, no vol bias
                        } else if q < 0.5 {
                            rel * (1.0 - base_vol)
                        } else {
                            rel * (1.0 + base_vol)
                        };
                        step.push((1.0 / 7.0, dev.max(0.1)));
                    }
                } else {
                    
                    // 0=5-node {0.10,0.35,0.65,0.90}+median (CTRL bit-exact)
                    // 1=7-node {0.05,0.10,0.35,0.65,0.90,0.95}+median (extreme tails)
                    // 2=4-node {0.10,0.35,0.65,0.90} no median (speed + minor weight shift)
                    let q_spread: &[f64] = match hp.emp_tail_nodes {
                        1 => &[0.05_f64, 0.10, 0.35, 0.65, 0.90, 0.95],
                        _ => &[0.10_f64, 0.35, 0.65, 0.90],
                    };
                    for (qi, &q) in q_spread.iter().enumerate() {
                        let idx = (((pool.len() - 1) as f64) * q).round() as usize;
                        let rel = pool[idx] / avg;
                        let tail_w = if q < 0.2 || q > 0.8 { hp.emp_tail_scale * rho_j } else { 0.5 * rho_j };
                        let base_vol = amp_scale * (1.0 + tail_w);
                        let dev = if q < 0.5 { rel * (1.0 - base_vol) } else { rel * (1.0 + base_vol) };
                        let w = if hp.emp_tail_nodes == 0 && hp.emp_trap_weights {
                            [0.175_f64, 0.200, 0.200, 0.175][qi.min(3)]
                        } else { 0.20 };
                        step.push((w, dev.max(0.1)));
                    }
                    if hp.emp_tail_nodes != 2 {
                        let med_w = if hp.emp_tail_nodes == 0 && hp.emp_trap_weights { 0.150 } else { 0.20 };
                        step.push((med_w, pool[pool.len() / 2] / avg));
                    }
                }
                let sw: f64 = step.iter().map(|(w, _)| w).sum();
                for (w, _) in &mut step { *w /= sw; }
                scen[ck] = step;
            }
            scen
        } else { Vec::new() };

        // i24 VarB (use_jump_mixture): split fused variance back into diffusion + explicit jump atoms.
        // Diffusion sigma = raw volatility (NOT sqrt(sigma^2+jump_var)); jumps = 2 atoms of magnitude
        // sqrt(jump_var/rho_j) (per-jump RMS), weight rho_j/2 each; GH5 nodes reweighted by (1-rho_j).
        // First two moments match the fused model; skew/kurtosis of jumps now explicit.
        let (jm_sigma_diff, jm_atom, jm_rho) = if hp.use_sdp && hp.use_jump_mixture {
            let sigma = challenge.market.params.volatility;
            let rho_j = challenge.market.params.jump_probability.clamp(0.0, 0.5);
            let alpha_j = challenge.market.params.tail_index;
            let jump_var = if alpha_j > 2.0 { rho_j * alpha_j / (alpha_j - 2.0) } else { rho_j * 4.0 };
            let atom = (jump_var / rho_j.max(1e-6)).sqrt();
            (sigma * hp.sdp_sigma_scale, atom, rho_j)
        } else { (0.0, 0.0, 0.0) };
        ...
        for b in 0..num_b {
            ...
            for ck in (0..k_actual).rev() {
                ...
                let mut regret_wts: Vec<f64> = Vec::new();
                ...
                for i in 0..soc_levels {
                    ...
                    // i30 L6 TEMPOREL: compute value-weighted scenario weights once per (b,ck) on i==0.
                    // Weights scenarios by cross-scenario DP value variance at median SoC (NeurPRISE regret).
                    // CTRL: use_regret_weights=false → skipped, regret_wts empty, falls to uniform weights below.
                    if i == 0 && hp.use_sdp && hp.use_emp_scenarios && hp.use_regret_weights && !hp.emp_center_nodes {
                        let scen = &emp_scenarios[ck];
                        let n_scen = scen.len();
                        // VarB static: distance to center quantile (|q_k - 0.5|)
                        // q_pos matches the scenario order built by emp_tail_nodes mode.
                        let q_pos: &[f64] = match hp.emp_tail_nodes {
                            1 => &[0.05_f64, 0.10, 0.35, 0.65, 0.90, 0.95, 0.50],
                            2 => &[0.10_f64, 0.35, 0.65, 0.90],
                            _ => &[0.10_f64, 0.35, 0.65, 0.90, 0.50],
                        };
                        let varb: Vec<f64> = (0..n_scen).map(|k| {
                            let q = q_pos.get(k).copied().unwrap_or(0.5);
                            (q - 0.5_f64).abs().max(0.02)
                        }).collect();
                        // VarA: cross-scenario DP value variance at median SoC using already-computed v_next
                        let vara: Vec<f64> = if hp.regret_weight_mode != 2 {
                            let i_mid = soc_levels / 2;
                            let soc_mid = bat.soc_min_mwh + soc_span * (i_mid as f64) / ((soc_levels - 1).max(1) as f64);
                            let ch_lim = if bat.efficiency_charge > 0.0 { (bat.soc_max_mwh - soc_mid) / (bat.efficiency_charge * dt) } else { 0.0 };
                            let dis_lim = if bat.efficiency_discharge > 0.0 { (soc_mid - bat.soc_min_mwh) * bat.efficiency_discharge / dt } else { 0.0 };
                            let u_min_m = -(bat.power_charge_mw * hp.network_derating).min(ch_lim.max(0.0));
                            let u_max_m = (bat.power_discharge_mw * hp.network_derating).min(dis_lim.max(0.0)).max(u_min_m);
                            let vals: Vec<f64> = scen.iter().map(|&(_, mult)| {
                                let p = (p_da * mult + extra).max(1e-6);
                                dp_analytic_max(bat, p, p, soc_mid, u_min_m, u_max_m, v_next_slice, soc_levels, soc_span, deg_coeff)
                            }).collect();
                            let mean_v = vals.iter().sum::<f64>() / (n_scen.max(1) as f64);
                            vals.iter().map(|&v| (v - mean_v).powi(2).max(1e-12)).collect()
                        } else {
                            vec![1.0_f64; n_scen]
                        };
                        let raw: Vec<f64> = (0..n_scen).map(|k| match hp.regret_weight_mode {
                            1 => vara[k],
                            2 => varb[k],
                            _ => (vara[k] * varb[k]).sqrt(),
                        }).collect();
                        let sw: f64 = raw.iter().sum::<f64>();
                        regret_wts = raw.iter().map(|&w| w / sw.max(1e-12)).collect();
                    }

                    let max_val = if hp.use_sdp && hp.use_emp_scenarios {
                        // i24 VarA: empirical quantile scenarios (multipliers per cluster)
                        
                        let mut val_sum = 0.0_f64;
                        if hp.use_regret_weights && !regret_wts.is_empty() {
                            for (k, &(_, mult)) in emp_scenarios[ck].iter().enumerate() {
                                if let Some(&w) = regret_wts.get(k) {
                                    let p = (p_da * mult + extra).max(1e-6);
                                    val_sum += w * dp_analytic_max(bat, p, p, soc, u_min, u_max, v_next_slice, soc_levels, soc_span, deg_coeff);
                                }
                            }
                        } else {
                            for &(w, mult) in &emp_scenarios[ck] {
                                let p = (p_da * mult + extra).max(1e-6);
                                val_sum += w * dp_analytic_max(bat, p, p, soc, u_min, u_max, v_next_slice, soc_levels, soc_span, deg_coeff);
                            }
                        }
                        val_sum
                    } else if hp.use_sdp && hp.use_jump_mixture {
                        // i24 VarB: GH5 diffusion (weight 1-rho_j) + 2 explicit jump atoms (weight rho_j/2 each)
                        let sig_d = if hp.sdp_sigma_het_alpha != 0.0 {
                            jm_sigma_diff * (p_da / mean_p.max(1e-9)).powf(hp.sdp_sigma_het_alpha) * c_rate_scale[b]
                        } else {
                            jm_sigma_diff * c_rate_scale[b]
                        };
                        let mut val_sum = 0.0_f64;
                        for k in 0..5 {
                            let p = (p_da * (1.0 + sig_d * GH5_Z[k]) + extra).max(1e-6);
                            val_sum += (1.0 - jm_rho) * GH5_W[k] * dp_analytic_max(bat, p, p, soc, u_min, u_max, v_next_slice, soc_levels, soc_span, deg_coeff);
                        }
                        for &mult in &[1.0 + jm_atom, (1.0 - jm_atom).max(0.1)] {
                            let p = (p_da * mult + extra).max(1e-6);
                            val_sum += (jm_rho * 0.5) * dp_analytic_max(bat, p, p, soc, u_min, u_max, v_next_slice, soc_levels, soc_span, deg_coeff);
                        }
                        val_sum
                    } else if hp.use_sdp {
                        ...
                        for k in 0..5 {
                            ...
                            // i24 VarC: asymmetric sell wedge inside GH5 nodes; false → p_sell=p (bit-exact CTRL)
                            let p_sell = if hp.use_sdp_jump_wedge {
                                (p_da * (1.0 + sigma_t * GH5_Z[k]) * (1.0 + hp.jump_premium) + extra).max(1e-6)
                            } else { p };
                            val_sum += GH5_W[k] * dp_analytic_max(bat, p, p_sell, soc, u_min, u_max, v_next_slice, soc_levels, soc_span, deg_coeff);
                        }
                        val_sum
                    } else {
    ...
    fn rcb_full_horizon_solve(
        ...
        if hp.anticipate_lmp && num_l > 0 {
            let base_prem = 20.0 * stress_adaptive_premium_scale(challenge, hp);
            for t in 0..num_t {
...
mod track_baseline {
    ...
    fn defaults() -> TrackHp {
        TrackHp {
            ...
            use_stress_adaptive_premium: false,
            jump_premium: 0.02,
            prune_ratio: 0.00,
            deflator_iters: 15,
            flow_margin: 1e-4,
            ...
            use_emp_scenarios: false,
            use_jump_mixture: false,
            use_sdp_jump_wedge: false,
            emp_tail_scale: 1.5,
            emp_trap_weights: false,
            emp_center_nodes: false,
            emp_disp_overlay: false,
            use_regret_weights: false,
            regret_weight_mode: 0,
            emp_tail_nodes: 0,
        }
    }
...
pub struct Hyperparameters {
    ...
    pub use_stress_adaptive_premium: Option<bool>,
