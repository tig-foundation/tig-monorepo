...
pub struct VtConfig {
 ...
 pub lp_momentum: f64,
 /// If true, after ASCA+deflator+Lagrangian, also solve a joint LP
 /// over (charge, discharge) legs per battery with PTDF flow
 /// constraints; keep the LP result only when its per-step value
 /// strictly exceeds ASCA's. Off on tracks where ASCA already
 /// saturates the per-track clamp.
 pub use_joint_lp: bool,
 /// Seed the per-step dispatch with a stateful realized-RT-price
 /// heuristic (titan_v4 capstone port) before the grid scan + ASCA/LP
 /// refine it. Learns the per-node RT-DA residual wedge and the RT
 /// percentile band across the horizon, then triggers opportunistic
 /// spike-discharge / dip-charge the static DA-only DP can't see. The
 /// edge grows with congestion, so it's the capstone lever. Off
 /// elsewhere. Uses the per-solve `RtHistory` carried by the closure.
 pub use_history_seed: bool,
 /// Day-ahead steps the history seed looks ahead to set per-battery
 /// charge/discharge price thresholds. 4 = one hour. Ignored when
 /// `use_history_seed = false`.
 pub lookahead_horizon: usize,
 /// Let the history seed DRIVE the dispatch instead of being a V_t-gated
 /// candidate. When true, the seed action is taken directly and only the
 /// network-feasibility passes (deflator + shrink) run — the V_t-based
 /// grid scan and ASCA/LP value re-optimisation are skipped, because they
 /// revert exactly the realized-RT-exploiting actions (spike-discharge /
 /// dip-charge) that the DA-only V_t undervalues. Requires
 /// `use_history_seed = true`. Off elsewhere.
 pub history_seed_drives: bool,
}
...
/// Analytic per-branch quadratic argmax + 1-3 evaluations. Replaces the
/// grid inner-max in `build_vt_table` and (the runtime sibling)
/// `compute_action_vt`.
///
...
// ────────────────────────────────────────────────────────────────────
// History-aware seed (titan_v4 capstone port)
// ────────────────────────────────────────────────────────────────────

const SEED_EPS: f64 = 1e-9;

/// Market-impact-dampened action fraction: scale magnitude by how far the
/// price is past the threshold (relative to the band), floored at 0.35 so a
/// confirmed signal still moves meaningfully. Faithful port of titan_v4.
...
/// Battery fullness in [0, 1] between its SOC bounds.
...
fn percentile_sorted(sorted: &[f64], numerator: usize, denominator: usize) -> f64 {
 if sorted.is_empty() {
 return 0.0;
 }
...
/// Per-node realized-RT-price history, accumulated across timesteps within
/// one solve (carried by the policy closure, mutated each step). Mirrors
/// titan_v4's stateful tracker: it learns the RT-DA residual wedge and the
/// realized RT percentile band per node, which the static DA-only DP cannot
/// see. The exploitable structure (persistent, time-correlated congestion)
/// is strongest on the largest network, so this drives the capstone gain.
#[derive(Default)]
pub struct RtHistory {
 num_nodes: usize,
 values: Vec<Vec<f64>>,
 residuals: Vec<Vec<f64>>,
}

/// Compute the per-battery history-aware seed action vector for the current
/// step AND append this step's prices/residuals to `history` for the next
/// call. Returns actions already clamped into `state.action_bounds`; the
/// caller blends them with the DP grid scan (keeps whichever scores higher).
///
/// Port of titan_v4's `policy()` seed (the threshold dispatch from a
/// residual-shifted DA lookahead, plus spike/dip triggers off the learned RT
/// band, plus terminal-drain end-of-horizon logic). Efficiencies are taken
/// per-battery (our challenge exposes them per battery) rather than from a
/// global round-trip constant.
fn history_aware_seed(
 challenge: &Challenge,
 state: &State,
 cfg: &VtConfig,
 history: &RefCell<RtHistory>,
) -> Vec<f64> {
 ...
 let num_b = challenge.num_batteries;
 let num_n = challenge.network.num_nodes;
 let mut target = vec![0.0_f64; num_b];
 if n_remaining == 0 || num_n == 0 {
 return target;
 }

 let horizon = cfg.lookahead_horizon.min(n_remaining).max(1);
 let friction = 2.0 * KAPPA_TX;
 let hours_left = (n_remaining as f64) * DELTA_T;
 let allow_charge = hours_left >= 1.5;

 // Rank batteries by fullness so the fullest discharge first near the end.
 let mut soc_ranks: Vec<(f64, usize)> = (0..num_b)
 .map(|b| (relative_soc_pressure(&challenge.batteries[b], state.socs[b]), b))
 .collect();
 soc_ranks.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
 let mut terminal_rank = vec![num_b; num_b];
 for (rank, &(_, b)) in soc_ranks.iter().enumerate() {
 terminal_rank[b] = rank;
 }

 let mut hist = history.borrow_mut();
 if t == 0 || hist.num_nodes != num_n {
 hist.num_nodes = num_n;
 hist.values = vec![Vec::new(); num_n];
 hist.residuals = vec![Vec::new(); num_n];
 }
 // Learn the per-node RT band (q15/q85 over >=16 samples) and the residual
 // shift (slow EMA of RT-DA over >=8 samples).
 let mut rt_bands: Vec<Option<(f64, f64)>> = vec![None; num_n];
 let mut residual_shift = vec![0.0_f64; num_n];
 for node in 0..num_n {
 if hist.values[node].len() >= 16 {
 let mut sorted = hist.values[node].clone();
 sorted.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
 let q15 = percentile_sorted(&sorted, 15, 100);
 let q85 = percentile_sorted(&sorted, 85, 100);
 if q85 - q15 > 2.0 {
 rt_bands[node] = Some((q15, q85));
 }
 }
 if hist.residuals[node].len() >= 8 {
 let mut sorted = hist.residuals[node].clone();
 sorted.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
 let median = percentile_sorted(&sorted, 50, 100);
 let recent = *hist.residuals[node].last().unwrap_or(&median);
 ...
 let eta_rt = (battery.efficiency_charge * battery.efficiency_discharge).max(1e-9);

 // Residual-shifted DA lookahead → quartile charge/discharge thresholds.
 let end = (t + horizon).min(n_steps);
 let shift = residual_shift[node];
 let mut future: Vec<f64> = (t..end)
 .map(|tau| challenge.market.day_ahead_prices[tau][node] + shift)
 .collect();
 ...
 let q_low = future[n / 4];
 let q_high = future[((3 * n) / 4).min(n - 1)];
 let price_band = (q_high - q_low).abs();
 let charge_max = q_high * eta_rt - friction;
 let discharge_min = q_low / eta_rt + friction;

 // End-of-horizon drain logic: empty the battery into the remaining
 // profitable window rather than carrying value past the terminal.
 let discharge_steps_to_min = if u_max > SEED_EPS {
 let withdrawable = (state.socs[b] - battery.soc_min_mwh).max(0.0);
 let per_step = u_max * DELTA_T / battery.efficiency_discharge.max(1e-9);
 if per_step > SEED_EPS {
 (withdrawable / per_step).ceil() as usize
 } else {
 usize::MAX
 }
 } else {
 ...
 let rank_frac = (terminal_rank[b] as f64 + 1.0) / (num_b.max(1) as f64);
 ...
 let fraction =
 (0.25 + 0.55 * urgency + 0.35 * fullness + 0.25 * rank_boost).clamp(0.35, 1.0);
 a = u_max * fraction;
 } else if u_max > 0.0 && current_price > discharge_min {
 a = u_max * edge_sized_fraction(current_price - discharge_min, price_band);
 } else if allow_charge && u_min < 0.0 && current_price < charge_max {
 a = u_min * edge_sized_fraction(charge_max - current_price, price_band);
 }

 // Spike/dip exploitation off the learned realized-RT band.
 ...
 let spike = u_max * edge_sized_fraction(current_price - rt_high - friction, rt_band);
 if spike.abs() > a.abs() || a < 0.0 {
 a = spike;
 }
 } else if allow_charge && u_min < 0.0 && current_price < rt_low * eta_rt - friction {
 let dip =
 u_min * edge_sized_fraction(rt_low * eta_rt - friction - current_price, rt_band);
 if dip.abs() > a.abs() || a > 0.0 {
 a = dip;
 }
 }
 }

 target[b] = a.clamp(u_min, u_max);
 }

 // Append this step's observations for the next call.
 for node in 0..num_n {
 hist.values[node].push(state.rt_prices[node]);
 hist.residuals[node].push(state.rt_prices[node] - challenge.market.day_ahead_prices[t][node]);
 }
...
pub fn vt_value_function_policy(
 ...
 // Per-solve realized-RT-price history for the optional history-aware seed.
 let history: RefCell<RtHistory> = RefCell::new(RtHistory::default());
 challenge.grid_optimize(&move |challenge: &Challenge, state: &State| {
 compute_action_vt(challenge, state, &cache, &cfg, &warm_mu, &history)
 })
}
...
fn compute_action_vt(
 ...
 // Optional history-aware seed (titan_v4 capstone port). Computed once per
 // step (it also advances the per-node price history). Per battery the grid
 // scan below keeps whichever of {grid argmax, history action} scores higher
 // under the same V_t objective, so it can only help.
 let hist_seed = if cfg.use_history_seed {
 Some(history_aware_seed(challenge, state, cfg, history))
 } else {
 None
 };

 // Seed-driven dispatch: take the heuristic action directly and only enforce
 // network feasibility. The V_t grid scan + ASCA/LP below re-optimise against
 // the DA-only value function, which reverts the realized-RT-exploiting
 // actions (spike-discharge / dip-charge) the seed exists to make — so when
 // the seed drives, we skip them and finish with feasibility passes alone.
 if cfg.history_seed_drives {
 if let Some(seed) = &hist_seed {
 for i in 0..n_bat {
 let (lo, hi) = state.action_bounds[i];
 action[i] = seed[i].clamp(lo, hi);
 }
 }
 if cfg.deflator_iters > 0 {
 run_deflator(challenge, state, cache, cfg, &mut action);
 }
 return shrink_until_feasible(challenge, state, action, cfg.shrink_factor);
 }
 ...
 // Grid scan seeds the per-battery action.
 ...
 // Blend in the history-aware candidate at the same V_t objective. It is
 // evaluated against the *full* action bounds (the heuristic already
 // accounts for deliverability), so it can reach beyond the derated grid
 // envelope when the realized-price signal justifies it.
 if let Some(seed) = &hist_seed {
 let u_h = seed[i].clamp(full_u_min, full_u_max);
 let ns = next_soc(u_h, soc, battery);
 let candidate =
 battery_profit(u_h, rt_price, battery.capacity_mwh) + table.lookup(t_next, ns);
 if candidate > best_score {
 best_score = candidate;
 chosen_u = u_h;
 }
 ...
 // Joint LP: replace `action` with the LP-dispatch only when the LP
 // proposes a value-improving feasible action. Compares per-step
 // value (immediate profit + V_{t+1}) of the ASCA result vs the
 // LP result; tie-broken in favour of ASCA so a marginal LP
 // proposal can't burn warm-mu state on no real gain.
 if cfg.use_joint_lp {
 let zero_action = vec![0.0_f64; n_bat];
 let inj_base = challenge.compute_total_injections(state, &zero_action);
 let flows_base = challenge.network.compute_flows(&inj_base);
 if let Some(mut lp_act) = joint_lp_dispatch(challenge, state, cache, cfg, &flows_base) {
 // Snap LP-proposed action into the full bounds before
 // running the deflator safety net (the LP can produce
 // float-imprecise overshoot at the bounds).
 for b in 0..n_bat {
 let (lo, hi) = state.action_bounds[b];
 lp_act[b] = lp_act[b].clamp(lo, hi);
 }
 if cfg.deflator_iters > 0 {
 run_deflator(challenge, state, cache, cfg, &mut lp_act);
 }
 // Accept the LP candidate only when feasible and strictly above
 // the ASCA per-step value.
 if is_feasible(challenge, state, &lp_act) {
 let val_asca = total_value(challenge, state, cache, &action);
 let val_lp = total_value(challenge, state, cache, &lp_act);
 if val_lp > val_asca {
 action = lp_act;
 }
 }
...
/// Sum per-step value (immediate profit + V_{t+1}(soc_next)) across all
/// batteries for the candidate dispatch. Mirrors what `vt_value_function_policy`
/// implicitly maximises per battery, used as a tiebreaker between ASCA and
/// joint-LP candidates in `compute_action_vt`.
fn total_value(
 challenge: &Challenge,
 state: &State,
 cache: &VtCache,
 ...
 let row_start = t_next * table.soc_levels;
 let v_next = &table.values[row_start..row_start + table.soc_levels];
 let price = state.rt_prices[battery.node];
 acc += eval_runtime(state.socs[b], action[b], price, &sc, v_next);
 }
 acc
}
...
#[inline]
/// Joint LP dispatch: solve a single LP over per-battery (discharge, charge)
/// legs with PTDF flow constraints and SOC + bound feasibility, returning the
/// LP-recommended action vector or `None` on solver failure / infeasible.
///
/// The LP linearises the per-step quadratic degradation term — it appears in
/// the objective only as the linear `dv/eta_d` SOC-marginal-value the DP
/// table already encodes. Degradation re-enters truthfully when the candidate
/// is scored by `total_value` (which calls `eval_runtime`, which uses the
/// true quadratic). Honours `cfg.flow_margin` by tightening per-line limits,
/// matching the same convention `run_asca` / `run_deflator` already use.
/// Solver lives in the `lp_simplex` submodule at the bottom of this file.
fn joint_lp_dispatch(
 ...
 let mut a_mat: Vec<Vec<f64>> = vec![vec![0.0_f64; n]; m];
 ...
 let sc = BatScratch::from(bat);
 let table = &cache.tables[b];
 let soc = state.socs[b];
 let row_start = t_next * table.soc_levels;
 let v_row = &table.values[row_start..row_start + table.soc_levels];
 let dv = vt_slope_at(v_row, soc, sc.soc_min, sc.soc_span, table.soc_levels);

 let node = bat.node;
 let rt = if node < state.rt_prices.len() { state.rt_prices[node] } else { 0.0 };

 // Discharge leg: u > 0. Revenue rt*u*dt; tx -tx_cost*u*dt; SOC
 // delta drops by u/eta_d*dt, so V_{t+1} drops by dv/eta_d*dt.
 c_obj[b] = (rt - KAPPA_TX) * DELTA_T - dv / sc.eff_d * DELTA_T;
 // Charge leg: u < 0 → c = -u > 0. Cost +(rt+tx_cost)*c*dt with
 // sign flip; SOC delta rises by eta_c*c*dt, so V_{t+1} rises by
 // dv*eta_c*dt.
 c_obj[num_b + b] = -(rt + KAPPA_TX) * DELTA_T + dv * sc.eff_c * DELTA_T;

 let (u_min, u_max) = state.action_bounds[b];
 let r = 4 * b;
 // d_b <= u_max (discharge power bound; u_max >= 0)
 a_mat[r][b] = 1.0;
 b_vec[r] = u_max.max(0.0);
 // c_b <= -u_min (charge power bound; u_min <= 0 so -u_min >= 0)
 a_mat[r + 1][num_b + b] = 1.0;
 b_vec[r + 1] = (-u_min).max(0.0);
 // d_b/eta_d - c_b*eta_c <= soc - soc_min (SOC lower bound)
 a_mat[r + 2][b] = DELTA_T / sc.eff_d;
 a_mat[r + 2][num_b + b] = -sc.eff_c * DELTA_T;
 b_vec[r + 2] = (soc - sc.soc_min).max(0.0);
 // -d_b/eta_d + c_b*eta_c <= soc_max - soc (SOC upper bound)
 a_mat[r + 3][b] = -DELTA_T / sc.eff_d;
 a_mat[r + 3][num_b + b] = sc.eff_c * DELTA_T;
 b_vec[r + 3] = (sc.soc_max - soc).max(0.0);
 }
 ...
 let limit = challenge.network.flow_limits[l];
 if limit <= 1e-6 {
 // Skip the constraint pair: leave the row at "0 <= 0" so
 // the simplex's b >= 0 check stays satisfied.
 continue;
 }
 let exo = flows_base[l];
 let limit_eff = (limit - cfg.flow_margin).max(0.0);
 let rp = row_f + 2 * l;
 let rn = rp + 1;
 for &(b, impact) in &cache.ptdf_sparse[l] {
 // Forward flow direction: Σ ptdf*(d - c) <= limit_eff - exo
 a_mat[rp][b] += impact;
 a_mat[rp][num_b + b] -= impact;
 // Reverse flow direction: Σ ptdf*(c - d) <= limit_eff + exo
 a_mat[rn][b] -= impact;
 a_mat[rn][num_b + b] += impact;
 }
 b_vec[rp] = (limit_eff - exo).max(0.0);
 b_vec[rn] = (limit_eff + exo).max(0.0);
 }

 let opt_x = lp_simplex::lp_solve(n, m, &c_obj, &a_mat, &b_vec)?;
...
/// Slope of the value function row at `soc` using the cell containing
/// `soc`. Equivalent to `(v[idx0+1] - v[idx0]) / delta_soc`. Used in the
/// joint-LP objective to keep the linearisation consistent with the
/// per-cell shape the DP table already uses.
#[inline]
fn vt_slope_at(v_row: &[f64], soc: f64, soc_min: f64, soc_span: f64, soc_levels: usize) -> f64 {
 if soc_levels < 2 {
 return 0.0;
 }
 let idx_f = (soc - soc_min) / soc_span * ((soc_levels - 1) as f64);
 let idx0 = (idx_f.floor() as isize).max(0) as usize;
 let idx0 = idx0.min(soc_levels - 2);
 let delta_soc = soc_span / ((soc_levels - 1) as f64);
 (v_row[idx0 + 1] - v_row[idx0]) / delta_soc.max(1e-12)
}

/// Minimal dense-simplex LP solver, isolated so the surrounding helpers
/// stay easy to grep. Solves: maximise c^T x s.t. A x &lt;= b, x &gt;= 0.
/// Assumes b_i &gt;= 0 (returns None otherwise). Pure `std` — no extra
/// crates.
mod lp_simplex {
