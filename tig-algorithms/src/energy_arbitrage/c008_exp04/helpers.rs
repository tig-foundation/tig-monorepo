...
const RT_MOMENTUM_ALPHA: f64 = 0.25;
const RT_MOMENTUM_THRESHOLD: f64 = 0.08;
const RT_MOMENTUM_MAX_BIAS: f64 = 150.0;
...
impl TrackHp {
    pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
        ...
        if let Some(v) = m.get("rt_momentum_scale").and_then(|v| v.as_f64()) {
            self.rt_momentum_scale = v.clamp(0.0, 2.0);
        }
    }
}
...
fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
    STATE.with(|s| -> Result<Vec<f64>> {
        ...
        let hp = &inner.hp;
        let momentum_price_biases = update_rt_momentum(
            challenge,
            state,
            cache,
            &mut inner.rt_ema,
            hp.rt_momentum_scale,
        );
...
fn update_rt_momentum(
    ...
    if rt_ema.len() != state.rt_prices.len() {
        rt_ema.clear();
        let da_row = challenge
            .market
            .day_ahead_prices
            .get(state.time_step)
            .or_else(|| challenge.market.day_ahead_prices.last());
        for node in 0..state.rt_prices.len() {
            let fallback = state.rt_prices[node];
            let da = da_row
    ...
    let mut node_biases = vec![0.0; state.rt_prices.len()];
    for node in 0..state.rt_prices.len() {
        let current = state.rt_prices[node];
        let previous = rt_ema[node];
        let delta = current - previous;
        let rel = delta / previous.abs().max(10.0);
        if rel.abs() > RT_MOMENTUM_THRESHOLD {
            node_biases[node] =
                (momentum_scale * delta).clamp(-RT_MOMENTUM_MAX_BIAS, RT_MOMENTUM_MAX_BIAS);
        }
        rt_ema[node] = RT_MOMENTUM_ALPHA * current + (1.0 - RT_MOMENTUM_ALPHA) * previous;
    }

    ca.batt_nodes
        .iter()
        .map(|&node| node_biases.get(node).copied().unwrap_or(0.0))
        .collect()
}
...
fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> TitanCache {
    ...
    for b in 0..num_b {
        ...
        for t in (0..num_t).rev() {
            ...
            let p_expected = expected_rt_price(challenge, p_da, extra, t, node, b);
            let p_sell = p_expected * (1.0 + hp.jump_premium);
            let p_buy = p_expected;
...
fn expected_rt_price(
    ...
    let alpha = challenge.market.params.tail_index.max(1.01);
    let mut rng = SplitMix64::new(
        ((time_step as u64) << 32)
            ...
            ^ challenge.seed[0..8]
                .iter()
                .fold(0_u64, |acc, &x| (acc << 8) ^ x as u64),
    );
    let mut sum = 0.0;
    for _ in 0..N_SCENARIOS {
        let z_common = standard_normal(&mut rng);
        let eps_i = standard_normal(&mut rng);
        let rho = 0.70_f64;
        let xi_i = rho.sqrt() * z_common + (1.0 - rho).sqrt() * eps_i;
        let mut price = p_da * (1.0 + sigma * xi_i) + extra;
        if rng.next_f64() < rho_jump {
            let u = rng.next_f64().clamp(1e-10, 1.0 - 1e-10);
            price += p_da * (1.0 - u).powf(-1.0 / alpha);
        }
        sum += price.clamp(-200.0, 5000.0);
    }
...
fn eval_profit(
    ...
    let dt = 0.25_f64;
    let abs_u = u.abs();
    let effective_price = rt_price + momentum_price_bias;
    let revenue = u * effective_price * dt;
    let tx = 0.25 * abs_u * dt;
    let deg_base = (abs_u * dt) / bat.capacity_mwh.max(1e-9);
    let deg = deg_base * deg_base;
    let profit = revenue - tx - deg;
...
fn run_asca(
    ...
    for _sweep in 0..hp.asca_iters {
        ...
        for &b in &order {
            ...
            let momentum_price_bias = momentum_price_biases.get(b).copied().unwrap_or(0.0);
            let mut best_v = eval_profit(challenge, state, ca, b, best_u, momentum_price_bias);

            let v0 = eval_profit(challenge, state, ca, b, 0.0, momentum_price_bias);
            ...
            if u_min < 0.0 {
                ...
                if lo < hi {
                    let (u, v) = ternary_search(
                        |u| eval_profit(challenge, state, ca, b, u, momentum_price_bias),
                        lo,
                        hi,
                        hp.ternary_iters,
                    );
...
fn potential(challenge: &Challenge, state: &State, ca: &TitanCache, b: usize) -> f64 {
    ...
    let v_lo = eval_profit(challenge, state, ca, b, u_lo, 0.0);
    let v_hi = eval_profit(challenge, state, ca, b, u_hi, 0.0);
    let v0 = eval_profit(challenge, state, ca, b, 0.0, 0.0);
        ...
        for l in 0..num_l {
            ...
            for &(b, impact) in &ca.ptdf_sparse[l] {
                ...
                if contrib * sign > 1e-9 {
                    let momentum_price_bias = momentum_price_biases.get(b).copied().unwrap_or(0.0);
                    let val_curr =
                        eval_profit(challenge, state, ca, b, actions[b], momentum_price_bias);
                    let val_zero = eval_profit(challenge, state, ca, b, 0.0, momentum_price_bias);
