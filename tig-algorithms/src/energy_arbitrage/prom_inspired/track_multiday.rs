...
/// Approximates the marginal continuation value from the next-step SOC slice.
...
/// Pushes a bounded candidate once so 1D action searches stay small and stable.
fn push_unique_candidate(candidates: &mut Vec<f64>, value: f64, lo: f64, hi: f64) {
    ...
    if candidates
        .iter()
        .all(|&x| (x - v).abs() > 1e-9 * (1.0 + x.abs().max(v.abs())))
    {
        candidates.push(v);
    }
}

/// Builds a compact 1D candidate set from bounds, local stationary points, and
/// nearby DP SOC-grid kinks instead of scanning a dense uniform action grid.
fn targeted_action_candidates(
    dp: &BatteryDP,
    battery: &Battery,
    t: usize,
    soc: f64,
    ...
    push_unique_candidate(&mut candidates, current, lo, hi);
    push_unique_candidate(&mut candidates, 0.0, lo, hi);
    push_unique_candidate(&mut candidates, lo, lo, hi);
    push_unique_candidate(&mut candidates, hi, lo, hi);
    if hi - lo > EPS {
        push_unique_candidate(&mut candidates, 0.5 * (lo + hi), lo, hi);
    }

    let center = current.clamp(lo, hi);
    let next_soc = battery.apply_action_to_soc(center, soc);
    let dv = dv_dsoc(dp, t, next_soc);
    let cap2 = battery.capacity_mwh.powi(2).max(1e-9);
    let quad = 2.0 * KAPPA_DEG * DELTA_T * DELTA_T / cap2;

    if quad > 1e-14 {
        if hi > 0.0 {
            let discharge_root =
                (price * DELTA_T - KAPPA_TX * DELTA_T + dv * (-DELTA_T / ETA_DISCHARGE)) / quad;
            if discharge_root.is_finite() {
                push_unique_candidate(&mut candidates, discharge_root, lo.max(0.0), hi);
            }
        }
        if lo < 0.0 {
            let charge_root =
                (price * DELTA_T + KAPPA_TX * DELTA_T + dv * (-ETA_CHARGE * DELTA_T)) / quad;
            if charge_root.is_finite() {
                push_unique_candidate(&mut candidates, charge_root, lo, hi.min(0.0));
            }
        }
    }

    let last = dp.levels - 1;
    let pos = ((next_soc - dp.soc_lo) * dp.soc_step_inv).clamp(0.0, last as f64);
    let base_idx = pos.floor() as isize;
    for idx in [base_idx - 1, base_idx, base_idx + 1, base_idx + 2] {
        if idx < 0 || idx > last as isize {
            continue;
        }
        let grid_soc = dp.soc_lo + (idx as f64) / dp.soc_step_inv;
        if hi > 0.0 && battery.efficiency_discharge > 0.0 {
            let u = (soc - grid_soc) * battery.efficiency_discharge / DELTA_T;
            if u >= 0.0 {
                push_unique_candidate(&mut candidates, u, lo.max(0.0), hi);
            }
        }
        if lo < 0.0 && battery.efficiency_charge > 0.0 {
            let u = (soc - grid_soc) / (battery.efficiency_charge * DELTA_T);
            if u <= 0.0 {
                push_unique_candidate(&mut candidates, u, lo, hi.min(0.0));
            }
        }
    }
...
/// Queries the per-battery DP with a derivative-guided 1D candidate set rather
/// than a dense policy-time action grid.
fn pick_dp_action(
    dp: &BatteryDP,
    battery: &Battery,
    t: usize,
    ...
    for action in targeted_action_candidates(dp, battery, t, soc, price, lo, hi, best_action) {
...
/// Precomputes each battery's signed PTDF effect on every transmission line.
...
/// Converts near-binding baseline line congestion into battery-specific
/// effective RT prices by subtracting signed PTDF shadow penalties.
fn congestion_effective_prices(
    challenge: &Challenge,
    sens: &[Vec<f64>],
    base_flows: &[f64],
    rt_prices: &[f64],
) -> (Vec<f64>, f64) {
    let mut max_abs_price = 0.0_f64;
    for &price in rt_prices {
        max_abs_price = max_abs_price.max(price.abs());
    }
    let price_scale = max_abs_price.clamp(20.0, 90.0);

    let mut max_base_ratio = 0.0_f64;
    let mut shadow = vec![0.0_f64; challenge.num_batteries];
    for l in 0..challenge.network.num_lines {
        let limit = challenge.network.flow_limits[l].max(1e-9);
        let flow = base_flows[l];
        let ratio = (flow.abs() / limit).clamp(0.0, 5.0);
        max_base_ratio = max_base_ratio.max(ratio);

        let tight = ((ratio - 0.80) / 0.20).clamp(0.0, 2.0);
        if tight <= 0.0 {
            continue;
        }
        let sign = if flow >= 0.0 { 1.0 } else { -1.0 };
        let line_penalty = price_scale * 0.35 * tight * tight;
        for b in 0..challenge.num_batteries {
            shadow[b] += line_penalty * sign * sens[l][b];
        }
    }

    let shadow_cap = 0.65 * price_scale;
    let mut effective_prices = vec![0.0_f64; challenge.num_batteries];
    for b in 0..challenge.num_batteries {
        let raw_price = rt_prices[challenge.batteries[b].node];
        effective_prices[b] = raw_price - shadow[b].clamp(-shadow_cap, shadow_cap);
    }
    (effective_prices, max_base_ratio)
}

/// Uses congestion-adjusted price only when baseline lines are tight and the
/// battery-specific shadow shift is large enough to matter economically.
fn dp_query_price(raw_price: f64, effective_price: f64, max_base_ratio: f64) -> f64 {
    let shadow_shift = effective_price - raw_price;
    let shift_gate = (2.5 * KAPPA_TX).max(0.06 * raw_price.abs().max(15.0));
    if max_base_ratio <= 0.84 || shadow_shift.abs() <= shift_gate {
        raw_price
    } else {
        let tight = ((max_base_ratio - 0.84) / 0.16).clamp(0.0, 1.0);
        raw_price + tight * shadow_shift
    }
}

/// Smoothly blends the market action toward the average per-step action needed
/// to reach a simple end-of-horizon SOC target without forcing bad prices.
fn blend_terminal_schedule(
    battery: &Battery,
    soc: f64,
    bounds: (f64, f64),
    n_remaining: usize,
    ...
    let (u_min, u_max) = bounds;
    if n_remaining == 0 {
        return market_action.clamp(u_min, u_max);
    }

    let span = (battery.soc_max_mwh - battery.soc_min_mwh).max(1e-9);
    let price_band = (q_high - q_low).abs().max(8.0);
    let midpoint = 0.5 * (q_low + q_high);
    let fullness = relative_soc_pressure(battery, soc);
    let reserve_support = ((n_remaining as f64 - 8.0) / 32.0).clamp(0.0, 1.0);
    let cheap_hold = if allow_charge {
        ((midpoint - current_price) / price_band).clamp(0.0, 1.0)
    } else {
        0.0
    };
    let reserve_frac =
        (0.12 * reserve_support * cheap_hold * (1.0 - 0.35 * fullness)).clamp(0.0, 0.12);
    let target_soc = battery.soc_min_mwh + reserve_frac * span;

    let avg_steps = (n_remaining as f64).max(1.0);
    let schedule = if soc > target_soc + 1e-9 && u_max > 0.0 {
        ((soc - target_soc) * battery.efficiency_discharge / (avg_steps * DELTA_T))
            .clamp(0.0, u_max)
    } else if allow_charge
        && soc < target_soc - 1e-9
        && u_min < 0.0
        && battery.efficiency_charge > 0.0
    {
        (-(target_soc - soc) / (battery.efficiency_charge * avg_steps * DELTA_T))
            .clamp(u_min, 0.0)
    } else {
        0.0
    };
    ...
    let gap_frac = ((soc - target_soc).abs() / span).clamp(0.0, 1.0);
    let urgency = (1.0 - ((n_remaining as f64 - 1.0) / 36.0).clamp(0.0, 1.0)).clamp(0.0, 1.0);
    let favorability = if schedule > 0.0 {
        ((current_price - q_low) / price_band).clamp(0.0, 1.0)
    } else {
        ((q_high - current_price) / price_band).clamp(0.0, 1.0)
    };
    let weight = (gap_frac * (0.10 + 0.65 * urgency + 0.25 * favorability)).clamp(0.0, 0.95);

    let blended = if market_action.abs() <= EPS {
        (0.35 + 0.65 * weight) * schedule
    } else if market_action.signum() == schedule.signum() {
        if market_action.abs() >= schedule.abs() {
            market_action
        } else {
            let mix = (0.25 + 0.75 * weight).clamp(0.0, 1.0);
            market_action + mix * (schedule - market_action)
        }
    } else {
        (1.0 - weight) * market_action + weight * schedule
    };

    blended.clamp(u_min, u_max)
}
...
/// Chooses full optimization only when congestion or terminal urgency justify
/// the fuel, otherwise downgrading to a direct DP or heuristic seed.
fn select_policy_mode(
    challenge: &Challenge,
    max_base_ratio: f64,
    n_remaining: usize,
    fuel_left: u64,
    ...
    let hard_floor = reserve_floor.max(1);
    if fuel_left <= hard_floor {
        return PolicyMode::Zero;
    }

    let very_slack = max_base_ratio < 0.45;
    let slack = max_base_ratio < 0.60;
    let congested = max_base_ratio >= 0.88;
    let late = n_remaining <= 8;
    let very_late = n_remaining <= 3;
    let dense = challenge.num_batteries >= 8 || challenge.network.num_lines >= 16;
    let conserving = fuel_left <= budget_floor.saturating_add(hard_floor / 2);
    let scarce = fuel_left <= budget_floor.max(hard_floor.saturating_mul(3) / 2);

    if very_late || congested {
        return if scarce && !very_late {
            PolicyMode::DirectTarget
        } else {
            PolicyMode::Full
        };
    }

    if very_slack && (dense || n_remaining > 12) {
        return PolicyMode::DirectDp;
    }

    if slack {
        if late {
            return PolicyMode::DirectTarget;
        }
        if dense || conserving {
            return PolicyMode::DirectDp;
        }
...
/// Initializes and appends the current RT observation so cheap gated paths keep
/// the rolling history consistent without paying for history-derived signals.
fn push_history_observation(challenge: &Challenge, state: &State) {
    ...
    for node in 0..challenge.network.num_nodes {
        ...
        history.residuals[node]
            .push(state.rt_prices[node] - challenge.market.day_ahead_prices[state.time_step][node]);
    }
}

/// Projects a direct seed once and preserves the cheap path's validity with the
/// same safety fallback used by the heavier joint optimizer.
fn finalize_direct_action(
    challenge: &Challenge,
    state: &State,
    mut action: Vec<f64>,
    sens: &[Vec<f64>],
...
/// Filters the raw gradient into the locally free cone by removing components
/// that push farther into active box bounds or near-binding line constraints.
fn active_set_direction(
    challenge: &Challenge,
    state: &State,
    sens: &[Vec<f64>],
    base_flows: &[f64],
    ...
    let m = action.len();
    let mut dir = grad.to_vec();
    let mut free = vec![true; m];

    for b in 0..m {
        let (lo, hi) = state.action_bounds[b];
        let tol = 1e-8 * (hi - lo).abs().max(1.0);
        if (action[b] - lo).abs() <= tol && dir[b] < 0.0 {
            dir[b] = 0.0;
            free[b] = false;
        } else if (hi - action[b]).abs() <= tol && dir[b] > 0.0 {
            dir[b] = 0.0;
            free[b] = false;
        }
    }

    let mut flows = Vec::with_capacity(sens.len());
    for l in 0..sens.len() {
        flows.push(line_flow(&sens[l], action, base_flows[l]));
    }
    ...
    for _ in 0..2 {
        ...
        for l in 0..sens.len() {
            ...
            let flow = flows[l];
            let active_tol = 0.02 * limit.max(1.0) + 1e-6;
            if limit - flow.abs() > active_tol {
                continue;
            }
            ...
            let mut norm_sq = 0.0;
            for b in 0..m {
                let coeff = sign * sens[l][b];
                if !free[b] || coeff.abs() <= 1e-10 {
                    continue;
                }
                dot += coeff * dir[b];
                norm_sq += coeff * coeff;
            }
            if dot <= 1e-12 || norm_sq <= 1e-14 {
                continue;
            }
...
/// Runs projected ascent with an active-set-aware direction so line search
/// avoids steps that projection would mostly cancel under tight constraints.
fn projected_gradient_ascent(
    ...
    let mut action = seed;
    safe_project_to_feasible(challenge, state, &mut action, sens, base_flows, hp);
    let mut current_value = total_step_value(challenge, state, dps, &action);
    ...
    for _ in 0..hp.grad_outer_iters {
        ...
        let dir = active_set_direction(challenge, state, sens, base_flows, &action, &grad);
        ...
        for _ in 0..hp.grad_ls_iters {
            ...
            let move_norm_sq: f64 = trial
                .iter()
                .zip(action.iter())
                .map(|(x, a)| (x - a) * (x - a))
                .sum();
            if move_norm_sq <= 1e-14 * (1.0 + cur_lr * cur_lr) {
                cur_lr *= 0.5;
                continue;
            }

            let v = total_step_value(challenge, state, dps, &trial);
            if v > current_value + 1e-9 {
                action = trial;
                current_value = v;
...
/// Incrementally polishes each battery over its exact PTDF-feasible interval
/// using a small derivative-guided candidate set and flow updates in place.
fn coordinate_polish_step(
    ...
    let mut battery_values: Vec<f64> = (0..challenge.num_batteries)
        .map(|b| {
            let battery = &challenge.batteries[b];
            dp_action_value(
                &dps[b],
    ...
    for _ in 0..hp.coord_polish_passes {
        ...
        for b in 0..challenge.num_batteries {
            ...
            if net_hi - net_lo <= EPS {
                continue;
            }

            let current_value = battery_values[b];
            let mut best_b_action = cur;
            let mut best_b_value = current_value;
            let mut best_b_total = best_value;

            for candidate in targeted_action_candidates(
                &dps[b],
                battery,
                state.time_step,
                state.socs[b],
                ...
                let candidate_value =
                    dp_action_value(&dps[b], battery, state.time_step, state.socs[b], price, candidate);
                let total_value = best_value - current_value + candidate_value;
                if total_value > best_b_total + 1e-9 {
                    best_b_total = total_value;
                    best_b_value = candidate_value;
            ...
            if (best_b_action - cur).abs() > EPS {
                ...
                battery_values[b] = best_b_value;
                best_value = best_b_total;
                for l in 0..challenge.network.num_lines {
...
/// Applies regime-aware fuel gating: slack states use a direct seed, while
/// congested or urgent states still receive the full joint refinement path.
fn policy(
    challenge: &Challenge,
    state: &State,
    dps: &[BatteryDP],
    ...
    // Baseline flows depend only on this step's exogenous injections.
    let zero = vec![0.0_f64; challenge.num_batteries];
    let base_flows = compute_flows(challenge, state, &zero);
    let (effective_prices, max_base_ratio) =
        congestion_effective_prices(challenge, sens, &base_flows, &state.rt_prices);
    let mode = select_policy_mode(
        challenge,
        max_base_ratio,
        n_remaining,
        fuel_remaining(),
    ...
    if mode == PolicyMode::Zero {
        return Ok(zero);
    }

    if mode == PolicyMode::DirectDp {
        let dp_seed: Vec<f64> = (0..challenge.num_batteries)
            .map(|b| {
                let battery = &challenge.batteries[b];
                let raw_price = state.rt_prices[battery.node];
                let price = dp_query_price(raw_price, effective_prices[b], max_base_ratio);
                pick_dp_action(
                    &dps[b],
                    battery,
                    t,
            ...
            .collect();
        push_history_observation(challenge, state);
        return Ok(finalize_direct_action(
            challenge,
            state,
            dp_seed,
            sens,
    ...
    for (b, battery) in challenge.batteries.iter().enumerate() {
        ...
        let raw_price = state.rt_prices[node];
        let current_price = if n_remaining <= 4 {
            raw_price
        } else {
            effective_prices[b]
        };
        ...
        let mut a = 0.0_f64;
        if u_max > 0.0 && current_price > discharge_min {
        ...
        target[b] = blend_terminal_schedule(
            battery,
            state.socs[b],
            state.action_bounds[b],
            n_remaining,
    ...
    if mode == PolicyMode::DirectTarget {
        return Ok(finalize_direct_action(
            challenge,
            state,
            target,
    ...
    // Joint projected gradient ascent over a pure terminal-aware heuristic seed
    // late in slack regimes, a congestion-aware DP seed earlier in slack
    // regimes, and both seeds only when congestion is material.
    let seeds = if max_base_ratio < 0.60 && n_remaining <= 12 {
        vec![target]
    } else {
        let dp_seed: Vec<f64> = (0..challenge.num_batteries)
            .map(|b| {
...
pub fn solve_challenge(
    ...
    // Reserve ~1/28 of the post-setup fuel as a hard safety floor, but replace
    // the old single static cutoff with regime-aware gating inside `policy` so
    // slack states can spend cheap direct seeds and save fuel for hard states.
    let available = fuel_remaining();
    let reserve_floor = available / 28;
    let max_spend = available.saturating_sub(reserve_floor);
    let target_spend = if hp.fuel_budget == 0 {
        max_spend
    } else {
        hp.fuel_budget.min(max_spend)
    };
    let budget_floor = available - target_spend;
    let solution = challenge.grid_optimize(&|c, s| {
        if fuel_remaining() <= reserve_floor {
            return Ok(vec![0.0; c.num_batteries]);
        }
        policy(c, s, &dps, &sens, &hp, budget_floor, reserve_floor)
    })?;
