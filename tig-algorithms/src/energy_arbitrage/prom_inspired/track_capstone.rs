...
fn build_battery_dp(
    ...
    let deg_factor = KAPPA_DEG / battery.capacity_mwh.powi(2);
    ...
    for t in (0..num_steps).rev() {
        ...
        for s_idx in 0..levels {
            ...
            for &raw in &actions {
                ...
                let tx_cost = KAPPA_TX * throughput;
                let deg_cost = deg_factor * throughput * throughput;

                best_low = best_low.max(action * price_low * DELTA_T - tx_cost - deg_cost + future);
                best_high = best_high.max(action * price_high * DELTA_T - tx_cost - deg_cost + future);
                best_jump_low = best_jump_low.max(action * price_jump_low * DELTA_T - tx_cost - deg_cost + future);
                best_jump_high = best_jump_high.max(action * price_jump_high * DELTA_T - tx_cost - deg_cost + future);
            }
...
fn dp_action_value(
    ...
    let next_soc = battery.apply_action_to_soc(action, soc);
    let throughput = action.abs() * DELTA_T;
    let profit = action * price * DELTA_T - KAPPA_TX * throughput - dp.deg_factor * throughput * throughput;
    profit + interp_value(
            &dp.values[next_t],
            next_soc,
            dp.soc_lo,
...
fn coordinate_polish_step(
    ...
    for _ in 0..hp.coord_polish_passes {
        ...
        for b in 0..challenge.num_batteries {
            ...
            let cur_value = dp_action_value(&dps[b], battery, state.time_step, state.socs[b], price, cur);
            let base_value_excl_b = best_value - cur_value;
            ...
            for &candidate in candidates.iter() {
                ...
                for l in 0..n_lines {
                    let trial_flow = flows[l] + sens[l][b] * delta;
                    if trial_flow.abs() > limits[l] + limits[l].max(1.0) * EPS_FLOW {
                        feasible = false;
                        break;
                    }
                }
                ...
                let candidate_value = dp_action_value(&dps[b], battery, state.time_step, state.socs[b], price, candidate);
                let value = base_value_excl_b + candidate_value;
...
fn policy(
    ...
    for node in 0..challenge.network.num_nodes {
        if history.values[node].len() >= 16 {
            let vals = &mut history.values[node];
            vals.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            let q15 = percentile(vals, 15, 100);
            let q85 = percentile(vals, 85, 100);
            if q85 - q15 > 2.0 {
                rt_bands[node] = Some((q15, q85));
            }
        }
        if history.residuals[node].len() >= 8 {
            let recent = history.residuals[node].last().copied().unwrap_or(0.0);
            history.residuals[node].sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            let median = percentile(&history.residuals[node], 50, 100);
