...
mod helpers {
    ...
    impl TrackHp {
        pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
            ...
            if let Some(v) = m.get("lp_flat_tableau").and_then(|v| v.as_bool()) { self.lp_flat_tableau = v; }
            if let Some(v) = m.get("lp_dantzig").and_then(|v| v.as_bool()) { self.lp_dantzig = v; }
            if let Some(v) = m.get("lp_master_sparse").and_then(|v| v.as_bool()) { self.lp_master_sparse = v; }
            if let Some(v) = m.get("lp_probe").and_then(|v| v.as_bool()) { self.lp_probe = v; }
        }
    }
    ...
    pub fn solve_with_hp(
        ...
        let probe = hp_probe_flag();
        super::lp::probe_reset();
        let out = challenge.grid_optimize(&policy_entry);
        if probe {
            let (solves, pivots, maxpiv, area) = super::lp::probe_report();
            eprintln!(
                "[T49LP] solves={} pivots={} maxpiv={} area={} mean_piv={:.2}",
                solves, pivots, maxpiv, area,
                if solves > 0 { pivots as f64 / solves as f64 } else { 0.0 }
            );
        }
    ...
    fn hp_probe_flag() -> bool {
        STATE.with(|s| s.borrow().as_ref().map(|i| i.hp.lp_probe).unwrap_or(false))
    }
    ...
    fn run_column_generation(
        ...
        for _iter in 0..hp.cg_iters {
            ...
            let (sol, duals, _pivots) = if hp.lp_flat_tableau || hp.lp_dantzig {
                super::lp::lp_solve_with_duals_flat(
                    master.nvars, master.ncons, &master.c, &master.a, &master.b, 3000, hp.lp_dantzig,
                )
            } else {
                super::lp::lp_solve_with_duals(
        ...
        let (sol, final_duals_opt, _) = if hp.lp_flat_tableau || hp.lp_dantzig {
            super::lp::lp_solve_with_duals_flat(
                master.nvars, master.ncons, &master.c, &master.a, &master.b, 3000, hp.lp_dantzig,
            )
        } else {
    ...
    fn build_master_lp(
        ...
        for l in 0..num_l {
            ...
            if hp.lp_master_sparse {
                for &(b, impact) in &ca.ptdf_sparse[l] {
                    let start = col_start[b];
                    for j in 0..columns[b].len() {
                        a_mat[rp][start + j] += impact * columns[b][j];
                    }
                }
            } else {
            for b in 0..num_b {
...
mod lp {
    ...
    pub fn lp_solve_with_duals(
        ...
        probe_note(pivots_used, (m + 1) * n_cols);
    ...
    thread_local! {
        static PROBE: std::cell::Cell<(u64, u64, u64, u64)> = std::cell::Cell::new((0, 0, 0, 0));
    }

    pub fn probe_reset() {
        PROBE.with(|p| p.set((0, 0, 0, 0)));
    }

    pub fn probe_report() -> (u64, u64, u64, u64) {
        PROBE.with(|p| p.get())
    }

    #[inline]
    fn probe_note(pivots: usize, area: usize) {
        PROBE.with(|p| {
            let (s, tot, mx, ar) = p.get();
            let pv = pivots as u64;
            p.set((s + 1, tot + pv, if pv > mx { pv } else { mx }, ar + area as u64));
        });
    }

    pub fn lp_solve_with_duals_flat(
        ...
        let mut x = vec![0.0_f64; n];
        let mut in_basis = vec![false; n_vars];
        for (i, &bv) in basis.iter().enumerate() {
            if bv < n {
        ...
        for i in 0..m {
            let slack_col = n + i;
            if !in_basis[slack_col] {
                y[i] = -tab[obj_r + slack_col];
                if y[i].abs() < LP_EPS { y[i] = 0.0; }
            } else {
                y[i] = 0.0;
            }
