...
mod helpers {
    ...
    pub struct TrackHp {
        ...
        pub oco_dp_res_div: usize,
        pub oco_dp_res_mul: usize,
        pub reclaim_noop_dw: bool,
        pub reclaim_dead_det_dp: bool,
        pub dw_extra_pivot_budget: usize,
        pub sdp_k: usize,
    ...
    impl TrackHp {
        pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
            ...
            if let Some(v) = m.get("oco_dp_res_div").and_then(|v| v.as_u64()) { self.oco_dp_res_div = (v as usize).max(1).min(8); }
            if let Some(v) = m.get("oco_dp_res_mul").and_then(|v| v.as_u64()) { self.oco_dp_res_mul = (v as usize).max(1).min(4); }
            if let Some(v) = m.get("reclaim_noop_dw").and_then(|v| v.as_bool()) { self.reclaim_noop_dw = v; }
            if let Some(v) = m.get("reclaim_dead_det_dp").and_then(|v| v.as_bool()) { self.reclaim_dead_det_dp = v; }
            if let Some(v) = m.get("dw_extra_pivot_budget").and_then(|v| v.as_u64()) { self.dw_extra_pivot_budget = v as usize; }
    ...
    fn build_dw_dual_prices(
        ...
        'dw_outer: for _dw_iter in 0..hp.dw_iters {
            ...
            if total_pivots_used > hp.dw_total_pivot_budget.saturating_add(hp.dw_extra_pivot_budget) {
                break 'dw_outer;
            }
    ...
    fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> AycdicdbCache {
        ...
        let line_weights: Vec<f64> = {
            if hp.use_cos_cs_weights {
                ...
                let mut mean_proba = vec![0.0_f64; num_l];
                let mut mean_excess = vec![0.0_f64; num_l];
                for t in 0..num_t {
                    let f_exo = challenge.network.compute_flows(&challenge.exogenous_injections[t]);
                    for l in 0..num_l {
                        let limit = challenge.network.flow_limits[l];
        ...
        let dp = if hp.use_dw && num_l > 0 {
            let dw_budget_eff = hp.dw_total_pivot_budget.saturating_add(hp.dw_extra_pivot_budget);
            let dw_is_noop = hp.reclaim_noop_dw
                && (hp.dw_iters == 0 || hp.dw_pivot_budget_per_solve > dw_budget_eff);
            let dw_solve = |ep: &Vec<Vec<f64>>| -> (Vec<Vec<Vec<f64>>>, Vec<Vec<f64>>) {
                let mu_dw = if dw_is_noop {
                ...
                build_dw_dual_prices(challenge, state, hp, &batt_nodes, &b_to_lines, ps)
                };
            ...
            if hp.use_ptdf_constraint_tracking && hp.coord_premium_mode == 0 && num_l > 0 {
                ...
                let oco_div = hp.oco_dp_res_div.max(1);
                let oco_mul = hp.oco_dp_res_mul.max(1);
                hp_oco.soc_levels = (hp.soc_levels * oco_mul / oco_div).max(31);
                hp_oco.action_grid = (hp.action_grid * oco_mul / oco_div).max(15);
    ...
    fn build_dp_parallel_or_serial(
        ...
        if hp.reclaim_dead_det_dp && hp.use_sdp {
...
mod lp {
    ...
    pub fn lp_solve_with_budget(
        ...
        for pivot in 0..max_pivots {
            ...
            let entering = match pick_entering(&tab[m], n_vars, false) {
                Some(j) => j,
                None => break,
            };
...
mod track_congested {
    ...
    fn defaults() -> TrackHp {
        TrackHp {
            ...
            oco_dp_res_div: 2,
            oco_dp_res_mul: 1,
            reclaim_noop_dw: false,
            reclaim_dead_det_dp: false,
            dw_extra_pivot_budget: 0,
            sdp_k: 3,
