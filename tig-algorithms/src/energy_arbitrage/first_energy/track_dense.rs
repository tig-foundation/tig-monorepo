...
static RT_RECENT_RESIDUALS: OnceLock<Mutex<Vec<f64>>> = OnceLock::new();
...
fn recent_residuals_lock() -> &'static Mutex<Vec<f64>> {
    RT_RECENT_RESIDUALS.get_or_init(|| Mutex::new(Vec::new()))
}

#[inline]
fn history_value_order(a: f64, b: f64) -> std::cmp::Ordering {
    a.partial_cmp(&b).unwrap_or(std::cmp::Ordering::Equal)
}

#[inline]
fn insert_sorted_history(sorted: &mut Vec<f64>, value: f64) {
    let mut lo = 0usize;
    let mut hi = sorted.len();
    while lo < hi {
        let mid = (lo + hi) >> 1;
        if history_value_order(sorted[mid], value) == std::cmp::Ordering::Greater {
            hi = mid;
        } else {
            lo = mid + 1;
        }
...
static PROJECTION_CACHE: OnceLock<Mutex<ProjectionCache>> = OnceLock::new();

fn projection_cache_lock() -> &'static Mutex<ProjectionCache> {
    PROJECTION_CACHE.get_or_init(|| {
        Mutex::new(ProjectionCache {
            num_lines: 0,
            num_batteries: 0,
            gram: Vec::new(),
            row_norm_sq: Vec::new(),
...
fn project_polytope(
    ...
    let n_b = action.len();
    let cache = projection_cache_lock().lock().unwrap();
    let use_cache = cache.num_lines == n_lines
        && cache.num_batteries == n_b
        && cache.gram.len() == n_lines * n_lines
        && cache.row_norm_sq.len() == n_lines;

    let mut current_flows = vec![0.0_f64; n_lines];
    for l in 0..n_lines {
        current_flows[l] = line_flow(&sens[l], action, base_flows[l]);
    }
    ...
    for _ in 0..max_iters {
        ...
        let norm_sq = if use_cache {
            cache.row_norm_sq[worst_l]
        } else {
        ...
        if use_cache {
            let col = worst_l;
            for l in 0..n_lines {
                current_flows[l] += step * cache.gram[l * n_lines + col];
            }
        } else {
            for l in 0..n_lines {
                let mut delta_f = 0.0_f64;
                for b in 0..n_b {
                    delta_f += sens[l][b] * row[b];
                }
                current_flows[l] += step * delta_f;
            }
        }
    }
...
fn actions_eps_equal(a: &[f64], b: &[f64]) -> bool {
    if a.len() != b.len() {
        return false;
    }
    for i in 0..a.len() {
...
fn canonicalize_and_dedup_seeds(
    challenge: &Challenge,
    state: &State,
    sens: &[Vec<f64>],
    base_flows: &[f64],
    ...
    let mut unique: Vec<Vec<f64>> = Vec::with_capacity(seeds.len());
    for mut seed in seeds {
        clamp_to_bounds(&mut seed, &state.action_bounds);
        safe_project_to_feasible(challenge, state, &mut seed, sens, base_flows, hp);
        if !unique.iter().any(|u| actions_eps_equal(u.as_slice(), seed.as_slice())) {
            unique.push(seed);
        }
    }
    unique
...
fn joint_optimize_step(
    ...
    let seeds = canonicalize_and_dedup_seeds(challenge, state, sens, base_flows, hp, seeds);
    for seed in seeds {
...
fn coordinate_polish_step(
    ...
    if challenge.network.verify_flows(&current_flows).is_err() {
        return action;
    }
    ...
    for _ in 0..hp.coord_polish_passes {
        ...
        for b in 0..challenge.num_batteries {
            ...
            for l in 0..num_lines {
                ...
                let limit = challenge.network.flow_limits[l] * (1.0 + EPS_FLOW) + 1e-6;
            ...
            if net_lo > net_hi + EPS {
                continue;
            }
            let net_lo = net_lo.clamp(lo, hi);
            let net_hi = net_hi.clamp(lo, hi);
            let net_span = net_hi - net_lo;

            let mut candidates = [0.0_f64; 13];
            let mut n_candidates = 0usize;
            let mut push_candidate = |cand: f64| {
                let cand = cand.clamp(lo, hi);
                for idx in 0..n_candidates {
                    if (candidates[idx] - cand).abs() < EPS {
                        return;
                    }
                }
            ...
            push_candidate(lo);
            push_candidate(hi);
            push_candidate(lo + 0.25 * span);
            push_candidate(lo + 0.50 * span);
            push_candidate(lo + 0.75 * span);
            push_candidate(cur - 0.25 * span);
            push_candidate(cur + 0.25 * span);
            if net_span > EPS {
                push_candidate(net_lo);
                push_candidate(net_hi);
                push_candidate(net_lo + 0.25 * net_span);
                push_candidate(net_lo + 0.50 * net_span);
                push_candidate(net_lo + 0.75 * net_span);
            }

            let current_b_value = dp_action_value(&dps[b], battery, state.time_step, soc, price, cur);
            let mut best_b_action = cur;
            let mut best_b_value = best_value;
            for idx in 0..n_candidates {
                let candidate = candidates[idx];
                if (candidate - cur).abs() <= EPS || candidate < net_lo - EPS || candidate > net_hi + EPS {
                    continue;
                }
                let candidate_b_value =
                    dp_action_value(&dps[b], battery, state.time_step, soc, price, candidate);
                let value = best_value - current_b_value + candidate_b_value;
            ...
            let marginal_dir = {
                let next_soc = battery.apply_action_to_soc(best_b_action, soc);
                let dv = dv_dsoc(&dps[b], state.time_step, next_soc);
                let imm_sign = if best_b_action > EPS { 1.0 } else if best_b_action < -EPS { -1.0 } else { 0.0 };
                let cap2 = battery.capacity_mwh.powi(2).max(1e-9);
                let imm = price * DELTA_T - imm_sign * KAPPA_TX * DELTA_T - 2.0 * KAPPA_DEG * DELTA_T * DELTA_T * best_b_action / cap2;
                let dsoc_du = if best_b_action > 0.0 {
                    -DELTA_T / ETA_DISCHARGE
                } else if best_b_action < 0.0 {
                    -ETA_CHARGE * DELTA_T
                } else {
                    -0.5 * (DELTA_T / ETA_DISCHARGE + ETA_CHARGE * DELTA_T)
                };
                let deriv = imm + dv * dsoc_du;
                let step_sign = if deriv > 0.0 { 1.0 } else { -1.0 };
                let max_abs_step = (hi - lo).min((best_b_action - lo).abs().max((hi - best_b_action).abs()).min(span * 0.1)).max(1e-9);
                step_sign * max_abs_step * 0.5
            };

            let ls_steps: usize = 8;
            let mut trial_action_val = best_b_value;
            let mut trial_action = best_b_action;
            for ls in 0..ls_steps {
                let candidate_unclamped = trial_action + marginal_dir * ((ls + 1) as f64);
                if candidate_unclamped < lo - EPS || candidate_unclamped > hi + EPS {
                    break;
                }
                let candidate = candidate_unclamped.clamp(lo, hi);
                if candidate < net_lo - EPS || candidate > net_hi + EPS {
                    break;
                }
                let candidate_b_value =
                    dp_action_value(&dps[b], battery, state.time_step, soc, price, candidate);
                let val = best_value - current_b_value + candidate_b_value;
                if val > trial_action_val + 1e-9 {
                    trial_action_val = val;
                    trial_action = candidate;
                } else {
                    break;
                }
            }
            if trial_action_val > best_b_value + 1e-9 {
                best_b_value = trial_action_val;
                best_b_action = trial_action;
            }
...
fn policy(
    ...
    hp: &Hyperparameters,
    delta_cong: &[Vec<f64>],
    agg_dp_for_seed: Option<&BatteryDP>,
) -> Result<Vec<f64>> {
    let t = state.time_step;
    ...
    for node in 0..challenge.network.num_nodes {
        if history.values[node].len() >= 16 {
            let sorted = &history.values[node];
        ...
        if history.residuals[node].len() >= 8 {
            let sorted = &history.residuals[node];
            let median = percentile(sorted, 50, 100);
            let recent = recent_residuals[node];
            residual_shift[node] = (0.65 * median + 0.35 * recent).clamp(-25.0, 25.0);
        }
    }

    let node_q_low: Vec<f64> = (0..challenge.network.num_nodes)
        .map(|node| state.rt_prices[node])
        .collect();
    let node_q_high: Vec<f64> = node_q_low.clone();
    let node_charge_max: Vec<f64> = node_q_high.iter()
        .map(|&price| price * eta_rt - friction)
        .collect();
    let node_discharge_min: Vec<f64> = node_q_low.iter()
        .map(|&price| price / eta_rt + friction)
        .collect();
    let node_price_band: Vec<f64> = node_charge_max.iter()
        .zip(node_discharge_min.iter())
        .map(|(&charge_max, &discharge_min)| (discharge_min - charge_max).abs().max(5.0))
        .collect();
    ...
    for (b, battery) in challenge.batteries.iter().enumerate() {
        ...
        let q_low = node_q_low[node];
        let q_high = node_q_high[node];
        let price_band = node_price_band[node];
        let charge_max = node_charge_max[node];
        let discharge_min = node_discharge_min[node];
        ...
        if terminal_drain && u_max > 0.0 && current_price > friction {
            ...
            let mut best_drain_val = dp_action_value(&dps[b], battery, t, state.socs[b], current_price, 0.0);
            let mut best_drain_frac = 0.0;
            for &frac in &[0.3_f64, 0.5, 0.8, 1.0] {
                let cand_a = u_max * frac;
                let val = dp_action_value(&dps[b], battery, t, state.socs[b], current_price, cand_a);
                if val > best_drain_val + 1e-9 {
                    best_drain_val = val;
                    best_drain_frac = frac;
                }
            }
            a = u_max * best_drain_frac;
    ...
    for node in 0..challenge.network.num_nodes {
        insert_sorted_history(&mut history.values[node], state.rt_prices[node]);
        let residual = state.rt_prices[node] - challenge.market.day_ahead_prices[t]
            .get(node).copied()
            .or_else(|| challenge.market.day_ahead_prices[t].last().copied())
            .unwrap_or(0.0);
        insert_sorted_history(&mut history.residuals[node], residual);
        recent_residuals[node] = residual;
    }
    drop(recent_residuals);
    drop(history);
    ...
    let mut seeds = match (hp.use_zero_seed, hp.use_dp_seed) {
    ...
    if let Some(agg_dp) = agg_dp_for_seed {
        if n_remaining > 12 {
            let total_cap: f64 = challenge.batteries.iter()
                .map(|b| b.capacity_mwh).sum::<f64>().max(1.0);
            let total_discharge_mw: f64 = challenge.batteries.iter()
                .map(|b| b.power_discharge_mw).sum();
            let total_charge_mw: f64 = challenge.batteries.iter()
                .map(|b| b.power_charge_mw).sum();

            let e_agg_now: f64 = state.socs.iter().sum();
            let e_agg_min: f64 = challenge.batteries.iter()
                .map(|b| b.soc_min_mwh).sum();
            let e_agg_max: f64 = challenge.batteries.iter()
                .map(|b| b.soc_max_mwh).sum();
            let headroom = (e_agg_max - e_agg_now).max(0.0);
            let available = (e_agg_now - e_agg_min).max(0.0);
            let max_c = (headroom / (ETA_CHARGE * DELTA_T)).min(total_charge_mw).max(0.0);
            let max_d = (available * ETA_DISCHARGE / DELTA_T).min(total_discharge_mw).max(0.0);
            let agg_lo = -max_c;
            let agg_hi = max_d;

            let avg_rt_price: f64 = {
                let mut sum = 0.0_f64;
                for (_b, batt) in challenge.batteries.iter().enumerate() {
                    sum += batt.capacity_mwh * state.rt_prices[batt.node];
                }
                sum / total_cap
            };

            let next_t = (t + 1).min(agg_dp.values.len() - 1);
            let apply = |action: f64| -> f64 {
                if action < 0.0 {
                    e_agg_now + (-action) * ETA_CHARGE * DELTA_T
                } else if action > 0.0 {
                    e_agg_now - action * DELTA_T / ETA_DISCHARGE
                } else {
                    e_agg_now
                }
            };
            let imm_profit = |action: f64| -> f64 {
                let throughput = action.abs() * DELTA_T;
                action * avg_rt_price * DELTA_T
                    - KAPPA_TX * throughput
                    - KAPPA_DEG * (throughput / total_cap).powi(2)
            };
            ...
            let mut best_agg_action = 0.0_f64;
            for i in 0..grid_size {
                let frac = i as f64 / (grid_size - 1) as f64;
                let cand = agg_lo + (agg_hi - agg_lo) * frac;
                let next_soc = apply(cand).clamp(e_agg_min, e_agg_max);
                let future = interp_value(
                    &agg_dp.values[next_t],
                    next_soc,
                    agg_dp.soc_lo,
                    agg_dp.soc_step_inv,
                    agg_dp.levels - 1,
                );
                let val = imm_profit(cand) + future;
                if val > best_val {
                    best_val = val;
                    best_agg_action = cand;
                }
            }

            let mut agg_action = vec![0.0_f64; challenge.num_batteries];
            if best_agg_action > 0.0 && total_discharge_mw > 0.0 {
                let share = best_agg_action / total_discharge_mw;
                for b in 0..challenge.num_batteries {
                    let cap = challenge.batteries[b].power_discharge_mw;
                    agg_action[b] = (share * cap).clamp(
                        state.action_bounds[b].0,
                        state.action_bounds[b].1,
                    );
                }
            } else if best_agg_action < 0.0 && total_charge_mw > 0.0 {
                let share = best_agg_action / total_charge_mw;
                for b in 0..challenge.num_batteries {
                    let cap = challenge.batteries[b].power_charge_mw;
                    agg_action[b] = (share * cap).clamp(
                        state.action_bounds[b].0,
                        state.action_bounds[b].1,
                    );
            ...
            safe_project_to_feasible(challenge, state, &mut agg_action, sens, &base_flows, hp);
            seeds.push(agg_action);
        }
    }
...
pub fn solve_challenge(
    ...
    {
        let num_lines = sens.len();
        let num_batteries = challenge.num_batteries;
        let mut gram = vec![0.0_f64; num_lines * num_lines];
        let mut row_norm_sq = vec![0.0_f64; num_lines];
        for i in 0..num_lines {
            let row_i = &sens[i];
            let base_i = i * num_lines;
            for j in i..num_lines {
                let row_j = &sens[j];
                let mut dot = 0.0_f64;
                ...
                gram[base_i + j] = dot;
                gram[j * num_lines + i] = dot;
            }
            row_norm_sq[i] = gram[base_i + i];
        }
        let mut cache = projection_cache_lock().lock().unwrap();
        cache.num_lines = num_lines;
        cache.num_batteries = num_batteries;
        cache.gram = gram;
        cache.row_norm_sq = row_norm_sq;
    }
            ...
            .map(|(b, battery)| {
                if !touched[b] {
                    ...
                    let da_ct: Vec<f64> = (0..n_t)
                        .map(|t| cached_node_price(challenge, t, node) + ep_ct[t][b])
                        .collect();
                    build_battery_dp(
                        battery,
                        &da_ct,
    ...
    let fleet_agg_dp_seed = if challenge.num_batteries <= 200 && challenge.num_steps > 12 {
        let n_b = challenge.num_batteries;
        let total_cap: f64 = challenge.batteries.iter().map(|b| b.capacity_mwh).sum::<f64>().max(1.0);
        let fleet_da: Vec<f64> = (0..challenge.num_steps)
            .map(|t| {
                let mut p = 0.0_f64;
                for b in 0..n_b {
                    p += challenge.batteries[b].capacity_mwh
                        * challenge.market.day_ahead_prices[t][challenge.batteries[b].node];
                }
                p / total_cap
            })
            .collect();
        let fleet_premium: Vec<f64> = (0..challenge.num_steps)
            .map(|t| expected_premiums[t].iter().sum::<f64>() / (n_b as f64).max(1.0))
            .collect();
        let fleet_da_with_cong: Vec<f64> = fleet_da.iter().zip(fleet_premium.iter())
            .map(|(da, prem)| da + prem)
            .collect();
        Some(build_aggregate_dp(
            &challenge.batteries,
            &fleet_da_with_cong,
            challenge.num_steps,
            sigma,
