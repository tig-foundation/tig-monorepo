...
mod helpers {
    ...
    fn lns_dw_per_step(
        ...
        for _cg_iter in 0..hp.lns_cg_iters {
            ...
            for b in 0..num_b {
                ...
                // DP-driven column: optimal unconstrained action under effective price
    ...
    fn run_deflator(
        ...
        for _ in 0..rounds {
            ...
            for &(_, l_anchor) in &line_rank {
                ...
                // Build touched lines from bundle
                ...
                // Solve a small LP to find optimal delta for the bundle
                ...
                if n_act > 0 && bundle_size > 0 {
                    let n_vars = 2 * bundle_size; // delta_plus, delta_minus per batt
                    ...
                    if let Some(delta) = opt_delta {
                        ...
                        // check feasibility and profit gain
    ...
    fn polish_cd(
        ...
        // Allow a second sweep on small instances to capture any remaining
        // feasible improvements after the first pass.
        ...
        let mut mu2 = vec![0.0_f64; num_l];
        {
            ...
            if active_for_mu.len() > 0 {
                ...
                if let Some(duals) = duals_opt {
                    ...
                    // Quick LDD-like update to refine mu2 when LP fails
