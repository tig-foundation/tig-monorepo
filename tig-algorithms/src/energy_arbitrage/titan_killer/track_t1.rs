...
mod helpers {
    ...
    pub struct TrackHp {
        ...
        /// Number of dual-ascent rounds used to price network congestion over the whole
        /// horizon before the value tables are built. 0 keeps the historical behaviour.
        pub cong_iters: usize,
        /// Initial dual step size of that ascent.
        pub cong_alpha: f64,
        /// Harmonic decay of the step size.
        pub cong_decay: f64,
        /// Multiplier applied to the resulting price correction when it is handed to the
        /// value tables (1.0 = the raw dual price).
        pub cong_scale: f64,
    }
    ...
    impl TrackHp {
        pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
            ...
            if let Some(v) = m.get("cong_iters").and_then(|v| v.as_u64()) { self.cong_iters = v as usize; }
            if let Some(v) = m.get("cong_alpha").and_then(|v| v.as_f64()) { self.cong_alpha = v.max(0.0); }
            if let Some(v) = m.get("cong_decay").and_then(|v| v.as_f64()) { self.cong_decay = v.max(0.0); }
            if let Some(v) = m.get("cong_scale").and_then(|v| v.as_f64()) { self.cong_scale = v.clamp(0.0, 5.0); }
        }
    }
    ...
    /// Slope of a concave piecewise-linear function whose derivative is given as
    /// (slope, length) segments in non-increasing slope order, at offset `x` from the
    /// left end of the domain.
    #[inline]
    fn seg_slope(segs: &[(f64, f64)], x: f64) -> f64 {
        let mut acc = 0.0_f64;
        for &(s, l) in segs {
            acc += l;
            if x <= acc {
        ...
        segs.last().map(|z| z.0).unwrap_or(0.0)
    }

    /// EXACT single-battery arbitrage plan for a fully known price path.
    ///
    /// With the wear term neglected (it is ~1e-5 of the revenue here) the one-step gain is
    /// linear in the energy moved, so the value-to-go is CONCAVE and piecewise linear in the
    /// stored energy. Its derivative is a non-increasing step function stored as
    /// (slope, length) segments spanning [soc_min, soc_max].
    ///
    /// Backward step: V_t(E) = max_d [ g(d) + V_{t+1}(E+d) ] = (ghat # V_{t+1})(E) with
    /// ghat(y) = g(-y), a sup-convolution. For concave pieces a sup-convolution is just the
    /// sorted merge of the two derivative segment lists, followed by a trim back to the
    /// original domain: `add_max` off the high-slope end, `rem_max` off the low-slope end.
    /// This is exact and costs O(T * segments) with no grid at all.
    fn pwl_plan(bat: &Battery, ptilde: &[f64], soc0: f64, out: &mut [f64]) {
        let dt = 0.25_f64;
        let ktx = 0.25_f64;
        let num_t = ptilde.len();
        if num_t == 0 {
            return;
        }
        let e_min = bat.soc_min_mwh;
        let e_max = bat.soc_max_mwh;
        let span = (e_max - e_min).max(1e-12);
        let etac = bat.efficiency_charge.max(1e-9);
        let etad = bat.efficiency_discharge.max(1e-9);
        let add_max = etac * bat.power_charge_mw * dt;
        let rem_max = bat.power_discharge_mw * dt / etad;

        // Flat storage of the derivative segments of V_t for t = 0..=num_t.
        let mut buf: Vec<(f64, f64)> = Vec::with_capacity((num_t + 1) * 8);
        let mut range: Vec<(usize, usize)> = vec![(0, 0); num_t + 1];
        buf.push((0.0, span));
        range[num_t] = (0, 1);

        let mut cur: Vec<(f64, f64)> = vec![(0.0, span)];
        let mut merged: Vec<(f64, f64)> = Vec::with_capacity(2 * num_t + 8);
        for t in (0..num_t).rev() {
            let lam = ptilde[t];
            let p_sell = (lam - ktx) * etad; // $ per MWh taken out of storage
            let p_buy = (lam + ktx) / etac; // $ per MWh put into storage
            let (hi_s, hi_l, lo_s, lo_l) = if p_buy >= p_sell {
                (p_buy, add_max, p_sell, rem_max)
            } else {
                (p_sell, rem_max, p_buy, add_max)
            };
            merged.clear();
            let mut done_hi = false;
            let mut done_lo = false;
            for &(sl, ln) in cur.iter() {
                if !done_hi && hi_s > sl {
                    merged.push((hi_s, hi_l));
                    done_hi = true;
                }
                if done_hi && !done_lo && lo_s > sl {
                    merged.push((lo_s, lo_l));
                    done_lo = true;
                }
                merged.push((sl, ln));
            }
            ...
            // trim `add_max` from the front, `rem_max` from the back
            let mut left = add_max;
            let mut i0 = 0usize;
            while left > 1e-15 && i0 < merged.len() {
                let l = merged[i0].1;
                if l <= left + 1e-15 {
                    left -= l;
                    i0 += 1;
            ...
            let mut right = rem_max;
            let mut i1 = merged.len();
            while right > 1e-15 && i1 > i0 {
                let l = merged[i1 - 1].1;
                if l <= right + 1e-15 {
                    right -= l;
                    i1 -= 1;
                } else {
                    merged[i1 - 1].1 = l - right;
                    right = 0.0;
                }
            }
            cur.clear();
            cur.extend_from_slice(&merged[i0..i1]);
            let st = buf.len();
            buf.extend_from_slice(&cur);
            range[t] = (st, buf.len());
        }

        let mut soc = soc0.clamp(e_min, e_max);
        for t in 0..num_t {
            let (a, b) = range[t + 1];
            let w = seg_slope(&buf[a..b], soc - e_min);
            let lam = ptilde[t];
            let p_sell = (lam - ktx) * etad;
            let p_buy = (lam + ktx) / etac;
            let head = (e_max - soc).max(0.0);
            let avail = (soc - e_min).max(0.0);
            let u = if p_sell > w {
                bat.power_discharge_mw.min(avail * etad / dt).max(0.0)
            } else if p_buy < w {
                -(bat.power_charge_mw.min(head / (etac * dt)).max(0.0))
            } else {
                0.0
            };
            out[t] = u;
            soc = if u < 0.0 {
                (soc + etac * (-u) * dt).clamp(e_min, e_max)
            } else {
                (soc - u * dt / etad).clamp(e_min, e_max)
            };
        }
    }

    /// Horizon-wide congestion prices.
    ///
    /// The flow limits couple the batteries only WITHIN a time step, while the storage
    /// dynamics couple a battery only ACROSS time steps. Attaching a price to each
    /// (line, step) limit and subtracting its effect from each battery's nodal price
    /// therefore splits the horizon problem into independent single-battery arbitrages,
    /// each solved exactly by `pwl_plan`. Ascending on those prices makes the value tables
    /// aware of the congestion the naive nodal price hides.
    ///
    /// Returns the per-(step, battery) price correction to subtract from the nodal price.
    fn congestion_offsets(
        challenge: &Challenge,
        price_table: &[Vec<f64>],
        ...
        let mut offs = vec![vec![0.0_f64; num_b]; num_t];
        if hp.cong_iters == 0 || num_l == 0 || num_b == 0 || price_table.len() != num_t {
            return offs;
        }
        let batt_nodes: Vec<usize> = challenge.batteries.iter().map(|b| b.node).collect();
        let nodal: Vec<Vec<f64>> = (0..num_t)
            .map(|t| {
                (0..num_b)
                    .map(|b| {
                        let row = &price_table[t];
            ...
            .collect();
        let mut fexo = vec![0.0_f64; num_l * num_t];
        for t in 0..num_t {
            let f = challenge
                .network
        ...
        let lim = &challenge.network.flow_limits;
        let mut mup = vec![0.0_f64; num_l * num_t];
        let mut mun = vec![0.0_f64; num_l * num_t];
        let mut plan = vec![0.0_f64; num_b * num_t];
        let mut ptilde = vec![0.0_f64; num_t];
        let mut u = vec![0.0_f64; num_t];
        let mut flow = vec![0.0_f64; num_l];
        for it in 0..hp.cong_iters {
            for b in 0..num_b {
                for t in 0..num_t {
                    let mut corr = 0.0_f64;
                    for &(l, a) in &b_to_lines[b] {
                        let k = l * num_t + t;
                        corr += a * (mup[k] - mun[k]);
                    }
                    ptilde[t] = nodal[t][b] - corr;
                }
                let s0 = socs0.get(b).copied().unwrap_or(challenge.batteries[b].soc_initial_mwh);
                pwl_plan(&challenge.batteries[b], &ptilde, s0, &mut u);
                plan[b * num_t..(b + 1) * num_t].copy_from_slice(&u);
            }
            let step = hp.cong_alpha / (1.0 + hp.cong_decay * it as f64);
            for t in 0..num_t {
                for l in 0..num_l {
                    flow[l] = fexo[l * num_t + t];
                }
                for b in 0..num_b {
                    let ub = plan[b * num_t + t];
                    if ub == 0.0 {
                        continue;
                    }
                    for &(l, a) in &b_to_lines[b] {
                ...
                for l in 0..num_l {
                    ...
                    let up = flow[l] - lim[l];
                    let dn = -flow[l] - lim[l];
                    mup[k] = (mup[k] + step * up).max(0.0);
                    mun[k] = (mun[k] + step * dn).max(0.0);
                }
            }
        }
        for b in 0..num_b {
    ...
    fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> TitanCache {
        ...
        // Horizon-wide congestion prices (no-op when cong_iters == 0).
        let cong_off: Vec<Vec<f64>> = match price_table.as_ref() {
            Some(pt) if use_expanded => {
                congestion_offsets(challenge, pt, &b_to_lines, &state.socs, hp)
            }
            _ => vec![vec![0.0_f64; num_b]; num_t],
        };
