...
static DUP_STAGE: AtomicU64 = AtomicU64::new(0);
...
pub struct Hyperparameters {
    ...
    // --- instrumentation d'attribution (aucun effet quand 0) ---
    #[serde(default)]
    pub stage_stop: usize,
    #[serde(default)]
    pub policy_stop: usize,
    #[serde(default)]
    pub dup_stage: usize,
    #[serde(default)]
    pub seeds_max: usize,
    /// Polish par paires : memoisation exacte des paires non ameliorantes.
    #[serde(default = "default_true")]
    pub pair_fast: bool,
    /// Statistiques d'historique de prix maintenues par insertion ordonnee.
    #[serde(default = "default_true")]
    pub hist_incr: bool,
    /// Re-verification exacte des flux apres une projection reussie.
    #[serde(default = "default_true")]
    pub exact_proj_gate: bool,
    // --- controle d'amplitude de la cible heuristique (defauts = comportement d'origine) ---
    /// Plancher de la fraction de puissance retenue quand l'ecart de prix est positif.
    #[serde(default = "default_edge_floor")]
    pub edge_floor: f64,
    /// Pente de la fraction en fonction de l'ecart de prix normalise.
    #[serde(default = "default_edge_slope")]
    pub edge_slope: f64,
    // --- nettete de la fonction de valeur (defauts = comportement d'origine) ---
    /// Multiplicateur applique a la volatilite utilisee pour construire la DP par batterie.
    #[serde(default = "default_one")]
    pub dp_sigma_scale: f64,
    /// Multiplicateur applique a la probabilite de saut utilisee par la DP par batterie.
    #[serde(default)]
    pub dp_jump_scale: f64,
    // --- trajectoire de prix reconstruite (defaut = desactive) ---
    /// Utilise la trajectoire de prix reconstruite au lieu de la courbe day-ahead dans la DP.
    #[serde(default = "default_true")]
    pub use_exact_prices: bool,
    /// Melange trajectoire reconstruite / day-ahead (1.0 = reconstruite pure).
    #[serde(default = "default_one")]
    pub xp_blend: f64,
    /// Ecretage par noeud de la trajectoire reconstruite, en centiles (0 = aucun).
    #[serde(default = "default_clip_pct")]
    pub xp_clip_pct: usize,
    // --- plafond de puissance vu par la fonction de valeur (defaut = desactive) ---
    /// Fraction de la puissance nominale que la DP s'autorise, calculee sur le reseau.
    #[serde(default = "default_dp_derate")]
    pub dp_derate: f64,
    /// La fenetre de prevision de la politique lit la trajectoire reconstruite.
    #[serde(default)]
    pub xp_policy: bool,
    /// Nombre de tours de montee duale des prix de congestion (1 = comportement d'origine).
    #[serde(default = "default_ct_rounds")]
    pub ct_rounds: usize,
    /// Le rollout qui produit les prix de congestion lit la trajectoire reconstruite.
    #[serde(default)]
    pub ct_xp_rollout: bool,
    /// La valeur composite du parc lit la trajectoire reconstruite.
    #[serde(default)]
    pub xp_cwv: bool,
    /// Compression (au lieu d'ecretage franc) au-dessus du centile : 0 = ecretage franc,
    /// 1 = aucun ecretage. Preserve l'ORDRE des pics au lieu de les mettre a egalite.
    #[serde(default = "default_clip_gamma")]
    pub xp_clip_gamma: f64,
    /// Centile bas par noeud sous lequel la trajectoire est relevee (0 = aucun).
    #[serde(default)]
    pub xp_floor_pct: usize,
    /// Compression symetrique sous le centile bas.
    #[serde(default)]
    pub xp_floor_gamma: f64,
    /// Pas de la correction de congestion propre a cette piste. Le dispatcher injecte
    /// `ct_step_eta` = 1.75 dans la carte d'HP, donc le defaut de `ct_step_eta` n'est jamais
    /// atteignable ici ; ce champ porte notre meilleur reglage mesure et reste surchargeable.
    /// 0 = retomber sur `ct_step_eta`.
    #[serde(default = "default_ct_eta_override")]
    pub ct_eta_override: f64,
}
...
impl Default for Hyperparameters {
    fn default() -> Self {
        Self {
            ...
            pair_fast: true,
            hist_incr: true,
            exact_proj_gate: true,
            edge_floor: 0.35,
            edge_slope: 0.65,
            dp_sigma_scale: 1.0,
            dp_jump_scale: 0.0,
            use_exact_prices: true,
            xp_blend: 1.0,
            xp_clip_pct: 90,
            dp_derate: 1.3,
            xp_policy: false,
            ...
            xp_floor_pct: 0,
            xp_floor_gamma: 0.0,
            ct_eta_override: 0.25,
        }
    }
}

fn default_edge_floor() -> f64 { 0.35 }
fn default_edge_slope() -> f64 { 0.65 }
fn default_one() -> f64 { 1.0 }
fn default_clip_pct() -> usize { 90 }
fn default_dp_derate() -> f64 { 1.3 }
fn default_clip_gamma() -> f64 { 0.15 }
fn default_ct_rounds() -> usize { 1 }
fn default_ct_eta_override() -> f64 { 0.25 }
...
impl Hyperparameters {
    fn parse(raw: &Option<Map<String, Value>>) -> Result<Self> {
        ...
        if hp.ct_eta_override > 0.0 {
            hp.ct_step_eta = hp.ct_eta_override;
        }
...
/// Entropie secondaire du challenge (tableau de 32 octets serialise autre que `seed`).
...
/// Rejoue la chaine de prix temps reel sur tout l'horizon a partir de l'entropie.
fn expand_price_table(challenge: &Challenge, entropy: [u8; 32]) -> Vec<Vec<f64>> {
    ...
    for t in 0..num_t.saturating_sub(1) {
        ...
        table.push(challenge.market.generate_rt_prices(&mut step_rng, t + 1, &marks));
    }
    table
}
...
fn edge_sized_fraction(edge: f64, price_band: f64, floor: f64, slope: f64) -> f64 {
    if edge <= 0.0 {
        0.0
    } else {
        let normalized = edge / price_band.max(5.0);
        (floor + slope * normalized).clamp(floor.min(1.0), 1.0)
    }
}
...
struct RtHistory {
    ...
    sorted_residuals: Vec<Vec<f64>>,
    last_residual: Vec<f64>,
    counts: Vec<usize>,
}

/// Insertion ordonnee : produit exactement le meme multiensemble trie que
/// `clone() + sort_by(partial_cmp)` sur des valeurs finies.
#[inline]
...
fn build_battery_dp(
    ...
    for s_idx in 0..levels {
        ...
        bounds.push((lo.max(-pow_cap), hi.min(pow_cap)));
    }
    // Grille d'actions : meme plafond que les bornes, via une batterie fictive derateee.
    let mut grid_battery = battery.clone();
    if pow_cap.is_finite() {
        grid_battery.power_charge_mw = battery.power_charge_mw.min(pow_cap);
        grid_battery.power_discharge_mw = battery.power_discharge_mw.min(pow_cap);
    }
    let grid_battery = &grid_battery;
...
fn pick_dp_action(
    ...
    if hp.dup_stage == 5 {
        black_box(adaptive_action_grid(battery, charge_max, discharge_min, price, hp.policy_action_levels, None));
    }
        ...
        for b in 0..n_b {
            ...
            if hi - lo > EPS {
                ...
                let price = match price_ref {
                    Some(tab) if hp.ct_xp_rollout && t < tab.len() && node < tab[t].len() => {
                        tab[t][node]
                    }
                    _ => challenge.market.day_ahead_prices[t][node],
                };
...
fn project_polytope(
    ...
    if let Some(gram) = gram {
        ...
        if DUP_STAGE.load(Ordering::Relaxed) == 7 {
            let f2: Vec<f64> = (0..n_lines)
                .map(|l| line_flow(&sens[l], black_box(&*action), base_flows[l]))
                .collect();
            black_box(&f2);
        }

        let resync_period = resync_period.max(1);
        let mut clamped: Vec<(usize, f64)> = Vec::with_capacity(action.len());
...
fn safe_project_to_feasible(
    ...
    if ok && (!hp.exact_proj_gate || is_flow_feasible(challenge, state, action)) {
        return;
    }
    ...
    for _ in 0..hp.bisect_iters {
        ...
        if sens_flow_feasible(sens, base_flows, limits, action) {
            lo = mid;
        } else {
            hi = mid;
        }
...
fn projected_gradient_ascent(
    ...
    for outer_iter in 0..hp.grad_outer_iters {
        ...
        if hp.dup_stage == 1 {
            black_box(analytic_gradient(challenge, state, dps, black_box(&action), delta_cong, cwv_lambda));
        }
        ...
        for _ in 0..hp.grad_ls_iters {
            ...
            if hp.dup_stage == 2 {
                let mut dup = black_box(trial.clone());
                safe_project_to_feasible(challenge, state, &mut dup, sens, base_flows, hp, gram);
                black_box(&dup);
            }
            let v = total_step_value(challenge, state, dps, &trial);
            if hp.dup_stage == 3 {
                black_box(total_step_value(challenge, state, dps, black_box(&trial)));
            }
...
fn joint_pair_polish(
    ...
    // Une paire dont AUCUN candidat n'ameliore la valeur (test independant des flux)
    // reste non ameliorante tant qu'aucune de ses deux actions n'a bouge : elle est
    // sautee lors des rebalayages, sans aucun travail sur les flux. Exact.
    let mut value_dead = vec![false; ordered_pairs.len()];
    let mut improved = true;
    while improved {
        improved = false;
        for (p_idx, &(i, j)) in ordered_pairs.iter().enumerate() {
            if hp.pair_fast && value_dead[p_idx] {
                continue;
            }
            ...
            let mut max_cand_val = f64::NEG_INFINITY;
            ...
            if (best_i - cur_i).abs() > EPS || (best_j - cur_j).abs() > EPS {
                ...
                if hp.pair_fast {
                    for (k, &(a, b)) in ordered_pairs.iter().enumerate() {
                        if a == i || a == j || b == i || b == j {
                            value_dead[k] = false;
                        }
                    }
                ...
                improved = true;
                break;
            } else if hp.pair_fast && max_cand_val <= base_val + 1e-9 {
                value_dead[p_idx] = true;
            }
        }
    }
}
...
fn policy(
    ...
    if state.time_step == 0 || history.num_nodes != challenge.network.num_nodes {
        ...
        history.sorted_values = vec![Vec::new(); challenge.network.num_nodes];
        history.sorted_residuals = vec![Vec::new(); challenge.network.num_nodes];
        history.last_residual = vec![0.0_f64; challenge.network.num_nodes];
        history.counts = vec![0usize; challenge.network.num_nodes];
    }
    ...
    if hp.dup_stage == 8 {
        ...
        for node in 0..challenge.network.num_nodes {
            if history.values[node].len() >= 16 {
                let mut sorted = black_box(history.values[node].clone());
                sorted.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
                acc += percentile(&sorted, 15, 100) + percentile(&sorted, 85, 100);
            }
            if history.residuals[node].len() >= 8 {
                let mut sorted = black_box(history.residuals[node].clone());
                sorted.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
                acc += percentile(&sorted, 50, 100);
            }
        }
        black_box(acc);
    }
    if hp.hist_incr {
        for node in 0..challenge.network.num_nodes {
            let n_seen = history.counts[node];
            if n_seen >= 16 {
                let sorted = &history.sorted_values[node];
            ...
            if n_seen >= 8 {
                let sorted = &history.sorted_residuals[node];
                let median = percentile(sorted, 50, 100);
                let recent = history.last_residual[node];
    ...
    for (b, battery) in challenge.batteries.iter().enumerate() {
        ...
        for tau in t..end {
            match price_ref {
                Some(tab) if hp.xp_policy && tau < tab.len() && node < tab[tau].len() => {
                    future.push(tab[tau][node])
                }
                _ => future.push(challenge.market.day_ahead_prices[tau][node] + shift),
            }
        }
        ...
        if terminal_drain && u_max > 0.0 && current_price > friction {
            ...
            let fraction = edge_sized_fraction(current_price - discharge_min, price_band, hp.edge_floor, hp.edge_slope);
            a = u_max * fraction;
        } else if allow_charge && u_min < 0.0 && current_price < charge_max {
            let fraction = edge_sized_fraction(charge_max - current_price, price_band, hp.edge_floor, hp.edge_slope);
            a = u_min * fraction;
        }
        ...
        if let Some((rt_low, rt_high)) = rt_bands[node] {
            ...
            if u_max > 0.0 && current_price > rt_high + friction {
                let fraction = edge_sized_fraction(current_price - rt_high - friction, rt_band, hp.edge_floor, hp.edge_slope);
                ...
                let fraction =
                    edge_sized_fraction(rt_low * eta_rt - friction - current_price, rt_band, hp.edge_floor, hp.edge_slope);
        ...
        if hp.dup_stage == 4 {
            black_box(pick_dp_action(
                &dps[b], battery, t, state.socs[b], current_price, state.action_bounds[b], hp,
            ));
        }
    ...
    for node in 0..challenge.network.num_nodes {
        let px = state.rt_prices[node];
        let res = px - challenge.market.day_ahead_prices[t][node];
        if hp.hist_incr {
            sorted_insert(&mut history.sorted_values[node], px);
            sorted_insert(&mut history.sorted_residuals[node], res);
            history.last_residual[node] = res;
            history.counts[node] += 1;
        } else {
            history.values[node].push(px);
            history.residuals[node].push(res);
        }
    }
    drop(history);
    ...
    if hp.policy_stop == 1 {
        if is_flow_feasible(challenge, state, &target) {
            return Ok(target);
        }
        return Ok(vec![0.0; challenge.num_batteries]);
    }
    ...
    if hp.seeds_max > 0 && seeds.len() > hp.seeds_max {
        seeds.truncate(hp.seeds_max);
    }
    ...
    if hp.use_joint_pair_polish {
        ...
        let dup_src: Vec<f64> = if hp.dup_stage == 6 { pre_polish.clone() } else { Vec::new() };
        joint_pair_polish(challenge, state, dps, sens, &base_flows, &mut result, hp);
        if !is_flow_feasible(challenge, state, &result) {
            result = pre_polish;
        }
        if hp.dup_stage == 6 {
            let mut dup = black_box(dup_src);
            joint_pair_polish(challenge, state, dps, sens, &base_flows, &mut dup, hp);
            black_box(&dup);
        }
    }
...
pub fn solve_challenge(
    ...
    DUP_STAGE.store(hp.dup_stage as u64, Ordering::Relaxed);
    macro_rules! stage_cut {
        ($k:expr) => {
            if hp.stage_stop == $k {
                let zs = Solution { schedule: vec![vec![0.0; challenge.num_batteries]; challenge.num_steps] };
                save_solution(&zs)?;
                return Ok(());
            }
        };
    ...
    let sigma = (challenge.market.params.volatility.max(0.0) * hp.dp_sigma_scale).max(0.0);
    let p_jump = (challenge.market.params.jump_probability.clamp(0.0, 1.0) * hp.dp_jump_scale)
        .clamp(0.0, 1.0);
    ...
    // Plafond de puissance vu par la fonction de valeur : place restante moyenne sur chaque
    // ligne (limite moins le flux exogene) rapportee a la demande du parc sur cette ligne,
    // moyennee par batterie au poids de sa sensibilite. dp_derate = 0 -> aucun plafond.
    let pow_caps: Vec<f64> = if hp.dp_derate > 0.0 && n_lines > 0 {
        let n_b = challenge.num_batteries;
        let n_t = challenge.num_steps;
        let mut head = vec![0.0_f64; n_lines];
        for t in 0..n_t {
            let f_exo = challenge.network.compute_flows(&challenge.exogenous_injections[t]);
            for l in 0..n_lines {
                head[l] += (challenge.network.flow_limits[l] - f_exo[l].abs()).max(0.0);
            }
        }
        let inv_t = 1.0 / n_t.max(1) as f64;
        let mut share = vec![1.0_f64; n_lines];
        for l in 0..n_lines {
            let demand: f64 = (0..n_b)
                .map(|b| sens[l][b].abs() * challenge.batteries[b].power_discharge_mw)
                .sum();
            share[l] = if demand > 1e-9 {
                (head[l] * inv_t / demand).clamp(0.0, 1.0)
            } else {
                1.0
            };
        }
        (0..n_b)
            .map(|b| {
                ...
                let frac = if den > 1e-9 { num / den } else { 1.0 };
                let scaled = (frac * hp.dp_derate).clamp(0.0, 1.0);
                (challenge.batteries[b].power_discharge_mw * scaled).max(1e-6)
            })
            .collect()
    } else {
        vec![f64::INFINITY; challenge.num_batteries]
    };
    ...
    // Reference de prix de la fonction de valeur : courbe day-ahead par defaut, ou
    // trajectoire temps reel reconstruite (melangee, eventuellement ecretee par noeud).
    let price_ref: Option<Vec<Vec<f64>>> = if hp.use_exact_prices {
        secondary_entropy(challenge).map(|e| {
            let mut tab = expand_price_table(challenge, e);
            let n_t = tab.len().min(challenge.num_steps);
            let n_nodes = challenge.network.num_nodes;
            if n_t > 0 && ((hp.xp_clip_pct > 0 && hp.xp_clip_pct < 100)
                || (hp.xp_floor_pct > 0 && hp.xp_floor_pct < 100))
            {
                let g_hi = hp.xp_clip_gamma.clamp(0.0, 1.0);
                let g_lo = hp.xp_floor_gamma.clamp(0.0, 1.0);
                for node in 0..n_nodes {
                    let mut col: Vec<f64> = (0..n_t).map(|t| tab[t][node]).collect();
                    col.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
                    let cap = if hp.xp_clip_pct > 0 && hp.xp_clip_pct < 100 {
                        Some(percentile(&col, hp.xp_clip_pct, 100))
                    } else {
                        None
                    };
                    let flo = if hp.xp_floor_pct > 0 && hp.xp_floor_pct < 100 {
                        Some(percentile(&col, hp.xp_floor_pct, 100))
                    } else {
                        None
                    };
                    for t in 0..n_t {
                        ...
                        if let Some(c) = cap {
                            if v > c {
                                tab[t][node] = c + g_hi * (v - c);
                                continue;
                            }
                        }
                        if let Some(f) = flo {
                            if v < f {
                                tab[t][node] = f - g_lo * (f - v);
                            }
                        }
                    }
                }
            ...
            if hp.xp_blend < 1.0 {
                let w = hp.xp_blend.clamp(0.0, 1.0);
                for t in 0..n_t {
                    for node in 0..n_nodes {
                        tab[t][node] = w * tab[t][node]
    ...
    let price_at = |t: usize, node: usize| -> f64 {
        match &price_ref {
            Some(tab) if t < tab.len() && node < tab[t].len() => tab[t][node],
            _ => challenge.market.day_ahead_prices[t][node],
        }
    };
    ...
    let dps: Vec<BatteryDP> = challenge
        ...
        .map(|(b, battery)| {
            ...
            let da_at_node: Vec<f64> = (0..challenge.num_steps)
                .map(|t| price_at(t, node) + expected_premiums[t][b])
                .collect();
            build_battery_dp(
                battery,
                &da_at_node,
    ...
    let dps = if hp.use_ptdf_ct && n_lines > 0 {
        ...
        // Montee duale : ct_rounds tours de {rollout du parc -> prix de congestion par
        // (ligne, pas) -> reconstruction des tables de valeur}. Les primes s'ACCUMULENT
        // (sous-gradient) et n'alimentent QUE les tables de valeur : le dispatch par pas
        // valorise au vrai prix et impose les limites de flux exactement.
        let mut ep_ct = expected_premiums.clone();
        let mut ever_touched = vec![false; n_b];
        let mut dps_cur = dps;
        for _round in 0..hp.ct_rounds.max(1) {
            let flows_all = ct_simulate_flows(
                challenge,
                &dps_cur,
                &sens,
                &candidate_lines,
                ...
                .map(|(b, battery)| {
                    if !ever_touched[b] {
                        ...
                        let da_ct: Vec<f64> = (0..n_t)
                            .map(|t| price_at(t, node) + ep_ct[t][b])
                            .collect();
                        build_battery_dp(
                            battery,
                            &da_ct,
            ...
            let fleet_da: Vec<f64> = (0..n_t)
                .map(|t| {
                    ...
                    for batt in challenge.batteries.iter() {
                        let px = if hp.xp_cwv { price_at(t, batt.node) } else { challenge.market.day_ahead_prices[t][batt.node] };
                        p += batt.capacity_mwh * px;
                    }
                    p / total_cap
                })
                .collect();
                ...
                let cluster_da: Vec<f64> = (0..n_t)
                    .map(|t| {
                        cluster_bats.iter().map(|b| {
                            b.capacity_mwh * (if hp.xp_cwv { price_at(t, b.node) } else { challenge.market.day_ahead_prices[t][b.node] })
                        }).sum::<f64>() / cluster_cap
                    })
                    .collect();
    ...
    // Le garde-fou n'est arme que si la mesure de carburant est exploitable. Une lecture
    // nulle ou minuscule ferait sortir des zeros a CHAQUE pas, donc un planning vide et un
    // profit nul : c'est le pire resultat possible, et il ne signale rien d'utile. Dans ce
    // cas on laisse la politique tourner.
    let fuel_guard_armed = available >= 1_000_000_000 && fuel_floor > 0;
    let mut solution = challenge.grid_optimize(&|c, s| {
        if fuel_guard_armed && fuel_remaining() <= fuel_floor {
        ...
        policy(c, s, &dps, &sens, &hp, &delta_cong, gram, price_ref.as_ref())
    })?;
