...
// ===========================================================================
// FULL-HORIZON PLAN POLISH
// ---------------------------------------------------------------------------
// The per-step policy above is causal: it commits an action before the rest of
// the price path is known. The whole price path is in fact recoverable from the
// instance, so once the causal rollout has produced a FEASIBLE schedule we can
// re-optimise that schedule over the whole horizon, off-line, against the true
// prices.
//
// The horizon problem couples batteries only through the per-step line limits.
// Freeze every battery but one and those limits collapse into a per-step action
// INTERVAL for the remaining battery; inside that interval its 192-step
// trajectory is an independent single-storage arbitrage on a known price series,
// solved by dynamic programming over the state of charge. Sweeping battery by
// battery is a block-coordinate ascent on the true objective: every accepted
// block keeps the schedule feasible by construction and can only raise profit,
// so the causal schedule is a floor.
// ===========================================================================
...
/// Flows produced by the exogenous injections alone, step by step.
...
/// Single-battery arbitrage on a known price series, restricted to a per-step
/// action interval. Backward DP over a state-of-charge grid, then a forward roll
/// that re-selects the action at the exact (continuous) state of charge.
fn plan_dp_battery(
    battery: &Battery,
    lam: &[f64],
    lo: &[f64],
    ...
    for t in (0..n_t).rev() {
        ...
        for i in 0..levels {
            let soc = smin + h * i as f64;
            let mut best = f64::NEG_INFINITY;
            for j in 0..acts.len() {
                let ns = soc + de[j];
                if ns < smin - 1e-9 || ns > smax + 1e-9 {
                    continue;
                }
                let v = imm[j] + plan_interp(nxt, smin, h_inv, last, ns);
                if v > best {
                    best = v;
                }
            }
            cur[i] = if best.is_finite() { best } else { -1e30 };
        }
    }
...
/// True profit of one battery's trajectory (the objective is separable given the prices).
...
/// Exact feasibility audit of a whole schedule against the challenge's own rules.
...
/// Block-coordinate ascent over the whole horizon. Returns an improved schedule,
/// or None if nothing beat the incumbent.
fn plan_polish_schedule(
    ...
    let na = hp.plan_act_levels.max(3);
    let margin = hp.plan_flow_margin.clamp(0.0, 1e-2);
    let limits = &challenge.network.flow_limits;

    // u[b][t] and the per-battery price series
    ...
    // lines each battery actually loads
    ...
    'sweeps: for _sweep in 0..hp.plan_sweeps {
        ...
        for b in 0..n_b {
            ...
            for t in 0..n_t {
                // the incumbent is feasible, so never exclude it
                if u[b][t] < lo[t] {
                    lo[t] = u[b][t];
                }
                if u[b][t] > hi[t] {
...
pub fn solve_challenge(
    ...
    if hp.plan_polish != 0 && fuel_remaining() > fuel_floor {
        if let Some(prices) = exact_price_table(challenge) {
            if let Some(better) =
                plan_polish_schedule(challenge, &solution.schedule, prices, &sens, &hp, fuel_floor)
            {
                save_solution(&Solution { schedule: better })?;
            }
        }
    }
    Ok(())
