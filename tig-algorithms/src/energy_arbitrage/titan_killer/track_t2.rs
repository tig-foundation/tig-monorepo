...
mod helpers {
    ...
    pub struct TrackHp {
        ...
        pub derate_mode: usize,
        pub derate_floor: f64,
        pub derate_scale: f64,
        pub derate_share_pow: f64,
        pub derate_calib_rounds: usize,
        pub derate_smooth: usize,
        pub derate_defl_passes: usize,
        pub derate_reach_p: f64,
    ...
    impl TrackHp {
        pub fn override_from_map(&mut self, h: &Option<Map<String, Value>>) {
            ...
            if let Some(v) = m.get("derate_mode").and_then(|v| v.as_u64()) { self.derate_mode = v as usize; }
            if let Some(v) = m.get("derate_floor").and_then(|v| v.as_f64()) { self.derate_floor = v.clamp(0.0, 1.0); }
            if let Some(v) = m.get("derate_scale").and_then(|v| v.as_f64()) { self.derate_scale = v.clamp(0.01, 20.0); }
            if let Some(v) = m.get("derate_share_pow").and_then(|v| v.as_f64()) { self.derate_share_pow = v.clamp(0.0, 4.0); }
            if let Some(v) = m.get("derate_calib_rounds").and_then(|v| v.as_u64()) { self.derate_calib_rounds = (v as usize).min(6); }
            if let Some(v) = m.get("derate_smooth").and_then(|v| v.as_u64()) { self.derate_smooth = (v as usize).min(48); }
            if let Some(v) = m.get("derate_defl_passes").and_then(|v| v.as_u64()) { self.derate_defl_passes = (v as usize).clamp(1, 40); }
            if let Some(v) = m.get("derate_reach_p").and_then(|v| v.as_f64()) { self.derate_reach_p = v.clamp(1.0, 4.0); }
    ...
    thread_local! {
        ...
        static DERATE: RefCell<Option<Vec<Vec<[f64; 2]>>>> = RefCell::new(None);
    }
    ...
    pub fn solve_with_hp(
        ...
        DERATE.with(|cell| *cell.borrow_mut() = None);
        let solution = out?;
    ...
    fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
        STATE.with(|s| -> Result<Vec<f64>> {
            ...
            let (actions, used_lns) = if hp.use_lns {
                if let Some(mut lns_act) = lns_dw_per_step(challenge, state, cache, hp, &flows_base, &actions, &uncons_actions) {
            ...
            polish_cd(challenge, state, cache, hp, &flows_base, &mut actions);
    ...
    fn ldd_simulate_flows_and_socs(
        challenge: &Challenge,
        state: &State,
        dp: &[Vec<Vec<f64>>],
        batt_nodes: &[usize],
        ...
        let mut socs_traj = vec![vec![0.0_f64; num_b]; num_t];

        for t in 0..num_t {
            for b in 0..num_b {
                socs_traj[t][b] = socs[b];
            }
    ...
    #[inline]
    fn optimal_unconstrained_action_with_price(
        challenge: &Challenge,
        state: &State,
        ca: &AycdicdbCache,
        b: usize,
        ...
        let mut best_u = 0.0_f64;
        let mut best_val = eval_profit_with_price(challenge, state, ca, b, 0.0, price);
        if !best_val.is_finite() { best_val = f64::NEG_INFINITY; }

        if u_min < -1e-12 {
            let hi = 0.0_f64.min(u_max);
            if u_min < hi {
                let b_c = dt * (lambda * bat.efficiency_charge - price - 0.25);
                if two_deg > 1e-30 {
                    let raw_charge = (-b_c) / two_deg; 
                    let cand = (-raw_charge.clamp(0.0, (-u_min).max(0.0))).clamp(u_min, hi);
                    if cand >= u_min && cand <= hi {
                        let v = eval_profit_with_price(challenge, state, ca, b, cand, price);
                        if v.is_finite() && v > best_val { best_val = v; best_u = cand; }
                    }
                }
                {
                    let v_lo = eval_profit_with_price(challenge, state, ca, b, u_min, price);
                    if v_lo.is_finite() && v_lo > best_val { best_val = v_lo; best_u = u_min; }
                }
            }
        }
        ...
        if u_max > 1e-12 {
            ...
            if lo < u_max {
                let eff_d = bat.efficiency_discharge.max(1e-9);
                let b_d = dt * (price - 0.25 - lambda / eff_d);
                if two_deg > 1e-30 {
                    let raw_disch = b_d / two_deg;
                    let cand = raw_disch.clamp(lo, u_max);
                    if cand >= lo && cand <= u_max {
                ...
                {
                    let v_hi = eval_profit_with_price(challenge, state, ca, b, u_max, price);
                    if v_hi.is_finite() && v_hi > best_val { best_u = u_max; }
                }
            }
        }
    ...
    fn lns_dw_per_step(
        ...
        for _cg_iter in 0..hp.lns_cg_iters {
            ...
            for b in 0..num_b {
                ...
                let dp_cand = optimal_unconstrained_action_with_price(challenge, state, ca, b, eff_price);
                if dp_cand.is_finite() {
                    let (lo, hi) = state.action_bounds[b];
                    let dp_clamped = dp_cand.clamp(lo, hi);
                    let exists2 = col_pools[b].iter().any(|&ex| (ex - dp_clamped).abs() < 1e-4);
                    if !exists2 && col_pools[b].len() < hp.lns_cg_column_limit {
                        col_pools[b].push(dp_clamped);
                        added_any = true;
                    }
                }
            }
    ...
    fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> AycdicdbCache {
        ...
        DERATE.with(|c| {
            *c.borrow_mut() = if hp.derate_mode > 0 && num_l > 0 {
                Some(build_derate_table(challenge, hp, &ptdf_sparse, &b_to_lines))
            } else {
                None
            };
        });

        // Self-calibration of the network throttling factor. The value function is
        // built with the current guess, that policy is rolled forward with the line
        // limits actually enforced, and the fraction of each requested action that
        // survived becomes the next guess. A fixed point of "what the network lets
        // this battery do", instead of one hand-set constant for the whole fleet.
        if hp.derate_mode == 2 && num_l > 0 {
            let zero_mu = vec![vec![0.0_f64; num_l]; num_t];
            let zero_ep = vec![vec![0.0_f64; num_b]; num_t];
            for _ in 0..hp.derate_calib_rounds {
                let dp_probe = build_dp_parallel_or_serial(
                    challenge, hp, &batt_nodes, &zero_ep, &b_to_lines, &zero_mu,
                );
                let phi = simulate_throttle(
                    challenge, state, &dp_probe, &batt_nodes, &ptdf_sparse, hp,
                );
                let tbl = phi_to_derate(phi, hp);
                DERATE.with(|c| *c.borrow_mut() = Some(tbl));
            }
        }

        let mut expected_premiums = vec![vec![0.0_f64; num_b]; num_t];
        if !use_expanded() && hp.use_action_aware_premium && hp.anticipate_lmp && num_l > 0 {
            ...
            for t in 0..num_t {
                ...
                for l in 0..num_l {
                    ...
                    if ratio > hp.lmp_threshold {
                        ...
                        for &(b, impact) in &ptdf_sparse[l] {
                            if impact.abs() > 1e-6 {
                                seed_premiums[t][b] += -impact * sign_f * premium;
                            }
                        }
                    }
                }
            ...
            let (flows_total, _) = ldd_simulate_flows_and_socs(challenge, state, &dp_probe, &batt_nodes, &ptdf_sparse);
            for t in 0..num_t {
                for l in 0..num_l {
                    let limit = challenge.network.flow_limits[l];
                    if limit <= 1e-6 { continue; }
                    ...
                    if ratio > hp.lmp_threshold {
                        ...
                        for &(b, impact) in &ptdf_sparse[l] {
                            if impact.abs() > 1e-6 {
                                expected_premiums[t][b] += -impact * sign_f * premium;
                            }
                        }
                    }
                }
            }
        } else if !use_expanded() && hp.anticipate_lmp && num_l > 0 {
            let base_premium = 20.0 * hp.lmp_premium_scale;
            for t in 0..num_t {
                let f_exo = challenge.network.compute_flows(&challenge.exogenous_injections[t]);
                for l in 0..num_l {
        ...
        let dp = if hp.use_dw && num_l > 0 {
            let dw_solve = |ep: &Vec<Vec<f64>>| -> (Vec<Vec<Vec<f64>>>, Vec<Vec<f64>>) {
                let prescreen_lines: Vec<usize> = if hp.use_dw_prescreen {
                    let zero_mu = vec![vec![0.0_f64; num_l]; num_t];
                    let dp0 = build_dp_parallel_or_serial(challenge, hp, &batt_nodes, ep, &b_to_lines, &zero_mu);
                    let (flows_all, _) = ldd_simulate_flows_and_socs(challenge, state, &dp0, &batt_nodes, &ptdf_sparse);
            ...
            if hp.use_ptdf_constraint_tracking && hp.coord_premium_mode == 0 && num_l > 0 {
                let (flows_all, _) = ldd_simulate_flows_and_socs(challenge, state, &dp1, &batt_nodes, &ptdf_sparse);
            ...
            if hp.use_ldd_proximal {
                ...
                let soc_ref: Vec<Vec<f64>> = if hp.ct_ref_kappa > 0.0 {
                    let (_, sref) = ldd_simulate_flows_and_socs(challenge, state, &dp, &batt_nodes, &ptdf_sparse);
                    sref
                } else {
                    Vec::new()
                };
                ...
                for ldd_iter in 0..hp.ldd_iters {
                    let (flows_all, soc_cur) = ldd_simulate_flows_and_socs(challenge, state, &dp, &batt_nodes, &ptdf_sparse);
                ...
                if fallback {
                    ...
                    dp = build_dp_parallel_or_serial(challenge, hp, &batt_nodes, &expected_premiums, &b_to_lines, &mu);
                    for ldd_iter in 0..hp.ldd_iters {
                        let (flows_all, _) = ldd_simulate_flows_and_socs(challenge, state, &dp, &batt_nodes, &ptdf_sparse);
            ...
            let (flows_all, _) = ldd_simulate_flows_and_socs(challenge, state, &dp_low, &batt_nodes, &ptdf_sparse);
            for t in 0..num_t {
                for l in 0..num_l {
                    let limit = challenge.network.flow_limits[l];
                    if limit <= 1e-6 { continue; }
    ...
    fn run_asca(
        ...
        let max_flow_util = (0..num_l).map(|l| {
            let limit = (challenge.network.flow_limits[l] - hp.flow_margin).max(0.0);
            if limit > 1e-6 { flows[l].abs() / limit } else { 0.0 }
        }).fold(0.0_f64, f64::max);
        let asca_iters_local = if num_b <= 20 && max_flow_util > 0.92 { hp.asca_iters + 2 } else { hp.asca_iters };
        for _sweep in 0..asca_iters_local {
    ...
    fn run_deflator(
        ...
        for _ in 0..rounds {
            ...
            for &(_, l_anchor) in &line_rank {
                ...
                let mut active_line_idxs: Vec<usize> = Vec::new();
                for l in 0..num_l {
                    if touched_lines[l] {
                        active_line_idxs.push(l);
                    }
                }
                let n_act = active_line_idxs.len();
                if n_act > 0 && bundle_size > 0 {
                    let n_vars = 2 * bundle_size;
                    let m = 2 * bundle_size + 2 * n_act;
                    let mut c_vec = vec![0.0_f64; n_vars];
                    let mut a_mat = vec![vec![0.0_f64; n_vars]; m];
                    let mut b_vec = vec![0.0_f64; m];
                    ...
                    for (i, &b) in bundle.iter().enumerate() {
                        ...
                        let cur_u = actions[b];
                        let deg_marginal_d = 2.0 * cur_u.max(0.0) * (dt / cap).powi(2);
                        let deg_marginal_c = 2.0 * (-cur_u).max(0.0) * (dt / cap).powi(2);
                        let c_disch = (rt - 0.25) * dt - lambda * dt / eta_d - deg_marginal_d;
                        let c_charge = -(rt + 0.25) * dt + lambda * eta_c * dt - deg_marginal_c;
                        let idx_plus = 2 * i;
                        let idx_minus = 2 * i + 1;
                        c_vec[idx_plus]  = c_disch;
                        c_vec[idx_minus] = c_charge;
                        let (lo, hi) = state.action_bounds[b];
                        let d_max_plus = (hi - cur).max(0.0);
                        let d_max_minus = (cur - lo).max(0.0);
                        a_mat[2 * i][idx_plus] = 1.0;
                        b_vec[2 * i] = d_max_plus;
                        a_mat[2 * i + 1][idx_minus] = 1.0;
                        b_vec[2 * i + 1] = d_max_minus;
                    }
                    let row_base = 2 * bundle_size;
                    for (ai, &l) in active_line_idxs.iter().enumerate() {
                        let limit = (challenge.network.flow_limits[l] - hp.flow_margin).max(0.0);
                        let cur_flow = flows[l];
                        let rp = row_base + 2 * ai;
                        let rn = rp + 1;
                        for (i, &b) in bundle.iter().enumerate() {
                        ...
                        b_vec[rp] = (limit - cur_flow).max(0.0);
                        b_vec[rn] = (limit + cur_flow).max(0.0);
                    }

                    let mut best_gain = 0.0_f64;
                    let mut best_move: Option<Vec<(usize, f64)>> = None;
                    let lp_budget = 200;
                    let (opt_delta, _) = super::lp::lp_solve_with_budget(n_vars, m, &c_vec, &a_mat, &b_vec, lp_budget);
                    if let Some(delta) = opt_delta {
                        let mut new_actions = actions.to_vec();
                        for (i, &b) in bundle.iter().enumerate() {
                            let d_plus = delta[2 * i];
                            let d_minus = delta[2 * i + 1];
                            let du = d_plus - d_minus;
                            let cur = actions[b];
                            let new_u = (cur + du).clamp(state.action_bounds[b].0, state.action_bounds[b].1);
                            new_actions[b] = new_u;
                        }
                        let mut feasible = true;
                        for l in 0..num_l {
                            if !touched_lines[l] { continue; }
                            let limit = (challenge.network.flow_limits[l] - hp.flow_margin).max(0.0);
                            let mut flow_new = flows[l];
                            for &b in &bundle {
                                let du = new_actions[b] - actions[b];
                                if du.abs() < 1e-12 { continue; }
                        ...
                        if feasible {
                            let gain: f64 = bundle.iter().map(|&b| {
                                eval_profit(challenge, state, ca, b, new_actions[b])
                                    - eval_profit(challenge, state, ca, b, actions[b])
                            }).sum();
                            if gain > 1e-9 {
                                best_gain = gain;
                                best_move = Some(bundle.iter().map(|&b| (b, new_actions[b])).collect());
                            }
                        }
                    }
    ...
    fn polish_cd(
        ...
        let mut flows = flows_base.to_vec();
        for l in 0..num_l {
            for &(b, p) in &ca.ptdf_sparse[l] { flows[l] += p * actions[b]; }
        }
        let ternary_iters = hp.ternary_iters.min(12);
        let max_sweeps = if num_b <= 24 { 2 } else { 1 };
        for _ in 0..max_sweeps {
            for b in 0..num_b {
                let cur = actions[b];
                let (lo, hi) = state.action_bounds[b];
                let (mut u_min, mut u_max) = (lo, hi);
                for &(l, p) in &ca.b_to_lines[b] {
                    if p.abs() < 1e-12 { continue; }
                    let limit = challenge.network.flow_limits[l];
                    let f_other = flows[l] - p * cur;
                    ...
                    let x2 = ( limit - f_other) / p;
                    let (llo, hhi) = if x1 < x2 { (x1, x2) } else { (x2, x1) };
                    if llo > u_min { u_min = llo; }
                    if hhi < u_max { u_max = hhi; }
                }
                if u_min > cur { u_min = cur; }
                if u_max < cur { u_max = cur; }
                if u_max - u_min < 1e-12 { continue; }
                let eval = |u: f64| -> f64 { eval_profit(challenge, state, ca, b, u) };
                let mut best_u = cur;
                let mut best_v = eval(cur);
                {
                    let v_lo = eval(u_min);
                    ...
                    if u_min < 0.0 {
                        let lo_neg = u_min; let hi_neg = 0.0_f64.min(u_max);
                        if lo_neg < hi_neg {
                            let (u, v) = ternary_search(|x| eval(x), lo_neg, hi_neg, ternary_iters);
                            if v > best_v { best_v = v; best_u = u; }
                        }
                    }
                    if u_max > 0.0 {
                        let lo_pos = 0.0_f64.max(u_min); let hi_pos = u_max;
                        if lo_pos < hi_pos {
                            let (u, v) = ternary_search(|x| eval(x), lo_pos, hi_pos, ternary_iters);
                            if v > best_v { best_u = u; }
                        }
                    }
                }
        ...
        let mut mu2 = vec![0.0_f64; num_l];
        {
            ...
            if active_for_mu.len() > 0 {
                ...
                if let Some(duals) = duals_opt {
                    ...
                    let step_scale = price_scale * 0.5;
                    for _iter in 0..2 {
                        let alpha = step_scale / ((_iter as f64).max(1.0));
                        for l in 0..num_l {
                            let lim = challenge.network.flow_limits[l];
                            if lim < 1e-6 { continue; }
                            let flow = total_flow1[l];
                            let abs_flow = flow.abs();
                            if abs_flow > lim {
                                let viol = abs_flow - lim;
                                let sign = flow.signum();
                                mu2[l] += alpha * viol * sign;
                                mu2[l] = mu2[l].abs().min(price_scale * 2.0);
                            }
                        }
                    }
                }
                ...
                for l in 0..num_l {
                    ...
                    let util = total_flow1[l].abs() / lim;
                    if util > 0.75 {
                        let excess_frac = ((util - 0.75_f64)
                            / (1.0_f64 - 0.75_f64).max(1e-6_f64)).min(1.0_f64);
        ...
        let mut infos: Vec<LineInfo> = raw.into_iter()
            .map(|(l, exo_util, hinted_util, sens_mass, violated, norm_sq, violation_mag)| {
                ...
                if hinted_util > 0.95 {
                    ...
                    stress += 1.5 * (exo_util - 0.95);
                }
                if violated {
                    stress += 10.0 + 20.0 * violation_mag.min(2.0);
                } else if hinted_util > 0.85 {
                    stress += 2.0 * (hinted_util - 0.85) * 10.0;
                } else if exo_util > 0.85 {
                    stress += 2.0 * (exo_util - 0.85) * 10.0;
                }
    ...
    /// Per-battery, per-step share of nominal power that the transmission network
    /// can actually absorb. Replaces the single scalar `network_derating` that the
    /// value-function builder used to apply uniformly to every battery and step.
    ///
    /// For each step t we take the flow produced by the exogenous injections alone
    /// (the same balancing rule as `compute_total_injections`: the slack entry is
    /// overwritten, not added). The residual room on line l is
    ///     H[l][t] = max(0, limit[l] - |f_exo[l][t]|)
    /// and the total room the battery fleet would need if every unit ran flat out is
    ///     R[l]    = sum_b |a[l][b]| * P_b .
    /// The line can therefore honour only the fraction s[l][t] = min(1, H/R) of the
    /// fleet's nominal swing. A battery is exposed to a line in proportion to its own
    /// sensitivity |a[l][b]|, so its share is the sensitivity-weighted mean of s over
    /// the lines it actually moves. The result multiplies the nominal power inside the
    /// dynamic-programming value function only; the dispatch itself keeps enforcing the
    /// true limits step by step, so this can never create an infeasible flow.
    fn build_derate_table(
        challenge: &Challenge,
        hp: &TrackHp,
        ptdf_sparse: &[Vec<(usize, f64)>],
        b_to_lines: &[Vec<(usize, f64)>],
        ...
        // R[l] : total nominal swing the fleet can put on line l
        // How much swing the fleet can really put on line l. p = 1 is the worst case
        // (every battery pushing the same way at once); p = 2 is the opposite belief
        // (impacts independent, so they partly cancel). The exponent says how much
        // cancellation to expect between batteries sharing a line.
        let pw = hp.derate_reach_p;
        let mut reach = vec![0.0_f64; num_l];
        for l in 0..num_l {
            let mut acc = 0.0_f64;
            for &(b, impact) in &ptdf_sparse[l] {
                let bat = &challenge.batteries[b];
                ...
                let x = impact.abs() * p;
                acc += if (pw - 1.0).abs() < 1e-12 { x } else { x.powf(pw) };
            }
            reach[l] = if (pw - 1.0).abs() < 1e-12 { acc } else { acc.powf(1.0 / pw) };
        }

        let mut out = vec![vec![[hp.network_derating; 2]; num_b]; num_t];
        let mut inj = vec![0.0_f64; num_n];
        let directional = hp.derate_mode >= 3;
        for t in 0..num_t {
            if t >= challenge.exogenous_injections.len() { break; }
            let exo = &challenge.exogenous_injections[t];
            let mut sum = 0.0_f64;
            for k in 0..num_n {
                inj[k] = if k == slack { 0.0 } else { exo[k] };
                if k != slack { sum += inj[k]; }
            }
            inj[slack] = -sum;
            let f_exo = challenge.network.compute_flows(&inj);

            // Room left on each line, SIGNED. A line already carrying +f can still
            // take (limit - f) more in the positive direction but (limit + f) in the
            // negative one: congestion has a direction, and a battery that would
            // relieve the line is not constrained by it at all.
            let mut room_pos = vec![1.0_f64; num_l];
            let mut room_neg = vec![1.0_f64; num_l];
            for l in 0..num_l {
                let limit = challenge.network.flow_limits[l];
                if limit <= 1e-6 || reach[l] <= 1e-9 { continue; }
                if directional {
                    room_pos[l] = ((limit - f_exo[l]).max(0.0) / reach[l]).min(1.0);
                    room_neg[l] = ((limit + f_exo[l]).max(0.0) / reach[l]).min(1.0);
                } else {
                    let sh = ((limit - f_exo[l].abs()).max(0.0) / reach[l]).min(1.0);
                    room_pos[l] = sh;
                    room_neg[l] = sh;
                }
            }
            ...
            for b in 0..num_b {
                ...
                for &(l, impact) in &b_to_lines[b] {
                    let limit = challenge.network.flow_limits[l];
                    if limit <= 1e-6 { continue; }
                    let w = if (hp.derate_share_pow - 1.0).abs() < 1e-12 {
                        impact.abs()
                    } else {
                        impact.abs().powf(hp.derate_share_pow)
                    };
                    if w <= 0.0 { continue; }
                    wsum += w;
                    // discharging (u > 0) moves the line by +impact, charging by -impact
                    if impact >= 0.0 {
                        acc_d += w * room_pos[l];
                        acc_c += w * room_neg[l];
                    } else {
                ...
                let (dd, dc) = if wsum > 1e-12 { (acc_d / wsum, acc_c / wsum) } else { (1.0, 1.0) };
                out[t][b] = [
                    (dc * hp.derate_scale).clamp(hp.derate_floor, 1.0),
                    (dd * hp.derate_scale).clamp(hp.derate_floor, 1.0),
                ];
            }
        }
        out
    ...
    /// Nominal-power multiplier used by the value-function builders at step `t`
    /// for battery `b`: the measured network share when it has been built, the
    /// legacy scalar otherwise.
    fn derate_at(hp: &TrackHp, t: usize, b: usize) -> [f64; 2] {
        if hp.derate_mode == 0 { return [hp.network_derating; 2]; }
        DERATE.with(|c| {
            match c.borrow().as_ref() {
                Some(tbl) if t < tbl.len() && b < tbl[t].len() => tbl[t][b],
                _ => [hp.network_derating; 2],
            }
        })
    }


    /// Exogenous-only flows at step `t`, built with the challenge's own balancing
    /// rule (the slack entry is OVERWRITTEN by the negated sum, never added to).
    fn exogenous_flows_at(challenge: &Challenge, t: usize) -> Vec<f64> {
        let num_n = challenge.network.num_nodes;
        let slack = challenge.network.slack_bus;
        let exo = &challenge.exogenous_injections[t];
        let mut inj = vec![0.0_f64; num_n];
        ...
        for k in 0..num_n {
            if k != slack {
                inj[k] = if k < exo.len() { exo[k] } else { 0.0 };
                sum += inj[k];
            }
        }
        inj[slack] = -sum;
    ...
    /// Proportional deflation of an action vector until every line is inside its
    /// limit. Only the batteries that push a line further into violation are cut,
    /// each in proportion to how much it contributes — the same rule the per-step
    /// dispatch ends up applying, replayed cheaply over the whole horizon.
    fn deflate_actions(
        challenge: &Challenge,
        ptdf_sparse: &[Vec<(usize, f64)>],
        exo_flows: &[f64],
        ...
        for _ in 0..passes {
            ...
            for l in 0..num_l {
                let limit = challenge.network.flow_limits[l];
                if limit <= 1e-6 { continue; }
                let mut f = exo_flows[l];
                for &(b, impact) in &ptdf_sparse[l] { f += impact * actions[b]; }
                let viol = f.abs() - limit;
                if viol > worst { worst = viol; worst_l = l; }
            }
            if worst_l == usize::MAX || worst <= 1e-9 { return; }
            let l = worst_l;
            let mut f = exo_flows[l];
            for &(b, impact) in &ptdf_sparse[l] { f += impact * actions[b]; }
            let sign = if f >= 0.0 { 1.0_f64 } else { -1.0_f64 };
            let mut contrib = 0.0_f64;
            for &(b, impact) in &ptdf_sparse[l] {
                let c = impact * actions[b] * sign;
                if c > 0.0 { contrib += c; }
            }
            if contrib <= 1e-12 { return; }
            let shrink = (1.0 - worst / contrib).max(0.0);
            for &(b, impact) in &ptdf_sparse[l] {
                if impact * actions[b] * sign > 0.0 { actions[b] *= shrink; }
            }
        }
    }

    /// Roll the value-function greedy forward over the whole horizon WITH the line
    /// limits enforced, and record, for each step and battery, the fraction of the
    /// action it asked for that the network actually let through.
    ///
    /// This is the quantity the value-function builder needs: not "how much power
    /// does this battery have" (its nameplate) and not "how much does it use" (which
    /// is near zero whenever the price is uninteresting), but "when it wants to move,
    /// how much of that move survives the network". A single global constant was
    /// standing in for it.
    fn simulate_throttle(
        ...
        let mut socs: Vec<f64> = state.socs.clone();
        let mut phi = vec![vec![1.0_f64; num_b]; num_t];

        for t in 0..num_t {
            let exo_flows = exogenous_flows_at(challenge, t);
            let mut want = vec![0.0_f64; num_b];
            ...
            for b in 0..num_b {
                ...
                let soc_span = (bat.soc_max_mwh - bat.soc_min_mwh).max(1e-9);
                let soc_levels = dp[b][0].len();
                let p_node = cached_node_price(challenge, t, batt_nodes[b]);
                ...
                for j in 0..=sim_pts {
                    ...
                    let abs_u = u.abs();
                    let deg_base = (abs_u * dt) / bat.capacity_mwh.max(1e-9);
                    let profit = u * p_node * dt - 0.25 * abs_u * dt - deg_base * deg_base;
                    let next_soc_raw = if u < 0.0 {
                        soc + bat.efficiency_charge * (-u) * dt
                    } else {
                        soc - u / bat.efficiency_discharge.max(1e-9) * dt
            ...
            deflate_actions(challenge, ptdf_sparse, &exo_flows, &mut got, hp.derate_defl_passes);

            for b in 0..num_b {
                let bat = &challenge.batteries[b];
                let nominal = bat.power_charge_mw.max(bat.power_discharge_mw).max(1e-9);
                if want[b].abs() > 1e-4 * nominal {
                    phi[t][b] = (got[b].abs() / want[b].abs()).clamp(0.0, 1.0);
                }
                let u = got[b];
                let next_soc_raw = if u < 0.0 {
                    socs[b] + bat.efficiency_charge * (-u) * dt
    ...
    /// Turn a measured throttling record into the multiplier the value function uses:
    /// optional moving average over time, then the scale/floor guards.
    fn phi_to_derate(phi: Vec<Vec<f64>>, hp: &TrackHp) -> Vec<Vec<[f64; 2]>> {
        let num_t = phi.len();
        if num_t == 0 { return Vec::new(); }
        let num_b = phi[0].len();
        let w = hp.derate_smooth;
        let mut out = vec![vec![[0.0_f64; 2]; num_b]; num_t];
        for t in 0..num_t {
            let lo = t.saturating_sub(w);
            let hi = (t + w).min(num_t - 1);
            let cnt = (hi - lo + 1) as f64;
            for b in 0..num_b {
                let mut acc = 0.0_f64;
                for k in lo..=hi { acc += phi[k][b]; }
                let v = ((acc / cnt) * hp.derate_scale).clamp(hp.derate_floor, 1.0);
                out[t][b] = [v, v];
            }
        }
        out
    ...
    fn build_dp_with_mu_for_battery(
        ...
        for t in (0..num_t).rev() {
            let p_da = cached_node_price(challenge, t, node);
            ...
            let p_sell = if expanded { (p_da + extra).max(1e-6) } else { p_da * (1.0 + hp.jump_premium) + extra };
            let p_buy = if expanded { (p_da + extra).max(1e-6) } else { p_da + extra };

            let dr = derate_at(hp, t, b);
            let max_pwr_c = bat.power_charge_mw * dr[0];
            let max_pwr_d = bat.power_discharge_mw * dr[1];
...
mod lp {
    ...
    pub fn lp_solve_with_budget(
        ...
        let mut tab = vec![0.0_f64; rows * n_cols];

        for i in 0..m {
            let row_base = i * n_cols;
            for j in 0..n {
                tab[row_base + j] = a[i][j];
            }
            tab[row_base + n + i] = 1.0;
            tab[row_base + rhs_col] = b[i].max(0.0);
        }

        let obj_base = m * n_cols;
        for j in 0..n {
            tab[obj_base + j] = -c[j];
        }
        ...
        for pivot in 0..max_pivots {
            ...
            let entering = match pick_entering(&tab[obj_base..obj_base + n_cols], n_vars, false) {
                Some(j) => j,
                None => break,
            };
            ...
            let leaving_row = (0..m)
                ...
                .min_by(|&i1, &i2| {
                    let a1 = tab[i1 * n_cols + entering];
                    let a2 = tab[i2 * n_cols + entering];
                    let r1 = tab[i1 * n_cols + rhs_col] / a1;
                    let r2 = tab[i2 * n_cols + rhs_col] / a2;
            ...
            let lr_base = leaving_row * n_cols;
            let pivot_val = tab[lr_base + entering];
            if pivot_val.abs() < LP_EPS {
                return (None, 0);
            }
            for j in 0..n_cols {
                tab[lr_base + j] /= pivot_val;
            }

            for i in 0..rows {
                if i != leaving_row {
                    let row_base = i * n_cols;
                    let factor = tab[row_base + entering];
                    if factor.abs() > 1e-15 {
                        for j in 0..n_cols {
                            tab[row_base + j] -= factor * tab[lr_base + j];
                        }
                    }
                }
            }
    ...
    pub fn lp_solve_with_duals(
        ...
        for pivot_count in 0..max_pivots {
            let entering_d = match pick_entering(&tab[obj_base..obj_base + n_cols], n_vars, use_dantzig) {
                Some(j) => j,
                None => {
                    let duals = (0..m).map(|i| {
                        let slack_col = n + i;
                        let val = -tab[obj_base + slack_col];
            ...
            let lr_d = (0..m)
                .filter(|&i| tab[i * n_cols + entering_d] > LP_EPS)
                .min_by(|&i1, &i2| {
                    let a1 = tab[i1 * n_cols + entering_d];
                    let a2 = tab[i2 * n_cols + entering_d];
                    let r1 = tab[i1 * n_cols + rhs_col] / a1;
                    let r2 = tab[i2 * n_cols + rhs_col] / a2;
                    r1.partial_cmp(&r2).unwrap_or(std::cmp::Ordering::Equal)
                });
            ...
            let (entering, leaving_row) = if use_dantzig && bland_degen {
                if let Some(lr) = lr_d {
                    if tab[lr * n_cols + rhs_col] <= LP_EPS {
                        let obj_slice = &tab[obj_base..obj_base + n_cols];
                        if let Some(bland_j) = (0..n_vars).find(|&j| obj_slice[j] < -LP_EPS) {
                            let lr_b = (0..m)
                                .filter(|&i| tab[i * n_cols + bland_j] > LP_EPS)
                                .min_by(|&i1, &i2| {
                                    let a1 = tab[i1 * n_cols + bland_j];
                                    let a2 = tab[i2 * n_cols + bland_j];
                                    let r1 = tab[i1 * n_cols + rhs_col] / a1;
                                    let r2 = tab[i2 * n_cols + rhs_col] / a2;
                                    r1.partial_cmp(&r2).unwrap_or(std::cmp::Ordering::Equal)
                                });
...
mod track_congested {
    ...
    fn defaults() -> TrackHp {
        TrackHp {
            ...
            derate_floor: 0.10,
            derate_scale: 1.0,
            derate_share_pow: 1.0,
            derate_calib_rounds: 1,
            derate_smooth: 0,
            derate_defl_passes: 8,
            derate_reach_p: 1.0,
            dual_iters: 0,
            da_step_size: 0.01,
            ldd_iters: 4,
            ...
            use_primal_refine: false,
            use_lmp_premiums_kkt: false,
