// energy_solver_multiday_6: multiday_5 with profit_weight_power exposed; tests if soften weighting differs on s=multiday.
...
pub struct Hyperparameters {
    pub profit_weight_power: f64,
}

impl Default for Hyperparameters {
    fn default() -> Self {
        Self {
            profit_weight_power: 2.545,
        }
    }
}
...
pub fn help() {
    ...
    println!("  profit_weight_power: float range=[0.0, 4.0] default=2.545  # exponent on |price_diff| in soften weighting (0=uniform)");
    println!();
    println!("Refines multiday_5 by re-tuning profit_weight_power. Thresholds (ct=10, dt=2), end_window=48,");
    println!("end_factor=0.0, slope=1.273, lookahead=72 held fixed.");
}

fn parse_hp(h: &Option<Map<String, Value>>) -> Hyperparameters {
    let mut out = Hyperparameters::default();
    if let Some(m) = h {
        if let Some(v) = m.get("profit_weight_power").and_then(|v| v.as_f64()) {
            out.profit_weight_power = v;
        }
    }
    out
}
...
const END_WINDOW: usize = 48;
const END_FACTOR: f64 = 0.0;
const SLOPE_FACTOR: f64 = 1.273;
const CHARGE_THRESHOLD: f64 = 10.0;
const DISCHARGE_THRESHOLD: f64 = 2.0;
...
fn policy_with_hp(challenge: &Challenge, state: &State, hp: &Hyperparameters) -> Result<Vec<f64>> {
    ...
    enforce_feasible(challenge, state, action, &profit, hp.profit_weight_power)
}
