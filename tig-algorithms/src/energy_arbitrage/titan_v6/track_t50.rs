...
mod helpers {
    ...
    pub struct TrackHp {
        ...
        // i86: DP stochastic uncertainty model params, was HARDCODED at build_dp_stochastic_for_battery
        ...
        // P12 (t50/i45): endogenous coordinated congestion premium from DW shadow prices.
    ...
    fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> AycdicdbCache {
        ...
        let dp = if hp.use_dw && num_l > 0 {
            ...
            // coordinated DW optimum (dual on the line-flow constraint; cf Hindsight 4237833e / 6929733d).
            ...
            if hp.use_ptdf_constraint_tracking && hp.coord_premium_mode == 0 && num_l > 0 {
                ...
                if hp.use_ct_adaptive_per_line {
                    // i60: per-line adaptive step. Two-pass per timestep:
                ...
                // i59: reference tracking (3rd Huang ingredient). Damp the CT correction by (1-kappa)
            ...
            if hp.use_ldd_proximal {
                ...
                // i59: reference tracking (3rd Huang ingredient). Anchor LDD correction on the
                ...
                // i65: OC-reference (4th Huang ingredient). Anchor LDD correction on ∂V/∂soc (water value)
...
mod lp {
    ...
    /// i21: flag passed directly per call-site (no thread_local, no global state).
    #[inline(always)]
    ...
    pub fn lp_solve_with_duals(
        ...
        for pivot_count in 0..max_pivots {
            ...
            // i23: applied at LNS call-site only (bland_degen flag).
...
mod track_congested {
    ...
    pub fn solve(
        ...
        // i21: dantzig flags are now per-call-site (dantzig_in_dw/lns/kkt), no global set needed
