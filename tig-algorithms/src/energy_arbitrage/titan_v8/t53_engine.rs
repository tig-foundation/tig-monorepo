...
pub struct Hyperparameters {
    ...
    /// P31 virtual-queue drift-plus-penalty (Zhou-Wang-Li 2024, port from t52/i45 KEPT +0.71%).
    ...
    /// L4 (port P-curvature from t50/i13, KEPT +5,262Q on t50): decision-regret
    ...
    /// mispriced dispatch carries the largest realized-congestion regret. Hindsight
    ...
    /// L6-bis FALLBACK ratio-of-averages congestion-premium estimator (Hindsight `b9dcf01f`/
    ...
    /// only. t52/i42 closed η=1.0 multi-round (monotone overshoot) but explicitly
    ...
    /// DP-only power derate rho in (0,1]. Scales the power limits used to BUILD the per-battery
    /// value function so it prices DELIVERABLE, not nameplate, capacity. The dispatch itself still
    /// uses full nameplate bounds -- only the value function is derated. 1.0 = bit-exact sentinel.
    #[serde(default = "default_dp_power_derate")]
    pub dp_power_derate: f64,

    /// Directional split of the derate. Discharge empties SOC 1/(eta_c*eta_d)=1.108x faster than
    /// charge fills it, and the fleet moves coherently, so the deliverable fraction is asymmetric.
    /// Applied as rho_c = dp_power_derate / dp_derate_asym, rho_d = dp_power_derate * dp_derate_asym,
    /// which keeps the GEOMETRIC MEAN pinned at dp_power_derate so only the asymmetry varies.
    /// 1.0 = symmetric = bit-exact with the scalar-rho behaviour.
    #[serde(default = "default_dp_derate_asym")]
    pub dp_derate_asym: f64,

    /// Congestion ratio threshold for the policy-time LMP premium (shipped const 0.5).
    #[serde(default = "default_lmp_threshold")]
    pub lmp_threshold_hp: f64,

    /// Scale of the policy-time LMP premium (shipped const 2.0). Tuned on the DENSE track, never
    /// on CAPSTONE, despite capstone having the tightest limits (gamma_cong 0.40).
    #[serde(default = "default_lmp_premium_scale")]
    pub lmp_premium_scale_hp: f64,

    /// Weight on the additive congestion premium when SCORING pair exchanges in
    /// `joint_pair_polish` (and, transitively, `basin_hop_restart`). The premium is already
    /// computed by `use_coupling_cut` but reaches only `pick_dp_action` for seed #1; the pair
    /// polish -- the engine's actual counter-trading primitive -- scores on raw RT prices.
    /// 0.0 = bit-exact sentinel.
    #[serde(default)]
    pub coupling_pair_w: f64,

    /// Add a 4th multi-start seed built by `pick_dp_action` at the CONGESTION-ADJUSTED price
    /// (rt_prices[node] + cong_seed_scale * coupling_prems[t][b]). The existing `dp_seed` uses the
    /// RAW price and `target` only sees the premium through an EPS-biased heuristic comparison, so
    /// no seed today is purely congestion-priced. Default false = bit-exact.
    #[serde(default)]
    pub use_cong_dp_seed: bool,

    /// Dual-ladder dose theta for the congestion-priced seed(s). The blind analysis shows restarts
    /// only ever change the SIGN PATTERN sigma = sign(c(mu)), so diversity must be in mu-space; a
    /// ladder of doses spans it. Default 1.0.
    #[serde(default = "default_cong_seed_scale")]
    pub cong_seed_scale: f64,

    /// Add a 5th seed at a SECOND dose, forming a 2-rung dual ladder. 0.0 = disabled (bit-exact).
    #[serde(default)]
    pub cong_seed_scale2: f64,

    /// Per-battery derate shape strength. 0.0 = uniform (bit-exact sentinel). Positive values
    /// spread rho_b across batteries by binding-line PTDF exposure, in log space, renormalised so
    /// the geometric mean stays at dp_power_derate (level unchanged, shape only).
    #[serde(default)]
    pub dp_derate_pb: f64,
}
...
fn default_dp_power_derate() -> f64 {
    1.0
}

fn default_dp_derate_asym() -> f64 {
    1.0
}

fn default_lmp_threshold() -> f64 {
    LMP_THRESHOLD
}

fn default_lmp_premium_scale() -> f64 {
    LMP_PREMIUM_SCALE
}

fn default_cong_seed_scale() -> f64 {
    1.0
}
...
/// L4 (port P-curvature from t50/i13): reshape the linear congestion-premium ramp
...
impl Default for Hyperparameters {
    fn default() -> Self {
        Self {
            ...
            dp_power_derate: 1.0,
            dp_derate_asym: 1.0,
            lmp_threshold_hp: LMP_THRESHOLD,
            lmp_premium_scale_hp: LMP_PREMIUM_SCALE,
            coupling_pair_w: 0.0,
            use_cong_dp_seed: false,
            cong_seed_scale: 1.0,
            cong_seed_scale2: 0.0,
            dp_derate_pb: 0.0,
        }
    }
}
...
impl Hyperparameters {
    ...
    fn parse(raw: &Option<Map<String, Value>>) -> Result<Self> {
        ...
        hp.dp_power_derate = hp.dp_power_derate.clamp(0.05, 1.0);
        hp.dp_derate_asym = hp.dp_derate_asym.clamp(0.25, 4.0);
        hp.lmp_threshold_hp = hp.lmp_threshold_hp.clamp(0.05, 0.99);
        hp.lmp_premium_scale_hp = hp.lmp_premium_scale_hp.clamp(0.0, 20.0);
        hp.coupling_pair_w = hp.coupling_pair_w.clamp(0.0, 5.0);
        hp.cong_seed_scale = hp.cong_seed_scale.clamp(0.0, 10.0);
        hp.cong_seed_scale2 = hp.cong_seed_scale2.clamp(0.0, 10.0);
        hp.dp_derate_pb = hp.dp_derate_pb.clamp(0.0, 2.0);
...
fn build_battery_dp(
    ...
    // DP-only power derate: value stored energy at deliverable power, not nameplate.
    // rho == 1.0 takes no branch => bit-exact with the unmodified engine.
    let derated_owned;
    let battery: &Battery = if hp.dp_power_derate < 1.0 - 1e-12 || (hp.dp_derate_asym - 1.0).abs() > 1e-12 || (pb_mult - 1.0).abs() > 1e-12 {
        let mut b = battery.clone();
        b.power_charge_mw *= pb_mult * hp.dp_power_derate / hp.dp_derate_asym;
        b.power_discharge_mw *= pb_mult * hp.dp_power_derate * hp.dp_derate_asym;
        derated_owned = b;
        &derated_owned
    } else {
        battery
...
fn basin_hop_restart(
    ...
    joint_pair_polish(challenge, state, dps, sens, base_flows, actions, hp, pair_prem);
...
fn joint_pair_polish(
    ...
    // Additive, sign-carrying congestion adjustment. w == 0.0 contributes exactly 0.0.
    let prem_at = |b: usize| -> f64 {
        if hp.coupling_pair_w != 0.0 && b < pair_prem.len() {
            hp.coupling_pair_w * pair_prem[b]
        } else {
            0.0
        }
    };
    ...
    let pair_order: Vec<(usize, usize)> = {
        ...
        if hp.pair_price_sorted {
            let mut scored: Vec<(usize, usize, f64)> = pairs
                .into_iter()
                .map(|(i, j)| {
                    let price_i = state.rt_prices[challenge.batteries[i].node] + prem_at(i);
                    let price_j = state.rt_prices[challenge.batteries[j].node] + prem_at(j);
                    let (lo_i, hi_i) = state.action_bounds[i];
                    let (lo_j, hi_j) = state.action_bounds[j];
                    let span_i = hi_i - lo_i;
                    let span_j = hi_j - lo_j;
    ...
    while improved {
        ...
        'outer: for &(i, j) in pair_order.iter() {
            ...
            let price_i = state.rt_prices[batt_i.node] + prem_at(i);
            let soc_i = state.socs[i];
            let (lo_i, hi_i) = state.action_bounds[i];
            let cur_i = actions[i];
            let span_i = hi_i - lo_i;
            let batt_j = &challenge.batteries[j];
            let price_j = state.rt_prices[batt_j.node] + prem_at(j);
            ...
            if (best_i - cur_i).abs() > EPS || (best_j - cur_j).abs() > EPS {
                ...
                if hp.pair_alpha_interval {
                    ...
                    // deeper swaps with fine moves (Hindsight 083c5a4b/174cc8d0)
...
fn policy(
    ...
    // Per-battery congestion premium for THIS step (empty when use_coupling_cut is off).
    let pair_prem_t: Vec<f64> = if t < coupling_prems.len() {
        coupling_prems[t].clone()
    } else {
        Vec::new()
    };
    ...
    let mut result = if hp.use_dual_dispatch {
        ...
        // Congestion-priced seeds (dual ladder). Appended AFTER truncate so num_seeds keeps its
        // original meaning and these are purely additive. Each is re-scored on the TRUE objective.
        if hp.use_cong_dp_seed && !pair_prem_t.is_empty() {
            for &theta in [hp.cong_seed_scale, hp.cong_seed_scale2].iter() {
                if theta <= 0.0 {
                    continue;
                }
                let cong_seed: Vec<f64> = (0..challenge.num_batteries)
                    .map(|b| {
                        let battery = &challenge.batteries[b];
                        let prem = if b < pair_prem_t.len() { pair_prem_t[b] } else { 0.0 };
                        pick_dp_action(
                            &dps[b],
                            battery,
                            t,
                            state.socs[b],
                            state.rt_prices[battery.node] + theta * prem,
                            state.action_bounds[b],
                            hp,
                        )
                    })
                    .collect();
                seeds.push(cong_seed);
            }
        }
    ...
    if hp.use_joint_pair_polish {
        ...
        joint_pair_polish(challenge, state, dps, sens, &base_flows, &mut result, hp, &pair_prem_t);
        if !is_flow_feasible(challenge, state, &result) {
            result = pre_polish;
        }
    }

    // P59 cross-poll: basin-hopping restart post-convergence (port from t51/i42).
    if hp.use_basin_hop {
        basin_hop_restart(challenge, state, dps, sens, &base_flows, &mut result, hp, &pair_prem_t);
    }
...
pub fn solve_challenge(
    ...
    // Per-battery derate shape from binding-line PTDF exposure (dp_derate_pb == 0.0 => all 1.0).
    // w_l = fraction of steps line l runs >=80% loaded under the KNOWN exogenous injections;
    // E_b = sum_l w_l * |sens[l][b]| / limit_l. Higher exposure => less deliverable => lower rho_b.
    let pb_mults: Vec<f64> = if hp.dp_derate_pb > 1e-12 {
        let n_l = challenge.network.num_lines;
        let n_b = challenge.num_batteries;
        let mut w = vec![0.0_f64; n_l];
        for t in 0..challenge.num_steps {
            ...
            for l in 0..n_l {
                ...
                if lim > 1e-9 && (f[l].abs() / lim) >= 0.8 {
                    w[l] += 1.0;
                }
            }
        }
        let inv_t = 1.0 / challenge.num_steps.max(1) as f64;
        for l in 0..n_l {
            w[l] *= inv_t;
        }
        let mut e = vec![0.0_f64; n_b];
        for l in 0..n_l {
            let lim = challenge.network.flow_limits[l];
            if lim <= 1e-9 || w[l] <= 0.0 {
                continue;
            ...
            for b in 0..n_b {
                e[b] += w[l] * sens[l][b].abs() / lim;
            }
        }
        // log-space spread around the mean exposure, renormalised to unit geometric mean
        let mean_e = e.iter().sum::<f64>() / n_b.max(1) as f64;
        let mut lg = vec![0.0_f64; n_b];
        for b in 0..n_b {
            let rel = if mean_e > 1e-12 { e[b] / mean_e } else { 1.0 };
            lg[b] = -hp.dp_derate_pb * (rel.max(1e-9)).ln();
        }
        let mean_lg = lg.iter().sum::<f64>() / n_b.max(1) as f64;
        lg.iter().map(|v| (v - mean_lg).exp().clamp(0.1, 10.0)).collect()
    } else {
        vec![1.0; challenge.num_batteries]
    };
    ...
    let expected_premiums: Vec<Vec<f64>> = if hp.anticipate_lmp && n_lines > 0 {
        let base_premium = 20.0 * hp.lmp_premium_scale_hp;
        let threshold = hp.lmp_threshold_hp;
        let n_t = challenge.num_steps;
        let n_b = challenge.num_batteries;
        let mut prem = vec![vec![0.0_f64; n_b]; n_t];
        if hp.use_ratio_avg_premium {
            // L6-bis ratio-of-averages: compute mean_t(|f_exo[l]|)/limit first (Hindsight
    ...
    let coupling_prems: Vec<Vec<f64>> = if hp.use_coupling_cut && n_lines > 0 {
        let n_b = challenge.num_batteries;
        let n_t = challenge.num_steps;
        let base = 20.0 * hp.lmp_premium_scale_hp;
        let threshold = hp.lmp_threshold_hp;
    ...
    let dps = if hp.use_ptdf_ct && n_lines > 0 {
        ...
        if vq_v > 0.0 {
            // P31 virtual-queue drift-plus-penalty (Zhou-Wang-Li 2024, port from t52/i45).
        ...
        // limit·0.25). t52/i42 root cause (η=1.0 stacking on a saturated μ) is avoided
