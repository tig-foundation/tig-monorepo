...
pub struct Hyperparameters {
    ...
    /// LP dispatch oracle alongside the PGA (port of t53 CASSURE CF=15, measured +44,220
    /// tuned / +47,776 held-out on CAPSTONE). Linearises the DP value function via dv/dsoc
    /// and solves the per-timestep dispatch EXACTLY on the PTDF polytope with an embedded
    /// dense simplex. The LP answer replaces the PGA answer iff it survives projection, is
    /// flow-feasible, and strictly beats it on `total_step_value`. It is inserted BEFORE the
    /// polish chain, so the winner is still polished.
    /// false = iso-baseline: the branch is dead code and the engine is BIT-EXACT.
    #[serde(default)]
    pub use_lp_dispatch: bool,
    /// Legacy line selector: keep only the K lines with the largest STATIC |base_flow|/limit.
    /// 0 = all lines (the configuration that produced the measured win). Truncation is known
    /// to be worthless (+47 at K=80 on capstone) because the ranking has zero dependence on
    /// `sens`; use `lp_screen_mode = 1` instead if the LP must be made cheaper.
    #[serde(default)]
    pub lp_max_lines: usize,
    /// Max simplex pivots per LP call. 0 = default (2000), identical to t53.
    #[serde(default)]
    pub lp_pivot_budget: usize,
    /// Drop the two provably redundant per-battery SOC rows. `state.action_bounds` is already
    /// min(soc-derived, nameplate) and is recomputed from the current SOC every step by the
    /// framework, so those rows are dominated by the box rows. Optimum unchanged; removes
    /// 2*num_batteries rows and the main source of ratio-test degeneracy. Default false.
    #[serde(default)]
    pub lp_drop_soc_rows: bool,
    /// 0 = legacy selector (`lp_max_lines`). 1 = CERTIFIED box-dominance screening: keep line
    /// l iff |base_flow[l]| + sum_b max(|sens[l][b]*lo_b|, |sens[l][b]*hi_b|) > limit[l]. A
    /// dropped line cannot be violated by any action in the box, so the optimum is provably
    /// unchanged. Default 0.
    #[serde(default)]
    pub lp_screen_mode: usize,
    /// 0 = evaluate dv/dsoc at the CURRENT soc (t53 behaviour). 1 = per-limb midpoint tangent:
    /// discharge coefficient at soc - 0.5*u_max*dt/eta_d, charge coefficient at
    /// soc + 0.5*(-u_min)*eta_c*dt. V is concave, so the current-soc tangent over-values BOTH
    /// limbs; the midpoint derivative equals the secant slope exactly for a locally quadratic
    /// V. Default 0.
    #[serde(default)]
...
impl Hyperparameters {
    ...
    fn parse(raw: &Option<Map<String, Value>>) -> Result<Self> {
        ...
        // LP oracle: clamp the mode selectors so an out-of-range hp_json cannot silently
        // select an unimplemented branch. 0 is the bit-exact sentinel in both cases.
...
fn policy(
    ...
    // LP dispatch oracle (port of t53 CASSURE CF=15). Competes with the PGA result on
    // `total_step_value` -- the SAME metric `joint_optimize_step` uses -- BEFORE the polish
    // chain, so whichever wins still gets coordinate/pair/triplet/basin-hop polish.
    // Feasibility: projected, then gated on the framework's own all-lines flow check.
    // use_lp_dispatch = false => the branch is dead code => bit-exact with the baseline.
...
// ---- LP dispatch oracle (ported from t53_engine.rs, CASSURE CF=15) ----

/// Exact LP dispatch for one timestep: maximise the linearised value function subject to the
/// per-battery action box and the PTDF flow polytope.
///
/// Variables are split into discharge `d >= 0` and charge magnitude `c >= 0` with
/// `action = d - c`, which makes every RHS non-negative so the slack basis is primal feasible
/// and NO Phase-1 is needed.
///
/// Objective (per battery, matching `dp_action_value` / `total_step_value` to first order):
///   discharge: (price - kappa_tx)*dt - dv*(dt/eta_d)
///   charge:    dv*eta_c*dt - (price + kappa_tx)*dt
/// The kappa_deg quadratic is deliberately omitted (it is O(1e-3) $ against O(1e2) $ of
/// revenue at nominal parameters), exactly as in t53. The LP is scored afterwards WITH the
/// penalty by `total_step_value`, so this is a modelling bias, never a validity bug.
///
/// Deterministic: no RNG, no clock, no threads, no global state, fixed iteration order.
fn lp_dispatch_step(
    challenge: &Challenge,
    state: &State,
    dps: &[BatteryDP],
    ...
    // Select the lines to include in the LP.
    let line_indices: Vec<usize> = if hp.lp_screen_mode == 1 {
        // CERTIFIED box-dominance screening. swing_l is the maximum |delta flow| that ANY
        // action in the box can induce on line l; if |base| + swing <= limit the line cannot
        // be violated by any feasible LP answer, so its two rows are redundant for every
        // solution. Dominance proof, not a ranking heuristic. O(L*B) per step.
        (0..n_lines_total)
            .filter(|&l| {
                let row = &sens[l];
        ...
        // Legacy STATIC selector (kept for exact parity with t53). Known-weak: the ranking
        // has zero dependence on `sens`, so it cannot identify the lines that actually bind.
    ...
    // LP: maximise c^T x, s.t. A x <= b, x >= 0.
    // x[0..num_b] = discharge d_b, x[num_b..2*num_b] = charge magnitude c_b.
    ...
    for b in 0..num_b {
        ...
        // Marginal value of stored energy. Mode 0 = tangent at the current SOC (t53).
        // Mode 1 = per-limb midpoint tangent: V is concave, so the current-SOC tangent
        // over-values BOTH limbs; the midpoint derivative is the exact secant slope for a
        // locally quadratic V.
        ...
        if !hp.lp_drop_soc_rows {
            // d_b * dt / eta_d <= soc - soc_min   (redundant: implied by u_max)
            a_mat[r + 2][b] = DELTA_T / ETA_DISCHARGE;
            b_vec[r + 2] = (soc - battery.soc_min_mwh).max(0.0);
            // c_b * eta_c * dt <= soc_max - soc   (redundant: implied by u_min)
    ...
    for (li, &l) in line_indices.iter().enumerate() {
        ...
        for b in 0..num_b {
            let ptdf = sens[l][b];
            // Positive direction:  sum ptdf*(d - c) <=  limit - exo
            a_mat[rp][b] += ptdf;
            a_mat[rp][num_b + b] -= ptdf;
            // Negative direction: -sum ptdf*(d - c) <=  limit + exo
            a_mat[rn][b] -= ptdf;
    ...
    // Non-finite guard. An overflowed dense tableau can emit +inf, and f64::clamp maps +inf to
    // `hi`, i.e. a garbage answer would masquerade as a legitimate all-bounds dispatch. NaN is
    // already rejected downstream (NaN > pga_val is false) but only by accident. Skip instead.
...
/// Minimal dense simplex embedded for the LP dispatch oracle.
/// Solves: maximise c^T x, s.t. A x <= b, x >= 0.  Assumes b_i >= 0 (guaranteed by the
/// discharge/charge split above), so the slack basis is primal feasible and no Phase-1 is
/// needed.
///
/// Entering rule is Bland's (first index with negative reduced cost). The leaving rule is
/// min-ratio with `min_by`, which returns the FIRST minimising row on ties -- deterministic,
/// but NOT Bland's leaving rule, so anti-cycling is not guaranteed; termination is enforced by
/// `max_pivots`. On budget exhaustion the current basis is returned: primal feasible but
/// possibly non-optimal. That is safe here because the caller projects and then gates on the
/// framework's own flow check, and only accepts on a strict `total_step_value` improvement.
///
/// `LP_EPS` below is module-scoped and deliberately distinct from the crate-level `LP_EPS`
/// used by `solve_battery_kkt`; they happen to share a value but are separate items.
mod lp_solver {
    const LP_EPS: f64 = 1e-9;
