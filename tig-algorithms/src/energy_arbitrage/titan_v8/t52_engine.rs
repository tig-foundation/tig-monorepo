...
pub struct Hyperparameters {
    ...
    /// PORT of t53 CASSURE CF=15 (capstone: +44,220 tuned / +47,776 held-out, 183W/16L,
    /// p=4.2e-37). Solve the per-timestep dispatch EXACTLY as a linear program over the DP
    /// value-function tangent (dv/dsoc) and the full PTDF polytope, and take it instead of the
    /// projected-gradient answer whenever it is feasible AND strictly better on
    /// `total_step_value`. PGA is a first-order local method on a non-smooth objective over 400
    /// halfspaces; the LP is a different solver CLASS and reaches points PGA cannot.
    /// false = iso-baseline (PGA only), BIT-EXACT sentinel.
    #[serde(default)]
    pub use_lp_dispatch: bool,

    /// Number of lines to include in the LP, ranked by |base_flow_l|/limit_l. 0 = ALL lines.
    /// KEEP 0. The ranking is purely static exogenous loading with zero dependence on `sens`,
    /// so a truncated set both omits lines the fleet can saturate and wastes rows on lines it
    /// cannot influence; on capstone lp_max_lines=80 bought +47 vs +44,220 for 0.
    #[serde(default)]
    pub lp_max_lines: usize,

    /// Max simplex pivots per LP call. 0 = built-in default (2000). Dense has m = 4*60 + 2*200 =
    /// 640 rows, so the textbook 2m..3m = 1280..1920 pivot range fits inside 2000; raise this only
    /// if instrumentation shows the cap is being hit (cap exhaustion silently returns a
    /// primal-feasible but NON-optimal vertex).
    #[serde(default)]
...
fn policy(
    ...
    // ---- LP dispatch oracle (port of t53 CASSURE CF=15) --------------------------------------
    // Exact LP over the DP tangent + the FULL 200-line PTDF polytope, competing with the PGA
    // answer. Accepted only if it survives projection, the framework feasibility check, and
    // strictly beats PGA on `total_step_value` (the same metric joint_optimize_step maximises),
    // so acceptance can never lower the step value and can never emit an infeasible flow.
    // `pga_val` is computed INSIDE the branch so the disabled path costs nothing and is bit-exact.
    // `None` for gram mirrors t53: exact per-sweep flow recomputation, no incremental drift.
...
// ---- LP dispatch (port of t53 CASSURE CF=15) ----

/// Exact LP dispatch per-timestep: maximize the linearized value function subject to
...
/// b >= 0 holds and no Phase-1 is required.
///
/// lp_max_lines: if > 0, only include the K most-loaded lines (by |base_flow|/limit).
/// KEEP IT 0 (= all lines): the ranking ignores `sens` entirely, so truncation is worthless.
fn lp_dispatch_step(
    challenge: &Challenge,
    state: &State,
    dps: &[BatteryDP],
    ...
    for (li, &l) in line_indices.iter().enumerate() {
        ...
        // The generator rescales exogenous flows to <= 0.85 * limit, so both RHS are already
        // strictly positive and `.max(0.0)` is a no-op; it is kept as a hard guard because a
        // negative RHS would silently break the simplex's b >= 0 assumption.
    ...
    // Non-finite guard (LPAUDIT F1). Behaviour-preserving: a non-finite component makes
    // total_step_value NaN and `NaN > pga_val` is already false, so this can only ever reject an
    // action that was going to be rejected anyway -- but it removes the dependence on IEEE NaN
    // comparison semantics propagating correctly through verify_flows and the acceptance test.
...
/// Minimal dense simplex LP solver. Solves: maximize c^T x, s.t. A x <= b, x >= 0.
/// Assumes b_i >= 0 (guaranteed by the d/c split above), so the slack basis is primal feasible.
/// Entering rule = first index with negative reduced cost; leaving rule = min ratio, first row on
/// ties. Fully deterministic: fixed loop bounds, no RNG, no clock, no parallelism.
mod lp_solver {
