// Generated single-file submission from codex_ea_hybrid_v1.
// Source components: titan_v2 for small tracks, near_arb_v1 for large tracks.
    ...
    STATE.with(|s| -> Result<Vec<f64>> {
        ...
        for b in 0..num_b {
            ...
            for &u_try in &[lo, hi, 0.5 * lo, 0.5 * hi] {
                ...
                if v > best_v { best_v = v; best_u = u_try; }
            }
            actions_b[b] = best_u;
        }
        ...
        let val_a: f64 = (0..num_b).map(|b| eval_profit(challenge, state, cache, b, actions_a[b])).sum();
        let val_b: f64 = (0..num_b).map(|b| eval_profit(challenge, state, cache, b, actions_b[b])).sum();
        let val_c: f64 = (0..num_b).map(|b| eval_profit(challenge, state, cache, b, actions_c[b])).sum();
...
fn joint_pair_refine(
    ...
    for l in 0..num_l {
        ...
        for i in 0..sorted.len() {
            if tried >= max_pairs_per_line { break; }
            for j in (i + 1)..sorted.len() {
                ...
                let v0 = pair_val(0.0);
                let (delta_best, v_best) = ternary_search(pair_val, d_lo, d_hi, hp.ternary_iters.max(8));

                if v_best <= v0 + 1e-9 { continue; }
...
pub fn solve_challenge(
    ...
    match challenge.num_batteries {
        ...
        n => Err(anyhow!(
            "near_arb_v1: unsupported num_batteries={} (expected one of 10/20/40/60/100)", n
        )),
    }
}
...
mod helpers {
        ...
        for b in 0..num_b {
            ...
            for t in (0..num_t).rev() {
                ...
                for i in 0..soc_levels {
                    ...
                    let v_next_slice = &dp[b][t + 1];

                    let max_val = if hp.use_sdp {
                        // K=5 Gauss-Hermite SDP: 5 RT scenarios [z_k, w_k]
                        let mut val_sum = 0.0_f64;
                        for k in 0..5 {
                            let p = (p_da * (1.0 + sdp_sigma_eff * GH5_Z[k]) + extra).max(1e-6);
        ...
        for _sweep in 0..hp.asca_iters {
            ...
            for &b in &order {
                ...
                if u_min < 0.0 {
                    let lo = u_min; let hi = 0.0_f64.min(u_max);
                    if lo < hi {
                        let b_coeff = 0.25_f64 * (-rt_price - 0.25 + lambda * bat.efficiency_charge);
                        let cand = if deg_coeff > 1e-30 {
                            let x = b_coeff / (2.0 * deg_coeff);
                ...
                if u_max > 0.0 {
                    ...
                    if lo < hi {
                        let eff_d = bat.efficiency_discharge.max(1e-9);
                        let b_coeff = 0.25_f64 * (rt_price - 0.25 - lambda / eff_d);
                        let cand = if deg_coeff > 1e-30 {
                            let x = b_coeff / (2.0 * deg_coeff);
                            x.clamp(lo, hi)
                        } else { hi };
    ...
    fn run_column_generation(
        ...
        for _iter in 0..hp.cg_iters {
            let master = build_master_lp(challenge, state, ca, &columns, flows_base, hp.flow_margin);
            ...
            for b in 0..num_b {
                let (u_min, u_max) = state.action_bounds[b];
                let (u_star, obj_star) = golden_section_subproblem(
...
mod track_baseline {
    ...
    fn defaults() -> TrackHp {
        TrackHp {
            ...
            cg_iters: 10,           // max master-pricing iterations
        }
    }
...
mod helpers {
    ...
    fn lns_dw_per_step(
        ...
        for _cg_iter in 0..hp.lns_cg_iters {
            if pivot_budget < 50 { break; } // Not enough pivots for a meaningful solve
            ...
            if !added_any { break; } // No new columns, converged
        }
...
pub fn solve_challenge(
    ...
    match challenge.num_batteries {
        n if n <= 30 => titan_v2::solve_challenge(challenge, save_solution, hyperparameters),
        n if n <= 150 => near_arb_v1::solve_challenge(challenge, save_solution, hyperparameters),
        n => Err(anyhow!(
            "codex_ea_hybrid: unsupported num_batteries={} (expected one of 10/20/40/60/100)",
            n
        )),
    }
}
...
pub fn help() {
    println!(
        "codex_ea_hybrid: track-routed c008 policy. Uses titan_v2 on baseline/congested tracks and near_arb_v1 on multiday/dense/capstone tracks. Defaults are tuned for quality at the current c008 fuel budget; pass null hyperparameters for recommended behavior."
    );
}
