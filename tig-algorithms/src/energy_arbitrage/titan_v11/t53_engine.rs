...
pub struct Hyperparameters {
    ...
    /// LP-ONLY replace mode: skip multi-seed PGA, use exact convex LP each step
    /// (fallback to PGA only if LP infeasible/None). Tests LP-only Q/time on t53.
    /// BAKED v11: default=true (helper) car #[serde(default)] simple => bool false (piege serde
    /// per-field, ignore le Default impl). hp={} doit donner LP-only sur t53 (gain +2.28% a fuel egal).
    #[serde(default = "bake_lp_true")]
    ...
    #[serde(default)]
    pub lp_pw_k: usize,
    /// LP intermittent: run exact LP every `lp_stride` steps (PGA between). 0/1=every step.
    #[serde(default)]
    pub lp_stride: usize,
...
impl Default for Hyperparameters {
    fn default() -> Self {
        Self {
            ...
            use_lp_only: true,  // BAKED v11: LP-only per-step (exact) sur t53, +2.28% a fuel egal (inval=0)
            lp_max_lines: 0,
            lp_pivot_budget: 0,
            lp_pw_k: 0,
            lp_stride: 0,
...
// BAKED v11: serde default helper => use_lp_only=true quand hp={} (bake LP-only sur t53).
fn bake_lp_true() -> bool { true }
...
fn build_battery_dp(
    ...
    for t in (0..num_steps).rev() {
        ...
        for s_idx in 0..levels {
            ...
            // i68 : RETEST parabole-action t53 sous le régime NO-POLISH (i42 +184 inerte était mesuré
            // avec l'ancienne chaîne coord+pair, retirée depuis par vid5509/5511). curv INTACT.
            let fut = |a: f64| -> f64 {
                let ns = battery.apply_action_to_soc(a, soc);
                if hp.use_dp_value_shift {
                    interp_value_q(next, ns, soc_lo, soc_step_inv, last, hp.dp_value_curv_coef)
                } else {
                    interp_value(next, ns, soc_lo, soc_step_inv, last)
                }
            };
            let mut ca: Vec<f64> = Vec::with_capacity(actions.len());
            let mut fu: Vec<f64> = Vec::with_capacity(actions.len());
            for &raw in &actions {
                let a = raw.clamp(lo, hi);
                ca.push(a);
                fu.push(fut(a));
            }
            let refine = |price: f64| -> f64 {
                let mut bi = 0usize;
                let mut bv = f64::NEG_INFINITY;
                for k in 0..ca.len() {
                    let v = immediate_profit(battery, ca[k], price) + fu[k];
                    if v > bv {
                        bv = v;
                        bi = k;
                    }
                ...
                if bi > 0 && bi + 1 < ca.len() {
                    ...
                    let x1 = ca[bi];
                    let x2 = ca[bi + 1];
                    let y0 = immediate_profit(battery, x0, price) + fu[bi - 1];
                    let y1 = bv;
                    let y2 = immediate_profit(battery, x2, price) + fu[bi + 1];
                    let d10 = x1 - x0;
                    let d12 = x1 - x2;
                    let num = d10 * d10 * (y1 - y2) - d12 * d12 * (y1 - y0);
                    let den = d10 * (y1 - y2) - d12 * (y1 - y0);
                    if den.abs() > 1e-12 {
                        let xv = x1 - 0.5 * num / den;
                        if xv > x0.min(x2) && xv < x0.max(x2) {
                            let xv = xv.clamp(lo, hi);
                            let vv = immediate_profit(battery, xv, price) + fut(xv);
                            if vv > bv {
                                bv = vv;
                            }
                        }
            ...
            current[s_idx] = w_low * refine(price_low)
                + w_high * refine(price_high)
                + w_jump_low * refine(price_jump_low)
                + w_jump_high * refine(price_jump_high);
...
fn dv_dsoc(dp: &BatteryDP, t: usize, soc: f64) -> f64 {
    ...
    // ITER t53/i13 : 4-step forward [low,low+4]/4 (encore + large, centre low+2) vs i12 3-step.
    if low + 4 <= last {
        (values[low + 4] - values[low]) * 0.25 * dp.soc_step_inv
    } else if low + 3 <= last {
        (values[low + 3] - values[low]) / 3.0 * dp.soc_step_inv
    } else if low + 2 <= last {
        (values[low + 2] - values[low]) * 0.5 * dp.soc_step_inv
    } else {
...
fn policy(
    ...
    let mut result = if hp.use_dual_dispatch {
        ...
        let lp_this_step = hp.use_lp_only && (t % hp.lp_stride.max(1) == 0);
        let pga_result = if lp_this_step {
            // Freshness probe (P-V7-7): emit once per process so the bench stdout proves
            // this branch is in the running .so (not a stale artifact).
            {
                use std::sync::atomic::{AtomicBool, Ordering};
                static PROBED: AtomicBool = AtomicBool::new(false);
                if !PROBED.swap(true, Ordering::Relaxed) {
                    eprintln!("MI_PROBE USE_LP_ONLY_BRANCH_ACTIVE v9_t53_lp_only_v2");
                }
            }
            // LP-ONLY (replace): skip the multi-seed PGA, use the exact convex LP
            // dispatch directly. Fall back to PGA only if the LP is None/infeasible
            // (rare; on t51 the LP was 0-invalid across all benches). Mirror of t51.
            match lp_dispatch_step(challenge, state, dps, sens, &base_flows, hp) {
                Some(mut lp_act) => {
    ...
    // i54 (DÉPLOYÉ) : sous LP-only t53, skip coord+pair polish — loi d'échelle mesurée (contribution
    // polish : +8384 @n≤50, +1487 @n≤80, +68≈0 @n≤150) ; coût 17 s/161 s (10,6 %). Mesure appariée
    // vid5508 vs vid5434 : Q 3800968 (+68) @144 s vs 3800900 @161 s = Pareto-amélioration (Q ET temps).
    // t51/t52 GARDENT leur polish (contribution mesurée +8384/+1487).
    if !hp.use_lp_only {
    ...
    if hp.use_joint_pair_polish && !hp.use_lp_only {
    ...
    // i55 (DÉPLOYÉ) : sous LP-only, basin_hop = ZÉRO mesuré (vid5510 vs vid5509 : Q BIT-IDENTIQUE
    // 3800968, −5 s/−3,3 %) — ses cycles perturbation+re-polish n'acceptent jamais d'amélioration
    // sur l'optimum LP. Skip = Pareto sans risque (sortie identique, temps en moins).
    if hp.use_basin_hop && !hp.use_lp_only {
...
fn lp_dispatch_step(
    ...
    let t = state.time_step;
    let limits = &challenge.network.flow_limits;
    // PIECEWISE-LINEAR V (lambda-method, K tranches/direction/batterie). Chaque tranche = un segment SOC
    // avec sa pente dv (valeur marginale au milieu du segment) et sa capacite energie. Le LP voit la COURBE
    // de V (concave: dv monte quand soc baisse en decharge) => dispatch plus juste que la pente unique.
    // Layout colonne: col = (dir*K + k)*num_b + b ; dir 0=decharge(+1), 1=charge(-1).
    let k_tr = if hp.lp_pw_k > 0 { hp.lp_pw_k } else { 2 };
    let n = 2 * k_tr * num_b;
    let mut c_obj = vec![0.0_f64; n];
    let mut ub_col = vec![0.0_f64; n];
    let mut col_bat = vec![0usize; n];
    let mut col_sign = vec![0.0_f64; n];
    let mut col_dir = vec![0usize; n];
    let mut rate_bound = vec![0.0_f64; 2 * num_b];
    for b in 0..num_b {
        let battery = &challenge.batteries[b];
        let price = state.rt_prices[battery.node];
        let soc = state.socs[b];
        ...
        // FIX: les tranches couvrent la plage SOC ATTEIGNABLE EN UN PAS (bornee par le rate), pas la plage
        // complete [soc_min, soc]. Sinon dv est evalue loin de soc (tranches profondes non atteignables/pas)
        // => sur-penalise le dispatch. dsoc_dis = deplacement SOC max en decharge sur un pas (rate ou energie).
        let dsoc_dis = (u_max.max(0.0) * DELTA_T / ETA_DISCHARGE).min((soc - smin).max(0.0));
        let dsoc_chg = ((-u_min).max(0.0) * ETA_CHARGE * DELTA_T).min((smax - soc).max(0.0));
        let dseg = dsoc_dis / k_tr as f64;
        let cseg = dsoc_chg / k_tr as f64;
        rate_bound[b * 2] = u_max.max(0.0);
        rate_bound[b * 2 + 1] = (-u_min).max(0.0);
        for k in 0..k_tr {
            let cd = k * num_b + b; // dir 0 (discharge)
            let mid = soc - (k as f64 + 0.5) * dseg;
            let dv = dv_dsoc(&dps[b], t, mid);
            c_obj[cd] = (price - KAPPA_TX) * DELTA_T - dv * (DELTA_T / ETA_DISCHARGE);
            ub_col[cd] = dseg * ETA_DISCHARGE / DELTA_T;
            col_bat[cd] = b; col_sign[cd] = 1.0; col_dir[cd] = 0;
            let cc = (k_tr + k) * num_b + b; // dir 1 (charge)
            let midc = soc + (k as f64 + 0.5) * cseg;
            let dvc = dv_dsoc(&dps[b], t, midc);
            c_obj[cc] = dvc * ETA_CHARGE * DELTA_T - (price + KAPPA_TX) * DELTA_T;
            ub_col[cc] = cseg / (ETA_CHARGE * DELTA_T);
            col_bat[cc] = b; col_sign[cc] = -1.0; col_dir[cc] = 1;
        }
    }

    // Active-set init: top-K lines by base load ratio.
    let k0 = if hp.lp_max_lines > 0 { hp.lp_max_lines.min(n_lines_total) } else { 80.min(n_lines_total) };
    let mut scored: Vec<(f64, usize)> = (0..n_lines_total)
        .map(|l| { let lim = limits[l]; (if lim > 1e-6 { base_flows[l].abs() / lim } else { 0.0 }, l) })
        .collect();
    scored.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
    // ITER i76 (2026-08-20) : WARM-START de l'active-set lignes depuis l'ensemble FINAL du pas precedent
    // (injections exogenes de rang 2 => lignes bindantes stables dans le temps). Repli top-k0 au 1er pas.
    // Correctness inchangee : la boucle externe ajoute toute ligne violee et ne sort qu'a 0 violation.
    static PREV_ACTIVE: std::sync::Mutex<Vec<usize>> = std::sync::Mutex::new(Vec::new());
    let mut active: Vec<usize> = {
        let prev = PREV_ACTIVE.lock().map(|g| g.clone()).unwrap_or_default();
        let prev: Vec<usize> = prev.into_iter().filter(|&l| l < n_lines_total).collect();
        if t > 0 && !prev.is_empty() { prev } else { scored.iter().take(k0).map(|&(_, l)| l).collect() }
    };
    let mut in_active = vec![false; n_lines_total];
    for &l in &active { in_active[l] = true; }

    let budget = if hp.lp_pivot_budget > 0 { hp.lp_pivot_budget } else { 4000 };
    let mut final_x: Option<Vec<f64>> = None;

    let mut col_in = vec![false; n];
    for j in 0..n { if c_obj[j] > 1e-12 { col_in[j] = true; } }
    let mut active_cols: Vec<usize> = (0..n).filter(|&j| col_in[j]).collect();

    for _cgen in 0..8 {
        let cg_last = _cgen == 7;
        if cg_last { for j in 0..n { col_in[j] = true; } active_cols = (0..n).collect(); }
        let nc = active_cols.len();
        if nc == 0 { return Some(vec![0.0_f64; num_b]); }
        let c_obj_r: Vec<f64> = active_cols.iter().map(|&j| c_obj[j]).collect();
        let mut duals: Vec<f64> = Vec::new();
        let mut solve_lines: Vec<usize> = Vec::new();

        for _iter in 0..10 {
            let num_l = active.len();
            // rows: nc energy caps + 2*num_b rate-coupling rows + 2*num_l lines
            // ITER i93 : simplexe a VARIABLES BORNEES — les caps de colonnes deviennent des bornes superieures
            // (flips), plus de rangees identite. m passe de nc+2num_b+2num_l a 2num_b+2num_l.
            let rate_f = 0;
            let line_f = 2 * num_b;
            let m = line_f + 2 * num_l;
            let mut a_mat = vec![vec![0.0_f64; nc]; m];
            let mut b_vec = vec![0.0_f64; m];
            let ub_r: Vec<f64> = active_cols.iter().map(|&j| ub_col[j]).collect();
            for k in 0..nc {
                let j = active_cols[k];
                a_mat[rate_f + col_bat[j] * 2 + col_dir[j]][k] += 1.0;
            }
            for bd in 0..2 * num_b { b_vec[rate_f + bd] = rate_bound[bd]; }
            for (li, &l) in active.iter().enumerate() {
                let rp = line_f + 2 * li;
                let rn = rp + 1;
                for k in 0..nc {
                    let j = active_cols[k];
                    let ptdf = sens[l][col_bat[j]] * col_sign[j];
                    a_mat[rp][k] += ptdf; a_mat[rn][k] -= ptdf;
                }
                b_vec[rp] = (limits[l] - base_flows[l]).max(0.0);
                b_vec[rn] = (limits[l] + base_flows[l]).max(0.0);
            }
            let (opt_x_r, _, dvec) = lp_solver::lp_solve_duals_bounded(nc, m, &c_obj_r, &ub_r, &a_mat, &b_vec, budget);
            let opt_x_r = opt_x_r?;
            let mut opt_x = vec![0.0_f64; n];
            for k in 0..nc { opt_x[active_cols[k]] = opt_x_r[k]; }
            final_x = Some(opt_x.clone());
            duals = dvec;
            solve_lines = active.clone();
            // net dispatch per battery
            let mut net_b = vec![0.0_f64; num_b];
            for j in 0..n { net_b[col_bat[j]] += col_sign[j] * opt_x[j]; }
            let mut any_violated = false;
            let last = _iter == 9;
            let mut next_active: Vec<usize> = Vec::new();
            let mut next_in = vec![false; n_lines_total];
            for l in 0..n_lines_total {
                let mut net = 0.0;
                for b in 0..num_b { net += sens[l][b] * net_b[b]; }
                let flow = (base_flows[l] + net).abs();
                let lim = limits[l];
                let violated = flow > lim + 1e-6 * lim.max(1.0);
                let binding = flow >= lim - 1e-4 * lim.max(1.0);
                if violated { any_violated = true; }
                if violated || (in_active[l] && (binding || last)) { next_active.push(l); next_in[l] = true; }
            }
            active = next_active; in_active = next_in;
            if !any_violated { break; }
        }

        if cg_last { break; }
        // reduced cost: rc_j = c_obj[j] - y[rate_row(b,dir)] - sum_lines y*(±ptdf)
        let line_f = 2 * num_b;
        let mut added = false;
        for j in 0..n {
            if col_in[j] { continue; }
            let b = col_bat[j];
            let sgn = col_sign[j];
            let mut ya = duals.get(col_bat[j] * 2 + col_dir[j]).copied().unwrap_or(0.0);
            for (li, &l) in solve_lines.iter().enumerate() {
                let ip = line_f + 2 * li;
                let in_ = ip + 1;
                if in_ >= duals.len() { break; }
                let ptdf = sens[l][b] * sgn;
                ya += duals[ip] * ptdf - duals[in_] * ptdf;
            }
            let rc = c_obj[j] - ya;
            if rc > 1e-7 { col_in[j] = true; added = true; }
        }
        if added { active_cols = (0..n).filter(|&j| col_in[j]).collect(); } else { break; }
    }

    let opt_x = final_x?;
    if let Ok(mut g) = PREV_ACTIVE.lock() { *g = active.clone(); } // i76 : memorise l'ensemble final
    let mut actions = vec![0.0_f64; num_b];
    for j in 0..n { actions[col_bat[j]] += col_sign[j] * opt_x[j]; }
    for b in 0..num_b {
        let (lo, hi) = state.action_bounds[b];
...
mod lp_solver {
    ...
    /// Same primal simplex as lp_solve_with_budget, but also returns the DUALS
    /// (dual_i = tab[m][n+i] at optimum). Used by col-gen for reduced costs.
    /// ITER i80 (2026-08-20) : meme algorithme (Dantzig, memes tie-breaks, memes operations flottantes dans le meme
    /// ordre) mais tableau PLAT et saut des colonnes ou la ligne pivot vaut exactement 0.0 (x - f*0.0 == x en IEEE,
    /// hors NaN) => resultat BIT-IDENTIQUE, cout par pivot ~ nnz(ligne pivot) au lieu de n_cols.
    /// ITER i93 : simplexe primal a variables bornees 0 <= x <= u (Dantzig), A x <= b (b >= 0), max c^T x.
    /// Tableau plat sur les colonnes structurelles + slacks ; non-basiques a la borne inf (flag=0) ou sup (flag=1).
    /// Convention : on travaille sur x' = x - lb avec flips ; la colonne non-basique a la borne sup est stockee
    /// "substituee" (x = u - x'') via inversion de signe de la colonne (technique standard de flip).
    pub fn lp_solve_duals_bounded(
        n: usize, m: usize, c: &[f64], u: &[f64], a: &[Vec<f64>], b: &[f64], max_pivots: usize,
    ) -> (Option<Vec<f64>>, usize, Vec<f64>) {
        if b.iter().any(|&x| x < -1e-6) { return (None, 0, Vec::new()); }
        let n_vars = n + m;
        let rhs_col = n_vars;
        let n_cols = n_vars + 1;
        ...
        // at_upper[j] : la variable non-basique j est a sa borne sup (substituee x_j = u_j - x'_j => colonne negee)
        let mut at_upper = vec![false; n_vars];
        let ubv: Vec<f64> = (0..n_vars).map(|j| if j < n { u[j].max(0.0) } else { f64::INFINITY }).collect();
        let mut basis: Vec<usize> = (n..n + m).collect();
        let mut is_basic = vec![false; n_vars];
        for i in 0..m { is_basic[n + i] = true; }
        ...
        for pivot in 0..max_pivots {
            pivots_used = pivot + 1;
            // entrant : plus negatif cout reduit parmi les non-basiques (colonne deja negee si at_upper)
            let entering = {
                let mut bj: Option<usize> = None;
                let mut bv = -LP_EPS;
                let obj = &tab[m * n_cols..m * n_cols + n_vars];
                for j in 0..n_vars { if is_basic[j] { continue; } let v = obj[j]; if v < bv { bv = v; bj = Some(j); } }
                match bj { Some(j) => j, None => break }
            };
            // test de ratio : (a) rangee i avec a_ie > 0 : rhs_i / a_ie ; (b) rangee i avec a_ie < 0 et variable basique
            // bornee : (ub_basic - rhs_i) / (-a_ie) ; (c) flip de l entrant lui-meme : ub_e.
            let ub_e = ubv[entering];
            let mut best_t = ub_e;
            let mut best_row: Option<usize> = None; // None => flip
            let mut best_kind = 0u8; // 1 = (a), 2 = (b)
            for i in 0..m {
                let aie = tab[i * n_cols + entering];
                let rhs = tab[i * n_cols + rhs_col];
                if aie > LP_EPS {
                    let t = rhs / aie;
                    if t < best_t - 1e-12 { best_t = t; best_row = Some(i); best_kind = 1; }
                } else if aie < -LP_EPS {
                    let ubb = ubv[basis[i]];
                    if ubb.is_finite() {
                        let t = (ubb - rhs) / (-aie);
                        if t < best_t - 1e-12 { best_t = t; best_row = Some(i); best_kind = 2; }
                    }
                }
            }
            if best_t.is_infinite() && best_row.is_none() { return (None, 0, Vec::new()); }
            match best_row {
                None => {
                    // flip de l entrant : x_e passe a l autre borne ; rhs_i -= a_ie * ub_e ; colonne negee
                    if !ub_e.is_finite() { return (None, 0, Vec::new()); }
                    for i in 0..=m {
                        let base = i * n_cols;
                        let aie = tab[base + entering];
                        if i < m { tab[base + rhs_col] -= aie * ub_e; }
                        tab[base + entering] = -aie;
                    }
                    at_upper[entering] = !at_upper[entering];
                }
                Some(lr) => {
                    if best_kind == 2 {
                        // la variable basique sort a sa borne SUP : on la substitue (x = u - x') avant le pivot
                        let bj = basis[lr];
                        let ubb = ubv[bj];
                        for i in 0..=m {
                            let base = i * n_cols;
                            let aij = tab[base + bj];
                            if i < m { tab[base + rhs_col] -= aij * ubb; }
                            tab[base + bj] = -aij;
                        }
                        at_upper[bj] = !at_upper[bj];
                    }
                    let pivot_val = tab[lr * n_cols + entering];
                    if pivot_val.abs() < LP_EPS { return (None, 0, Vec::new()); }
                    {
                        let row = &mut tab[lr * n_cols..(lr + 1) * n_cols];
                    ...
                    is_basic[basis[lr]] = false;
                    basis[lr] = entering;
                    is_basic[entering] = true;
                }
            }
        }
        // duals des rangees : cout reduit des slacks (colonne negee si le slack est "at_upper" — impossible, ub=inf)
        let duals: Vec<f64> = (0..m).map(|i| tab[m * n_cols + n + i]).collect();
        let mut x = vec![0.0_f64; n];
        for j in 0..n { if !is_basic[j] && at_upper[j] { x[j] = ubv[j]; } }
        for i in 0..m {
            let bj = basis[i];
            if bj < n {
                let v = tab[i * n_cols + rhs_col];
                x[bj] = if at_upper[bj] { ubv[bj] - v } else { v };
            }
        }
        (Some(x), pivots_used, duals)
    }

    pub fn lp_solve_duals(
        ...
        for pivot in 0..max_pivots {
            ...
            let entering = {
                ...
                let mut bv = -LP_EPS;
                let obj = &tab[m * n_cols..m * n_cols + n_vars];
                for j in 0..n_vars { let v = obj[j]; if v < bv { bv = v; bj = Some(j); } }
                match bj { Some(j) => j, None => break }
            };
            let leaving_row = (0..m)
            ...
            let leaving_row = match leaving_row { Some(r) => r, None => return (None, 0, Vec::new()) };
        ...
        let duals: Vec<f64> = (0..m).map(|i| tab[m * n_cols + n + i]).collect();
        let mut x = vec![0.0_f64; n];
        for i in 0..m { if basis[i] < n { x[basis[i]] = tab[i * n_cols + rhs_col]; } }
        (Some(x), pivots_used, duals)
    }
    ...
    pub fn lp_solve_with_budget(
...

        {
            ...
            static SCA: AtomicU64 = AtomicU64::new(0);
            static SM: AtomicU64 = AtomicU64::new(0);
            static SB: AtomicU64 = AtomicU64::new(0);
            let cact = basis.iter().filter(|&&bv| bv < n).count();
            let slack_in_basis = basis.iter().filter(|&&bv| bv >= n).count();
            let bind = m.saturating_sub(slack_in_basis);
            let c = SOLVES.fetch_add(1, Ordering::Relaxed) + 1;
            SN.fetch_add(n as u64, Ordering::Relaxed);
            SCA.fetch_add(cact as u64, Ordering::Relaxed);
            SM.fetch_add(m as u64, Ordering::Relaxed);
            SB.fetch_add(bind as u64, Ordering::Relaxed);
            if c % 100 == 0 {
                let cf = c as f64;
                println!("MI_PROBE LP_ACTIVESET solves={} cols_posed={:.1} cols_active={:.1} rows_posed={:.1} rows_binding={:.1}",
                    c, SN.load(Ordering::Relaxed) as f64/cf, SCA.load(Ordering::Relaxed) as f64/cf,
                    SM.load(Ordering::Relaxed) as f64/cf, SB.load(Ordering::Relaxed) as f64/cf);
            }
        }
