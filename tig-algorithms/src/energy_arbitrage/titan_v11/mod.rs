...
#[used]
#[no_mangle]
pub static TITANV7_BUILD_TAG: [u8; 40] = *b"TITANV7_BUILD=v35_t52_piecewise_test____";

// P-V7-7 SIZE-DISCRIMINATOR : un bake numérique-seul (150->80, 5500->80) NE change ni la
// taille ni le nombre de lignes du .so ⇒ le cache (taille,mtime) sert le .so STALE de l'iter
// précédente (vu : v4880 = même 5 733 072 B que v4875, hp={} rendait le vieux champion). Ce
// static &str force une TAILLE .so distincte à chaque bake pour défaire ce cache. Bumper le
// texte (donc la longueur) à chaque bake numérique-seul.
#[used]
#[no_mangle]
pub static TITANV7_SIZE_DISCRIMINATOR: &str =
    "c008-t52-piecewise-port-test-2026aug14";
...
pub fn solve_challenge(
    ...
    match challenge.num_batteries {
        n if n <= 15 => {
            // Par defaut ce segment (n<=15) utilise le moteur LP-portfolio (t49_ext).
            // Le moteur gradient historique reste disponible via HP t49_alt_solver=true.
            if hyperparameters.as_ref().and_then(|m| m.get("t49_alt_solver"))
                .and_then(|v| v.as_bool()).unwrap_or(false)
            {
                let hp = merge_hp(hyperparameters, vec![
                    ...
                    ("use_cg_lp_combine", b(true)),
                ]);
                return track_t49::solve_challenge(challenge, save_solution, &hp);
            }
            let hp = merge_hp(hyperparameters, vec![
                ("asca_iters", self::n(15)),
                ("soc_levels", self::n(44)),
                ("action_grid", self::n(40)),
                ("prune_ratio", f(0.5)),
                ("jump_premium", f(0.02)),
                ("lmp_threshold", f(0.85)),
                ("deflator_iters", self::n(10)),
                ("convergence_tol", f(0.001)),
                ("network_derating", f(1.0)),
                ("lmp_premium_scale", f(0.8)),
            ]);
            t49_ext::solve_challenge(challenge, save_solution, &hp)
        }
        n if n <= 30 => {
            let hp = merge_hp(hyperparameters, vec![
                ("ct_ref_kappa", f(0.2)),
        ...
        n if n <= 50 => {
            let hp = merge_hp(hyperparameters, vec![
                ...
                // titan_v7/t51/i6 BAKE, RE-LIVRÉ ici par t51/i12 — fast-frontier Q-max
                // point (i5/4092, job 25325) : 2,566,624 @ 4,500 ms, 32/32 0-invalid,
                // reproduit bit-exact 3×. La consolidation v8/v9 a hérité l'arm t51
                // PRÉ-BAKE du mod.rs de t53/i12 : à hp={} la prod retombait sur le coin Q
                ...
                // i12 : 0 occurrence) — poser la clé seule ici aurait été un NO-OP
                // SILENCIEUX. i12 porte donc AUSSI le moteur (t51_engine.rs de i6, qui est
                ...
                // = 2,549,922 @ 3,500 ms.
                ("seed_sel_mode", self::n(2)),   // i6 BAKE (défaut moteur = 0)
                ("use_lp_only", b(true)),        // champ_v2 — LP-only t51 (i20 : 2 678 231 @6,5s, domine)
                ("dp_soc_levels", self::n(81)),  // i6 BAKE (était 97)
                ...
                ("coord_polish_passes", self::n(0)),  // i6 BAKE (était 2)
                ...
                ("use_joint_triplet_polish", b(false)), // i6 BAKE (était true; sans valeur, i4)
                ("use_arb_seed", b(true)),
                ("arb_pct", self::n(75)),
                ("arb_inverse", b(true)),
                ...
                ("use_basin_hop", b(false)),  // i6 BAKE (était true; -6,549 Q empilé sur pair)
        ...
        n if n <= 80 => {
            let hp = merge_hp(hyperparameters, vec![
                ...
                ("policy_action_levels", self::n(65)),
                ("proj_max_iters", self::n(80)),
                // NO-OP MESURÉ (30788, v4891): grad75 baked = 2 424 201 @27s = IDENTIQUE à grad110
                // ⇒ grad_outer_iters SATURÉ ici (PGA converge avant 75 iters). Le « +23% débit / base 22s »
                // espéré venait d'un AUTRE build (v4758), non reproductible par grad seul sur ce candidat.
                // Gardé à 75 (≡110, inoffensif). Vrai levier débit t52 = drop-arb seeds (bar-dépendant).
                ("grad_outer_iters", self::n(110)),  // champ_v3 t52 grad110
                ("use_lp_only", b(true)),  // t52-LP port test = 2424201 bat concurrent 2422214
                ...
                ("ct_step_eta", f(0.70)),  // LP-optimal (was 0.50 PGA)
                ("use_dp_seed", b(false)),
                ("use_ptdf_ct", b(true)),
                ("ct_ref_kappa", f(0.0)),
                ...
                ("ct_vq_v", f(200.0)),  // LP-optimal (was 100 PGA)
            ]);
        ...
        n if n <= 150 => {
            let hp = merge_hp(hyperparameters, vec![
                ...
                // = 3 670 420 @43,5s vs champion 3 675 174 @65,5s = -0,13% Q pour +51% débit,
                // vérifié apparié hp_json (30716). Polish 5500 sur-provisionné (80 capture 99,87%).
                ("grad_outer_iters", self::n(80)),
                ("grad_ls_iters", self::n(12)),
                ("bisect_iters", self::n(30)),
                ("coord_polish_passes", self::n(1)),
