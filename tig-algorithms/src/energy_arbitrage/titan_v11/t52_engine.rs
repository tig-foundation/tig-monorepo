...
pub struct Hyperparameters {
    #[serde(default)] pub use_lp_only: bool,
    #[serde(default)] pub lp_max_lines: usize,
    #[serde(default)] pub lp_pivot_budget: usize,
...
impl Default for Hyperparameters {
    fn default() -> Self {
        Self {
            use_lp_only: false, lp_max_lines: 0, lp_pivot_budget: 0,
            dp_soc_levels: 33,
            dp_action_levels: 17,
...
fn dv_dsoc(dp: &BatteryDP, t: usize, soc: f64) -> f64 {
    ...
    // ITER t53/i3 (port t52) : difference CENTRALE (derivee lissee sur [low,low+2]).
    if low + 4 <= last {
...
fn policy(
    ...
    let mut result = if hp.use_lp_only {
        match lp_dispatch_step(challenge, state, dps, sens, &base_flows, hp) {
            Some(mut a) => { safe_project_to_feasible(challenge, state, &mut a, sens, &base_flows, hp, None); if is_flow_feasible(challenge, state, &a) { a } else { joint_optimize_step(challenge, state, dps, sens, &base_flows, seeds, hp, delta_cong, cwv_lambda, gram) } }
            None => joint_optimize_step(challenge, state, dps, sens, &base_flows, seeds, hp, delta_cong, cwv_lambda, gram),
        }
    } else {
        joint_optimize_step(challenge, state, dps, sens, &base_flows, seeds, hp, delta_cong, cwv_lambda, gram)
    };
    result = coordinate_polish_step(challenge, state, dps, sens, result, hp);

    // i59 (DÉPLOYÉ) : sous LP-only, le pair-polish NUIT (décomposition mesurée : coord seul +2358 ;
    // pair PAR-DESSUS coord = −871, vid5527 2480524 @22,5 s vs vid5511 2479653 @24,0 s). Ses échanges
    // par paires dégradent le point LP+coord. Skip = Pareto (Q +871 ET −1,5 s). Coord GARDÉ (+2358).
...
// ===== LP ported from t51_engine (t52 port) =====
fn lp_dispatch_step(
    challenge: &Challenge,
    state: &State,
    dps: &[BatteryDP],
    ...
    // ITER i77 (port i76 t53) : WARM-START de l'active-set lignes depuis l'ensemble FINAL du pas precedent.
    ...
    for _cgen in 0..8 {
        ...
        for _iter in 0..10 {
            ...
            // ITER i94 (port i93) : simplexe a variables bornees, plus de rangees identite.
            let rate_f = 0;
            let line_f = 2 * num_b;
    ...
    let opt_x = final_x?;
    if let Ok(mut g) = PREV_ACTIVE.lock() { *g = active.clone(); } // i77
...
mod lp_solver {
    ...
    pub fn lp_solve_with_budget(
        ...
        for pivot in 0..max_pivots {
            ...
            for j in 0..n_cols {
                tab[(leaving_row) * n_cols + (j)] /= pivot_val;
            }
            for i in 0..=m {
                if i != leaving_row {
                    let factor = tab[(i) * n_cols + (entering)];
                    if factor.abs() > 1e-15 {
                        for j in 0..n_cols {
                            tab[(i) * n_cols + (j)] -= factor * tab[(leaving_row) * n_cols + (j)];
                        }
                    }
                }
            }
