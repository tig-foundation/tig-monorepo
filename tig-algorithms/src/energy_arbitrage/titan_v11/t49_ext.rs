// Moteur T49 alternatif (LP-portfolio), auto-contenu. Active uniquement via
// l'hyperparametre `t49_alt_solver=true` sur le segment n<=15.
...
use super::*;
    ...
    fn policy_entry(challenge: &Challenge, state: &State) -> Result<Vec<f64>> {
        STATE.with(|s| -> Result<Vec<f64>> {
            ...
            let inner = guard.as_mut().expect("alt_t49: STATE not initialised");
    ...
    fn build_cache(challenge: &Challenge, state: &State, hp: &TrackHp) -> AltT49Cache {
        ...
        AltT49Cache { dp, ptdf_sparse, b_to_lines, batt_nodes, battery_derating }
    }
    ...
    #[inline]
    fn eval_profit(challenge: &Challenge, state: &State, ca: &AltT49Cache, b: usize, u: f64) -> f64 {
        let bat = &challenge.batteries[b];
    ...
    #[inline]
    fn potential(challenge: &Challenge, state: &State, ca: &AltT49Cache, b: usize) -> f64 {
...
pub fn solve_challenge(
    ...
    match challenge.num_batteries {
        ...
        n => Err(anyhow!(
            "alt_t49: unsupported num_batteries={} (expected one of 10/20/40/60/100)", n
        )),
    }
}
