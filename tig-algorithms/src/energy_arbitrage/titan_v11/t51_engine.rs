...
pub struct Hyperparameters {
    ...
    /// i2/t51 (v7) — adaptive seed SCREENING for `joint_optimize_step`. The baked
    /// engine runs a full `projected_gradient_ascent` (grad_outer_iters=100) from
    /// EVERY seed of `[target, dp_seed, zero]` and keeps the argmax: 3 PGA runs per
    /// timestep, of which 2 are discarded. Screening picks ONE seed up-front, so PGA
    /// runs once — the ~3x PGA cut that made P69 the titan_v6 t51 champion (i89:
    /// +22,181 Q AND -4,000 ms vs i86). Modes differ ONLY by the fidelity of the
    /// score used to rank seeds (3 distinct mechanisms, increasing cost & realism):
    ///   0 = OFF (default) → all-seeds multi-start, BYTE-EXACT baseline.
    ///   1 = raw score: `total_step_value(seed)` as-is. Literal P69 transplant.
    ///       Separable across batteries and BLIND to the flow polytope (i91 root
    ///       cause: this metric makes dp_seed the systematic argmax).
    ///   2 = projected score: `safe_project_to_feasible(seed)` first, then score the
    ///       projected point. Coupling-aware for ~1 projection/seed instead of ~100
    ///       gradient+projection iterations. The projected seed is what PGA actually
    ///       starts from, so the ranking scores the true starting point.
    ///   3 = race: every seed gets a short PGA prefix of `seed_race_prefix` outer
    ///       iterations; the best REACHED value wins and is re-optimised for the full
    ///       `grad_outer_iters` budget. Ranks descent potential, not initial height —
    ///       the only mode that can prefer a low-scoring seed sitting in a better basin
    ///       (the failure mode flagged by Hindsight a2cd267d: on t51 the "cold" seeds
    ///       are cheap in value but valuable in convergence).
    #[serde(default)]
    pub seed_sel_mode: usize,
    /// Outer-iteration budget of the screening prefix in `seed_sel_mode=3`. Only read
    /// when mode==3. Kept well under grad_outer_iters so the race stays cheap; the
    /// WINNER always gets the full `grad_outer_iters` budget afterwards, so the
    /// `grad_outer<50` cliff (dead-list) is never applied to the retained seed.
...
impl Hyperparameters {
    ...
    fn parse(raw: &Option<Map<String, Value>>) -> Result<Self> {
        ...
        // Unknown seed_sel_mode values fall back to OFF (baseline multi-start) rather
        // than to an arbitrary mode; the race prefix must run at least one iteration.
        ...
        for s_idx in 0..levels {
            ...
            for &raw in &actions {
                ...
                let future = {
                    ...
                    // i38 : box-smooth ±3 (7-pts) — comble le trou du sweep (±2 pic mesuré, ±4 −47849 ; ±3 jamais benché).
                    let h = 1.0 / soc_step_inv;
                    (interp_value(next, next_soc - 3.0 * h, soc_lo, soc_step_inv, last)
                        + interp_value(next, next_soc - 2.0 * h, soc_lo, soc_step_inv, last)
                        + interp_value(next, next_soc - h, soc_lo, soc_step_inv, last)
...
fn dv_dsoc(dp: &BatteryDP, t: usize, soc: f64) -> f64 {
    ...
    // ITER t53/i18 : test t51 4-step [low,low+4]/4 held-out (le 3-step overfittait n<=50 ;
    // mesurer si le 4-step overfit AUSSI ou generalise comme sur t52/t53).
    if low + 4 <= last {
...
fn projected_gradient_ascent(
    ...
    seed: Vec<f64>,
    hp: &Hyperparameters,
    // Outer-iteration budget for THIS call. Every production call passes
    // `hp.grad_outer_iters` (iso-baseline); only the `seed_sel_mode=3` screening race
    // passes a shorter prefix. Taken as a parameter rather than read from `hp` so the
    // truncated race cannot leak into the full solve.
    outer_iters: usize,
) -> (Vec<f64>, f64) {
...
fn policy(
    ...
    // i2/t51 (v7) — adaptive seed SCREENING (see `seed_sel_mode`). The baked engine
    // spends `n_seeds` full PGA runs per timestep and discards all but the argmax.
    // Screening ranks the seeds with a cheap surrogate and hands ONE of them to
    // `joint_optimize_step`, so the expensive loop runs once. `seed_sel_mode=0`
    // (default) skips the whole block ⇒ BYTE-EXACT baseline.
    let seeds = if hp.seed_sel_mode == 0 || seeds.len() < 2 {
        seeds
    } else {
        // Score every seed under the selected fidelity. Modes 1/2 score a point;
        // mode 3 scores the value REACHED by a short ascent and carries that
        // partially-optimised action forward as the retained seed (no work thrown away).
        ...
        for seed in seeds.into_iter() {
            match hp.seed_sel_mode {
                ...
                2 => {
                    ...
                    // Retain the RAW seed: PGA projects its input as its first act, so
                    // handing it the raw seed is equivalent and avoids a second
                    // projection (and the float drift it would introduce).
                    carried.push(seed);
                }
                _ => {
        ...
        // Deterministic argmax: strict `>` keeps the FIRST maximum on ties, so an
        // exact tie reproduces the baseline seed order.
    ...
    let mut result = if hp.use_lp_only {
        match lp_dispatch_step(challenge, state, dps, sens, &base_flows, hp) {
            Some(mut a) => { safe_project_to_feasible(challenge, state, &mut a, sens, &base_flows, hp);
                if is_flow_feasible(challenge, state, &a) { a } else { joint_optimize_step(challenge, state, dps, sens, &base_flows, seeds, hp) } }
            None => joint_optimize_step(challenge, state, dps, sens, &base_flows, seeds, hp),
        }
...
// ---- LP dispatch porté de t53_engine (iter t51_lp) ----
fn lp_dispatch_step(
    challenge: &Challenge,
    state: &State,
    dps: &[BatteryDP],
