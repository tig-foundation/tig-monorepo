...
impl VRPTWLoader {
    pub fn load(path: &str) -> Result<Problem> {
        let file =
            File::open(path).map_err(|e| anyhow!("Cannot open VRPTW file '{}': {}", path, e))?;
        ...
        for line in lines {
            ...
            nodes.push((
                p[0].parse::<usize>()
                    .map_err(|_| anyhow!("Invalid VRPTW node id '{}'", p[0]))?,
                p[1].parse::<i32>()
                    .map_err(|_| anyhow!("Invalid VRPTW x '{}'", p[1]))?,
                p[2].parse::<i32>()
                    .map_err(|_| anyhow!("Invalid VRPTW y '{}'", p[2]))?,
                p[3].parse::<i32>()
                    .map_err(|_| anyhow!("Invalid VRPTW demand '{}'", p[3]))?,
                p[4].parse::<i32>()
                    .map_err(|_| anyhow!("Invalid VRPTW ready time '{}'", p[4]))?,
                p[5].parse::<i32>()
                    .map_err(|_| anyhow!("Invalid VRPTW due time '{}'", p[5]))?,
                p[6].parse::<i32>()
                    .map_err(|_| anyhow!("Invalid VRPTW service time '{}'", p[6]))?,
            ));
        }
        ...
        let lb_vehicles = ((total_demand + max_capacity as i64 - 1) / max_capacity as i64) as usize;
