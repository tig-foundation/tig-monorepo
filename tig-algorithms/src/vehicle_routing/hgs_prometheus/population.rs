    ...
    pub fn print_trace(
        ...
        let (feas_best_routes_str, feas_best_str, feas_avg_str) = if feas_n == 0 {
            ...
            (
                ...
                (top_mu_best_cost(&self.feasible).unwrap_or(0) + self.data.fixed_distance_offset)
                    .to_string(),
                (top_mu_avg_cost(&self.feasible).unwrap_or(0) + self.data.fixed_distance_offset)
                    .to_string(),
            )
        };
        ...
        let (infeas_best_routes_str, infeas_best_str, infeas_avg_str) = if infeas_n == 0 {
            ...
            (
                ...
                (top_mu_best_cost(&self.infeasible).unwrap_or(0) + self.data.fixed_distance_offset)
                    .to_string(),
                (top_mu_avg_cost(&self.infeasible).unwrap_or(0) + self.data.fixed_distance_offset)
                    .to_string(),
            )
        };
