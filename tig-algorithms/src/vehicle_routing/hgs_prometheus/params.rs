...
impl Params {
    ...
    pub(super) fn preset(exploration_level: usize, data: &Problem) -> Self {
        ...
        match exploration_level {
            ...
            5 => Self {
                ...
                penalty_capa: p_capa,
                target_ratio: 0.4,
                max_it_noimprov: 2_000,
                max_it_total: 20_000,
                nb_it_adapt_penalties: 50,
                nb_it_traces: 200,
                nb_it_compression: 100,
                mu: 25,
                mu_start: 50,
                lambda: 40,
                nb_close: 5,
    ...
    ///
    /// The five active TIG tracks use the benchmarker-derived HGS profiles
    /// that were used to validate Lightspeed. Unknown instance sizes retain
    /// the original exploration-level-4 preset.
    pub fn defaults(data: &Problem) -> Self {
        let mut params = Self::preset(4, data);

        let track_profile = match data.nb_nodes {
            600 => Some((true, 35, 45, 0.6, 25, 200, 9)),
            700 => Some((true, 30, 45, 0.4, 30, 233, 8)),
            800 => Some((true, 25, 35, 0.3, 10, 200, 11)),
            900 => Some((true, 30, 40, 0.5, 20, 150, 12)),
            1000 => Some((true, 30, 40, 0.5, 20, 200, 12)),
            _ => None,
        };
        ...
        if let Some((
            ...
            granularity,
            granularity2,
            swapstar_capa_filter,
            max_credit_deterioration,
            decomp_target_size,
            decomp_nb_phases,
        )) = track_profile
        {
            params.allow_swap3 = allow_swap3;
            params.granularity = granularity;
            params.granularity2 = granularity2;
            params.swapstar_capa_filter = swapstar_capa_filter;
            params.max_credit_deterioration = max_credit_deterioration;
            params.decomp_target_size = decomp_target_size;
            params.decomp_nb_phases = decomp_nb_phases;
        }
