...
impl Sequence {
    ...
    /// Compose only the temporal state and endpoints.  In intra-route scans
    /// the growing middle fragment is used solely by the time-warp evaluator;
    /// carrying load and distance through every step was dead work.
    #[inline(always)]
    pub fn join_tw(data: &Problem, s1: &Sequence, s2: &Sequence) -> Sequence {
        let travel = data.dm(s1.last_node as usize, s2.first_node as usize);
        Self::join_tw_with_travel(s1, s2, travel)
    }
    ...
    /// Time-warp component for two sequences when the connecting travel arc
    /// has already been loaded by a distance/capacity lower-bound evaluation.
    #[inline(always)]
    ...
    /// Time-warp component for three sequences with both connecting arcs
    /// supplied by the caller. This is algebraically identical to `eval3`'s
    /// time-window calculation and deliberately omits distance/load work.
    #[inline(always)]
    ...
    pub fn eval4(
        ...
        // Join s0 and s1
        let travel01 = data.dm(s0.last_node as usize, s1.first_node as usize);
    ...
    pub fn eval5(
        ...
        let travel34 = data.dm(s3.last_node as usize, s4.first_node as usize);
