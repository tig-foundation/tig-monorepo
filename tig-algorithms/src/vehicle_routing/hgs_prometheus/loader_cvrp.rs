...
impl CVRPLoader {
    pub fn load(path: &str) -> Result<Problem> {
        let file =
            File::open(path).map_err(|e| anyhow!("Cannot open CVRP file '{}': {}", path, e))?;
        ...
        let node_coord_pos =
            node_coord_pos.ok_or_else(|| anyhow!("CVRP NODE_COORD_SECTION not found"))?;
        ...
        for i in 0..nb_nodes {
            ...
            let id = p[0]
                .parse::<usize>()
                .map_err(|_| anyhow!("Invalid CVRP node id '{}'", p[0]))?;
            if id != i + 1 {
                return Err(anyhow!("CVRP node numbering should be contiguous from 1"));
            }
            let x = p[1]
                .parse::<i32>()
                .map_err(|_| anyhow!("Invalid CVRP x coordinate '{}'", p[1]))?;
            let y = p[2]
                .parse::<i32>()
                .map_err(|_| anyhow!("Invalid CVRP y coordinate '{}'", p[2]))?;
        ...
        for i in 0..nb_nodes {
            ...
            let id = p[0]
                .parse::<usize>()
                .map_err(|_| anyhow!("Invalid CVRP demand node id '{}'", p[0]))?;
            if id != i + 1 {
                return Err(anyhow!("CVRP demand numbering should be contiguous from 1"));
            }
            let d = p[1]
                .parse::<i32>()
                .map_err(|_| anyhow!("Invalid CVRP demand '{}'", p[1]))?;
            demands[i] = d;
