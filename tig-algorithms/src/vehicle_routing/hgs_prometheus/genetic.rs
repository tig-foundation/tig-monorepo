...
impl Genetic {
    ...
    fn pen_beta_x(
        ...
        let ri = copy_at(r, i);
        if ri > 0 {
            beta * (ri + w_x - copy_at(w, copy_at(q_first_warp, i)))
        } else {
            0
        }
    }
    ...
    fn dominates_ab_x(
        ...
        let lhs = copy_at(prev_layer, i) + copy_at(dist_from_depot, i) - copy_at(edge_prefix, i)
            ...
            + Self::pen_alpha_x(demand_x, copy_at(demand_prefix, i), max_capacity, alpha)
            + Self::pen_beta_x(i, r, w_x, w, q_first_warp, beta);
        let rhs = copy_at(prev_layer, j)
            + copy_at(dist_from_depot, j)
            + Self::pen_alpha_x(demand_x, copy_at(demand_prefix, j), max_capacity, alpha)
    ...
    fn split_linear(&self, giant: &Vec<usize>, target_routes: usize) -> Vec<Vec<usize>> {
        ...
        for x in 1..=n {
            ...
            if duration > time_limit {
                let next_w = copy_at(&w, x - 1) + (duration - time_limit);
                write_at(&mut w, x, next_w);
                duration = time_limit;
            } else {
                let next_w = copy_at(&w, x - 1);
                write_at(&mut w, x, next_w);
            }
        }
                    ...
                    for x in kk..=n {
                        ...
                        if queue.no_warp != NIL {
                            ...
                            loop {
                                ...
                                let left =
                                    copy_at(&dist_from_depot, nw) + (s_t - copy_at(&s_prefix, nw));
                                let right =
                                    copy_at(&tw_start, j_wait) + (s_t - copy_at(&s_prefix, j_wait));
                                let rw = (left.max(right) - tw_end_t).max(0);
                                write_at(&mut r, nw, rw);
                                if rw > 0 {
                                    write_at(&mut q_first_warp, nw, x);
                                    queue.no_warp = copy_at(&queue.next, nw);
                        ...
                        while queue.feas != NIL {
                            let feas = queue.feas;
                            let pa_feas = Self::pen_alpha_x(
                                demand_x,
                                copy_at(&demand_prefix, feas),
                                max_capacity,
                                alpha,
                            );
                            let pb_feas = Self::pen_beta_x(feas, &r, w_x, &w, &q_first_warp, beta);
                            if !(pa_feas > 0 || pb_feas > 0) {
                                break;
                            }
                            ...
                            queue.feas = copy_at(&queue.next, feas);
                            if queue.feas == NIL {
                                break;
                            }
                        ...
                        while queue.size > 1 {
                            let Some(f2) = queue.front2() else {
                                break;
                            };
                            let pa_f2 = Self::pen_alpha_x(
                                demand_x,
                                copy_at(&demand_prefix, f2),
                                max_capacity,
                                alpha,
                            );
                        ...
                        let route_dist_best = copy_at(&dist_from_depot, best)
                            ...
                            + dist_to_depot_t;
                        let cand = copy_at(&prev_layer, best) + route_dist_best + pa_best + pb_best;
                        if cand < copy_at(&cur_layer, x) {
                            write_at(&mut cur_layer, x, cand);
                            write_at(&mut pred, kk * pred_stride + x, best);
                        }
                    }
    ...
    pub fn crossover_srex(
        ...
        let mut p1_rank: Vec<(usize, usize)> = p1
            .routes
            .iter()
            .enumerate()
            .map(|(idx, r)| {
            ...
            .collect();
        p1_rank.sort_unstable_by(|a, b| b.0.cmp(&a.0).then_with(|| a.1.cmp(&b.1)));

        let remove_count = selected_from_p2.len().min(p1.routes.len());
        let mut remove_p1 = vec![0u8; p1.routes.len()];
        for k in 0..remove_count {
            remove_p1[p1_rank[k].1] = 1;
        }
        ...
        for id in unplanned {
            ...
            for (rid, st) in states.iter().enumerate() {
                ...
                for pos in 1..route.len() {
                    let cand_cost =
                        Sequence::eval3(data, params, &st.head[pos - 1], &ins_seq, &st.tail[pos]);
        ...
        let routes = states
            .into_iter()
            .map(|st| st.route.expect("Final route state should contain a route"))
            .filter(|r| r.len() > 2)
            .collect();
        routes
    }
    ...
    pub fn crossover_ox(
        ...
        // Keep the original modulo-loop semantics: when `stop == start`, the
        // copied segment is empty. Expanding this into an inclusive wrapped
        // range would copy the full tour and alter HGS's deterministic path.
        let stop = (end + 1) % n;
        let mut j = start;
        while j % n != stop {
            let idx = j % n;
