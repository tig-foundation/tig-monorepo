        ...
        while improved {
            ...
            for i in 1..best_route.len() - 2 {
                for j in i + 1..best_route.len() - 1 {
                    ...
                    let old_distance = calculate_route_distance(&best_route, distance_matrix);
...
mod simple_solver {
    ...
    pub fn solve_sub_instance_simple(
        ...
        while let Some(node) = nodes.pop() {
            ...
            // Apply 2-opt to the route after construction
            route = apply_2opt(&route, distance_matrix, service_time, ready_times, due_times);
        ...
        while improved {
            ...
            for (_corr, node, node2) in &proximity_pairs {
                ...
                if let Some((best_pos, _delta_cost)) = find_best_insertion_in_route(
                    ...
                    if new_routes[route1_idx].len() > pos1 && new_routes[route1_idx][pos1] == node {
                        ...
                        new_routes[route1_idx] = apply_2opt(&new_routes[route1_idx], distance_matrix, service_time, ready_times, due_times);
                        new_routes[route2_idx] = apply_2opt(&new_routes[route2_idx], distance_matrix, service_time, ready_times, due_times);
            ...
            if !improved {
                ...
                for route_idx in 0..current_routes.len() {
                    ...
                    let improved_route = apply_2opt(route, distance_matrix, service_time, ready_times, due_times);
                    ...
                    for i in 1..route.len() - 1 {
                        for j in i + 1..route.len() - 1 {
                            ...
                            if new_route_distance < old_route_distance {
                                // Apply 2-opt after swap
                                let optimized_route = apply_2opt(&new_route, distance_matrix, service_time, ready_times, due_times);
                                let mut new_routes = best_routes.clone();
                        ...
                        for route2_idx in route1_idx + 1..best_routes.len() {
                            ...
                            for i in 1..route1.len() - 1 {
                                ...
                                for j in 1..route2.len() - 1 {
                                    ...
                                    // Apply 2-opt to both routes after swap
                                    new_route1 = apply_2opt(&new_route1, distance_matrix, service_time, ready_times, due_times);
                                    new_route2 = apply_2opt(&new_route2, distance_matrix, service_time, ready_times, due_times);
                        ...
                        if route1_demand + route2_demand <= max_capacity {
                            ...
                            if let Some(pos) = best_insertion_pos {
                                ...
                                if is_route_feasible(
                                    ...
                                    new_route = apply_2opt(&new_route, distance_matrix, service_time, ready_times, due_times);
