...
impl<'a> LocalOps<'a> {
    pub fn new(data: &'a Instance, params: Config) -> Self {
        ...
        for i in 1..n {
            ...
            for j in 1..n {
                ...
                if (data.nd(j).demand - di).abs() <= diff_limit {
                    let dij = data.dm(i, j);
                    prox.push((dij, j));
                }
            }
    ...
    fn load_from_routes(&mut self, routes: &Vec<Vec<usize>>) {
        ...
        for v in self.node_route.iter_mut() { *v = 0; }
        for v in self.node_pos.iter_mut() { *v = 0; }
        ...
        for v in self.when_last_modified[..nr].iter_mut() { *v = 0; }
        for v in self.when_last_tested.iter_mut() { *v = 0; }
        self.nb_moves = 1;
    ...
    fn update_route(&mut self, rid: usize) {
        self.update_route_from(rid, 0);
    }

    fn update_route_from(&mut self, rid: usize, from: usize) {
        let data = self.data;
        let sings = &self.singletons;
        let r = &mut self.routes[rid];
        let len = r.nodes.len();

        let start_fwd = if from == 0 { 0 } else { from.min(len - 1) };
        if start_fwd == 0 {
            let acc = sings[r.nodes[0].id];
            r.nodes[0].seq0_i = acc;
        }
        let mut acc_fwd = r.nodes[start_fwd].seq0_i;
        for pos in (start_fwd + 1)..len {
            acc_fwd = RouteEval::join2(data, &acc_fwd, &sings[r.nodes[pos].id]);
            r.nodes[pos].seq0_i = acc_fwd;
        }

        let end_bwd = if from == 0 { 0 } else { from.saturating_sub(1) };
        {
            let acc = sings[r.nodes[len - 1].id];
            r.nodes[len - 1].seqi_n = acc;
        }
        let mut acc_bwd = r.nodes[len - 1].seqi_n;
        for pos in (0..len - 1).rev() {
            acc_bwd = RouteEval::join2(data, &sings[r.nodes[pos].id], &acc_bwd);
            r.nodes[pos].seqi_n = acc_bwd;
            if pos <= end_bwd { break; }
        }

        let seq_start = if from > 0 { from.saturating_sub(1) } else { 0 };
        for pos in seq_start..len {
        ...
        let epos = self.empty_routes.iter().position(|&eid| eid == rid);
        match (is_empty, epos) {
    ...
    pub fn run_intra_route_oropt(&mut self, r1: usize, pos1: usize, l: usize) -> bool {
        ...
        let applied = {
            ...
            let block_seq = if l == 2 {
                ...
                RouteEval::join2(self.data, &route.nodes[pos1].seq12, &self.singletons[route.nodes[pos1 + 2].id])
            };
            ...
            if x_next.id != 0 && self.params.allow_swap3 {
                ...
                if v.id != 0 {
                    ...
                    if y.id != 0 {
                        ...
                        let result43 = RouteEval::eval3(data, &self.params, &u_pred.seq0_i, &v_seq21_sw3, &x2_next.seqi_n)
                            + RouteEval::eval3(data, &self.params, &v_pred.seq0_i, &u_seq123, &y_next.seqi_n);
                        update_best(4, 2, result42);
                        update_best(4, 3, result43);

                        if y_next.id != 0 {
                            let v_seq123 = RouteEval::join2(data, &v.seq12, &self.singletons[y_next.id]);
    ...
    pub fn runls(
        ...
        let mut loop_id = 0;
        let max_passes = self.params.max_ls_passes;
        while improved && loop_id < max_passes {
            improved = false;
            loop_id += 1;
