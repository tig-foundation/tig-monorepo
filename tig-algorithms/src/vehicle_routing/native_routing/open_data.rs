...
mod utils {
    ...
    fn is_route_time_feasible_fast(
        route: &Vec<usize>,
        distance_matrix: &Vec<Vec<i32>>,
        service_time: i32,
        ready_times: &Vec<i32>,
...
mod complex_solver {
    ...
    fn do_local_searches(
        ...
        let max_swap_iterations = 5;
        let max_merge_iterations = 5;
                    ...
                    for i in 1..route.len() - 1 {
                        for j in i + 1..route.len() - 1 {
                            ...
                            if new_route_distance < old_route_distance {
                                let optimized_route = apply_size_filtered_local_search(&new_route, distance_matrix, service_time, ready_times, due_times);
                                let mut new_routes = best_routes.clone();
                                new_routes[route_idx] = optimized_route;
                        ...
                        for route2_idx in route1_idx + 1..best_routes.len() {
                            ...
                            for i in 1..route1.len() - 1 {
                                ...
                                for j in 1..route2.len() - 1 {
                                    ...
                                    new_route1 = apply_size_filtered_local_search(&new_route1, distance_matrix, service_time, ready_times, due_times);
                                    new_route2 = apply_size_filtered_local_search(&new_route2, distance_matrix, service_time, ready_times, due_times);
                        ...
                        if route1_demand + route2_demand <= max_capacity {
                            ...
                            if let Some(pos) = best_insertion_pos {
                                ...
                                if is_route_feasible(
                                    ...
                                    new_route = apply_size_filtered_local_search(&new_route, distance_matrix, service_time, ready_times, due_times);
                                        ...
                                        match calc_routes_total_distance(
                                            ...
                                            Ok(total_distance) => {
                                                if total_distance < best_distance {
                                                    best_distance = total_distance;
                                                    best_routes = new_routes;
                                                    for (route_idx, route) in best_routes.iter().enumerate() {
                                                        for (j, &node) in route[1..route.len() - 1].iter().enumerate() {
                                                            node_positions[node] = (route_idx, j + 1);
                                                        }
                                                    }
