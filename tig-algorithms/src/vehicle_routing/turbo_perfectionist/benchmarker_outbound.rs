...
pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    ...
    for sub_instance in &challenge.sub_instances {
        let sub_solution = simple_solver::solve_sub_instance_simple(sub_instance)?;
...
mod utils {
    ...
    pub fn find_best_insertion(
        ...
        for &insert_node in remaining_nodes {
            let base_c2 = lambda * distance_matrix[0][insert_node];
            let mut curr_time = 0;
            let mut curr_node = 0;
    ...
    pub fn compute_proximity(
        ...
        let expr1 = (ready_times[j] - time_ij - service_time - due_times[i]).max(0)
            + (ready_times[i] + service_time + time_ij - due_times[j]).max(0);
        let expr2 = (ready_times[i] - time_ij - service_time - due_times[j]).max(0)
            + (ready_times[j] + service_time + time_ij - due_times[i]).max(0);
        time_ij + expr1.min(expr2)
    }
    ...
    pub fn check_feasible_insertion(
        ...
        if insert_pos == route.len() - 1 {
            ...
            let _final_arrival = departure + distance_matrix[insert_node][0];
    ...
    pub fn apply_efficient_2opt(
        ...
        let mut iteration = 0;
        let max_iterations = if route.len() > 80 { 25 } else { (route.len() / 2).min(20) };

        let mut new_route: Vec<usize> = Vec::with_capacity(best_route.len());
        while improved && iteration < max_iterations {
            ...
            for i in 1..best_route.len() - 2 {
                for j in i + 2..best_route.len() - 1 {
                    ...
                    let delta = (distance_matrix[prev][b] + distance_matrix[a][next])
                        - (distance_matrix[prev][a] + distance_matrix[b][next]);
                    if delta >= 0 {
                        continue;
                    }
                        ...
                        )
                    {
                        ...
                        let new_distance = best_distance + delta;
                        if new_distance < best_distance {
                            best_distance = new_distance;                            
                            std::mem::swap(&mut best_route, &mut new_route);
                            improved = true;
                            break;
                        }
                    }
            ...
            for segment_size in 1..=max_seg {
                ...
                for i in 1..len - segment_size {
                    ...
                    for insert_pos in 1..len {
                        ...
                        let delta_remove_segment = distance_matrix[prev_before][after_segment]
                            - distance_matrix[prev_before][first]
                            - distance_matrix[last][after_segment];
                        let delta_insert_segment = distance_matrix[prev_ins][first]
                            + distance_matrix[last][succ_ins]
                            - distance_matrix[prev_ins][succ_ins];
...
mod simple_solver {
    ...
    pub fn solve_sub_instance_simple(
        ...
        while let Some(node) = nodes.pop() {
            ...
            let max_insertion_attempts = if num_nodes >= 2000 { 3 } else if num_nodes >= 1500 { 3 } else { 2 };
            let mut candidates: Vec<usize> = Vec::new();
            
            while insertion_attempts < max_insertion_attempts {
                candidates.clear();
                if candidates.capacity() < remaining.len() {
                    candidates.reserve(remaining.len() - candidates.capacity());
                }
                for &n in remaining.iter() {
                    if route_demand + demands[n] <= max_capacity {
                        candidates.push(n);
                    }
                }
        ...
        if need_fallback || validated_routes.is_empty() {
            ...
            return Ok(Some(SubSolution { routes: fallback }));
        }

        Ok(Some(SubSolution { routes: validated_routes }))
    }
    ...
    fn do_local_searches(
        ...
        let mut route_distances: Vec<i32> = best_routes
            .iter()
            .map(|r| utils::calculate_route_distance(r, distance_matrix))
            .collect();
        let mut best_distance: i32 = route_distances.iter().sum();
        let mut improved = true;
        let mut iteration_count = 0;
        let max_total_iterations = if num_nodes >= 2500 { 80 } else if num_nodes >= 2000 { 65 } else if num_nodes >= 1500 { 50 } else { 45 };
        
        let neighbor_count = if num_nodes >= 2500 { 7usize } else if num_nodes >= 2000 { 6usize } else if num_nodes >= 1500 { 4usize } else { 3usize };
        let pool = if num_nodes >= 2500 { 40usize } else if num_nodes >= 2000 { 36usize } else if num_nodes >= 1500 { 24usize } else { 18usize };
        let mut proximity_pairs: Vec<(i32, usize, usize)> = Vec::with_capacity(num_nodes * neighbor_count);
        for i in 1..num_nodes {
            ...
            for j in 1..num_nodes {
                ...
                if pool_vec.len() < pool {
                    ...
                    let mut idx = 0;
                    let mut val = pool_vec[0].0;
                    for (t, &(dv, _)) in pool_vec.iter().enumerate().skip(1) {
                        if dv > val {
                            val = dv;
                            idx = t;
                        }
        ...
        proximity_pairs.sort_unstable_by_key(|x| x.0);
        ...
        let mut cand_r1: Vec<usize> = Vec::new();
        let mut cand_r2: Vec<usize> = Vec::new();
        let mut swap_r1: Vec<usize> = Vec::new();
        let mut swap_r2: Vec<usize> = Vec::new();
                ...
                if target_route_demand + demands[node] <= max_capacity {
                    if let Some((best_pos, _delta_cost)) = find_best_insertion_in_route(
                        ...
                        if best_routes[route1_idx].len() > pos1 && best_routes[route1_idx][pos1] == node {                            
                            ...
                            let delta_remove = distance_matrix[prev1][next1]
                                - distance_matrix[prev1][node]
                                - distance_matrix[node][next1];
                            let delta_insert = distance_matrix[prev2][node]
                                + distance_matrix[node][next2]
                            ...
                            let r1 = apply_size_filtered_local_search(&cand_r1, distance_matrix, service_time, ready_times, due_times);
                            let r2 = apply_size_filtered_local_search(&cand_r2, distance_matrix, service_time, ready_times, due_times);

                            if r1.len() >= 3 && r1[0] == 0 && r1[r1.len() - 1] == 0 &&
                               r2.len() >= 3 && r2[0] == 0 && r2[r2.len() - 1] == 0 {
                   ...
                   route_demands[route2_idx] - demands[node2] + demands[node] <= max_capacity {                    
                    if best_routes[route1_idx].len() > pos1 && best_routes[route1_idx][pos1] == node &&
                       best_routes[route2_idx].len() > pos2 && best_routes[route2_idx][pos2] == node2 {
                        ...
                        let delta1 = distance_matrix[prev1][node2]
                            + distance_matrix[node2][next1]
                            - distance_matrix[prev1][node]
                            - distance_matrix[node][next1];
                        let delta2 = distance_matrix[prev2][node]
                            + distance_matrix[node][next2]
                            - distance_matrix[prev2][node2]
                            - distance_matrix[node2][next2];
                        if delta1 + delta2 >= 0 {
                            continue;
                        }
                        ...
                        if is_route_time_feasible_fast(&swap_r1, distance_matrix, service_time, ready_times, due_times) &&
                           is_route_time_feasible_fast(&swap_r2, distance_matrix, service_time, ready_times, due_times) {
                            ...
                            if new_distance < best_distance {
                                ...
                                let old_vec1 = std::mem::replace(&mut best_routes[route1_idx], std::mem::take(&mut swap_r1));
                                let old_vec2 = std::mem::replace(&mut best_routes[route2_idx], std::mem::take(&mut swap_r2));
                                
                                route_distances[route1_idx] = new_d1;
                                route_distances[route2_idx] = new_d2;
                                route_demands[route1_idx] = route_demands[route1_idx] - demands[node] + demands[node2];
                                route_demands[route2_idx] = route_demands[route2_idx] - demands[node2] + demands[node];
