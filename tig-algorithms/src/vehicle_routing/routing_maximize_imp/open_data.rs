...
fn analyze_problem(challenge: &Challenge) -> ProblemStats {
    ...
    let p_ratio = challenge.max_total_distance as f32 / 
...
fn find_clusters(challenge: &Challenge, avg_distance: f32) -> (bool, Vec<usize>) {
    let num_nodes = challenge.difficulty.num_nodes;
...
fn solve_with_clusters(challenge: &Challenge, stats: &ProblemStats) -> anyhow::Result<Option<Solution>> {
    let num_nodes = challenge.difficulty.num_nodes;
    ...
    let mut solution = Solution { routes };
    ...
    if cost <= challenge.max_total_distance {
        Ok(Some(solution))
    } else {
        Ok(None)
    }
...
fn solve_with_demand_priority(challenge: &Challenge, _stats: &ProblemStats) -> anyhow::Result<Option<Solution>> {
    let num_nodes = challenge.difficulty.num_nodes;
...
fn solve_with_savings(challenge: &Challenge, stats: &ProblemStats) -> anyhow::Result<Option<Solution>> {
