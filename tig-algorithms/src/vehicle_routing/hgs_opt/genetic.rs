...
pub struct Genetic<'a> {
    ...
    crossover_child: Vec<usize>,
    crossover_used: Vec<bool>,
}
...
impl<'a> Genetic<'a> {
    pub fn new(data: &'a Problem, params: Params) -> Self {
        ...
        Self { 
            ...
            params, 
            population,
            crossover_child: vec![0; n_clients],
            crossover_used: vec![false; data.nb_nodes],
        }
    }
    ...
    pub fn split(&self, giant: &Vec<usize>, target_routes: usize) -> Vec<Vec<usize>> {
        ...
        // Flattened 1D arrays for better cache locality
        // dp[kk * stride + j] = best cost to cover first j clients with exactly kk non-empty routes
        let stride = n + 1;
        let mut dp = vec![inf; (k + 1) * stride];
        let mut pred = vec![0usize; (k + 1) * stride];
        dp[0] = 0; // dp[0][0]
        ...
        assert!(dp[k * stride + n] < inf, "Exact-K Split failed unexpectedly");

        // Backtrack routes
        let mut routes: Vec<Vec<usize>> = Vec::with_capacity(k);
        let mut j = n;
        for kk in (1..=k).rev() {
            let i = pred[kk * stride + j];
    ...
    pub fn crossover_ox(&mut self, parent1: &Vec<usize>, parent2: &Vec<usize>, rng: &mut SmallRng) -> Vec<usize> {
        ...
        // Reuse preallocated vectors
        self.crossover_child.fill(0);
        self.crossover_used.fill(false);
        ...
        while j % n != stop {
            let idx = j % n;
            let v = parent1[idx];
            self.crossover_child[idx] = v;
            self.crossover_used[v] = true;
            j += 1;
        }
        ...
        for t in 0..n {
            ...
            if !self.crossover_used[v] {
                self.crossover_child[pos] = v;
                self.crossover_used[v] = true;
                pos = (pos + 1) % n;
            }
        }
        debug_assert!(self.crossover_child.iter().all(|&x| x != 0), "Child giant tour must be fully filled");
        self.crossover_child.clone()
    }
}
