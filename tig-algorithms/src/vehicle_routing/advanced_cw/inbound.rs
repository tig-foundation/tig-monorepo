...
pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    ...
    for init_value in [1.0, 2.0] {
        ...
        if (current_cost as f32 * 0.96) > challenge.max_total_distance as f32 && !promising {
            return Ok(None);
        } else {
            promising = true;
        }
...
unsafe fn unsafe_find_best_swap_with_value(
    ...
    for i in 1..(r1_len - 1) {
        for j in 1..(r2_len - 1) {
            let [r1_im1, r1_i, r1_ip1] = [*r1_ptr.add(i - 1), *r1_ptr.add(i), *r1_ptr.add(i + 1)];

            let [r2_jm1, r2_j, r2_jp1] = [*r2_ptr.add(j - 1), *r2_ptr.add(j), *r2_ptr.add(j + 1)];
