...
impl MoveArgs {
    pub fn new(route1: usize, pos1: usize, route2: usize, pos2: usize) -> Self {
        Self { route1, pos1, route2, pos2, profit: 0 }
    }
}
...
impl TigLoader {
    pub fn load(params: Parameters, challenge: &SubInstance) -> Data {
        Data {
            seed: challenge.seed,
            ...
            distance_matrix: challenge.distance_matrix.iter().map(|row| row.iter()
                .map(|&x| x as isize).collect()).collect(),
            service_times: std::iter::once(0).chain(std::iter::repeat(challenge.service_time as isize)
                .take(challenge.difficulty.num_nodes)).collect(),
...
impl Route {
    ...
    pub fn preprocess(&mut self, data: &Data) {
        ...
        for pos in 0..len {
            ...
            if pos + 1 < len {
                ...
                self.nodes[pos].seq12 = Sequence::join2(data, &Sequence::singleton(data, id), &Sequence::singleton(data, id_next));
                self.nodes[pos].seq21 = Sequence::join2(data, &Sequence::singleton(data, id_next), &Sequence::singleton(data, id));
            }
        }
    ...
    pub fn to_string(&self) -> String {
        ...
        result.push_str(&format!("] (Cost: {}, Load: {}, TW: {})", self.cost, self.load, self.tw));
        result
    }
}
...
impl Swap {
    ...
    pub fn perform(data: &Data, ind: &mut Individual, args: &MoveArgs) {
        ...
        ind.routes[args.route1].nodes[args.pos1] = replace(&mut ind.routes[args.route2].nodes[args.pos2], temp);
...
impl Constructive {
    ...
    fn find_best_insertion(route: &Vec<usize>, remaining_nodes: Vec<usize>, data: &Data) -> Option<(usize, usize)> {
        let alpha1 = 1;
        let alpha2 = 0;
        let lambda = 1;
        ...
        for insert_node in remaining_nodes {
            ...
            for pos in 1..route.len() {
                ...
                let old_arrival_time =
                    data.start_tw[next_node].max(curr_time + data.distance_matrix[curr_node][next_node]);
...
impl<'a> LocalSearch<'a> {
    pub fn new(data: &'a Data) -> Self {
        ...
        Self { data, neighbors_before }
    }

    pub fn run_intra_route_relocate(&mut self, ind: &mut Individual, r1: usize, pos1: usize) -> bool {
        ...
        for t in 1..len {
            ...
            let new_cost = Sequence::eval3(self.data, &left_excl[t], &route.nodes[pos1].seq1, &right_excl[t]);
    ...
    pub fn run_intra_route_swap_right(&mut self, ind: &mut Individual, r1: usize, pos1: usize) -> bool {
        let route = &ind.routes[r1];
        let len   = route.nodes.len();
        if len < pos1 + 4 { return false; } 
        ...
        for pos2 in (pos1 + 2)..(len - 1) {
            let new_cost = Sequence::eval_n(self.data,
    ...
    pub fn run_2opt(&mut self, ind: &mut Individual, r1: usize, pos1: usize) -> bool {
        ...
        for pos2 in (pos1 + 1)..(len - 1) {
            let new_cost = Sequence::eval3(self.data, &route.nodes[pos1 - 1].seq0_i, &mid_rev, &route.nodes[pos2 + 1].seqi_n);
            if new_cost < best_cost {
                best_cost = new_cost;
                best_pos = Some(pos2);
            }
            ...
            for c1 in c1_order {
                ...
                for r2 in 0..ind.routes.len()  {
                    ...
                    if ind.routes[r2].nodes.len() == 2 {
                        if loop_id == 1 || tested_empty_route { continue } ;
                ...
                improved |= self.run_intra_route_relocate(ind, ind.node_route[c1], ind.node_pos[c1]);
                improved |= self.run_intra_route_swap_right(ind, ind.node_route[c1], ind.node_pos[c1]);
            }
        }
    }
}
...
impl Solver {
    fn solve(data: Data) -> anyhow::Result<Option<SubSolution>> {
...
pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    ...
    for sub_instance in &challenge.sub_instances {
        match Solver::solve_sub_instance(sub_instance)? {
