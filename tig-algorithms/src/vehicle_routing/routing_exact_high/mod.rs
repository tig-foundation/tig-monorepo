...
mod simple_solver {
    ...
    fn do_local_searches(
        ...
        let mut proximity_pairs: Vec<(i32, usize, usize)> =
            Vec::with_capacity(num_nodes * k_neighbors);
        for i in 1..num_nodes {
            ...
            for j in 1..num_nodes {
                ...
                let p =
                    compute_proximity(i, j, distance_matrix, ready_times, due_times, service_time);
                match best1 {
                    None => {
                        best1 = Some((p, j));
                    }
            ...
            for (_corr, node, node2) in &proximity_pairs {
                    ...
                    && route_demands[route2_idx] - demands[node2] + demands[node] <= max_capacity
                {
                        ...
                        && best_routes[route2_idx][pos2] == node2
                    {
                        ...
                        let delta1 = distance_matrix[prev1][node2] + distance_matrix[node2][next1]
                            - distance_matrix[prev1][node]
                            - distance_matrix[node][next1];
                        let delta2 = distance_matrix[prev2][node] + distance_matrix[node][next2]
