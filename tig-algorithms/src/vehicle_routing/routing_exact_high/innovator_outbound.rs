...
mod utils {
    ...
    pub fn apply_efficient_2opt(
        ...
        let mut iteration = 0;
        let max_iterations = (route.len() / 2).min(20);
...
mod simple_solver {
    ...
    fn do_local_searches(
        ...
        let max_total_iterations = 45;
        
        let k_neighbors = 2usize;
        let mut proximity_pairs: Vec<(i32, usize, usize)> = Vec::with_capacity(num_nodes * k_neighbors);
        for i in 1..num_nodes {                        
            let mut best1: Option<(i32, usize)> = None;
            let mut best2: Option<(i32, usize)> = None;
            for j in 1..num_nodes {
                if j == i {
                    continue;
                }                
                if let Some((p2, _)) = best2 {
                    let lower_bound = distance_matrix[i][j];
                    if lower_bound >= p2 {
                        continue;
                    }
                }
            ...
            if let Some((p, j)) = best1 {
                proximity_pairs.push((p, i, j));
            }
            if let Some((p, j)) = best2 {
                proximity_pairs.push((p, i, j));
            }
        }
