...
Licensed under the TIG Benchmarker Outbound Game License v3.0 (the "License"); you
...
mod utils {
    ...
    pub fn find_best_insertion_in_route(
        ...
        unsafe {
            ...
            while pos < len {
                ...
                let delta = *row_prev.get_unchecked(node)
                    + *row_node.get_unchecked(next_node)
    ...
    pub fn apply_efficient_2opt(
        ...
        let mut iteration = 0;
        let max_iterations = if route.len() > 80 { 40 } else { (route.len() / 2).min(35) };
        ...
        while improved && iteration < max_iterations {
            ...
            for i in 1..end - 1 {
                for j in i + 2..end {
                    ...
                    let row_a = &dm[a];
                    let row_b = &dm[b];
                    let delta = (row_prev[b] + row_a[next])
                        - (row_prev[a] + row_b[next]);
                    if delta >= 0 {
                        continue;
                    }
            ...
            for segment_size in 1..=max_seg {
                ...
                for i in 1..len - segment_size {
                    ...
                    for insert_pos in 1..len {
                        ...
                        let delta_insert_segment = row_prev_ins[first]
                            + row_last[succ_ins]
                            - row_prev_ins[succ_ins];
    ...
    pub fn try_two_opt_star(
        ...
        let old_dist = distance_matrix[prev1][curr1] + distance_matrix[prev2][curr2];
        let new_dist = distance_matrix[prev1][curr2] + distance_matrix[prev2][curr1];
        ...
        let mut new_route1 = Vec::with_capacity(r1.len() + r2.len());
        let mut new_route2 = Vec::with_capacity(r1.len() + r2.len());
    ...
    pub fn try_2_2_cross_exchange(
        ...
        let demand_diff1 = demands[node2a] + demands[node2b] - demands[node1a] - demands[node1b];
        let demand_diff2 = demands[node1a] + demands[node1b] - demands[node2a] - demands[node2b];
        ...
        let row_n1a = &distance_matrix[node1a];
        let row_n1b = &distance_matrix[node1b];
        let row_n2a = &distance_matrix[node2a];
        let row_n2b = &distance_matrix[node2b];

        let delta1 = row_prev1[node2a] + row_n2a[node2b] + row_n2b[next1]
            - (row_prev1[node1a] + row_n1a[node1b] + row_n1b[next1]);
        let delta2 = row_prev2[node1a] + row_n1a[node1b] + row_n1b[next2]
            - (row_prev2[node2a] + row_n2a[node2b] + row_n2b[next2]);
    ...
    pub fn try_1_2_cross_exchange(
        ...
        let demand_diff1 = demands[node2a] + demands[node2b] - demands[node1];
        let demand_diff2 = demands[node1] - demands[node2a] - demands[node2b];
        ...
        let row_prev1 = &distance_matrix[prev1];
        let row_prev2 = &distance_matrix[prev2];
        let row_n1 = &distance_matrix[node1];
        let row_n2a = &distance_matrix[node2a];
        let row_n2b = &distance_matrix[node2b];

        let delta1 = row_prev1[node2a] + row_n2a[node2b] + row_n2b[next1]
            - (row_prev1[node1] + row_n1[next1]);
        let delta2 = row_prev2[node1] + row_n1[next2]
            - (row_prev2[node2a] + row_n2a[node2b] + row_n2b[next2]);
...
mod simple_solver {
    ...
    pub fn solve_sub_instance_simple(
        ...
        while let Some(node) = nodes.pop() {
            ...
            let base_attempts = 2;
            let scale_factor = (num_nodes as f32 / 1000.0).sqrt();
            (base_attempts as f32 * scale_factor).round() as usize
        }.clamp(2, 6);
    ...
    fn do_local_searches(
        ...
        let improvement_threshold = (best_distance as f32 * 0.00009).max(0.5) as i32; 
        let max_total_iterations = {
            let base_iterations = 150;
            let scale_factor = if num_nodes <= 1500 {
                1.0 + (num_nodes - 1000) as f32 / 2000.0  
            } else {
                1.167 + (num_nodes - 1500) as f32 / 2000.0   
            };
            ((base_iterations as f32 * scale_factor).round() as usize).clamp(150, 330)
        };
        
        let neighbor_count = {
            let base_neighbors = 30;
            let scale_factor = (num_nodes as f32 / 1500.0).powf(0.7);
            (base_neighbors as f32 * scale_factor).round() as usize
        }.clamp(28, 60);
        let pool = {
            let base_pool = 44;
            let scale_factor = (num_nodes as f32 / 1500.0).powf(0.8);
        ...
        for i in 1..num_nodes {
            ...
            unsafe {
                ...
                for &(_d, j) in pool_vec.iter() {
                    ...
                    let time_compatibility = (di_due - ri).min(dj - rj) as f32 / ((di_due - ri).max(dj - rj) + 1) as f32;
                    ...
                    if target_route_demand + demands[node] <= max_capacity {
                        if let Some((best_pos, _delta_cost)) = find_best_insertion_in_route(
                            ...
                            if best_routes[route1_idx].len() > pos1 && best_routes[route1_idx][pos1] == node {                            
                                ...
                                let delta_remove = row_prev1[next1]
                                    - row_prev1[node]
                                    - row_node[next1];
                                let delta_insert = row_prev2[node]
                                    + row_node[next2]
                                    - row_prev2[next2];
                   ...
                   route_demands[route2_idx] - demands[node2] + demands[node] <= max_capacity {                    
                    if best_routes[route1_idx].len() > pos1 && best_routes[route1_idx][pos1] == node &&
                       best_routes[route2_idx].len() > pos2 && best_routes[route2_idx][pos2] == node2 {
                        ...
                        let delta1 = row_prev1[node2]
                            + row_node2[next1]
                            - row_prev1[node]
                            - row_node[next1];
                        let delta2 = row_prev2[node]
                            + row_node[next2]
                            - row_prev2[node2]
                            - row_node2[next2];
                        ...
                        if is_route_time_feasible_fast(&swap_r1, distance_matrix, service_time, ready_times, due_times) &&
                           is_route_time_feasible_fast(&swap_r2, distance_matrix, service_time, ready_times, due_times) {
                            ...
                            if new_distance < best_distance {
                                ...
                                let temp_r1 = best_routes[route1_idx].clone();
                                let temp_r2 = best_routes[route2_idx].clone();
                                
                                best_routes[route1_idx] = swap_r1.clone();
                                best_routes[route2_idx] = swap_r2.clone();
