        ...
        while improved && iteration < max_iterations {
            ...
            for i in 1..best_route.len() - 2 {
                for j in i + 2..best_route.len() - 1 {
                    ...
                    if new_route.len() >= 3 && new_route[0] == 0 && new_route[new_route.len() - 1] == 0 &&
                       is_route_time_feasible_fast(&new_route, distance_matrix, service_time, ready_times, due_times) {
        ...
        while improved && iteration_count < max_total_iterations {
            ...
            for (_corr, node, node2) in &proximity_pairs {
                ...
                if target_route_demand + demands[node] <= max_capacity {
                    ...
                    if let Some((best_pos, _delta_cost)) = find_best_insertion_in_route(
                        ...
                        if new_routes[route1_idx].len() > pos1 && new_routes[route1_idx][pos1] == node {
                            ...
                            new_routes[route1_idx] = apply_size_filtered_local_search(&new_routes[route1_idx], distance_matrix, service_time, ready_times, due_times);
                            new_routes[route2_idx] = apply_size_filtered_local_search(&new_routes[route2_idx], distance_matrix, service_time, ready_times, due_times);

                            if new_routes[route1_idx].len() >= 3 && new_routes[route1_idx][0] == 0 && 
                               new_routes[route1_idx][new_routes[route1_idx].len() - 1] == 0 &&
                               new_routes[route2_idx].len() >= 3 && new_routes[route2_idx][0] == 0 && 
                               new_routes[route2_idx][new_routes[route2_idx].len() - 1] == 0 {

                                let old_d1 = utils::calculate_route_distance(&best_routes[route1_idx], distance_matrix);
                                let old_d2 = utils::calculate_route_distance(&best_routes[route2_idx], distance_matrix);
                                let new_d1 = utils::calculate_route_distance(&new_routes[route1_idx], distance_matrix);
                                let new_d2 = utils::calculate_route_distance(&new_routes[route2_idx], distance_matrix);
                ...
                if route_demands[route1_idx] - demands[node] + demands[node2] <= max_capacity &&
                   route_demands[route2_idx] - demands[node2] + demands[node] <= max_capacity {
                    ...
                    if new_routes[route1_idx].len() > pos1 && new_routes[route1_idx][pos1] == node &&
                       new_routes[route2_idx].len() > pos2 && new_routes[route2_idx][pos2] == node2 {
                        
                        new_routes[route1_idx][pos1] = node2;
                        new_routes[route2_idx][pos2] = node;
                        
                        if is_route_time_feasible_fast(&new_routes[route1_idx], distance_matrix, service_time, ready_times, due_times) &&
                           is_route_time_feasible_fast(&new_routes[route2_idx], distance_matrix, service_time, ready_times, due_times) {
            ...
            if !improved {
                ...
                for route_idx in 0..current_routes.len() {
                    ...
                    let improved_route = apply_size_filtered_local_search(route, distance_matrix, service_time, ready_times, due_times);
                    
                    if improved_route != *route && improved_route.len() >= 3 && 
                       improved_route[0] == 0 && improved_route[improved_route.len() - 1] == 0 {
                        ...
                        let new_d = utils::calculate_route_distance(&new_routes[route_idx], distance_matrix);
