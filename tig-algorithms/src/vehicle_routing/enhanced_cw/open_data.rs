...
#[inline]
fn create_initial_savings_list(challenge: &Challenge, raw_savings: &mut [Vec<f32>]) -> Vec<(u32, u8, u8)> {
    let num_nodes = challenge.difficulty.num_nodes;
...
pub fn postprocess_solution(
    ...
    let first_cost = calculate_solution_cost(&Solution { routes: solution.routes.clone() }, distance_matrix);
