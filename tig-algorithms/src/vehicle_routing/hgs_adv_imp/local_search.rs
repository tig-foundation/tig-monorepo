...
impl LocalSearch {
    pub fn new(data: Arc<Problem>, params: Params, _rng: &mut SmallRng) -> Self {
        ...
        // Compute avg distance per demand unit once for combined proxy scaling.
        // Use depot distances and demands as a representative sample (O(n) instead of O(n^2)).
        let mut sum_depot_dist: i64 = 0;
        let mut sum_demand: i64 = 0;
        for i in 1..n {
            sum_depot_dist += data.dm(0, i) as i64;
            sum_demand += data.nd(i).demand as i64;
        }
        // avg_dist_per_demand_unit: scale factor mapping demand diff to distance units.
        // Avoid division by zero; if total_demand is 0 (all equal demands), use 1.
        let avg_dist_per_demand: i64 = if sum_demand > 0 {
            sum_depot_dist / sum_demand
        } else {
            1
        };
        ...
        for i in 1..n {
            let di = data.nd(i).demand as i64;
            // Single-pass combined proxy: 5*dist + 3*|demand_diff|*avg_dist_per_demand
            // This keeps customers that are both close AND have similar demand,
            // rather than hard-filtering by demand first (which may discard close neighbors).
            let mut prox: Vec<(i64, usize)> = Vec::with_capacity(n.saturating_sub(2));
            for j in 1..n {
                if j == i { continue; }
                let dist_ij = data.dm(i, j) as i64;
                let demand_diff = (data.nd(j).demand as i64 - di).abs();
                let proxy = 5 * dist_ij + 3 * demand_diff * avg_dist_per_demand;
                prox.push((proxy, j));
            }
            prox.sort_by_key(|&(p, _)| p);
    ...
    pub fn run_inter_route(&mut self, r1: usize, pos1: usize, r2: usize, pos2: usize, apply: bool) -> Option<i64> {
        ...
        if x.id != 0 {
            ...
            // Send {U,X,Xnext}: allow when params flag is set OR source route has >= 6 clients
            // (removing 3 still leaves at least 2 clients, avoiding degenerate single-client routes)
            let allow_triple = self.params.allow_swap3 || (ru.nodes.len() >= 6);
            if x_next.id != 0 && allow_triple {
        ...
        if x.id != 0 {
            ...
            // Send {U,X,Xnext}: use same allow_triple flag computed above
            let allow_triple = self.params.allow_swap3 || (ru.nodes.len() >= 6);
            if x_next.id != 0 && allow_triple {
                let x2_next = &ru.nodes[pos1 + 3];
    ...
    pub fn run_swapstar(&mut self, r1: usize, pos1: usize, r2: usize, pos2: usize, apply: bool) -> Option<i64> {
        ...
        // Tighter lower bound for inserting v into r1 \ {u} using triangle inequality.
        // For any metric distance, dm(a,v)+dm(v,b) >= dm(a,b), so each insertion detour >= 0.
        // We also know the detour at the u-vacated position (pu,nu) is free to compute.
        // Use that as initial estimate. Before the O(n) scan, apply a triangle-ineq lb:
        // min_detour(v into r1\{u}) >= max(0, dm(pu,v) - d_pu_nu, dm(nu,v) - d_pu_nu) using
        // the single free arc at u's position. If lb_new_total + this lb > max, skip scan.
        let ins_v_at_u = data.dm(pu, v) + data.dm(v, nu) - d_pu_nu;
        // Scan all candidate positions for v: find the minimum insertion detour.
        // Early termination when detour reaches 0 (triangle-inequality lower bound for metric distances).
        let mut best_ins_v = ins_v_at_u;
        for t in 1..pos1 {
            ...
            if delta < best_ins_v {
                best_ins_v = delta;
                // Early termination: for metric distances, detour >= 0, so 0 is optimal lb.
                if best_ins_v <= 0 { break; }
            }
        }
        if best_ins_v > 0 {
        ...
        // best detour for inserting u into r2 \ {v}, with symmetric early termination
    ...
    /// Route consolidation: scan all routes with 1-2 clients, attempt to insert each client
    /// into the best position in any other route (respecting capacity). If all clients of a
    /// short route can be placed and the combined delta is negative, apply the moves and free
    /// the route. Returns true if any consolidation was applied.
    fn run_route_consolidation(&mut self) -> bool {
        let nr = self.routes.len();
        if nr < 2 { return false; }

        // Collect short routes (1 or 2 clients) sorted by cost descending
        let mut short_routes: Vec<usize> = (0..nr)
            .filter(|&rid| {
                let len = self.routes[rid].nodes.len();
                len == 3 || len == 4 // 1 or 2 clients (plus 2 depots)
            })
            .collect();

        if short_routes.is_empty() { return false; }

        // Sort by number of clients ascending (try single-client routes first - easiest to merge)
        short_routes.sort_by_key(|&rid| self.routes[rid].nodes.len());

        let max_cap = self.data.max_capacity;
        let mut any_applied = false;

        'outer: for &r_src in &short_routes {
            // Re-check that this route is still short (previous consolidations may have changed it)
            let src_len = self.routes[r_src].nodes.len();
            if src_len < 3 || src_len > 4 { continue; }

            let n_clients = src_len - 2; // 1 or 2

            // Collect client ids and their demands from this short route
            let mut clients: Vec<(usize, i32)> = Vec::with_capacity(n_clients);
            for pos in 1..src_len - 1 {
                let node = &self.routes[r_src].nodes[pos];
                clients.push((node.id, node.demand));
            }

            // Cost of the short route as-is
            let old_src_cost = self.routes[r_src].cost;

            // For each client, find best insertion in any other feasible route.
            // We need to verify all clients can be placed simultaneously.
            // Strategy: for each client in the short route, find the globally best
            // insertion position across all other routes (greedily, one client at a time).

            // We'll try to place all clients; if any can't be placed feasibly, skip.
            // Use a greedy approach: try placing clients in order of decreasing demand
            // (harder to place first).
            let mut placement: Vec<(usize, usize, i64)> = Vec::new(); // (r_dst, ins_pos, new_cost_dst)
            let mut total_delta: i64 = -old_src_cost; // removing src route saves old_src_cost

            // Simulate placements: track modified routes as (route_id -> new_load after insertion)
            // For simplicity, greedily find best position for each client independently.
            // This is correct when clients go to different routes; approximate when same route.
            // We track tentative loads per route.
            let mut tentative_loads: Vec<i32> = self.routes.iter().map(|r| r.load).collect();

            for (c_id, c_demand) in &clients {
                let c_seq1 = {
                    // Build singleton sequence for this client
                    Sequence::singleton(self.data.as_ref(), *c_id)
                };

                let mut best_ins_cost: i64 = i64::MAX;
                let mut best_r_dst = usize::MAX;
                let mut best_ins_pos = usize::MAX;
                let mut best_new_cost = i64::MAX;

                for r_dst in 0..self.routes.len() {
                    if r_dst == r_src { continue; }
                    let new_load = tentative_loads[r_dst] + c_demand;
                    if new_load > max_cap { continue; } // capacity constraint

                    let dst_len = self.routes[r_dst].nodes.len();
                    if dst_len < 2 { continue; }

                    for t in 1..dst_len {
                        let left = self.routes[r_dst].nodes[t - 1].seq0_i;
                        let right = self.routes[r_dst].nodes[t].seqi_n;
                        let new_cost_dst = Sequence::eval3(
                            self.data.as_ref(), &self.params, &left, &c_seq1, &right
                        );
                        if new_cost_dst < best_ins_cost {
                            best_ins_cost = new_cost_dst;
                            best_r_dst = r_dst;
                            best_ins_pos = t;
                            best_new_cost = new_cost_dst;
                        }
                    }
                }

                if best_r_dst == usize::MAX {
                    // Could not place this client anywhere feasibly
                    continue 'outer;
                }

                let old_dst_cost = self.routes[best_r_dst].cost;
                total_delta += best_new_cost - old_dst_cost;
                tentative_loads[best_r_dst] += c_demand;
                placement.push((best_r_dst, best_ins_pos, best_new_cost));
            }

            // Only apply if total delta is strictly negative (improvement)
            if total_delta >= 0 { continue; }

            // Apply: insert each client into its target route, then clear source route
            // We must apply in a safe order. Insert clients into target routes first,
            // then remove them from source route.

            // Collect client nodes from source route (positions 1..src_len-1)
            let mut client_nodes: Vec<Node> = Vec::with_capacity(n_clients);
            for _ in 0..n_clients {
                // Always remove position 1 (first client), since route shrinks
                client_nodes.push(self.routes[r_src].nodes.remove(1));
            }

            // Insert each client node into its target route
            for (idx, (r_dst, ins_pos, _)) in placement.iter().enumerate() {
                let node = client_nodes[idx].clone();
                // ins_pos was computed before any insertions in r_dst; if r_dst received
                // a previous client at a position <= ins_pos, the index shifted.
                // Count how many previous placements went into the same r_dst at pos <= ins_pos
                let offset = placement[..idx].iter()
                    .filter(|(rd, ip, _)| *rd == *r_dst && *ip <= *ins_pos)
                    .count();
                let adjusted_pos = ins_pos + offset;
                self.routes[*r_dst].nodes.insert(adjusted_pos, node);
            }

            self.nb_moves += 1;
            self.update_route(r_src);

            // Update all destination routes that received clients
            let mut updated: Vec<usize> = placement.iter().map(|(rd, _, _)| *rd).collect();
            updated.sort();
            updated.dedup();
            for r_dst in updated {
                self.update_route(r_dst);
            }

            // Recompute self.cost from scratch since multiple routes changed.
            self.cost = self.routes.iter().map(|r| r.cost).sum();
    ...
    /// Main search loop: iterative improvement with repeat-on-improvement and LNS injection
    /// after convergence to escape local optima.
    fn search(&mut self, rng: &mut SmallRng) {
        ...
        // After main loop converges, attempt route consolidation to free short routes.
        // If successful, re-run one more improvement pass to exploit the new structure.
        if self.run_route_consolidation() {
            let nb = self.nb_moves;
            for rid in 0..self.routes.len() {
                self.when_last_modified[rid] = nb;
            }
            let n = self.data.nb_nodes;
            for c in 0..n {
                self.when_last_tested[c] = 0;
            }
            let mut improved2 = true;
            let mut loop_id2 = 0usize;
            while improved2 || loop_id2 < 1 {
                improved2 = false;
                loop_id2 += 1;
                for idx in 0..self.loop_order_nodes.len() {
                    let c1 = self.loop_order_nodes[idx];
                    let last_tested = self.when_last_tested[c1];
                    self.when_last_tested[c1] = self.nb_moves;
                    if let Some(delta) = self.evaluate_and_apply_best_move_for_customer(c1, last_tested, 2) {
                        if delta < 0 { improved2 = true; }
                    }
                }
            }
        }
