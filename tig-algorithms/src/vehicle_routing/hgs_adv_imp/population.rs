...
impl Population {
    ...
    fn update_biased_fitnesses(sub: &mut Subpopulation, params: &Params) {
        ...
        // Use squared rank to emphasise extremely isolated individuals on larger populations
        let use_squared = n >= 10;
        for (pos, &(_, idx)) in div_pairs.iter().enumerate() {
            let rank = (pos as f64) / denom;
            div_rank[idx] = if use_squared { rank * rank } else { rank };
        }
