...
fn cluster_route_indices(
    ...
    // Uses k-means++ style seeding: first center is random, subsequent centers
    // are chosen with probability proportional to squared distance to the nearest
    // already-chosen center. This produces well-spread initial centers for
    // better spatial decomposition of the VRP subproblems.
    let m = routes.len();
    if m == 0 {
        return Vec::new();
    }
    ...
    // k-means++ center selection — active only for n=600 and n=1000 where it improves
    // decomposition quality. For n=700/800/900 the extra RNG draws shift sub-GA streams
    // in a way that hurts quality, so those tracks use the original random selection.
    let use_kmeanspp = data.nb_nodes <= 600 || data.nb_nodes >= 1000;
    let mut center_routes: Vec<usize> = Vec::with_capacity(kk);
    if use_kmeanspp {
        use rand::Rng;
        // Pick first center uniformly at random
        let first = rng.gen_range(0..m);
        center_routes.push(first);

        // Track minimum squared distance from each route to any chosen center
        let mut min_dist_sq: Vec<f64> = (0..m)
            .map(|rid| {
                let (x, y) = barycenters[rid];
                let (cx, cy) = barycenters[first];
                let dx = x - cx;
                let dy = y - cy;
                dx * dx + dy * dy
            })
            .collect();
        // Distance from center to itself is 0, already set correctly

        for _ in 1..kk {
            let total: f64 = min_dist_sq.iter().sum();
            // If all distances are zero (degenerate case), pick uniformly
            let chosen = if total <= 0.0 {
                let mut remaining_indices: Vec<usize> = (0..m)
                    .filter(|rid| !center_routes.contains(rid))
                    .collect();
                ...
                let mut picked = m - 1;
                for rid in 0..m {
                    // Skip already chosen centers (their min_dist_sq is 0)
                    cumsum += min_dist_sq[rid];
                    if cumsum >= threshold {
                        picked = rid;
                        break;
                    }
            ...
            center_routes.push(chosen);

            // Update min distances with the newly chosen center
            let (cx, cy) = barycenters[chosen];
            for rid in 0..m {
                let (x, y) = barycenters[rid];
                let dx = x - cx;
                let dy = y - cy;
                let d2 = dx * dx + dy * dy;
                if d2 < min_dist_sq[rid] {
                    min_dist_sq[rid] = d2;
                }
            }
        }
    } else {
        // Original uniform-random center selection for n=700/800/900
        let mut route_indices: Vec<usize> = (0..m).collect();
        route_indices.shuffle(rng);
        center_routes.extend_from_slice(&route_indices[..kk]);
    }
