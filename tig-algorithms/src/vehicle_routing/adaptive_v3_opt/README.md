...
* **Algorithm Name:** adaptive_v3_opt
...
**This solver is fully compatible with TIG v0.0.5+ multi-track submission requirements.**

### Key Changes in v0.0.5
- Benchmarkers no longer choose tracks; a random track is assigned at precommit
- `submit-precommit` now requires hyperparameters and num_bundles as a **map** with one entry per `track_id`
- Once precommit is confirmed, a random track is selected and the corresponding settings are used

### Multi-Track Support
This solver handles all vehicle_routing track sizes (different node counts) through adaptive parameter tuning:

```rust
// Generate settings for all tracks at once
let track_settings = generate_all_track_settings(
    1,    // exploration_level (0-6)
    10    // base num_bundles
);

// The Config::preset() function automatically adapts parameters based on problem size
// making this solver well-suited for random track selection
```

### Using the New Submission Format
Helper functions are provided to generate track settings:

- `generate_track_hyperparameters(n_nodes, exploration_level)` - Parameters for a specific track
- `generate_all_track_settings(exploration_level, num_bundles)` - Settings for all common tracks
- `print_track_settings(track_settings)` - Debug output for track settings
- `example_v0_0_5_submission()` - Complete example of the new format

See the function documentation in `mod.rs` for detailed usage.
