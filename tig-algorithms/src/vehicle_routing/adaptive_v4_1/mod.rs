...
// Inline module: encoder (InstanceEncoder stub)
mod encoder {
    use serde::{Deserialize, Serialize};
    ...
    impl InstanceEncoder {
        ...
        pub fn encode(&self, _instance: &serde_json::Value) -> Vec<f32> {
            vec![0.0f32; self.dim]
        }
    }
}

// Inline module: embedding_cache (LRU + TTL)
mod embedding_cache {
    use std::collections::{HashMap, VecDeque};
    use std::time::{SystemTime, UNIX_EPOCH, Duration};

    pub struct EmbeddingCache<V> {
    ...
    impl<V> EmbeddingCache<V> {
        ...
        pub fn get(&mut self, key: &u64) -> Option<&V> {
            if let Some((v, ts)) = self.map.get(key) {
                let now = Self::now_secs();
                if now.saturating_sub(*ts) > self.ttl_seconds {
                    return None;
                }
                self.order.retain(|k| k != key);
                self.order.push_front(*key);
                return self.map.get(key).map(|(val, _)| val);
            }
            None
        }
...
mod repair {
    ...
    impl FeasibilityPredictor {
        ...
        pub fn predict(&self, _state: &Value) -> f32 { 0.0 }

        pub fn predict_metrics(&self, _distance: i32, _tw_violation: i32, _load_excess: i32, _nb_nodes: usize) -> f32 {
            0.0
        }
    }
    ...
    pub enum RepairAction {
        ReinsertCustomer { customer_id: usize, to_route: usize },
        SwapSegments { route_a: usize, route_b: usize },
        MergeRoutes { route_a: usize, route_b: usize },
        DelayService { customer_id: usize, delay_minutes: i32 },
    }

    #[derive(Debug, Clone)]
    pub struct RepairOrchestrator {
        pub predictor: FeasibilityPredictor,
    }

    impl RepairOrchestrator {
        pub fn new(predictor: FeasibilityPredictor) -> Self {
            Self { predictor }
        }

        pub fn predict(&self, state: &Value) -> f32 {
            self.predictor.predict(state)
        }

        pub fn predict_metrics(&self, distance: i32, tw_violation: i32, load_excess: i32, nb_nodes: usize) -> f32 {
            self.predictor.predict_metrics(distance, tw_violation, load_excess, nb_nodes)
        }

        pub fn propose_repair(&self, state: &Value) -> Option<RepairAction> {
            let prob = self.predictor.predict(state);
            if prob > 0.85 {
                None
            } else {
                None
        ...
        pub fn apply_repair(&self, _action: &RepairAction) -> bool {
            true
        }
    }
}

// Inline module: multi_agent (MA-ADP stub)
mod multi_agent {
    use serde_json::Value;
    ...
    impl MultiAgentADP {
        ...
        pub fn evaluate(&self, _state: &Value) -> (Vec<f32>, f32) {
            (Vec::new(), 0.0)
        }

        pub fn update_value(&mut self, _route_id: usize, _observed_cost: f32, _pred_value: f32) {
        }
    }
}
pub struct VrpData {
...
impl Constructive {
    ...
    pub fn build_routes_fast(data: &Problem, _rng: &mut SmallRng) -> Vec<Vec<usize>> {
        ...
        for start in 1..n {
            ...
            // start a new route at the first unused client
            used[start] = true;
            ...
            let mut curr = start;
            let mut load = data.demands[start];
            // arrive time after serving start
            let mut curr_time = data.start_tw[start].max(data.dm(0, start)) + data.service_times[start];
            ...
            loop {
                ...
                for c in 1..n {
                    if used[c] { continue; }
                    let dmd = data.dm(curr, c);
                    if load + data.demands[c] > data.max_capacity { continue; }
                    let arrive = (curr_time + dmd).max(data.start_tw[c]);
                    if arrive > data.end_tw[c] { continue; }
                    if dmd < best_d {
                        best_d = dmd;
                        best = c;
                    }
                ...
                used[best] = true;
                route.push(best);
                load += data.demands[best];
                curr_time = (curr_time + data.dm(curr, best)).max(data.start_tw[best]) + data.service_times[best];
                curr = best;
            }
...
impl<'a> Genetic<'a> {
    ...
    pub fn run(
        ...
        // Fast-mode early exit: build cheap routes and do a tiny local improvement.
        ...
        if let Some(best) = self.population.best_feasible() {
            ...
            let mut chosen = if best_after.tw_violation == 0
            ...
            // Self-Healing Repair Mechanism (SHRM) hook: attempt repairs when infeasible.
            // Use metrics-based predictor to avoid allocating JSON unless necessary.
            let mut repairor = repair::RepairOrchestrator::new(repair::FeasibilityPredictor::new());
            if chosen.tw_violation != 0 || chosen.load_excess != 0 {
                for _attempt in 0..3 {
                    let prob = repairor.predict_metrics(chosen.distance, chosen.tw_violation, chosen.load_excess, self.data.nb_nodes);
                    if prob <= 0.85 { break; }

                    // only build full state if predictor signals a repair should be proposed
                    let state = serde_json::json!({
                        "routes": chosen.routes,
                        "distance": chosen.distance,
                        "tw_violation": chosen.tw_violation,
                        "load_excess": chosen.load_excess,
                        "nb_nodes": self.data.nb_nodes,
                    });

                    if let Some(_action) = repairor.propose_repair(&state) {
                        // Best-effort: call existing repair routine which runs local search and
                        // tries to add repaired individuals to the population. If successful,
                        // replace `chosen` with the new feasible best.
                        self.repair_and_maybe_add(&mut ls, rng);
                        if let Some(new_best) = self.population.best_feasible() {
                            chosen = new_best;
                            if chosen.tw_violation == 0 && chosen.load_excess == 0 { break; }
                        }
                    } else {
                        break;
                    }
    ...
    pub fn fast_local_improve(&self, routes: &mut Vec<Vec<usize>>, _rng: &mut SmallRng) {
        ...
        // 1) Bounded 2-opt passes (very local)
        let max_passes = 2usize;
        for r_idx in 0..routes.len() {
            let route = &mut routes[r_idx];
            if route.len() < 4 { continue; }
            for _pass in 0..max_passes {
                let mut improved = false;
                let mut i = 1usize;
                while i + 2 < route.len() {
                    let j_max = (i + 10).min(route.len() - 2);
                    let mut j = i + 1;
                    while j <= j_max {
                        let prev = route[i - 1];
                        let a = route[i];
                        let b = route[j];
                        let next = route[j + 1];
                        let delta = (data.dm(prev, b) + data.dm(a, next)) - (data.dm(prev, a) + data.dm(b, next));
                        if delta < 0 {
                            route[i..=j].reverse();
                            improved = true;
                            break;
        ...
        // 2) Tiny relocate-to-route-start moves using cheap lower-bound
        'outer: loop {
            let mut moved = false;
            let route_firsts: Vec<Option<usize>> = routes.iter().map(|r| if r.len() > 2 { Some(r[1]) } else { None }).collect();

            for src in 0..routes.len() {
                let rlen = routes[src].len();
                if rlen <= 2 { continue; }
                let mut p = 1usize;
                while p + 1 < routes[src].len() {
                    let node = routes[src][p];
                    let prev = routes[src][p - 1];
                    let next = routes[src][p + 1];
                    let hole = data.dm(prev, next) - data.dm(prev, node) - data.dm(node, next);

                    for tgt in 0..routes.len() {
                        if tgt == src { continue; }
                        if routes[tgt].len() <= 2 { continue; }
                        if let Some(first_t) = route_firsts[tgt] {
                            let insert_cost = data.dm(0, node) + data.dm(node, first_t) - data.dm(0, first_t);
                            if hole + insert_cost < 0 {
                                // perform cheap move: remove node from src and insert at position 1 of tgt
                                let _val = routes[src].remove(p);
                                routes[tgt].insert(1, node);
                                moved = true;
                                break;
                            }
                        }
