...
mod simple_solver {
    ...
    fn do_local_searches(
        ...
        let mut proximity_pairs: Vec<(f64, usize, usize)> = Vec::with_capacity(num_nodes * k_neighbors);
        for i in 1..num_nodes {                        
            let mut best1: Option<(f64, usize)> = None;
            let mut best2: Option<(f64, usize)> = None;
        ...
        while improved && iteration_count < max_total_iterations {
            ...
            for (_corr, node, node2) in &proximity_pairs {
                ...
                if target_route_demand + demands[node] <= max_capacity {
                    if let Some((best_pos, _delta_cost)) = find_best_insertion_in_route(
                        ...
                        let mut r1 = best_routes[route1_idx].clone();
                        let mut r2 = best_routes[route2_idx].clone();

                        if r1.len() > pos1 && r1[pos1] == node {
                            r1.remove(pos1);
                            r2.insert(best_pos, node);

                            r1 = apply_size_filtered_local_search(&r1, distance_matrix, service_time, ready_times, due_times);
                            r2 = apply_size_filtered_local_search(&r2, distance_matrix, service_time, ready_times, due_times);
                ...
                if route_demands[route1_idx] - demands[node] + demands[node2] <= max_capacity &&
                   route_demands[route2_idx] - demands[node2] + demands[node] <= max_capacity {                    
                    ...
                    if r1.len() > pos1 && r1[pos1] == node &&
                       r2.len() > pos2 && r2[pos2] == node2 {
                        
                        r1[pos1] = node2;
                        r2[pos2] = node;
                        
                        if is_route_time_feasible_fast(&r1, distance_matrix, service_time, ready_times, due_times) &&
                           is_route_time_feasible_fast(&r2, distance_matrix, service_time, ready_times, due_times) {
            ...
            if !improved {
                for route_idx in 0..best_routes.len() {                    
                    let current_route = best_routes[route_idx].clone();

                    let improved_route = apply_size_filtered_local_search(&current_route, distance_matrix, service_time, ready_times, due_times);
                    
                    if improved_route != current_route && improved_route.len() >= 3 && 
