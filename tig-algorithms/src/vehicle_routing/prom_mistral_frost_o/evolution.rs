...
impl<'a> Evolution<'a> {
    ...
    pub fn run(
        ...
        // Reserve 10% of runtime for fallback LS
        let time_budget = t0.elapsed().as_secs_f64() + 255.0;
        let fallback_time = time_budget * 0.1;
        let main_time = time_budget - fallback_time;

        while it_noimprov < self.params.max_it_noimprov && it_total < self.params.max_it_total {
            // Check if we're approaching the fallback time
            ...
            if cur.better_than(best_metric) {
                ...
                if self.last_feasible_cost < best_saved_feasible_cost {
        ...
        // Fallback LS phase with remaining time
        if let Some(best) = self.population.best_feasible() {
            let mut best_routes = best.routes.clone();
            let remaining_time = (time_budget - t0.elapsed().as_secs_f64()).max(0.0);
            let ls_iterations = if remaining_time > 0.0 {
                (remaining_time * 100.0).min(500.0) as usize
            } else {
                0
            };
