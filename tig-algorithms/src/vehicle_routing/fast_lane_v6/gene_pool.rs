...
impl<'a> GenePool<'a> {
    ...
    pub fn add(&mut self, ind: Individual, params: &Config) {
        ...
        Self::order_cost_insert(sub, new_idx);
        Self::update_biased_fitnesses(sub, params);
    ...
    fn prox_add(sub: &mut Subpopulation, data: &Instance, new_idx: usize) {
        ...
        let edge_cache = Self::build_broken_pairs_edge_cache(data, &sub.indivs[new_idx]);

        for i in 0..new_idx {
            let d = Self::broken_pairs_distance_from_cache(&edge_cache, &sub.indivs[i]);
    ...
    fn build_broken_pairs_edge_cache(data: &Instance, indiv_a: &Individual) -> Vec<usize> {
        let n_clients = data.nb_nodes - 1;
        let pred_a = &indiv_a.pred[1..];
        let succ_a = &indiv_a.succ[1..];
        let mut cache = Vec::with_capacity(n_clients);

        for j in 0..n_clients {
            cache.push((succ_a[j] << 1) | ((pred_a[j] == 0) as usize));
        }

        cache
    }

    fn broken_pairs_distance_from_cache(cache_a: &[usize], indiv_b: &Individual) -> f64 {
        let pred_b = &indiv_b.pred[1..];
        let succ_b = &indiv_b.succ[1..];

        let mut differences = 0usize;
        for j in 0..cache_a.len() {
            let edge_a = cache_a[j];
            let succ_a = edge_a >> 1;
            let pred_bj = pred_b[j];
            let succ_bj = succ_b[j];

            if succ_a != succ_bj && succ_a != pred_bj {
                differences += 1;
            }
            if (edge_a & 1) != 0 && pred_bj != 0 && succ_bj != 0 {
                differences += 1;
            }
        }

        (differences as f64) / (cache_a.len() as f64)
    }
    ...
    fn order_cost_insert(sub: &mut Subpopulation, idx: usize) {
        let cost = sub.indivs[idx].cost;
        let pos = sub.order_cost.partition_point(|&i| sub.indivs[i].cost <= cost);
        sub.order_cost.insert(pos, idx);
    }
    ...
    fn update_biased_fitnesses(sub: &mut Subpopulation, params: &Config) {
        ...
        let nb_close_f = nb_close as f64;

        let mut div_pairs: Vec<(f64, usize)> = Vec::with_capacity(n);
        for i in 0..n {
            ...
            div_pairs.push((-(sum / nb_close_f), i));
        }
        ...
        for (pos, &idx) in sub.order_cost.iter().enumerate() {
            cost_pos[idx] = pos;
            sub.biased_fitness[idx] = (pos as f64) / denom;
        }

        let scale = 1.0 - (params.nb_elite as f64) / (n as f64);
        for (pos, &(_, idx)) in div_pairs.iter().enumerate() {
            if cost_pos[idx] >= params.nb_elite {
                sub.biased_fitness[idx] += scale * ((pos as f64) / denom);
            }
        }
    }
