...
mod utils {
    ...
    pub fn find_best_insertion_pair_in_route(
        ...
        unsafe {
            ...
            let mut pos = 1usize;
            while pos < len {
                let prev = *r.get_unchecked(pos - 1);
                ...
                let mut candidate: Vec<usize> = Vec::with_capacity(len + 2);
                candidate.extend_from_slice(&route[..pos]);
                candidate.push(node_a);
                candidate.push(node_b);
                candidate.extend_from_slice(&route[pos..]);

                if is_route_time_feasible_fast(&candidate, dm, service_time, ready_times, due_times) {
                    if delta < best_delta {
                        best_delta = delta;
                        best_pos = Some(pos);
                    }
...
mod simple_solver {
        ...
        
        {
            ...
            if !leftover.is_empty() && !routes.is_empty() {
                ...
                while progress && !leftover.is_empty() && iter_count < max_global_iters {
                    iter_count += 1;
                    progress = false;
                    let mut best_choice: Option<(usize, usize, usize, i32)> = None;
                    for &n in &leftover {
                        for (r_idx, r) in routes.iter().enumerate() {
                            if route_demands_pre[r_idx] + demands[n] > max_capacity { continue; }
                            if let Some((pos, delta)) = find_best_insertion_in_route(
                                ...
                                match best_choice {
                                    None => best_choice = Some((n, r_idx, pos, delta)),
                                    Some((_bn, _ri, _bp, bdelta)) => {
                                        if delta < bdelta {
                                            best_choice = Some((n, r_idx, pos, delta));
                                        }
                                    }
                                }
                            }
                    ...
                    if let Some((n, r_idx, pos, _delta)) = best_choice {
                        routes[r_idx].insert(pos, n);
                        route_demands_pre[r_idx] += demands[n];
                        remaining[n] = false;
    ...
    fn do_local_searches(
        ...
        let improvement_threshold = (best_distance as f32 * 0.00009).max(0.1) as i32; 
        ...
        let neighbor_count = {
            ...
            let scale_factor = (num_nodes as f32 / 1500.0).powf(0.5);
        ...
        while improved && iteration_count < max_total_iterations {
            ...
            if !improved {
                for (_corr, node, node2) in &proximity_pairs {
                    ...
                    let add_dem = demands[node] + demands[node_b];
                    ...
                    if let Some((best_pos2, delta_insert2)) = find_best_insertion_pair_in_route(
                        ...
                        if is_route_time_feasible_fast(&cand_r1, distance_matrix, service_time, ready_times, due_times) &&
                           is_route_time_feasible_fast(&cand_r2, distance_matrix, service_time, ready_times, due_times) {
                            ...
                            let old_d1 = route_distances[route1_idx];
                            let old_d2 = route_distances[route2_idx];
                            let new_d1 = utils::calculate_route_distance(&cand_r1, distance_matrix);
                            let new_d2 = utils::calculate_route_distance(&cand_r2, distance_matrix);
