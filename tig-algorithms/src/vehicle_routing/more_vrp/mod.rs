...
impl<'a> Genetic<'a> {
    ...
    pub fn generate_crossover_individual(&mut self, rng: &mut SmallRng, ls: &mut LocalSearch) {
        ...
        if self.data.nb_nodes < 1000 && rng.gen_ratio(1, 8) {
            let n = child_tour.len();
            if n > 3 {
                let i = rng.gen_range(0..n-2);
                let j = rng.gen_range(i+2..n);
                child_tour[i..=j].reverse();
            }
        }
    ...
    pub fn run(
        ...
        let diversity_boost = if self.data.nb_nodes < 1000 { 2 } else { 0 };
...
impl TigLoader {
    pub fn load(challenge: &Challenge) -> Problem {
        let nb_nodes = challenge.difficulty.num_nodes;
