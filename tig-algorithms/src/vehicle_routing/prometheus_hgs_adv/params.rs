...
pub struct Params {
    ...
    /// Number of cross-sector route-subset polish rounds run after the
    /// reverse-mode phase schedule. Each round re-solves one incumbent route
    /// together with its spatially/temporally closest neighbour routes
    /// (regardless of angular sector) as a small sub-VRPTW.
    /// 0 disables the polish pass.
    pub polish_rounds: usize,

    /// Iteration budget of the final full-instance genetic polish run after
    /// the reverse-mode phase schedule and route-subset polish (0 disables).
    /// The GA is warm-started from the incumbent (inserted immediately via
    /// `seed_immediate`) so SREX/OX recombination, SISR/ejection perturbations
    /// and consensus compression act on the complete instance, moving
    /// customers across sector boundaries no sub-instance exposes jointly.
    pub final_ga_iters: usize,

    /// Insert the reserved seed individual at the first GA iteration instead
    /// of waiting for the mid-run trigger. Used by the final master polish,
    /// where the incumbent should anchor the population from the start.
    pub seed_immediate: bool,
