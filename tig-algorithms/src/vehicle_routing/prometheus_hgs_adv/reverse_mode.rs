...
fn cluster_route_indices(
    ...
    // Angular sweep around the depot: order routes by the polar angle of
    // their customer barycenter (with a seed-driven random rotation so
    // successive phases cut the plane at different sector boundaries), then
    // split the circular order into `kk` contiguous groups balanced by
    // client count. Routes with capacity 200 and demands <= 35 are short and
    // geographically compact, so contiguous angular sectors yield tiny,
    // spatially coherent sub-instances.
    let (dx0, dy0) = (
        data.node_positions[0].0 as f64,
        data.node_positions[0].1 as f64,
    );
    let rotation = rng.gen_range(0.0..std::f64::consts::TAU);
    let mut angle_order: Vec<(f64, usize)> = routes
        .iter()
        .enumerate()
        .map(|(rid, r)| {
            let (bx, by) = route_barycenter(data, r);
            let mut a = (by - dy0).atan2(bx - dx0) + rotation;
            if a < 0.0 {
                a += std::f64::consts::TAU;
            } else if a >= std::f64::consts::TAU {
                a -= std::f64::consts::TAU;
            }
            (a, rid)
        })
        .collect();
    angle_order.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap_or(std::cmp::Ordering::Equal));

    let route_clients: Vec<usize> = routes.iter().map(|r| r.len().saturating_sub(2)).collect();
    let total_clients: usize = route_clients.iter().sum();

    let mut clusters: Vec<Vec<usize>> = vec![Vec::new(); kk];
    let mut acc = 0usize;
    for &(_, rid) in &angle_order {
        // Assign each route to the sector its cumulative client-count
        // midpoint falls into, keeping cluster sizes balanced.
        let mid = acc + route_clients[rid] / 2;
        let mut c = if total_clients > 0 {
            (mid * kk) / total_clients
        } else {
            0
        };
        if c >= kk {
        ...
        clusters[c].push(rid);
        acc += route_clients[rid];
    }

    // Drop empty sectors (possible when a large route spans several buckets).
...
pub(super) fn solve_reversed_mode(
    ...
    for (phase_idx, &sub_exploration_level) in exploration_schedule.iter().enumerate() {
        ...
        for (cid, route_group) in clusters.iter().enumerate() {
            ...
            let (sub_problem, local_to_global) = build_subproblem(
                data_arc.as_ref(),
                &clients,
                // Fleet size = cluster's route count + 1 spare vehicle,
                // but only when a globally unused vehicle actually exists.
                ((route_group.len() as i32) + residual_free_routes.clamp(0, 1)).max(1) as usize,
            );
    ...
    // ---- Cross-sector route-subset polish ----------------------------------
    // The angular decomposition only re-optimizes routes jointly when they
    // fall in the same sector; routes that are adjacent across a sector
    // boundary only ever interact through the global LS pass. This final pass
    // round-robins over the incumbent's routes and re-solves each together
    // with its spatially/temporally closest neighbour routes (regardless of
    // sector) as a small sub-VRPTW, keeping strict improvements only.
    const POLISH_MAX_ROUTES: usize = 5;
    const POLISH_MAX_CUSTOMERS: usize = 40;
    const POLISH_SUB_MAX_IT: usize = 300;
    const POLISH_SUB_MAX_IT_NOIMPROV: usize = 80;
    const POLISH_TW_GAP_WEIGHT: f64 = 0.3;

    let mut polish_improvement: i64 = 0;
    for round in 0..params.polish_rounds {
        let routes: Vec<Vec<usize>> = incumbent
            .routes
            .iter()
            .filter(|r| r.len() > 2)
            .cloned()
        ...
        // Route centroids and customer time-window spans.
        let mut centroids = Vec::with_capacity(n_routes);
        let mut spans = Vec::with_capacity(n_routes);
        for r in &routes {
            centroids.push(route_barycenter(data_arc.as_ref(), r));
            let mut smin = i64::MAX;
            let mut smax = i64::MIN;
            for &id in r.iter().skip(1).take(r.len() - 2) {
                let nd = data_arc.nd(id);
        ...
        // Round-robin seed route; rank the others by centroid distance plus
        // temporal gap so the subset is coherent in space and time.
        let seed_rid = round % n_routes;
        let mut ranked: Vec<(f64, usize)> = (0..n_routes)
            .filter(|&rid| rid != seed_rid)
            .map(|rid| {
                let dx = centroids[rid].0 - centroids[seed_rid].0;
                let dy = centroids[rid].1 - centroids[seed_rid].1;
                let gap = (spans[rid].0.max(spans[seed_rid].0)
                    - spans[rid].1.min(spans[seed_rid].1))
                    .max(0) as f64;
                ((dx * dx + dy * dy).sqrt() + POLISH_TW_GAP_WEIGHT * gap, rid)
            })
            .collect();
        ...
        for &(_, rid) in &ranked {
            if selected.len() >= POLISH_MAX_ROUTES {
                break;
            }
            let add = routes[rid].len() - 2;
            if selected.len() >= 2 && n_cust + add > POLISH_MAX_CUSTOMERS {
                break;
            }
            selected.push(rid);
            n_cust += add;
        ...
        let mut clients: Vec<usize> = Vec::with_capacity(n_cust);
        for &rid in &selected {
            clients.extend_from_slice(&routes[rid][1..routes[rid].len() - 1]);
        }
        // One spare vehicle, but only if the master fleet actually has one.
        let residual_free = ((data_arc.nb_vehicles as i32) - (n_routes as i32)).clamp(0, 1);
        let (sub_problem, local_to_global) = build_subproblem(
            data_arc.as_ref(),
            &clients,
            ((selected.len() as i32) + residual_free).max(1) as usize,
        );
        if sub_problem.nb_nodes < 5 {
            continue;
        }
        ...
        params_sub.max_it_total = POLISH_SUB_MAX_IT;
        params_sub.max_it_noimprov = POLISH_SUB_MAX_IT_NOIMPROV;
        params_sub.decomp_nb_phases = 0;
        ...
        if seed_sub_ind.load_excess != 0 || seed_sub_ind.tw_violation != 0 {
            continue; // defensive: seed extracted from a feasible incumbent
        }
        let mut sub_ga = Genetic::new(sub_problem, params_sub);
        let Some((sub_routes, _)) = sub_ga.run(&mut rng, t0, None, Some(&seed_sub_routes)) else {
            continue;
        };
        let cand_sub_ind = Individual::new_from_routes(
            sub_ga.root_data.as_ref(),
        ...
        let mut candidate_routes: Vec<Vec<usize>> = Vec::with_capacity(n_routes);
        for (rid, r) in routes.iter().enumerate() {
            if !in_subset[rid] {
                candidate_routes.push(r.clone());
            }
        }
        for r in &sub_routes {
            if r.len() > 2 {
                candidate_routes.push(r.iter().map(|&lid| local_to_global[lid]).collect());
            }
        }
        let candidate_ind =
            Individual::new_from_routes(data_arc.as_ref(), &params, candidate_routes.clone());
        if candidate_ind.load_excess == 0
            && candidate_ind.tw_violation == 0
            && candidate_ind.nb_routes <= data_arc.nb_vehicles
        {
            if candidate_ind.cost < incumbent.ind.cost {
                polish_improvement += incumbent.ind.cost - candidate_ind.cost;
                trace!(
                    "----- POLISH ROUND {}/{}: IMPROVED MASTER COST {} -> {}",
                    round + 1,
            ...
            promote_if_improved(save_solution, &mut incumbent, &candidate_routes, &candidate_ind);
        }
    }
    if params.polish_rounds > 0 {
        trace!(
            "----- CROSS-SECTOR POLISH DONE: {} ROUNDS, TOTAL IMPROVEMENT = {}",
            params.polish_rounds,
            polish_improvement
        );
    }

    // ---- Final full-instance genetic polish ---------------------------------
    // The phase schedule and the route-subset polish both improve the
    // incumbent through bounded sub-instances; in reversed mode the complete
    // instance is never searched globally. This last stage runs a short HGS
    // on the full instance, warm-started with the incumbent (inserted at
    // iteration 0 via seed_immediate), so master-level SREX/OX recombination,
    // SISR/ejection perturbations and consensus compression can move
    // customers across sector boundaries that no sub-instance exposed
    // jointly. Saves are filtered to strict improvements over the incumbent,
    // so a deadline kill during this stage can never regress the result.
    if params.final_ga_iters > 0 && data_arc.nb_nodes >= 5 {
        let full_data: Problem = data_arc.as_ref().clone();
        let mut params_final = Params::preset(3, &full_data);
        params_final.max_it_total = params.final_ga_iters;
        params_final.max_it_noimprov = (params.final_ga_iters / 3).max(30);
        params_final.mu_start = 4;
        params_final.decomp_nb_phases = 0;
        params_final.seed_immediate = true;
        params_final.nb_it_traces = 1000;
        params_final.display_traces = params.display_traces;

        let seed_routes: Vec<Vec<usize>> = incumbent
            .routes
            .iter()
            .filter(|r| r.len() > 2)
            .cloned()
        ...
        let best_saved_cost = std::cell::Cell::new(incumbent.ind.cost);
        let filtered_save = |sol: &Solution| -> Result<()> {
            let ind = Individual::new_from_routes(data_arc.as_ref(), &params, sol.routes.clone());
            if ind.load_excess == 0
                && ind.tw_violation == 0
                && ind.nb_routes <= data_arc.nb_vehicles
                && ind.cost < best_saved_cost.get()
            {
                best_saved_cost.set(ind.cost);
                if let Some(save) = save_solution {
                    return save(sol);
                }
            }
        ...
        let ga_save: Option<&dyn Fn(&Solution) -> Result<()>> =
            save_solution.map(|_| &filtered_save as &dyn Fn(&Solution) -> Result<()>);

        trace!(
            "----- FINAL MASTER GA: {} ITERATIONS ON FULL INSTANCE, SEED COST = {}",
            params.final_ga_iters,
            incumbent.ind.cost
        );
        let mut final_ga = Genetic::new(full_data, params_final);
        if let Some((routes, _)) = final_ga.run(&mut rng, t0, ga_save, Some(&seed_routes)) {
            let ind = Individual::new_from_routes(data_arc.as_ref(), &params, routes.clone());
            if ind.load_excess == 0
                && ind.tw_violation == 0
                && ind.nb_routes <= data_arc.nb_vehicles
            {
                if ind.cost < incumbent.ind.cost {
                    trace!(
                        "----- FINAL MASTER GA: IMPROVED MASTER COST {} -> {}",
                        incumbent.ind.cost,
                        ind.cost
                    );
                }
                promote_if_improved(save_solution, &mut incumbent, &routes, &ind);
            }
        }
    }
