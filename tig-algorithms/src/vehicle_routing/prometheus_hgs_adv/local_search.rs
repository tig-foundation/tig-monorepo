...
pub struct LocalSearch {
    ...
    ins_cache: Vec<Vec<InsCache>>,        // [customer][route] top-3 insertion gaps (SWAP* filters)
    cache_epoch: u64,                     // bumped per run_from_routes; nb_moves stamps reset then
}

/// Cached three cheapest distance-only insertion gaps of one customer into one
/// route, keyed by (epoch, route stamp). At most two gaps are adjacent to any
/// node removed from the route, so the best non-adjacent gap is always among
/// the top-3 - the SWAP* filter values derived from it stay exact.
#[derive(Clone, Copy)]
struct InsCache {
    epoch: u64,
    stamp: usize,
    best: [(i32, usize); 3], // (detour delta, gap position t), sentinel-padded
}

const INS_SENTINEL: (i32, usize) = (i32::MAX / 4, usize::MAX);

impl InsCache {
    const INVALID: Self = Self { epoch: 0, stamp: 0, best: [INS_SENTINEL; 3] };
}
...
/// Safety bound on heap pops per SMD pass, per node (plus a flat base),
/// guarding against rare zero-delta move cycles under deterioration credit.
const SMD_POPS_PER_NODE: usize = 128;
const SMD_POPS_BASE: usize = 2_000;

/// Static Move Descriptor: a cached candidate move with its evaluated delta
/// and the move-counter timestamp at evaluation time. Entries whose routes
/// were modified after `stamp` are stale and lazily recomputed on pop.
#[derive(Clone, Copy)]
struct SmdEntry {
    delta: i64,
    stamp: usize,
...
impl PartialEq for SmdEntry {
    fn eq(&self, other: &Self) -> bool {
        self.cmp(other) == std::cmp::Ordering::Equal
    }
}
impl Eq for SmdEntry {}
impl PartialOrd for SmdEntry {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
impl Ord for SmdEntry {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        // BinaryHeap is a max-heap: order so the smallest delta pops first,
        // with deterministic tie-breaking on customer id then timestamp.
        other
            .delta
            .cmp(&self.delta)
            .then_with(|| other.customer.cmp(&self.customer))
...
impl LocalSearch {
    pub fn new(data: Arc<Problem>, params: Params, _rng: &mut SmallRng) -> Self {
        ...
        Self {
            ...
            nb_moves: 0,
            move_credit: 0,
            ins_cache: vec![Vec::new(); n],
            cache_epoch: 1,
        }
    }

    /// Top-3 cheapest distance-only insertion gaps of customer `v` into route
    /// `r` (gap `t` = between nodes `t-1` and `t`), recomputed lazily when the
    /// route's modification stamp or the run epoch changed. `v` must not
    /// currently be on route `r`.
    fn best_insertions(&mut self, v: usize, r: usize) -> [(i32, usize); 3] {
        debug_assert!(self.node_route[v] != r);
        let stamp = self.when_last_modified[r];
        let nb_routes = self.routes.len();
        let row = &mut self.ins_cache[v];
        if row.len() < nb_routes {
            row.resize(nb_routes, InsCache::INVALID);
        }
        let entry = row[r];
        if entry.epoch == self.cache_epoch && entry.stamp == stamp {
            return entry.best;
        }

        let data = self.data.as_ref();
        let nodes = &self.routes[r].nodes;
        let mut best = [INS_SENTINEL; 3];
        for t in 1..nodes.len() {
            let a = nodes[t - 1].id;
            let b = nodes[t].id;
            let delta = data.dm(a, v) + data.dm(v, b) - nodes[t - 1].dist_to_succ;
            if delta < best[2].0 {
                if delta < best[0].0 {
                    best[2] = best[1];
                    best[1] = best[0];
        ...
        self.ins_cache[v][r] = InsCache { epoch: self.cache_epoch, stamp, best };
        best
    }
    ...
    pub fn run_from_routes(
        ...
        self.nb_moves = 1;
        // nb_moves and route stamps restart each run: retire all cached
        // insertion entries from previous runs in O(1).
        self.cache_epoch += 1;
    ...
    pub fn run_swapstar(&mut self, r1: usize, pos1: usize, r2: usize, pos2: usize, apply: bool) -> Option<i64> {
        ...
        let (u, v, u_demand, v_demand, seqno1_u_load, seqno1_u_dist, seqno1_v_load, seqno1_v_dist, pu, nu, pv, nv) = {
            ...
            (
                node_u.id, node_v.id, node_u.demand, node_v.demand,
                node_u.seqno1.load, node_u.seqno1.distance,
                node_v.seqno1.load, node_v.seqno1.distance,
                route1_nodes[pos1 - 1].id, route1_nodes[pos1 + 1].id,
                route2_nodes[pos2 - 1].id, route2_nodes[pos2 + 1].id,
            )
        };

        // First filter on route costs
        let new_load1 = seqno1_u_load + v_demand;
        let new_load2 = seqno1_v_load + u_demand;
        ...
        let cost_lb_r1_after_removal = (seqno1_u_dist as i64) + new_pen1;
        let cost_lb_r2_after_removal = (seqno1_v_dist as i64) + new_pen2;
        ...
        let d_pu_nu = self.data.dm(pu, nu);
        let d_pv_nv = self.data.dm(pv, nv);

        // best detour for inserting v into r1 \ {u}: either at U's freed slot,
        // or the best cached gap not adjacent to U. At most two gaps touch U,
        // so the top-3 cache always contains the true best non-adjacent gap.
        let mut best_ins_v = self.data.dm(pu, v) + self.data.dm(v, nu) - d_pu_nu; // insertion at U place
        for &(delta, t) in self.best_insertions(v, r1).iter() {
            if t != pos1 && t != pos1 + 1 && delta < best_ins_v { best_ins_v = delta; }
        }
        ...
        let mut best_ins_u = self.data.dm(pv, u) + self.data.dm(u, nv) - d_pv_nv; // insertion at V place
        for &(delta, t) in self.best_insertions(u, r2).iter() {
            if t != pos2 && t != pos2 + 1 && delta < best_ins_u { best_ins_u = delta; }
        }
        ...
        // Values needed only for the exact TW-aware evaluation
        let data = self.data.as_ref();
        let route1_nodes = &self.routes[r1].nodes;
        let route2_nodes = &self.routes[r2].nodes;
    ...
    #[inline(always)]
    fn find_best_move_for_customer(
        &mut self,
        c1: usize,
        last_tested: usize,
        loop_id: usize,
    ) -> Option<(i64, CandidateMove)> {
        ...
        best_move.map(|mv| (best_delta, mv))
    }

    #[inline]
    fn move_routes(mv: &CandidateMove) -> (usize, Option<usize>) {
        match *mv {
            CandidateMove::InterRoute { r1, r2, .. } => (r1, Some(r2)),
            CandidateMove::TwoOptStar { r1, r2, .. } => (r1, Some(r2)),
            CandidateMove::SwapStar { r1, r2, .. } => (r1, Some(r2)),
            CandidateMove::IntraRelocate { r1, .. } => (r1, None),
            CandidateMove::IntraOrOpt2 { r1, .. } => (r1, None),
            CandidateMove::IntraSwap { r1, .. } => (r1, None),
            CandidateMove::Intra2Opt { r1, .. } => (r1, None),
        }
    }

    /// A descriptor is stale iff one of its routes changed after evaluation.
    /// Non-stale descriptors have exact deltas and valid positions, so they
    /// can be applied directly without any rescan.
    #[inline]
    fn entry_is_stale(&self, entry: &SmdEntry) -> bool {
        let (r1, r2) = Self::move_routes(&entry.mv);
        if self.when_last_modified[r1] > entry.stamp {
            return true;
        }
        if let Some(r2) = r2 {
            if self.when_last_modified[r2] > entry.stamp {
                return true;
            }
        }
        false
    ...
    fn search(&mut self, rng: &mut SmallRng) {
        ...
        while improved || loop_id < 2 {
            ...
            if self.smd_pass(loop_id) {
                improved = true;
            }
        }
    }

    /// Static-move-descriptor pass: seed a best-first heap with each
    /// customer's cached best move delta, then repeatedly pop the most
    /// improving descriptor. Entries whose routes were modified after their
    /// timestamp are lazily recomputed on pop instead of eagerly rescanned,
    /// so each step costs near O(log n) plus targeted re-evaluations.
    fn smd_pass(&mut self, loop_id: usize) -> bool {
        let mut improved = false;
        let mut heap: BinaryHeap<SmdEntry> =
            BinaryHeap::with_capacity(self.loop_order_nodes.len() + 1);

        // Seed the descriptor cache (uses when_last_tested pruning as before).
        for idx in 0..self.loop_order_nodes.len() {
            ...
            if let Some((delta, mv)) = self.find_best_move_for_customer(c1, last_tested, loop_id) {
                heap.push(SmdEntry { delta, stamp: self.nb_moves, customer: c1, mv });
            }
        }

        let pop_budget = SMD_POPS_PER_NODE
            .saturating_mul(self.loop_order_nodes.len())
            .saturating_add(SMD_POPS_BASE);
        let mut pops = 0usize;
        while let Some(entry) = heap.pop() {
            ...
            let c1 = entry.customer;
            if self.entry_is_stale(&entry) {
                // Lazy refresh: only descriptors touching changed routes are
                // recomputed, and only when they reach the top of the heap.
                let last_tested = self.when_last_tested[c1];
                self.when_last_tested[c1] = self.nb_moves;
                if let Some((delta, mv)) = self.find_best_move_for_customer(c1, last_tested, loop_id) {
                    heap.push(SmdEntry { delta, stamp: self.nb_moves, customer: c1, mv });
            ...
            match self.apply_candidate_move(entry.mv) {
                Some(delta) => {
                    improved = true;
                    // Refresh the moved customer's descriptor immediately
                    // while its context is hot (mirrors the former repeat loop).
                    if delta < 0 {
                        let last_tested = self.when_last_tested[c1];
                        self.when_last_tested[c1] = self.nb_moves;
                        if let Some((d2, mv2)) =
                            self.find_best_move_for_customer(c1, last_tested, loop_id)
                        {
                            heap.push(SmdEntry { delta: d2, stamp: self.nb_moves, customer: c1, mv: mv2 });
                        }
                    }
                }
                None => {
                    // Move credit shrank since evaluation; recompute lazily.
                    let last_tested = self.when_last_tested[c1];
                    self.when_last_tested[c1] = self.nb_moves;
