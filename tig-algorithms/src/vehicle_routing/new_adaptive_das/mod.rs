...
pub struct Hints {
    pub demand_priors: Option<Vec<f64>>,
    pub zone_time_weights: Option<Vec<f64>>,
    /// Optional warmstart giant tour (sequence of client node ids, excludes depot 0)
    pub warmstart_tour: Option<Vec<usize>>,
}
...
impl Constructive {
    pub fn build_routes(data: &Problem, rng: &mut SmallRng, randomize: bool) -> Vec<Vec<usize>> {
        ...
        let n = nodes.len();
        // If the user passed demand priors or zone/time weights, bias initial node ordering
        let compute_key = |node: usize| -> f64 {
            let base = data.dm(0, node) as f64;
            let prior = data.hints.demand_priors.as_ref().and_then(|v| v.get(node).copied()).unwrap_or(0.0);
            let ztw = data.hints.zone_time_weights.as_ref().and_then(|v| v.get(node).copied()).unwrap_or(0.0);
            // fixed scaling factors chosen to have sensible impact relative to distances
            base - 50.0 * prior - 30.0 * ztw
        };

        nodes.sort_by(|&a, &b| {
            let ka = compute_key(a);
            let kb = compute_key(b);
    ...
    /// Ultra-fast greedy constructive builder — cheap, single-shot solution
    pub fn build_routes_fast(data: &Problem, rng: &mut SmallRng) -> Vec<Vec<usize>> {
        let n = data.nb_nodes;
        let mut used = vec![false; n];
        used[0] = true;
        let mut routes: Vec<Vec<usize>> = Vec::new();

        while used.iter().any(|&v| !v) {
            // pick first unused client
            let mut seed = None;
            for i in 1..n {
                if !used[i] { seed = Some(i); break; }
            }
            if seed.is_none() { break; }
            let mut route: Vec<usize> = Vec::new();
            route.push(0);
            let mut current = seed.unwrap();
            route.push(current);
            used[current] = true;
            let mut load = data.demands[current];
            ...
            loop {
                ...
                for node in 1..n {
                    ...
                    if load + data.demands[node] > data.max_capacity { continue; }
                    let travel = data.dm(current, node);
                    let arrive = (curr_time + travel).max(data.start_tw[node]);
                    if arrive > data.end_tw[node] { continue; }
                    if best.is_none() || travel < best.unwrap().1 {
                        best = Some((node, travel));
                    }
                }

                if let Some((node, travel)) = best {
                    curr_time += travel;
                    if curr_time < data.start_tw[node] { curr_time = data.start_tw[node]; }
...
impl<'a> Genetic<'a> {
    ...
    pub fn run(
        ...
        // Fast-mode early exit: build cheap solution and return immediately
        if self.params.fast_mode {
            let mut routes = Constructive::build_routes_fast(self.data, rng);
            self.fast_local_improve(&mut routes, rng);
            if let Some(save) = save_solution {
                let _ = save(&Solution { routes: routes.clone() });
            }
            let ind = Individual::new_from_routes(self.data, &self.params, routes.clone());
            return Some((routes, ind.cost as i32));
        }

        let mut ls = LocalSearch::new(self.data, self.params);
        // If a warmstart giant tour is provided via hints, seed the population with it
        if let Some(tour) = &self.data.hints.warmstart_tour {
            if !tour.is_empty() {
                // split warmstart tour into a reasonable number of routes (cap to fleet size)
                let target_routes = self.data.nb_vehicles.max(self.data.lb_vehicles);
                let mut warm_routes = self.split(tour, target_routes);
                ls.runls(&mut warm_routes, rng, self.params, false, 0);
                let warm_ind = Individual::new_from_routes(self.data, &self.params, warm_routes);
                self.population.add(warm_ind, &self.params);
            }
        }
    ...
    /// Lightweight in-place fast local improvement
    pub fn fast_local_improve(&self, routes: &mut Vec<Vec<usize>>, _rng: &mut SmallRng) {
        let dm = |i: usize, j: usize| self.data.dm(i, j);

        // Bounded 2-opt passes
        ...
        for route in routes.iter_mut() {
            ...
            while passes < max_passes {
                ...
                for i in 1..(len - 2) {
                    let j_max = (i + 10).min(len - 2);
                    for j in i..=j_max {
                        let prev = route[i - 1];
                        let a = route[i];
                        let b = route[j];
                        let next = route[j + 1];
                        let cur = dm(prev, a) + dm(b, next);
                        let nw = dm(prev, b) + dm(a, next);
                        if nw < cur {
                            route[i..=j].reverse();
                            improved = true;
                            break;
        ...
        // Tiny relocate-to-route-start moves
        let mut loads: Vec<i32> = routes.iter().map(|r| r.iter().map(|&n| self.data.demands[n]).sum()).collect();
        for s in 0..routes.len() {
            let mut k = 1usize;
            while k + 1 < routes[s].len() {
                let node = routes[s][k];
                if node == 0 { k += 1; continue; }
                let prev = routes[s][k - 1];
                let next = routes[s][k + 1];
                let hole = dm(prev, next) - dm(prev, node) - dm(node, next);
                let mut moved = false;
                for t in 0..routes.len() {
                    if t == s { continue; }
                    if routes[t].len() <= 2 { continue; }
                    let first = routes[t][1];
                    let insert_cost = dm(0, node) + dm(node, first) - dm(0, first);
                    let est = hole + insert_cost;
                    if est < 0 && loads[t] + self.data.demands[node] <= self.data.max_capacity {
                        // remove node from s at k and insert at pos 1 of t
                        routes[s].remove(k);
                        routes[t].insert(1, node);
                        loads[s] -= self.data.demands[node];
                        loads[t] += self.data.demands[node];
                        moved = true;
                        break;
                    }
                }
...
impl TigLoader {
    pub fn load(challenge: &Challenge, hyperparameters: &Option<Map<String, Value>>) -> Problem {
        ...
        // parse optional hints from hyperparameters
        let mut hints = Hints::default();
        if let Some(map) = hyperparameters {
            if let Some(val) = map.get("demand_priors") {
                if let Value::Array(arr) = val {
                    let mut pri: Vec<f64> = Vec::with_capacity(arr.len());
                    for v in arr.iter() {
                        match v {
                            Value::Number(n) => pri.push(n.as_f64().unwrap_or(0.0)),
                            Value::String(s) => pri.push(s.parse::<f64>().unwrap_or(0.0)),
                            _ => pri.push(0.0),
                        }
                    }
                    hints.demand_priors = Some(pri);
                }
            }
            if let Some(val) = map.get("zone_time_weights") {
                if let Value::Array(arr) = val {
                    let mut zt: Vec<f64> = Vec::with_capacity(arr.len());
                    for v in arr.iter() {
                        match v {
                            Value::Number(n) => zt.push(n.as_f64().unwrap_or(0.0)),
                            Value::String(s) => zt.push(s.parse::<f64>().unwrap_or(0.0)),
                            _ => zt.push(0.0),
                        }
                    }
                    hints.zone_time_weights = Some(zt);
                }
            }
            if let Some(val) = map.get("warmstart_tour") {
                if let Value::Array(arr) = val {
                    let mut tour: Vec<usize> = Vec::with_capacity(arr.len());
                    for v in arr.iter() {
                        match v {
                            Value::Number(n) => tour.push(n.as_u64().unwrap_or(0) as usize),
                            Value::String(s) => tour.push(s.parse::<usize>().unwrap_or(0)),
                            _ => {}
                        }
                    }
                    if !tour.is_empty() { hints.warmstart_tour = Some(tour); }
                }
            }
        }
...
impl Solver {
    ...
    pub fn solve_challenge_instance(
        ...
        let t0 = Instant::now();
        let data = TigLoader::load(&challenge, hyperparameters);
...
pub fn solve_challenge(
    ...
    // Route based on number of nodes. For now all handled by the same solver.
    let n_nodes = challenge.num_nodes;
    match n_nodes {
        600 => {
            match Solver::solve_challenge_instance(challenge, hyperparameters, Some(save_solution))? {
                Some(solution) => { let _ = save_solution(&solution); Ok(()) }
                None => Ok(()),
            }
        }
        700 => {
...
pub fn help() {
        println!("TIG-native approach — Notes & Run Examples");
        println!("");
        println!("{}", r#"Concept:
    Encode "scheduled ahead" intelligence as additional algorithm parameters:
        - demand_priors (per-node or per-zone priors),
        - zone_time_weights (bias by zone & time-of-day),
        - warmstart_tour / warmstart_tours (seed solution(s)).
    Usage pattern:
        - Pass demand priors instead of making external API requests.
        - Bias constructive builder and GA seeding with zone/time weights.
        - Warm-start value function and population with precomputed tours.
    Behaviour:
        - Deterministic and TIG-compliant: algorithm behaves as if it had
            advance knowledge encoded via hints (no external scheduling API required)."#);
        println!("");
        println!("{}", r#"Implementation summary:
    - `adaptive_tig_adp_v5` accepts Hints: `demand_priors`, `zone_time_weights`, `warmstart_tour` (and warmstart_tours in hyperparams).
    - Hyperparameters are parsed via loader and attached to `Problem` data as `hints`.
    - Constructive builder and GA population are biased/seeded from hints.
    - Scaling factors (e.g. `prior_scale`, `zone_weight_scale`) are exposed as hyperparameters.
    - Warmstart seeding is deterministic and optional; exploration level controls how strongly warmstarts are exploited.
"#);
        println!("");
        println!("{}", r#"Hyperparameters (examples / notes):
    - `exploration_level` : primary tuning knob (0..6). Lower -> faster, higher -> more search.
    - `demand_priors` : map or vector of floats; higher values bias earlier picks.
    - `zone_time_weights` : nested map or list-of-tuples to bias by zone/time slots.
    - `warmstart_tour` / `warmstart_tours` : one or many giant tours (node id sequences) used to seed population.
    - `prior_scale`, `zone_weight_scale`, `warmstart_confidence` : optional floats to tune influence strength.
"#);
        println!("");
        println!("Run Examples:");
        println!("");
        println!("{}", r#"Basic:
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":3}' --nonces 50 --workers 16"#);
        println!("");
        println!("{}", r#"Different exploration / budget / seed:
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":1,"random_seed":42,"budget_ms":120000}' --nonces 100 --workers 8"#);
        println!("");
        println!("{}", r#"Simple demand priors (inline map):
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":3,"demand_priors":{"10":5.0,"23":2.0,"87":3.5}}' --nonces 50 --workers 16"#);
        println!("");
        println!("{}", r#"Demand priors as vector (index = node id or position):
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":3,"demand_priors":[0.1,0.2,0.3,0.0,0.5]}' --nonces 20 --workers 4"#);
        println!("");
        println!("{}", r#"Zone/time weights (nested map):
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":3,"zone_time_weights":{"zoneA":{"08":2.0,"09":1.5},"zoneB":{"14":3.0}}}' --nonces 50 --workers 16"#);
        println!("");
        println!("{}", r#"Zone/time weights as list of tuples:
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"zone_time_weights":[{"zone":"A","time":"08","weight":2.5},{"zone":"B","time":"14","weight":3.0}]}' --nonces 30 --workers 8"#);
        println!("");
        println!("{}", r#"Warmstart tour inline (single tour):
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":3,"warmstart_tour":[0,5,12,45,78,0]}' --nonces 10 --workers 4"#);
        println!("");
        println!("{}", r#"Warmstart population (multiple tours):
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"warmstart_tours":[[0,10,20,0],[0,3,7,9,0]],"exploration_level":2}' --nonces 30 --workers 8"#);
        println!("");
        println!("{}", r#"Combine demand priors + zone/time weights + warmstart:
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":4,"demand_priors":{"12":5.0},"zone_time_weights":{"Z1":{"07":2.0}},"warmstart_tour":[0,12,34,56,0]}' --nonces 80 --workers 16"#);
        println!("");
        println!("{}", r#"Tune prior/weight scaling:
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"demand_priors":{"1":10},"prior_scale":50.0,"zone_time_weights":{"A":{"09":3}},"zone_weight_scale":30.0}' --nonces 40 --workers 12"#);
        println!("");
        println!("{}", r#"Aggressive warmstart + low exploration (exploit):
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":0,"warmstart_tour":[0,1,2,3,4,0],"warmstart_confidence":0.95}' --nonces 5 --workers 2"#);
        println!("");
        println!("{}", r#"High exploration + no warmstart (explore):
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"exploration_level":10,"random_seed":7}' --nonces 200 --workers 32"#);
        println!("");
        println!("{}", r#"Per-zone demand priors with fallback vector:
    test_algorithm adaptive_tig_adp_v5 n_nodes=600 '{"demand_priors_by_zone":{"zoneA":[0.2,0.3,0.5],"zoneB":[0.1,0.0,0.9]}}' --nonces 60 --workers 12"#);
        println!("");
        println!("Notes:");
        println!("  - Hints are optional and backwards-compatible: missing fields are ignored.");
        println!("  - Start with `exploration_level` 2-4 for sensible default behavior.");
        println!("  - Use `prior_scale` / `zone_weight_scale` to tune influence when priors/weights are on different numeric scales.");
        println!("  - Warmstarts seed population but do not force a single deterministic outcome when exploration>0.");
}
