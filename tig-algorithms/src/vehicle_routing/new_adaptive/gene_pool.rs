...
pub struct Subpopulation {
    ...
    // reusable temporary buffers to avoid repeated allocations
    pub avg_closest: Vec<f64>,
    pub div_pairs: Vec<(f64, usize)>,
    pub cost_pos: Vec<usize>,
    pub prox_sorted: bool,
}

impl Default for Subpopulation {
    fn default() -> Self {
        Self {
            indivs: Vec::new(),
            prox: Vec::new(),
            biased_fitness: Vec::new(),
            order_cost: Vec::new(),
            avg_closest: Vec::new(),
            div_pairs: Vec::new(),
            cost_pos: Vec::new(),
...
impl<'a> GenePool<'a> {
    ...
    pub fn recompute_costs(&mut self, params: &Config) {
        let threads = thread::available_parallelism().map(|n| n.get()).unwrap_or(1) as usize;
        let do_work = |inds: &mut [Individual]| {
            if inds.is_empty() { return; }
            // heuristic: parallelize only when total work justifies thread overhead
            let min_chunk = 64usize;
            let min_total = threads.saturating_mul(256);
            if threads <= 1 || inds.len() < min_total {
                for ind in inds.iter_mut() { ind.recompute_cost(params); }
                return;
            }
            let chunk = ((inds.len() + threads - 1) / threads).max(min_chunk);
            thread::scope(|s| {
                for part in inds.chunks_mut(chunk) {
                    s.spawn(|| {
                        for ind in part.iter_mut() { ind.recompute_cost(params); }
                    });
                }
            });
        };

        do_work(&mut self.feasible.indivs);
        do_work(&mut self.infeasible.indivs);
    }
    ...
    fn worst_index_biased_with_clone_priority(sub: &mut Subpopulation) -> usize {
        // ensure prox lists sorted before using first neighbor
        if !sub.prox_sorted {
            for list in sub.prox.iter_mut() {
                list.sort_unstable_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
            }
            sub.prox_sorted = true;
        }
        // ensure prox lists are sorted; this function often called from survivors_selection
        // but sorting requires &mut; callers pass &mut so change signature in-place below.
    ...
    fn prox_add(sub: &mut Subpopulation, data: &Instance, new_idx: usize) {
        ...
        for i in 0..new_idx {
            ...
            // append to lists; defer sorting to a batch sort to amortize insert costs
            sub.prox[i].push((d, new_idx));
            sub.prox[new_idx].push((d, i));
        }
        // mark unsorted; next consumer will sort lazily
        sub.prox_sorted = false;
    }
    ...
    fn remove_at_index(sub: &mut Subpopulation, idx: usize) {
        ...
        // mark prox unsorted because positions changed
        sub.prox_sorted = false;
    ...
    fn update_biased_fitnesses(sub: &mut Subpopulation, params: &Config) {
        ...
        // ensure prox lists are sorted (prox_add defers sorting for batch efficiency)
        if !sub.prox_sorted {
            for list in sub.prox.iter_mut() {
                list.sort_unstable_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
            }
        ...
        sub.avg_closest.clear();
        sub.avg_closest.resize(n, 0.0);
        let prox_ref = &sub.prox;
        let threads = thread::available_parallelism().map(|n| n.get()).unwrap_or(1) as usize;
        let min_chunk = 64usize;
        let min_total = threads.saturating_mul(256);
        if threads <= 1 || n <= 1 || n < min_total {
            for i in 0..n {
                let neighbors = &prox_ref[i];
                let mut sum = 0.0;
                for t in 0..nb_close { sum += neighbors[t].0; }
                sub.avg_closest[i] = sum / (nb_close as f64);
            }
        } else {
            let chunk = ((n + threads - 1) / threads).max(min_chunk);
            thread::scope(|s| {
                for (chunk_idx, avg_slice) in sub.avg_closest.chunks_mut(chunk).enumerate() {
                    let start = chunk_idx * chunk;
                    let prox_ref = prox_ref;
                    s.spawn(move || {
                        for (off, slot) in avg_slice.iter_mut().enumerate() {
                            let i = start + off;
                            let neighbors = &prox_ref[i];
                            let mut sum = 0.0;
                            for t in 0..nb_close { sum += neighbors[t].0; }
                            *slot = sum / (nb_close as f64);
                        }
                    });
                }
            });
        ...
        // reuse div_pairs buffer
        sub.div_pairs.clear();
        sub.div_pairs.reserve(n);
        for i in 0..n { sub.div_pairs.push((-sub.avg_closest[i], i)); }
        sub.div_pairs.sort_unstable_by(|a, b| a.0.partial_cmp(&b.0).unwrap());

        let denom = (n - 1) as f64;
        // reuse div_rank via avg_closest buffer (we no longer need avg_closest after div_pairs)
        let mut div_rank = &mut sub.avg_closest;
        // div_rank is sized n already
        for (pos, &(_, idx)) in sub.div_pairs.iter().enumerate() {
            div_rank[idx] = (pos as f64) / denom;
        }

        // reuse cost_pos buffer
        sub.cost_pos.clear();
        sub.cost_pos.resize(n, 0usize);
        for (pos, &idx) in sub.order_cost.iter().enumerate() {
            sub.cost_pos[idx] = pos;
        }
        let fit_rank: Vec<f64> = sub.cost_pos.iter().map(|&p| (p as f64) / denom).collect();

        let scale = 1.0 - (params.nb_elite as f64) / (n as f64);
        for i in 0..n {
            if sub.cost_pos[i] < params.nb_elite {
...
// removed persistent thread-pool; use scoped threads with adaptive threshold
