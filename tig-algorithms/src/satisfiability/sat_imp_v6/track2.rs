...
pub fn solve(
    ...
    let mut has_initial_unit = false;
    let mut cl = Vec::with_capacity(challenge.clauses.len() * 3);
    let mut co = Vec::with_capacity(challenge.clauses.len() + 1);
    co.push(0u32);
    ...
    for orig in &challenge.clauses {
        ...
        if b == a && c == a {
            has_initial_unit = true;
        }
        co.push(cl.len() as u32);
    }

    let mut nc = good_clauses as usize;
...

    {
        ...
        for ci in 0..nc {
            for j in co[ci] as usize..co[ci + 1] as usize {
                let l = cl[j];
                let v = (l.abs() - 1) as usize;
                if l > 0 {
                    p_data[p_cnt[v] as usize] = ci as u32;
                    p_cnt[v] += 1;
                } else {
                    n_data[n_cnt[v] as usize] = ci as u32;
                    n_cnt[v] += 1;
                }
            }
        }
    ...
    let mut forced = vec![-1i8; nv];
    if has_initial_unit {
    let mut remaining = vec![0u8; nc];
    let mut satisfied = vec![false; nc];
    let mut queue = Vec::new();
    for ci in 0..nc {
        let len = co[ci + 1] - co[ci];
        remaining[ci] = len as u8;
        if len == 1 {
            queue.push(cl[co[ci] as usize]);
        }
    }
    ...
    while queue_pos < queue.len() {
        ...
        let (sat_s, sat_e, sat_data, unsat_s, unsat_e, unsat_data) = if value != 0 {
            (
                p_off[v] as usize,
                p_off[v + 1] as usize,
                &p_data,
        ...
        for k in sat_s..sat_e {
            satisfied[sat_data[k] as usize] = true;
        }

        for k in unsat_s..unsat_e {
            let ci = unsat_data[k] as usize;
            if satisfied[ci] {
                continue;
            }
            remaining[ci] -= 1;
            ...
            if remaining[ci] == 1 {
                ...
                for j in co[ci] as usize..co[ci + 1] as usize {
                    let candidate = cl[j];
                    if forced[(candidate.abs() - 1) as usize] < 0 {
                        unit = candidate;
                        break;
                    }
                }
    ...
    if !contradiction {
        ...
        let mut write_ci = 0usize;
        for ci in 0..nc {
            let read_end = co[ci + 1] as usize;
            if !satisfied[ci] {
                for j in read_pos..read_end {
                    let lit = cl[j];
                    if forced[(lit.abs() - 1) as usize] < 0 {
                        cl[write_pos] = lit;
                        write_pos += 1;
                    }
                }
                write_ci += 1;
                co[write_ci] = write_pos as u32;
            }
            read_pos = read_end;
        }
        cl.truncate(write_pos);
        co.truncate(write_ci + 1);
        nc = write_ci;

        if nc == 0 {
            let mut variables = vec![false; nv];
            for v in 0..nv {
                variables[v] = forced[v] > 0;
            }
        ...
        p_data = vec![0u32; p_off[nv] as usize];
        n_data = vec![0u32; n_off[nv] as usize];
        p_cnt.copy_from_slice(&p_off[..nv]);
        n_cnt.copy_from_slice(&n_off[..nv]);
        for ci in 0..nc {
            for j in co[ci] as usize..co[ci + 1] as usize {
    ...
    let total_nodes = nv + nc;
    let mut degree = vec![0u32; total_nodes];
    for v in 0..nv {
        degree[v] = p_off[v + 1] - p_off[v] + n_off[v + 1] - n_off[v];
    }
    for ci in 0..nc {
        degree[nv + ci] = co[ci + 1] - co[ci];
    }
    let mut peel = Vec::with_capacity(total_nodes);
    for node in 0..total_nodes {
        if degree[node] <= 1 {
            peel.push(node);
        }
    }
    let mut peeled = vec![false; total_nodes];
    while let Some(node) = peel.pop() {
        if peeled[node] {
            continue;
        }
        peeled[node] = true;
        if node < nv {
            for k in p_off[node]..p_off[node + 1] {
                let other = nv + p_data[k as usize] as usize;
                if !peeled[other] {
                    degree[other] -= 1;
                    if degree[other] <= 1 {
                        peel.push(other);
            ...
            for k in n_off[node]..n_off[node + 1] {
                let other = nv + n_data[k as usize] as usize;
                if !peeled[other] {
                    degree[other] -= 1;
                    if degree[other] <= 1 {
                        peel.push(other);
    ...
    if peeled.iter().all(|&done| done) {
        let mut adj = vec![Vec::<usize>::new(); total_nodes];
        for ci in 0..nc {
            let cnode = nv + ci;
            for j in co[ci] as usize..co[ci + 1] as usize {
                let v = (cl[j].abs() - 1) as usize;
                adj[v].push(cnode);
                adj[cnode].push(v);
            }
        }
        ...
        let mut parent = vec![unvisited; total_nodes];
        let mut order = Vec::with_capacity(total_nodes);
        let mut stack = Vec::new();
        for ci in 0..nc {
            let root = nv + ci;
            if parent[root] != unvisited {
                continue;
            }
        ...
        let mut can_false = vec![false; total_nodes];
        let mut can_true = vec![false; total_nodes];
            ...
            if node < nv {
                ...
                for &other in &adj[node] {
                    if parent[other] == node {
                        false_ok &= can_false[other];
                        true_ok &= can_true[other];
                    }
                }
                can_false[node] = false_ok;
                can_true[node] = true_ok;
            } else {
                let ci = node - nv;
                let pv = parent[node];
                let mut children_ok = true;
                let mut child_can_satisfy = false;
                let mut parent_lit = 0i32;
                for j in co[ci] as usize..co[ci + 1] as usize {
                    let lit = cl[j];
                    let v = (lit.abs() - 1) as usize;
                    if v == pv {
                        parent_lit = lit;
                    } else if parent[v] == node {
                        children_ok &= can_false[v] || can_true[v];
                        child_can_satisfy |= if lit > 0 { can_true[v] } else { can_false[v] };
                    }
                }
                if pv == total_nodes {
                    let ok = children_ok && child_can_satisfy;
                    can_false[node] = ok;
                    can_true[node] = ok;
                } else {
                    can_false[node] = children_ok && (parent_lit < 0 || child_can_satisfy);
                    can_true[node] = children_ok && (parent_lit > 0 || child_can_satisfy);
                }
            }
        }

        let mut tree_vars = vec![false; nv];
        for v in 0..nv {
            if forced[v] >= 0 {
                tree_vars[v] = forced[v] != 0;
            }
        }
        ...
        for &node in &order {
            ...
            for j in co[ci] as usize..co[ci + 1] as usize {
                ...
                let v = (lit.abs() - 1) as usize;
                if parent[v] == node {
                    tree_vars[v] = if can_false[v] { false } else { true };
                }
            }
            ...
            for j in co[ci] as usize..co[ci + 1] as usize {
                let lit = cl[j];
                let v = (lit.abs() - 1) as usize;
                if (lit > 0 && tree_vars[v]) || (lit < 0 && !tree_vars[v]) {
                    clause_satisfied = true;
                    break;
                }
                if parent[v] == node {
                    if lit > 0 && can_true[v] {
                        candidate = Some((v, true));
                    } else if lit < 0 && can_false[v] {
                        candidate = Some((v, false));
                    }
                }
            }
            if !clause_satisfied {
                if let Some((v, value)) = candidate {
                    tree_vars[v] = value;
                } else {
                    tree_ok = false;
                    break;
        ...
        if tree_ok {
            let _ = save_solution(&Solution { variables: tree_vars });
            return Ok(());
        }
    }
    ...
    let mut var_appearances = Vec::with_capacity(nv);
    for v in 0..nv {
        p_cnt[v] -= p_off[v];
        n_cnt[v] -= n_off[v];
        var_appearances.push((p_cnt[v] + n_cnt[v]) as usize);
    }
    ...
    let restart_attempts = hp
        .as_ref()
        .and_then(|h| h.restart_attempts)
        .unwrap_or_else(|| raw_restarts.saturating_sub(if density > 5.0 { 1 } else { 0 }))
        .max(1)
        .min(max_flips.max(1));    
    let extra_attempts = hp.as_ref().and_then(|h| h.extra_attempts).unwrap_or(2);
    let abandon_ck: Vec<(usize, usize)> = {
        let (cks, ths): (Vec<f64>, Vec<usize>) = match hp.as_ref() {
            Some(h) if h.abandon_checkpoints_mflips.is_some() || h.abandon_thresholds.is_some() => (
                h.abandon_checkpoints_mflips.clone().unwrap_or_default(),
                h.abandon_thresholds.clone().unwrap_or_default(),
            ),
            _ => (vec![90.0, 130.0], vec![12, 8]),
        };
        let mut v: Vec<(usize, usize)> = cks
            .iter()
            .zip(ths.iter())
            .map(|(c, t)| ((c * 1_000_000.0) as usize, *t))
            .collect();
        v.sort_unstable();
        v
    };
    ...
    for attempt in 0..restart_attempts + extra_attempts {
        ...
        let mut attempt_min = unsat_count;
        ...
        unsafe {
            loop {
                ...
                if rounds % variance_interval == 0 && rounds > 0 {
                    ...
                    if min_unsat_window < attempt_min { attempt_min = min_unsat_window; }
                    max_unsat_window = unsat_count;
                    min_unsat_window = unsat_count;

                    if attempt == 0 && next_ck < abandon_ck.len() && rounds >= abandon_ck[next_ck].0 {
                        if attempt_min > abandon_ck[next_ck].1 {
                            abandoned = true;
                            break;
                        }
                        next_ck += 1;
