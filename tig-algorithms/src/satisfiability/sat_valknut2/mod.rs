...
pub fn help() {
    println!("SAT Vanguard v9 - per-track files (T1/T3/T4/T5/T38)");
}
...
pub fn solve_challenge(
    ...
    match (nv, nc_total) {
        (10000, 42670) => {
            let mut hp = Hparams::for_t1().merge_user(user);
            hp.stagnation_limit = hp.stagnation_limit.or(Some((nv / 20).max(2)));
            track_t1::solve(challenge, save_solution, &hp)
        }
        (100000, 415000) => {
            let mut hp = Hparams::for_t3().merge_user(user);
            hp.stagnation_limit = hp.stagnation_limit.or(Some((nv / 20).max(2)));
            track_t3::solve(challenge, save_solution, &hp)
        }
        (5000, 21335) => {
            let mut hp = Hparams::for_t4().merge_user(user);
            hp.stagnation_limit = hp.stagnation_limit.or(Some((nv / 20).max(2)));
            track_t4::solve(challenge, save_solution, &hp)
        }
        (7500, 32002) => {
            let mut hp = Hparams::for_t5().merge_user(user);
            hp.stagnation_limit = hp.stagnation_limit.or(Some((nv / 20).max(2)));
            track_t5::solve(challenge, save_solution, &hp)
        }
        (100000, 420000) => {
            let mut hp = Hparams::for_t38().merge_user(user);
            hp.stagnation_limit = hp.stagnation_limit.or(Some((nv / 20).max(2)));
            track_t38::solve(challenge, save_solution, &hp)
        }
        _ => Err(anyhow::anyhow!(
            "sat_vanguard_v9: unknown track config (num_variables={}, num_clauses={})",
            nv, nc_total
        )),
    }
}
