...
#[inline]
fn csr_slice<'a>(offsets: &[usize], items: &'a [usize], idx: usize) -> &'a [usize] {
...
fn solve_high_density_suma(
    ...
    let mut p_single = vec![0u8; challenge.num_variables];
    let mut n_single = vec![0u8; challenge.num_variables];
    ...
    while !dead {
        ...
        for c in &clauses_ {
            ...
            for &l in c.iter() {
                ...
                if (p_single[idx] != 0 && l > 0) || (n_single[idx] != 0 && l < 0) {
                    skip = true;
                    break;
                }
                if p_single[idx] != 0 || n_single[idx] != 0 {
                    done = false;
                    continue;
                }
                c_.push(l);
            ...
            match c_[..] {
                [l] => {
                    ...
                    if l > 0 {
                        if n_single[(l.abs() - 1) as usize] != 0 {
                            dead = true;
                            break;
                        } else {
                            p_single[(l.abs() - 1) as usize] = 1;
                        }
                    } else {
                        if p_single[(l.abs() - 1) as usize] != 0 {
                            dead = true;
                            break;
                        } else {
                            n_single[(l.abs() - 1) as usize] = 1;
                        }
                    }
                }
                [] => {
    ...
    let mut p_counts = vec![0usize; num_variables];
    let mut n_counts = vec![0usize; num_variables];
    ...
    let mut p_offsets = vec![0usize; num_variables + 1];
    let mut n_offsets = vec![0usize; num_variables + 1];
    for v in 0..num_variables {
        p_offsets[v + 1] = p_offsets[v] + p_counts[v];
        n_offsets[v + 1] = n_offsets[v] + n_counts[v];
    }
    let mut p_items = vec![0usize; p_offsets[num_variables]];
    let mut n_items = vec![0usize; n_offsets[num_variables]];
    let mut p_write = p_offsets[..num_variables].to_vec();
    let mut n_write = n_offsets[..num_variables].to_vec();
    ...
    let var_appearances: Vec<usize> = (0..num_variables)
        .map(|v| (p_offsets[v + 1] - p_offsets[v]) + (n_offsets[v + 1] - n_offsets[v]))
        .collect();
    ...
    for v in 0..num_variables {
        let num_p = p_offsets[v + 1] - p_offsets[v];
        let num_n = n_offsets[v + 1] - n_offsets[v];
    ...
    let mut in_queue = vec![0u8; num_clauses];
    ...
    unsafe {
        loop {
            ...
            if !residual_.is_empty() {
                ...
                let mut c = clauses.get_unchecked_mut(i);
                ...
                'outer: for &l in c.iter() {
                   ...
                   let clauses_to_check: &[usize] = if *variables.get_unchecked(abs_l) {
                       csr_slice(&p_offsets, &p_items, abs_l)
                   } else {
                       csr_slice(&n_offsets, &n_items, abs_l)
                   };
                ...
                let clauses_to_decrement: &[usize] = if was_true {
                    csr_slice(&p_offsets, &p_items, v)
                } else {
                    csr_slice(&n_offsets, &n_items, v)
                };
                let clauses_to_increment: &[usize] = if was_true {
                    csr_slice(&n_offsets, &n_items, v)
                } else {
                    csr_slice(&p_offsets, &p_items, v)
                };
                ...
                for &cid in clauses_to_decrement {
                    ...
                    *num_good = new_val;
                    if new_val == 0 && *in_queue.get_unchecked(cid) == 0 {
                        *in_queue.get_unchecked_mut(cid) = 1;
                        residual_.push(cid);
                    }
                }
                ...
                if progress <= 0 {
                    ...
                    if stagnation >= stagnation_limit {                        
                        if stagnation >= 5 && best_residual < residual_.len() {
                            ...
                            for (i, &num_good) in num_good_so_far.iter().enumerate() {
                                if num_good == 0 {
                                    *in_queue.get_unchecked_mut(i) = 1;
                                    residual_.push(i);
                                } else {
                                    *in_queue.get_unchecked_mut(i) = 0;
                                }
                            }
                        }
                        ...
                        for _ in 0..kicks {
                            ...
                            let was_true = *variables.get_unchecked(v);
                            let inc: &[usize] = if was_true {
                                csr_slice(&n_offsets, &n_items, v)
                            } else {
                                csr_slice(&p_offsets, &p_items, v)
                            };
                            let dec: &[usize] = if was_true {
                                csr_slice(&p_offsets, &p_items, v)
                            } else {
                                csr_slice(&n_offsets, &n_items, v)
                            };
                            ...
                            for &cid2 in dec {
                                ...
                                *num_good = new_val;
                                if new_val == 0
                                    && *in_queue.get_unchecked(cid2) == 0
                                {
                                    *in_queue.get_unchecked_mut(cid2) = 1;
                                    residual_.push(cid2);
                                }
                            }
                            *variables.get_unchecked_mut(v) = !was_true;
            ...
            if !residual_.is_empty() {
                ...
                let v = if let Some(abs_l) = zero_found {
                    ...
                    for &l in c.iter() {
                        ...
                        if sad == 0 {
                            let curr_appearances = (p_offsets[abs_l + 1] - p_offsets[abs_l])
                                + (n_offsets[abs_l + 1] - n_offsets[abs_l]);
                            let age_bonus = (*var_age.get_unchecked(abs_l) as usize) / 4; 
                            let adjusted_weight = curr_appearances.saturating_sub(age_bonus);
                            if min_sad > 0 || adjusted_weight < min_weight {
                                min_sad = 0;
                            ...
                            if min_sad > 0 {
                                let appearances = (p_offsets[abs_l + 1] - p_offsets[abs_l])
                                    + (n_offsets[abs_l + 1] - n_offsets[abs_l]);
