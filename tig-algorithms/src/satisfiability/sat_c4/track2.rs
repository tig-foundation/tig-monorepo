...
// ---- grafted give-up (n_vars=7500,ratio=4267). Integer-only: no clock, no RNG
// draw, no float, no map iteration. It can only ever STOP the loop, never steer
// it, so every nonce it does not abandon is bit-identical to sat_imp_v4.
// Absolute and fuel-invariant, same reasoning as track3. 67.5M = 5% of the 280e9
// calibration budget = 2.26x the worst traced first passage (2.22% of budget),
// against the 2.24x required at S=4 seeds.
const GIVEUP_MIN_UNSAT: usize = 16;
const GIVEUP_DEADLINE_FLIPS: usize = 67_500_000;
...
pub fn solve(
    ...
    let giveup_deadline: usize = GIVEUP_DEADLINE_FLIPS;
    let mut giveup_latched = false;
    let mut giveup_abandon = false;
    for attempt in 0..restart_attempts {
        ...
        unsafe {
            loop {
                ...
                if !giveup_latched {
                    if unsat_count <= GIVEUP_MIN_UNSAT {
                        giveup_latched = true;
                    } else if rounds >= giveup_deadline {
                        giveup_abandon = true;
                        break;
                    }
                }
