...
pub fn solve(
    ...
    let mut giveup_latched: bool = best_unsat <= GIVEUP_LEVEL;
    ...
    unsafe {
        loop {
            ...
            if rounds % check_interval == 0 && rounds > 0 {
                if !giveup_latched && rounds >= giveup_deadline { break; }
            ...
            if clen > 1 {
                ...
                let ri = if clen == 3 { rv % 3 } else { rv % clen };
                cl.swap(cs, cs + ri);
            }

            let clen_actual = clen.min(WBUF);
            ...
            *vars.get_unchecked_mut(v_idx) = !was_true;
            ...
            if true_unsat < best_unsat {
                ...
                if best_unsat <= GIVEUP_LEVEL { giveup_latched = true; }
            }
