...
pub fn solve(
    ...
    // ---- FLAT4 (ported from track5). Stride-4 clause literals: clause i owns
    // cl4[4i..4i+3], zero-padded. One aligned block per clause, and `co` leaves
    // the hot loop entirely -- 1.66 MB of DRAM traffic removed at r4150.
    ...
    unsafe {
        loop {
            ...
            let v_idx = if zero_cnt > 0 {
                ...
                (cl4.get_unchecked(cs).abs() - 1) as usize
            } else {
                let mut min_sad = usize::MAX;
                let mut v_min = (cl4.get_unchecked(cs).abs() - 1) as usize;
