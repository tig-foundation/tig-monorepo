...
pub fn help() {
    println!("SAT Vanguard v4 - per-track files (T1/T3/T4/T5/T38)");
}
...
pub fn solve_challenge(
    ...
    if density >= 4.25 {
        ...
        if nv <= 7500 {
            return track_t5::solve(&hp, &mut rng, nv, nc, density, p_cnt, n_cnt, &all_off, &p_bound, &all_data, &mut cl, &co, save_solution);
        }
        return track_t1::solve(&hp, &mut rng, nv, nc, density, p_cnt, n_cnt, &all_off, &p_bound, &all_data, &mut cl, &co, save_solution);
    }
    if density < 4.18 {
        return track_t3::solve(&hp, &mut rng, nv, nc, density, p_cnt, n_cnt, &all_off, &p_bound, &all_data, &mut cl, &co, save_solution);
    }
    track_t38::solve(&hp, &mut rng, nv, nc, density, p_cnt, n_cnt, &all_off, &p_bound, &all_data, &mut cl, &co, save_solution)
}
