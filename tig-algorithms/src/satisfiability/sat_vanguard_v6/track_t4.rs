...
pub fn solve(
    ...
    // BON restart buffers (allocated once to avoid repeated allocation)
    ...
    unsafe {
        loop {
            ...
            if stagnation_count >= REINIT_STAGNATION && best_unsat >= REINIT_MIN_UNSAT && reinit_count < MAX_REINITS {
                ...
                // Best-of-N restart: sample N random assignments, keep the one with fewest unsat
                ...
                // Rebuild num_good for the selected best candidate (now in vars)
                num_good.fill(0);
                for c in 0..nc {
                    let s = co[c] as usize;
                    let e = co[c + 1] as usize;
