...
pub(crate) fn solve_c001_target(
    ...
    let default_max_fuel = match hp.hw_profile.as_deref() {
    ...
    let mut vars = target_initial_assignment_counts(nv, &p_cnt, &n_cnt, &mut rng, hp);
    ...
    unsafe {
        loop {
            ...
            {
                reinit_count += 1;
                vars = target_initial_assignment_counts(nv, &p_cnt, &n_cnt, &mut rng, hp);
                ...
                for c in 0..nc {
                    ...
                    for j in s..e {
                        ...
                        let v = (lit.abs() - 1) as usize;
                        if (lit > 0 && *vars.get_unchecked(v))
                            || (lit < 0 && !*vars.get_unchecked(v))
                        {
                            good += 1;
                        }
                    }
                    *num_good.get_unchecked_mut(c) = good;
                }
...
fn target_initial_assignment_counts(
    ...
    let random_threshold = hp
        .init_noise
        .unwrap_or(if nv >= 30_000 { 0.01 } else { 0.003 })
        .clamp(0.0, 0.5);
