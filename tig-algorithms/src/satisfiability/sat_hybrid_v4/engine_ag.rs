...
pub fn solve(
    ...
    match hp.sel_variant.unwrap_or(DEFAULT_VARIANT) {
        ...
        0 => run_buf::<false, true, true, false, 2, 0, 1, 0, true, false, true, false, true, true, true, true, false, false>(challenge, save_solution, &hp),
        ...
        1 => run_buf::<false, true, true, false, 2, 0, 1, 0, true, false, true, false, true, true, true, true, true, false>(challenge, save_solution, &hp),
        ...
        2 => run_buf::<false, true, true, false, 2, 0, 1, 0, true, false, true, false, true, true, true, true, false, true>(challenge, save_solution, &hp),
        ...
        _ => run_buf::<false, true, true, false, 2, 0, 1, 0, true, false, true, false, true, true, true, true, true, true>(challenge, save_solution, &hp),
    }
}
...
>(
    ...
    let mut rounds = 0usize;
    // t4/i23 — PORTAGE de `t1/i12` (mesure -1,795 % de fuel sur `engine_d`, jamais porte ici).
    // Equivalence EXACTE : `check_interval` est calcule une fois (l.1019) et jamais reassigne,
    // `rounds` part de 0 et croit de EXACTEMENT 1 en un seul point (l.1374)
    // => `rounds % ci == 0 && rounds > 0`  <=>  `rounds == next_check`.
    ...
    unsafe {
        loop {
            ...
            if HOIST == 0 {
                ...
                // t4/i23 : `next_check` est le prochain multiple STRICTEMENT apres `rounds`
                // (il vient d'etre avance si le test a tire) => `next_check - rounds` est
                // exactement `ci - rounds % ci`. Une division de moins par tour.
                let n_chk = next_check - rounds;
                let nblk = if HOIST == 1 {
