...
fn run<const SV: u8, const PROBE: bool>(
    ...
    // i6 : `global_flips % RESTART_PERIOD == 0 && > 0` <=> `global_flips == next_restart`.
    // `global_flips` croit de EXACTEMENT 1 par tour (ligne unique d'increment par chemin) donc
    // il passe par TOUS les multiples => equivalence STRICTE, trajectoire identique.
    // Meme patron que celui mesure a -1,795 % de FUEL sur t1 (`engine_d.rs`, en-tete de boucle).
    let mut next_restart: u64 = RESTART_PERIOD;
    ...
    unsafe {
        'main: loop {
            ...
            if global_flips == next_restart {
                next_restart += RESTART_PERIOD;
                if false {   // t5/i9 : bloc mort (rec=0 et sav=0 sur 32/32)
            ...
            // -- i7 HOIST -- bloc de `nblk` flips ou (A) et (D) NE PEUVENT PAS tirer.
            //   (A) `global_flips >= max_flips`  : +1 exactement par tour => borne EXACTE.
            //   (D) `global_flips == next_restart` : `next_restart` n'est modifie que dans le
            //       bloc de restart ci-dessus => borne EXACTE.
            //   (B) `ulen == 0` NON hoistable => re-teste en FIN de bloc via `continue 'main`,
            //       ce qui preserve l'ORDRE d'origine (tete = ulen puis restart).
            // t5/i9 : decoupage RESTAURE (i8 a mesure qu'il vaut -39,7 % de fuel).
            let nblk = (max_flips - global_flips).min(next_restart - global_flips);
            for _ in 0..nblk {

            let cur_unsat = ulen;
            // t5/i8 : la mise a jour de `period_best_unsat` alimentait UNIQUEMENT le bloc de
            // reinitialisation, dont la sonde a montre qu'il n'est JAMAIS entre (compteur `rec`
            // et `sav` a 0 sur 32/32 nonces). Une comparaison + branche PAR FLIP pour rien.
            ...
            // t5/i12 : `vars` est un buffer de 3 CANDIDATS, memsette A CHAQUE FLIP.
            // PREUVE GLOBALE : ses elements sont des indices de variables, donc < nv = 7 500,
            // tres en deca de u32::MAX. `usize` (24 o) -> `u32` (12 o) : memset halve.
            let mut vars = [0u32; 3];
            let mut breaks = [0u8; 3];
            let mut nvars = 0;

            // ═══ t5/i13 — V_c3 : DEROULAGE DU CHEMIN `len == 3` ══════════════════════════
            // Port du levier qui a paye DEUX fois : -3,18 % sur t38 (p = 0,029) et -2,0 % sur t3.
            // `engine_o` avait deja `lp`, `V_polsel` et les arms const-generiques, mais PAS ce
            // deroulage (audit : `t5_audit_engine_o.md`). Levier MEMOIRE : deplier les 3 iterations
            // rend les 3 chaines de gather INDEPENDANTES visibles au compilateur, qui peut les
            // ENTRELACER => MLP => latence masquee. C est ce que reclame un code memory-bound.
            // COUVERTURE : t5 est du 3-SAT, `len == 3` couvre ~99,96 % des clauses (nv = 7 500).
            // EQUIVALENCE : corps VERBATIM, memes indices dans le meme ordre (off, off+1, off+2).
            // Le `break` sur `b == 0` devient un court-circuit par `picked_v == usize::MAX` : dans
            // les DEUX versions, quand `b == 0` on ecrit `vars[nvars]`/`breaks[nvars]`, on pose
            // `picked_v = v`, on N INCREMENTE PAS `nvars`, et les iterations restantes sont sautees.
            // AUCUN tirage RNG dans ce bloc => trajectoire bit-identique => Q doit rester 93 750.
            if len == 3 {
                macro_rules! scan1 { ($idx:expr) => {{
                    let i = $idx;
                    let v = decode_v::<SV>(&cl, &lp, i);
                    *vars.get_unchecked_mut(nvars) = v as u32;
                    ...
                    *breaks.get_unchecked_mut(nvars) = b;
                    if b == 0 { picked_v = v; }
                    if picked_v == usize::MAX { nvars += 1; }
                }}}
                scan1!(off);
                if picked_v == usize::MAX { scan1!(off + 1); }
                if picked_v == usize::MAX { scan1!(off + 2); }
            } else {
            for i in off..end {
                let v = decode_v::<SV>(&cl, &lp, i);
                *vars.get_unchecked_mut(nvars) = v as u32;
            ...
            if picked_v == usize::MAX {
                if nvars == 0 {
                    ...
                    picked_v = *vars.get_unchecked(rng.gen_range(0..nvars)) as usize;
                } else {
                    // (preuve d'équivalence : `engine_n:390-405`, i=0 déroulé + cache `best_age`)
                    let mut best_idx = 0usize;
                    let mut min_b = *breaks.get_unchecked(0);
                    let mut best_age = *var_age.get_unchecked(*vars.get_unchecked(0) as usize);
                    for i in 1..nvars {
                        let b = *breaks.get_unchecked(i);
                        let age = *var_age.get_unchecked(*vars.get_unchecked(i) as usize);
                    ...
                    picked_v = *vars.get_unchecked(best_idx) as usize;
                }
            }
            ...
            *assignment.get_unchecked_mut(picked_v) = new_val;
            ...
            if cur_unsat < best_unsat {
                ...
                if !PROBE {
                    best_assignment.copy_from_slice(&assignment);
                    let _ = save_solution(&Solution { variables: best_assignment.clone() });
                    if cur_unsat == 0 { break 'main; }
                }
            }
            if ulen == 0 { continue 'main; }
...
unsafe fn rmw_inc(
    ...
    for k in start..stop {
        ...
        *tlc.get_unchecked_mut(c_idx) = trues + 1;
        // F5 : branchless STRUCTUREL rebranche — les ecritures ubuf/cpos etaient
        // INCONDITIONNELLES (poubelle si !cond) = travail memoire paye pour rien.
        if trues == 0 {
...
unsafe fn rmw_dec(
    ...
    for k in start..stop {
        ...
        let trues = *tlc.get_unchecked(c_idx);
        *tlc.get_unchecked_mut(c_idx) = trues - 1;
        // F5 : idem, ecritures rendues CONDITIONNELLES.
        if trues == 1 {
            *ulen += 1;
            let wpos = *ulen;
            *ubuf.get_unchecked_mut(wpos) = c_idx;
            *cpos.get_unchecked_mut(c_idx) = wpos;
        }
    }
}
