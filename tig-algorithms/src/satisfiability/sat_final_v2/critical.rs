...
pub fn solve_track_5_impl(
    ...
    let mut seen: Vec<u32> = vec![0u32; challenge.num_variables * 2];
    let mut epoch: u32 = 1;
    ...
    while i > 0 {
        ...
        if clause.len() > 1 {
            ...
            for read in 0..clause.len() {
                let lit = unsafe { *clause.get_unchecked(read) };
                let var = (lit.abs() as usize) - 1;
                let sign = (lit < 0) as usize;
                let idx = var * 2 + sign;
                let opp = var * 2 + (1 ^ sign);

                unsafe {
                    if *seen.get_unchecked(opp) == epoch {
                        is_tautology = true;
                        break;
                    }
                    if *seen.get_unchecked(idx) == epoch {
                        continue;
                    }
                    *seen.get_unchecked_mut(idx) = epoch;
                    *clause.get_unchecked_mut(write) = lit;
                }
                write += 1;
            }
    ...
    let mut pos_seen = vec![false; challenge.num_variables];
    let mut neg_seen = vec![false; challenge.num_variables];
    ...
    while !dead {
        ...
        for x in pos_seen.iter_mut() { *x = false; }
        for x in neg_seen.iter_mut() { *x = false; }
        for c in &clauses {
            for &l in c.iter() {
                let idx = (l.abs() - 1) as usize;
                if l > 0 { pos_seen[idx] = true; } else { neg_seen[idx] = true; }
            }
        }

        for v in 0..challenge.num_variables {
            if p_single[v] && n_single[v] { dead = true; break; }
            if !p_single[v] && !n_single[v] {
                if pos_seen[v] && !neg_seen[v] { p_single[v] = true; done = false; }
                else if neg_seen[v] && !pos_seen[v] { n_single[v] = true; done = false; }
            }
        }
    ...
    for c in clauses.iter() {
        for &l in c.iter() {
            ...
            if l > 0 { p_cnt[v] += 1; } else { n_cnt[v] += 1; }
        }
    }
    ...
    let mut last_best_residual = usize::MAX;
    let mut no_improve_streak = 0usize;
    let plateau_floor = 50usize;
    ...
    unsafe {
        loop {
            ...
            if rounds % check_interval == 0 && rounds > unsat_check_threshold {
                ...
                let plateau_limit = (max_num_rounds / check_interval.max(1)) / 5;
                if min_residual_seen < last_best_residual {
                    last_best_residual = min_residual_seen;
                    no_improve_streak = 0;
                } else if fuel_used_pct > 0.30 {
                    no_improve_streak += 1;
                    if no_improve_streak >= plateau_limit && min_residual_seen > plateau_floor {
                        return Ok(());
                    }
                }
            }
                ...
                if progress <= 0 {
                    ...
                    if stagnation >= stagnation_limit {
                        ...
                        for _ in 0..kicks {
                            ...
                            let inc = if was_true { occ_slice(&n_flat, &n_off, v) } else { occ_slice(&p_flat, &p_off, v) };
                            let dec = if was_true { occ_slice(&p_flat, &p_off, v) } else { occ_slice(&n_flat, &n_off, v) };

                            for &cid2_u in inc {
                                let cid2 = cid2_u as usize;
                                let shift = (cid2 & 3) << 1;
                                *num_good_so_far.get_unchecked_mut(cid2 >> 2) += 1u8 << shift;
                            }
                            for &cid2_u in dec {
                                let cid2 = cid2_u as usize;
                                let shift = (cid2 & 3) << 1;
                                let byte_ptr = num_good_so_far.get_unchecked_mut(cid2 >> 2);
                                let before = (*byte_ptr >> shift) & 3;
                                *byte_ptr -= 1u8 << shift;
                                if before == 1 && *in_queue.get_unchecked(cid2) == 0u8 {
            ...
            if !residual_.is_empty() {
                ...
                while !residual_.is_empty() {
                    ...
                    let mut best_id = if clauses.get_unchecked(cid2).len() < clauses.get_unchecked(cid1).len() { id2 } else { id1 };
                    let id3 = rng.gen::<usize>() % residual_.len();
                    let cid3 = residual_[id3] as usize;
                    let best_cid = residual_[best_id] as usize;
                    if clauses.get_unchecked(cid3).len() < clauses.get_unchecked(best_cid).len() { best_id = id3; }

                    i_u32 = residual_[best_id];
                    let i = i_u32 as usize;
                    if (*num_good_so_far.get_unchecked(i >> 2) >> ((i & 3) << 1)) & 3 > 0 {
                        in_queue[i] = 0u8;
                ...
                'outer: for &l in c.iter() {
                    ...
                    for &cid_u in clauses_to_check {
                        let c2 = cid_u as usize;
                        if (*num_good_so_far.get_unchecked(c2 >> 2) >> ((c2 & 3) << 1)) & 3 == 1 {
                            continue 'outer;
                        }
                    }
                ...
                let v = if let Some(abs_l) = zero_found {
                    ...
                    for &l in c.iter() {
                        ...
                        let appearances = (*p_off.get_unchecked(abs_l + 1) - *p_off.get_unchecked(abs_l)
                            + *n_off.get_unchecked(abs_l + 1) - *n_off.get_unchecked(abs_l)) as usize;

                        if sad == 0 {
                            if min_sad > 0 || appearances < min_weight {
                                min_sad = 0;
                                min_weight = appearances;
                                v_min_sad = abs_l;
                            }
                        } else if min_sad > 0 {
                            let combined_weight = sad * sad * 1024 + appearances;
                ...
                let was_true = *variables.get_unchecked(v);
                let clauses_to_decrement = if was_true { occ_slice(&p_flat, &p_off, v) } else { occ_slice(&n_flat, &n_off, v) };
                let clauses_to_increment = if was_true { occ_slice(&n_flat, &n_off, v) } else { occ_slice(&p_flat, &p_off, v) };

                for &cid_u in clauses_to_increment {
                    let cid = cid_u as usize;
                    let shift = (cid & 3) << 1;
                    *num_good_so_far.get_unchecked_mut(cid >> 2) += 1u8 << shift;
                }

                for &cid_u in clauses_to_decrement {
                    let cid = cid_u as usize;
                    let shift = (cid & 3) << 1;
                    let byte_ptr = num_good_so_far.get_unchecked_mut(cid >> 2);
                    let before = (*byte_ptr >> shift) & 3;
                    *byte_ptr -= 1u8 << shift;
                    if before == 1 && *in_queue.get_unchecked(cid) == 0u8 {
