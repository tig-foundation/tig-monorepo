...
pub fn solve_track_2_clause_activity_impl(
    ...
    // Stochastic local-search SAT solver with clause-activity guidance.
    // Mutation: speed up preprocessing by sorting literals per clause to deduplicate and detect tautologies in linear scan.
    let _ = seeded_hasher;
    ...
    while i > 0 {
        ...
        // Sort by (abs(lit), lit) so duplicates/opposites are adjacent.
        clause.sort_unstable_by(|a, b| {
            let aa = a.abs();
            let bb = b.abs();
            if aa == bb { a.cmp(b) } else { aa.cmp(&bb) }
        });

        // In-place unique + tautology detection in one pass.
        let mut out_len = 0usize;
        let mut prev_abs: i32 = 0;
        let mut prev_lit: i32 = 0;
        let mut first = true;
        let mut taut = false;

        // Fix borrow checker: iterate over a snapshot of literals, then write back in-place.
        let lits_snapshot: Vec<i32> = clause.to_vec();
        for lit in lits_snapshot.into_iter() {
            let a = lit.abs();
            if first {
                clause[out_len] = lit;
                out_len += 1;
            ...
            if a == prev_abs {
                if lit == prev_lit {
                    ...
                    // Opposite signs with same variable => tautology.
                    taut = true;
                    break;
                }
            } else {
