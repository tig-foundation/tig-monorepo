...
fn build_var_appearances_if_needed(
    enabled: bool,
    nv: usize,
    p_cnt: &[u32],
    n_cnt: &[u32],
...
#[inline(always)]
unsafe fn bump_high_var_age(var: usize, var_age: &mut [u16]) {
    let age = var_age.get_unchecked_mut(var);
    *age = age.saturating_add(1);
}

#[inline(always)]
unsafe fn bump_high_clause_var_ages(cs: usize, ce: usize, cl: &[i32], var_age: &mut [u16]) {
    for j in cs..ce {
        let lit = *cl.get_unchecked(j);
        bump_high_var_age(lit_var_index(lit), var_age);
    }
}

#[inline(always)]
unsafe fn bump_high_clause_var_ages3(cs: usize, cl: &[i32], var_age: &mut [u16]) {
    debug_assert!(cs + 2 < cl.len());
    bump_high_var_age(lit_var_index(*cl.get_unchecked(cs)), var_age);
    bump_high_var_age(lit_var_index(*cl.get_unchecked(cs + 1)), var_age);
    bump_high_var_age(lit_var_index(*cl.get_unchecked(cs + 2)), var_age);
}

#[inline(always)]
unsafe fn remove_residual_at_exact(residual: &mut Vec<u32>, residual_pos: &mut [u32], rid: usize) {
    let c = *residual.get_unchecked(rid) as usize;
    let last_idx = residual.len() - 1;
    if rid != last_idx {
        let last_c = *residual.get_unchecked(last_idx) as usize;
        *residual.get_unchecked_mut(rid) = last_c as u32;
        *residual_pos.get_unchecked_mut(last_c) = rid as u32;
    }
    residual.set_len(last_idx);
    *residual_pos.get_unchecked_mut(c) = u32::MAX;
}

#[inline(always)]
unsafe fn remove_residual_clause_exact(
    ...
    if pos != u32::MAX {
        remove_residual_at_exact(residual, residual_pos, pos as usize);
    }
}

fn clear_exact_residual_positions(residual: &[u32], residual_pos: &mut [u32]) {
    debug_assert!(residual
        .iter()
        .enumerate()
        .all(|(idx, &cid)| { residual_pos.get(cid as usize).copied() == Some(idx as u32) }));
    for &cid in residual {
        residual_pos[cid as usize] = u32::MAX;
    }
    debug_assert!(residual_pos.iter().all(|&pos| pos == u32::MAX));
}
...
pub fn solve(
    ...
    debug_assert_eq!(
        all_three_clauses,
        high_clause_offsets_are_three(nc, co, cl.len())
    );
    ...
    let (greedy_clause_order, greedy_all_three_clauses) = if use_greedy_init {
        if all_three_clauses {
            ...
            build_greedy_clause_order_with_all_three(nc, co)
        }
    } else {
        (Vec::new(), false)
    };
    ...
    let var_appearances = build_var_appearances_if_needed(!use_probsat_pick, nv, &p_cnt, &n_cnt);
    let probsat_break = if use_probsat_pick {
        build_probsat_break_weights(avg_clause_size, &p_cnt, &n_cnt)
    } else {
        Vec::new()
    };
    ...
    unsafe {
        loop {
                    ...
                    if stagnation >= 4 {
                        for _ in 0..3 {
                            ...
                            if *num_good.get_unchecked(pcid) > 0 {
                                remove_residual_at_exact(&mut residual, &mut residual_pos, rid);
                                continue;
                            }
                            let (pcs, pce) =
                                high_clause_bounds_unchecked(pcid, co, all_three_clauses);
                            if pcs == pce {
                                continue;
                            }
            ...
            let (cs, ce) = high_clause_bounds_unchecked(cid, co, all_three_clauses);
            let clen = ce - cs;
            ...
            let v_idx = if use_probsat_pick {
                if all_three_clauses {
                    ...
                    choose_high_probsat_var_generic(
                        cs,
                        ce,
                        cl,
                        &vars,
                ...
                if all_three_clauses {
                    ...
                    choose_high_non_probsat_var_generic(
                        cs,
                        ce,
                        cl,
                        &vars,
            ...
            for k in is..ie {
                ...
                let good = *num_good.get_unchecked(c);
                if good == 0 {
                    remove_residual_clause_exact(&mut residual, &mut residual_pos, c);
                }
                *num_good.get_unchecked_mut(c) = good.saturating_add(1);
            }
            for k in ds..de {
            ...
            *vars.get_unchecked_mut(v_idx) = !was_true;
            if !use_probsat_pick {
                *var_age.get_unchecked_mut(v_idx) = 0;
                if all_three_clauses {
                    bump_high_clause_var_ages3(cs, cl, &mut var_age);
                } else {
                    bump_high_clause_var_ages(cs, ce, cl, &mut var_age);
                }
            }
...
#[inline(always)]
unsafe fn high_break_sad_capped(
    v: usize,
    cap: usize,
    vars: &[bool],
    all_off: &[u32],
...
#[inline(always)]
unsafe fn consider_high_non_probsat_candidate(
    v: usize,
    vars: &[bool],
    all_off: &[u32],
    p_bound: &[u32],
    ...
    let sad = high_break_sad_capped(v, *min_sad, vars, all_off, p_bound, all_data, num_good);
    if sad == 0 {
...
#[inline(always)]
unsafe fn choose_high_non_probsat_var_generic(
    cs: usize,
    ce: usize,
    cl: &[i32],
    vars: &[bool],
    ...
    for j in cs..ce {
        ...
        if high_break_is_zero(abs_l, vars, all_off, p_bound, all_data, num_good) {
            return abs_l;
        }
    }
    ...
    for j in cs..ce {
        let abs_l = lit_var_index(*cl.get_unchecked(j));
        consider_high_non_probsat_candidate(
            abs_l,
            vars,
            all_off,
            p_bound,
...
#[inline(always)]
unsafe fn choose_high_non_probsat_var3(
    cs: usize,
    cl: &[i32],
    vars: &[bool],
    all_off: &[u32],
    ...
    if high_break_is_zero(v0, vars, all_off, p_bound, all_data, num_good) {
        return v0;
    }

    let v1 = lit_var_index(*cl.get_unchecked(cs + 1));
    if high_break_is_zero(v1, vars, all_off, p_bound, all_data, num_good) {
        return v1;
    }

    let v2 = lit_var_index(*cl.get_unchecked(cs + 2));
    if high_break_is_zero(v2, vars, all_off, p_bound, all_data, num_good) {
        return v2;
    }
...
#[inline(always)]
unsafe fn choose_high_probsat_var_generic(
    cs: usize,
    ce: usize,
    cl: &[i32],
    vars: &[bool],
    ...
    for idx in 0..limit {
        ...
        let sad = high_break_sad(abs_l, vars, all_off, p_bound, all_data, num_good);
...
#[inline(always)]
unsafe fn choose_high_probsat_var3(
    cs: usize,
    cl: &[i32],
    vars: &[bool],
    all_off: &[u32],
    ...
    let v0 = lit_var_index(*cl.get_unchecked(cs));
    let sad0 = high_break_sad(v0, vars, all_off, p_bound, all_data, num_good);
    if sad0 == 0 {
        return v0;
    }

    let v1 = lit_var_index(*cl.get_unchecked(cs + 1));
    let sad1 = high_break_sad(v1, vars, all_off, p_bound, all_data, num_good);
    if sad1 == 0 {
        return v1;
    }

    let v2 = lit_var_index(*cl.get_unchecked(cs + 2));
    let sad2 = high_break_sad(v2, vars, all_off, p_bound, all_data, num_good);
    if sad2 == 0 {
        return v2;
    }

    let w0 = *probsat_break.get_unchecked(sad0.min(probsat_break_limit));
    let w1 = *probsat_break.get_unchecked(sad1.min(probsat_break_limit));
    let w2 = *probsat_break.get_unchecked(sad2.min(probsat_break_limit));
...
mod tests {
    ...
    #[test]
    fn high_lit_var_index_matches_abs_index_for_valid_literals() {
    ...
    #[test]
    fn high_var_appearances_only_builds_when_needed() {
        let p_cnt = vec![1_u32, 4, 0, 70_000];
        let n_cnt = vec![2_u32, 1, 7, 60_000];

        assert!(build_var_appearances_if_needed(false, 10_000, &p_cnt, &n_cnt).is_empty());
        let appearances: Vec<u32> = build_var_appearances_if_needed(true, 4, &p_cnt, &n_cnt);
        assert_eq!(appearances, vec![3_u32, 5, 7, 130_000]);
    }

    #[test]
    fn high_var_age_only_builds_when_needed() {
        assert!(build_var_age_if_needed(false, 10_000).is_empty());
        assert_eq!(build_var_age_if_needed(true, 4), vec![0_u16; 4]);
    }

    #[test]
    fn bump_high_clause_var_ages3_matches_generic_loop() {
        let cl = [99, 1, -2, 2, 100];
        let mut generic_age = [5_u16, u16::MAX - 1, 17];
        let mut fast_age = generic_age;

        unsafe {
            bump_high_clause_var_ages(1, 4, &cl, &mut generic_age);
            bump_high_clause_var_ages3(1, &cl, &mut fast_age);
        }
    ...
    #[test]
    fn high_weighted3_selector_matches_array_reference() {
    ...
    #[test]
    fn high_probsat_break_weights_stop_at_occurrence_bound() {
    ...
    #[test]
    fn high_probsat_var3_matches_generic_and_rng_consumption() {
        let cl = vec![1, -2, 3];
        let vars = vec![true, false, true];
        let all_off = [0_u32, 2, 4, 6];
        let p_bound = [1_u32, 3, 5];
        let all_data = [0_u32, 3, 0, 2, 1, 3];
        let probsat_break = vec![1.0_f64, 0.5, 0.25, 0.125];
        ...
        for num_good in num_good_cases {
            for seed in 0_u64..16 {
                ...
                assert_eq!(fast, generic, "seed={seed} num_good={num_good:?}");
                assert_eq!(
                    fast_rng.gen::<u64>(),
                    generic_rng.gen::<u64>(),
                    "seed={seed} num_good={num_good:?}"
                );
            }
        }
    }
    ...
    #[test]
    fn high_non_probsat_var3_matches_generic_and_rng_consumption() {
        fn occurrence_fixture(sads: [usize; 3]) -> (Vec<u32>, Vec<u32>, Vec<u32>, Vec<u8>) {
            let mut all_off = Vec::with_capacity(4);
            let mut p_bound = Vec::with_capacity(3);
            let mut all_data = Vec::new();
            let mut num_good = Vec::new();
            all_off.push(0);

            for sad in sads {
                p_bound.push(all_data.len() as u32);
                for _ in 0..sad {
                    all_data.push(num_good.len() as u32);
                    num_good.push(1);
                }
                all_off.push(all_data.len() as u32);
            }

            (all_off, p_bound, all_data, num_good)
        }

        let cl = vec![1, -2, 3];
        let vars = vec![false, false, false];
        let var_appearances = vec![120_u32, 80, 60];
        let var_age = vec![0_u16, 8, 16];
        let sad_cases = [
            [0_usize, 2, 3],
            [2, 0, 3],
            [2, 3, 0],
        ...
        for sads in sad_cases {
            let (all_off, p_bound, all_data, num_good) = occurrence_fixture(sads);
            for current_prob in [0.0_f64, 0.37, 1.0] {
                for seed in 0_u64..24 {
                    ...
                    assert_eq!(
                        fast, generic,
                        "seed={seed} current_prob={current_prob} sads={sads:?}"
                    );
                    assert_eq!(
                        fast_rng.gen::<u64>(),
                        generic_rng.gen::<u64>(),
    ...
    #[test]
    fn high_pair_occurrence_bound_matches_zip_max_reference() {
        assert_eq!(
    ...
    #[test]
    fn high_direct_assignment_fill_matches_legacy_rng_path() {
    ...
    #[test]
    fn high_greedy_init_is_only_enabled_by_explicit_hp() {
        let hp = Hyperparameters::default();

        assert!(!target_greedy_init_enabled(10_000, 4.267, 513, &hp));
        assert!(!target_greedy_init_enabled(10_000, 4.267, 514, &hp));
        assert!(!target_greedy_init_enabled(7_500, 4.267, 591, &hp));
    ...
    #[test]
    fn greedy_shortest_all_three_fast_path_matches_generic_reference() {
        let nv = 6;
    ...
    #[test]
    fn high_greedy_clause_order_keeps_u32_stable_order() {
    ...
    #[test]
    fn high_greedy_clause_order_falls_back_for_long_clauses() {
    ...
    #[test]
    fn high_greedy_clause_order_all_three_literal_returns_natural_order() {
    ...
    #[test]
    fn high_clause_bounds_all_three_fast_path_matches_offsets() {
        let co = [0_u32, 3, 6, 9, 12];
        assert!(high_clause_offsets_are_three(4, &co, 12));
        for cid in 0..4 {
            assert_eq!(
                unsafe { high_clause_bounds_unchecked(cid, &co, true) },
    ...
    #[test]
    fn high_clause_offsets_reject_mixed_lengths_with_average_three() {
        let co = [0_u32, 2, 5, 9];
        assert!(!high_clause_offsets_are_three(3, &co, 9));
        assert_eq!(
            unsafe { high_clause_bounds_unchecked(1, &co, false) },
            (2, 5)
        );
    }
    ...
    fn rebuild_state_overwrites_existing_counts_without_prefill() {
        ...
        let mut residual = vec![0_u32, 1];
        let mut residual_pos = vec![0_u32, 1, u32::MAX];
    ...
    #[test]
    fn high_clause_good_count_matches_generic_reference() {
        ...
        for (s, e, expected) in [(0, 0, 0_u8), (0, 1, 1), (0, 3, 3), (3, 5, 0), (5, 10, 3)] {
            ...
            assert_eq!(high_clause_good_count(s, e, &cl, &vars), expected);
        }
    }

    #[test]
    fn high_clause_has_satisfied_matches_generic_reference() {
        ...
        for (s, e, expected) in [
            ...
            assert_eq!(generic, expected);
            assert_eq!(high_clause_has_satisfied(s, e, &cl, &vars), expected);
        }
    }

    #[test]
    fn remove_residual_at_exact_updates_moved_clause_position() {
        let mut residual = vec![2_u32, 5, 7];
        let mut residual_pos = vec![u32::MAX; 8];
        residual_pos[2] = 0;
        residual_pos[5] = 1;
        residual_pos[7] = 2;

        unsafe {
            remove_residual_at_exact(&mut residual, &mut residual_pos, 1);
        }

        assert_eq!(residual, vec![2, 7]);
        assert_eq!(residual_pos[2], 0);
        assert_eq!(residual_pos[5], u32::MAX);
        assert_eq!(residual_pos[7], 1);
    }

    #[test]
    fn remove_residual_at_exact_handles_last_entry_without_moving() {
        let mut residual = vec![2_u32, 5, 7];
        let mut residual_pos = vec![u32::MAX; 8];
        residual_pos[2] = 0;
        residual_pos[5] = 1;
        ...
        unsafe {
            remove_residual_at_exact(&mut residual, &mut residual_pos, 2);
        }

        assert_eq!(residual, vec![2_u32, 5]);
        assert_eq!(residual_pos[2], 0);
        assert_eq!(residual_pos[5], 1);
        assert_eq!(residual_pos[7], u32::MAX);

        unsafe {
            remove_residual_at_exact(&mut residual, &mut residual_pos, 0);
        }

        assert_eq!(residual, vec![5_u32]);
        assert_eq!(residual_pos[2], u32::MAX);
        assert_eq!(residual_pos[5], 0);
        assert_eq!(residual_pos[7], u32::MAX);
    }

    #[test]
    fn remove_residual_clause_exact_ignores_absent_and_uses_position_table() {
        let mut residual = vec![2_u32, 5, 7];
        let mut residual_pos = vec![u32::MAX; 8];
        residual_pos[2] = 0;
        residual_pos[5] = 1;
        ...
        unsafe {
            remove_residual_clause_exact(&mut residual, &mut residual_pos, 4);
        }

        assert_eq!(residual, vec![2_u32, 5, 7]);
        assert_eq!(residual_pos[4], u32::MAX);

        unsafe {
            remove_residual_clause_exact(&mut residual, &mut residual_pos, 7);
        }
        ...
        unsafe {
            remove_residual_clause_exact(&mut residual, &mut residual_pos, 2);
        }
    ...
    #[test]
    fn rebuild_state_fresh_matches_clearing_rebuild_on_clean_sidecars() {
        let co = [0_u32, 2, 4, 5];
        let cl = [1, -2, -1, 2, 3];
        let vars = [true, true, false];

        let mut fresh_num_good = vec![9_u8; 3];
        let mut fresh_residual = Vec::new();
        ...
        let mut clearing_num_good = vec![9_u8; 3];
        let mut clearing_residual = vec![0_u32, 1];
        ...
        rebuild_state(
            ...
            &mut clearing_residual,
            &mut clearing_pos,
            false,
        );

        assert_eq!(fresh_num_good, clearing_num_good);
        assert_eq!(fresh_num_good, vec![1, 1, 0]);
        assert_eq!(fresh_residual, clearing_residual);
        assert_eq!(fresh_residual, vec![2]);
        assert_eq!(fresh_pos, clearing_pos);
        assert_eq!(fresh_pos, vec![u32::MAX, u32::MAX, 0]);
    }

    #[test]
    fn rebuild_state_all_three_fast_path_matches_generic_reference() {
        let nc = 6;
        let co = [0_u32, 3, 6, 9, 12, 15, 18];
        let cl = [
            1, -2, 3, -1, 2, -3, 1, 2, 3, -1, -2, -3, 2, -3, 4, -2, 3, -4,
        ];
        let vars = [true, false, false, true];

        let mut expected_good = vec![0_u8; nc];
        let mut expected_residual = Vec::new();
        let mut expected_pos = vec![u32::MAX; nc];
        for c in 0..nc {
            let good = high_clause_good_count(co[c] as usize, co[c + 1] as usize, &cl, &vars);
            expected_good[c] = good;
            if good == 0 {
                expected_pos[c] = expected_residual.len() as u32;
                expected_residual.push(c as u32);
            }
        }

        let mut actual_good = vec![9_u8; nc];
        let mut actual_residual = vec![1_u32, 3, 5];
...
fn target_greedy_init_enabled(
    ...
    if let Some(enabled) = hp.target_greedy_init {
        return enabled;
    }
...
fn initial_assignment_greedy_shortest(
    ...
    let mut vars = Vec::with_capacity(nv);
    let (clause_order, all_three_clauses) = build_greedy_clause_order_with_all_three(nc, co);
    initial_assignment_greedy_shortest_with_order_into(
        nv,
        p_cnt,
        n_cnt,
...
fn build_greedy_clause_order(nc: usize, co: &[u32]) -> Vec<u32> {
    build_greedy_clause_order_with_all_three(nc, co).0
}

fn build_greedy_clause_order_with_all_three(nc: usize, co: &[u32]) -> (Vec<u32>, bool) {
    ...
    if counts[1] == 0 && counts[2] == 0 && total == nc {
        return ((0..nc as u32).collect(), true);
    }
...
#[inline(always)]
fn high_clause_offsets_are_three(nc: usize, co: &[u32], cl_len: usize) -> bool {
...
#[inline(always)]
unsafe fn high_clause_bounds_unchecked(
    cid: usize,
    co: &[u32],
    all_three_clauses: bool,
) -> (usize, usize) {
...
fn initial_assignment_greedy_shortest_with_order_into(
    ...
    if all_three_clauses {
        ...
        for cid in 0..(cl.len() / 3) {
            let s = cid * 3;
            let e = s + 3;
            if high_clause_has_satisfied(s, e, cl, vars) {
                continue;
            }
...
fn rebuild_state_impl<const CLEAR_STATE: bool>(
    nc: usize,
    co: &[u32],
    cl: &[i32],
    vars: &[bool],
    ...
    debug_assert!(residual_pos.len() >= nc);
    if CLEAR_STATE {
        clear_exact_residual_positions(residual, residual_pos);
        residual.clear();
    } else {
        debug_assert!(residual.is_empty());
        debug_assert!(residual_pos[..nc].iter().all(|&pos| pos == u32::MAX));
    }
    debug_assert!(!all_three_clauses || high_clause_offsets_are_three(nc, co, cl.len()));
    if all_three_clauses {
        ...
        for i in 0..nc {
            let good = high_clause3_good_count(i * 3, cl, vars);
            num_good[i] = good;
            if good == 0 {
    ...
    for i in 0..nc {
        ...
        let good = high_clause_good_count(s, e, cl, vars);
        num_good[i] = good;
        if good == 0 {
            residual_pos[i] = residual.len() as u32;
            residual.push(i as u32);
...
#[inline(always)]
fn high_clause3_good_count(s: usize, cl: &[i32], vars: &[bool]) -> u8 {
...
#[inline(always)]
fn high_clause_good_count(s: usize, e: usize, cl: &[i32], vars: &[bool]) -> u8 {
    match e - s {
...
#[inline(always)]
fn high_clause_has_satisfied(s: usize, e: usize, cl: &[i32], vars: &[bool]) -> bool {
    match e - s {
