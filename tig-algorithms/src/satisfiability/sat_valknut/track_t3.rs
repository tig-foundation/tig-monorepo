...
pub fn solve(
    ...
    let max_random_prob = hp.max_prob.unwrap_or(0.95);
    let prob_adjustment_factor = 0.035;
    ...
    let perturbation_flips = hp.perturbation_flips
        .unwrap_or(1 + (2.5 * size_scale) as usize);
    let stagnation_limit = hp.stagnation_limit
        .unwrap_or(3 + (2.0 * (1.0 - (density / 5.0).min(1.0))) as usize);
