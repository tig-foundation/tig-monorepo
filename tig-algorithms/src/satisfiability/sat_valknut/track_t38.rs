...
pub fn solve(
    ...
    let stagnation_limit = hp.stagnation_limit
        .unwrap_or(3 + (2.0 * (1.0 - (density / 5.0).min(1.0))) as usize);  // Increased base stagnation limit
    ...
    unsafe {
        loop {
            ...
            if stagnation >= stagnation_limit {
                let kicks = if stagnation >= 6 {
                    (perturbation_flips * 15).min(150)
                } else if stagnation >= 4 {
                    (perturbation_flips * 8).min(80)
                } else if stagnation >= 3 {
                    (perturbation_flips * 4).min(30)
                } else {
                    (perturbation_flips + 3).min(12)
                };
