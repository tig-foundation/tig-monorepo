...
pub fn solve(
    ...
    for v in 0..nv {
        ...
        // Weighted initial assignment with slight bias towards satisfying more clauses
        let total = np + nn;
        if total > 0 {
            let pos_ratio = np as f64 / total as f64;
            // Apply sigmoid-like bias to favor assignments that satisfy more clauses
            let bias = 0.5 + 0.5 * (pos_ratio - 0.5) * 2.0;
            vars[v] = rng.gen_bool(bias.clamp(0.4, 0.6));
        } else {
    ...
    let final_vars = if best_unsat == 0 { best_vars } else { vars };
