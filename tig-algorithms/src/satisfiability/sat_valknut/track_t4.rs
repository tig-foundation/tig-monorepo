...
pub fn solve(
    ...
    for v in 0..nv {
        ...
        if vad <= nad {
            ...
            // Apply stronger bias for better initial satisfaction
            let adjusted_prob = if prob > 0.5 { (prob + 0.1).min(0.95) } else { (prob - 0.1).max(0.05) };
            vars[v] = rng.gen_bool(adjusted_prob);
        }
    }
    ...
    let mut w = vec![1u8; nc];
    // Adaptive weight initialization based on clause satisfaction potential
    for c in 0..nc {
        let s = co[c] as usize;
        let e = co[c + 1] as usize;
        ...
        for j in s..e {
            if cl[j] > 0 { pos_count += 1; } else { neg_count += 1; }
        }
        // Give slightly higher weight to clauses with more balanced literals
        if pos_count.abs_diff(neg_count) <= 1 { w[c] = 3; } else if pos_count.abs_diff(neg_count) <= 2 { w[c] = 2; }
    }
    ...
    const REINIT_STAGNATION: usize = 1_000_000;
    ...
    // Use best found assignment instead of current
