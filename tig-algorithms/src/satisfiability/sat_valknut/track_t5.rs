...
pub fn solve(
    ...
    const WALK_P: f64 = 0.55;
    const RESTART_PERIOD: u64 = 45_000_000;
    const REINIT_MIN_UNSAT: usize = 15;
    ...
    unsafe {
        loop {
            ...
            if global_flips > 0 && global_flips % RESTART_PERIOD == 0 {
                if period_best_unsat >= REINIT_MIN_UNSAT && best_unsat > 0 {
            ...
            // Dynamic clause selection with slight randomness
            ...
            // Randomize clause representation for better exploration
            if len > 1 {
                let ri = rng.gen::<usize>() % len;
                cl.swap(off, off + ri);
            }
