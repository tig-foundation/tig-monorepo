...
/// Preprocesses clauses into compact incidence arrays and dispatches by density regime.
pub fn solve_challenge(
...

    {
        ...
        for ci in 0..nc {
            let s = co[ci] as usize;
            let e = co[ci + 1] as usize;
            for j in s..e {
                let lit = cl[j];
                let v = (lit.abs() - 1) as usize;
                if lit > 0 {
                    p_data[p_pos[v] as usize] = ci as u32;
                    p_pos[v] += 1;
                } else {
                    n_data[n_pos[v] as usize] = ci as u32;
                    n_pos[v] += 1;
                }
            }
        }
...
/// Runs a high-density WalkSAT search with focused unsatisfied-clause sampling and break-based literal choice.
fn solve_high_density(
    ...
    unsafe {
        loop {
            ...
            let clen = ce - cs;
            let ri = if clen > 1 { rand_val % clen } else { 0 };
            let first_j = cs + ri;
            let first_lit = *cl.get_unchecked(first_j);

            let mut zero_found: Option<usize> = None;
            'outer_h: for off in 0..clen {
                let j = cs + if off == 0 { ri } else if off < ri { off } else if off == ri { 0 } else { off };
            ...
            let v_idx = if let Some(v) = zero_found {
                ...
                (first_lit.abs() - 1) as usize
            } else {
                let mut min_sad = usize::MAX;
                let mut v_min = (first_lit.abs() - 1) as usize;
...
/// Runs a low-density WalkSAT search with packed counts, focused unsatisfied-clause sampling, and break-based literal choice.
