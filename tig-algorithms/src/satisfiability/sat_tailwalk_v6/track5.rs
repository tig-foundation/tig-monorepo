...
pub fn solve(
    ...
    let base_fuel =
        (2000.0 + 100.0 * difficulty_factor) * (nv as f64).sqrt() * scale_factor;
    ...
    let random_threshold =
        0.003 + 0.007 / (1.0 + (-(nvf - 30000.0) / 8000.0).exp());
    ...
    unsafe {
        loop {
            ...
            if countdown == 0 {
                ...
                let progress =
                    last_check_residual as i64 - unsat_list.len() as i64;
                ...
                if progress <= 0 {
                    ...
                    if stagnation >= stagnation_limit {
                        ...
                        for _ in 0..kicks {
                            ...
                            let pcid =
                                *unsat_list.get_unchecked(rid) as usize;
                            ...
                            let lit = *cl.get_unchecked(
                                pcs + rng.gen::<usize>() % (pce - pcs),
                            );
                            ...
                            for k in is..ie {
                                let c =
                                    *all_data.get_unchecked(k) as usize;
                                let shift = (c & 3) << 1;
                                let byte_idx = c >> 2;
                                let old = (*num_good
                                    ...
                                    >> shift)
                                    & 3;
                                *num_good.get_unchecked_mut(byte_idx) +=
                                    1u8 << shift;
                                if old == 0 {
                                    let pos =
                                        *unsat_pos.get_unchecked(c) as usize;
                                    let last_idx = unsat_list.len() - 1;
                                    let last_c = *unsat_list
                                        .get_unchecked(last_idx)
                                        as usize;
                                    *unsat_list.get_unchecked_mut(pos) =
                                        last_c as u32;
                                    *unsat_pos.get_unchecked_mut(last_c) =
                                        pos as u32;
            ...
            let cid =
                *unsat_list.get_unchecked(rand_val % unsat_list.len()) as usize;
            ...
            let v_idx = if zero_cnt > 0 {
                ...
                for idx in 0..limit {
                    ...
                    let score =
                        *probs_break.get_unchecked(sad.min(255));
                    sum_scores += score;
                ...
                for idx in 0..limit {
                    ...
                    if accum >= threshold {
                        v_sel =
                            (cl.get_unchecked(cs + idx).abs() - 1) as usize;
                        break;
                    }
                }
                v_sel
