...
pub fn solve_challenge(
    ...
    if density >= 4.25 {
        if nv <= 5000 {
            return solve_high_density_sparse(&hp, &mut rng, nv, nc, density, p_cnt, n_cnt, &all_off, &p_bound, &all_data, &mut cl, &co, save_solution);
        }
        return solve_high_density(&hp, &mut rng, nv, nc, density, p_cnt, n_cnt, &all_off, &p_bound, &all_data, &mut cl, &co, save_solution);
    }

    solve_low_density(&hp, &mut rng, nv, nc, density, p_cnt, n_cnt, &all_off, &p_bound, &all_data, &mut cl, &co, save_solution)
}
