...
fn solve_high_density(
    ...
    let nad = 1.0;
    let random_threshold = if num_variables >= 30000 { 0.0075 } else { 0.0025 };
    ...
    let max_random_prob = 0.82;
    let prob_adjustment_factor = 0.020;
    let smoothing_factor = 0.70;
...
fn solve_low_density(
    ...
    let base_prob = 0.42 + 0.08 * (density / 5.0).min(1.0);
    let mut current_prob = base_prob;
    let large_problem_scale = ((num_variables as f64 - 25000.0) / 35000.0).max(0.0).min(1.0);
    let base_interval = 60.0 - 30.0 * large_problem_scale;
    ...
    let max_random_prob = 0.84;
    let prob_adjustment_factor = if density >= 4.18 && density <= 4.22 { 0.032 } else { 0.025 };
    let smoothing_factor = if density >= 4.18 && density <= 4.22 { 0.68 } else { 0.72 };

    let mut last_check_residual = residual_.len();
    let initial_residual_size = residual_.len();
    let size_scale = 1.0 / (1.0 + (-(nv - 30000.0) / 7000.0).exp());
    let perturbation_flips = if density >= 4.195 { 3 } else { 1 + (1.0 * size_scale) as usize };
