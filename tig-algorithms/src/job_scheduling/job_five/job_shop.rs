...
pub fn solve(
    ...
    // Fewer constructions, max depth tabu search
    let num_restarts = 200usize;
    ...
    let ts_starts = top_solutions.len().min(8);
    let ts_iters = effort.job_shop_iters.max(100000);
...
fn tabu_search_phase(
    ...
    let kick_threshold = (max_no_improve / 2).max(40);
