// Verbatim copy of the challenge baseline
// (tig-challenges/src/job_scheduling/baselines/dispatching_rules.rs).
//
// It cannot be replaced by a call: tig-algorithms depends on tig-challenges with the
// `hide_verification` feature unconditionally, which makes the baselines module private.
//
// Only change from the source: the first line, `crate::` -> `tig_challenges::`. The build
// guard fails if this file drifts by more than 2 lines, so it is not annotated line by line.
// `DEFAULT_EFFORT` and `solve_challenge` are unused here and kept only so the file stays
// diffable against the source; hence the global allow below.
