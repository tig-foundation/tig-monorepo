...
static HFS_NONCE_COUNTER: AtomicUsize = AtomicUsize::new(0);
static HFS_ELITE_POOL: Mutex<Vec<(Solution, u64)>> = Mutex::new(Vec::new());
const HFS_MAX_POOL_SIZE: usize = 8;
const HFS_COLD_START_NONCES: usize = 2;
            ...
            for job in 0..num_jobs {
                ...
                for &(m, pt) in &op.machines {
                    ...
                    let base = match rule {
                        ...
                        Rule::MostFlex => {
                            // Inverse of LeastFlex: prioritize ops with HIGH flexibility (many eligible machines)
                            let agile = (0.0 - flex_u) * (1.0 + sat_scarcity);
                            agile + rem_min_u + next_u + base_bias - end_u - pop_pen + route_term
                        }
                    };
...
pub fn solve(
    ...
    let nonce_id = HFS_NONCE_COUNTER.fetch_add(1, Ordering::SeqCst);
    let mut rng = SmallRng::from_seed(challenge.seed);

    let allow_most_flex = pre.high_flex > 0.40;
    ...
    if allow_most_flex { rules.push(Rule::MostFlex); }
    ...
    // Cross-nonce warm-start: from nonce 2 onward, seed top_solutions with
    // up to 3 cross-nonce elites. If any pool elite has a lower makespan,
    // update best_makespan + best_solution and save.
    if nonce_id >= HFS_COLD_START_NONCES {
        let pool_snapshot = {
            let lp = HFS_ELITE_POOL.lock().unwrap();
            lp.clone()
        };
        for (elite_sol, elite_mk_u64) in pool_snapshot.iter().take(3) {
            let elite_mk = *elite_mk_u64 as u32;
            push_top_solutions(&mut top_solutions, elite_sol, elite_mk, 25);
            if elite_mk < best_makespan {
                best_makespan = elite_mk;
                best_solution = Some(elite_sol.clone());
                save_solution(elite_sol)?;
            }
        }
    }
    ...
    // Persist best solution to cross-nonce elite pool before move.
    if let Some(ref best) = best_solution {
        let mk = best_makespan as u64;
        let mut lp = HFS_ELITE_POOL.lock().unwrap();
        if !lp.iter().any(|(_, m)| *m == mk) {
            lp.push((best.clone(), mk));
            lp.sort_by_key(|(_, m)| *m);
            lp.truncate(HFS_MAX_POOL_SIZE);
        }
    }
