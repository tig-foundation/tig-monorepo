...
pub mod types {
    ...
    pub struct EffortConfig {
        ...
        // (nombre de mouvements / de graines), jamais en temps mur.
        
        pub fjsp_high_esc_mode: usize,
        pub fjsp_high_esc_iters: usize,
        pub fjsp_high_esc_seeds: usize,
        
        // `push_top_solutions_diverse` et la couverture des trois raffineurs
        // (CBMLS / cp_descent / cp_window) qui lisent `top_solutions.len().min(width)`.
        
        pub fjsp_high_pool_width: usize,
    }

    
    pub const ESC_MODE_DEFAULT: usize = 1;
    pub const ESC_ITERS_DEFAULT: usize = 350;
    pub const ESC_SEEDS_DEFAULT: usize = 6;
    
    pub const POOL_WIDTH_DEFAULT: usize = 15;

    impl EffortConfig {
        pub fn default_effort() -> Self {
            Self { num_restarts: 2000, job_shop_iters: 100000, flow_shop_iters: 20000, hybrid_flow_shop_iters: 8000, fjsp_medium_iters: 5000, fjsp_high_iters: 5000, fjsp_high_esc_mode: ESC_MODE_DEFAULT, fjsp_high_esc_iters: ESC_ITERS_DEFAULT, fjsp_high_esc_seeds: ESC_SEEDS_DEFAULT, fjsp_high_pool_width: POOL_WIDTH_DEFAULT }
        }

        pub fn from_str(s: &str) -> Self {
            match s.to_lowercase().as_str() {
                "medium" => Self { num_restarts: 3000, job_shop_iters: 10000, flow_shop_iters: 12000, hybrid_flow_shop_iters: 8000, fjsp_medium_iters: 8000, fjsp_high_iters: 8000, ..Self::default_effort() },
                "high" => Self { num_restarts: 4000, job_shop_iters: 15000, flow_shop_iters: 18000, hybrid_flow_shop_iters: 12000, fjsp_medium_iters: 12000, fjsp_high_iters: 12000, ..Self::default_effort() },
                "extreme" => Self { num_restarts: 6000, job_shop_iters: 20000, flow_shop_iters: 25000, hybrid_flow_shop_iters: 15000, fjsp_medium_iters: 15000, fjsp_high_iters: 15000, ..Self::default_effort() },
                _ => Self::default_effort(),
            }
        }

        pub fn from_value(v: usize) -> Self {
            Self { num_restarts: v.clamp(1, 20000), job_shop_iters: 100000, flow_shop_iters: 20000, hybrid_flow_shop_iters: 5000, fjsp_medium_iters: 5000, fjsp_high_iters: 5000, ..Self::default_effort() }
        }
        ...
        pub fn with_fjsp_high_esc_mode(mut self, v: usize) -> Self {
            self.fjsp_high_esc_mode = v.min(3);
            self
        }

        pub fn with_fjsp_high_esc_iters(mut self, v: usize) -> Self {
            self.fjsp_high_esc_iters = v.min(20000);
            self
        }

        pub fn with_fjsp_high_esc_seeds(mut self, v: usize) -> Self {
            self.fjsp_high_esc_seeds = v.min(64);
            self
        }

        
        // interdire une explosion du temps de raffinement (3 raffineurs × width runs).
        pub fn with_fjsp_high_pool_width(mut self, v: usize) -> Self {
            self.fjsp_high_pool_width = v.clamp(1, 32);
            self
        }
    }
...
pub mod infra {
    ...
    pub fn eval_disj(ds: &DisjSchedule, buf: &mut EvalBuf) -> Option<(u32, usize)> {
        ...
        // `machine_seq` partitions the n nodes (each op is pushed exactly once by
        // `build_disj_from_solution`; apply_insert/apply_reroute preserve the
        // partition), so writing machine_succ for every node below covers the whole
        // array — the previous `fill(NONE_USIZE)` pre-pass was redundant work.
        for seq in &ds.machine_seq {
            ...
            buf.machine_succ[seq[l - 1]] = NONE_USIZE;
        }

        // Seeding order kept byte-identical to the original ascending scan: the
        // LIFO pop order decides `best_pred` / `mk_node` on ties.
        for i in 0..n {
            if buf.indeg[i] == 0 {
                buf.stack.push(i);
            }
    ...
    /// Screening-only eval: identical longest-path computation to `eval_disj`
    /// but skips `best_pred` maintenance and mk_node tracking (only needed at
    /// acceptance time to trace the critical path). Returns the same makespan
    /// byte-exactly; `buf.start` is filled identically. `buf.best_pred` is left
    /// STALE (from the last full eval) — callers must not read it after a
    /// screening eval without a full `eval_disj` first.
    pub fn eval_disj_screen(ds: &DisjSchedule, buf: &mut EvalBuf) -> Option<u32> {
        let n = ds.n;
        ...
        // Single pass over `machine_seq` (a partition of the n nodes) does three
        // jobs that used to cost three O(n) passes: write every machine arc, write
        // the NONE terminator of each machine (making the old `machine_succ.fill`
        // redundant), and seed the ready stack. A node has indeg 0 iff it is first
        // on its machine AND has no job predecessor, so the seed set is exactly
        // {seq[0] : indeg_job[seq[0]] == 0} — the old `for i in 0..n` scan is
        // therefore replaced by an O(num_machines) test. `buf.start` and the
        // returned makespan are longest-path labels: independent of pop order.
        for seq in &ds.machine_seq {
            let l = seq.len();
            if l == 0 {
                continue;
    ...
    /// Bounded screening eval: same longest-path computation as `eval_disj_screen`
    /// but aborts the topological sweep as soon as a finalised operation end
    /// reaches `bound`, returning `None`.
    ///
    /// Decision-neutrality (the reason this is byte-exact, not an approximation):
    /// a node is popped only once its in-degree hits 0, i.e. after every
    /// predecessor was relaxed, so `start[u]` — and hence `end_u` — is final at
    /// pop time. The makespan is `max_u end_u`, therefore
    ///   * if some `end_u >= bound` the true makespan is `>= bound`, and every
    ///     caller uses the result only through `mk < bound`, so reporting `None`
    ///     ("no improvement") is exactly what the full eval would have led to;
    ///   * if the true makespan is `< bound`, no `end_u` can reach `bound`, the
    ///     abort never fires and the sweep returns the exact makespan.
    /// The cycle case (`processed != n`) also maps to `None`, as in the unbounded
    /// version. `buf.start` is left partially filled on abort — callers must run a
    /// full `eval_disj` before reading it again (they already do: `descent_phase`
    /// re-evaluates on acceptance and its caller re-evaluates on return).
    pub fn eval_disj_screen_bounded(ds: &DisjSchedule, buf: &mut EvalBuf, bound: u32) -> Option<u32> {
        let n = ds.n;
    ...
    /// Taillard tails: q[u] = pt[u] + longest path from the END of u to the sink
    /// (0 if u is terminal), so that makespan == max_u start[u] + q[u].
    /// Computed by reverse Kahn on the current disjunctive graph (machine arcs
    /// rebuilt from `ds.machine_seq`, independent of `buf`). Returns false if the
    /// graph is inconsistent (caller must then disable LB screening).
    pub fn compute_tails_q(
        ds: &DisjSchedule,
        q: &mut [u32],
        mpred: &mut [usize],
        msucc: &mut [usize],
        ...
        while let Some(u) = stk.pop() {
            ...
            let ms = msucc[u];
            if ms != NONE_USIZE && q[ms] > best {
                best = q[ms];
            }
            q[u] = ds.node_pt[u].saturating_add(best);
            if ds.indeg_job[u] > 0 {
                let jp = u - 1;
                outdeg[jp] = outdeg[jp].saturating_sub(1);
                if outdeg[jp] == 0 {
                    stk.push(jp);
                }
            }
            let mp = mpred[u];
            if mp != NONE_USIZE {
                outdeg[mp] = outdeg[mp].saturating_sub(1);
                if outdeg[mp] == 0 {
                    stk.push(mp);
                }
            }
    ...
    /// Taillard lower bound on the makespan after swapping adjacent ops
    /// (seq[i], seq[i+1]) on machine m. Uses base-state heads (`base_start`) and
    /// tails (`q`), whose values at the referenced neighbor nodes are provably
    /// unchanged by the swap when the resulting graph is acyclic. If the swap
    /// creates a cycle the candidate is discarded by eval anyway, so the bound
    /// is safe in both cases: true_new_mk >= LB always holds for kept candidates.
    #[inline]
    pub fn taillard_swap_lb(
        ds: &DisjSchedule,
        base_start: &[u32],
        q: &[u32],
        m: usize,
        ...
        let e_pm = if i > 0 {
            let pm = seq[i - 1];
            base_start[pm].saturating_add(ds.node_pt[pm])
        } else {
            0
        };
        let e_jpv = if ds.indeg_job[v] > 0 {
            base_start[v - 1].saturating_add(ds.node_pt[v - 1])
        } else {
            0
        };
        let e_jpu = if ds.indeg_job[u] > 0 {
            base_start[u - 1].saturating_add(ds.node_pt[u - 1])
        } else {
            0
        };
        let s_v = e_pm.max(e_jpv);
        let s_u = s_v.saturating_add(ds.node_pt[v]).max(e_jpu);
        let q_after_u_ms = if i + 2 < seq.len() { q[seq[i + 2]] } else { 0 };
        let q_after_u_js = if ds.job_succ[u] != NONE_USIZE { q[ds.job_succ[u]] } else { 0 };
        let q_u = ds.node_pt[u].saturating_add(q_after_u_ms.max(q_after_u_js));
        let q_jsv = if ds.job_succ[v] != NONE_USIZE { q[ds.job_succ[v]] } else { 0 };
        (s_u.saturating_add(q_u)).max(s_v.saturating_add(ds.node_pt[v]).saturating_add(q_jsv))
    }
    ...
    pub fn descent_phase(
        ...
        // Taillard incremental-eval scratch (heads snapshot + tails), recomputed
        // once per descent iteration on the base state.
        let n_nodes = ds.n;
        let mut q_tail: Vec<u32> = vec![0; n_nodes];
        let mut t_mpred: Vec<usize> = vec![NONE_USIZE; n_nodes];
        let mut t_msucc: Vec<usize> = vec![NONE_USIZE; n_nodes];
        let mut t_outdeg: Vec<u8> = vec![0; n_nodes];
        let mut t_stk: Vec<usize> = Vec::with_capacity(n_nodes);
        let mut base_start: Vec<u32> = vec![0; n_nodes];
        ...
        for _iter in 0..max_iters {
            ...
            // Base-state heads snapshot + Taillard tails: buf.start is valid for
            // the current base ds here (last full eval), and screening evals
            // below will trash buf — snapshot before the loop.
            let tails_ok =
                compute_tails_q(ds, &mut q_tail, &mut t_mpred, &mut t_msucc, &mut t_outdeg, &mut t_stk);
            if tails_ok {
                base_start.clone_from(&buf.start);
            }
            ...
            for cand in &cands {
                if cand.kind == 0 {
                    ...
                    // sh==1 insert is an adjacent swap: O(1) Taillard LB screen.
                    // Skip full eval iff LB >= best_mk (true mk >= LB, so the
                    // candidate provably cannot improve — decision-neutral).
                    if tails_ok {
                        let swap_i = if cand.to == cand.from + 1 {
                            Some(cand.from)
                        } else if cand.to + 1 == cand.from {
                            Some(cand.to)
                        } else {
                            None
                        };
                        if let Some(si) = swap_i {
                            if si + 1 < ds.machine_seq[m].len()
                                && taillard_swap_lb(ds, &base_start, &q_tail, m, si) >= best_mk
                            {
                                continue;
                            }
                        }
                    ...
                    if let Some(mk2) = eval_disj_screen_bounded(ds, buf, best_mk) {
                        if mk2 < best_mk {
                            best_mk = mk2;
                            best_cand = Some(*cand);
                        }
                    ...
                    if tails_ok
                        && taillard_swap_lb(ds, &base_start, &q_tail, m, cand.from) >= best_mk
                    {
                        continue;
                    }
    ...
    //
    // Constat de la carte des briques : TOUTES les phases de raffinement du
    // moteur (CBM, `iterative_cp_descent`, `cp_window_exhaustive`) sont des
    // DESCENTES STRICTES — `descent_phase` n'accepte un mouvement que si
    // `next_eval.0 < cur_mk` et `break` sinon. Le moteur n'a donc AUCUN
    // mécanisme d'échappement des optima locaux : la diversité vient
    // uniquement du pool d'élites et des kicks aléatoires de CBM.
    //
    // Cette fonction garde le voisinage EXACTEMENT tel quel (mouvements de
    // blocs critiques + reroutes — la famille « élargir le voisinage » est
    
    //   mode 1 = TS-RN  : liste tabou sur l'attribut « opération déplacée »
    //                     + critère d'aspiration, déplacement FORCÉ vers le
    //                     meilleur voisin admissible même s'il dégrade.
    //   mode 2 = LAHC   : acceptation retardée (mémoire des coûts sur L pas).
    //   mode 3 = TA     : acceptation par seuil décroissant (Threshold Accepting).
    //
    // Budget 100 % en TRAVAIL (`max_moves`), jamais en temps mur.
    // Retourne TOUJOURS le MEILLEUR état rencontré (`global_best_ds`), jamais
    // l'état courant, et `None` si aucune amélioration stricte n'a été trouvée
    // ⇒ la phase est additive et monotone au niveau de l'appelant.
    // ========================================================================

    #[inline]
    fn esc_moved_node(ds: &DisjSchedule, c: &MoveCand) -> usize {
        let m = c.m_from;
    ...
    pub fn rn_escape_search(
        ...
        // Mémoire tabou (mode 1) : itération jusqu'à laquelle l'opération est
        // interdite de re-déplacement. Tenure proportionnelle à la taille de
        // l'instance (règle usuelle TS-N8), bornée.
        let mut tabu_until: Vec<u32> = vec![0u32; ds.n];
        let tenure: u32 = (6 + ds.n / 20).min(24) as u32;

        // Mémoire LAHC (mode 2).
        const LAHC_LEN: usize = 30;
        let mut lahc: Vec<u32> = vec![cur_mk; LAHC_LEN];

        // Scratch Taillard (heads snapshot + tails), comme dans `descent_phase`.
        let n_nodes = ds.n;
        let mut q_tail: Vec<u32> = vec![0; n_nodes];
        let mut t_mpred: Vec<usize> = vec![NONE_USIZE; n_nodes];
        let mut t_msucc: Vec<usize> = vec![NONE_USIZE; n_nodes];
        ...
        for it in 0..max_moves {
            ...
            // Seuil d'acceptation du mode 3 : 1.2 % du record, décroissance
            // linéaire en TRAVAIL restant (aucun temps mur).
            let thr: u32 = if mode == 3 {
                let left = (max_moves - it) as u64;
                (((global_best_mk as u64) * 12 * left) / (1000u64 * (max_moves as u64))) as u32
            } else {
                0
            };

            let mut best_free: Option<MoveCand> = None;
            let mut best_free_mk: u32 = u32::MAX;
            let mut best_tabu: Option<MoveCand> = None;
            let mut best_tabu_mk: u32 = u32::MAX;

            for cand in &cands {
                let node = esc_moved_node(&ds, cand);
                let is_tabu = mode == 1 && node != NONE_USIZE && tabu_until[node] > it as u32;
                // Un candidat tabou ne peut servir que par aspiration (battre le
                // record global) : sa borne d'évaluation est donc plus serrée.
                let bound = if is_tabu {
                    global_best_mk.min(best_tabu_mk)
                } else {
                    best_free_mk
                };
                if bound == 0 {
                ...
                if cand.kind == 0 {
                    ...
                    // sh==1 insert = swap adjacent : écran LB O(1) de Taillard.
                    // LB est une borne INFÉRIEURE vraie du makespan résultant, donc
                    // LB >= bound ⇒ `eval_disj_screen_bounded` aurait rendu None.
                    // L'élagage est neutre pour la décision.
                    if tails_ok {
                        let swap_i = if cand.to == cand.from + 1 {
                            Some(cand.from)
                        } else if cand.to + 1 == cand.from {
                        ...
                        if let Some(si) = swap_i {
                            if si + 1 < ds.machine_seq[m].len()
                                && taillard_swap_lb(&ds, &base_start, &q_tail, m, si) >= bound
                            {
                                continue;
                            }
                        }
                    ...
                    let new_idx = apply_insert(&mut ds.machine_seq[m], cand.from, cand.to);
                    if let Some(mk2) = eval_disj_screen_bounded(&ds, &mut buf, bound) {
                        if is_tabu {
                            if mk2 < best_tabu_mk {
                                best_tabu_mk = mk2;
                                best_tabu = Some(*cand);
                            }
                        } else if mk2 < best_free_mk {
                            best_free_mk = mk2;
                            best_free = Some(*cand);
                        }
                    }
                    ...
                    if tails_ok && taillard_swap_lb(&ds, &base_start, &q_tail, m, cand.from) >= bound {
                        continue;
                    }
                    if !apply_swap(&mut ds.machine_seq[m], cand.from) {
                        continue;
                    ...
                    let node2_chk = ds.machine_seq[m_from][cand.from];
                    if ds.node_machine[node2_chk] != m_from {
                        continue;
                    }
                    let applied = apply_reroute(&mut ds, m_from, cand.from, m_to, cand.to, cand.new_pt);
                    if let Some((node2, old_pt, ins_idx)) = applied {
                        if let Some(mk2) = eval_disj_screen_bounded(&ds, &mut buf, bound) {
                            if is_tabu {
                                if mk2 < best_tabu_mk {
                        ...
                        let _ = undo_reroute(&mut ds, m_from, cand.from, m_to, ins_idx, node2, old_pt);
                    }
                }
            }

            // --- règle d'acceptation (le SEUL point qui distingue les 3 modes) ---
            let chosen: Option<(MoveCand, u32)> = match mode {
                1 => {
                    let free = best_free.map(|c| (c, best_free_mk));
                    let asp = best_tabu
                        .map(|c| (c, best_tabu_mk))
                        .filter(|&(_, mk)| mk < global_best_mk);
                    match (free, asp) {
                        (Some(f), Some(a)) => Some(if a.1 < f.1 { a } else { f }),
                        (Some(f), None) => Some(f),
                        (None, Some(a)) => Some(a),
                        (None, None) => None,
                    }
                ...
                2 => {
                    let v = lahc[it % LAHC_LEN];
                    best_free
                        .map(|c| (c, best_free_mk))
                        .filter(|&(_, mk)| mk <= v || mk <= cur_mk || mk < global_best_mk)
                }
                _ => best_free
                    .map(|c| (c, best_free_mk))
                    .filter(|&(_, mk)| mk <= cur_mk.saturating_add(thr) || mk < global_best_mk),
            };

            let Some((bc, _)) = chosen else { break };

            // Attribut tabou capturé AVANT application (les indices bougent après).
            let moved_node = esc_moved_node(&ds, &bc);

            let applied_ok = if bc.kind == 0 {
                let m = bc.m_from;
                ...
                apply_reroute(&mut ds, bc.m_from, bc.from, bc.m_to, bc.to, bc.new_pt).is_some()
            };
            if !applied_ok {
                break;
            }

            let Some(next_eval) = eval_disj(&ds, &mut buf) else { break };
            cur_eval = next_eval;
            cur_mk = cur_eval.0;

            if moved_node != NONE_USIZE {
                tabu_until[moved_node] = (it as u32).saturating_add(tenure);
            }
            if mode == 2 {
                lahc[it % LAHC_LEN] = cur_mk;
            }
        ...
        // RÈGLE DURE : on rend le MEILLEUR état suivi, jamais l'état courant.
        if global_best_mk >= initial_mk {
            return Ok(None);
        }
        ds = global_best_ds;
...
pub mod solver {
    ...
    fn parse_effort(hyperparameters: &Option<Map<String, Value>>) -> EffortConfig {
        ...
        if let Some(map) = hyperparameters {
            ...
            if let Some(Value::Number(n)) = map.get("fjsp_high_esc_mode") {
                if let Some(v) = n.as_u64() {
                    cfg = cfg.with_fjsp_high_esc_mode(v as usize);
                }
            }
            if let Some(Value::Number(n)) = map.get("fjsp_high_esc_iters") {
                if let Some(v) = n.as_u64() {
                    cfg = cfg.with_fjsp_high_esc_iters(v as usize);
                }
            }
            if let Some(Value::Number(n)) = map.get("fjsp_high_esc_seeds") {
                if let Some(v) = n.as_u64() {
                    cfg = cfg.with_fjsp_high_esc_seeds(v as usize);
                }
            }
            
            if let Some(Value::Number(n)) = map.get("fjsp_high_pool_width") {
                if let Some(v) = n.as_u64() {
                    cfg = cfg.with_fjsp_high_pool_width(v as usize);
                }
            }
        }
        cfg
...
pub mod hybrid_flow_shop {
    ...
    // O(1) estimated swap + reassign on disjunctive graph
    // ============================================================================
...
pub mod fjsp_high {
    ...
    // verbatim as the bit-exactness reference the parity harness checks against;
    // hence the expected `dead_code` allowance.
    #[allow(dead_code)]
    #[allow(clippy::too_many_arguments)]
    #[inline]
    fn score_candidate(
    ...
    //
    // `score_candidate` above is evaluated once per (ready job x qualifying idle
    // machine) on EVERY pass of the construction inner loop, i.e. ~total_ops x
    // |ready| x |qualifying| times per restart, 7000 restarts per nonce. Yet the
    // bulk of its body does not depend on the machine at all:
    //   * the rule weight block (end_w/cp_w/reg_w/bn_w/load_w/density_w/mpen_w,
    //     next_w_base, slack_w, route_gain, pop coefficient) depends ONLY on
    //     `progress`, `rule` and `pre` -> constant for the whole PASS;
    //   * rem_*_n, ops_n, end_n, reg_n, scarcity_urg, density_n, next_term,
    //     slack_u/slack_term, flow_term, flex_inv depend only on the JOB.
    // Measured (standalone probe, -O3): LLVM does NOT hoist these across the
    // job/candidate loop nest -- lifting them by hand is -8 % at 1 candidate per
    // job and -38 %/-51 % at 2/3 candidates, bit-identical output.
    //
    // BIT-EXACTNESS. f64 + and * are not associative, so only the computation of
    // individual FACTORS is lifted; every arm's final summation chain below is a
    // verbatim transcription of the corresponding arm of `score_candidate` --
    // same operands, same order, same associativity. Sub-chains are never
    // pre-summed. Hoisting `a*b` out of a loop and reusing the value is exact:
    // it is the same IEEE-754 operation on the same operands, performed once.
    // ---------------------------------------------------------------------------
    struct PassCoef {
        avg_flex_inv: f64, mpen_gain: f64,
        route_wg: f64, pop_coef: f64,
        ff15: f64, ff20: f64, ff30: f64, ff35: f64, ff55: f64, ff62: f64, ff65: f64,
        a_next_w: f64, a_slack_w: f64,
        a_end_w: f64, a_cp_w: f64, a_bn_w: f64, a_density_w: f64,
        a_reg_ff: f64, a_load_w: f64, a_load_ff: f64,
        a_mpen_full: f64, a_proc_coef: f64, a_jb_coef: f64,
    }

    #[inline]
    fn pass_coef(pre: &Pre, rule: Rule, progress: f64, route_w: f64) -> PassCoef {
        let js = pre.jobshopness; let fl = 1.0 - js;
        let p2 = progress*progress; let next_w_base = 0.12 + p2*0.28;
        let slack_w = pre.slack_base*(0.25 + 0.75*progress);
        let route_gain = (0.70 + 0.80*(1.0 - progress)).clamp(0.70, 1.40);
        let mut c = PassCoef {
            avg_flex_inv: 1.0/pre.flex_avg.max(1.0),
            mpen_gain: 1.0 + 0.85*pre.high_flex,
            route_wg: route_w*route_gain,
            pop_coef: (0.07 + 0.15*(1.0 - progress)).clamp(0.05, 0.24),
            ff15: 0.15*pre.flex_factor, ff20: 0.20*pre.flex_factor,
            ff30: 0.30*pre.flex_factor, ff35: 0.35*pre.flex_factor,
            ff55: 0.55*pre.flex_factor, ff62: 0.62*pre.flex_factor,
            ff65: 0.65*pre.flex_factor,
            a_next_w: 0.0, a_slack_w: 0.0,
            a_end_w: 0.0, a_cp_w: 0.0, a_bn_w: 0.0, a_density_w: 0.0,
            a_reg_ff: 0.0, a_load_w: 0.0, a_load_ff: 0.0,
            a_mpen_full: 0.0, a_proc_coef: 0.0, a_jb_coef: 0.0,
        };
        match rule {
            Rule::CriticalPath => { c.a_next_w = next_w_base*0.30; c.a_slack_w = slack_w; }
            Rule::MostWork     => { c.a_next_w = next_w_base*0.25; }
            Rule::LeastFlex    => { c.a_next_w = next_w_base*0.20; }
            Rule::ShortestProc => { c.a_next_w = next_w_base*0.20; }
            Rule::Regret       => { c.a_next_w = next_w_base*0.25; }
            Rule::EndTight => {
                let reg_w = (0.55 + 0.20*(1.0 - progress))*(0.85 + 0.60*js);
                let mpen_w = (0.10 + 0.45*pre.high_flex)*pre.flex_factor;
                c.a_end_w = 1.10 + 1.00*progress + 0.35*pre.high_flex;
                c.a_cp_w = 1.15 + 0.30*js;
                c.a_reg_ff = reg_w*pre.flex_factor;
                c.a_mpen_full = c.mpen_gain*mpen_w;
                c.a_next_w = next_w_base*(0.45 + 0.55*js);
                c.a_slack_w = slack_w*(0.70 + 0.40*js);
            }
            Rule::BnHeavy => {
                let reg_w = (0.60 + 0.25*(1.0 - progress))*(0.85 + 0.35*js);
                let load_w = if pre.hi_flex { -0.35 } else { 0.55 + 0.25*js };
                let mpen_w = (0.12 + 0.30*js)*pre.flex_factor*(0.95 + 0.65*pre.high_flex);
                c.a_bn_w = (0.90 + 0.55*js)*pre.bn_focus;
                c.a_end_w = 0.65 + 0.70*progress;
                c.a_reg_ff = reg_w*pre.flex_factor;
                c.a_load_ff = load_w*pre.flex_factor;
                c.a_mpen_full = c.mpen_gain*mpen_w;
                c.a_next_w = next_w_base*(0.55 + 0.75*js);
                c.a_slack_w = slack_w*(0.45 + 0.55*js);
            }
            Rule::Adaptive => {
                let reg_w = (0.50*fl + 0.78*js) + 0.18*(1.0 - progress);
                let load_sign = if pre.hi_flex { -1.0 } else { 1.0 };
                let mpen_w = (0.08*fl + 0.28*js)*pre.flex_factor*(1.0 + 0.85*pre.high_flex);
                c.a_end_w = (0.90*fl + 0.72*js) + (0.62 + 0.12*fl)*progress + 0.18*pre.high_flex;
                c.a_bn_w = ((0.45 + 0.40*js) + 0.25*(1.0 - progress))*pre.bn_focus;
                c.a_load_w = load_sign*(0.45*fl + 0.75*js)*pre.flex_factor;
                c.a_density_w = 0.08*fl + 0.20*js;
                c.a_reg_ff = reg_w*pre.flex_factor;
                c.a_mpen_full = c.mpen_gain*mpen_w;
                c.a_proc_coef = 0.18*fl + 0.12*js;
                c.a_jb_coef = 0.62 + 0.06*js;
                c.a_next_w = next_w_base*(0.50*fl + 1.50*js);
                c.a_slack_w = slack_w*(0.55*fl + 0.85*js);
            }
            Rule::FlexBalance => {
                let mpen_w = (0.55 + 0.65*pre.high_flex).clamp(0.55, 1.15);
                let reg_w = (0.35 + 0.25*(1.0 - progress)).clamp(0.35, 0.70);
                c.a_end_w = (0.85 + 0.70*progress + 0.15*js).clamp(0.85, 1.75);
                c.a_cp_w = (1.00 + 0.30*js + 0.15*(1.0 - progress)).clamp(0.95, 1.45);
                c.a_load_w = (0.55 + 0.35*pre.high_flex).clamp(0.55, 0.95)*pre.flex_factor;
                c.a_mpen_full = mpen_w*(1.0 + 0.85*pre.high_flex);
                c.a_reg_ff = reg_w*pre.flex_factor;
                c.a_jb_coef = 0.58 + 0.10*pre.high_flex;
                c.a_next_w = next_w_base*0.40;
                c.a_slack_w = 0.70*slack_w;
            }
        }
        c
    }
    ...
    struct JobInv {
        flex_inv: f64, rem_min_n: f64, rem_avg_n: f64, bn_n: f64, ops_n: f64,
        end_n: f64, reg_n: f64, scarcity_urg: f64, density_n: f64,
        next_term: f64, slack_term: f64, flow_term: f64,
        rigidity: f64, flex_ge2: bool, route_on: bool,
    }

    #[allow(clippy::too_many_arguments)]
    #[inline]
    fn job_inv(
        pre: &Pre, c: &PassCoef, rule: Rule, job: usize, product: usize, op_idx: usize,
        ops_rem: usize, op: &OpInfo, time: u32, target_mk: Option<u32>,
        best_end: u32, second_end: u32, best_cnt_total: usize, progress: f64, route_w: f64,
    ) -> JobInv {
        let rem_min = pre.product_suf_min[product][op_idx] as f64;
        let rem_avg = pre.product_suf_avg[product][op_idx]; let rem_bn = pre.product_suf_bn[product][op_idx];
        let flex_f = (op.flex as f64).max(1.0); let flex_inv = 1.0/flex_f;
        ...
        let slack_u = slack_urgency_fh(pre, target_mk, time, product, op_idx);
        let slack_reg_boost = 1.0 + 0.50 * reg_n / (1.0 + slack_u);
        // FlexBalance is the one arm whose slack term carries no regret boost.
        let slack_term = match rule {
            Rule::FlexBalance => c.a_slack_w*slack_u,
            _ => c.a_slack_w*slack_u*slack_reg_boost,
        };
        JobInv {
            flex_inv, rem_min_n, rem_avg_n, bn_n, ops_n, end_n, reg_n, scarcity_urg, density_n,
            next_term: c.a_next_w*next_term_raw,
            slack_term,
            flow_term: pre.flow_w*pre.job_flow_pref[job]*(0.65+0.70*(1.0-progress)),
            rigidity: (0.60*flex_inv+0.40*scarcity_urg).clamp(0.0,2.5),
            flex_ge2: op.flex >= 2,
            route_on: route_w > 0.0 && op.flex >= 2,
        }
    }

    #[allow(clippy::too_many_arguments)]
    #[inline]
    fn score_candidate_hoisted(
        pre: &Pre, c: &PassCoef, j: &JobInv, rule: Rule, product: usize, op_idx: usize,
        machine: usize, pt: u32, job_bias: f64, machine_penalty: f64, dynamic_load: f64,
        route_pref_counts: Option<&RoutePrefCounts>, jitter: f64,
    ) -> f64 {
        // machine-dependent factors only
        let load_n = dynamic_load/pre.avg_machine_load.max(1e-9);
        let scar_n = pre.machine_scarcity[machine]/pre.avg_machine_scarcity.max(1e-9);
        let proc_n = (pt as f64)/pre.avg_op_min.max(1.0);
        let scarce_match = scar_n*(j.flex_inv-c.avg_flex_inv);
        let mpen = machine_penalty.clamp(0.0,1.0);
        let pop_pen = if pre.chaotic_like && j.flex_ge2 { let pop=pre.machine_best_pop[machine]; c.pop_coef*pop*pre.flex_factor } else { 0.0 };
        let route_term = if j.route_on { c.route_wg*route_pref_bonus_fh(route_pref_counts,product,op_idx,machine) } else { 0.0 };
        // hoisted aliases — identical values, computed once per pass / per job
        let (rem_min_n, rem_avg_n, bn_n, ops_n) = (j.rem_min_n, j.rem_avg_n, j.bn_n, j.ops_n);
        let (end_n, reg_n, scarcity_urg, density_n) = (j.end_n, j.reg_n, j.scarcity_urg, j.density_n);
        let (flex_inv, next_term, slack_term, flow_term) = (j.flex_inv, j.next_term, j.slack_term, j.flow_term);
        // summation chains below: verbatim from `score_candidate`
        match rule {
            Rule::CriticalPath => { (1.03*rem_min_n)+(0.10*ops_n)+(0.24*scarcity_urg)+c.ff20*flex_inv+next_term+0.10*slack_term-(0.70*end_n)-pop_pen+(0.45*job_bias)+flow_term+route_term+jitter }
            Rule::MostWork => { (1.00*rem_avg_n)+(0.12*ops_n)+(0.18*scarcity_urg)+c.ff15*flex_inv+next_term-(0.62*end_n)-pop_pen+(0.45*job_bias)+flow_term+route_term+jitter }
            Rule::LeastFlex => { (1.00*flex_inv)+(0.28*rem_min_n)+(0.22*scarcity_urg)+next_term-(0.55*end_n)-pop_pen+(0.35*job_bias)+flow_term+route_term+jitter }
            Rule::ShortestProc => { (-1.00*proc_n)+(0.25*rem_min_n)+(0.12*scarcity_urg)+next_term-(0.20*end_n)-pop_pen+(0.25*job_bias)+flow_term+route_term+jitter }
            Rule::Regret => { (1.05*reg_n)+(0.55*rem_min_n)+(0.22*scarcity_urg)+next_term-(0.68*end_n)-pop_pen+(0.35*job_bias)+flow_term+route_term+jitter }
            Rule::EndTight => { (c.a_cp_w*rem_min_n)+0.12*rem_avg_n+0.08*ops_n+0.18*scarcity_urg+c.ff30*flex_inv+c.ff20*scarce_match+c.a_reg_ff*reg_n+next_term+slack_term-c.a_end_w*end_n-0.22*proc_n-pop_pen-c.a_mpen_full*mpen+0.55*job_bias+flow_term+route_term+jitter }
            Rule::BnHeavy => { (0.95*rem_min_n)+(0.30*rem_avg_n)+(c.a_bn_w*bn_n)+(0.22*density_n)+(0.10*ops_n)+c.ff65*flex_inv+c.ff35*scarce_match+c.a_load_ff*load_n+c.a_reg_ff*reg_n+0.18*scarcity_urg+next_term+slack_term-c.a_end_w*end_n-0.18*proc_n-pop_pen-c.a_mpen_full*mpen+0.60*job_bias+flow_term+route_term+jitter }
            Rule::Adaptive => { (1.05*rem_min_n)+(0.48*rem_avg_n)+(c.a_bn_w*bn_n)+c.a_density_w*density_n+(0.08*ops_n)+c.ff62*flex_inv+c.ff55*scarce_match+c.a_load_w*load_n+c.a_reg_ff*reg_n+c.ff20*scarcity_urg+next_term+slack_term-c.a_end_w*end_n-c.a_proc_coef*proc_n-pop_pen-c.a_mpen_full*mpen+c.a_jb_coef*job_bias+flow_term+route_term+jitter }
            Rule::FlexBalance => { (c.a_cp_w*rem_min_n)+0.55*rem_avg_n+0.08*ops_n+0.06*density_n+0.08*scarcity_urg+next_term+slack_term-c.a_end_w*end_n-0.16*proc_n-pop_pen-c.a_load_w*load_n-c.a_mpen_full*mpen+c.a_reg_ff*reg_n+c.a_jb_coef*job_bias+flow_term+route_term+jitter }
        }
    }
    ...
    fn construct_solution_conflict(
        ...
        let mut machine_work: Vec<u64>=if chaotic_like{vec![0u64;num_machines]}else{vec![]};
        let mut sum_work: u64=0;
        // --- Phase-1 candidate-generation memo -------------------------------------
        // Each pass of the inner loop schedules exactly ONE operation, i.e. makes exactly
        // ONE machine busy, yet the legacy code re-ran `best_second_and_counts` (O(flex))
        // plus a second O(flex) rescan of `op.machines` for EVERY ready job. The tuple
        // (best_end, second_end, cnt_total) and the ordered list of qualifying idle
        // machines depend only on `time` and on `machine_avail` restricted to the job's
        // own op, so they are unchanged for every job whose current op does not contain
        // the machine that was just taken. We cache them and invalidate by machine mask.
        let mask_words = (num_machines + 63) / 64;
        let mut max_flex = 1usize;
        for pops in &pre.product_ops { for o in pops { if o.machines.len() > max_flex { max_flex = o.machines.len(); } } }
        let mut job_mask: Vec<u64> = vec![0u64; num_jobs * mask_words];
        for j in 0..num_jobs {
            if pre.job_ops_len[j] == 0 { continue; }
            let o = &pre.product_ops[pre.job_products[j]][0];
            let base = j * mask_words;
            for &(mm, _) in &o.machines { job_mask[base + (mm >> 6)] |= 1u64 << (mm & 63); }
        }
        let mut memo_valid: Vec<bool> = vec![false; num_jobs];
        let mut memo_best: Vec<u32> = vec![0u32; num_jobs];
        let mut memo_second: Vec<u32> = vec![0u32; num_jobs];
        let mut memo_cnt: Vec<u32> = vec![0u32; num_jobs];
        let mut memo_qlen: Vec<u32> = vec![0u32; num_jobs];
        let mut memo_qual: Vec<(usize, u32)> = vec![(0usize, 0u32); num_jobs * max_flex];
        // Unfinished jobs, kept in ascending order so the scan order below is byte-identical
        // to the legacy `for job in 0..num_jobs`.
        let mut active: Vec<usize> = (0..num_jobs).filter(|&j| pre.job_ops_len[j] > 0).collect();
        while remaining_ops > 0 {
            loop {
                idle_machines.clear();
                for m in 0..num_machines { if machine_avail[m]<=time { idle_machines.push(m); } }
                ...
                let progress=1.0-(remaining_ops as f64)/(pre.total_ops as f64).max(1.0);
                // Rule weights and every other `progress`-only quantity of the scoring
                // kernel are constant for this whole pass: compute them once here
                // instead of once per (job x candidate).
                let pc=pass_coef(pre,rule,progress,route_w);
                let cap_per_machine=if k==0{12usize}else{(k+6).min(12)};
                ...
                for ai in 0..active.len() {
                    ...
                    let product=pre.job_products[job]; let op=&pre.product_ops[product][op_idx];
                    if op.flex==0||op.machines.is_empty()||op.min_pt>=INF{continue;}
                    let qbase=job*max_flex;
                    let (best_end,second_end,best_cnt_total,best_cnt_idle);
                    if memo_valid[job] {
                        best_end=memo_best[job]; second_end=memo_second[job];
                        best_cnt_total=memo_cnt[job] as usize; best_cnt_idle=memo_qlen[job] as usize;
                    } else {
                        // Fused pass: identical reduction to `best_second_and_counts`, and it
                        // additionally emits the qualifying idle machines (`avail<=time` and
                        // `time+pt==best_end`) in `op.machines` order — the exact order the
                        // legacy rescan visited them in, hence the same RNG draw sequence.
                        let mut best=INF; let mut second=INF;
                        let mut cnt_best=0usize; let mut cnt_idle=0usize;
                        for &(m,pt) in &op.machines {
                            let av=machine_avail[m]; let idle=av<=time;
                            let end=time.max(av).saturating_add(pt);
                            if end<best {
                                second=best; best=end; cnt_best=1;
                                if idle { memo_qual[qbase]=(m,pt); cnt_idle=1; } else { cnt_idle=0; }
                            } else if end==best {
                                cnt_best+=1;
                                if idle { memo_qual[qbase+cnt_idle]=(m,pt); cnt_idle+=1; }
                            } else if end<second { second=end; }
                        }
                        if cnt_best>1 { second=best; }
                        best_end=best; second_end=second; best_cnt_total=cnt_best.max(1); best_cnt_idle=cnt_idle;
                        memo_best[job]=best_end; memo_second[job]=second_end;
                        memo_cnt[job]=best_cnt_total as u32; memo_qlen[job]=best_cnt_idle as u32;
                        memo_valid[job]=true;
                    }
                    if best_end>=INF||best_cnt_idle==0{continue;}
                    let ops_rem=pre.job_ops_len[job]-op_idx; let jb=job_bias.map(|v|v[job]).unwrap_or(0.0);
                    // Everything the scoring kernel derives from this job alone (and no
                    // longer from the candidate machine) is computed once here. `reg_n`
                    // and `rigidity` were already duplicated between this loop and
                    // `score_candidate`; they now come from the single shared source.
                    let ji=job_inv(pre,&pc,rule,job,product,op_idx,ops_rem,op,time,target_mk,best_end,second_end,best_cnt_total,progress,route_w);
                    let regn=ji.reg_n; let rigidity=ji.rigidity;
                    for qi in 0..best_cnt_idle {
                        let (m,pt)=memo_qual[qbase+qi];
                        demand[m]=demand[m].saturating_add(1);
                        let mp = if diversify {
                        ...
                        let jitter=if k>0{rng.gen::<f64>()*1e-9}else{0.0};
                        let base=score_candidate_hoisted(pre,&pc,&ji,rule,product,op_idx,m,pt,jb,mp,machine_load[m],route_pref_counts,jitter);
                ...
                let job=chosen.job; let machine=chosen.machine; let pt=chosen.pt;
                let product=pre.job_products[job]; let op_idx=job_next_op[job]; let op=&pre.product_ops[product][op_idx];
                // `chosen` was produced by this very pass, so the job's memo entry is live
                // and `machine_avail` has not moved since it was filled.
                let best_end_now=if memo_valid[job]{memo_best[job]}else{best_second_and_counts(time,&machine_avail,op).0};
                let end_check=time.max(machine_avail[machine]).saturating_add(pt);
                if machine_avail[machine]>time||end_check!=best_end_now{break;}
                let end_time=time.saturating_add(pt);
                job_schedule[job].push((machine,time)); job_next_op[job]+=1; job_ready_time[job]=end_time; machine_avail[machine]=end_time; remaining_ops-=1;
                if chaotic_like{machine_work[machine]=machine_work[machine].saturating_add(pt as u64);sum_work=sum_work.saturating_add(pt as u64);}
                if op.min_pt<INF&&op.flex>0&&!op.machines.is_empty(){let delta=(op.min_pt as f64)/(op.flex as f64).max(1.0);if delta>0.0{for &(mm,_) in &op.machines{let v=machine_load[mm]-delta;machine_load[mm]=if v>0.0{v}else{0.0};}}}
                // Only `machine` changed availability: drop the memo of the active jobs whose
                // current op can use it. The chosen job also advanced, so its op (and mask)
                // must be refreshed; a job with no op left leaves `active`.
                let iw=machine>>6; let ibit=1u64<<(machine&63);
                for ai in 0..active.len() {
                    let j=active[ai];
                    if job_mask[j*mask_words+iw]&ibit!=0 { memo_valid[j]=false; }
                }
                memo_valid[job]=false;
                let mbase=job*mask_words;
                for w in 0..mask_words { job_mask[mbase+w]=0; }
                let next_op_idx=job_next_op[job];
                if next_op_idx<pre.job_ops_len[job] {
                    let o=&pre.product_ops[product][next_op_idx];
                    for &(mm,_) in &o.machines { job_mask[mbase+(mm>>6)]|=1u64<<(mm&63); }
                } else if let Some(p)=active.iter().position(|&x|x==job) {
                    active.remove(p);
                }
                if remaining_ops==0{break;}
            }
            ...
            for j in 0..num_jobs{let op_idx=job_next_op[j];if op_idx<pre.job_ops_len[j]&&job_ready_time[j]>time{let t=job_ready_time[j];next_time=Some(next_time.map_or(t,|b|b.min(t)));}}
            time=next_time.ok_or_else(||anyhow!("Stalled"))?;
            // `time` moved: every cached (best_end, second_end, qualifying set) is stale,
            // since each entry is a function of `time` as well as of `machine_avail`.
            for v in memo_valid.iter_mut() { *v=false; }
        }
        let mk=machine_avail.into_iter().max().unwrap_or(0);
        Ok((Solution{job_schedule},mk))
    }
    ...
    fn iterative_cp_descent(
        ...
        let mut ds = build_disj_from_solution(pre, challenge, base_sol)?;
        let mut buf = EvalBuf::new(ds.n);
        let Some(mut current_mk) = eval_disj_screen(&ds, &mut buf) else { return Ok(None); };
        let initial_mk = current_mk;
        let n = ds.n;
            ...
            for node in candidate_nodes {
                ...
                for &(new_m, new_pt) in &op_info.machines {
                    ...
                    for &pos in &positions {
                        ...
                        if let Some(tmk) = eval_disj_screen(&ds, &mut buf) {
                            if tmk < best_mk {
                                best_mk = tmk;
                                best_m = new_m;
                                best_pt = new_pt;
                ...
                if best_mk < current_mk {
                    ...
                    if let Some(mk_after) = eval_disj_screen(&ds, &mut buf) {
                        current_mk = mk_after;
                    } else {
                        current_mk = best_mk;
                    }
        ...
        for _iter in 0..max_iters {
            ...
            for &nd in &cp_flex {
                ...
                if best_m != cur_m {
                    ...
                    current_mk = best_mk;
                    let _ = eval_disj_screen(&ds, &mut buf);
        ...
        let Some(final_mk) = eval_disj_screen(&ds, &mut buf) else { return Ok(None); };
                        ...
                        for state in &beam {
                            ...
                            for &(new_m, new_pt) in &alternatives {
                                ...
                                for &pos in &positions {
                                    ...
                                    if let Some(tmk) = eval_disj_screen(&ds_copy, &mut tmp_buf) {
                                        evals += 1;
    ...
    pub fn solve(
        ...
        if pre.flow_route.is_some()&&pre.flow_pt_by_job.is_some() {
            if let Ok((sol,mk))=neh_reentrant_flow_solution(pre,challenge.num_jobs,challenge.num_machines) {
                if mk<best_makespan{best_makespan=mk;best_solution=Some(sol.clone());save_solution(&sol)?;}
                push_top_solutions_diverse(pre, challenge, &mut top_solutions, &sol, mk, effort.fjsp_high_pool_width);
            }
        }
        let mut ranked: Vec<(Rule,u32,Solution)>=Vec::with_capacity(rules.len());
        for &rule in &rules {
        ...
        for (sol,mk) in refine_results{push_top_solutions_diverse(pre, challenge, &mut top_solutions, &sol, mk, effort.fjsp_high_pool_width);}
        top_solutions.sort_by_key(|x| x.1);
        let ls_runs=top_solutions.len().min(effort.fjsp_high_pool_width);
        ...
        for base_sol in &ls_seeds {
            if let Some((sol2,mk2))=critical_block_move_local_search_ex(pre,challenge,base_sol,8,128,24)?{
                if mk2<best_makespan{best_makespan=mk2;best_solution=Some(sol2.clone());save_solution(&sol2)?;}
                push_top_solutions_diverse(pre, challenge, &mut top_solutions, &sol2, mk2, effort.fjsp_high_pool_width);
            }
        }

        top_solutions.sort_by_key(|x| x.1);
        let cp_runs = top_solutions.len().min(effort.fjsp_high_pool_width);
        ...
        top_solutions.sort_by_key(|x| x.1);
        let cpw_runs = top_solutions.len().min(effort.fjsp_high_pool_width);
        ...
        // Toutes les phases de raffinement amont sont des descentes STRICTES : le
        // moteur n'a aucun mécanisme d'échappement des optima locaux. Cette phase
        // rejoue le MÊME voisinage réduit (blocs critiques) en changeant seulement
        // la règle d'acceptation, en repartant de l'incumbent et des élites.
        // Elle est ADDITIVE et MONOTONE (n'accepte qu'une amélioration stricte du
        // makespan) et ne tire RIEN sur `rng` ⇒ tout l'amont reste byte-exact.
        if effort.fjsp_high_esc_mode > 0
            && effort.fjsp_high_esc_iters > 0
            && effort.fjsp_high_esc_seeds > 0
        {
            top_solutions.sort_by_key(|x| x.1);
            let mut esc_seeds: Vec<Solution> =
                Vec::with_capacity(effort.fjsp_high_esc_seeds.saturating_add(1));
            if let Some(ref sol) = best_solution {
                esc_seeds.push(sol.clone());
            }
            for x in top_solutions.iter().take(effort.fjsp_high_esc_seeds) {
                esc_seeds.push(x.0.clone());
            }
            for base_sol in &esc_seeds {
                if let Ok(Some((sol2, mk2))) = rn_escape_search(
                    pre,
                    challenge,
                    base_sol,
                    effort.fjsp_high_esc_mode,
                    effort.fjsp_high_esc_iters,
                    64,
                ) {
                    if mk2 < best_makespan {
                        best_makespan = mk2;
