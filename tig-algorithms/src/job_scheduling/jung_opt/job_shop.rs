...
fn collect_guided_kick_moves(
    ...
    used_machine.fill(0);
    let mut u = mk_node;
    // E21-S4: `best_pred[v]` is valid exactly when `start[v] != 0` (it is written in the same
    // statement that raises `start[v]`), so testing `start` replaces the NONE sentinel test
    // and lets `eval()` skip clearing `best_pred`.  Same walk, same nodes, same order.
    loop {
        ...
        if start[u] == 0 { break; }
        u = best_pred[u] as usize;
    }
}

// ---------------------------------------------------------------------------
// E19: O(1) acyclicity guards for disjunctive-graph moves.
//
// The disjunctive graph here has, for every node x, arcs x -> job_succ[x] and
// x -> machine_succ[x], each of weight pt[x].  The forward longest-path pass computes
//     heads[x] == g.start[x] == L(0, x)            (longest path source -> x, EXCLUDING pt[x])
// and the backward pass computes
//     tails[x]                                     with  pt[x] + tails[x] == L(x, sink).
// Both are only defined when the graph is acyclic, which it is at every point where the
// guards below are evaluated.
//
// E23 note on the merge with the E21 speed patches: the guards read `heads` and `tails`
// only.  Under E21 those come from `FastGraph::eval` / `FastGraph::tails` instead of
// `eval_disj` / the Kahn backward pass, but both are bit-identical replacements with the
// SAME semantics -- `g.start[x]` is still L(0,x) excluding pt[x] (it is the same relaxation
// written branchlessly), and `g.tail[x]` is still the max over successors of
// pt[succ] + tail[succ] (a tail label is a max, so any reverse-topological order, including
// the reversed pop order of the matching `eval()`, yields the same values).  So the
// certificates below are unchanged in both form and validity.
//
// The O(1) makespan estimates (`estimate_swap_mk`, `estimate_block_insertion_*`) look only
// at the endpoints of the moved block, so they cannot detect that a move closes a cycle;
// the resulting `ds` then makes the longest-path pass return None and the tabu phase
// aborts.  The two tests below are the standard sufficient conditions of Balas &
// Vazacopoulos (1998), which certify acyclicity before the move is ever applied.
// ---------------------------------------------------------------------------

/// Forward interchange: `u` is removed from its position and re-inserted immediately AFTER
/// `v` (same machine, `u` currently before `v`; everything strictly between them shifts one
/// slot left).  The removed arcs are MP(u)->u, u->x and v->MS(v); the added arcs are
/// MP(u)->x, v->u and u->MS(v).  A cycle must use v->u, i.e. needs a path u ~> v in the new
/// graph; that path can only leave u through JS(u) (MS(v) sits after v, and reaching v from
/// there would already be a cycle in the old graph).  So:
///
///     new graph cyclic  <=>  the old graph contains a path JS(u) ~> v.
///
/// Any such path has weight >= pt[JS(u)] (its first arc), and L(a,sink) >= w(a~>b) + L(b,sink)
/// for every path, so a path JS(u) ~> v forces
///     L(JS(u),sink) >= pt[JS(u)] + L(v,sink).
/// Contrapositive, with L(x,sink) == pt[x] + tails[x], the pt[JS(u)] terms cancel:
///
///     tails[JS(u)] < pt[v] + tails[v]      ==>   no path   ==>   the move is acyclic.
///
/// (The degenerate JS(u) == v -- only reachable with re-entrant routings -- is a zero-arc
/// "path" and is excluded explicitly.)
///
/// E23: generic over the index width so the same code serves the `usize` arrays of the
/// property test and the `u32` arrays of `FastGraph`.  Monomorphised; no runtime cost.
#[inline]
fn move_after_is_acyclic<I: NodeIdx>(u: usize, v: usize, tails: &[u32], pt: &[u32], job_succ: &[I]) -> bool {
    let js_u = job_succ[u];
    if js_u.is_none() { return true; }
    if js_u.idx() == v { return false; }
    tails[js_u.idx()] < pt[v].saturating_add(tails[v])
}

/// Backward interchange: `v` is removed from its position and re-inserted immediately BEFORE
/// `u` (same machine, `u` currently before `v`).  Mirror of the above: a cycle must use the
/// added arc v->u, i.e. needs a path u ~> JP(v) closing u ~> JP(v) -> v -> u.  Such a path has
/// weight >= pt[u], and L(0,b) >= L(0,a) + w(a~>b), so it forces
///     heads[JP(v)] >= heads[u] + pt[u].
/// Contrapositive:
///
///     heads[JP(v)] < heads[u] + pt[u]      ==>   no path   ==>   the move is acyclic.
#[inline]
fn move_before_is_acyclic<I: NodeIdx>(u: usize, v: usize, heads: &[u32], pt: &[u32], job_pred: &[I]) -> bool {
    let jp_v = job_pred[v];
    if jp_v.is_none() { return true; }
    if jp_v.idx() == u { return false; }
    heads[jp_v.idx()] < heads[u].saturating_add(pt[u])
}

/// The two shapes emitted by the critical-block loop, in terms of its `(is_bwd, from, to)`:
///   is_bwd == true  : from < to, `relocate_machine_seq` rotates left  -> seq[from] lands
///                     immediately after seq[to]           (forward interchange)
///   is_bwd == false : to < from, `relocate_machine_seq` rotates right -> seq[from] lands
///                     immediately before seq[to]          (backward interchange)
/// For d == 1 both reduce to reversing a critical arc, and both tests then hold
/// automatically, so this rejects nothing that the d == 1 neighbourhood used to generate.
#[inline]
fn relocation_is_acyclic<I: NodeIdx>(
    is_bwd: bool, node_from: usize, node_to: usize,
    heads: &[u32], tails: &[u32], pt: &[u32], job_pred: &[I], job_succ: &[I],
) -> bool {
    if is_bwd {
        move_after_is_acyclic(node_from, node_to, tails, pt, job_succ)
    } else {
        move_before_is_acyclic(node_to, node_from, heads, pt, job_pred)
    }
}

/// Adjacent swap of `a` (position p) and `b` (position p+1) on one machine, as performed by
/// the guided kicks.  NOT used in production -- the kick site uses the exact test instead,
/// see there -- but measured by the property test so the trade-off is documented.
/// It is simultaneously "move a after b" and "move b before a" and both produce the same
/// graph, so either sufficient condition certifies it.  Unlike the block moves, (a,b) here
/// need NOT be a critical arc, which is why these kicks can create cycles.
#[cfg(feature = "guard_verify")]
#[inline]
fn adjacent_swap_is_acyclic<I: NodeIdx>(
    a: usize, b: usize,
    heads: &[u32], tails: &[u32], pt: &[u32], job_pred: &[I], job_succ: &[I],
) -> bool {
    move_after_is_acyclic(a, b, tails, pt, job_succ)
        || move_before_is_acyclic(a, b, heads, pt, job_pred)
}

/// Exactly the backward (tail) pass that the tabu main loop used before E21, factored out for
/// the property test.  Requires an acyclic graph and `machine_succ`/`machine_pred_node`
/// consistent with `ds.machine_seq`.
#[cfg(feature = "guard_verify")]
fn compute_tails_into(
    n: usize, ds: &DisjSchedule, machine_succ: &[usize],
    job_pred_node: &[usize], machine_pred_node: &[usize], job_back_deg: &[u16],
    tail: &mut [u32], back_deg: &mut [u16], back_stack: &mut Vec<usize>,
) {
    tail[..n].fill(0);
    back_deg[..n].copy_from_slice(&job_back_deg[..n]);
    for i in 0..n { if machine_succ[i] != NONE_USIZE { back_deg[i] += 1; } }
...
/// Apply `relocate_machine_seq` on machine `m` and restore every incremental index the tabu
/// loop maintains.  Used only by the guard_mode == 2 undo/fallback path, so the baseline path
/// is untouched.
///
/// E23 -- THIS IS A `machine_seq` MUTATION SITE, so P3's incremental machine-arc repair MUST
/// run here too, or `g.mach_succ` / `g.mach_pred` / `g.indeg0` go stale and the next
/// `g.eval()` / `g.tails()` silently computes labels for a different graph.  `old_first` is
/// captured BEFORE the rotation because `repair_machine_arcs` needs it to fix `indeg0` when
/// position 0 of the sequence changes.  Note that `repair_machine_arcs` recomputes every arc
/// in the window `lo..=hi` plus the two boundary arcs from the sequence as it now stands, so
/// calling this with `(to, from)` is an exact inverse of calling it with `(from, to)`: the
/// window is the same and the arcs are re-derived, not incrementally toggled.
#[inline]
fn apply_machine_move(
    ds: &mut DisjSchedule, g: &mut FastGraph, m: usize, from: usize, to: usize,
    current_pos: &mut [usize],
) {
    let old_first = ds.machine_seq[m][0];
    relocate_machine_seq(&mut ds.machine_seq[m], from, to);
    let (lo, hi) = if from < to { (from, to) } else { (to, from) };
    let seq = &ds.machine_seq[m];
    for i in lo..=hi { current_pos[seq[i]] = i; }
    g.repair_machine_arcs(seq, lo, hi, old_first);
}
...
fn tabu_search_phase(pre: &Pre, challenge: &Challenge, base_sol: &Solution, max_iterations: usize, tenure_base: usize) -> Result<Option<(Solution, u32)>> {
    super::knobs::stat(&super::knobs::TS_PHASES);
    let mut ds = build_disj_from_solution(pre, challenge, base_sol)?;
    let n = ds.n;
    let mut g = FastGraph::new(&ds);
    let Some((initial_mk, mut mk_node)) = g.eval() else { return Ok(None) };
    let mut cur_mk = initial_mk; let mut best_global_mk = initial_mk; let mut best_global_machine_seq = ds.machine_seq.clone();
    let tenure = tenure_base.max(5); let tenure_delta = (tenure/3).max(2); let max_no_improve = (max_iterations/super::knobs::no_improve_div(2)).max(60);
    ...
    let kick_threshold = (max_no_improve*2/3).max(40); let diversify_threshold = (max_no_improve/3).max(20); let diversify_unit = (pre.avg_op_min * (0.10 + 0.16*pre.jobshopness + 0.06*pre.high_flex)).max(1.0) as u32; let mut kicks_left = super::knobs::ts_kicks(4);
    // 0 = off (baseline), 1 = filter cycle-creating moves out of the neighbourhood,
    // 2 = keep the full neighbourhood and fall back to a certified move if the chosen one
    //     turns out to close a cycle.  1 and 2 both make the phase abort impossible.
    let guard_mode = super::knobs::acyclic_guard();
    let acyclic_filter = guard_mode == 1;
    let acyclic_certify = guard_mode == 2;
    // E23: the guided-kick site tests feasibility with the stock `eval_disj`, which needs its
    // own `EvalBuf` now that the main loop runs on `FastGraph`.  At knob 0 that site is dead,
    // so the buffer is allocated at length 0 and the baseline path allocates nothing extra.
    let mut kick_buf = EvalBuf::new(if guard_mode == 0 { 0 } else { n });
    #[cfg(feature = "guard_verify")]
    const GUARD_VERIFY_STRIDE: u64 = 512;
    #[cfg(feature = "guard_verify")]
    let mut cand_ctr: u64 = 0;
    #[cfg(feature = "guard_verify")]
    let mut verify_sample: Vec<(usize, usize, usize, bool)> = Vec::new();
    #[cfg(feature = "guard_verify")]
    let mut verify_buf = EvalBuf::new(n);
    for iter in 0..max_iterations {
        ...
        if no_improve >= max_no_improve {
            ...
            let Some((mk, node)) = g.eval() else { super::knobs::stat(&super::knobs::TS_ABORTS); ds.machine_seq.clone_from(&best_global_machine_seq); cur_mk = best_global_mk; break; };
        ...
        if no_improve > 0 && no_improve % kick_threshold == 0 && kicks_left > 0 {
            ...
            collect_guided_kick_moves(&ds, &g.best_pred, &g.start, mk_node, &current_pos, &node_machine, ref_first, &mut guided_used_machine, &mut guided_kicks);
            let ref_pos = if guided_kicks.is_empty() {
                collect_guided_kick_moves(&ds, &g.best_pred, &g.start, mk_node, &current_pos, &node_machine, ref_second, &mut guided_used_machine, &mut guided_kicks);
                ref_second
            } else {
                ref_first
            };
            if !guided_kicks.is_empty() {
                ...
                for &(_, m, pos) in &guided_kicks {
                    ...
                    // E19: these kicks swap a pair that need NOT be a critical arc, on several
                    // machines at once, so they really can close a cycle.  This site runs at
                    // most `ts_kicks` (4) times per phase with at most 4 swaps each, so the
                    // EXACT test (apply, one Kahn pass, undo on failure) costs ~16 O(n) passes
                    // per phase -- nothing against 10^5..10^6 tabu iterations -- and unlike the
                    // O(1) certificate it discards no feasible kick.  (`adjacent_swap_is_acyclic`
                    // above certifies 88% of them, but ~86% of the ones it rejects are in fact
                    // feasible; measured, see patch_E19_acyclic.md SECTION 3.)
                    //
                    // E23 / P3 interaction -- `machine_seq` MUTATION SITE.  The test uses the
                    // stock `eval_disj`, which re-derives `machine_succ` and all in-degrees
                    // from `ds.machine_seq` on every call, so it needs no incremental repair
                    // and it does not read or write `g`.  `g`'s machine arcs therefore stay at
                    // the pre-kick sequence throughout this loop, which is harmless because
                    // every path out of the kick branch below runs a full
                    // `g.rebuild_machine_arcs(&ds)` before the next `g.eval()`.
                    if guard_mode != 0 && eval_disj(&ds, &mut kick_buf).is_none() {
                        ds.machine_seq[m].swap(pos, pos + 1);
                        super::knobs::stat(&super::knobs::TS_GUARD_KICK_SKIPS);
                        continue;
                    }
        ...
        #[cfg(feature = "prof")] let _p5 = super::knobs::prof::now();
        #[cfg(feature = "prof")] let _p0 = super::knobs::prof::now();
        // E21-S2: the q-labels (tails) are a max over successors, so ANY reverse-topological
        // order produces the same values.  `buf.topo` is the pop order of the eval_disj that
        // produced the current `buf.start`/`mk_node`, i.e. a topological order of exactly this
        // graph; iterating it backwards is therefore a valid reverse-topological order and
        // makes the whole Kahn scaffolding (back_deg / back_stack / the two O(n) scans)
        // unnecessary.  Values are bit-identical.
        // INCR wave 4: with incremental evaluation on, a successful `incr_move` has already
        // brought `tail[]` up to date for the current graph, so this full backward pass is
        // skipped.  At `incr_eval == 0` `tails_fresh` is always false here (every `eval()`
        // clears it), so the call is unconditional exactly as before.
        if !g.tails_fresh { g.tails(); }
        #[cfg(feature = "prof")] super::knobs::prof::acc(0, _p0);
        #[cfg(feature = "prof")] let _p1 = super::knobs::prof::now();
        for &m in &crit_pos_machines { crit_pos_by_machine[m].clear(); }
        crit_pos_machines.clear();
        let mut u = mk_node;
        // E21-S4: `best_pred[v]` is valid exactly when `start[v] != 0`; see FastGraph::eval.
        loop {
            let m = node_machine[u];
            if crit_pos_by_machine[m].is_empty() { crit_pos_machines.push(m); }
            crit_pos_by_machine[m].push(current_pos[u]);
            #[cfg(feature = "prof")] super::knobs::prof::bump(6, 1);
            if g.start[u] == 0 { break; }
            u = g.best_pred[u] as usize;
        }
        crit_pos_machines.sort_unstable();
        #[cfg(feature = "prof")] super::knobs::prof::acc(1, _p1);
        #[cfg(feature = "prof")] let _p2 = super::knobs::prof::now();
        ...
        let mut best_cert: Option<(usize,usize,usize,u32)> = None; let mut best_cert_key = u32::MAX;
        let mut relaxed_cert: Option<(usize,usize,usize,u32)> = None; let mut relaxed_cert_key = u32::MAX;
        for &m in &crit_pos_machines {
            ...
            for idx in 1..positions.len() {
                ...
                let node = seq[pos];
                if pos == prev_pos + 1 && g.start[node] == g.start[prev_node].saturating_add(g.pt[prev_node]) {
                    run_end = pos;
                } else {
                    if run_end > run_start {
                        let block_len = run_end-run_start+1;
                        let max_dist = super::knobs::max_dist(4).min(block_len - 1);
                        for d in 1..=max_dist {
                            for is_bwd in [true, false] {
                                if !is_bwd && block_len <= 2 { continue; }
                                let (from, to) = if is_bwd { (run_start, run_start + d) } else { (run_end, run_end - d) };
                                super::knobs::stat(&super::knobs::TS_CANDIDATES);
                                // E19 guard.  For d == 1 this is the N1/N5 shape (reversal of a
                                // critical arc), which can never create a cycle -- and the tests
                                // provably accept it, so no d == 1 move is ever lost (asserted by
                                // the `d1_would_reject == 0` counter, see patch_E19_acyclic.md SECTION 3).
                                // E23: `g.start` / `g.tail` are the FastGraph heads/tails, which are
                                // bit-identical to the `buf.start` / `tail` this guard was validated
                                // against, with identical semantics.
                                let certified = guard_mode == 0
                                    || relocation_is_acyclic(is_bwd, seq[from], seq[to], &g.start, &g.tail, &g.pt, &g.job_pred, &g.job_succ);
                                #[cfg(feature = "guard_verify")]
                                {
                                    let raw = relocation_is_acyclic(is_bwd, seq[from], seq[to], &g.start, &g.tail, &g.pt, &g.job_pred, &g.job_succ);
                                    if d == 1 && !raw { super::knobs::statv(&super::knobs::VERIFY_D1_REJECT); }
                                    cand_ctr += 1;
                                    if cand_ctr % GUARD_VERIFY_STRIDE == 0 { verify_sample.push((m, from, to, raw)); }
                                }
                                if acyclic_filter && !certified {
                                    super::knobs::stat(&super::knobs::TS_GUARD_SKIPS);
                                    continue;
                                }
                                let est_mk = if d == 1 {
                                    let (u, v) = if is_bwd { (seq[from], seq[to]) } else { (seq[to], seq[from]) };
                                    estimate_swap_mk(u, v, &g.start, &g.tail, &g.pt, &g.job_pred, &g.job_succ, &g.mach_pred, &g.mach_succ)
                                } else {
                                    if is_bwd {
                                        estimate_block_insertion_backward(from, to, seq, &g.start, &g.tail, &g.pt, &g.job_pred, &g.job_succ, &g.mach_pred, &g.mach_succ)
                                    } else {
                                        estimate_block_insertion_forward(to, from, seq, &g.start, &g.tail, &g.pt, &g.job_pred, &g.job_succ, &g.mach_pred, &g.mach_succ)
                                    }
                                };
                                #[cfg(feature = "prof")] super::knobs::prof::bump(7, 1);
                                ...
                                if acyclic_certify && certified {
                                    if (!is_tabu||aspiration) && adj_mk<best_cert_key { best_cert_key=adj_mk; best_cert=Some((m,from,to,est_mk)); }
                                    if adj_mk<relaxed_cert_key { relaxed_cert_key=adj_mk; relaxed_cert=Some((m,from,to,est_mk)); }
                                }
                            }
                        }
                    }
        ...
        #[cfg(feature = "prof")] super::knobs::prof::acc(2, _p2);
        // Ground-truth cross-check of the sampled candidates: apply, run the full Kahn
        // pass, undo. `ds` is restored exactly, so the search trajectory is untouched.
        // E23: this uses `eval_disj` on its own buffer and never touches `g`; `ds` is
        // returned to its exact previous permutation, so `g`'s incrementally maintained
        // machine arcs stay valid without any repair.
        #[cfg(feature = "guard_verify")]
        {
            for k in 0..verify_sample.len() {
                let (vm, vfrom, vto, verdict) = verify_sample[k];
                relocate_machine_seq(&mut ds.machine_seq[vm], vfrom, vto);
                let actual_acyclic = eval_disj(&ds, &mut verify_buf).is_some();
                relocate_machine_seq(&mut ds.machine_seq[vm], vto, vfrom);
                match (verdict, actual_acyclic) {
                    (true, true) => super::knobs::statv(&super::knobs::VERIFY_TRUE_ACCEPT),
                    (true, false) => super::knobs::statv(&super::knobs::VERIFY_UNSOUND_ACCEPT),
                    (false, false) => super::knobs::statv(&super::knobs::VERIFY_TRUE_REJECT),
                    (false, true) => super::knobs::statv(&super::knobs::VERIFY_FALSE_REJECT),
                }
            }
            verify_sample.clear();
        }
        ...
        match chosen {
            Some((m,from,to,_est)) => {
                #[cfg(feature = "prof")] let _p3 = super::knobs::prof::now();
                ...
                machine_last_pick[m] = iter;
                let seq = &ds.machine_seq[m];
                // INCR wave 4 -- `machine_seq` MUTATION SITE #8.  When incremental evaluation
                // is armed AND the incremental state is live (`ord_valid` after the last full
                // eval, `tails_fresh` from the top of this iteration), `incr_move` performs the
                // arc surgery ITSELF (remove the old chain in the window, install the new one
                // arc by arc through Pearce-Kelly) and repairs `ord`/`start`/`best_pred`/`tail`
                // over just the perturbed region -- so `repair_machine_arcs` must NOT also run.
                // In every other case the original O(hi-lo) arc repair runs and a full pass
                // follows, exactly as in E23.
                let incr_res = if g.incr && g.ord_valid && g.tails_fresh {
                    super::knobs::stat(&super::knobs::INCR_MOVES);
                    Some(g.incr_move(seq, lo, hi))
                } else {
                    g.repair_machine_arcs(seq, lo, hi, old_first);
                    None
                };
                ...
                #[cfg(feature = "prof")] super::knobs::prof::acc(3, _p3);
                #[cfg(feature = "prof")] let _p4 = super::knobs::prof::now();
                let evres = match incr_res {
                    Some(IncrOut::Ok(a, b)) => Some((a, b)),
                    // Pearce-Kelly proved the move closes a cycle; identical verdict to a full
                    // `eval()` returning None, and `g`'s machine arcs already match `ds`.
                    Some(IncrOut::Cycle) => { super::knobs::stat(&super::knobs::INCR_CYCLES); None }
                    // perturbed region too large (or incremental state invalidated): the arcs
                    // are consistent, so a plain full pass recovers everything.
                    Some(IncrOut::Bail) => { super::knobs::stat(&super::knobs::INCR_BAILS); g.eval() }
                    None => g.eval(),
                };
                #[cfg(feature = "incr_verify")]
                if let Some((vmk, vnode)) = evres {
                    if let Err(e) = g.verify_against_full(&ds, vmk, vnode) {
                        panic!("incr_verify failed at iter {}: {}", iter, e);
                    }
                }
                let (mk, node) = match evres {
                    Some(x) => x,
                    None => {
                        // The applied move closed a cycle; the O(1) head/tail estimates cannot
                        // see that.  With guard_mode == 2 we undo it exactly and take the best
                        // candidate that the acyclicity certificate proved safe -- the phase
                        // keeps going and the full neighbourhood was still searched.
                        //
                        // E23: both `apply_machine_move` calls repair `g`'s machine arcs, so
                        // the second `g.eval()` runs on a graph that matches `ds.machine_seq`
                        // exactly.  A failed `g.eval()` leaves `g.start` / `g.topo` partially
                        // written, but the only readers are the `g.tails()` and critical-path
                        // walk at the top of the NEXT iteration, and both are preceded by a
                        // successful `g.eval()` on every path that reaches them.
                        super::knobs::stat(&super::knobs::TS_ABORTS);
                        let mut recovered = None;
                        if acyclic_certify {
                            apply_machine_move(&mut ds, &mut g, m, to, from, &mut current_pos);
                            if let Some((m2, from2, to2, _)) = best_cert.or(relaxed_cert) {
                                let node_a2 = ds.machine_seq[m2][from2]; let node_b2 = ds.machine_seq[m2][to2];
                                apply_machine_move(&mut ds, &mut g, m2, from2, to2, &mut current_pos);
                                machine_last_pick[m2] = iter;
                                let la2 = node_local[node_a2]; let lb2 = node_local[node_b2];
                                let (a2, b2) = if la2 < lb2 { (la2, lb2) } else { (lb2, la2) };
                                let tabu_idx2 = pair_offsets[m2] + b2 * (b2 - 1) / 2 + a2;
                                tabu_expiry[tabu_idx2] = iter + this_tenure;
                                super::knobs::stat(&super::knobs::TS_FALLBACKS);
                                recovered = g.eval();
                            }
                        }
                        match recovered {
                            Some(x) => x,
                            None => { ds.machine_seq.clone_from(&best_global_machine_seq); cur_mk = best_global_mk; break; }
                        }
                    }
                };
                #[cfg(feature = "prof")] super::knobs::prof::acc(4, _p4);
                #[cfg(feature = "prof")] super::knobs::prof::acc(5, _p5);
                cur_mk = mk; mk_node = node;
            }
            None => break,
        }
    ...
    g.rebuild_machine_arcs(&ds);
    let Some((mk_final,_)) = g.eval() else { return Ok(None) };
    let sol = disj_to_solution(pre, &ds, &g.start)?;
    Ok(Some((sol, mk_final)))
}

#[inline]
fn estimate_block_insertion_forward<I: NodeIdx>(
    run_start: usize, run_end: usize, seq: &[usize],
    heads: &[u32], tails: &[u32], pt: &[u32],
    job_pred: &[I], job_succ: &[I],
    machine_pred: &[I], machine_succ: &[I]
) -> u32 {
    let v = seq[run_end];
    let u = seq[run_start];
    let w = seq[run_end - 1];
    let jp_v = job_pred[v];
    let r_jp_v = if !jp_v.is_none() { heads[jp_v.idx()].saturating_add(pt[jp_v.idx()]) } else { 0 };
    let mp_u = machine_pred[u];
    let r_mp_u = if !mp_u.is_none() { heads[mp_u.idx()].saturating_add(pt[mp_u.idx()]) } else { 0 };
    let new_r_v = r_jp_v.max(r_mp_u);
    let js_v = job_succ[v];
    let q_js_v = if !js_v.is_none() { pt[js_v.idx()].saturating_add(tails[js_v.idx()]) } else { 0 };
    let jp_u = job_pred[u];
    let r_jp_u = if !jp_u.is_none() { heads[jp_u.idx()].saturating_add(pt[jp_u.idx()]) } else { 0 };
    let new_r_u = r_jp_u.max(new_r_v.saturating_add(pt[v]));
    let js_u = job_succ[u];
    let q_js_u = if !js_u.is_none() { pt[js_u.idx()].saturating_add(tails[js_u.idx()]) } else { 0 };
    let js_w = job_succ[w];
    let q_js_w = if !js_w.is_none() { pt[js_w.idx()].saturating_add(tails[js_w.idx()]) } else { 0 };
    let ms_v = machine_succ[v];
    let q_ms_v = if !ms_v.is_none() { pt[ms_v.idx()].saturating_add(tails[ms_v.idx()]) } else { 0 };
...
#[inline]
fn estimate_block_insertion_backward<I: NodeIdx>(
    run_start: usize, run_end: usize, seq: &[usize],
    heads: &[u32], tails: &[u32], pt: &[u32],
    job_pred: &[I], job_succ: &[I],
    machine_pred: &[I], machine_succ: &[I]
    ...
    let v = seq[run_end];
    let jp_x = job_pred[x];
    let r_jp_x = if !jp_x.is_none() { heads[jp_x.idx()].saturating_add(pt[jp_x.idx()]) } else { 0 };
    let mp_u = machine_pred[u];
    let r_mp_u = if !mp_u.is_none() { heads[mp_u.idx()].saturating_add(pt[mp_u.idx()]) } else { 0 };
...
#[inline]
fn estimate_swap_mk<I: NodeIdx>(u: usize, v: usize, heads: &[u32], tails: &[u32], pt: &[u32], job_pred: &[I], job_succ: &[I], machine_pred: &[I], machine_succ: &[I]) -> u32 {
    ...
    let r_jp_v=if !jp_v.is_none(){heads[jp_v.idx()].saturating_add(pt[jp_v.idx()])}else{0}; let r_mp_u=if !mp_u.is_none(){heads[mp_u.idx()].saturating_add(pt[mp_u.idx()])}else{0};
    let new_r_v=r_jp_v.max(r_mp_u); let r_jp_u=if !jp_u.is_none(){heads[jp_u.idx()].saturating_add(pt[jp_u.idx()])}else{0}; let new_r_u=r_jp_u.max(new_r_v.saturating_add(pt[v]));
    let q_js_u=if !js_u.is_none(){pt[js_u.idx()].saturating_add(tails[js_u.idx()])}else{0}; let q_ms_v=if !ms_v.is_none(){pt[ms_v.idx()].saturating_add(tails[ms_v.idx()])}else{0};
    let new_q_u=q_js_u.max(q_ms_v); let q_js_v=if !js_v.is_none(){pt[js_v.idx()].saturating_add(tails[js_v.idx()])}else{0}; let new_q_v=q_js_v.max(pt[u].saturating_add(new_q_u));
...
fn critical_block_move_local_search_ex_disj(
    ...
    for _ in 0..iter_limit {
        ...
        const EVALCAP_MAX: usize = 24;
        let eval_cap = moves.len().min(super::knobs::eval_cap(2)).min(EVALCAP_MAX);
        let mut best_idx = NONE_USIZE;
        let mut best_actual_mk = cur_mk;
        let mut best_actual_node = NONE_USIZE;
        let mut tested = [(NONE_USIZE, NONE_USIZE, NONE_USIZE); EVALCAP_MAX];
        ...
        if best_idx != NONE_USIZE {
            ...
            if best_idx == 0 {
                ...
                // test_buf_b holds the LAST trial, not necessarily best_idx's, so
                // recompute exactly rather than installing a mismatched buffer.
                if eval_disj(ds, buf).is_none() { break; }
            }
            stalled = 0;
        } else {
            stalled += 1;
...
pub fn solve(
    ...
    let num_restarts = super::knobs::num_restarts(450);
    ...
    let ts_starts = top_solutions.len().min(super::knobs::ts_starts(10));
    let ts_iters = effort.job_shop_iters;
    let ts_tenure = super::knobs::ts_tenure(((pre.total_ops as f64).sqrt() as usize * (100 + (pre.load_cv * 60.0) as usize) / 100).clamp(8, 24));
    {
...
// ---------------------------------------------------------------------------
// E19 property test (feature `guard_verify`, harness only).
//
// Generates uniformly random FEASIBLE selections with a Bierwirth
// permutation-with-repetition decoder (any permutation of the multiset
// {job j repeated |ops(j)| times} decodes to an acyclic disjunctive graph), then
// checks the three guard predicates against ground truth obtained by actually
// applying the move and running the full topological pass in `eval_disj`.
//
// Returns, per move class, (tested, accepted, rejected, false_rejects, unsound_accepts).
// A non-zero `unsound_accepts` means the guard is too weak (cycles get through);
// a large `false_rejects` means it is too strict (the neighbourhood silently shrinks).
// ---------------------------------------------------------------------------
#[cfg(feature = "guard_verify")]
pub fn acyclicity_property_test(
    challenge: &Challenge, states: usize, moves_per_state: usize, seed0: u64,
) -> Result<[[u64; 5]; 3]> {
    #[inline]
    fn xs(s: &mut u64) -> u64 { *s ^= *s << 13; *s ^= *s >> 7; *s ^= *s << 17; *s }

    let pre = super::preprocess::build_pre(challenge)?;
    let (sol, _) = run_simple_greedy_baseline(challenge)?;
    let mut ds = build_disj_from_solution(&pre, challenge, &sol)?;
    let n = ds.n;
    let mut buf = EvalBuf::new(n);
    let mut buf2 = EvalBuf::new(n);

    let mut job_pred_node = vec![NONE_USIZE; n];
    for j in 0..ds.num_jobs { let b = ds.job_offsets[j]; let e = ds.job_offsets[j + 1]; for k in (b + 1)..e { job_pred_node[k] = k - 1; } }
    ...
    let mut chrom: Vec<usize> = Vec::with_capacity(n);
    for j in 0..ds.num_jobs { for _ in ds.job_offsets[j]..ds.job_offsets[j + 1] { chrom.push(j); } }
    let mut next_op = vec![0usize; ds.num_jobs];

    // class 0 = forward insertion (u after v), 1 = backward insertion (v before u),
    // 2 = adjacent swap (the guided-kick shape, deliberately NOT restricted to critical arcs)
    let mut out = [[0u64; 5]; 3];
    let mut rng: u64 = seed0 | 1;

    for _state in 0..states {
        for i in (1..chrom.len()).rev() { let r = xs(&mut rng) as usize; chrom.swap(i, r % (i + 1)); }
        for m in 0..ds.num_machines { ds.machine_seq[m].clear(); }
        for x in next_op.iter_mut() { *x = 0; }
        for &j in chrom.iter() {
            let id = ds.job_offsets[j] + next_op[j];
            next_op[j] += 1;
            let m = ds.node_machine[id];
            ds.machine_seq[m].push(id);
        }
        if eval_disj(&ds, &mut buf).is_none() {
            return Err(anyhow!("Bierwirth decode produced a cyclic selection -- decoder bug"));
        }
        rebuild_machine_pred_nodes(&ds, &mut machine_pred_node);
        compute_tails_into(n, &ds, &buf.machine_succ, &job_pred_node, &machine_pred_node, &job_back_deg, &mut tail, &mut back_deg, &mut back_stack);

        for _k in 0..moves_per_state {
            let m = (xs(&mut rng) as usize) % ds.num_machines;
            let len = ds.machine_seq[m].len();
            if len < 2 { continue; }
            let cls = (xs(&mut rng) % 3) as usize;
            let (i, d) = if cls == 2 {
                ((xs(&mut rng) as usize) % (len - 1), 1usize)
            } else {
                let d = 2 + (xs(&mut rng) as usize) % 3; // 2..=4, the insertion class
                if d >= len { continue; }
                ((xs(&mut rng) as usize) % (len - d), d)
            };
            let j = i + d;
            if j >= len { continue; }
            let u = ds.machine_seq[m][i];
            let v = ds.machine_seq[m][j];
            let (verdict, from, to) = match cls {
                0 => (move_after_is_acyclic(u, v, &tail, &ds.node_pt, &ds.job_succ), i, j),
                1 => (move_before_is_acyclic(u, v, &buf.start, &ds.node_pt, &job_pred_node), j, i),
                _ => (adjacent_swap_is_acyclic(u, v, &buf.start, &tail, &ds.node_pt, &job_pred_node, &ds.job_succ), i, j),
            };
            relocate_machine_seq(&mut ds.machine_seq[m], from, to);
            let actual = eval_disj(&ds, &mut buf2).is_some();
            relocate_machine_seq(&mut ds.machine_seq[m], to, from);
            out[cls][0] += 1;
            if verdict { out[cls][1] += 1; if !actual { out[cls][4] += 1; } }
            else { out[cls][2] += 1; if actual { out[cls][3] += 1; } }
        }
    }
    Ok(out)
}
