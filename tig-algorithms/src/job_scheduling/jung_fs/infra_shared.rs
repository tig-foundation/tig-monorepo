...
pub fn eval_disj(ds: &DisjSchedule, buf: &mut EvalBuf) -> Option<(u32, usize)> {
    let n = ds.n;
    // E43 `hfs_eval_lean`: ONE relaxed load per call, hoisted to a local so the Kahn loop
    // below only ever tests a loop-invariant `bool` (LLVM unswitches it; the branch is
    // perfectly predicted either way).  `lean == false` is the default and reproduces e42
    // exactly.  `lean == true` drops two pieces of per-node bookkeeping:
    //   * the `machine_succ` memset -- redundant, because `ds.machine_seq` partitions all
    //     `n` nodes (`build_disj_from_solution` puts every node into exactly one sequence and
    //     every mutator is a paired remove+insert), so the rebuild loop directly below
    //     overwrites every entry.  `adaptive_js_v9` never had this memset and E31 reproduced
    //     it 40/40 bit-identical on this track.
    //   * `buf.topo` / `buf.topo_len` -- read only by `job_shop.rs`.  Under `lean` they are
    //     left untouched (i.e. stale) and MUST NOT be read; `solver::solve_challenge` enforces
    //     this by clearing the knob on every path except the v9 hybrid engine, whose call tree
    //     never reads them.
    let lean = super::knobs::hfs_eval_lean() == 1;
    // E45 `fm_fuel_lean`: ONE relaxed load per `eval_disj` call (not per node), hoisted to a
    // loop-invariant local exactly like `lean` above.  It selects plain wrapping arithmetic
    // in place of `saturating_*` at the four sites below.
    //
    // WHY THIS IS BIT-IDENTICAL.  `saturating_add`/`saturating_sub` lower to
    // add/sub + overflow-test + cmov (3 priced instructions) where the plain op is 1.  The
    // saturating arm is UNREACHABLE at every one of these sites:
    //
    //   (1) `indeg[v] += 1` (build): `indeg_job` starts at the count of job predecessors,
    //       which is 0 or 1, and this loop adds at most one machine predecessor per node
    //       (each node appears in exactly one `machine_seq`, at one position).  So
    //       `indeg[v] <= 2`, against a u16 ceiling of 65535.
    //   (2) `start[u] + node_pt[u]`: `start[u]` is a longest path in the disjunctive graph,
    //       so `start[u] + pt[u] <= sum of all pt`.  For this challenge family
    //       `n = 50 jobs * ~35 ops ~ 1.8e3` and `pt <= base_proc_time_max * max_speed_factor
    //       = 200 * 1.2 = 240` (tig_challenges::job_scheduling::Challenge::generate), so the
    //       bound is < 5e5 -- five thousand times under u32::MAX.
    //   (3)+(4) `indeg[js/ms] -= 1`: `indeg` counts v's not-yet-relaxed in-edges.  We reach
    //       the decrement only by relaxing one such edge, and each edge is relaxed exactly
    //       once (a node is popped exactly once in a DAG Kahn scan), so `indeg >= 1` here.
    //
    // Since saturation never triggers, the stored values are identical bit-for-bit; only the
    // executed instruction count changes.  Both arms are written out rather than folded so
    // the `else` branch remains a verbatim copy of today's code.
    let fuel_lean = super::knobs::fm_fuel_lean() != 0;
    buf.indeg.clone_from_slice(&ds.indeg_job);
    buf.start.fill(0);
    buf.best_pred.fill(NONE_USIZE);
    if !lean { buf.machine_succ.fill(NONE_USIZE); }
    for seq in &ds.machine_seq {
        ...
        for &v in &seq[1..] {
            ...
            // (1) above.
            buf.indeg[v] = if fuel_lean { buf.indeg[v].wrapping_add(1) } else { buf.indeg[v].saturating_add(1) };
            prev = v;
        }
        buf.machine_succ[prev] = NONE_USIZE;
    }
    // E21-S1: branchless ready-stack. `buf.stack` is kept at physical length n+1 and
    // driven by an explicit `top` cursor, so a node can be written speculatively at
    // `stack[top]` and only *committed* by the conditional `top += (deg==0)`.
    // Bound proof: `top` is incremented at most once per node (each node's in-degree
    // reaches 0 exactly once in a DAG, and the initial scan visits each node once), so
    // `top <= n` always holds and the speculative store at index `top` needs n+1 slots.
    if buf.stack.len() != n + 1 { buf.stack.resize(n + 1, 0); }
    let stack = &mut buf.stack[..n + 1];
    let mut top = 0usize;
    for i in 0..n {
        stack[top] = i;
        top += (buf.indeg[i] == 0) as usize;
    }
    ...
    while top > 0 {
        top -= 1;
        let u = stack[top];
        if !lean { buf.topo[processed] = u; }
        processed += 1;
        // (2) above.
        let end_u = if fuel_lean { buf.start[u].wrapping_add(ds.node_pt[u]) } else { buf.start[u].saturating_add(ds.node_pt[u]) };
        if end_u > mk { mk = end_u; mk_node = u; }
        let js = ds.job_succ[u];
        if js != NONE_USIZE {
            // branchless label relaxation (cmov): identical values, no data-dependent branch
            let s = buf.start[js];
            let better = s < end_u;
            buf.start[js] = if better { end_u } else { s };
            let bp = buf.best_pred[js];
            buf.best_pred[js] = if better { u } else { bp };
            // (3) above.
            let d = if fuel_lean { buf.indeg[js].wrapping_sub(1) } else { buf.indeg[js].saturating_sub(1) };
            buf.indeg[js] = d;
            stack[top] = js;
            top += (d == 0) as usize;
        }
        ...
        if ms != NONE_USIZE {
            let s = buf.start[ms];
            let better = s < end_u;
            buf.start[ms] = if better { end_u } else { s };
            let bp = buf.best_pred[ms];
            buf.best_pred[ms] = if better { u } else { bp };
            // (4) above.
            let d = if fuel_lean { buf.indeg[ms].wrapping_sub(1) } else { buf.indeg[ms].saturating_sub(1) };
            buf.indeg[ms] = d;
            stack[top] = ms;
            top += (d == 0) as usize;
        }
    ...
    if !lean { buf.topo_len = processed; }
    Some((mk, mk_node))
}
...
pub fn route_pref_from_solution_lite(pre: &Pre, sol: &Solution, challenge: &Challenge) -> Result<RoutePrefLite> {
    ...
    for p in 0..np {
        ...
        for op_idx in 0..ol {
            ...
            // E45: memo the two `w as f64 / 255.0` divisions that `route_pref_bonus_fm`
            // performs per candidate.  Same expression, evaluated once per (product, op)
            // here instead of ~1e8 times in the candidate loop, so the f64 result is
            // identical.  Unconditional (it cannot change any track's output, and this
            // function runs O(30) times per nonce), read only under the knob.
            v.push(OpRoute {
                best_m: best_m.min(255) as u8,
                best_w,
                second_m: second_m.min(255) as u8,
                second_w,
                best_wf: (best_w as f64) / 255.0,
                second_wf: (second_w as f64) / 255.0,
            });
        }
...
pub fn push_top_solutions(top: &mut Vec<(Solution, u32)>, sol: &Solution, mk: u32, cap: usize) {
    ...
    // E45 `fm_fuel_lean`: skip a clone that the very next line throws away.
    //
    // BIT-IDENTICAL PROOF.  `pos` is the insertion index, so `pos <= top.len()`.  When
    // `top.len() == cap` and `pos >= cap`, the insert places the new entry at index
    // `pos >= cap` of a length-`cap+1` vector, leaving indices `0..cap` untouched (an insert
    // at `pos` shifts nothing below `pos`); the immediately following `truncate(cap)` then
    // drops exactly the entry just written.  `top` ends byte-for-byte as it started, so the
    // early return is the same function.
    //
    // The `==` is load-bearing and must NOT be relaxed to `>=`.  This vector is pushed with
    // TWO different caps -- 20 in the restart/local-search loops and 15 at the
    // `neh_reentrant` and `refine_results` sites -- so `top.len() > cap` is reachable (len 20
    // meeting a cap-15 call).  In that case the legacy path really does shrink the vector to
    // `cap`, and returning early would leave it at 20.  With `==` we take the legacy path
    // there and only short-circuit the no-op case.
    //
    // This fires on the large majority of the ~50k restarts, because the pool reaches its
    // cap early and later restarts rarely beat it.  Each avoided clone is ~51 allocations
    // (one per job row) plus a ~28 KB copy.
    if super::knobs::fm_fuel_lean() != 0 && top.len() == cap && pos >= cap { return; }
