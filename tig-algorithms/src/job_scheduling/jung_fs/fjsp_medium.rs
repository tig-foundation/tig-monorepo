...
#[inline]
fn route_pref_bonus_fm(rp: Option<&RoutePrefLite>, product: usize, op_idx: usize, machine: usize, lean: bool) -> f64 {
    ...
    // E45 B1: `best_wf`/`second_wf` are `(best_w as f64)/255.0` and `(second_w as f64)/255.0`,
    // computed once per (product, op) in `route_pref_from_solution_lite`.  Same f64 division
    // on the same operands, so the returned bits are identical; the `.min(255)` guard is kept
    // verbatim rather than argued away.
    if mu == r.best_m {
        if lean { r.best_wf } else { (r.best_w as f64) / 255.0 }
    } else if mu == r.second_m {
        if lean { r.second_wf } else { (r.second_w as f64) / 255.0 }
    } else { 0.0 }
}
...
fn score_candidate(
    ...
    ops_rem: usize, op: &OpInfo, machine: usize, mach_idx: usize, pt: u32, time: u32,
    ...
    route_pref: Option<&RoutePrefLite>, route_w: f64, jitter: f64, lean: bool,
) -> f64 {
    // ---------------------------------------------------------------------------------
    // E45 B1 (`fm_fuel_lean`) -- memo lookups replacing per-candidate recomputation.
    //
    // Every quantity in this tuple is a pure function of (product, op_idx), of the op, of
    // the machine, or of the small integer `best_cnt_total`.  None of them depends on
    // `progress`, `time`, `best_end`, the RNG, or any learned vector, yet all of them were
    // being rebuilt on every candidate: ~15k candidates per restart x 50k restarts.
    //
    // This is the one part of `score_candidate` that survives full inlining: the compiler
    // does hoist the loop-invariant `pre.*.max(..)` denominators out of the candidate loop
    // (LICM, `&Pre` being noalias-readonly), but it can NOT memoise across the ~1e8 separate
    // calls, because `product`/`op_idx`/`machine` vary per candidate.  Tables built once per
    // nonce in `preprocess::build_pre` do exactly that.
    //
    // BIT-IDENTICAL: each table entry is the same f64 expression on the same operands with
    // the same association -- see the contract comment in `preprocess.rs`.  The `else` arm
    // below is a verbatim copy of today's code with six bindings lifted out of it
    // (`load_n`, `end_n`, `regret`, `reg_n`, `p2`, `next_w_base`): those are per-candidate or
    // per-decision, so they are computed unconditionally on the lines immediately after.
    // None of the six feeds any of the ten tuple members, and none of the ten feeds any of
    // the six except through the untouched code below, so this is a pure reordering of
    // independent, side-effect-free `let` bindings and cannot change any value.
    // ---------------------------------------------------------------------------------
    let (flex_inv, rem_min_n, rem_avg_n, bn_n, ops_n, scar_n, proc_n, scarcity_urg, density_n, next_term_raw) = if lean {
        (
            op.flex_inv,
            pre.fm_rem_min_n[product][op_idx],
            pre.fm_rem_avg_n[product][op_idx],
            pre.fm_bn_n[product][op_idx],
            pre.fm_ops_n[product][op_idx],
            pre.fm_machine_scar_n[machine],
            op.pt_n[mach_idx],
            pre.fm_scarcity_urg[best_cnt_total],
            pre.fm_density_n[product][op_idx],
            pre.fm_next_term_raw[product][op_idx],
        )
    } else {
        ...
        (flex_inv, rem_min_n, rem_avg_n, bn_n, ops_n, scar_n, proc_n, scarcity_urg, density_n, next_term_raw)
    };
    ...
    // E45 B1: `*` is left-associative, so `flow_w * job_flow_pref[job]` is a complete
    // sub-product and folding it into `fm_flow_wj[job]` is exact.
    let flow_term = (if lean { pre.fm_flow_wj[job] } else { pre.flow_w*pre.job_flow_pref[job] })*(0.65+0.70*(1.0-progress));
    ...
    let route_term = if route_w>0.0 && op.flex>=2 { route_w*route_gain*route_pref_bonus_fm(route_pref,product,op_idx,machine,lean) } else { 0.0 };
    match rule {
        Rule::CriticalPath => {
...
fn choose_rule_bandit(rng: &mut SmallRng, rules: &[Rule], rule_best: &[u32], rule_tries: &[u32], global_best: u32, margin: u32, stuck: usize, chaos_like: bool, late_phase: bool) -> Rule {
    ...
    // E45 item I: the roulette weight of every rule was computed TWICE per restart -- once to
    // form `sum`, once again inside the selection scan.  Each weight costs an `exp`, a `sqrt`
    // and (in the chaos/late arms) a `powf`; libm transcendentals are many basic blocks each.
    //
    // BIT-IDENTICAL: nothing between the two loops mutates `rule_best`, `rule_tries`,
    // `best_seen`, `scale`, `explore_mix`, `chaos_like` or `late_phase`, so the recomputed
    // `ww.max(0.0)` is by construction the same f64 as the first pass produced.  Caching it
    // reproduces `sum` and every partial `r` exactly.
    //
    // RNG SAFETY: the `rng.gen::<f64>()` below and the `gen_range` in the `sum <= 0` bail-out
    // stay exactly where they were, so the draw count and draw order are untouched.
    //
    // `rules` is built by at most 9 `push`es in `solve`, so `<= 9` always holds; the guard is
    // free insurance that falls back to the legacy double-compute if that ever changes.
    let lean = super::knobs::fm_fuel_lean() != 0 && rules.len() <= 9;
...
fn construct_solution_conflict_det(
    ...
    // E45: one relaxed load per construct call, hoisted out of every loop below.
    let fuel_lean = super::knobs::fm_fuel_lean() != 0;
    ...
    while remaining_ops > 0 {
        loop {
            ...
            for &job in &ready_jobs {
                ...
                // E45 B1: `.enumerate()` supplies `mach_idx`, the index into `op.machines`
                // and hence into the parallel `op.pt_n` memo.  Iteration order and count are
                // unchanged.
                for (mach_idx, &(m, pt)) in op.machines.iter().enumerate() {
            ...
            // E45 item D: this guard is PROVABLY never taken, and it costs a full extra
            // `best_second_and_counts` scan of `op.machines` for every scheduled operation.
            //
            // Proof 1 (from the code).  `chosen` came out of the candidate scan above, which
            // admits `(job, m, pt)` only when `machine_avail[m] <= time` (so the first
            // disjunct is false and `time.max(machine_avail[machine]) == time`) and when
            // `time.saturating_add(pt) == best_end`, where `best_end` is the first component
            // of `best_second_and_counts(time, &machine_avail, op)` for this same `op`.
            // Nothing mutates `machine_avail`, `time` or `op` between that scan and here, so
            // `best_end_now == best_end` and `end_check == time.saturating_add(pt) == best_end`
            // -- both disjuncts are false.
            //
            // Proof 2 (empirical, independent).  `adaptive_js_opt` does not contain this block
            // at all, and our output is 40/40 bit-identical to theirs on this track, so it
            // cannot ever have fired on the measured bundle.
            if !fuel_lean {
...
fn construct_solution_conflict_topk(
    ...
    // E45: one relaxed load per construct call, hoisted out of every loop below.  This is the
    // hot construction path (`k >= 2` on every one of the ~50k restarts), so the knob must
    // never be re-read per decision or per candidate.
    let fuel_lean = super::knobs::fm_fuel_lean() != 0;
    ...
    while remaining_ops > 0 {
        loop {
            ...
            for &job in &ready_jobs {
                ...
                // E45 B1: `.enumerate()` supplies `mach_idx`, the index into `op.machines`
                // and hence into the parallel `op.pt_n` memo.  Iteration order, iteration
                // count and every `continue` are unchanged, so the number and order of
                // `rng.gen::<f64>()` jitter draws below is untouched.
                for (mach_idx, &(m, pt)) in op.machines.iter().enumerate() {
                    if machine_avail[m] > time {
                        continue;
                    }
...
fn tabu_search_hybrid(pre: &Pre, challenge: &Challenge, base_sol: &Solution, max_iterations: usize, tenure_base: usize) -> Result<Option<(Solution, u32)>> {
    // E45: ONE relaxed load of each knob per tabu run, never inside the `for iter` loop.  This
    // engine runs ~33.7k iterations per call and ~77 calls per nonce, so a per-iteration
    // atomic load here would be a measurable cost in its own right.  `topo_ok` folds in the
    // `hfs_eval_lean` precondition for item C (see the tail pass below); knobs are written
    // once by `solve_challenge` before any search begins, so it cannot change under us.
    let fuel_lean = super::knobs::fm_fuel_lean() != 0;
    let topo_ok = fuel_lean && super::knobs::hfs_eval_lean() == 0;
    // E46 G2: one relaxed load per tabu run, never inside the `for iter` loop.
    let pos_buf_on = super::knobs::fm_pos_buf() != 0;
    let mut pos_buf = [0usize; 6];
    ...
    for iter in 0..effective_iterations {
        ...
        if no_improve>0&&no_improve%kick_threshold==0&&kicks_left>0 {
            ...
            if super::knobs::xt_fjm_kick_spacing()!=0 { no_improve=no_improve.saturating_add(1); }
        ...
        // E45 item C: the tail (q-label) pass used to derive its own reverse-topological
        // order with a second full Kahn sort -- an out-degree build, a seed scan and a
        // stack-driven pop loop, ~26 priced instructions per node on top of the ~31 that
        // `eval_disj` just spent.  But `eval_disj` (called immediately above, and returning
        // `Some`, so `processed == n`) has already written a valid topological order into
        // `buf.topo[0..n]`.  Iterating it backwards IS a reverse-topological order.
        //
        // BIT-IDENTICAL: `tail[x]` is defined as `max over successors s of (pt[s] + tail[s])`,
        // with 0 for sinks.  The legacy loop pushes that max backwards through
        // `job_pred_node` / `machine_pred_node`; the lean loop pulls the same max forwards
        // through `ds.job_succ` / `buf.machine_succ`.  Those two pairs are exact inverses:
        //   * `job_pred_node[k] = k-1` for every non-first op of a job (set once at fn entry)
        //     and `build_disj_from_solution` sets `job_succ[id] = id+1` on exactly the same
        //     set, NONE elsewhere;
        //   * `machine_pred_node` is rebuilt from `ds.machine_seq` two lines above and
        //     `buf.machine_succ` is rebuilt from the same `ds.machine_seq` inside `eval_disj`.
        // A node's tail is final before it is read in either order (legacy: it is only popped
        // once its out-degree hits 0; lean: all successors appear later in `topo`, hence
        // earlier in the reverse walk).  `max` is order-independent and exact in u32, so the
        // two loops agree bit-for-bit.  The `wrapping_add` below carries the same bound proof
        // as `eval_disj` case (2): `pt[s] + tail[s] <= sum of all pt < 5e5 << u32::MAX`, so
        // the `saturating_add` it replaces could never have saturated either.
        //
        // PRECONDITION -- `buf.topo` / `buf.topo_len` must be live.  `eval_disj` only fills
        // them when `hfs_eval_lean == 0`.  `solver::solve_challenge` force-clears that knob on
        // every path except the v9 hybrid engine, which this function is not part of, so the
        // precondition holds today.  It is checked at run time anyway rather than trusted:
        // if someone ever enables `hfs_eval_lean` here, we silently take the legacy path
        // instead of reading a stale order.
        let topo_live = topo_ok && buf.topo_len == n;
        if topo_live {
            tail.fill(0);
            for ti in (0..n).rev() {
                let nd=buf.topo[ti];
                ...
                if js!=NONE_USIZE{let c=ds.node_pt[js].wrapping_add(tail[js]);if c>t{t=c;}}
                let ms=buf.machine_succ[nd];
                if ms!=NONE_USIZE{let c=ds.node_pt[ms].wrapping_add(tail[ms]);if c>t{t=c;}}
                tail[nd]=t;
            }
        } else {
        ...
        for m in 0..ds.num_machines {
            ...
            for &(bstart,bend) in &blocks {
                ...
                for si in 0..num_swaps {
                    ...
                    let est_mk=estimate_swap_mk_fm(node_u,node_v,&buf.start,&tail,&ds.node_pt,&job_pred_node,&ds.job_succ,&machine_pred_node,&buf.machine_succ,fuel_lean);
        ...
        if iter%reassign_freq==0 {
            for node in 0..n {
                ...
                for &(new_m,new_pt) in &op_info.machines {
                    ...
                    // E46 G2: knob-gated scratch-buffer variant; identical values and order.
                    let np = if pos_buf_on {
                        find_candidate_insert_positions_fm_buf(&ds,&buf.start,node,new_m,new_pt,&job_pred_node,&mut pos_buf)
                    } else {
                        let v=find_candidate_insert_positions_fm(&ds,&buf.start,node,new_m,new_pt,&job_pred_node);
                        let n=v.len(); for (i,&x) in v.iter().enumerate(){pos_buf[i]=x;} n
                    };
                    for &insert_pos in pos_buf[..np.min(reassign_pos_cap)].iter() {
                        let est_mk=estimate_reassign_mk_fm(&ds,&buf.start,&tail,node,new_m,new_pt,insert_pos,&job_pred_node,&machine_pred_node,&buf.machine_succ,fuel_lean);
    ...
    match eval_disj(&ds,&mut buf) {
        ...
        None => {
            if super::knobs::xt_fjm_keep_best()==0 { return Ok(None); }
        }
    }
...
/// E46 G3 (`fm_tails_topo`).  Kahn-style reverse sweep replacing the start-time sort.
///
/// The sort exists only to guarantee that every successor of `nd` already has its final
/// tail when `nd` is processed.  Descending start time is one valid topological order of
/// the (job-succ, machine-succ) DAG, but so is peeling nodes whose out-degree has reached
/// zero -- and that costs O(n) instead of O(n log n) plus the `order` allocation.
///
/// BIT-IDENTICAL: `tails[nd] = max over successors of (node_pt[s] + tails[s])` is a
/// well-defined fixpoint on a DAG; any topological order yields the same values, and the
/// same `saturating_add` is used. Only the traversal order and the allocation change.
fn compute_tails_pulsar_topo(ds: &DisjSchedule, buf: &EvalBuf) -> Vec<u32> {
    let n = ds.n;
    let mut tails = vec![0u32; n];
    let mut out_deg = vec![0u8; n];
    for nd in 0..n {
        if ds.job_succ[nd] != NONE_USIZE { out_deg[nd] += 1; }
        if buf.machine_succ[nd] != NONE_USIZE { out_deg[nd] += 1; }
    }
    // predecessors: job_pred is nd-1 inside the same job; machine_pred from the sequences.
    let mut machine_pred = vec![NONE_USIZE; n];
    for seq in &ds.machine_seq {
        for i in 1..seq.len() { machine_pred[seq[i]] = seq[i - 1]; }
    }
    let mut stack: Vec<usize> = Vec::with_capacity(n);
    for nd in 0..n { if out_deg[nd] == 0 { stack.push(nd); } }
    while let Some(nd) = stack.pop() {
        let contrib = ds.node_pt[nd].saturating_add(tails[nd]);
        let job = ds.node_job[nd];
        let jp = if nd > ds.job_offsets[job] { nd - 1 } else { NONE_USIZE };
        if jp != NONE_USIZE {
            if contrib > tails[jp] { tails[jp] = contrib; }
            out_deg[jp] = out_deg[jp].saturating_sub(1);
            if out_deg[jp] == 0 { stack.push(jp); }
        }
        let mp = machine_pred[nd];
        if mp != NONE_USIZE {
            if contrib > tails[mp] { tails[mp] = contrib; }
            out_deg[mp] = out_deg[mp].saturating_sub(1);
            if out_deg[mp] == 0 { stack.push(mp); }
        }
    }
    tails
}
...
#[inline]
fn compute_tails_dispatch(ds: &DisjSchedule, buf: &EvalBuf) -> Vec<u32> {
    if super::knobs::fm_tails_topo() != 0 { compute_tails_pulsar_topo(ds, buf) }
    else { compute_tails_pulsar(ds, buf) }
}
...
fn targeted_cp_kick(
    ...
    let tails = compute_tails_dispatch(&ds, &buf);
...
/// E46 G2 (`fm_pos_buf`).  Caller-owned scratch variant of
/// `find_candidate_insert_positions_fm`: writes into a fixed `[usize; 6]` and returns the
/// count, instead of heap-allocating a `Vec` on every call.  This runs once per (critical
/// node, alternative machine) pair on every reassignment sweep, so the allocation is paid
/// ~1e5-1e6 times per nonce.
///
/// BIT-IDENTICAL: the six `push_uniq` calls happen in the same order with the same operands,
/// the same `p <= len` and duplicate guards, the same empty-fallback, and the same cap of 5.
/// Only the storage changes.
fn find_candidate_insert_positions_fm_buf(
    ds: &DisjSchedule,
    starts: &[u32],
    node: usize,
    new_machine: usize,
    ...
    #[inline]
    fn lb_gt(seq: &[usize], starts: &[u32], value: u32) -> usize {
        let mut lo = 0usize;
        let mut hi = seq.len();
        while lo < hi {
            let mid = (lo + hi) >> 1;
            if starts[seq[mid]] <= value { lo = mid + 1; } else { hi = mid; }
        }
        lo
    }
    #[inline]
    fn lb_ge(seq: &[usize], starts: &[u32], value: u32) -> usize {
        let mut lo = 0usize;
        let mut hi = seq.len();
        while lo < hi {
            let mid = (lo + hi) >> 1;
    ...
    let pos_after_jp = lb_gt(seq, starts, job_pred_end).min(len);
    let cur_start = starts[node];
    let pos_by_cur = lb_ge(seq, starts, cur_start).min(len);

    let mut n = 0usize;
    #[inline]
    fn push_uniq(out: &mut [usize; 6], n: &mut usize, p: usize, len: usize) {
        if p <= len && !out[..*n].contains(&p) {
            out[*n] = p;
            *n += 1;
        }
    }

    push_uniq(out, &mut n, pos_after_jp, len);
    push_uniq(out, &mut n, pos_after_jp.saturating_sub(1), len);
    push_uniq(out, &mut n, pos_by_cur, len);
    push_uniq(out, &mut n, pos_by_cur.saturating_sub(1), len);
    push_uniq(out, &mut n, 0, len);
    push_uniq(out, &mut n, len, len);
...
/// E45 item A, move-estimate sites.  `saturating_add` lowers to add + overflow-test + cmov
/// (3 priced instructions) where the plain add is 1.
///
/// BIT-IDENTICAL: every operand fed to this helper is a head (`buf.start[..]`), a tail
/// (`tail[..]`) or a processing time in the disjunctive graph, and each of those is bounded
/// by the total work `sum of all pt <= n_ops * max_pt`.  For this challenge family
/// `n_ops = 50 jobs * ~35 ops ~ 1.8e3` and `pt <= base_proc_time_max * max_speed_factor =
/// 200 * 1.2 = 240`, so every quantity is < 5e5; the largest expression below chains three
/// of them and is still < 2e6, i.e. ~2000x under `u32::MAX`.  Saturation is unreachable, so
/// both arms return the same bits.
#[inline(always)]
fn fadd_fm(a: u32, b: u32, lean: bool) -> u32 { if lean { a.wrapping_add(b) } else { a.saturating_add(b) } }

fn estimate_reassign_mk_fm(ds: &DisjSchedule, heads: &[u32], tails: &[u32], node: usize, new_machine: usize, new_pt: u32, insert_pos: usize, job_pred: &[usize], machine_pred: &[usize], machine_succ: &[usize], lean: bool) -> u32 {
    let jp=job_pred[node]; let js=ds.job_succ[node]; let old_mp=machine_pred[node]; let old_ms=machine_succ[node];
    let jp_end=if jp!=NONE_USIZE{fadd_fm(heads[jp],ds.node_pt[jp],lean)}else{0};
    let new_seq=&ds.machine_seq[new_machine];
    let new_mp_end=if insert_pos>0&&!new_seq.is_empty(){let pred=new_seq[insert_pos.min(new_seq.len())-1];fadd_fm(heads[pred],ds.node_pt[pred],lean)}else{0};
    let new_start=jp_end.max(new_mp_end); let new_end=fadd_fm(new_start,new_pt,lean);
    let js_tail=if js!=NONE_USIZE{fadd_fm(ds.node_pt[js],tails[js],lean)}else{0};
    let new_ms_tail=if insert_pos<new_seq.len(){let succ=new_seq[insert_pos];fadd_fm(ds.node_pt[succ],tails[succ],lean)}else{0};
    let node_path=fadd_fm(new_end,js_tail.max(new_ms_tail),lean);
    let old_reconnect=if old_mp!=NONE_USIZE&&old_ms!=NONE_USIZE{let old_mp_end=fadd_fm(heads[old_mp],ds.node_pt[old_mp],lean);fadd_fm(fadd_fm(old_mp_end,ds.node_pt[old_ms],lean),tails[old_ms],lean)}else{0};
    node_path.max(old_reconnect)
}

#[inline]
fn estimate_swap_mk_fm(u: usize, v: usize, heads: &[u32], tails: &[u32], pt: &[u32], job_pred: &[usize], job_succ: &[usize], machine_pred: &[usize], machine_succ: &[usize], lean: bool) -> u32 {
    ...
    let r_jp_v=if jp_v!=NONE_USIZE{fadd_fm(heads[jp_v],pt[jp_v],lean)}else{0};let r_mp_u=if mp_u!=NONE_USIZE{fadd_fm(heads[mp_u],pt[mp_u],lean)}else{0};
    let new_r_v=r_jp_v.max(r_mp_u);let r_jp_u=if jp_u!=NONE_USIZE{fadd_fm(heads[jp_u],pt[jp_u],lean)}else{0};let new_r_u=r_jp_u.max(fadd_fm(new_r_v,pt[v],lean));
    let q_js_u=if js_u!=NONE_USIZE{fadd_fm(pt[js_u],tails[js_u],lean)}else{0};let q_ms_v=if ms_v!=NONE_USIZE{fadd_fm(pt[ms_v],tails[ms_v],lean)}else{0};
    let new_q_u=q_js_u.max(q_ms_v);let q_js_v=if js_v!=NONE_USIZE{fadd_fm(pt[js_v],tails[js_v],lean)}else{0};let new_q_v=q_js_v.max(fadd_fm(pt[u],new_q_u,lean));
    (fadd_fm(fadd_fm(new_r_v,pt[v],lean),new_q_v,lean)).max(fadd_fm(fadd_fm(new_r_u,pt[u],lean),new_q_u,lean))
}
...
fn crossover_majority_vote(
    ...
    for job in 0..num_jobs {
        ...
        for op_idx in 0..num_ops {
            ...
            if super::knobs::fm_legacy_votes() != 0 {
                // LEGACY: iterates a std HashMap; RandomState reseeds per process, so equal vote
                // counts resolve by iteration order => nondeterministic output. Kept only as a
                // measurement control; never ship this path.
                ...
                // Deterministic: scan eligible machines in their fixed Vec order and look the count
                // up, instead of iterating the map. Ties now resolve to the lowest-index eligible
                // machine, identically in every process.
                for &(m, _) in op_info.machines.iter() {
                    let cnt = match vote_counts[op_idx].get(&m) { Some(&c) => c, None => 0 };
                    if cnt > best_votes {
                        best_machine = m;
                        best_votes = cnt;
                    }
