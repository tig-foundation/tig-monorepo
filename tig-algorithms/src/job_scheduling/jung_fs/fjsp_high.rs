    ...
    for _iter in 0..max_iters {
        ...
        for &nd in &cp_flex {
            ...
            if best_m != cur_m {
                ...
                neutral_left = 0;
                iter_improved = true;
            } else if super::knobs::xt_fjh_same_machine() != 0 && best_mk < current_mk {
                // XTRACK-1: argmin came from the same-machine re-insertion scan, which
                // `best_m != cur_m` cannot see; mirrors the greedy pre-pass exactly.
                let Some(old_pos) = ds.machine_seq[cur_m].iter().position(|&x| x == nd) else { continue };
                ds.machine_seq[cur_m].remove(old_pos);
                let ins = best_ins.min(ds.machine_seq[cur_m].len());
                ds.machine_seq[cur_m].insert(ins, nd);
                current_mk = best_mk;
                let _ = eval_disj(&ds, &mut buf);
                start_snapshot.clone_from(&buf.start);
                neutral_left = 0;
...
fn push_top_solutions_diverse(
    ...
    while pool.len() > cap {
        ...
        // R2 (`fh_elite_div`): diversity-first archive. The baseline rule below is purely
        // elitist -- it always evicts an entry whose makespan equals the pool maximum, and only
        // uses the nearest-neighbour distance to break ties among those. Over tens of thousands
        // of restarts that collapses the archive into near-clones of the incumbent, so every
        // downstream polish seed attacks the same basin. With the knob on, the best
        // `cap - q` entries by makespan are protected and the most redundant of the rest is
        // evicted instead, which keeps `q` genuinely distinct basins alive.
        //
        // Determinism: the ordering key is a total order on (makespan, pool index) and the
        // eviction key is a total order on (min_nn, inverted makespan, inverted index), so the
        // choice never depends on sort stability or on iteration order of any hash container.
        let div_q = super::knobs::fh_elite_div();
        if div_q > 0 && cap >= 2 {
            let q = div_q.min(cap - 1).max(1);
            let protect = cap - q;
            let mut order: Vec<usize> = (0..len).collect();
            order.sort_by_key(|&i| (pool[i].1, i));
            let mut drop_idx: Option<usize> = None;
            let mut drop_key: (u32, u32, usize) = (u32::MAX, u32::MAX, usize::MAX);
            for (rank, &i) in order.iter().enumerate() {
                if rank < protect {
                    continue;
                }
                let key = (min_nn[i], u32::MAX - pool[i].1, len - 1 - i);
                if drop_idx.is_none() || key < drop_key {
                    drop_idx = Some(i);
                    drop_key = key;
                }
            }
            let di = drop_idx.unwrap_or(len - 1);
            pool.swap_remove(di);
            continue;
        }
...
pub fn solve(
    ...
    // ---------------------------------------------------------------------------------------
    // R1 (`fh_portfolio`, `fh_seg_iters`): the whole search below is one *segment*. The baseline
    // runs exactly one segment, seeded from `challenge.seed`, with `effort.fjsp_high_iters`
    // restarts -- i.e. a single best-of-N sampler whose outcome concentrates hard. With the knob
    // on we run S such segments, each with its own RNG stream, elite archive, learned consensus,
    // bandit statistics and stagnation counters, and keep the best result across segments.
    //
    // Determinism: every segment's RNG is derived only from `challenge.seed` and the segment
    // index, and the number of RNG draws inside a segment depends only on (seed, nonce, knobs).
    // Segment 0's stream is byte-identical to the baseline's.
    // ---------------------------------------------------------------------------------------
    let n_seg = super::knobs::fh_portfolio().max(1);
    let seg_iters = {
        let v = super::knobs::fh_seg_iters();
        if v == 0 { effort.fjsp_high_iters } else { v }
    };
    let relearn_p = super::knobs::fh_relearn();
    let portfolio_on = n_seg > 1;
    let mut global_best_mk = u32::MAX;
    let mut global_best: Option<Solution> = None;
    for seg in 0..n_seg {
    let mut rng = {
        let mut s = challenge.seed;
        if seg > 0 {
            let mix = (seg as u64)
                .wrapping_mul(0x9E37_79B9_7F4A_7C15)
                .wrapping_add(0xA076_1D64_78BD_642F);
            let mb = mix.to_le_bytes();
            let nb = s.len().min(8);
            for i in 0..nb { s[i] ^= mb[i]; }
        }
        SmallRng::from_seed(s)
    };
    // Guarded save: never let a later segment overwrite the portfolio-wide best with something
    // worse, so an abort part-way through a segment still leaves the best-so-far saved. For
    // segment 0 the floor is `u32::MAX`, so every call forwards exactly as the baseline does.
    let seg_floor: u32 = if seg == 0 { u32::MAX } else { global_best_mk };
    let save_best = |s: &Solution, m: u32| -> Result<()> {
        if m < seg_floor { save_solution(s)?; }
        Ok(())
    };
    ...
    if pre.flow_route.is_some()&&pre.flow_pt_by_job.is_some() {
        if let Ok((sol,mk))=neh_reentrant_flow_solution(pre,challenge.num_jobs,challenge.num_machines) {
            if mk<best_makespan{best_makespan=mk;best_solution=Some(sol.clone());save_best(&sol, mk)?;}
    ...
    for r in 0..num_restarts {
        ...
        push_top_solutions_diverse(pre, challenge, &mut top_solutions, &sol, mk, 15);
        if mk<best_makespan{best_makespan=mk;best_solution=Some(sol.clone());save_best(&sol, mk)?;stuck=0;if learn_updates_left>0{let (jb,mp,route_counts)=consensus_learning_from_elites(pre,challenge,&top_solutions)?;learned_jb=Some(jb);learned_mp=Some(mp);learned_route_counts=Some(route_counts);learn_updates_left-=1;}}else{stuck=stuck.saturating_add(1);}
        stuck_ema = 0.9 * stuck_ema + 0.1 * ((stuck as f64) / 200.0).min(1.0).max(0.0);
        // R5 (`fh_relearn`): the baseline refreshes the learned job bias / machine penalty /
        // route preferences only on an improving restart, and freezes them for good after 10
        // such refreshes -- typically within the first few hundred restarts, after which the
        // remaining tens of thousands are all steered by one early, noisy estimate. With the
        // knob on we additionally refresh every P restarts and re-arm the improvement-driven
        // budget, so a segment keeps re-committing to whatever basin its own archive now
        // favours. No RNG is consumed here, so the draw sequence is unchanged.
        if relearn_p > 0 && (r + 1) % relearn_p == 0 && !top_solutions.is_empty() {
            let (jb, mp, route_counts) = consensus_learning_from_elites(pre, challenge, &top_solutions)?;
            learned_jb = Some(jb);
            learned_mp = Some(mp);
            learned_route_counts = Some(route_counts);
            learn_updates_left = 10;
        }
    }
    ...
    for base_sol in &ls_seeds {
        if let Some((sol2,mk2))=critical_block_move_local_search_ex(pre,challenge,base_sol,8,128,24)?{
            if mk2<best_makespan{best_makespan=mk2;best_solution=Some(sol2.clone());save_best(&sol2, mk2)?;}
        ...
        if let Ok(Some((sol2, mk2))) = iterative_cp_descent(pre, challenge, sol, 15) {
            ...
            if let Ok(Some((sol4, mk4))) = iterative_cp_descent(pre, challenge, &cur, 6) {
                if mk4 < best_makespan {
                    // The baseline updates `best_solution` here but forgets `best_makespan`.
                    // Nothing downstream read it before, so leaving it stale was harmless; under
                    // R1 the stale value is what the cross-segment comparison below uses, which
                    // would let a genuinely worse segment win. Gated so knob 0 is untouched.
                    if portfolio_on { best_makespan = mk4; }
    ...
    // ---- end of segment: fold this segment's result into the portfolio-wide best ----------
    if let Some(sol) = best_solution {
        if global_best.is_none() || best_makespan < global_best_mk {
            global_best_mk = best_makespan;
            global_best = Some(sol);
        }
    }
    // Re-assert the portfolio-wide best as the last-written solution before the next segment
    // starts saving its own (initially far worse) incumbents. Skipped entirely when there is
    // only one segment, so the baseline's call sequence is unchanged.
    if n_seg > 1 {
        if let Some(ref s) = global_best { save_solution(s)?; }
    }
    }

    if let Some(sol)=global_best{save_solution(&sol)?;}
    Ok(())
}