...
fn tabu_ils_hfs(
    ...
    // E43 knobs, hoisted once per call (this function runs a handful of times per nonce, so
    // the relaxed loads are free and the inner loop only ever sees plain locals).
    let ig_mode = knobs::hfs_ig_mode();
    let ig_d = knobs::hfs_ig_d();
    let kick_swaps_count = knobs::hfs_kick_swaps();
    let kick_pct = knobs::hfs_kick_pct();
    let kick_space = knobs::hfs_kick_space();
    // E43: kick_pct == 0 is the sentinel and reproduces the original expression exactly.
    ...
    // E43: hfs_kicks() == 0 is the sentinel and returns 5, the original hardcoded budget.
    let mut kicks_left = knobs::hfs_kicks();
    ...
    for iter in 0..max_iters {
        ...
        if no_improve > 0 && no_improve % kick_threshold == 0 && kicks_left > 0 {
            ...
            if ig_mode >= 1 {
                // E43 IG-RECON kick: destruction-reconstruction perturbation (Iterated Greedy).
                ...
                // `ig_pool` is built in index order, the xorshift is drawn exactly `ig_moves`
                // times (the draw precedes every `continue`/`break` inside the body), and
                // `op_info.machines` is sorted by machine id in `preprocess.rs:99` so the
                // strict `<` argmin resolves ties to the first candidate in a fixed order.
            ...
            kicks_left -= 1;
            // E43 kick spacing. The branch above `continue`s without touching `no_improve`, so
            // at `no_improve == kick_threshold` it re-fires every following iteration until the
            // budget is spent -- every kick burns in one burst and the restart-to-best branch
            // can never be reached. kick_space == 1 clears the counter so each kick is followed
            // by a full search window and the budget survives to reach that branch.
            // kick_space == 0 leaves `no_improve` untouched: byte-exact e42.
            if kick_space == 1 {
                no_improve = 0;
            }
            continue;
