...
fn patch_eval_machine_state_one_machine(
    ...
    // E44 R6 (knob `fs_alloc_lean`) -- single diff pass instead of clear-then-set.
    // The node *set* of `seq` is invariant under the moves this is called for
    // (insert / adjacent swap / pair swap all permute the sequence), so every node
    // that could hold a stale `machine_succ` is still visited below: positions
    // `0 .. len-2` get their new successor, position `len-1` is cleared.
    // Where the successor is unchanged the pair of `saturating_sub` /
    // `saturating_add` on the same counter cancels, so it is skipped.  No counter
    // can underflow (indeg is at most one job arc plus one machine arc) and none
    // can reach u16::MAX, so dropping the strict decrement-then-increment ordering
    // leaves the final `indeg_base` and `machine_succ` bit-identical.
    let len = seq.len();
    if len == 0 {
        return;
    }
    ...
    let last = seq[len - 1];
    let old_v = buf.machine_succ[last];
    if old_v != NONE_USIZE {
        indeg_base[old_v] = indeg_base[old_v].saturating_sub(1);
        buf.machine_succ[last] = NONE_USIZE;
    }
}

/// E44 R3 (knob `fs_kahn_roots`) -- seed the Kahn stack.
///
/// BIT-IDENTITY ARGUMENT.  `build_disj_from_solution` lays operation `k` of job `j`
/// at node id `job_offsets[j] + k` and sets `indeg_job[id] = 1` for every `k >= 1`
/// (the job-precedence arc).  `indeg` is `indeg_job` plus machine predecessors, so
/// `indeg[i] == 0` implies `i` is the first operation of its job, i.e. `i` is one of
/// the `job_offsets[j]`.  The converse is not automatic (a root can have a machine
/// predecessor), which is why the root loop keeps the `indeg[root] == 0` test.
/// `job_offsets` is non-decreasing, so roots are visited in increasing node id --
/// the same order the `0..n` scan produced.  Same set, same push order, therefore
/// the same LIFO pop order and the same `mk_node` / `best_pred` tie-breaks.
#[inline]
fn seed_kahn_roots(ds: &DisjSchedule, buf: &mut EvalBuf) {
    buf.stack.clear();
    if knobs::fs_kahn_roots() == 1 {
...
/// E44 R1 (knob `fs_trial_bound`) -- makespan-only, bound-pruned longest-path pass.
///
/// Two differences from `eval_disj_stateful`:
///   1. `buf.best_pred` is neither cleared nor written.  `best_pred` is a pure
///      *output* of the pass -- it is written but never read while the pass runs --
///      so omitting it cannot change `mk`.  It costs `n * size_of::<usize>()` of
///      `fill` plus one store per relaxed arc, which is roughly half the memory
///      traffic of a trial evaluation.
///   2. The pass returns `None` as soon as the running makespan reaches `bound`.
///
/// Returns `None` for "cycle" and for "pruned" alike; both mean the same thing to
/// the only caller (skip this candidate), so they need not be distinguished.
#[inline]
fn eval_disj_trial(
    ds: &DisjSchedule,
    buf: &mut EvalBuf,
...
/// Evaluate one trial move.  `bound` is the caller's incumbent `best_mk`; see the
/// bit-identity argument at the call site in `descent_phase`.
#[inline]
fn descent_trial_eval(
    ds: &DisjSchedule,
    buf: &mut EvalBuf,
    indeg_base: &[u16],
    bound: u32,
) -> Option<u32> {
    if knobs::fs_trial_bound() == 1 {
        eval_disj_trial(ds, buf, indeg_base, bound)
    } else {
        eval_disj_stateful(ds, buf, indeg_base).map(|(mk, _)| mk)
    }
}
...
fn critical_block_move_local_search_ex(
    ...
    // E44 R6 (knob `fs_alloc_lean`).  `descent_phase` only ever mutates
    // `ds.machine_seq`, and it leaves `cur_eval` equal to the evaluation of the `ds`
    // it returns: a proposed move is only kept after its own `eval_disj_stateful`
    // succeeded and was written into `cur_eval`, and a rejected move is undone.  So
    // `cur_eval.0` is already the makespan the re-evaluations below recompute, and
    // every other `DisjSchedule` field is loop-invariant, which makes `machine_seq`
    // the only state a save/restore has to carry.
    let lean = knobs::fs_alloc_lean() == 1;
    descent_phase(&mut ds, &mut buf, &mut crit, pre, &mut cur_eval, max_iters, top_cands);
    let mut global_best_mk = if lean {
        cur_eval.0
    } else {
    ...
    for _cycle in 0..perturb_cycles {
        if lean {
            ds.machine_seq.clone_from(&global_best_ds.machine_seq);
        } else {
        ...
        if lean {
            if cur_eval.0 < global_best_mk {
                ...
                global_best_ds.machine_seq.clone_from(&ds.machine_seq);
            }
        } else if let Some((mk_now, _)) = eval_disj(&ds, &mut buf) {
    ...
    if lean {
        ds.machine_seq = global_best_ds.machine_seq;
    } else {
        ds = global_best_ds;
    }
    // Kept in both paths: this pass is what fills `buf.start` for `disj_to_solution`.
...
fn descent_phase(
    ...
    // E44 R2 (knob `fs_trial_memo`) -- per-iteration memo of trial evaluations,
    // keyed on (kind, m_from, from, to).  Allocated once per `descent_phase`,
    // cleared at the top of every iteration.  Bounded by `prescreen_k <= 48`.
    let mut trial_memo: Vec<(u8, usize, usize, usize, Option<u32>)> = Vec::with_capacity(48);
    ...
    for iter_ix in 0..max_iters {
        ...
        // E44 R2 (knob `fs_trial_memo`).  The shift list at the candidate-generation
        // site above is `[1, 2, max_shift]`; when `max_shift == 1` the `sh == 2` entry
        // is skipped and `sh == max_shift` repeats `sh == 1`, and when `max_shift == 2`
        // the `sh == max_shift` entry repeats `sh == 2`.  Critical blocks of length 2
        // and 3 dominate in a flow shop, so a fifth to a third of `cands` are literal
        // duplicates -- the same (kind, m_from, from, to) evaluated more than once.
        // The memo keeps `cands` and the selection order exactly as they are and only
        // skips the redundant longest-path pass.  A duplicate could never have won
        // anyway: the test below is a strict `<` against a `best_mk` that its own
        // first occurrence has already lowered to at most its value.
        let memo_on = knobs::fs_trial_memo() == 1;
        trial_memo.clear();
        ...
        for cand in &cands {
            ...
            let memo_hit: Option<Option<u32>> = if memo_on {
                trial_memo
                    .iter()
                    .find(|e| {
                        e.0 == cand.kind
            ...
            // E44 R1 (knob `fs_trial_bound`) -- BIT-IDENTITY ARGUMENT for `bound = best_mk`.
            //
            // A trial evaluation is consumed in exactly one way: the test below,
            // `mk2 < best_mk && (!cand_tabu || mk2 < best_seen + aspiration)`.  Both
            // conjuncts must hold, so any candidate with `mk2 >= best_mk` is rejected
            // regardless of the tabu clause.  `eval_disj_trial` returns `None` precisely
            // when it has proved `mk2 >= bound`, and `None` here means "no accept test",
            // i.e. the same rejection.  Nothing else escapes a trial: `buf.start` and
            // `buf.best_pred` are recomputed in full by the winner's `eval_disj_stateful`
            // further down (the `bc.kind` branches), and `ds.machine_seq`, `machine_succ`
            // and `indeg_base` are restored by the undo immediately after each trial.
            //
            // `best_mk` only ever decreases within this loop, so a memoised `None`
            // recorded against an earlier, looser bound `B1` still implies `mk2 >= B1 > B2`
            // for the current bound `B2` -- the memo and the bound compose safely.
            //
            // `best_mk` starts at `u32::MAX`, so the first candidate is never pruned.
            let mk_opt: Option<u32> = if let Some(cached) = memo_hit {
                cached
            } else {
                let computed: Option<u32> = if cand.kind == 0 {
                    let m = cand.m_from;
                    ...
                    let r = descent_trial_eval(ds, buf, &indeg_base, best_mk);
                    ...
                    // Unreachable for generated candidates; the original if/else-if
                    // chain simply fell through without evaluating anything.
                    None
                };
                if memo_on {
                    trial_memo.push((cand.kind, cand.m_from, cand.from, cand.to, computed));
                }
                computed
            };

            if let Some(mk2) = mk_opt {
...
/// E44 R6 -- the body of `job_bias_from_solution`, split out so a caller that has
/// just evaluated the same schedule can reuse its `(ds, buf, mk_node)` instead of
/// paying for another `build_disj_from_solution` + `EvalBuf::new` + `eval_disj`.
/// Pure function of its inputs, so the reused and the rebuilt path agree exactly
/// whenever the caller's `ds`/`buf` describe the same schedule.
...
/// Budget scale for this track. Exactly identity at the default: for any `base`,
/// `base * FLOW_SHOP_ITERS_BASE / FLOW_SHOP_ITERS_BASE == base` in integer arithmetic.
#[inline]
fn fs_scale(base: usize, effort: &EffortConfig) -> usize {
    base.saturating_mul(effort.flow_shop_iters) / FLOW_SHOP_ITERS_BASE
}
...
pub fn solve(
    ...
    // E44 knobs.  Read once, up front, so the whole `solve` sees one consistent
    // configuration; `set_knobs` runs before any search begins and the solver is
    // single-threaded, so these are deterministic.  All default to today's values.
    let lean = knobs::fs_alloc_lean() == 1;
    let skip_grasp_descent = knobs::fs_skip_grasp_descent() == 1;
    let skip_perturb = knobs::fs_skip_perturb() == 1;
    ...
    if let (Some(route), Some(pt)) = (&pre.flow_route, &pre.flow_pt_by_job) {
        if let Ok(neh_seq) = neh_best_sequence(pre, challenge.num_jobs, challenge.num_machines) {
            ...
            if unique && !neh_seq.is_empty() && route.len() == pt[neh_seq[0]].len() {
                ...
                seed[0] ^= 0x6B;
                let mut rng = SmallRng::from_seed(seed);
                let total_iters = fs_scale(2200, effort);
    ...
    if flow_is_reentrant {
        ...
        let num_restarts = fs_scale(600, effort);
        for r in 0..num_restarts {
            // E44 R6: bias captured from this restart's own evaluation, if any.
            // Scoped to the restart so it can never leak into a later one.
            ...
            if let Ok((mut sol, mut mk)) = construct_solution_conflict(
                ...
                // E44 R6 (knob `fs_alloc_lean`): `job_bias_from_solution` rebuilds the
                // disjunctive graph and re-evaluates a schedule this block has just
                // evaluated.  `build_disj_from_solution` recovers `machine_seq` by
                // sorting each machine's operations on (start, id), and starts along a
                // machine are strictly increasing because every processing time is at
                // least 1, so the rebuild reproduces this exact `ds` and therefore this
                // exact `mk_node`.  Capturing the bias here is that same function value.
                if !skip_grasp_descent && mk <= best_mk.saturating_add(best_mk / 20) {
                    if let Ok(mut ds) = build_disj_from_solution(pre, challenge, &sol) {
                        ...
                        if let Some((initial_mk, mk_node)) = eval_disj(&ds, &mut buf) {
                            ...
                            if improved {
                                if let Some((new_mk, new_mk_node)) = eval_disj(&ds, &mut buf) {
                                    if new_mk < mk {
                ...
                if let Ok(checked_mk) = challenge.evaluate_makespan(&sol) {
                    // E44 R6: `commit_valid_best` with `allow_equal == false` returns
                    // `Ok(false)` before any side effect unless `mk < best_mk`, so
                    // testing first only skips a `Solution` clone (~1,750 tuples) that
                    // the callee would have thrown away.
                    if !lean || mk < best_mk {
    ...
    if !is_strict_perm {
        // E44 R5 plumbing (budget knobs; each defaults to today's exact value).
        let ls_runs = top_solutions.len().min(knobs::fs_ls_runs(15));
        let perturb_cycles =
            knobs::fs_perturb_cycles(((pre.total_ops / 40) as usize).clamp(10, 40));
        let ls_iters = knobs::fs_ls_iters(7);
        let ls_cands = knobs::fs_ls_cands(80);
        for i in 0..ls_runs {
            let base_sol = &top_solutions[i].0;
            if let Ok(Some((sol2, mk2))) =
                critical_block_move_local_search_ex(pre, challenge, base_sol, ls_iters, ls_cands, perturb_cycles)
            {
                if let Ok(checked_mk) = challenge.evaluate_makespan(&sol2) {
                    // E44 R6: same early-return argument as in the GRASP loop.
                    if !lean || mk2 < best_mk {
        ...
        if !skip_perturb {
            ...
            let perturb_rounds = knobs::fs_perturb_rounds(fs_scale(20, effort));
            // E44 R6 (knob `fs_alloc_lean`) -- the perturbation base is a pure function
            // of `best_sol`, which only changes on a successful `commit_valid_best`.
            // Rebuilding it every round costs a `build_disj_from_solution` +
            // `EvalBuf::new` + `eval_disj` + critical-path walk that reproduces the
            // previous round's answer verbatim.  Under the knob it is rebuilt only when
            // `best_sol` has actually moved; the pristine `machine_seq` is kept so each
            // round still perturbs the unmodified base, exactly as a fresh build did.
            // Tuple: (working ds, its EvalBuf, pristine machine_seq, non-critical machines).
            let mut perturb_base: Option<(DisjSchedule, EvalBuf, Option<Vec<Vec<usize>>>, Vec<usize>)> =
                None;
            let mut base_dirty = true;
            for _ in 0..perturb_rounds {
                if base_dirty || !lean {
                    perturb_base = match build_disj_from_solution(pre, challenge, &best_sol) {
                        Ok(ds) => {
                            let mut buf = EvalBuf::new(ds.n);
                            match eval_disj(&ds, &mut buf) {
                                Some((_, mk_node)) => {
                                    ...
                                    let pristine =
                                        if lean { Some(ds.machine_seq.clone()) } else { None };
                                    Some((ds, buf, pristine, non_crit_machines))
                                }
                                None => None,
                            }
                        }
                ...
                let Some((ds, buf, pristine, non_crit_machines)) = perturb_base.as_mut() else {
                    continue;
                };
                if non_crit_machines.is_empty() {
                    continue;
                }
                // Restore the base before perturbing. In the non-lean path `ds` was just
                // rebuilt from `best_sol` and is already pristine.
                if let Some(p) = pristine.as_ref() {
                    ds.machine_seq.clone_from(p);
                }
        ...
        let extra_iters = fs_scale(1600, effort);
        if let (Some(route), Some(pt)) = (&pre.flow_route, &pre.flow_pt_by_job) {
            let unique = route_is_unique(route, challenge.num_machines);
