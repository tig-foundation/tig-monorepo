...
pub struct OpInfo {
    ...
    pub flex: usize,
    pub bn_avg: f64,
    /// E45 (`fm_fuel_lean`) memo: `1.0 / (flex as f64).max(1.0)`, i.e. exactly the
    /// `flex_inv` that `fjsp_medium::score_candidate` recomputes on every candidate.
    pub flex_inv: f64,
    /// E45 memo, parallel to `machines`: `pt_n[i] == (machines[i].1 as f64) / avg_op_min_d`
    /// where `avg_op_min_d == pre.avg_op_min.max(1.0)`.
    pub pt_n: Vec<f64>,
}
...
pub struct OpRoute {
    ...
    pub second_m: u8,
    pub second_w: u8,
    /// E45 memo: `(best_w as f64) / 255.0` and `(second_w as f64) / 255.0`, the two values
    /// `route_pref_bonus_fm` divides out on every candidate.  Filled by
    /// `infra_shared::route_pref_from_solution_lite`; `Default` leaves them 0.0, which is
    /// only ever read when the matching `*_w` is also 0.
    pub best_wf: f64,
    pub second_wf: f64,
}
...
pub struct Pre {
    ...
    // ----------------------------------------------------------------------
    // E45 (`fm_fuel_lean`) memo tables.  Each entry is the *same f64 expression*
    // `fjsp_medium::score_candidate` evaluates today, hoisted from once-per-candidate
    // (~15k/restart x 50k restarts) to once-per-nonce.  Indexed `[product][op_idx]` and
    // sized `product_ops[p].len() + 1`, mirroring `product_suf_*` exactly.
    //
    // These are memoisations ACROSS calls, which no compiler pass can perform, so they
    // are the part of the score_candidate work that survives full inlining + LICM.
    //
    // They are built unconditionally in `preprocess::build_pre` (a one-off ~5e4
    // instructions, i.e. ~2e-8 of the 5e12 budget) and READ only under the knob, so they
    // cannot perturb any track's output at knob 0.
    // ----------------------------------------------------------------------
    /// `(product_suf_min[p][o] as f64) / horizon_d`
    pub fm_rem_min_n: Vec<Vec<f64>>,
    /// `product_suf_avg[p][o] / max_job_avg_work_d`
    pub fm_rem_avg_n: Vec<Vec<f64>>,
    /// `product_suf_bn[p][o] / max_job_bn_d`
    pub fm_bn_n: Vec<Vec<f64>>,
    /// `(ops_rem as f64) / max_ops_d`, with `ops_rem = product_ops[p].len() - o`
    pub fm_ops_n: Vec<Vec<f64>>,
    /// `((rem_min / (ops_rem as f64).max(1.0)) / avg_op_min_d).clamp(0.0, 4.0)`
    pub fm_density_n: Vec<Vec<f64>>,
    /// `(0.55*next_min_n + 0.45*next_flex_inv) * (1.0 + 0.30*density_n*high_flex)`
    pub fm_next_term_raw: Vec<Vec<f64>>,
    /// `machine_scarcity[m] / avg_machine_scarcity_d`
    pub fm_machine_scar_n: Vec<f64>,
    /// `1.0 / (c as f64).max(1.0)` indexed by `c == best_cnt_total` (which is
    /// `cnt_best.max(1)`, so `1 <= c <= num_machines`).
    pub fm_scarcity_urg: Vec<f64>,
    /// `flow_w * job_flow_pref[job]` -- the leading, left-associated pair of the
    /// three-factor `flow_term` product.
    pub fm_flow_wj: Vec<f64>,
}
