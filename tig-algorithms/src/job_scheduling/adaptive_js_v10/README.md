...
* **Algorithm Name:** adaptive_js_v10
...
Fixed a bug which resulted in incorrect baseline calculations for track `flow_shop`

Fixed a bug which allowed track `fjsp_medium` to run for excessive amounts of time

Higher qualities across all tracks
...
- `{"job_shop_iters":<value>}` — Tabu search depth for the `job_shop` track (default 25,000, min 100, max 200,000). Scales strongly, higher values give meaningfully better quality at the cost of runtime. Increasing this parameter WILL increase runtime significantly, tune carefully.
- `{"hybrid_flow_shop_iters":<value>}` — Restart budget for the `hybrid_flow_shop` track (default 2,000, min 100, max 100,000). Higher values trade runtime for quality.
- `{"fjsp_medium_iters":<value>}` — Restart budget for the `fjsp_medium` track (default 2,000, min 100, max 100,000). Also scales the tabu, critical-block, ALNS, and ILS budgets. Higher values trade runtime for quality.

The `flow_shop` and `fjsp_high` tracks have no tuneable hyperparameter; their iteration budgets are fixed internally. Any other hyperparameter keys are ignored.
