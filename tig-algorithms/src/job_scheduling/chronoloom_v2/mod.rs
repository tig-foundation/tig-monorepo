...
fn detect_track_simple(challenge: &Challenge) -> Track {
    ...
    if flex_avg > 4.0 {
        ...
        Track::FjspMedium
    } else if flex_avg > 1.5 && !is_flow_shop {
    ...
    let result = match track {
        Track::FjspHigh => {
            let hp = merge_hp(hyperparameters, vec![
                ...
                ("fjsp_high_iters".into(), Value::Number(Number::from(14000u64))),
            ]);
            track_t44::solve_challenge(challenge, &guarded_save, &hp)
        }
        Track::FjspMedium => {
            let hp = merge_hp(hyperparameters, vec![
                ("track".into(), Value::String("fjsp_medium".into())),
                ("fjsp_medium_iters".into(), Value::Number(Number::from(10000u64))),
            ]);
        ...
        Track::HybridFlowShop => {
            let hp = merge_hp(hyperparameters, vec![
                ...
                ("num_restarts".into(), Value::Number(Number::from(1500u64))),
                ("hybrid_flow_shop_iters".into(), Value::Number(Number::from(9000u64))),
            ]);
