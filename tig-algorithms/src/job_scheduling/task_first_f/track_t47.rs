// FLAT: track_t47.rs — auto-generated from t47/ sub-dir
...
pub mod types {
    ...
    impl EffortConfig {
        pub fn default_effort() -> Self {
            Self { num_restarts: 2000, job_shop_iters: 100000, flow_shop_iters: 20000, hybrid_flow_shop_iters: 8000, fjsp_medium_iters: 5000, fjsp_high_iters: 5000 }
        }
...
pub mod infra {
    ...
    pub fn disj_to_solution(pre: &Pre, ds: &DisjSchedule, start: &[u32]) -> Result<Solution> {
        ...
        for j in 0..num_jobs {
            ...
            // Guard against stale `start` slices (e.g. from an `eval_disj` that returned
            // `None` because the disjunctive graph contained a cycle: cycle nodes keep
            // start = 0). Such a `start` would yield a job-chain-violating schedule that
            // the verifier rejects ("operation starting before previous is complete").
            // Reject the whole solution so the caller falls back to its last valid best.
            let mut prev_end = 0u32;
            for k in 0..len {
                let id = base + k;
                let st = start[id];
                if st < prev_end {
                    return Err(anyhow!(
                        "Inconsistent disjunctive start times (stale eval / graph cycle) for job {}",
                        j
                    ));
                }
                prev_end = st.saturating_add(ds.node_pt[id]);
                v.push((ds.node_machine[id], st));
            }
            job_schedule.push(v);
        }
...
pub mod flow_shop {
    ...
    fn disj_to_solution(pre: &Pre, ds: &DisjSchedule, start: &[u32]) -> Result<Solution> {
        ...
        for j in 0..num_jobs {
            ...
            // Same guard as infra::disj_to_solution: reject job-chain-violating start times
            // (a stale `start` from an `eval_disj` that returned `None` on a cyclic graph
            // leaves cycle nodes at start = 0) so callers fall back to their last valid best.
            let mut prev_end = 0u32;
            for k in 0..len {
                let id = base + k;
                let st = start[id];
                if st < prev_end {
                    return Err(anyhow!("Inconsistent disjunctive start times (stale eval / graph cycle) for job {}", j));
                }
                prev_end = st.saturating_add(ds.node_pt[id]);
                v.push((ds.node_machine[id], st));
            }
...
pub mod hybrid_flow_shop {
    ...
    pub fn solve(
        ...
        // Multi-start SCD-SR from elite pool (6 restarts, 80 sweeps) + tabu chain
        {
            let mut pool_starts: Vec<(Solution, u32)> = Vec::new();
            if let Some(ref s) = best_solution { pool_starts.push((s.clone(), best_makespan)); }
            for (sol, mk) in top_solutions.iter().take(12) {
                if pool_starts.len() >= 6 { break; }
                if !pool_starts.iter().any(|(_, m)| *m == *mk) {
                    pool_starts.push((sol.clone(), *mk));
                }
            }
            let scd_tenure = ((pre.total_ops as f64).sqrt() as usize).clamp(5, 12);
            for (start_sol, start_mk) in pool_starts {
                if let Some((scd_sol, scd_mk)) = scd_sr_pass(pre, challenge, &start_sol, start_mk, 80) {
                    let (final_sol, final_mk) = match tabu_ils_hfs(
                        pre, challenge, &scd_sol, scd_mk, 10000, scd_tenure, 120, &mut rng, save_solution
                    ) {
                        Ok((s, m)) => if m < scd_mk { (s, m) } else { (scd_sol, scd_mk) },
                        Err(_) => (scd_sol, scd_mk),
                    };
                    if final_mk < best_makespan {
                        best_makespan = final_mk;
                        best_solution = Some(final_sol.clone());
                        save_solution(&final_sol)?;
                    }
                }
            }
        }

        // Final lightweight polish on global best after SCD-SR
        if let Some(ref sol) = best_solution.clone() {
            let mut polished_sol = sol.clone();
            let mut polished_mk = best_makespan;
            if let Ok(Some((s, m))) = window_exhaustive(pre, challenge, &polished_sol, &[3, 2]) {
                if m < polished_mk {
                    polished_mk = m;
                    polished_sol = s;
                    save_solution(&polished_sol)?;
                    best_makespan = polished_mk;
                    best_solution = Some(polished_sol.clone());
                }
            }
            if let Ok(Some((s, m))) = schrage_pass(pre, challenge, &polished_sol, 4) {
                if m < polished_mk {
                    polished_mk = m;
                    polished_sol = s;
                    save_solution(&polished_sol)?;
            ...
            if let Ok(Some((s, m))) = greedy_reassign_pass(pre, challenge, &polished_sol) {
                if m < polished_mk {
                    polished_mk = m;
                    polished_sol = s;
                    save_solution(&polished_sol)?;
            ...
            if let Ok(Some((s, m))) = cp_descent_bf(pre, challenge, &polished_sol, 6) {
                if m < polished_mk {
                    polished_mk = m;
                    polished_sol = s;
                    save_solution(&polished_sol)?;
        ...
        // Final SCD-SR + tabu on overall best for last-mile Q push
        if let Some(ref base) = best_solution.clone() {
            if let Some((scd2_sol, scd2_mk)) = scd_sr_pass(pre, challenge, base, best_makespan, 60) {
                let scd_tenure2 = ((pre.total_ops as f64).sqrt() as usize).clamp(5, 12);
                let (fin_sol, fin_mk) = match tabu_ils_hfs(
                    pre, challenge, &scd2_sol, scd2_mk, 8000, scd_tenure2, 100, &mut rng, save_solution
                ) {
                    Ok((s, m)) => if m < scd2_mk { (s, m) } else { (scd2_sol, scd2_mk) },
                    Err(_) => (scd2_sol, scd2_mk),
                };
                if fin_mk < best_makespan {
                    best_makespan = fin_mk;
                    best_solution = Some(fin_sol.clone());
                    save_solution(&fin_sol)?;
                }
            }
        }
