...
pub fn solve_challenge(
    ...
    //   * invalid schedules (e.g. a job operation starting before the previous finishes,
    //     from a stale/cyclic disjunctive eval) fail evaluate_makespan and are skipped;
    ...
    let result = match track {
        Track::FjspHigh => {
            // T44 → job_ten v1
            let hp = merge_hp(hyperparameters, vec![("track".into(), Value::String("fjsp_high".into()))]);
        ...
        Track::FlowShop => {
            // T46 → adaptive_js_v5 v1 with tuned HPs from job 20239
        ...
        Track::JobShop => {
            // T48 → adaptive_js_v5 v1 with job_shop_iters:110000
            let hp = merge_hp(hyperparameters, vec![
                ("track".into(), Value::String("job_shop".into())),
                ("job_shop_iters".into(), Value::Number(Number::from(110000u64))),
            ]);
...
pub fn help() {
    println!("job_eleven — per-track flat self-contained, picked iter per track + baked HPs");
    println!("  T44 → job_ten v1");
    println!("  T45 → job_ten t45_i11");
    println!("  T46 → adaptive_js_v5 v1 (HP: flow_shop tuned)");
    println!("  T47 → job_ten t47_i6");
    println!("  T48 → adaptive_js_v5 v1 (HP: job_shop_iters:110000)");
}
