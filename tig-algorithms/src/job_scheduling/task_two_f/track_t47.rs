...
pub mod infra {
    ...
    pub fn disj_to_solution(pre: &Pre, ds: &DisjSchedule, start: &[u32]) -> Result<Solution> {
        ...
        for j in 0..num_jobs {
            ...
            // Guard against stale `start` (cyclic eval leaves cycle nodes at 0) -> reject so caller falls back.
            let mut prev_end = 0u32;
            for k in 0..len {
                let id = base + k;
                let st = start[id];
...
pub mod solver {
    ...
    fn parse_effort(hyperparameters: &Option<Map<String, Value>>) -> EffortConfig {
        ...
        //         i2 mesurait Q=53666 rejeté sous gate 54701; sous gate SOTA 53666≥53302 ✅
        //         temps attendu ~116s vs best-kept 198s = -41% throughput gain
        if let Some(map) = hyperparameters {
            if let Some(Value::String(s)) = map.get("track") {
                if s == "hybrid_flow_shop" || s == "hybrid" {
                    cfg.num_restarts = 500;
                    cfg = cfg.with_hybrid_flow_shop_iters(4000);
                }
            }
        }
        if let Some(map) = hyperparameters {
