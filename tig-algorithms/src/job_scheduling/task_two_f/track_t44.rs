...
pub mod fjsp_high {
    ...
    pub fn solve(
        ...
        // PHASE 2: Refinement 3 elites × 10 (was 5; top-3 covers landscape post-Phase9 i16)
        let rwl = (rw*1.40).clamp(rw, 0.40);
        let er = pool.len().min(3);
        for i in 0..er {
        ...
        // T44 i8 THROUGHPUT: tabu_outer_max 115→90 saves ~13s; tabu_iters_per_seed=6000 unchanged.
        let tabu_tenure = ((pre.total_ops as f64).sqrt() as usize).clamp(5, 14);
        let tabu_iters_per_seed = 6_000usize;
        let tabu_outer_max = 90usize;
        ...
        if let Ok(Some((s, m))) = iterative_cp_descent(pre, challenge, &best_sol, 15) {
            if m < best_mk { best_mk = m; best_sol = s.clone(); save_solution(&s)?; }
        }
        // Final greedy reassign
        if let Ok(Some((s, m))) = greedy_reassign_pass(pre, challenge, &best_sol) {
