...
fn cp_window_exhaustive(
    ...
    fn undo_rec(ds: &mut DisjSchedule, rec: &MoveRec) {
        let cur_m = ds.node_machine[rec.node];
        let pos = ds.machine_seq[cur_m].iter().position(|&x| x == rec.node).unwrap_or(0);
        ds.machine_seq[cur_m].remove(pos);
    ...
    for _iter in 0..max_iters {
        ...
        'window_search: for &wsize in window_sizes {
            ...
            for w_start in 0..num_windows {
                ...
                for &node in &window {
                    beam.sort_by(|a, b| {
                        path_sort_key(&a.moves)
                            .cmp(&path_sort_key(&b.moves))
                            .then_with(|| a.mk.cmp(&b.mk))
                    });
