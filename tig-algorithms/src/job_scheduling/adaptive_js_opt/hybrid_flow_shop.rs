...
fn choose_rule_bandit(rng: &mut SmallRng, rules: &[Rule], rule_best: &[u32], rule_tries: &[u32], global_best: u32, margin: u32, stuck: usize, chaos_like: bool, late_phase: bool) -> Rule {
    ...
    for (i, &rule) in rules.iter().enumerate() {
        r-=weights[i]; if r<=0.0 { return rule; }
    }
    rules[rules.len()-1]
}
...
fn construct_solution_conflict_mode<const HAS_TARGET: bool, const HAS_JOB_BIAS: bool, const HAS_MACHINE_PENALTY: bool, const USE_ROUTE_PREF: bool>(
    ...
    let mut ready_by_machine: Vec<Vec<(usize,u32)>>=(0..num_machines).map(|_|Vec::with_capacity(16)).collect();
    let mut job_op_gen=vec![1u32;num_jobs];
    let mut candidate_job_seen=vec![0u32;num_jobs];
    let mut candidate_gen: u32=0;
    ...
    for j in 0..num_jobs {
        ...
        for &(m,_) in &op.machines {
            ready_by_machine[m].push((j,job_op_gen[j]));
        }
        in_ready[j] = true;
    ...
    while remaining_ops > 0 {
        ...
        candidate_gen=candidate_gen.wrapping_add(1);
        if candidate_gen==0{
            candidate_job_seen.fill(0);
            candidate_gen=1;
        }
        for &m in &idle_machines {
            ...
            for idx in 0..list.len() {
                let (job,g)=list[idx];
                if g==job_op_gen[job]&&job_next_op[job]<pre.job_ops_len[job]&&!job_op_ptr[job].is_null(){
                    list[write]=(job,g);
                    write+=1;
                    if in_ready[job]{
                        candidate_job_seen[job]=candidate_gen;
                    }
                }
            }
            list.truncate(write);
        ...
        for &job in &ready_jobs {
            if candidate_job_seen[job]!=candidate_gen{continue;}
        ...
        if job_next_op[job] < pre.job_ops_len[job] {
            ...
            job_op_gen[job]=job_op_gen[job].wrapping_add(1);
            if job_op_gen[job]==0{job_op_gen[job]=1;}
            ...
            for &(m,_) in &next_op.machines {
                ready_by_machine[m].push((job,job_op_gen[job]));
            }
            if end_time==time{
                in_ready[job]=true;
...
fn select_seed_portfolio_indices(
    ...
    let build_pool = |limit_mk: Option<u32>| -> Vec<usize> {
        let mut best_by_sig: Vec<(u64, usize)> = Vec::with_capacity(scan);
        for i in 0..scan {
            ...
            let sig = exact_sigs[i];
            let mut found = false;
            for (s, best_i) in &mut best_by_sig {
                if *s == sig {
            ...
            if !found {
                best_by_sig.push((sig, i));
            }
        }
        let mut pool: Vec<usize> = best_by_sig.into_iter().map(|(_, idx)| idx).collect();
...
fn greedy_reassign_try_node(
    ...
    buf: &mut EvalBuf,
    job_pred_node: &[usize],
    machine_pred_node: &[usize],
    machine_succ_node: &[usize],
    tails: &[u32],
    ...
    for &(new_m,new_pt) in &op_info.machines {
        ...
        for insert_pos in positions {
            ...
            let est_mk=estimate_reassign_hfs(ds,cur_starts.as_slice(),tails,node,new_m,new_pt,ins,job_pred_node,machine_pred_node,machine_succ_node);
            if est_mk>=best_mk{continue;}
            {
...
fn greedy_reassign_pass(pre: &Pre, challenge: &Challenge, base_sol: &Solution) -> Result<Option<(Solution, u32)>> {
    ...
    let mut machine_succ_node=vec![NONE_USIZE;n];
    ...
    while rounds<max_rounds {
        ...
        machine_succ_node.copy_from_slice(&buf.machine_succ);
        ...
        if flex01>0.55 {
            ...
            if !order.is_empty() {
                ...
                order.truncate(cap);
                for node in order {
                    if greedy_reassign_try_node(pre,&mut ds,&mut buf,&job_pred_node,&machine_pred_node,&machine_succ_node,&tails,&mut cur_starts,&mut current_mk,node)? {
...
fn bottleneck_bridge_repair(
    ...
    let mut node_product = vec![0usize; n];
    let mut node_op_idx = vec![0usize; n];
    let mut node_flex = vec![0usize; n];
    let mut node_op_valid = vec![false; n];
    for node in 0..n {
        let job = ds.node_job[node];
        let product = pre.job_products[job];
        let op_idx = ds.node_op[node];
        node_product[node] = product;
        node_op_idx[node] = op_idx;
        if op_idx < pre.product_ops[product].len() {
            node_op_valid[node] = true;
            node_flex[node] = pre.product_ops[product][op_idx].machines.len();
        }
    }
    ...
    for _round in 0..rounds {
        ...
        if evals < eval_cap && reassign_node_cap > 0 {
            ...
            for &(_, _, node) in ranked_nodes.iter().take(node_lim) {
                if !node_op_valid[node] {
                    continue;
                }
                let product = node_product[node];
                let op_idx = node_op_idx[node];
                let op_info = &pre.product_ops[product][op_idx];
...
fn tabu_ils_hfs(
    ...
    let mut node_op_ref: Vec<Option<&OpInfo>> = vec![None; n];
    let mut node_flex = vec![0usize; n];
    for node in 0..n {
        let job = ds.node_job[node];
        let product = pre.job_products[job];
        ...
        if op_idx < pre.product_ops[product].len() {
            let op_info = &pre.product_ops[product][op_idx];
            node_flex[node] = op_info.machines.len();
            node_op_ref[node] = Some(op_info);
        }
    }
    ...
    let mut machine_pred_node = vec![NONE_USIZE; n];
    let mut crit_gen = vec![0u32; n];
    let mut cur_crit_gen: u32 = 0;
    ...
    for iter in 0..max_iters {
        ...
        if no_improve > 0 && no_improve % kick_threshold == 0 && kicks_left > 0 {
            ...
            cur_crit_gen = cur_crit_gen.wrapping_add(1);
            if cur_crit_gen == 0 {
                crit_gen.fill(0);
                cur_crit_gen = 1;
            }
            ...
            for m in 0..ds.num_machines {
                ...
                for i in 0..(ds.machine_seq[m].len() - 1) {
                    if crit_gen[ds.machine_seq[m][i]] == cur_crit_gen || crit_gen[ds.machine_seq[m][i + 1]] == cur_crit_gen {
                        kick_swaps.push((m, i));
                    }
                }
            }
        ...
        for m in 0..ds.num_machines {
            ...
            while i < ds.machine_seq[m].len() {
                if crit_gen[ds.machine_seq[m][i]] != cur_crit_gen {
                    i += 1;
                    continue;
                }
                let bstart = i;
        ...
        if iter % reassign_every == 0 {
            for node in 0..n {
                if crit_gen[node] != cur_crit_gen || node_flex[node] <= 1 {
                    continue;
                }
                let Some(op_info) = node_op_ref[node] else {
                    continue;
                };
                let job = ds.node_job[node];
                let cur_machine = ds.node_machine[node];
