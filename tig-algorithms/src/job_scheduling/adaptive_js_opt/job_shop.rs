...
#[derive(Clone, Copy)]
struct JsMachineStaticCache {
    scar_n_ptr: *const f64,
    scar_n_len: usize,
    pop_ptr: *const f64,
    pop_len: usize,
    pen_ptr: *const f64,
...
impl JsMachineStaticCache {
    #[inline(always)]
    fn empty() -> Self {
        Self {
            scar_n_ptr: std::ptr::null(),
            scar_n_len: 0,
            pop_ptr: std::ptr::null(),
            pop_len: 0,
            pen_ptr: std::ptr::null(),
            pen_len: 0,
            u8_ptr: std::ptr::null(),
            u8_len: 0,
        }
...
thread_local! {
    static JS_MACHINE_STATIC_CACHE: Cell<JsMachineStaticCache> = Cell::new(JsMachineStaticCache::empty());
}

#[inline(always)]
fn set_js_machine_static_cache(scar_n: &[f64], pop: &[f64], pen: &[f64], mu8: &[u8]) {
    JS_MACHINE_STATIC_CACHE.with(|cache| {
        cache.set(JsMachineStaticCache {
            scar_n_ptr: scar_n.as_ptr(),
            scar_n_len: scar_n.len(),
            pop_ptr: pop.as_ptr(),
            pop_len: pop.len(),
            pen_ptr: pen.as_ptr(),
            pen_len: pen.len(),
...
#[inline(always)]
fn clear_js_machine_static_cache() {
    JS_MACHINE_STATIC_CACHE.with(|cache| cache.set(JsMachineStaticCache::empty()));
}
...
fn js_cached_scar_n(pre: &Pre, machine: usize) -> f64 {
    JS_MACHINE_STATIC_CACHE.with(|cache| {
        let cache = cache.get();
        if machine < cache.scar_n_len {
            unsafe { *cache.scar_n_ptr.add(machine) }
        } else {
...
fn js_cached_pop(pre: &Pre, machine: usize) -> f64 {
    JS_MACHINE_STATIC_CACHE.with(|cache| {
        let cache = cache.get();
        if machine < cache.pop_len {
            unsafe { *cache.pop_ptr.add(machine) }
        } else {
...
fn js_cached_penalty(machine: usize, machine_penalty: f64) -> f64 {
    JS_MACHINE_STATIC_CACHE.with(|cache| {
        let cache = cache.get();
        if machine < cache.pen_len {
            unsafe { *cache.pen_ptr.add(machine) }
        } else {
...
fn js_cached_machine_u8(machine: usize) -> u8 {
    JS_MACHINE_STATIC_CACHE.with(|cache| {
        let cache = cache.get();
        if machine < cache.u8_len {
            unsafe { *cache.u8_ptr.add(machine) }
        } else {
...
fn construct_solution_conflict_impl<const RULE: u8, const USE_JB: bool, const USE_MP: bool, const USE_ROUTE: bool, const CHAOTIC: bool>(
    ...
    set_js_machine_static_cache(&machine_scar_n, machine_pop, &machine_penalty_clamped, &machine_u8);
