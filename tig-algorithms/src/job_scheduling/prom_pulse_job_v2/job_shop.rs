...
pub fn solve(
    ...
    let mut base = ranked[0].2.clone();
    if let Ok(Some((polished, polished_mk))) = critical_block_move_local_search_ex(pre, challenge, &base, 4, 260, 100) {
        if polished_mk < best_makespan {
            best_makespan = polished_mk;
            best_solution = Some(polished.clone());
            save_solution(&polished)?;
        }
        push_top_solutions(&mut top_solutions, &polished, polished_mk, 15);
        base = polished;
    }
    ...
    {
        let reassign_pool: Vec<Solution> = top_solutions.iter().take(5).map(|(s,_)| s.clone()).collect();
        for base_sol in &reassign_pool {
            if let Some((re_sol, re_mk)) = greedy_reassign_pass(pre, challenge, base_sol)? {
                if re_mk < best_makespan { best_makespan = re_mk; best_solution = Some(re_sol.clone()); save_solution(&re_sol)?; }
                push_top_solutions(&mut top_solutions, &re_sol, re_mk, 20);
                if let Some((ts_sol, ts_mk)) = tabu_search_phase(pre, challenge, &re_sol, ts_iters / 2, ts_tenure)? {
                    if ts_mk < best_makespan { best_makespan = ts_mk; best_solution = Some(ts_sol.clone()); save_solution(&ts_sol)?; }
