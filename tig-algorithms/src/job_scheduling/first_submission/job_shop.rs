...
pub fn solve(
    ...
    push_top_solutions(&mut top_solutions, &greedy_sol, greedy_mk, 25);
    ...
    if pre.flow_route.is_some() && pre.flow_pt_by_job.is_some() {
        if let Ok((sol, mk)) = neh_reentrant_flow_solution(pre, challenge.num_jobs, challenge.num_machines) {
            ...
            push_top_solutions(&mut top_solutions, &sol, mk, 18);
        }
    }
    ...
    for &rule in &rules {
        ...
        if mk < best_makespan { best_makespan=mk; best_solution=Some(sol.clone()); save_solution(&sol)?; }
        push_top_solutions(&mut top_solutions, &sol, mk, 18); ranked.push((rule, mk, sol));
    }
    ...
    for (sol, mk) in refine_results { push_top_solutions(&mut top_solutions, &sol, mk, 18); }

    let ts_starts = top_solutions.len().min(11);
    ...
    // Phase 5: Enhanced local search with insertion + machine rerouting on best solutions
    // This covers moves the tabu search misses (non-adjacent insertions, machine reassignment)
    let mut ls_candidates: Vec<(Solution, u32)> = Vec::new();
    for (sol, mk) in ts_results.iter() {
        ls_candidates.push((sol.clone(), *mk));
    }
    // Also include top construction solutions that weren't improved by tabu
    for i in 0..top_solutions.len().min(8) {
        ls_candidates.push(top_solutions[i].clone());
    }
    ls_candidates.sort_by_key(|x| x.1);
    ls_candidates.truncate(8);

    let ls_iters = 8usize;
    let ls_top_cands = 48usize;
    let ls_perturb = 6usize;
    for (base_sol, _base_mk) in ls_candidates.iter() {
        if let Some((sol2, mk2)) = critical_block_move_local_search_ex(pre, challenge, base_sol, ls_iters, ls_top_cands, ls_perturb)? {
            if mk2 < best_makespan { best_makespan=mk2; best_solution=Some(sol2.clone()); save_solution(&sol2)?; }
        }
    }
