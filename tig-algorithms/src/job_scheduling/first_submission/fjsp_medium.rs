...
pub fn solve(
    ...
    // Run greedy reassign on top solutions, not just the best
    let mut reassign_pool: Vec<(Solution,u32)> = Vec::new();
    for i in 0..top_solutions.len().min(5) { reassign_pool.push(top_solutions[i].clone()); }
    if let Some(ref sol)=best_solution { reassign_pool.push((sol.clone(), best_makespan)); }
    reassign_pool.sort_by_key(|x| x.1);
    reassign_pool.dedup_by_key(|x| x.1);
    reassign_pool.truncate(3);
    for (base_sol, _) in reassign_pool.iter() {
        if let Ok(Some((sol2,mk2))) = greedy_reassign_pass(pre,challenge,base_sol) {
            if mk2 < best_makespan { best_makespan = mk2; best_solution = Some(sol2.clone()); save_solution(&sol2)?; }
        }
    }

    // Enhanced local search with insertion + rerouting on best solutions
    let mut ls_pool: Vec<(Solution,u32)> = Vec::new();
    for i in 0..top_solutions.len().min(3) { ls_pool.push(top_solutions[i].clone()); }
    if let Some(ref sol)=best_solution { ls_pool.push((sol.clone(), best_makespan)); }
    ls_pool.sort_by_key(|x| x.1);
    ls_pool.truncate(10);
    for (base_sol, _) in ls_pool.iter() {
        if let Some((sol2, mk2)) = critical_block_move_local_search_ex(pre, challenge, base_sol, 8, 48, 4)? {
            if mk2 < best_makespan { best_makespan = mk2; best_solution = Some(sol2.clone()); save_solution(&sol2)?; }
        }
    }
