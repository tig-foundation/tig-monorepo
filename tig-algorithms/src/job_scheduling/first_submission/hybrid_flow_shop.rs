...
pub fn solve(
    ...
    if let Some(ref sol)=best_solution.clone() {
        if pre.high_flex+pre.jobshopness > 0.55 {
            if let Some((sol2,mk2))=critical_block_move_local_search_ex(pre,challenge,sol,50,80,18)?{
                commit_best(save_solution,&mut best_makespan,&mut best_solution,&sol2,mk2)?;
            }
        }
    }

    // Second round of enhanced LS on the best solution with more perturbation
    if let Some(ref sol)=best_solution.clone() {
        if let Some((sol2,mk2))=critical_block_move_local_search_ex(pre,challenge,sol,30,96,12)?{
            commit_best(save_solution,&mut best_makespan,&mut best_solution,&sol2,mk2)?;
        }
    }

    // Run greedy reassign on top solutions
