    ...
    while improved&&passes<max_passes {
        ...
        for node in 0..n {
            ...
            for &(new_m,new_pt) in &op_info.machines {
                ...
                let pos0=sorted_pos;
                ds.machine_seq[new_m].insert(pos0,node);
                ...
                ds.machine_seq[new_m].remove(pos0);

                let pos1=sorted_pos.saturating_sub(1);
                if pos1!=pos0 {
                    ds.machine_seq[new_m].insert(pos1,node);
                    ...
                    ds.machine_seq[new_m].remove(pos1);
                }

                let pos2=target_len;
                if pos2!=pos0&&pos2!=pos1 {
                    ds.machine_seq[new_m].insert(pos2,node);
                    ...
                    ds.machine_seq[new_m].remove(pos2);
                }
...
fn iterative_cp_descent(
    ...
    for m in 0..ds.machine_seq.len() {
        for (pos, &nd) in ds.machine_seq[m].iter().enumerate() {
            node_pos[nd] = pos;
        }
    }
    let rescan_machine = |m: usize, ds: &DisjSchedule, node_pos: &mut Vec<usize>| {
        for (pos, &nd) in ds.machine_seq[m].iter().enumerate() {
            node_pos[nd] = pos;
        }
    };
    ...
    for _iter in 0..max_iters {
        ...
        for &nd in &order {
            ...
            let mut pos = node_pos[nd];
            if pos >= seq.len() || seq[pos] != nd {
                pos = match seq.iter().position(|&x| x == nd) {
                    Some(p) => p,
                    None => continue,
                };
                node_pos[nd] = pos;
        ...
        for &nd in &cp_flex {
            ...
            let mut old_pos = node_pos[nd];
            if old_pos >= ds.machine_seq[cur_m].len() || ds.machine_seq[cur_m][old_pos] != nd {
                old_pos = match ds.machine_seq[cur_m].iter().position(|&x| x == nd) {
                    Some(p) => p,
                    None => continue,
                };
                node_pos[nd] = old_pos;
            ...
            for &(new_m, new_pt) in &op_info.machines {
                ...
                let pos0 = spos;
                ds.machine_seq[new_m].insert(pos0, nd);
                if let Some((tmk, _)) = eval_disj(&ds, &mut buf) {
                    if tmk < best_mk { best_mk = tmk; best_m = new_m; best_pt = new_pt; best_ins = pos0; }
                }
                ds.machine_seq[new_m].remove(pos0);

                let pos1 = spos.saturating_sub(1);
                if pos1 != pos0 {
                    ds.machine_seq[new_m].insert(pos1, nd);
                    if let Some((tmk, _)) = eval_disj(&ds, &mut buf) {
                        if tmk < best_mk { best_mk = tmk; best_m = new_m; best_pt = new_pt; best_ins = pos1; }
                    }
                    ds.machine_seq[new_m].remove(pos1);
                }

                let pos2 = (spos + 1).min(tlen);
                if pos2 != pos0 && pos2 != pos1 {
                    ds.machine_seq[new_m].insert(pos2, nd);
                    if let Some((tmk, _)) = eval_disj(&ds, &mut buf) {
                        if tmk < best_mk { best_mk = tmk; best_m = new_m; best_pt = new_pt; best_ins = pos2; }
                    }
                    ds.machine_seq[new_m].remove(pos2);
                }

                let pos3 = tlen;
                if pos3 != pos0 && pos3 != pos1 && pos3 != pos2 {
                    ds.machine_seq[new_m].insert(pos3, nd);
                    if let Some((tmk, _)) = eval_disj(&ds, &mut buf) {
                        if tmk < best_mk { best_mk = tmk; best_m = new_m; best_pt = new_pt; best_ins = pos3; }
                    }
                    ds.machine_seq[new_m].remove(pos3);
                }
            ...
            if best_m != cur_m {
                let mut old_pos = node_pos[nd];
                if old_pos >= ds.machine_seq[cur_m].len() || ds.machine_seq[cur_m][old_pos] != nd {
                    old_pos = ds.machine_seq[cur_m].iter().position(|&x| x == nd).unwrap();
                    node_pos[nd] = old_pos;
                }
                ds.machine_seq[cur_m].remove(old_pos);
                ...
                current_mk = best_mk;
                rescan_machine(cur_m, &ds, &mut node_pos);
                rescan_machine(best_m, &ds, &mut node_pos);
                let _ = eval_disj(&ds, &mut buf);
                iter_improved = true;
            }
        }
        ...
        if cp_flex.len() >= 2 {
            let cand_positions = |ds: &DisjSchedule, buf: &EvalBuf, m: usize, nd: usize| -> (usize, usize, usize) {
                let cur_start = buf.start[nd];
                ...
                (spos, spos.saturating_sub(1), tlen)
            };

            for pair_idx in 0..(cp_flex.len() - 1) {
                let nd1 = cp_flex[pair_idx];
                let nd2 = cp_flex[pair_idx + 1];
                ...
                let mut pos1 = node_pos[nd1];
                if pos1 >= ds.machine_seq[cur_m1].len() || ds.machine_seq[cur_m1][pos1] != nd1 {
                    pos1 = match ds.machine_seq[cur_m1].iter().position(|&x| x == nd1) {
                        Some(p) => p,
                        None => continue,
                    };
                    node_pos[nd1] = pos1;
                }
                let mut pos2 = node_pos[nd2];
                if pos2 >= ds.machine_seq[cur_m2].len() || ds.machine_seq[cur_m2][pos2] != nd2 {
                    pos2 = match ds.machine_seq[cur_m2].iter().position(|&x| x == nd2) {
                        Some(p) => p,
                        None => continue,
                    };
                    node_pos[nd2] = pos2;
                ...
                for &(m1, pt1) in &opts1 {
                    for &(m2, pt2) in &opts2 {
                        if m1 != m2 {
                            ...
                            let (p10, p11, p12) = cand_positions(&ds, &buf, m1, nd1);
                            let (p20, p21, p22) = cand_positions(&ds, &buf, m2, nd2);

                            let try_p1 = |p1i: usize, ds: &mut DisjSchedule, buf: &mut EvalBuf, best_mk_pair: &mut u32, best_config: &mut Option<(usize, u32, usize, usize, u32, usize, u8)>| {
                                ds.machine_seq[m1].insert(p1i, nd1);

                                ds.machine_seq[m2].insert(p20, nd2);
                                if let Some((tmk, _)) = eval_disj(&ds, buf) {
                                    if tmk < *best_mk_pair {
                                        *best_mk_pair = tmk;
                                        *best_config = Some((m1, pt1, p1i, m2, pt2, p20, 0));
                                    }
                                }
                                ds.machine_seq[m2].remove(p20);

                                if p21 != p20 {
                                    ds.machine_seq[m2].insert(p21, nd2);
                                    if let Some((tmk, _)) = eval_disj(&ds, buf) {
                                        if tmk < *best_mk_pair {
                                            *best_mk_pair = tmk;
                                            *best_config = Some((m1, pt1, p1i, m2, pt2, p21, 0));
                                        }
                                    }
                                    ds.machine_seq[m2].remove(p21);
                                }

                                if p22 != p20 && p22 != p21 {
                                    ds.machine_seq[m2].insert(p22, nd2);
                                    if let Some((tmk, _)) = eval_disj(&ds, buf) {
                                        if tmk < *best_mk_pair {
                                            *best_mk_pair = tmk;
                                            *best_config = Some((m1, pt1, p1i, m2, pt2, p22, 0));
                                        }
                                    }
                                    ds.machine_seq[m2].remove(p22);
                                }

                                ds.machine_seq[m1].remove(p1i);
                            };

                            try_p1(p10, &mut ds, &mut buf, &mut best_mk_pair, &mut best_config);
                            if p11 != p10 {
                                try_p1(p11, &mut ds, &mut buf, &mut best_mk_pair, &mut best_config);
                            }
                            if p12 != p10 && p12 != p11 {
                                try_p1(p12, &mut ds, &mut buf, &mut best_mk_pair, &mut best_config);
                            }
                        } else {
                            let m = m1;
                            ...
                            for order_flag in 0u8..=1u8 {
                                ...
                                let (pa0, pa1, pa2) = cand_positions(&ds, &buf, m, a);

                                let try_pa = |pai: usize, ds: &mut DisjSchedule, buf: &mut EvalBuf, best_mk_pair: &mut u32, best_config: &mut Option<(usize, u32, usize, usize, u32, usize, u8)>| {
                                    ds.machine_seq[m].insert(pai, a);
                                    let (pb0, pb1, pb2) = cand_positions(&ds, &buf, m, b);

                                    ds.machine_seq[m].insert(pb0, b);
                                    if let Some((tmk, _)) = eval_disj(&ds, buf) {
                                        if tmk < *best_mk_pair {
                                            *best_mk_pair = tmk;
                                            if order_flag == 0 {
                                                *best_config = Some((m, pt1, pai, m, pt2, pb0, 0));
                                            } else {
                                                *best_config = Some((m, pt1, pb0, m, pt2, pai, 1));
                                            }
                                        }
                                    }
                                    ds.machine_seq[m].remove(pb0);

                                    if pb1 != pb0 {
                                        ds.machine_seq[m].insert(pb1, b);
                                        if let Some((tmk, _)) = eval_disj(&ds, buf) {
                                            if tmk < *best_mk_pair {
                                                *best_mk_pair = tmk;
                                                if order_flag == 0 {
                                                    *best_config = Some((m, pt1, pai, m, pt2, pb1, 0));
                                                } else {
                                                    *best_config = Some((m, pt1, pb1, m, pt2, pai, 1));
                                                }
                                            }
                                        }
                                        ds.machine_seq[m].remove(pb1);
                                    }

                                    if pb2 != pb0 && pb2 != pb1 {
                                        ds.machine_seq[m].insert(pb2, b);
                                        if let Some((tmk, _)) = eval_disj(&ds, buf) {
                                            if tmk < *best_mk_pair {
                                                *best_mk_pair = tmk;
                                                if order_flag == 0 {
                                                    *best_config = Some((m, pt1, pai, m, pt2, pb2, 0));
                                                } else {
                                                    *best_config = Some((m, pt1, pb2, m, pt2, pai, 1));
                                                }
                                            }
                                        }
                                        ds.machine_seq[m].remove(pb2);
                                    }

                                    ds.machine_seq[m].remove(pai);
                                };

                                try_pa(pa0, &mut ds, &mut buf, &mut best_mk_pair, &mut best_config);
                                if pa1 != pa0 {
                                    try_pa(pa1, &mut ds, &mut buf, &mut best_mk_pair, &mut best_config);
                                }
                                if pa2 != pa0 && pa2 != pa1 {
                                    try_pa(pa2, &mut ds, &mut buf, &mut best_mk_pair, &mut best_config);
                                }
                            }
                        }
                    }
                ...
                if let Some((bm1, bpt1, bp1, bm2, bpt2, bp2, order_flag)) = best_config {
                    if best_mk_pair < current_mk {
                        ...
                        rescan_machine(cur_m1, &ds, &mut node_pos);
                        rescan_machine(cur_m2, &ds, &mut node_pos);
                        rescan_machine(bm1, &ds, &mut node_pos);
                        rescan_machine(bm2, &ds, &mut node_pos);
                        let _ = eval_disj(&ds, &mut buf);
                        iter_improved = true;
                        continue;
                    }
