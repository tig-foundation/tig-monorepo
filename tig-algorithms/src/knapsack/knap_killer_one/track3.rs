...
mod inner {
            ...
            if n <= 1200 {
                if budget <= 5 {
                    Self {
                        ...
                        n_full_restarts: 1, rush_mode: 0, rush_ils: 0, rush_kick_stall: 12, rush_restarts: 1, rush_tabu: 0, ils_stall_stop: 0, xr_elite: 0, use_hub_pair: true,
                    ...
                    Self {
                        ...
                        sa_iter: 0, n_sa_members: 0, ils_rounds: 30,
                        ils_restart_interval: 1, perturb_base_frac: 100,
                        perturb_max_frac: 28, ils_vnd_level: 0, bounded_2_2_k: 0,
                        n_full_restarts: 8, rush_mode: 0, rush_ils: 0, rush_kick_stall: 12, rush_restarts: 1, rush_tabu: 0, ils_stall_stop: 3, xr_elite: 6, use_hub_pair: true,
                ...
                Self {
                    ...
                    n_full_restarts: 1, rush_mode: 0, rush_ils: 0, rush_kick_stall: 12, rush_restarts: 1, rush_tabu: 0, ils_stall_stop: 0, xr_elite: 0, use_hub_pair: false,
    ...
    // Same fill as greedy_reconstruct, minus the items the perturbation just dropped. Without
    // this exclusion the two stages rank on the same key, so the fill re-adds exactly what was
    // removed and ruin+repair composes to the identity.
    ...
    /// Cross-restart incumbent archive.
    ///
    /// The multistart loop below is a plain best-of-N over statistically INDEPENDENT runs: the
    /// only thing that separates restart k from restart k+1 is an rng offset, and the eight
    /// deterministic seeds (4 greedy + 4 hub pairs) are byte-identical in every one of them. The
    /// measured consequence on this track is the classic max-of-iid saturation -- 12 restarts buy
    /// 53063, 22 buy 53181, and it then takes 48 restarts plus twice the ILS budget to reach
    /// 53218, i.e. +37 points of value for +4.2 s of work. Sampling harder is the wrong lever.
    ///
    /// This archive is the information channel the loop was missing: every restart deposits its
    /// incumbent here, and the next restart receives them as extra population members, so the
    /// crossover/path-relink machinery recombines material found by OTHER restarts instead of
    /// re-deriving the same eight seeds. Membership is Hamming-separated so the archive stores
    /// distinct basins rather than N copies of the same optimum; inside a ball of radius 20 only
    /// the better incumbent survives.
    ///
    /// Fully deterministic: no rng, no clock, insertion order fixed by the restart index.
    ...
    fn run_one_instance_with_value(challenge: &Challenge, hp: &Hparams, rng_offset: usize, archive: &[SolState]) -> (Solution, i64, SolState) {
        ...
        run_one_instance_with_seed_cache_value(challenge, hp, rng_offset, &deterministic, archive)
    }

    fn run_one_instance_with_seed_cache_value(challenge: &Challenge, hp: &Hparams, rng_offset: usize, deterministic: &(Vec<SolState>, Vec<SolState>), archive: &[SolState]) -> (Solution, i64, SolState) {
        let n = challenge.num_items;
        ...
        // Cross-restart intensification. Injected AFTER the random constructions on purpose: the
        // constructors are the only consumers of the rng stream up to this point, so an empty
        // archive leaves this function bit-identical to the parent, and a non-empty one changes
        // the search WITHOUT desynchronising the seeds it is meant to be compared against.
        ...
        for round in 0..hp.ils_rounds {
            ...
            if state.total_value <= snap.value {
                ...
                // Truncate the tail of a restart that has stopped paying. The ILS budget is the
                // dominant per-restart cost and its last rounds are near-flat; cutting them frees
                // work for MORE restarts at equal total budget, which is where the value is.
    ...
    impl Solver {
        pub fn solve(
            ...
            if hp.rush_mode > 0 {
                ...
                // rush_mode>=4 saturates then drops, so every decision reads the true in-set
                // marginal contribution; the static ranking below never sees interaction structure.
                if hp.rush_mode >= 4 {
                ...
                if hp.rush_mode >= 2 {
                    ...
                    for _ in 0..passes {
                        ...
                        dp_refinement_hp(&mut st, core);
                        local_search_vnd_windowed(&mut st, win);
                ...
                if hp.rush_mode >= 5 && hp.rush_ils > 0 {
                    ...
                    // The walk owns a LOCAL incumbent distinct from the global best: without it a
                    // fresh construction is snapped back to `best` on its first non-improving round,
                    // which would erase the diversification before it can be exploited.
            ...
            // Empty and never written while xr_elite == 0 -> default path bit-identical to parent.
            ...
            for restart in 0..(n_restarts.saturating_sub(1)) {
                ...
                let (sol, val, snap) = if let Some(ref cache) = deterministic_seed_cache {
                    run_one_instance_with_seed_cache_value(challenge, &hp, restart, cache, &inject)
                } else {
            ...
            // Terminal recombination ACROSS restarts. Each restart already path-relinks inside its
            // own elite pool, but nothing ever relinked two restarts together -- the final answer
            // was a plain argmax. Relinking the global incumbent against every archived basin is
            // the one place where independent runs can actually combine, and it costs a handful of
            // relink walks instead of a whole extra restart.
