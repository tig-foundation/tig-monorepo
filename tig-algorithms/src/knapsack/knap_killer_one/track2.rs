...
mod inner {
    ...
    impl Hparams {
        ...
        fn from_map(h: &Option<Map<String, Value>>, n: usize, budget: u32) -> Self {
            ...
            if let Some(m) = h {
                ...
                // Attribution only: disables the synergy neighbourhood so a time batch can price
                // it. It is neither bounded nor windowed, so scaling `window_k` left it active in
                // both arms and it never appeared in the windowed-neighbourhood figure.
    ...
    // Attribution scaffolding. `stage_stop` (0 = production path, untouched) makes the
    // solver return right after a named construction stage so a time batch measures that
    // stage's cost in isolation. `tag` is folded into the returned pick so the stage's work
    // is genuinely observed and cannot be elided by the optimiser. The returned selection is
    // one feasible item, so the run stays valid; its quality is meaningless by construction
    // and these points must never enter a verdict.
    ...
    pub fn run_bp_algorithm(inst: &P1Instance, n_lambda_values: usize,
                            stage_stop: usize, bp_tag: &mut u64) -> QKPResult {
        ...
        if stage_stop == 2 {
            ...
            *bp_tag = t;
            return QKPResult { results: Vec::new() };
        }
    ...
    pub struct P2State {
        ...
        pub best_weight: i32,
        pub best_count:  i32,
        // Per-sweep memo of the neighbourhood candidate lists (see cached_best_unused).
    ...
    impl P2State {
        pub fn new(ins: P2Instance) -> Self {
            ...
            P2State {
                ...
                cache_on: false, vnd_resume: false, no_synergy: false, syn_bound: false, ins,
            }
        }
    ...
    // Same q as the edge-list path (own value on the diagonal, upper triangle mirrored) without
    // materialising the ~n^2/2 intermediate edge records the generic bridge goes through.
    ...
    // Only the `pool` best items by interaction-aware density get a row in q, so the quadratic
    // build is O(pool^2). Any selection stays feasible and is scored exactly, since every
    // interaction between two retained items is present in the submatrix.
    ...
    // The candidate lists are pure functions of the current selection and contributions. A sweep
    // of the neighbourhood ladder only advances while no move has been applied -- every mutation
    // is immediately followed by a return, which restarts the sweep -- so within one sweep the
    // list for a given window size cannot change. The ladder asks for windows 250, 125, 125, 250,
    // so the later neighbourhoods were rebuilding from scratch (a full scan of every item, a
    // partial selection and a sort) exactly what the earlier ones had just computed. Memoising
    // per sweep hands back the same vectors, so the moves chosen are bit-identical.
    ...
    fn apply_synergy_tail(s: &mut P2State) -> bool {
        ...
        // Exact upper bounds for the four blocks below. Every interaction profit and every
        // contribution is >= 0, so dropping the `- q[c][removed]` terms can only raise the
        // estimate: a candidate at index z or later contributes at most `sc_suf[z]` (suffix
        // maximum of the contributions, in the enumeration order) plus, for each already-fixed
        // incoming item, the largest interaction its row offers over the pool (`rowmax`). Both
        // tables cost O(pool^2) against inner loops running into the hundreds of thousands.
        // Acceptance is a strict `>` against a `best_delta` that only rises, so breaking on the
        // bound skips only provably non-improving candidates: the chosen move is unchanged.
    ...
    fn local_search_vnd(s: &mut P2State, window_k: usize, heavy: bool) {
        ...
        if !s.vnd_resume {
            ...
            // Stay in the neighbourhood that just fired instead of falling back to the cheapest
            // one after every accepted move; only step down the ladder when a neighbourhood is
            // exhausted. Trades a little exploration for far fewer re-descents.
            let mut lvl = 0usize;
            loop {
                let fired = match lvl {
    ...
    fn rush_search(s: &mut P2State, hp: &Hparams) {
        let total_interactions = compute_total_interactions(&s.ins);
        // Polish window as a percentage of the historical caps; 0 keeps them verbatim and
        // 100 reproduces them exactly, so any value is a controlled widening of the same shape.
        ...
        // The 5 reconstruction strategies are distinct score functions, so each yields a
        // different basin; beyond 5 the sequence repeats and the run is deterministic.
    ...
    impl Solver {
        pub fn solve(
            ...
            let mut bp_tag = 0u64;
            let qr = run_bp_algorithm(&inst, hp.n_lambda_values, hp.stage_stop, &mut bp_tag);
            if hp.stage_stop == 2 {
