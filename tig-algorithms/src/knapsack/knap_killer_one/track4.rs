...
mod inner_four {
    ...
    fn build_ejection_windows(state: &State, best_unused: &[usize], worst_used: &[usize]) -> (Vec<usize>, Vec<usize>) {
        ...
        if !unused.is_empty() && unused.len() < 32 {
                ...
                collect_cached_ejection_supplemental(state, &unused, need)
            {
                ...
                for &a in &unused { in_unused[a] = true; }
                let mut supplemental: Vec<(usize, i32)> = Vec::new();
                for i in 0..nn {
                    if state.selected_bit[i] || in_unused[i] { continue; }
    ...
    fn crossover_frequency(population: &[SolState], extra: &[SolState], ch: &Challenge, rng: &mut Rng) -> Vec<bool> {
        let n = ch.num_items;
    ...
    impl Hparams {
        fn defaults() -> Self {
            Self{n_random_starts: 5, n_crossover_gen: 13, ils_rounds: 105,
            ...
            rush_mode: 0, elite_chain: 2, elite_k: 2,}
        }
        ...
        fn from_map(h: &Option<Map<String, Value>>) -> Self {
            ...
            if let Some(m) = h {
                ...
                if let Some(v) = m.get("elite_chain").and_then(|v| v.as_u64()) { p.elite_chain = v as usize; }
                if let Some(v) = m.get("elite_k").and_then(|v| v.as_u64()) { p.elite_k = v as usize; }
            }
            p
        }
    }
    ...
    fn run_one_instance(
        ...
        // elite_chain=1 : les élites entrent dans la population (donc éligibles comme départ ILS),
        // le plafond monte d'autant pour n'évincer aucun membre existant.
        // elite_chain=2 : elles restent hors population et ne servent que de parents.
            ...
            if population.len() >= 2 {
                ...
                if !parents.is_empty() {
                    let child_bits = crossover_uniform(
                        &population[a], &parents[gen % parents.len()], challenge, &mut rng);
                    set_state_from_bits(&mut state, &child_bits);
                    dp_refinement_hp(&mut state, ch);
                    vnd_dispatch(&mut state, hp);
                    population.push(state.clone_solution());
        ...
        if final_state.total_value > best_val {
            ...
            (Solution { items: final_state.selected_items() }, final_state.total_value, elite)
        } else {
        ...
        pub fn solve(
            ...
            if hp.rush_mode > 0 {
                ...
                let sol = if st.total_weight <= challenge.max_weight {
                    Some(Solution { items: st.selected_items() })
                } else {
                    None
                };
                INTERACTION_NEIGHBOR_CACHE.with(|cache| {
            ...
            for restart in 0..n_restarts {
                ...
                if elite_k > 0 {
                    ...
                    // deux élites identiques pèseraient double dans le vote de fréquence
                    let mut seen: Vec<u64> = Vec::with_capacity(elites.len());
                    elites.retain(|s| {
                        if seen.contains(&s.hash) { false } else { seen.push(s.hash); true }
                    });
