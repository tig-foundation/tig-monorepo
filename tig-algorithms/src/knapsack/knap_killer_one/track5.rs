...
/// Compressed sparse row view of `interaction_values`, restricted to the NONZERO entries.
///
/// The whole solver spends its budget in `State::add_item` / `State::remove_item`, which walk a
/// full row of `interaction_values` (n = 5000 entries) to fold it into `contrib`. On this
/// instance family the matrix is a Jaccard similarity matrix: it is SPARSE (about 13 % of the
/// entries are nonzero) and every entry lies in `0..=1000`. A zero entry contributes nothing to
/// `contrib`, so walking it is pure overhead.
///
/// `Csr` stores only the nonzero entries, as `u16` column index (n < 65536) and `u16` value
/// (Jaccard * 1000 <= 1000). The fold becomes a walk over `deg(i)` entries instead of `n`, and
/// the payload shrinks from `4 * n` bytes per row to `4 * deg(i)`.
///
/// This is an EXACT reduction of the work, not of the model: the resulting `contrib` array is
/// bit-identical to the dense fold (the skipped entries add / subtract zero), so the search
/// trajectory, the accepted moves and the returned solution are byte-identical. `csr = None`
/// restores the dense fold verbatim and is the control.
struct Csr {
    starts: Vec<u32>,
    idx: Vec<u16>,
    val: Vec<u16>,
...
impl Csr {
    /// Returns `None` (dense fallback) when the instance does not fit the `u16` packing or when
    /// the matrix is too dense for the sparse walk to pay off.
    fn build(ch: &Challenge) -> Option<Self> {
        ...
        // Below ~50 % density the packed walk still reads less memory than the dense fold; above
        // it the dense, branch-free, unrolled loop is the better machine code. Bail out rather
        // than regress.
        if nnz * 2 > n.saturating_mul(n) { return None; }
        let mut idx: Vec<u16> = Vec::with_capacity(nnz);
        let mut val: Vec<u16> = Vec::with_capacity(nnz);
        for i in 0..n {
            let row = unsafe { ch.interaction_values.get_unchecked(i) };
            for j in 0..n {
                let v = unsafe { *row.get_unchecked(j) };
    ...
    fn add_item(&mut self, i: usize) {
        ...
        if let Some(csr) = self.csr {
            ...
            unsafe {
                ...
                // Unrolled by 4: the column indices inside a CSR row are distinct, so the four
                // read-modify-writes cannot alias and the order is irrelevant. Keeping the block
                // count per fold low matters as much as the element count.
                let mut k = s;
                while k + 4 <= e {
...

        {
            ...
            for &r1 in &worst_used {
                ...
                for &r2 in &tsn.friends[r1] {
                    ...
                    let v = state.ch.interaction_values[r1][r2] as i64
...
impl Hparams {
    fn defaults() -> Self {
        Self {
            n_random_starts: 5, n_crossover_gen: 3, ils_rounds: 150,
            ils_restart_interval: 11, perturb_base_frac: 6,
            perturb_max_frac: 4, n_full_restarts: 14, use_hub_pair: false,
            use_heavy_polish: false, window_k: 179, core_half_dp: 41,
            restart_stall_lim: 20, stall_cap: 20, back_to_best: 1,
            seed_every: 1, seed_ruin_div: 24, sparse_contrib: 1,
        }
    }
...
fn run_one_instance_seeded(challenge: &Challenge, hp: &Hparams, rng_offset: usize, shared_tsn: Option<&TopNeighbors>, total_interactions: &[i64], seed_sol: Option<&Solution>, det_seeds: Option<&[SolState]>, csr: Option<&Csr>) -> (Solution, i64) {
    ...
    for round in 0..cnt {
        ...
        if state.total_value <= snap.value {
            ...
            // The walk accepts every perturbation result unconditionally, so without an anchor it
            // drifts away from best_snap and never comes back within a restart.
...
impl Solver {
    pub fn solve(
        ...
        // Exact work reduction: fold only the nonzero entries of a row into `contrib`.
        // `sparse_contrib = 0` keeps the dense fold and is the byte-identical control.
