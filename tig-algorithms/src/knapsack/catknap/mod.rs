...
pub struct Hyperparameters {
    pub diff_lim: usize,
    pub core_half_dp: usize,
    pub core_half_ls: usize,
    pub polish_k: usize,
    pub initial_polish_iters: usize,
    pub dp_polish_iters: usize,
    pub perturb_polish_iters: usize,
    pub final_polish_iters_pre_vnd: usize,
    pub final_polish_iters_post_vnd: usize,
    pub perturb_removal_divisor: usize,
    pub perturb_min_removal: usize,
    pub perturb_max_removal: usize,
    pub swap22_window_limit: usize,
}
...
impl Default for Hyperparameters {
    fn default() -> Self {
        Self {
            ...
            core_half_ls: 80,
            polish_k: 30,
            initial_polish_iters: 3,
            dp_polish_iters: 3,
            perturb_polish_iters: 3,
            final_polish_iters_pre_vnd: 5,
            final_polish_iters_post_vnd: 5,
            perturb_removal_divisor: 5,
            perturb_min_removal: 3,
            perturb_max_removal: 20,
            swap22_window_limit: 15,
        }
    }
}
...
struct Params {
    ...
    core_half_ls: usize,
    polish_k: usize,
    initial_polish_iters: usize,
    dp_polish_iters: usize,
    perturb_polish_iters: usize,
    final_polish_iters_pre_vnd: usize,
    final_polish_iters_post_vnd: usize,
    perturb_removal_divisor: usize,
    perturb_min_removal: usize,
    perturb_max_removal: usize,
    swap22_window_limit: usize,
}
impl Params {
    fn from_hyperparameters(hyperparams: &Hyperparameters) -> Self {
        Self {
            diff_lim: hyperparams.diff_lim,
            core_half_dp: hyperparams.core_half_dp,
            core_half_ls: hyperparams.core_half_ls,
            polish_k: hyperparams.polish_k,
            initial_polish_iters: hyperparams.initial_polish_iters,
            dp_polish_iters: hyperparams.dp_polish_iters,
            perturb_polish_iters: hyperparams.perturb_polish_iters,
            final_polish_iters_pre_vnd: hyperparams.final_polish_iters_pre_vnd,
            final_polish_iters_post_vnd: hyperparams.final_polish_iters_post_vnd,
            perturb_removal_divisor: hyperparams.perturb_removal_divisor,
            perturb_min_removal: hyperparams.perturb_min_removal,
            perturb_max_removal: hyperparams.perturb_max_removal,
            swap22_window_limit: hyperparams.swap22_window_limit,
        }
    }
}
impl Default for Params {
    fn default() -> Self {
        Self::from_hyperparameters(&Hyperparameters::default())
    }
}
...
fn integer_core_target(ch: &Challenge, contrib: &[i32], core_half_dp: usize) -> Vec<usize> {
    ...
    // Cap myw to prevent excessive memory allocation that can cause segfaults
    // With core_half_dp=300, core size ~600, this uses: 
    // - dp array: 5M * 8 bytes = 40 MB
    // - choose array: 600 * 5M = 3 GB (manageable)
    const MAX_DP_CAPACITY: usize = 5_000_000;
    let myw = rem_cap.min(MAX_DP_CAPACITY);
...
fn apply_best_swap22_windowed(
    ...
    let wu_len = worst_used.len();
    let bu_len = best_unused.len();
    let limit = params.swap22_window_limit;
    
    for rm1_idx in 0..wu_len.min(limit) {
        let rm1 = worst_used[rm1_idx];
        for rm2_idx in (rm1_idx + 1)..wu_len.min(limit) {
            let rm2 = worst_used[rm2_idx];
            let w_rm_total = state.ch.weights[rm1] as i64 + state.ch.weights[rm2] as i64;
            
            for add1_idx in 0..bu_len.min(limit) {
                let add1 = best_unused[add1_idx];
                for add2_idx in (add1_idx + 1)..bu_len.min(limit) {
                    let add2 = best_unused[add2_idx];
                    let w_add_total = state.ch.weights[add1] as i64 + state.ch.weights[add2] as i64;
                    
                    if w_add_total > w_rm_total + state.slack() {
                        continue;
                    }
                    
                    let delta_rm = (state.contrib[rm1] as i64 + state.contrib[rm2] as i64)
                        - (state.ch.interaction_values[rm1][rm2] as i64);
                    let delta_add = (state.contrib[add1] as i64 + state.contrib[add2] as i64)
                        + (state.ch.interaction_values[add1][add2] as i64);
                    let delta = delta_add - delta_rm 
                        - (state.ch.interaction_values[add1][rm1] as i64)
                        - (state.ch.interaction_values[add1][rm2] as i64)
                        - (state.ch.interaction_values[add2][rm1] as i64)
                        - (state.ch.interaction_values[add2][rm2] as i64);
                    
                    if delta > 0 && best.map_or(true, |(_, _, _, _, bd)| delta > bd) {
                        best = Some((rm1, rm2, add1, add2, delta));
                    }
                }
            }
        }
    ...
    if let Some((rm1, rm2, add1, add2, _)) = best {
        ...
        State::remove_from_vec(best_unused, add1);
        State::remove_from_vec(best_unused, add2);
        worst_used.push(add1);
        worst_used.push(add2);
        true
    } else {
        false
    }
...
fn strategic_perturb_and_rebuild(state: &mut State, params: &Params) {
    ...
    let mut rem = (m / params.perturb_removal_divisor).max(params.perturb_min_removal);
    if rem > params.perturb_max_removal {
        rem = params.perturb_max_removal;
    }
    rem = rem.min(bad.len());
    for i in 0..rem {
        let r = bad[i];
...
fn local_search_vnd(state: &mut State, params: &Params) {
    ...
    loop {
        ...
        if apply_best_swap22_windowed(state, params, &mut best_unused, &mut worst_used) {
            continue;
        }
...
pub fn solve_challenge(
    ...
    let params = Params::from_hyperparameters(&hyperparams);
    ...
    for _ in 0..params.initial_polish_iters {
    ...
    items.sort_unstable();
    let _ = save_solution(&Solution { items: items.clone() });
    
    let mut best_value = state.total_value;

    // Run indefinitely until fuel exhaustion
    // Solutions are saved incrementally whenever improvements are found
    loop {
        ...
        local_search_vnd(&mut state, &params);
        for _ in 0..params.dp_polish_iters {
            polish_once(&mut state, &params);
        }
        ...
        if state.total_value > prev_val {
            if state.total_value > best_value {
                ...
                // Save improved solution immediately
                let mut items = state.selected_items();
                items.sort_unstable();
        ...
        local_search_vnd(&mut state, &params);
        for _ in 0..params.perturb_polish_iters {
            polish_once(&mut state, &params);
        }
        ...
        // Continue to next iteration regardless of improvement
    }
}
