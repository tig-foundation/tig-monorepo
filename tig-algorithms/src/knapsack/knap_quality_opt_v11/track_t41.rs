...
mod inner {
            ...
            if n <= 1200 {
                if budget <= 5 {
                    ...
                    Self {
                        ...
                        perturb_max_frac: 5, ils_vnd_level: 0, bounded_2_2_k: 10,
                        n_full_restarts: 8, use_hub_pair: true,
    ...
    fn run_one_instance(challenge: &Challenge, hp: &Hparams, rng_offset: usize, shared_tsn: Option<&TopNeighbors>, total_interactions: &[i64]) -> (Solution, i64) {
        ...
        let max_stall = if n > 1200 { (hp.ils_rounds / 3).max(10) } else { (hp.ils_rounds * 2 / 3).min(15) };
        let use_light_vnd = n > 1200 && tsn_ref.is_some();
        // L6-ter ADP: adaptive K escape state (memory cfc85fc0, 3b41a9c6)
        let mut adp_k: usize = 8;
        let mut adp_last5_q: i64 = best_val;
        for round in 0..hp.ils_rounds {
            if stall_count >= max_stall { break; }
            // ADP: grow K if no improvement in last 5 rounds, shrink if improved
            if round > 0 && round % 5 == 0 {
                if best_val <= adp_last5_q {
                    adp_k = (adp_k + 2).min(16);
                } else {
                    adp_k = 8usize.max(adp_k.saturating_sub(2));
                }
                adp_last5_q = best_val;
            }
            ...
            if state.total_value <= snap.value {
                ...
                // L6-ter ADP: random escape with adaptive K at stall==10
                if stall_count == 10 {
                    let mut pert = State::new_empty(challenge);
                    for &i in &best_sel { pert.add_item(i); }
                    let mut sel_items: Vec<usize> = (0..n).filter(|&i| pert.selected_bit[i]).collect();
                    // random shuffle then truncate to adp_k (no freq guidance)
                    for j in (1..sel_items.len()).rev() {
                        let ki = rng.next_usize(j + 1);
                        sel_items.swap(j, ki);
                    }
                    sel_items.truncate(adp_k);
                    for &i in &sel_items { pert.remove_item(i); }
                    let mut complement: Vec<usize> = (0..n).filter(|&i| !pert.selected_bit[i]).collect();
                    for j in (1..complement.len()).rev() {
                        let ki = rng.next_usize(j + 1);
                        complement.swap(j, ki);
                    }
                    for &ci in &complement {
                        if pert.total_weight + challenge.weights[ci] <= challenge.max_weight {
                            pert.add_item(ci);
                        }
                    }
                    local_search_vnd_fast(&mut pert);
                    if pert.total_value > best_val {
                        best_val = pert.total_value;
                        best_sel = pert.selected_items();
                        state.restore_solution(&pert.clone_solution());
                        stall_count = 0;
                        adp_k = 8usize.max(adp_k.saturating_sub(2));
                        adp_last5_q = best_val;
                        let h = compute_hash(&state.selected_bit);
                        if tabu_hashes.len() < 128 { tabu_hashes.push(h); }
                        else { tabu_hashes[round % 128] = h; }
                        continue;
                    }
                }
    ...
    // P13 helper: add bits to pool if Hamming distance >= 5 from all existing entries, pool_max=6.
    fn try_add_elite(pool: &mut Vec<Vec<bool>>, bits: &[bool]) {
        let hamming_ok = pool.iter().all(|e| {
            e.iter().zip(bits).filter(|(&a, &b)| a != b).count() >= 5
        });
        if hamming_ok { pool.truncate(5); pool.push(bits.to_vec()); }
    }
        ...
        pub fn solve(
            ...
            if n > 1200 {
                ...
                for restart in 0..n_restarts {
                    let (sol, val) = run_one_instance(challenge, &hp, restart, Some(&tsn), &total_interactions);
            ...
            // QKBP Lagrangian seed (i52 +238Q, O(30×n)). Memory: 023c5998, 83520371.
            {
                let cap = challenge.max_weight;
                let max_val = challenge.values.iter().cloned().max().unwrap_or(1) as f64;
                let min_w = challenge.weights.iter().cloned().min().unwrap_or(1) as f64;
                let mut lam_lo = 0.0f64;
                let mut lam_hi = max_val / min_w.max(1.0);
                for _ in 0..30 {
                    let lam_mid = (lam_lo + lam_hi) * 0.5;
                    let total_w: u64 = (0..n)
                        .filter(|&i| (challenge.values[i] as f64) > lam_mid * (challenge.weights[i] as f64))
                        .map(|i| challenge.weights[i] as u64)
                        .sum();
                    if total_w > cap as u64 { lam_lo = lam_mid; } else { lam_hi = lam_mid; }
                }
                let lam_star = lam_lo;
                let mut scored: Vec<(usize, f64)> = (0..n)
                    .map(|i| (i, challenge.values[i] as f64 - lam_star * (challenge.weights[i] as f64)))
                    .collect();
                scored.sort_unstable_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
                let mut qst = State::new_empty(challenge);
                for &(i, _) in &scored {
                    if qst.total_weight + challenge.weights[i] <= cap { qst.add_item(i); }
                }
                dp_refinement_hp(&mut qst, ch_dp);
                local_search_vnd_windowed(&mut qst, hp.window_k);
                let mut qkbp_best = qst.clone_solution();
                let mut stall = 0usize;
                for round in 0..20 {
                    if stall >= 10 { break; }
                    let snap = qst.clone_solution();
                    let strategy = (round + 4) % 8;
                    let strength = 4 + round / 3;
                    perturb_by_strategy(&mut qst, strength, stall, strategy, &mut rng, &hp, &total_interactions);
                    greedy_reconstruct(&mut qst, strategy % 10, &total_interactions);
                    local_search_vnd_windowed(&mut qst, hp.window_k);
                    dp_refinement_hp(&mut qst, ch_dp);
                    if qst.total_value > qkbp_best.value {
                        qkbp_best = qst.clone_solution();
                        stall = 0;
                    } else {
                        qst.restore_solution(&snap);
                        stall += 1;
                    }
                }
                qst.restore_solution(&qkbp_best);
                elite.push((Solution { items: qst.selected_items() }, qst.total_value));
                elite.sort_by_key(|&(_, v)| std::cmp::Reverse(v));
                elite.truncate(8);
            }
            ...
            // P11 elite_pool: diverse solutions (Hamming>8) collected during phase3 ILS.
            let mut elite_pool: Vec<Vec<bool>> = Vec::new();
            for (sol, _) in &elite {
                let mut bv = vec![false; n];
                for &i in &sol.items { bv[i] = true; }
                try_add_elite(&mut elite_pool, &bv);
            }
            ...
            for restart in 0..n_phase3 {
                ...
                for round in 0..20 {
                    ...
                    if state.total_value > best_snap.value {
                        best_snap = state.clone_solution();
                        try_add_elite(&mut elite_pool, &state.selected_bit);
                        stall = 0;
                    } else {
                        state.restore_solution(&snap);
                        stall += 1;
            ...
            // P11 pool-guided PR: bidirectional PR between best and each diverse guide.
            if elite_pool.len() >= 2 {
                if let Some((best_sol_ref, _)) = elite.first() {
                    let best_sol = best_sol_ref.clone();
                    let mut best_bv = vec![false; n];
                    for &i in &best_sol.items { best_bv[i] = true; }
                    for guide_bits in &elite_pool {
                        let hamming: usize = best_bv.iter().zip(guide_bits.iter())
                            .filter(|(&a, &b)| a != b).count();
                        if hamming < 4 { continue; }
                        let guide_sol = Solution { items: (0..n).filter(|&i| guide_bits[i]).collect() };
                        let sol_ab = path_relink(challenge, &best_sol, &guide_sol, &hp);
                        let val_ab = eval_solution(challenge, &sol_ab);
                        elite.push((sol_ab, val_ab));
                        let sol_ba = path_relink(challenge, &guide_sol, &best_sol, &hp);
                        let val_ba = eval_solution(challenge, &sol_ba);
                        elite.push((sol_ba, val_ba));
                    }
                    elite.sort_by_key(|&(_, v)| std::cmp::Reverse(v));
