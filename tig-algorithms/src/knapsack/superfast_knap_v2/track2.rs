...
mod inner {
    ...
    /* =========================================================
     * SECTION 5: HPF parametric min-cut
     * ========================================================= */
            ...
            unsafe {
                ...
                let sets: Vec<(i32, Vec<usize>)> = positions.iter().map(|&pos| {
                    let nodes: Vec<usize> = pos_items.iter()
                        .filter(|&&(p,_)| p == pos).map(|&(_,item)| item).collect();
                    (pos, nodes)
                }).collect();
    ...
    /* =========================================================
     * SECTION 7: run_greedy dispatcher
     * ========================================================= */
    ...
    fn run_greedy(inst: &P1Instance, beta: f64,
        ...
        for bi in 0..n_budgets {
            ...
            let res = run_greedy_left(&um, n_nodes, left_init, &all_nodes,
                                      budget, beta, &inst.weights, Some(&mut ws_left));
            left_nodes.push(res);
        }
        ...
        for bi in (0..n_budgets).rev() {
            ...
            let after_right = run_greedy_right(&um, n_nodes, selected_right.clone(),
                                                budget, beta, &inst.weights, Some(&mut ws_right));
    ...
    /* =========================================================
     * SECTION 11: Bridge Phase-1 -> Phase-2
     * ========================================================= */
    ...
    fn build_best_unused(s: &P2State, max_k: usize) -> Vec<ItemScore> {
        let n = s.ins.n;
        let mut arr: Vec<ItemScore> = (0..n).filter(|&i| s.sel[i] == 0).map(|i| {
    ...
    /* =========================================================
     * SECTION 14: Local search moves (no fuel)
     * ========================================================= */
    ...
    fn apply_best_swap11(s: &mut P2State, window_k: usize) -> bool {
        ...
        for x in 0..cs {
            let rm = used[x].id; let budget = s.slack() + s.ins.w[rm];
            for y in 0..cu {
                let add = unused[y].id;
    ...
    fn apply_best_swap21(s: &mut P2State, window_k: usize) -> bool {
        ...
        for ai in 0..cu {
            let add = unused[ai].id; let cadd = s.contrib[add] as Ll;
            for x in 0..cs {
                let r1 = used[x].id;
                for y in (x+1)..cs {
                    let r2 = used[y].id;
        ...
        for x in 0..cs {
            ...
            for y in (x+1)..cs {
                ...
                let lost   = s.contrib[r1] as Ll + s.contrib[r2] as Ll
                             - s.ins.q_get(r1, r2) as Ll;
                for a in 0..cu {
                    let p  = unused[a].id; let wp = s.ins.w[p];
    ...
    /* =========================================================
     * SECTION 16: Perturbation & greedy reconstruction
     * ========================================================= */
    ...
    /* =========================================================
     * SECTION 17: Hybrid ILS solver (track2 tuning, no fuel)
     * ========================================================= */
    ...
    fn solve_hybrid(s:          &mut P2State,
        ...
        while round < ils_rounds {
            ...
            let mut iter = 0;
            // track2 strong perturbation when stuck (stall >= 40)
            if stall >= 45{
                s.restore_best();
    ...
    /* =========================================================
     * SECTION 18: Main Solver entry point
     * ========================================================= */
