...
// ===================== i9: Lagrangian core-reduction + exact component refinement =====================
// User directive 19/07 (PUSH HAUT): "core-reduction lagrangienne + refinement exact par
// composantes". lambda = critical density at the incumbent (Dantzig break on contrib/w
// ordering). Reduced cost r_i = contrib[i] - lambda*w_i. Items with |r_i| small form the
// frontier (undecided under the relaxation); everything else is locked as-is. The frontier
// is then refined by one of 3 family-distinct mechanisms (VarA/B/C), each downside-bounded.

/// Critical density lambda at the incumbent: density of the first item rejected by a
/// greedy fill over the contrib/weight ordering (same break used by dp_refinement_hp).
fn lagr_lambda_break(state: &State) -> f64 {
    let n = state.ch.num_items;
    let contrib = &state.contrib;
    let weights = &state.ch.weights;
    let mut by_density: Vec<usize> = (0..n).collect();
    ...
    let mut rem = state.ch.max_weight;
    for &i in &by_density {
        let w = weights[i];
        if w <= rem { rem -= w; }
        else {
            let wd = (weights[i] as f64).max(1.0);
            return (contrib[i] as f64) / wd;
        }
    }
    0.0
}

/// Frontier = the `cap_f` items with smallest |contrib[i] - lambda*w_i| (most undecided
/// under the Lagrangian relaxation at the incumbent). Returns indices, |r| ascending.
fn lagr_frontier_items(state: &State, lambda: f64, cap_f: usize) -> Vec<usize> {
    let n = state.ch.num_items;
    let mut scored: Vec<(usize, i64)> = (0..n).map(|i| {
        let r = state.contrib[i] as f64 - lambda * state.ch.weights[i] as f64;
        // scale to keep ordering deterministic in integer space
        (i, (r.abs() * 1024.0) as i64)
    }).collect();
    scored.sort_unstable_by_key(|&(i, s)| (s, i));
    scored.truncate(cap_f);
    scored.into_iter().map(|(i, _)| i).collect()
}

/// VarA — exact refinement by components: partition the frontier into connectivity blocks
/// (BFS over TSN top-k positive-interaction edges, block size <= max_block), then solve each
/// block EXACTLY by Gray-code subset enumeration of the true quadratic objective (linear
/// coupling to the locked selection frozen, pairwise terms inside the block exact), under
/// the residual capacity. Applies a block's best subset only if strictly improving.
/// Returns true if any improvement was applied.
fn lagr_component_exact(state: &mut State, tsn: &TopNeighbors, lambda: f64, cap_f: usize, max_block: usize) -> bool {
    let frontier = lagr_frontier_items(state, lambda, cap_f);
    if frontier.is_empty() { return false; }
    let n = state.ch.num_items;
    let mut in_frontier = vec![false; n];
    for &i in &frontier { in_frontier[i] = true; }

    // Connectivity blocks over the frontier via TSN edges (deterministic BFS order).
    let mut visited = vec![false; n];
    let mut blocks: Vec<Vec<usize>> = Vec::new();
    let kmax = max_block.clamp(4, 20);
    for &start in &frontier {
        if visited[start] { continue; }
        let mut block = Vec::with_capacity(kmax);
        let mut queue = std::collections::VecDeque::new();
        queue.push_back(start);
        visited[start] = true;
        while let Some(u) = queue.pop_front() {
            block.push(u);
            if block.len() >= kmax { break; }
            for &v in &tsn.friends[u] {
                if in_frontier[v] && !visited[v] {
                    visited[v] = true;
                    queue.push_back(v);
                    if block.len() + queue.len() >= kmax { break; }
                }
            }
        }
        // drain any queued-but-unplaced items into the block up to kmax
        while block.len() < kmax {
            if let Some(u) = queue.pop_front() { block.push(u); } else { break; }
        }
        blocks.push(block);
    }
    ...
    for block in &blocks {
        ...
        // Residual capacity once the block's currently-selected weight is released.
        let mut block_sel_weight: u64 = 0;
        for &i in block { if state.selected_bit[i] { block_sel_weight += state.ch.weights[i] as u64; } }
        let residual = (state.ch.max_weight as u64) - (state.total_weight as u64) + block_sel_weight;

        // base_i = contribution of i w.r.t. the OUTSIDE-block selection (freeze coupling).
        // contrib[i] already includes interactions with selected block members -> subtract them.
        let mut base = vec![0i64; k];
        let mut pair = vec![0i64; k * k];
        for a in 0..k {
            let ia = block[a];
            let row = &state.ch.interaction_values[ia];
            let mut b_val = state.contrib[ia] as i64;
            for b in 0..k {
                if a == b { continue; }
                let ib = block[b];
                let v = row[ib] as i64;
                pair[a * k + b] = v;
                if state.selected_bit[ib] { b_val -= v; }
            }
            base[a] = b_val;
        }
        let w_of = |a: usize| state.ch.weights[block[a]] as u64;

        // Current block subset value (exact, same frozen-coupling accounting).
        let mut cur_mask: u32 = 0;
        for a in 0..k { if state.selected_bit[block[a]] { cur_mask |= 1 << a; } }
        let eval_mask = |mask: u32| -> (i64, u64) {
            let mut v = 0i64;
            let mut w = 0u64;
            for a in 0..k {
                ...
                w += w_of(a);
                for b in (a + 1)..k {
                    if mask & (1 << b) != 0 { v += pair[a * k + b]; }
                }
            }
            (v, w)
        };
        let (cur_val, _) = eval_mask(cur_mask);

        // Gray-code exhaustive enumeration: one bit flips per step, O(k) incremental delta.
        let mut best_mask = cur_mask;
        let mut best_val = cur_val;
        let mut g_prev: u32 = 0;
        let mut val: i64 = 0; // value of empty set
        let mut wgt: u64 = 0;
        let total: u64 = 1u64 << k;
        for g in 1..total {
            let gray = (g ^ (g >> 1)) as u32;
            let flipped = gray ^ g_prev;
            let a = flipped.trailing_zeros() as usize;
            let now_in = gray & flipped != 0;
            // delta of flipping bit a against the PREVIOUS subset (g_prev)
            let mut delta = base[a];
            for b in 0..k {
                if b != a && g_prev & (1 << b) != 0 { delta += pair[a * k + b]; }
            }
            if now_in { val += delta; wgt += w_of(a); } else { val -= delta; wgt -= w_of(a); }
            g_prev = gray;
            if wgt <= residual && val > best_val {
                best_val = val;
                best_mask = gray;
            }
        }

        if best_mask != cur_mask && best_val > cur_val {
            // apply flips through State ops (keeps contrib/total exact for later blocks);
            // removals first so capacity is freed before additions
            for a in 0..k {
                let i = block[a];
                if best_mask & (1 << a) == 0 && state.selected_bit[i] { state.remove_item(i); }
            }
            for a in 0..k {
                let i = block[a];
                if best_mask & (1 << a) != 0 && !state.selected_bit[i] { state.add_item(i); }
            }
            improved = true;
        }
    }
...
/// VarB — reduced-cost wide-core DP: same skeleton as dp_refinement_hp but the ordering is
/// by Lagrangian reduced cost (contrib - lambda*w, descending) instead of raw density, and
/// the core window is wide. Linearized (frozen contrib) like dp_refinement_hp.
fn lagr_core_dp_deep(state: &mut State, lambda: f64, core_half: usize) {
    let n = state.ch.num_items;
    let cap = state.ch.max_weight;
    let weights = &state.ch.weights;

    let mut by_rc: Vec<usize> = (0..n).collect();
    by_rc.sort_unstable_by(|&a, &b| {
        let ra = ((state.contrib[a] as f64 - lambda * weights[a] as f64) * 1024.0) as i64;
        let rb = ((state.contrib[b] as f64 - lambda * weights[b] as f64) * 1024.0) as i64;
        rb.cmp(&ra).then(a.cmp(&b))
    });
    // zero-crossing of the reduced cost = Lagrangian break point
    let mut cross = n;
    for (idx, &i) in by_rc.iter().enumerate() {
        let r = state.contrib[i] as f64 - lambda * weights[i] as f64;
        if r <= 0.0 { cross = idx; break; }
    }
    let left = cross.saturating_sub(core_half);
    let right = (cross + core_half).min(n);
    let locked: Vec<usize> = by_rc[..left].to_vec();
    let core: Vec<usize> = by_rc[left..right].to_vec();

    let used_locked: u64 = locked.iter().map(|&i| weights[i] as u64).sum();
    if used_locked > cap as u64 { return; }
    let rem_cap = (cap as u64 - used_locked) as usize;
    let myk = core.len();
    if myk == 0 || rem_cap == 0 { return; }

    let total_core_weight: usize = core.iter().map(|&i| weights[i] as usize).sum();
    let myw = rem_cap.min(total_core_weight);
    let dp_size = myw + 1;
    let choose_size = myk * dp_size;
    if state.dp_cache.len() < dp_size { state.dp_cache.resize(dp_size, i64::MIN / 4); }
...
/// VarC — lambda-reseed: rebuild a solution from scratch by reduced-cost ordering computed
/// AT the incumbent (its contribs), greedily add while it fits, then polish (core DP +
/// TSN VND fixpoint). A construction-family probe of the Lagrangian signal.
fn lagr_reseed_polish(challenge: &Challenge, incumbent: &SolState, lambda: f64, hp: &Hparams, tsn: &TopNeighbors) -> SolState {
    let n = challenge.num_items;
    let mut order: Vec<usize> = (0..n).collect();
    order.sort_unstable_by(|&a, &b| {
        let ra = ((incumbent.contrib[a] as f64 - lambda * challenge.weights[a] as f64) * 1024.0) as i64;
        let rb = ((incumbent.contrib[b] as f64 - lambda * challenge.weights[b] as f64) * 1024.0) as i64;
        rb.cmp(&ra).then(a.cmp(&b))
    });
    let mut st = State::new_empty(challenge);
    for &i in &order {
        let r = incumbent.contrib[i] as f64 - lambda * challenge.weights[i] as f64;
        if r <= 0.0 { break; }
        if challenge.weights[i] <= st.slack() { st.add_item(i); }
    }
    dp_refinement_hp(&mut st, hp.core_half_dp);
    loop {
        let v_before = st.total_value;
        local_search_vnd_tsn_light(&mut st, tsn);
        dp_refinement_hp(&mut st, hp.core_half_dp);
        if st.total_value <= v_before { break; }
    }
    st.clone_solution()
}

/// Dispatcher: downside-bounded Lagrangian polish of the global best (n>1200 branch).
/// Returns the (possibly improved) solution and its value; never worse than the input.
fn lagr_polish_best(challenge: &Challenge, best_sol: &Solution, best_val: i64, hp: &Hparams, tsn: &TopNeighbors) -> (Solution, i64) {
    let mut state = State::new_empty(challenge);
    for &i in &best_sol.items { state.add_item(i); }

    match hp.lagr_mode {
        1 => {
            // VarA: exact block enumeration, lambda sweep, fixpoint (bounded rounds)
            for _round in 0..4 {
                let lam0 = lagr_lambda_break(&state);
                let mut any = false;
                for &mul in &[1.0f64, 0.85, 1.2] {
                    if lagr_component_exact(&mut state, tsn, lam0 * mul, hp.lagr_frontier, hp.lagr_block) {
                        any = true;
                    }
                }
                if !any { break; }
                // re-polish after structural flips
                loop {
                    let v_before = state.total_value;
                    local_search_vnd_tsn_light(&mut state, tsn);
        ...
        2 => {
            // VarB: reduced-cost wide-core DP, lambda sweep, fixpoint (bounded rounds)
            for _round in 0..4 {
                let v_start = state.total_value;
                let lam0 = lagr_lambda_break(&state);
                for &mul in &[1.0f64, 0.85, 1.2] {
                    let snap = state.clone_solution();
                    let v0 = state.total_value;
                    lagr_core_dp_deep(&mut state, lam0 * mul, 150);
                    if state.total_value < v0 { state.restore_solution(&snap); }
                }
                loop {
                    let v_before = state.total_value;
                    local_search_vnd_tsn_light(&mut state, tsn);
                ...
                if state.total_value <= v_start { break; }
            }
        }
        3 => {
            // VarC: lambda-reseed constructions, keep the best
            let incumbent = state.clone_solution();
            let lam0 = lagr_lambda_break(&state);
            let mut best_re = incumbent.clone();
            for &mul in &[1.0f64, 0.8, 1.25] {
                let cand = lagr_reseed_polish(challenge, &incumbent, lam0 * mul, hp, tsn);
                if cand.value > best_re.value { best_re = cand; }
            }
            if best_re.value > state.total_value { state.restore_solution(&best_re); }
        }
        _ => {}
    }

    if state.total_value > best_val {
        let v = state.total_value;
        (Solution { items: state.selected_items() }, v)
    } else {
        (Solution { items: best_sol.items.clone() }, best_val)
    }
}
// ===================== end i9 Lagrangian refinement =====================

// =====================================================================================
// i11 — POST-RESTART EXACT INTENSIFICATION (strategy=combine, transplant intra-algo)
//
// WHY: i10 proved 156561 is a plateau of MEDIAN, not of quality. bundle_quality is the
// median of 32 nonces = (155075 + 158048)/2. Escape restarts lifted ranks 13/19/24/25 with
// ZERO regressions, yet the median pair stayed BIT-IDENTICAL up to 118s of effort => every
// DIVERSIFICATION family is structurally invisible to the metric. The only median movers
// are the four nonces clustered at 153526 / 153974 / 154036 / 155075 — a span of just 1549
// points. Lifting that cluster by ~1600 moves the median by ~+800 (+0.51%).
//
// WHAT: the n>1200 branch is BARE — n_full_restarts restarts, argmax, return. It throws
// away 13 of its 14 converged local optima. The elite pool / path-relink / exact
// consensus-DP stage that would exploit them ALREADY EXISTS in this file but is gated to
// n<=1200. i11 transplants three DISTINCT exact-intensification families onto t42:
//   VarA kernel-exact DP over the disputed residual (consensus reduction)
//   VarB path-relinking between elites          (trajectory recombination)
//   VarC deep 2-out-2-in fixpoint                (exact wide neighbourhood)
// None is diversification, so none is covered by the median-blind proof, and none appears
// in track_dead_families for c003/t42.
//
// MONOTONE BY CONSTRUCTION: each returns Some(..) only on a STRICT improvement over the
// incumbent, so per-nonce Q >= baseline is guaranteed. The only risk is TIME (margin =
// 105030 - 89530 = 15.5s). Budgets below are WORK counts (rounds, item caps), never
// wall-clock — the runtime must stay deterministic.
// =====================================================================================

/// Rebuild a State from an item list, adding only what fits. Feasibility is never assumed.
fn state_from_items<'a>(challenge: &'a Challenge, items: &[usize]) -> State<'a> {
    let mut st = State::new_empty(challenge);
    for &i in items {
...
/// TSN + core-DP polish to fixpoint, bounded by `rounds` passes.
/// NOTE: `dp_refinement_hp` re-selects from LINEARIZED contributions and can therefore
/// LOWER the true quadratic value. So we snapshot the best seen and restore it at the end
/// — never return the current state.
fn intens_polish(state: &mut State, hp: &Hparams, tsn: &TopNeighbors, rounds: usize) {
    let mut best = state.clone_solution();
    for _ in 0..rounds {
        let before = state.total_value;
        local_search_vnd_tsn(state, tsn);
        dp_refinement_hp(state, hp.core_half_dp);
        if state.total_value > best.value { best = state.clone_solution(); }
        if state.total_value <= before { break; }
    }
    if state.total_value < best.value { state.restore_solution(&best); }
}

/// VarA `intens:elite_consensus_kernel_dp`.
/// Freeze every item that all top-k restarts agree on, then solve the disputed residual
/// EXACTLY by dynamic programming under the leftover capacity, then polish. This is the
/// Pisinger-style core reduction the bank recommends (mem b7dc5f5c / 122c1281): the elites
/// supply the consensus, the DP supplies optimality on what is left.
fn intens_consensus_kernel_dp(
    challenge: &Challenge,
    hp: &Hparams,
    tsn: &TopNeighbors,
    elite: &[(Solution, i64)],
    ...
    let n = challenge.num_items;
    let tk = elite.len().min(hp.intens_elite_k.max(2));
    if tk < 2 { return None; }

    let mut item_freq = vec![0u32; n];
    for (sol, _) in &elite[..tk] {
        for &i in &sol.items { item_freq[i] += 1; }
    }
    let tkc = tk as u32;

    // Frozen consensus core. Intersection of feasible sets => always fits.
    let always_in: Vec<usize> = (0..n).filter(|&i| item_freq[i] == tkc).collect();
    let mut state = state_from_items(challenge, &always_in);
    let rem_cap = (challenge.max_weight - state.total_weight) as usize;
    if rem_cap == 0 { return None; }

    // Disputed items ranked by contribution GIVEN the frozen core.
    let mut disputed: Vec<usize> = (0..n)
        .filter(|&i| item_freq[i] > 0 && item_freq[i] < tkc && !state.selected_bit[i])
        .collect();
    disputed.sort_unstable_by_key(|&i| std::cmp::Reverse(state.contrib[i]));
    disputed.truncate(hp.intens_disputed_cap.max(1));
    if disputed.is_empty() { return None; }

    // Bound the backtrack table (work budget). Halve the disputed set until it fits.
    const MAX_CHOOSE: usize = 48_000_000;
    let mut myw = rem_cap.min(disputed.iter().map(|&i| challenge.weights[i] as usize).sum());
    let mut dp_size = myw + 1;
    while disputed.len() > 1 && disputed.len().saturating_mul(dp_size) > MAX_CHOOSE {
        disputed.truncate(disputed.len() / 2);
        myw = rem_cap.min(disputed.iter().map(|&i| challenge.weights[i] as usize).sum());
        dp_size = myw + 1;
    }
    let dk = disputed.len();
    if myw == 0 || dk.saturating_mul(dp_size) > MAX_CHOOSE {
        eprintln!("T42_INTENS varA SKIP dk={} dp_size={} rem_cap={}", dk, dp_size, rem_cap);
        return None;
    }
    eprintln!("T42_INTENS varA dk={} dp_size={} rem_cap={} always_in={}", dk, dp_size, rem_cap, always_in.len());
    ...
    // Backtrack. Weights come from the challenge (immutable); contribs are NOT re-read here
    // because add_item mutates them — the DP already priced the whole batch.
    ...
    // The DP is linearized (it ignores pairwise profits AMONG disputed items), so the polish
    // is what recovers the quadratic structure.
    intens_polish(&mut state, hp, tsn, 6);

    if state.total_weight <= challenge.max_weight && state.total_value > inc_val {
        Some((Solution { items: state.selected_items() }, state.total_value))
    } else {
        None
    }
}

/// VarB `intens:elite_path_relink`.
/// Trajectory recombination: walk from one elite to another, harvesting the best point on
/// the path. Reaches solutions no restart can reach because it crosses the space BETWEEN
/// two converged optima rather than resampling a new basin.
fn intens_elite_path_relink(
    challenge: &Challenge,
    hp: &Hparams,
    tsn: &TopNeighbors,
    elite: &[(Solution, i64)],
    ...
    let k = elite.len().min(hp.intens_relink_k.max(2));
    if k < 2 { return None; }
    let mut best_val = inc_val;
    let mut best_items: Option<Vec<usize>> = None;
    let mut relinks = 0usize;
    for a in 0..k {
        for b in (a + 1)..k {
            for &(x, y) in &[(a, b), (b, a)] {
                let sol = path_relink(challenge, &elite[x].0, &elite[y].0, hp);
                let mut st = state_from_items(challenge, &sol.items);
                intens_polish(&mut st, hp, tsn, 4);
                relinks += 1;
                if st.total_weight <= challenge.max_weight && st.total_value > best_val {
                    best_val = st.total_value;
                    best_items = Some(st.selected_items());
                }
            }
        }
    }
    eprintln!("T42_INTENS varB relinks={} improved={}", relinks, best_items.is_some());
    best_items.map(|it| (Solution { items: it }, best_val))
}

/// VarC `intens:deep_exact_n2`.
/// The in-loop VND only ever explores 2-out-2-in on a frontier of bounded_2_2_k=22. This
/// drives the incumbent to a TRUE 2-2 fixpoint on a much wider frontier (intens_n2_k),
/// alternating with the TSN neighbourhood. `apply_swap_2_2_bounded` only applies strictly
/// improving moves, so this is monotone; the best snapshot guards the DP inside the polish.
fn intens_deep_exact_n2(
    challenge: &Challenge,
    hp: &Hparams,
    tsn: &TopNeighbors,
    incumbent: &Solution,
    ...
    let k = hp.intens_n2_k.max(hp.bounded_2_2_k);
    let mut st = state_from_items(challenge, &incumbent.items);
    let mut best = st.clone_solution();
    let mut moves = 0usize;
    for _ in 0..24 {
        let before = st.total_value;
        local_search_vnd_tsn(&mut st, tsn);
        dp_refinement_hp(&mut st, hp.core_half_dp);
        if st.total_value > best.value { best = st.clone_solution(); }
        if !apply_swap_2_2_bounded(&mut st, k) { break; }
        moves += 1;
        if st.total_value > best.value { best = st.clone_solution(); }
        if st.total_value <= before { break; }
    }
    eprintln!("T42_INTENS varC n2_k={} moves={} gain={}", k, moves, best.value - inc_val);
    if best.value > inc_val && best.weight <= challenge.max_weight {
        let mut fs = State::new_empty(challenge);
        fs.restore_solution(&best);
        return Some((Solution { items: fs.selected_items() }, best.value));
    }
    None
}
...
struct Hparams {
    ...
    window_k: usize,
    core_half_dp: usize,
    // i9 Lagrangian core-reduction + exact component refinement (user directive 19/07).
    // lagr_mode: 0=OFF (bit-exact champion path), 1=VarA exact block enumeration on
    // Lagrangian frontier, 2=VarB reduced-cost wide-core DP lambda-sweep, 3=VarC
    // lambda-reseed reconstruction+polish. All downside-bounded on the global best.
    lagr_mode: usize,
    lagr_frontier: usize,
    lagr_block: usize,
    // i10: number of I2PLS anti-freq escape restarts appended after the main restart loop.
    // 0 = OFF = byte-exact champion path (code-path bypass, not an identity value).
    escape_restarts: usize,
    // i11 POST-RESTART EXACT INTENSIFICATION (combine, transplant intra-algo).
    // The n>1200 branch is BARE: n_full_restarts restarts + argmax, then return. The whole
    // elite / path-relink / exact-consensus-DP stage that already exists in this very file
    // is n<=1200-ONLY. i11 transplants it onto the t42 branch behind one flag.
    // intens_mode: 0=OFF (byte-exact champion path, zero cost, elite pool never built)
    //   1 = VarA `intens:elite_consensus_kernel_dp` — freeze the items every top-k restart
    //       agrees on, solve the disputed residual EXACTLY by DP under the leftover
    //       capacity, then TSN-polish to fixpoint.
    //   2 = VarB `intens:elite_path_relink` — trajectory recombination between the top
    //       elites (both directions), TSN-polished.
    //   3 = VarC `intens:deep_exact_n2` — drive the incumbent to a true 2-out-2-in fixpoint
    //       on a frontier far wider than the in-loop bounded_2_2_k=22.
    // ALL THREE ARE MONOTONE BY CONSTRUCTION: they only ever replace the incumbent on a
    // STRICT improvement, so Q >= baseline per nonce. The only risk carried is TIME.
    intens_mode: usize,
    intens_elite_k: usize,
    intens_disputed_cap: usize,
    intens_relink_k: usize,
    intens_n2_k: usize,
}
...
impl Hparams {
        ...
        if n <= 1200 {
            if budget <= 5 {
                Self {
                    ...
                    lagr_mode: 0, lagr_frontier: 160, lagr_block: 16,
                    escape_restarts: 0,
                    intens_mode: 0, intens_elite_k: 6, intens_disputed_cap: 240,
                    intens_relink_k: 3, intens_n2_k: 72,
                }
            } else if budget <= 10 {
                Self {
                    n_random_starts: 3, n_crossover_gen: 8, sa_rounds: 0,
            ...
            // Confirmed on our own binary ver4142 with this exact config via hp_json:
            // bundle_quality=156535 @105s, 32/32, 0 invalid (job 23795, m13) = ties SOTA
            // weak default (n_full_restarts:16, ils_rounds:50, window_k:180 -> 156120);
            // 156535 is an HP config on the SAME solver, not missing code.
            // use_hub_pair/use_heavy_polish are NOT in the winning hp_json -> kept false
            // (matches the confirmed 156535 bench, which left them at default).
            Self {
                n_random_starts: 2, n_crossover_gen: 14, sa_rounds: 1,
                sa_iter: 0, n_sa_members: 1, ils_rounds: 110,
                ils_restart_interval: 16, perturb_base_frac: 7,
                perturb_max_frac: 6, ils_vnd_level: 1, bounded_2_2_k: 22,
                n_full_restarts: 14, use_hub_pair: false,
                use_heavy_polish: false, window_k: 260, core_half_dp: 39,
                lagr_mode: 0, lagr_frontier: 160, lagr_block: 16,
                    escape_restarts: 0,
                    intens_mode: 3, intens_elite_k: 6, intens_disputed_cap: 240,
                    intens_relink_k: 3, intens_n2_k: 72,
            }
        }
    }
    ...
    fn from_map(h: &Option<Map<String, Value>>, n: usize, budget: u32) -> Self {
        ...
        if let Some(m) = h {
            ...
            if let Some(v) = m.get("lagr_mode").and_then(|v| v.as_u64()) { p.lagr_mode = v as usize; }
            if let Some(v) = m.get("lagr_frontier").and_then(|v| v.as_u64()) { p.lagr_frontier = v as usize; }
            if let Some(v) = m.get("lagr_block").and_then(|v| v.as_u64()) { p.lagr_block = v as usize; }
            if let Some(v) = m.get("escape_restarts").and_then(|v| v.as_u64()) { p.escape_restarts = v as usize; }
            if let Some(v) = m.get("intens_mode").and_then(|v| v.as_u64()) { p.intens_mode = v as usize; }
            if let Some(v) = m.get("intens_elite_k").and_then(|v| v.as_u64()) { p.intens_elite_k = v as usize; }
            if let Some(v) = m.get("intens_disputed_cap").and_then(|v| v.as_u64()) { p.intens_disputed_cap = v as usize; }
            if let Some(v) = m.get("intens_relink_k").and_then(|v| v.as_u64()) { p.intens_relink_k = v as usize; }
            if let Some(v) = m.get("intens_n2_k").and_then(|v| v.as_u64()) { p.intens_n2_k = v as usize; }
        }
        p
    }
}
...
// I2PLS anti-freq: greedy start penalizing high-frequency items to escape the Q=156561 basin.
// Ported from t42/i9 (donor mechanism). i9 measured 4 nonces improved / 0 degraded, but on a
// STALE n_full_restarts=21 base (root code) instead of the i7 champion base (14) -> its 119.03s
// blew the 105030ms gate. Here the same mechanism rides the champion base.
fn build_anti_freq_start(state: &mut State, global_freq: &[u32], total_interactions: &[i64], variant: usize) {
    let n = state.ch.num_items;
    let max_f = global_freq.iter().copied().max().unwrap_or(1).max(1) as f64;
    let alpha = if variant == 0 { 2.0f64 } else { 4.0f64 };
    let mut order: Vec<usize> = (0..n).collect();
    order.sort_unstable_by(|&a, &b| {
        let wa = (state.ch.weights[a] as f64).max(1.0);
        let wb = (state.ch.weights[b] as f64).max(1.0);
        let fa = 1.0 + (global_freq[a] as f64 / max_f) * alpha;
        let fb = 1.0 + (global_freq[b] as f64 / max_f) * alpha;
        let synergy_a = (total_interactions[a] as f64 * 0.05).max(0.0);
        let synergy_b = (total_interactions[b] as f64 * 0.05).max(0.0);
        let sa = (state.ch.values[a] as f64 + synergy_a) / wa / fa;
        let sb = (state.ch.values[b] as f64 + synergy_b) / wb / fb;
...
impl Solver {
    pub fn solve(
        ...
        if n > 1200 {
            ...
            let mut last_improving_restart: usize = 0;
            let mut restarts_done: usize = 0;
            // i10: long-term frequency memory over all restarts. Pure counters — they never
            // feed back into the champion path, so escape_restarts=0 stays byte-exact.
            let mut global_freq = vec![0u32; n];
            // i11: elite pool of every converged restart. The champion path throws 13 of its
            // 14 local optima away. Guarded by intens_mode>0 so the sentinel stays byte-exact
            // and pays nothing.
            ...
            for restart in 0..n_restarts {
                ...
                if hp.intens_mode > 0 {
                    elite.push((Solution { items: sol.items.clone() }, val));
                }
                for &i in &sol.items { global_freq[i] = global_freq[i].saturating_add(1); }
                if val > best_quality {
                    best_quality = val;
                    best_sol = Some(sol);
                    restart_stall = 0;
                    last_improving_restart = restart;
                } else {
                    restart_stall += 1;
                    if restart_stall >= 5 { break; }
                }
            }
            eprintln!("T42_PROBE restarts_done={} last_improving={}", restarts_done, last_improving_restart);
            // i10 ESCAPE: I2PLS frequency-guided inter-basin escape. Extra restarts seeded from
            // anti-freq biased starts (items under-represented across all restarts). Strict
            // acceptance — an escape only replaces the incumbent if it strictly improves, so the
            // downside is bounded to time. escape_restarts=0 => loop never runs => byte-exact.
            for fe in 0..hp.escape_restarts {
                let mut fe_state = State::new_empty(challenge);
                build_anti_freq_start(&mut fe_state, &global_freq, &total_interactions, fe);
                let fe_seed = Solution { items: fe_state.selected_items() };
                let (sol, val) = run_one_instance_seeded(challenge, &hp, 200 + fe, Some(&tsn), &total_interactions, Some(&fe_seed));
                for &i in &sol.items { global_freq[i] = global_freq[i].saturating_add(1); }
                if val > best_quality {
                    best_quality = val;
                    best_sol = Some(sol);
            ...
            // i9: Lagrangian core-reduction + exact component refinement on the global
            // best (downside-bounded; lagr_mode=0 -> bit-exact champion path).
            if hp.lagr_mode > 0 {
                if let Some(sol) = best_sol.take() {
                    let (s2, _v2) = lagr_polish_best(challenge, &sol, best_quality, &hp, &tsn);
                    best_sol = Some(s2);
                }
            }
            // i11: post-restart EXACT intensification on the elite pool. Strictly monotone —
            // the incumbent is only replaced on a strict improvement, so per-nonce Q can only
            // go up. intens_mode=0 skips the whole block (byte-exact champion path).
            // i13: mode=4 = combine mode3 (deep_exact_n2, fixes rang16) then mode=2
            // (path_relink k=intens_relink_k, targets rang14) on the mode3-improved incumbent.
            if hp.intens_mode > 0 {
                if hp.intens_mode == 4 {
                    // Phase 1: mode=3 (deep_exact_n2) — drives rang16 to 2-2 fixpoint
                    if let Some(inc) = best_sol.as_ref() {
                        let m3 = intens_deep_exact_n2(challenge, &hp, &tsn, inc, best_quality);
                        if let Some((s3, v3)) = m3 {
                            if v3 > best_quality {
                                eprintln!("T42_INTENS mode4 phase3 ACCEPT {} -> {} (+{})", best_quality, v3, v3 - best_quality);
                                best_quality = v3;
                                elite.push((s3.clone(), v3));
                                best_sol = Some(s3);
                            }
                        }
                    }
                    // Phase 2: mode=2 (path_relink) — targets rang14 on updated elite/incumbent
                    let m2 = intens_elite_path_relink(challenge, &hp, &tsn, &elite, best_quality);
                    if let Some((s2, v2)) = m2 {
                        if v2 > best_quality {
                            eprintln!("T42_INTENS mode4 phase2 ACCEPT {} -> {} (+{})", best_quality, v2, v2 - best_quality);
                            best_quality = v2;
                            best_sol = Some(s2);
                        }
                    }
                } else if let Some(inc) = best_sol.as_ref() {
                    let improved = match hp.intens_mode {
                        1 => intens_consensus_kernel_dp(challenge, &hp, &tsn, &elite, best_quality),
                        2 => intens_elite_path_relink(challenge, &hp, &tsn, &elite, best_quality),
                        _ => intens_deep_exact_n2(challenge, &hp, &tsn, inc, best_quality),
                    };
                    if let Some((s2, v2)) = improved {
                        if v2 > best_quality {
                            eprintln!("T42_INTENS mode={} ACCEPT {} -> {} (+{})", hp.intens_mode, best_quality, v2, v2 - best_quality);
                            best_quality = v2;
                            best_sol = Some(s2);
                        }
                    }
...
// --- V2 refacto interface shim (iter t42/i4) ---
// `solve_challenge`. Delegate 1:1 (no behavior change, pure rename bridge).
#[inline(always)]
pub fn solve(
    challenge: &Challenge,
