...
// Greedy passes, left and right
    ...
    fn perturb_by_strategy(state: &mut State, strength: usize, stall_count: usize, strategy: usize, rng: &mut Rng, hp: &Hparams) {
        ...
        match strategy {
            ...
            6 => {
                let seed = selected[rng.next_usize(selected.len())];
                removal_candidates = selected.iter().map(|&i| {
                    if i == seed {
                        (i, i64::MIN)
                    } else {
                        (i, -(state.ch.interaction_values[i][seed] as i64))
                    }
                }).collect();
                removal_candidates.sort_unstable_by_key(|&(_, s)| s);
            },
    ...
    struct Hparams {
        ...
        window_k: usize,
        core_half_dp: usize,
        // i3 SEED HPF/QKBP : nombre de valeurs de lambda du max-flow parametrique
        n_lambda_values: usize,
        // i3 : 0 = champion BYTE-EXACT ; 1 = pass 0 HPF + les 12 restarts ;
        //      2 = pass 0 HPF SEUL (aucun restart ILS)
        t40_hpf_seed: usize,
        // i4 : 0 = off ; 1 = la solution HPF est INJECTEE DANS LA POPULATION de chaque
        // restart (calculee une seule fois), au lieu d'etre un candidat parallele.
        t40_hpf_pop: usize,
    }
    ...
    impl Hparams {
            ...
            if n <= 1200 {
                if budget <= 5 {
                    Self {
                        ...
                        use_heavy_polish: true, window_k: n, core_half_dp: 60, n_lambda_values: 1600, t40_hpf_seed: 0, t40_hpf_pop: 0,
                    }
                } else if budget <= 10 {
                    Self {
                    ...
                    Self {
                        ...
                        use_heavy_polish: false, window_k: 60, core_half_dp: 12, n_lambda_values: 1600, t40_hpf_seed: 0, t40_hpf_pop: 0,
                    }
                }
            } else {
                Self {
                    ...
                    use_heavy_polish: false, window_k: 200, core_half_dp: 50, n_lambda_values: 1600, t40_hpf_seed: 0, t40_hpf_pop: 0,
                }
            }
        }
        ...
        fn from_map(h: &Option<Map<String, Value>>, n: usize, budget: u32) -> Self {
            ...
            if let Some(m) = h {
                ...
                if let Some(v) = m.get("t40_hpf_seed").and_then(|v| v.as_u64()) { p.t40_hpf_seed = v as usize; }
                if let Some(v) = m.get("t40_hpf_pop").and_then(|v| v.as_u64()) { p.t40_hpf_pop = v as usize; }
            }
            p
        }
    }
    ...
    fn try_add_elite_t40(pool: &mut Vec<Vec<bool>>, bits: &[bool]) {
        let hamming_ok = pool.iter().all(|e| {
            e.iter().zip(bits.iter()).filter(|(&a, &b)| a != b).count() > 20
        });
        if hamming_ok {
            if pool.len() >= 4 { pool.remove(0); }
    ...
    fn path_relink_t40(
        ...
        let mut best_pr_val = state.total_value;
        let mut best_pr_bits = state.selected_bit.clone();
        ...
        while !to_add.is_empty() || !to_remove.is_empty() {
            ...
            if state.total_weight <= cap && state.total_value > best_pr_val {
                best_pr_val = state.total_value;
                best_pr_bits = state.selected_bit.clone();
            }
            ...
            if move_count % checkpoint_interval == 0 && state.total_weight <= cap {
                ...
                if tmp.total_value > best_pr_val {
                    best_pr_val = tmp.total_value;
                    best_pr_bits = tmp.selected_bit.clone();
                }
            }
        }
        ...
        if final_st.total_value > best_pr_val {
            best_pr_val = final_st.total_value;
            best_pr_bits = final_st.selected_bit.clone();
        }

        if best_pr_val > 0 { Some((best_pr_val, best_pr_bits)) } else { None }
    }

    fn run_one_instance(challenge: &Challenge, hp: &Hparams, rng_offset: usize, hpf_seed: Option<&SolState>) -> Solution {
        let n = challenge.num_items;
        ...
        // i4 : le seed HPF rejoint la POPULATION -> l'ILS part reellement de lui,
        // au lieu de le laisser concourir a cote sans jamais l'explorer.
        if let Some(bp) = hpf_seed { population.push(bp.clone()); }
        ...
        population.sort_unstable_by_key(|s| std::cmp::Reverse(s.value));
        {
            ...
            for p in population.drain(..) {
                ...
                for (i, &b) in p.bits.iter().enumerate() {
                    if b {
                        let mut v: u64 = 0x517CC1B727220A95;
                        v ^= (i as u64).wrapping_mul(0x9E3779B97F4A7C15);
                        v = v.rotate_left(17).wrapping_mul(0xBF58476D1CE4E5B9);
                        h ^= v;
                    }
                }
        ...
        // P11 elite pool — accumulated during ILS for post-ILS Path Relinking
        ...
        //            b8fdb50c (dynamic PR applied after local search phase — post-ILS pattern)
        // hamming_threshold=20 (b=25% wide budget → need structurally diverse pool)
        if elite_pool.len() >= 2 {
    ...
    impl Solver {
        pub fn solve(
            ...
            // i4 : calcule UNE SEULE FOIS (le HPF est deterministe pour une instance donnee) ;
            // None => chemin champion strictement inchange.
            let hpf_pop_seed: Option<SolState> = if hp.t40_hpf_pop > 0 {
                Some(run_bp_seeded_instance(challenge, &hp))
            } else { None };

            // ---- i3 SEED HPF/QKBP -----------------------------------------
            // superfast_knap_v1 (le seul algo qui nous bat au Pareto sur t40/t42)
            // n'a AUCUNE boucle de restart : il tire son point de depart d'un
            // max-flow/min-cut PARAMETRIQUE (pseudoflow de Hochbaum) sur
            // n_lambda_values valeurs de lambda, puis fait UNE passe de recherche
            // locale. Nous achetons la meme qualite par 12 repetitions.
            // Ici : la solution de breakpoint devient une passe 0, et le mode 2
            // supprime entierement les restarts pour mesurer ce que le seed vaut
            // seul. La question testee : HPF + 1 passe vaut-il 12 restarts ?
            if hp.t40_hpf_seed > 0 {
                let mut best_sol: Option<Solution> = None;
                let mut best_quality: i64 = i64::MIN;
                {
                    let bp = run_bp_seeded_instance(challenge, &hp);
                    if bp.value > best_quality {
                        best_quality = bp.value;
                        best_sol = Some(Solution {
                            items: (0..n).filter(|&i| bp.bits[i]).collect(),
                        });
                    }
                }
                let extra = if hp.t40_hpf_seed >= 2 { 0 } else { n_restarts };
                for restart in 0..extra {
                    let sol = run_one_instance(challenge, &hp, restart, hpf_pop_seed.as_ref());
                    let mut val: i64 = 0;
                    for &i in &sol.items {
