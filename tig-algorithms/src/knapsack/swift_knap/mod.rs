...
pub fn help() {
    println!("Quadratic Knapsack - Swift Multi-Start ILS with Adaptive Basin Discovery");
    println!();
    ...
    println!("  n_perturbation_rounds (default: 18)");
    ...
    println!("  dp_threshold (default: 3500)");
...
impl Hyperparameters {
    pub fn initialize(h: &Option<Map<String, Value>>) -> Self {
        let mut p = Self {
            n_perturbation_rounds: 18,     // +3 over near_knap default
            perturbation_strength_base: 3,
            dp_threshold: 3500,            // raised from 2500
        };
        if let Some(m) = h {
...
fn core_half_for(n: usize, density: f64) -> usize {
    ...
    if density < 0.05 {
        // Very sparse: widen core for better DP coverage
        (base as f64 * 1.4) as usize
    } else if density < 0.15 {
        // Moderate: slight increase
        (base as f64 * 1.15) as usize
    } else if density > 0.5 {
        // Very dense: narrow core to keep DP fast
...
fn run_one_instance(challenge: &Challenge, params: &Hyperparameters) -> Solution {
    ...
    if n <= 1500 && best.is_some() && second.is_some() {
        ...
        // OPTIMIZATION 7b: Third-best merge (if available)
        if let Some(ref b3) = third {
            let mut hyb = State::new_empty(challenge, dens);
            {
                let b1 = best.as_ref().unwrap();
                for i in 0..n {
    ...
    if n <= 600 && hard {
        max_rounds = max_rounds.saturating_add(4);  // +1 over near_knap
    }

    if n >= 4500 {
        max_rounds = max_rounds.min(10);     // +2 over near_knap
    } else if n >= 3000 {
        max_rounds = max_rounds.min(13);     // +2
    } else if n >= 2000 {
        max_rounds = max_rounds.min(15);     // +2
    }
