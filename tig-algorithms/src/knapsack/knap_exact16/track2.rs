...
mod inner {
    ...
    pub fn challenge_to_p1(challenge: &Challenge) -> P1Instance {
        ...
        for i in 0..n {
            edges.push(Edge {
                i,
                j: i,
                value: challenge.values[i] as f64,
            });
        }
        for i in 0..n {
    ...
    mod hpf {
        ...
        pub struct HpfState {
            ...
            /// H197: monotone high-water of the highest strong bucket ever populated. The
            /// scan in get_highest_strong_root starts here instead of at num_nodes-1, which
            /// skips only provably-empty buckets. See patch_hsl1.py for the proof.
            pub hsl_hi: i32,
        ...
        unsafe fn push_upward(
            ...
            // H197: branchless max -> cmov, so this adds no basic block on the hot path.
            {
                let h = (*s).hsl_hi;
                let l = lbl as i32;
                (*s).hsl_hi = if l > h { l } else { h };
            }
            add_to_strong_bucket(child, (*s).strong_roots[lbl].end);
        }

        unsafe fn push_downward(
            s: *mut HpfState,
            arc: *mut HpfArc,
            child: *mut HpfNode,
            parent: *mut HpfNode,
        ...
        unsafe fn find_weak_node(
            s: *mut HpfState,
            strong_node: *mut HpfNode,
            weak_node: *mut *mut HpfNode,
        ) -> *mut HpfArc {
        ...
        unsafe fn check_children(s: *mut HpfState, cur: *mut HpfNode) {
            while !(*cur).next_scan.is_null() {
                if (*(*cur).next_scan).label == (*cur).label {
                    return;
                }
        ...
        // Compute each capacity with the same f32 expression as the old table.
        // Retain the original guards and capacity += delta: assigning the new
        // capacity directly would change floating-point rounding behavior.
        unsafe fn update_capacities(s: *mut HpfState, theparam: i32) {
            let lambda = (*s).max_degree_ratio - theparam as f32 * (*s).step;
            let root = ((*s).source - 1) as usize;
            let count = (*s).adjacency_list[root].num_out_of_tree as usize;
            // Source/sink adjacency is finalized during initialization. The
            // maintained count never exceeds the allocated pointer array.
            let entries = (*s).adjacency_list[root].out_of_tree.as_ptr();
            macro_rules! update {
                ($i:expr) => {{
                    let arc = *entries.add($i);
                    let nd = (*arc).to;
            ...
            (*s).highest_strong_label = (*s).hsl_hi;
        }
        ...
        pub fn get_breakpoints(inst: &P1Instance, n_lambda_values: usize) -> BreakpointSets {
            ...
            let lambda0 = max_degree_ratio - 0.0f32 * step;

            for i in 2..num_nodes as usize {
                let wt = adjacency_list[i].wt;
                let cst = adjacency_list[i].cst;
                let src = wt - lambda0 * cst;
                let src_capacity = if num_params > 0 && src > 0.0 {
                    src
                } else {
                    0.0
                };
                ...
                let snk = lambda0 * cst - wt;
                let snk_capacity = if num_params > 0 && snk > 0.0 {
                    snk
                } else {
                    0.0
                };
            ...
            unsafe {
                ...
                for i in 0..num_arcs as usize {
                    ...
                    if source == from_num && to_num == sink {
                        ...
                        (*s).adjacency_list[(from_num - 1) as usize]
                            .out_of_tree
                            .push(arc_ptr);
                        (*s).adjacency_list[(from_num - 1) as usize].num_out_of_tree += 1;
                    } else if to_num == sink {
                        let arc_ptr = &mut (*s).arc_list[i] as *mut HpfArc;
                        (*s).adjacency_list[(to_num - 1) as usize]
                            .out_of_tree
                            .push(arc_ptr);
    ...
    #[inline]
    pub fn ia_contains(a: &IntArray, v: usize) -> bool {
        a.contains(&v)
    }
    ...
    fn run_greedy_left(
        ...
        let ws_cands_nonempty = ws
            .as_ref()
            .map(|w| !w.candidate_nodes.is_empty())
            .unwrap_or(false);
        ...
        if ws_valid && ws_cands_nonempty {
            ...
            if beta == 0.0 {
                ...
                for &nd in right_nodes {
                    ...
                    if weights[nd] as f64 > budget as f64 - cur_w {
                        continue;
                    }
                    cand_nodes.push(nd);
    ...
    fn run_greedy_right(
        ...
        if ws_valid && ws_cands_nonempty {
            ...
            for &nd in &right_nodes {
                let mut contrib: f64 = right_nodes
                    .iter()
                    .map(|&m| (-1.0 - beta) * um.get(nd, m))
                    .sum();
                contrib -= um.linear[nd];
                if beta != 0.0 {
                    for v in 0..n_nodes {
    ...
    fn run_greedy(
        ...
        for bi in (0..n_budgets).rev() {
            ...
            let mut after_right =
                run_greedy_right(&um, n_nodes, right_init, budget, beta, &inst.weights, None);
            seed_empty_left(&mut after_right, &interaction_totals, &inst.weights, budget);
            let final_left = run_greedy_left(
                &um,
                n_nodes,
                after_right,
                &all_nodes,
    ...
    pub fn run_bp_algorithm(inst: &P1Instance, n_lambda_values: usize) -> QKPResult {
        ...
        for bi in 0..inst.n_budgets {
            selected.fill(0);
            for &i in &gr.left_nodes[bi] {
                selected[i] |= 1;
            }
            for &i in &gr.right_nodes[bi] {
                selected[i] |= 2;
            }
    ...
    impl P2Instance {
        #[inline]
        pub fn q_get(&self, i: usize, j: usize) -> i32 {
            // SAFETY /* H249 */: q.len() == n*n (asserted by the patch) and every i, j
            // reaching here is an item id from a (0..n) filter, so i*n + j < n*n.
            unsafe { *self.q.get_unchecked(i * self.n + j) }
        }
        #[inline]
        pub fn q_set(&mut self, i: usize, j: usize, v: i32) {
            self.q[i * self.n + j] = v;
        }
    }
    ...
    impl P2State {
        ...
        pub fn slack(&self) -> i32 {
            self.ins.capacity - self.weight
        }
        pub fn add_item(&mut self, i: usize) {
            if i >= self.ins.n || self.sel[i] != 0 {
                return;
            }
            ...
            for (c, &q) in self.contrib.iter_mut().zip(row.iter()) {
                *c += q;
            }
        }
        ...
        pub fn save_best(&mut self) {
            if self.weight <= self.ins.capacity && self.value > self.best_value {
                ...
                // H131: one 4n-byte memcpy here buys ~76-157 O(n) add_item replays per restore.
                // Measured ratio saves:restores = 0.07, so this side of the trade is ~15x rarer.
                self.best_contrib.copy_from_slice(&self.contrib);
            }
        }
        pub fn restore_best(&mut self) {
            // H131: replaced an item-by-item replay (one O(n) add_item per selected item) with two
            // memcpys. contrib[j] = sum_{i in sel} q[i*n+j] is a pure function of the selected set,
            // so restoring the array reproduces the same values -- i32 addition is commutative and
            // associative and the same multiset of addends is summed either way. Bit-identical.
            if self.best_count < 0 {
                // save_best never fired; reproduce the old behaviour exactly (cleared state).
                self.clear();
                return;
            }
    ...
    fn compute_total_interactions(ins: &P2Instance) -> Vec<Ll> {
        let n = ins.n;
        (0..n)
            .map(|i| (0..n).map(|j| ins.q_get(i, j) as Ll).sum())
            .collect()
    }
    ...
    pub fn bridge_load_solution(s: &mut P2State, br: &BudgetResult) {
        ...
        for &id in &br.selected_items {
            if id < n && s.sel[id] == 0 && s.weight + s.ins.w[id] <= s.ins.capacity {
                s.sel[id] = 1;
                s.weight += s.ins.w[id];
                s.count += 1;
                accepted.push(id);
    ...
    fn build_best_unused(s: &P2State, max_k: usize) -> Vec<ItemScore> {
        ...
        let n = s.ins.n;
        let mut density: Vec<ItemScore> = (0..n)
            .filter(|&i| s.sel[i] == 0)
            .map(|i| {
                let w = s.ins.w[i].max(1) as Ll;
                ItemScore {
                    id: i,
                    score: s.contrib[i] as Ll * 1_000_000 / w,
                }
            })
            .collect();
        ...
        let mut marginal: Vec<ItemScore> = (0..n)
            .filter(|&i| s.sel[i] == 0)
            .map(|i| ItemScore {
                id: i,
                score: s.contrib[i] as Ll,
            })
            .collect();
    ...
    fn build_synergy_unused(s: &P2State) -> Vec<ItemScore> {
        ...
        for x in 0..ranked.len() {
            ...
            for y in (x + 1)..ranked.len() {
                let b = ranked[y].id;
                let score = s.contrib[a] as Ll + s.contrib[b] as Ll + s.ins.q_get(a, b) as Ll;
        ...
        if pool.len() < POOL_K {
            let mut extra: Vec<ItemScore> = (0..s.ins.n)
                ...
                .map(|i| {
                    ...
                    ItemScore {
                        id: i,
                        score: s.contrib[i] as Ll + best_pair,
                    }
                })
                .collect();
    ...
    fn exact_group_exchange(
        ...
        let w = |i: usize| s.ins.w[i] as i64;
        let c = |i: usize| s.contrib[i] as i64;
        let q = |i: usize, j: usize| s.ins.q_get(i, j) as i64;
        let adds = groups(unused, na, w, c, q, true);
        let removes = groups(used, nr, w, c, q, false);
        let movement = best(&adds, &removes, s.slack() as i64, add_outer, false, q);
        if let Some((r, a)) = movement {
            for &i in &r.ids[..r.len] {
                s.remove_item(i);
            }
    ...
    fn apply_synergy_tail(s: &mut P2State) -> bool {
        if s.ins.w.iter().any(|&w| w <= 0) {
            return apply_synergy_tail_scan(s);
        }
        if s.ins.n > 2000 {
            return false;
        }
        let unused: Vec<usize> = build_synergy_unused(s).iter().map(|x| x.id).collect();
        let used: Vec<usize> = build_worst_used(s, 32).iter().map(|x| x.id).collect();
        if exact_group_exchange(s, &unused, &used, 2, 1, false) {
            return true;
        }
        let used: Vec<usize> = build_worst_used(s, 24).iter().map(|x| x.id).collect();
        if exact_group_exchange(s, &unused[..unused.len().min(24)], &used, 3, 1, false) {
            return true;
        }
        if s.ins.n <= 1200 {
            let used: Vec<usize> = build_worst_used(s, 20).iter().map(|x| x.id).collect();
            if exact_group_exchange(s, &unused[..unused.len().min(20)], &used, 3, 2, false) {
                return true;
            }
        }
        if s.slack() <= 0 || unused.len() < 3 {
            return false;
        }
        use super::super::exact_compound::{best, groups, Group};
        let adds = groups(
            &unused,
            3,
            |i| s.ins.w[i] as i64,
            |i| s.contrib[i] as i64,
            |i, j| s.ins.q_get(i, j) as i64,
            true,
        );
        let empty = [Group {
            ids: [0; 3],
        ...
        if let Some((_, a)) = best(&adds, &empty, s.slack() as i64, false, false, |i, j| {
            s.ins.q_get(i, j) as i64
        }) {
            for &i in &a.ids[..a.len] {
                s.add_item(i);
            }
    ...
    fn apply_synergy_tail_scan(s: &mut P2State) -> bool {
        if s.ins.n > 2000 {
            return false;
        }
                ...
                for rm_score in &used {
                    ...
                    for x in 0..unused.len() {
                        ...
                        for y in (x + 1)..unused.len() {
                            let b = unused[y].id;
                            if wa + s.ins.w[b] > budget {
                                continue;
                            }
                            let delta = a_gain + s.contrib[b] as Ll - s.ins.q_get(b, rm) as Ll
                ...
                for rm_score in &used {
                    ...
                    for x in 0..limit {
                        ...
                        for y in (x + 1)..limit {
                            ...
                            for z in (y + 1)..limit {
                                let c = unused[z].id;
                                if wab + s.ins.w[c] > budget {
                                    continue;
                                }
                                let delta = a_gain + b_gain + s.contrib[c] as Ll
            ...
            if limit >= 3 && used.len() >= 2 {
                ...
                for x in 0..used.len() {
                    ...
                    for y in (x + 1)..used.len() {
                        ...
                        let lost = contrib[r1] as Ll + contrib[r2] as Ll
                            - unsafe { *q.get_unchecked(r1_base + r2) } /* H249 */ as Ll;
                ...
                for &(r1, r2, budget, lost) in &removed_pairs {
                    for x in 0..limit {
                        ...
                        let a_base = a * n;
                        let a_gain = contrib[a] as Ll
                            - unsafe { *q.get_unchecked(a_base + r1) } /* H249 */ as Ll
                            - unsafe { *q.get_unchecked(a_base + r2) } /* H249 */ as Ll;
                        for y in (x + 1)..limit {
                            let b = unused[y].id;
                            let wab = wa + w[b];
                            if wab > budget {
                            ...
                            let b_base = b * n;
                            let b_gain = contrib[b] as Ll
                                - unsafe { *q.get_unchecked(b_base + r1) } /* H249 */ as Ll
                                - unsafe { *q.get_unchecked(b_base + r2) } /* H249 */ as Ll;
                            for z in (y + 1)..limit {
                                let c = unused[z].id;
                                if wab + w[c] > budget {
                                    continue;
                                ...
                                let c_base = c * n;
                                let delta = a_gain + b_gain + contrib[c] as Ll
                                    - unsafe { *q.get_unchecked(c_base + r1) } /* H249 */ as Ll
                                    - unsafe { *q.get_unchecked(c_base + r2) } /* H249 */ as Ll
                                    + unsafe { *q.get_unchecked(a_base + b) } /* H249 */ as Ll
                                    + unsafe { *q.get_unchecked(a_base + c) } /* H249 */ as Ll
                                    + unsafe { *q.get_unchecked(b_base + c) } /* H249 */ as Ll
                                    - lost;
                                if delta > best_delta {
                                    best_delta = delta;
        ...
        for x in 0..unused.len() {
            ...
            for y in (x + 1)..unused.len() {
                ...
                for z in (y + 1)..unused.len() {
                    let c = unused[z].id;
                    if wab + s.ins.w[c] > rem {
                        continue;
                    }
                    let delta = s.contrib[a] as Ll
                        + s.contrib[b] as Ll
                        + s.contrib[c] as Ll
                        + s.ins.q_get(a, b) as Ll
                        + s.ins.q_get(a, c) as Ll
    ...
    fn apply_best_swap11(s: &mut P2State, window_k: usize) -> bool {
        if s.ins.w.iter().any(|&w| w <= 0) {
            return apply_best_swap11_scan(s, window_k);
        }
        let unused: Vec<usize> = build_best_unused(s, window_k)
            .iter()
            .map(|x| x.id)
            .collect();
        let used: Vec<usize> = build_worst_used(s, window_k).iter().map(|x| x.id).collect();
        exact_group_exchange(s, &unused, &used, 1, 1, false)
    }

    fn apply_best_swap11_scan(s: &mut P2State, window_k: usize) -> bool {
        let unused = build_best_unused(s, window_k);
        ...
        let mut br = None;
        let mut ba = None;
        // H166: q >= 0, so d = contrib[add] - contrib[rm] - q[add][rm] <= add_contrib - mincontrib.
        // Depends only on the used window; computed once per call. Measured: swap11 is 45.00% of
        // track2's inner iterations after s12p1 (batch_b80).
        ...
        for y in 0..cu {
            ...
            let row_base = add * n;
            // H166: STRICT `<`, not `<=`. track2 accepts a tie via the `ordinal` tie-break below, so
            // `ub == best_delta` can still change the answer and must not be skipped.
            if add_contrib.saturating_sub(s11_mincontrib) < best_delta {
                continue;
            }
            for x in 0..cs {
                let rm = used[x].id;
                if add_weight > slack + w[rm] {
                    continue;
                }
                let d = add_contrib
                    - contrib[rm] as Ll
                    - unsafe { *q.get_unchecked(row_base + rm) } /* H249 */ as Ll;
                let ordinal = x * cu + y;
                if d > best_delta || (d == best_delta && d > 0 && ordinal < best_ordinal) {
                    best_delta = d;
    ...
    fn apply_best_swap12(s: &mut P2State, window_k: usize) -> bool {
        if s.ins.w.iter().any(|&w| w <= 0) {
            return apply_best_swap12_scan(s, window_k);
        }
        let unused: Vec<usize> = build_best_unused(s, window_k)
            .iter()
            .map(|x| x.id)
            .collect();
        let used: Vec<usize> = build_worst_used(s, window_k).iter().map(|x| x.id).collect();
        exact_group_exchange(s, &unused, &used, 2, 1, false)
    }

    fn apply_best_swap12_scan(s: &mut P2State, window_k: usize) -> bool {
        let unused = build_best_unused(s, window_k);
        let used = build_worst_used(s, window_k);
        let cu = unused.len();
        let cs = used.len();
        ...
        let mut ba = None;
        let mut bb = None;
        // H98: `cb_eff` depends on (y, rm) only, so it is hoisted out of the x-loop into
        // `effective[]`, computed once per `si`. This mirrors track1's apply_best_swap12
        // (track1.rs:1671-1673) exactly. Pure loop-invariant motion: same integers, same
        // associativity, same enumeration order => bit-identical result.
        let n_items = s.ins.n;
        let mut effective: Vec<Ll> = vec![0; cu];
        // H154: the row half of the exact bound. max_{y>x} q[a_x][b_y] depends on x only, never on the
        // removal candidate, so it is hoisted out of the si loop: O(cu^2/2) once per call against an
        // inner loop of O(cs*cu^2/2). Measured fire rate 84-96% of x-iterations (batch_b75).
        ...
        for si in 0..cs {
            ...
            for x in 0..cu {
                let id = unused[x].id;
                effective[x] = s.contrib[id] as Ll
                    - unsafe { *s.ins.q.get_unchecked(id * n_items + rm) } /* H249 */ as Ll;
            }
            // H154: the other half of the bound -- suffix max of `effective`, O(cu) per removal candidate.
            let mut s12_sufmax: Vec<Ll> = vec![Ll::MIN; cu + 1];
            for x in (0..cu).rev() {
                s12_sufmax[x] = if effective[x] > s12_sufmax[x + 1] {
                    effective[x]
                } else {
                    s12_sufmax[x + 1]
                };
            ...
            for x in 0..cu {
                ...
                // H154: d(x,y) = ca_eff + effective[y] + q[a_x][b_y] - lost for every remaining y, so
                // ub = ca_eff + max effective + max row - lost dominates them all. Acceptance is a strict
                // `d > best_delta` and best_delta only rises, so ub <= best_delta proves no y can be taken.
                // Survivors keep their enumeration order, preserving the first-maximiser tie-break.
                if ca_eff
                ...
                let row_a = unsafe {
                    /* H195 */
                    std::slice::from_raw_parts(s.ins.q.as_ptr().add(a * n_items), n_items)
                };
                for y in (x + 1)..cu {
                    let b = unused[y].id;
                    if wa + s.ins.w[b] > budget {
    ...
    fn apply_best_swap21(s: &mut P2State, window_k: usize) -> bool {
        if s.ins.w.iter().any(|&w| w <= 0) {
            return apply_best_swap21_scan(s, window_k);
        }
        let unused: Vec<usize> = build_best_unused(s, window_k)
            .iter()
            .map(|x| x.id)
            .collect();
        let used: Vec<usize> = build_worst_used(s, window_k).iter().map(|x| x.id).collect();
        exact_group_exchange(s, &unused, &used, 1, 2, true)
    }

    fn apply_best_swap21_scan(s: &mut P2State, window_k: usize) -> bool {
        let unused = build_best_unused(s, window_k);
        let used = build_worst_used(s, window_k);
        let cu = unused.len();
        let cs = used.len();
        ...
        // H173: the weakest `lost` in the pair list. Free -- the build loop below already computes
        // every `lost`. Used by the outer early-out; see the comment at the `ai` loop.
        let mut s21_min_lost: Ll = Ll::MAX;
        let mut used_pairs: Vec<(usize, usize, i32, Ll)> = Vec::with_capacity(cs * cs / 2);
        for x in 0..cs {
            let r1 = used[x].id;
            let r1_base = r1 * n;
            for y in (x + 1)..cs {
        ...
        for ai in 0..cu {
            ...
            for rm_score in &used {
                let cross = unsafe { *q.get_unchecked(add_base + rm_score.id) } /* H249 */ as Ll;
                if cross < min_cross1 {
            ...
            // H173: for this `add`, every pair satisfies d <= cadd - min_cross - lost, and lost >=
            // s21_min_lost over the whole list, so cadd - min_cross - s21_min_lost dominates every pair.
            // Acceptance is a strict `>` against a best_delta that only rises, so if that bound cannot
            // beat best_delta the ENTIRE pair loop is dead. One compare replaces ~cs^2/2 of them.
            // Measured motivation: this function was 25.22% of track1 self-time while its delta line ran
            // 217 times per 20 nonces -- the sw21p1 bound skipped the delta but not the loop trip.
            if cadd.saturating_sub(min_cross).saturating_sub(s21_min_lost) <= best_delta {
                continue;
            }
            for &(r1, r2, budget, lost) in &used_pairs {
                ...
                if upper_bound <= best_delta {
                    continue;
                }
                let gained = cadd
                    - unsafe { *q.get_unchecked(add_base + r1) } /* H249 */ as Ll
                    - unsafe { *q.get_unchecked(add_base + r2) } /* H249 */ as Ll;
                let d = gained - lost;
                if d > best_delta {
                    best_delta = d;
                    br1 = Some(r1);
    ...
    fn apply_best_swap22(s: &mut P2State, window_k: usize) -> bool {
        ...
        let unused: Vec<usize> = unused.iter().map(|x| x.id).collect();
        let used: Vec<usize> = used.iter().map(|x| x.id).collect();
        let weights = &s.ins.w;
        let contrib = &s.contrib;
        let interaction = |a: usize, b: usize| s.ins.q[a * s.ins.n + b] as i64;
        let slack = s.slack() as i64;
    ...
    fn local_search_vnd(s: &mut P2State, window_k: usize, heavy: bool) {
        loop {
            // H122: apply_best_add1 removed (measured fire rate <= 1.7%)
            if apply_best_swap11(s, window_k) {
                continue;
            }
            // H122: apply_best_add2 removed (measured fire rate <= 1.7%)
            if apply_best_swap12(s, window_k / 2) {
                continue;
            }
            if apply_best_swap21(s, window_k / 2) {
                continue;
            }
            if heavy {
                if apply_best_swap22(s, window_k) {
                    continue;
                }
                if apply_synergy_tail(s) {
                    continue;
                }
            }
            break;
    ...
    fn dp_refinement(s: &mut P2State, core_half: usize) {
        ...
        let left = if idx_first_rej > core_half + 1 {
            idx_first_rej - core_half - 1
        } else {
            0
        };
        ...
        for i in 0..n {
            if s.sel[i] == 0 && target[i] != 0 && s.weight + s.ins.w[i] <= s.ins.capacity {
                s.add_item(i);
            }
        }
    }
    ...
    fn greedy_reconstruct(s: &mut P2State, strategy: usize, total_interactions: &[Ll]) {
        if s.ins.n > 4000 {
            let mut cand: Vec<ItemScore> = (0..s.ins.n)
                .filter(|&i| s.sel[i] == 0)
                .map(|i| ItemScore {
                    id: i,
                    score: reconstruction_score(s, i, strategy, total_interactions),
        ...
        loop {
            ...
            let Some((item, _)) = best else {
                ...
                let has_positive = pool
                    .iter()
                    .any(|is| reconstruction_score(s, is.id, strategy, total_interactions) > 0);
                if !has_positive {
                    break;
                }
                continue;
    ...
    fn solve_hybrid(
        ...
        let mut certified_refinement = |s: &mut P2State, half: usize, certified: &mut bool| {
    ...
    impl Solver {
        pub fn solve(
            ...
            let raw_seed = u64::from_le_bytes(challenge.seed[..8].try_into().unwrap_or([0u8; 8]));
            ...
            if chk_val != s.best_value {
                s.best_value = chk_val;
            }
            if chk_wt != s.best_weight {
                s.best_weight = chk_wt;
            }
            if chk_wt > budget {
                return Ok(None);
            }

            let items: Vec<usize> = (0..s.ins.n).filter(|&i| s.best_sel[i] != 0).collect();
