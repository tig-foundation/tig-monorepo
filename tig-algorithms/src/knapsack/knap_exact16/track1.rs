...
mod inner {
    ...
    mod hpf {
        ...
        impl HpfNode {
            pub fn zeroed(num_params: i32) -> Self {
                HpfNode {
                    wt: 0.0, cst: 1.0, number: 0, label: 0, excess: 0.0,
        ...
        unsafe fn push_upward(s: *mut HpfState, arc: *mut HpfArc,
            ...
            { let h = (*s).hsl_hi; let l = lbl as i32; (*s).hsl_hi = if l > h { l } else { h }; }
        ...
        unsafe fn update_capacities(s: *mut HpfState, theparam: i32) {
            ...
            for i in 0..size {
                ...
                // H177: guard removed -- unreachable (lambda strictly decreases, cst > 0);
                // verified 0 firings over the full gate before this form was emitted.
                // hpf_guard_removed
            ...
            // H197: was `num_nodes - 1` (= 1001), inherited from the upstream C. Every
            // lambda step then walked down ~1000 empty buckets. hsl_hi is an
            // upper bound on the highest non-empty strong bucket, so the scan
            // skips only buckets that are provably empty. Bit-identical.
        ...
        pub fn get_breakpoints(inst: &P1Instance,
            ...
            let mut state = HpfState {
                ...
                num_params,
                highest_strong_label: 1, hsl_hi: 1,
                max_degree_ratio,
                step,
                adjacency_list,
                strong_roots,
    ...
    impl P2State {
        ...
        pub fn eval_selected(ins: &P2Instance, bits: &[u8]) -> Ll {
            ...
            for i in 0..n {
                ...
                // H224: q[i][i] == 0 by construction (generator never writes the diagonal)
                for j in (i+1)..n {
    ...
    fn build_best_unused(s: &P2State, max_k: usize) -> Vec<ItemScore> {
        ...
        if n < 300 || max_k < 4 {
            let mut arr: Vec<ItemScore> = (0..n)
                ...
                .map(|i| {
                    ...
                    let gain = s.contrib[i] as Ll;  // H224: + q_get(i,i) deleted, provably 0
        ...
        // H192: cached argmin of `synergy`, valid whenever the buffer is full.
        let mut syn_worst = 0usize;
        let mut syn_worst_score = Ll::MAX;
        for i in 0..n {
            ...
            if synergy.len() < synergy_k {
                ...
                if synergy.len() == synergy_k {
                    // H192: seed the cached minimum with the identical scan.
                    syn_worst = 0usize;
                    for j in 1..synergy.len() {
                        if synergy[j].score < synergy[syn_worst].score {
                            syn_worst = j;
                        }
                    }
                    syn_worst_score = synergy[syn_worst].score;
                }
                continue;
            }

            // H192: the buffer changes only on a replacement, which the winprobe12 run
            // measured at repl_frac = 0.0059-0.0067. So the argmin re-scan recomputed an
            // unchanged value on ~99.4% of 2.5-5.3M trips per solve. Cache it and refresh
            // only after an actual replacement. The refresh uses the IDENTICAL scan --
            // start at 0, move only on a strict `<` -- so ties still resolve to the lowest
            // index and the buffer contents are the same at every step. Bit-identical.
            if score > syn_worst_score {
                synergy[syn_worst] = ItemScore { id: i, score };
                syn_worst = 0usize;
                for j in 1..synergy.len() {
                    if synergy[j].score < synergy[syn_worst].score {
                        syn_worst = j;
    ...
    fn apply_best_swap11(s: &mut P2State, window_k: usize) -> bool {
        ...
        let mut br = None;
        let mut ba = None;
        // H166: q >= 0 everywhere, so for every `add` this inner loop can reach,
        //     d = contrib[add] + q(add,add) - contrib[rm] - q(add,rm) <= maxbase - contrib[rm].
        // `maxbase` depends only on the unused window, so it is computed once per call rather
        // than per removal candidate. Measured: swap11 is 30.62% of track1's inner iterations
        // after s12p1 (batch_b80).
        ...
        for x in 0..cs {
            ...
            // H166: whole inner loop is dead for this rm -- nothing it could find can beat best_delta,
            // which starts at 0 and only rises. Enumeration order of the survivors is untouched, so
            // the first-maximiser tie-break is preserved.
            if s11_maxbase.saturating_sub(s.contrib[rm] as Ll) <= best_delta { continue; }
            for y in 0..cu {
                let add = unused[y].id;
    ...
    fn apply_best_swap12(s: &mut P2State, window_k: usize) -> bool {
        ...
        for si in 0..cs {
            ...
            for x in 0..cu {
                ...
                let row_a = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(a * n), n) };
                for y in (x+1)..cu {
                    let b = unused[y].id;
    ...
    fn apply_best_swap21(s: &mut P2State, window_k: usize) -> bool {
        ...
        for x in 0..cs {
            let r1 = used[x].id;
            let row_r1 = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(r1 * n), n) };
            for y in (x + 1)..cs {
                let r2 = used[y].id;
        ...
        for ai in 0..cu {
            ...
            let row_add = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(add * n), n) };
            let cadd = s.contrib[add] as Ll + row_add[add] as Ll;
            // H151: exact upper-bound prune, ported from track2's apply_best_swap21, which has
            // carried it all along while this copy had no prune at all. Every pair (r1, r2) drawn
            // from the removal window satisfies row_add[r1] + row_add[r2] >= min_cross1 + min_cross2
            // (r1 != r2, both in the window), so d <= cadd - min_cross - lost. Acceptance is a
            // strict `>`, and best_delta only increases, so skipping a pair whose bound cannot
            // exceed best_delta discards nothing. Survivor order is unchanged, preserving the
            // first-maximiser tie-break. Costs O(cs) per add against an O(cs^2/2) inner loop.
            ...
            for &(r1, r2, removed_weight, lost) in &removal_pairs {
                ...
                if cadd.saturating_sub(min_cross).saturating_sub(lost) <= best_delta { continue; }
    ...
    fn apply_best_swap22(s: &mut P2State, window_k: usize) -> bool {
        ...
        // H109: store the cross table TRANSPOSED (`tu[b * cu + a]`) so that a fixed removal index
        // selects a contiguous cu-element row. The read pattern below fixes x,y and varies p,q2,
        // which on the original a*cs+b layout was four stride-(cs*4) column walks per iteration.
        let mut tu = vec![0i32; cs * cu];
        for a in 0..cu {
            let row = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(unused[a].id * n), n) };
            for b in 0..cs {
        ...
        for a in 0..cu {
            let p = unused[a].id;
            let wp = s.ins.w[p];
            let row_p = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(p * n), n) };
            let p_base = s.contrib[p] as Ll + row_p[p] as Ll;
            for b in (a+1)..cu {
                let q2 = unused[b].id;
                let pair_w = wp + s.ins.w[q2];
                let row_q2 = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(q2 * n), n) };
        ...
        for x in 0..cs {
            ...
            for y in (x+1)..cs {
                ...
                let lost   = s.contrib[r1] as Ll + s.contrib[r2] as Ll
                             - row_r1[r2] as Ll;
                // m[p] = tu[x][p] + tu[y][p] = u2u[p][x] + u2u[p][y]; two contiguous cu-element
                // reads, then the inner loop indexes a 160-byte L1-resident array.
        ...
        for rm_score in &used {
            ...
            for ai in 0..unused.len() {
                ...
                for bi in (ai + 1)..unused.len() {
                    ...
                    let row_b = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(b * n), n) };
                    let db = s.contrib[b] as Ll + row_b[b] as Ll - row_b[rm] as Ll;

                    for ci in (bi + 1)..unused.len() {
                        let c = unused[ci].id;
                        if wab + s.ins.w[c] > budget { continue; }
                        let row_c = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(c * n), n) };
    ...
    fn apply_best_swap23(s: &mut P2State, window_k: usize) -> bool {
        ...
        // H243: per-candidate row maximum inside the ADD window, indexed by POSITION in
        // `unused`. Independent of (r1, r2), so hoisted out of the removal loops: O(cu^2) once.
        // Ported from apply_best_swap31's H182 (track1.rs:2149), which fires 47.66%. The GLOBAL
        // form (H178) fired 0.00% with mean slack +1,468 -- charging each pair to a DISTINCT
        // endpoint is the whole difference.
        let mut sb_rowmax: Vec<Ll> = vec![0; unused.len()];
        for i in 0..unused.len() {
            let ui = unused[i].id;
            let row_i = &q[ui * n..(ui + 1) * n];
            let mut mx = 0i64;
            for j in 0..unused.len() {
                if j != i {
                    let v = row_i[unused[j].id] as Ll;
                    if v > mx { mx = v; }
                }
            }
            sb_rowmax[i] = mx;
        ...
        for ri in 0..used.len() {
            ...
            for rj in (ri + 1)..used.len() {
                ...
                // H243 one-shot per (r1,r2): the three largest h dominate every ordered triple,
                // so this kills the whole C(cu,3) sub-nest. O(cu) fold, nothing added inside it.
                {
                    let (mut h1, mut h2, mut h3) = (Ll::MIN, Ll::MIN, Ll::MIN);
                    for (xi, x) in unused.iter().enumerate() {
                        let row_x = &q[x.id * n..(x.id + 1) * n];
                        let h = s.contrib[x.id] as Ll - row_x[r1] as Ll - row_x[r2] as Ll
                              + sb_rowmax[xi];
                        if h > h1 { h3 = h2; h2 = h1; h1 = h; }
                        else if h > h2 { h3 = h2; h2 = h; }
                        else if h > h3 { h3 = h; }
                    }
                    if h1.saturating_add(h2).saturating_add(h3).saturating_sub(lost)
                        <= best_delta { continue; }
                }
    ...
    fn apply_best_swap31(s: &mut P2State, window_k: usize) -> bool {
        ...
        // H182: per-candidate row maximum inside the removal window, indexed by POSITION in `used`.
        // H178 used a single global max over the window and applied it three times; that fired 0.00%
        // with a mean slack of +1,468 (exp325_sbprobe). This form fires 47.66% (exp326_sbv2).
        // Independent of `add`, so hoisted out of the `ai` loop: O(cs^2) once per call.
        let mut s31_rowmax: Vec<Ll> = vec![0; used.len()];
        for i in 0..used.len() {
            let ri_id = used[i].id;
            let row_i = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(ri_id * n), n) };
            let mut rm = 0i64;
            for j in 0..used.len() {
                if j != i {
        ...
        for ai in 0..unused.len() {
            ...
            // H178: delta = base_gain - (g(r1)+g(r2)+g(r3)) + (q[r1][r2]+q[r1][r3]+q[r2][r3]) with
            // g(u) = row_add[u] + contrib[u], so delta <= base_gain - (three smallest g) + 3*maxq.
            // Acceptance is a strict `>` against a best_delta that only rises, so if that bound cannot
            // beat it the whole C(cs,3) triple loop is dead for this `add`. O(cs) replaces O(cs^3/6).
            {
                // Each of the three pairs (r1,r2),(r1,r3),(r2,r3) is charged to a DISTINCT endpoint, so
                //   q[r1][r2]+q[r1][r3]+q[r2][r3] <= rowmax(r1)+rowmax(r2)+rowmax(r3).
                // Folding that into h(u) = rowmax(u) - (row_add[u] + contrib[u]) gives
                //   delta <= base_gain + (three largest h),
                // and the three largest h over the whole window dominate any ordered triple.
                let (mut h1, mut h2, mut h3) = (Ll::MIN, Ll::MIN, Ll::MIN);
            ...
            for ri in 0..used.len() {
                ...
                for rj in (ri + 1)..used.len() {
                    let r2 = used[rj].id;
                    let row_r2 = unsafe { /* H195 */ std::slice::from_raw_parts(q.as_ptr().add(r2 * n), n) };
    ...
    fn local_search_vnd(s: &mut P2State, window_k: usize, heavy: bool,
        ...
        loop {
            ...
            if tight {
                ...
                // H122: apply_best_refill_after_remove removed (measured fire rate <= 1.7%)
            ...
            if heavy {
                ...
                // H118: removed. Measured 6.77% fire rate (83 of 1,226 calls) for a VERIFIED
                // 23.30% of track1 self-time, running 608,400 inner iterations to completion on
                // every VND termination. Gated on the median being unchanged on BOTH seeds.
    ...
    fn greedy_reconstruct(s:                  &mut P2State,
        ...
        let mut cand: Vec<ItemScore> = (0..n)
            ...
            .map(|i| {
                let w = s.ins.w[i].max(1) as Ll;
                let score = match strategy {
                    0 => s.contrib[i] as Ll,  // H224
                    1 => (s.contrib[i] as Ll) * 1_000_000 / w,  // H224
                    2 => total_interactions[i] + s.contrib[i] as Ll,  // H224
                    3 => (s.contrib[i] as Ll) * 1_000_000 / w  // H224
                         + total_interactions[i] / 10,
                    _ => s.contrib[i] as Ll  // H224
    ...
    fn greedy_reconstruct_sign_aware(s: &mut P2State, total_interactions: &[Ll]) {
        ...
        let mut cand: Vec<ItemScore> = (0..n)
            ...
            .map(|i| {
                let w = s.ins.w[i].max(1) as Ll;
                let marginal = s.contrib[i] as Ll;  // H224
                ItemScore {
                    id: i,
