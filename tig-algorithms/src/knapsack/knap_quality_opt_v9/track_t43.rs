        ...
        if n <= 1200 {
            if budget <= 5 {
                Self { window_k: 200, core_dp: 50, ils_rounds: 400, perturb_base: 4, n_starts: 6,
                       sa_steps: 0, sa_moves: 0, sa_t0: 0.0, sa_decay: 0.98 }
            } else if budget <= 10 {
                Self { window_k: 200, core_dp: 50, ils_rounds: 400, perturb_base: 4, n_starts: 6,
                       sa_steps: 0, sa_moves: 0, sa_t0: 0.0, sa_decay: 0.98 }
            } else {
            ...
            Self { window_k: 70, core_dp: 40, ils_rounds: 60, perturb_base: 6, n_starts: 3,
                   sa_steps: 150, sa_moves: 24000, sa_t0: 5000.0, sa_decay: 0.98 }
        }
    }
...
fn run_fast_sa(st: &mut State, rng: &mut Rng, temp_steps: usize, moves_per_step: usize, temp_initial: f64, temp_decay: f64) -> SolState {
    ...
    for _step in 0..temp_steps {
        ...
        for _ in 0..moves_per_step {
            ...
            let rm = sa.sel_list[(rng.next_u64() as usize % sa.sel_list.len())] as usize;
            let add = sa.unsel_list[(rng.next_u64() as usize % sa.unsel_list.len())] as usize;
...
fn solve_t43_fisa(ch: &Challenge, save: &dyn Fn(&Solution) -> Result<()>, hp_map: &Option<Map<String, Value>>) -> Result<()> {
    ...
    for mode in 0..hp.n_starts.max(1) {
        ...
        let _ = run_fast_sa(&mut st, &mut rng, hp.sa_steps, fast_moves, hp.sa_t0, hp.sa_decay);
        for _ in 0..3 {
            let bef = st.total_value;
            local_search_vnd_windowed(&mut st, hp.window_k.max(150), 0);
            dp_refinement_hp(&mut st, hp.core_dp + 10);
...
pub fn solve_challenge(
    ...
    let sum_w: u64 = ch.weights.iter().map(|&w| w as u64).sum();
    let budget_pct = if sum_w > 0 { ((ch.max_weight as u64) * 100 / sum_w) as u32 } else { 25 };
    if n > 1200 && budget_pct > 17 {
        return solve_t43_fisa(ch, save, hp_map);
    }
    solve_challenge_legacy(ch, save, hp_map)
}
...
pub fn help() {
    println!("knap_quality_opt_v7.1: Zero-Alloc + Pruned O(1) VND + Fuel Safe");
}
