...
pub fn help() {
    println!("Quadratic Knapsack - Swift Sparse Specialist");
    println!();
    ...
    println!("  n_perturbation_rounds (default: 22)");
    ...
    println!("  dp_threshold (default: 4200)");
...
fn core_half_for(n: usize, density: f64) -> usize {
    ...
    if density < 0.05 {
        // Sparse specialist: spend more search budget on large cores.
        (base as f64 * 1.7) as usize
    } else if density < 0.15 {
        (base as f64 * 1.3) as usize
    } else if density > 0.5 {
        (base as f64 * 0.75) as usize
    } else {
        base
    }
}
...
fn run_one_instance(challenge: &Challenge, params: &Hyperparameters) -> Solution {
    ...
    if n <= 600 && hard {
        max_rounds = max_rounds.saturating_add(6);
    } else if n <= 1200 {
