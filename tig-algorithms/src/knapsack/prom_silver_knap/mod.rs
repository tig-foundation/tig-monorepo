...
fn apply_swap_1_3_bounded_tsn(
    ...
    let mut best_delta: i64 = 0;
    let mut best_move: Option<(usize, usize, usize, usize)> = None;
    let ks = worst_used.len().min(12);
    let ku = best_unused.len().min(16);
    let mut best_mask = vec![false; state.ch.num_items];
    for &i in &best_unused[..ku] {
        best_mask[i] = true;
    }

    for &rm in &worst_used[..ks] {
        let w_rm = state.ch.weights[rm] as i64;
        let c_rm = state.contrib[rm] as i64;
        let budget = state.slack() as i64 + w_rm;

        for &a1 in &best_unused[..ku] {
            if state.selected_bit[a1] {
                continue;
            }
            let w1 = state.ch.weights[a1] as i64;
            if w1 >= budget {
                continue;
            }
            let c1 = state.contrib[a1] as i64 - state.ch.interaction_values[a1][rm] as i64;

            for &a2 in &tsn.friends[a1] {
                if a2 == a1 || a2 == rm || !best_mask[a2] || state.selected_bit[a2] {
                    continue;
                }
                let w2 = state.ch.weights[a2] as i64;
                if w1 + w2 >= budget {
                    continue;
                }
                let c2 = state.contrib[a2] as i64 - state.ch.interaction_values[a2][rm] as i64;
                let syn12 = state.ch.interaction_values[a1][a2] as i64;

                for &a3 in &tsn.friends[a2] {
                    if a3 == a1 || a3 == a2 || a3 == rm || !best_mask[a3] || state.selected_bit[a3] {
                        continue;
                    }
                    let w3 = state.ch.weights[a3] as i64;
                    if w1 + w2 + w3 > budget {
                        continue;
                    }
                    let c3 =
                        state.contrib[a3] as i64 - state.ch.interaction_values[a3][rm] as i64;
                    let delta = c1
                        + c2
                        + c3
                        + syn12
                        + state.ch.interaction_values[a1][a3] as i64
                        + state.ch.interaction_values[a2][a3] as i64
                        - c_rm;
                    if delta > best_delta
                        && state.total_weight - w_rm as u32 + w1 as u32 + w2 as u32 + w3 as u32
                            <= cap
                    {
                        best_delta = delta;
                        best_move = Some((rm, a1, a2, a3));
                    }
                }
            }
        }
    ...
    if let Some((rm, a1, a2, a3)) = best_move {
        state.remove_item(rm);
        state.add_item(a1);
        state.add_item(a2);
        state.add_item(a3);
...
fn local_search_vnd_tsn(state: &mut State, tsn: &TopNeighbors) {
    ...
    for _ in 0..80 {
...

        {
            ...
            let mut bd = 0i64;
            let mut bm = None;
            let ks = worst_used.len().min(24);
            let ku = best_unused.len().min(24);
            for si in 0..ks {
                let rm = worst_used[si];
                let c_rm = state.contrib[rm] as i64;
                let w_rm = state.ch.weights[rm];
                ...
                for ui in 0..ku {
                    ...
                    let ca1_eff = state.contrib[a1] as i64
                        - state.ch.interaction_values[a1][rm] as i64;
                    for &a2 in &tsn.friends[a1] {
                        if state.selected_bit[a2] || a1 == a2 {
                            continue;
                        }
...

        {
            ...
            for i in 0..ks {
                ...
                for j in (i + 1)..ks {
                    ...
                    let cr2 = state.contrib[r2] as i64;
                    let wr2 = state.ch.weights[r2];
                    let syn12 = state.ch.interaction_values[r1][r2] as i64;
                    for k in (j + 1)..ks {
                        let r3 = worst_used[k];
                        let cr3 = state.contrib[r3] as i64;
                        let wr3 = state.ch.weights[r3];
                        let lost = cr1 + cr2 + cr3
                            - syn12
                            - state.ch.interaction_values[r1][r3] as i64
                            - state.ch.interaction_values[r2][r3] as i64;
                        let budget = state.slack() + wr1 + wr2 + wr3;
                        ...
                        for u in 0..ku {
                            ...
                            let ca1_eff = state.contrib[a1] as i64
                                - state.ch.interaction_values[a1][r1] as i64
                                - state.ch.interaction_values[a1][r2] as i64
                                - state.ch.interaction_values[a1][r3] as i64;
                            for &a2 in &tsn.friends[a1] {
                                if state.selected_bit[a2] || a1 == a2 {
                                    continue;
                                }
                                ...
                                let gained = ca1_eff + state.contrib[a2] as i64
                                    - state.ch.interaction_values[a2][r1] as i64
                                    - state.ch.interaction_values[a2][r2] as i64
                                    - state.ch.interaction_values[a2][r3] as i64
                                    + state.ch.interaction_values[a1][a2] as i64;
                                let delta = gained - lost;
                                if delta > bd
                                    && state.total_weight + wa1 + wa2 <= cap + wr1 + wr2 + wr3
                                {
                                    bd = delta;
                                    bm = Some((r1, r2, r3, a1, a2));
                                }
                            }
                        }
                    }
            ...
            if let Some((r1, r2, r3, a1, a2)) = bm {
                state.remove_item(r1);
                state.remove_item(r2);
                state.remove_item(r3);
                state.add_item(a1);
                state.add_item(a2);
                continue;
            }
...
fn local_search_vnd_tsn_light(state: &mut State, tsn: &TopNeighbors) {
    ...
    for _ in 0..40 {
...

        {
            ...
            let mut bd = 0i64;
            let mut bm = None;
            let ks = worst_used.len().min(36);
            let ku = best_unused.len().min(48);
            for si in 0..ks {
                let rm = worst_used[si];
                let c_rm = state.contrib[rm] as i64;
                let w_rm = state.ch.weights[rm];
...

        {
            ...
            let mut bd = 0i64;
            let mut bm = None;
            let ks = worst_used.len().min(16);
            let ku = best_unused.len().min(20);
            for i in 0..ks {
                let r1 = worst_used[i];
                let cr1 = state.contrib[r1] as i64;
                let wr1 = state.ch.weights[r1];
                        ...
                        for ui in 0..ku {
                            ...
                            for &a2 in &tsn.friends[a1] {
                                ...
                                let gained = ca1_eff + state.contrib[a2] as i64
                                    ...
                                    + state.ch.interaction_values[a1][a2] as i64;
                                let delta = gained - lost;
                                if delta > bd && state.total_weight + wa1 + wa2 <= cap + wr1 + wr2 + wr3 {
                                    bd = delta;
                                    bm = Some((r1, r2, r3, a1, a2));
                                }
                            }
        ...
        if apply_swap_1_3_bounded_tsn(state, tsn, &best_unused, &worst_used) {
            continue;
        }
...
fn path_relink_tsn_limited(
    ...
    let mut to_remove: Vec<usize> = (0..n).filter(|&i| in_a[i] && !in_b[i]).collect();
    let total_moves = to_add.len() + to_remove.len();
    if total_moves == 0 || total_moves > 900 {
        return None;
    }
    ...
    let mut best_bits = state.selected_bit.clone();
    let cap = challenge.max_weight;
    let checkpoint_interval = (total_moves / 5).max(4);
    let mut move_count = 0usize;
    ...
    local_search_vnd_tsn_light(&mut final_state, tsn);
    dp_refinement_hp(&mut final_state, hp.core_half_dp);
    let val = final_state.total_value;
    Some((
        Solution {
            items: final_state.selected_items(),
        },
    ...
    pub fn solve(
        ...
        if n > 1200 {
            ...
            for restart in 0..n_restarts {
                let (mut sol, mut val) = if restart >= 2 && restart % 2 == 0 && best_sol.is_some() {
                    run_one_instance_seeded(
                        challenge,
                        &hp,
                        restart,
                ...
                if let Some(best) = best_sol.as_ref() {
                    if let Some((rel_sol, rel_val)) =
                        path_relink_tsn_limited(challenge, best, &sol, &hp, &tsn)
                    {
                        if rel_val > val {
                            sol = rel_sol;
                            val = rel_val;
                    ...
                    if restart % 3 == 1 {
                        if let Some((rel_sol, rel_val)) =
                            path_relink_tsn_limited(challenge, &sol, best, &hp, &tsn)
                        {
                            if rel_val > val {
                                sol = rel_sol;
                                val = rel_val;
                ...
                if val > best_quality {
                    best_quality = val;
                    save_incumbent(save_solution, &sol)?;
                    best_sol = Some(sol);
                    restart_stall = 0;
                } else {
                    restart_stall += 1;
        ...
        for restart in 0..n_phase1 {
            ...
            if elite[0].1 > prev_best {
                save_incumbent(save_solution, &elite[0].0)?;
            }
            if val <= prev_best {
                restart_stall_1k += 1;
