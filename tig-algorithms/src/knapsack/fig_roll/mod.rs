    ...
    for &item in &order {
        ...
        for (beam, val, weight) in &beams {
            ...
            if weight + item_weight <= challenge.max_weight as i64 {                
                let mut synergy_bonus: i64 = 0;
                for &existing_item in beam.iter() {
                    synergy_bonus += challenge.interaction_values[item][existing_item] as i64;
                }
                
                let mut new_beam = beam.clone();
                new_beam.push(item);                
                candidates.push((new_beam, val + item_value + synergy_bonus, weight + item_weight));
            }
        }
...
fn build_static_bins(state: &mut State, core_half: usize) {
    let n = state.ch.num_items;
    let mut order: Vec<usize> = (0..n).collect();
    order.sort_unstable_by(|&a, &b| {
        let da = (state.contrib[a] as f64) / (state.ch.weights[a] as f64);
    ...
    for (pos, &i) in order.iter().enumerate() {
        let w = state.ch.weights[i] as i64;
        if pref_w + w > state.ch.max_weight as i64 {
            break_idx = pos;
            break;
        }
        pref_w += w;
    ...
    let left = break_idx.saturating_sub(core_half);
    let right = (break_idx + core_half + 1).min(n);
    let core = &order[left..right];
...
fn apply_best_swap11_equal_windowed(
    ...
    let sample_limit = if state.ch.num_items > 7000 {
        20  
    } else if state.ch.num_items > 5000 {
        25  
    } else {
        worst_used.len()  
    };
    
    let check_worst = sample_limit.min(worst_used.len());
    for i in 0..check_worst {
        let rm = worst_used[i];
        let w_rm = state.ch.weights[rm];
        
        for &cand in &best_unused[..sample_limit.min(best_unused.len())] {
...
fn strategic_perturb_and_rebuild(state: &mut State, params: &Params) {
    ...
    if state.slack() > 0 && best_unused.len() >= 2 {
        ...
        if best_unused.len() >= 3 {
            let triple_limit = 6.min(best_unused.len());  
        ...
        let limit = 8.min(best_unused.len());  
        for i in 0..limit {
...
fn try_compound_ejection(state: &mut State, params: &Params, strategy: usize) {
    let (best_unused, worst_used) = build_ls_windows(state, params.core_half_ls);
    
    if worst_used.len() < 2 || best_unused.len() < 2 {
        return;
    }
    
    let try_remove = 4.min(worst_used.len());  
    let try_add = 5.min(best_unused.len());    
    
    let mut best_improvement: Option<(Vec<usize>, Vec<usize>, i64)> = None;
    
    for num_rm in 2..=try_remove {
        for num_add in 2..=try_add {
            let snapshot_val = state.total_value;
            let snapshot_weight = state.total_weight;
            let snapshot_contrib = state.contrib.clone();
            let snapshot_sel = state.selected_items();
            
            let mut remove_candidates: Vec<(usize, f64)> = worst_used.iter()
                .map(|&idx| {
                    let mut total_synergy: i64 = 0;
                    let mut strong_connections = 0;
                    for &other in &snapshot_sel {
                        if other != idx {
                            let syn = state.ch.interaction_values[idx][other] as i64;
                            total_synergy += syn;
                            if syn >= 6 {
                    ...
                    let weight_penalty = (state.ch.weights[idx] as f64) / (state.contrib[idx].max(1) as f64);
                    
                    let score = match strategy % 3 {
                        0 => {
                            let synergy_penalty = 1.0 / (1.0 + total_synergy as f64);
                            weight_penalty * synergy_penalty * 100.0
                        }
                        1 => {
                            let bridge_score = if strong_connections <= 2 && strong_connections > 0 {
                                weight_penalty * 2.0  
                            } else {
                                weight_penalty / (1.0 + strong_connections as f64)
                            };
                            bridge_score * 100.0
                        }
                        _ => {
                            let connectivity_score = strong_connections as f64;
                            weight_penalty * (10.0 / (1.0 + connectivity_score))
                        }
                    };
                    (idx, score)
                })
                .collect();
            remove_candidates.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(Ordering::Equal));
            
            for i in 0..num_rm.min(remove_candidates.len()) {
                let idx = remove_candidates[i].0;
                if state.selected_bit[idx] {
                    state.remove_item(idx);
                }
            }
            
            let current_selected = state.selected_items();
            let mut add_candidates: Vec<(usize, i64)> = best_unused.iter()
                .filter(|&&cand| state.ch.weights[cand] as i64 <= state.slack())
                .map(|&cand| {
                    let mut total_synergy: i64 = state.contrib[cand] as i64;
                    let mut strong_connections = 0;
                    for &existing in &current_selected {
                        let syn = state.ch.interaction_values[cand][existing] as i64;
                        total_synergy += syn;
                        if syn >= 6 {
                            strong_connections += 1;
                        }
                    ...
                    let score = match strategy % 3 {
                        ...
                        1 => {
                            let hub_bonus = if strong_connections >= 5 {
                                strong_connections * 15
                            } else if strong_connections >= 3 {
                                strong_connections * 8
                            } else {
                                0
                            };
                            total_synergy + hub_bonus
                        }
                        _ => {
                            let efficiency = (total_synergy * 100) / (state.ch.weights[cand].max(1) as i64);
                            efficiency + total_synergy / 2
                        }
                    };
                    (cand, score)
                })
                .collect();
            add_candidates.sort_by(|a, b| b.1.cmp(&a.1));
            
            let mut added = Vec::new();
            for (cand, synergy_score) in add_candidates {
                if state.ch.weights[cand] as i64 <= state.slack() && synergy_score > 0 {
                    state.add_item(cand);
                    added.push(cand);
                    if added.len() >= num_add {
                        break;
                    }
                }
            }
            
            let improvement = state.total_value - snapshot_val;
            if improvement > best_improvement.as_ref().map_or(0, |(_, _, imp)| *imp) {
                let removed: Vec<usize> = worst_used[..num_rm].to_vec();
                best_improvement = Some((removed, added.clone(), improvement));
            }
            
            state.restore_snapshot(&snapshot_sel, &snapshot_contrib, snapshot_val, snapshot_weight);
        }
    }
    
    if let Some((to_remove, to_add, imp)) = best_improvement {
        if imp > 0 {
            for &rm in &to_remove {
            ...
            for &add in &to_add {
                if state.ch.weights[add] as i64 <= state.slack() {
                    state.add_item(add);
                }
            }
        }
...
fn local_search_vnd(state: &mut State, _params: &Params) {
    loop {
        if apply_best_add_windowed(state) {
            continue;
        }
...
pub fn solve_challenge(
    ...
    if use_beam {
        ...
        let beam_items = beam_search_construction(challenge, beam_width);
    ...
    build_static_bins(&mut state, params.core_half_ls);
    ...
    for _it in 0..params.n_maxils {
        ...
        try_compound_ejection(&mut state, &params, 0);
        if use_extended_vnd {
            local_search_extended_vnd(&mut state, &params);
        } else {
            local_search_vnd(&mut state, &params);
        ...
        if state.total_value <= prev_val {
            ...
            if use_extended_vnd {
                try_compound_ejection(&mut state, &params, 1);
                local_search_extended_vnd(&mut state, &params);
