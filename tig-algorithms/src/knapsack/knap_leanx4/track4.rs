...
mod inner_four {
    ...
    fn apply_chain_move(state: &mut State, unused: &[usize], used: &[usize]) -> bool {
        ...
        for &rm in used {
            ...
            for (ai, &a1) in unused.iter().enumerate() {
                ...
                let gain_a1 = state.contrib[a1] as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(rm) } /* H259 */ as i64;
    ...
    fn screen_compound_candidates(state: &State, best_unused: &[usize], worst_used: &[usize]) -> (Vec<usize>, Vec<usize>) {
        ...
        for &i in &source {
            ...
            for &j in &source {
                if i == j { continue; }
                let syn = unsafe { *state.ch.interaction_values.get_unchecked(i).get_unchecked(j) } /* H259 */ as i64;
                if syn > first {
                    third = second;
                    second = first;
                    first = syn;
    ...
    fn apply_swap_2_2_windowed(state: &mut State, best_unused: &[usize], worst_used: &[usize]) -> bool {
        ...
        if let Some((r1, r2, a1, a2)) = best_move {
            // H207: four full n=5000 passes -> one. The fused operator already exists in this
            // file at :258 and was simply not called here. Bit-identical by algebra (same wrapping
            // i32 adds into contrib, same total_weight order, same zobrist XORs).
            state.replace_two_with_two(r1, r2, a1, a2);
            true
        } else { false }
    }
