    ...
    match mode {
        ...
        3 => loop {
            ...
            for i in 0..n {
                ...
                let s = dw(c * 1000, w) + (state.ch.weights[i] as i64) * 3;
                if s > best_s {
                    best_s = s;
                    best_i = Some(i);
                }
    ...
    for i in 0..n {
        for j in (i+1)..n {
            if state.ch.weights[i] + state.ch.weights[j] <= cap {
                pairs.push((unsafe { *state.ch.interaction_values.get_unchecked(i).get_unchecked(j) } /* H259 */, i, j));
            }
        }
    }
...

        {
            ...
            for si in 0..ks {
                ...
                for ai in 0..ku {
                    ...
                    let ca1_eff = state.contrib[a1] as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(rm) } /* H259 */ as i64;
                    for bi in (ai+1)..ku {
                        let a2 = best_unused[bi];
                        let wa2 = state.ch.weights[a2];
                        if wa1 + wa2 > budget { continue; }
                        let ca2_eff = state.contrib[a2] as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a2).get_unchecked(rm) } /* H259 */ as i64;
                        let syn = unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(a2) } /* H259 */ as i64;
...
fn local_search_vnd_tsn(state: &mut State, tsn: &TopNeighbors) {
    ...
    for _ in 0..80 {
        ...
        worst_used.clear();
        for i in 0..n {
            // three bounds checks per element over ~83M element-visits per nonce; each is a whole
            // instrumented basic block. One unsafe block reads all three. See module docs.
            let (c, w, sel) = unsafe {
                (*state.contrib.get_unchecked(i) as i64,
                 (*state.ch.weights.get_unchecked(i) as i64).max(1),
                 *state.selected_bit.get_unchecked(i))
            };
            if sel { used_buf.push((i, c, w)); } else { unused_buf.push((i, c, w)); }
        }
                ...
                for &a1 in &best_unused {
                    ...
                    for &a2 in &tsn.friends[a1] {
                        ...
                        if wa1 + (state.ch.weights[a2] as i32) <= slack_i {
                            let delta = ca1 + state.contrib[a2] as i64 + unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(a2) } /* H259 */ as i64;
...

        {
            ...
            let mut bd = 0i64;
            let mut bm = None;
            // g[a1] = ca1 + max_{a2 valid} (contrib[a2] + A[a1][a2]); since A[.][rm] >= 0,
            // every triple through a1 has delta <= g[a1] - c_rm. See module docs.
            let mut g4: Vec<i64> = Vec::with_capacity(best_unused.len());
            for &a1 in &best_unused {
                let mut best = i64::MIN;
                for &a2 in &tsn.friends[a1] {
                    if state.selected_bit[a2] || a1 == a2 { continue; }
                    let v = state.contrib[a2] as i64
                        + unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(a2) } /* H259 */ as i64;
                    if v > best { best = v; }
                }
                g4.push(if best == i64::MIN { i64::MIN } else { state.contrib[a1] as i64 + best });
            }
            // g4 is independent of rm, so max(g4) gates the whole middle loop. See module docs.
            let g4max = g4.iter().copied().max().unwrap_or(i64::MIN);
            for &rm in &worst_used {
                let c_rm = state.contrib[rm] as i64;
                ...
                for (ai, &a1) in best_unused.iter().enumerate() {
                    ...
                    let wa1 = state.ch.weights[a1];
                    if wa1 >= budget { continue; }
                    let ca1_eff = ca1 - unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(rm) } /* H259 */ as i64;
                    for &a2 in &tsn.friends[a1] {
                        if state.selected_bit[a2] || a1 == a2 { continue; }
                        let wa2 = state.ch.weights[a2];
                        if wa1 + wa2 > budget { continue; }
                        let delta = ca1_eff + state.contrib[a2] as i64
                            ...
                            + unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(a2) } /* H259 */ as i64 - c_rm;
                        if delta > bd && state.total_weight - w_rm + wa1 + wa2 <= cap {
                            bd = delta; bm = Some((rm, a1, a2));
                        }
                    }
...

        {
            ...
            let mut bd = 0i64;
            let mut bm = None;
            // h[r1] = max_{r2 valid} (A[r1][r2] - contrib[r2]); since A[add][.] >= 0,
            // every triple through r1 has delta <= c_add - cr1 + h[r1]. See module docs.
            let mut h5: Vec<i64> = Vec::with_capacity(worst_used.len());
            for &r1 in &worst_used {
                let mut best = i64::MIN;
                for &r2 in &tsn.friends[r1] {
                    if !state.selected_bit[r2] || r1 == r2 { continue; }
                    let v = unsafe { *state.ch.interaction_values.get_unchecked(r1).get_unchecked(r2) } /* H259 */ as i64
                        - state.contrib[r2] as i64;
                    if v > best { best = v; }
                }
                h5.push(best);
            }
            // gate regroups as c_add + (h5[ri] - cr1) <= bd; m5 = max over ri. See module docs.
            let mut m5 = i64::MIN;
            for (ri, &r1) in worst_used.iter().enumerate() {
                if h5[ri] == i64::MIN { continue; }
                let v = h5[ri] - state.contrib[r1] as i64;
                if v > m5 { m5 = v; }
            }
...

        {
            ...
            for i in 0..ks {
                ...
                for &r2 in &tsn.friends[r1] {
                    ...
                    let cr2 = state.contrib[r2] as i64;
                    let wr2 = state.ch.weights[r2];
                    let lost = cr1 + cr2 - unsafe { *state.ch.interaction_values.get_unchecked(r1).get_unchecked(r2) } /* H259 */ as i64;
                    let budget = state.slack() + wr1 + wr2;
                    for u in 0..ku {
...
fn build_det_seeds(challenge: &Challenge, hp: &Hparams, tsn_ref: Option<&TopNeighbors>, total_interactions: &[i64]) -> Vec<SolState> {
...
fn run_one_instance(challenge: &Challenge, hp: &Hparams, rng_offset: usize, shared_tsn: Option<&TopNeighbors>, total_interactions: &[i64], det_seeds: Option<&[SolState]>) -> (Solution, i64) {
    run_one_instance_seeded(challenge, hp, rng_offset, shared_tsn, total_interactions, None, det_seeds)
}

fn run_one_instance_seeded(challenge: &Challenge, hp: &Hparams, rng_offset: usize, shared_tsn: Option<&TopNeighbors>, total_interactions: &[i64], seed_sol: Option<&Solution>, det_seeds: Option<&[SolState]>) -> (Solution, i64) {
...
fn eval_solution(ch: &Challenge, sol: &Solution) -> i64 {
    ...
    for &i in &sol.items {
        ...
        for &j in &sol.items {
            if j > i { val += unsafe { *ch.interaction_values.get_unchecked(i).get_unchecked(j) } /* H259 */ as i64; }
        }
    }
    val
}
    ...
    pub fn solve(
        ...
        let total_interactions: Vec<i64> = {
            ...
            for i in 0..n {
                ...
                debug_assert_eq!(row[i], 0, "H211 assumes a zero diagonal; the generator changed");
        ...
        if n > 1200 {
            ...
            for restart in 0..n_restarts {
                let (sol, val) = if restart >= 2 && restart % 2 == 0 && best_sol.is_some() {
                    run_one_instance_seeded(challenge, &hp, restart, Some(&tsn), &total_interactions, best_sol.as_ref(), Some(&det_seeds))
                } else {
                    run_one_instance(challenge, &hp, restart, Some(&tsn), &total_interactions, Some(&det_seeds))
                };
                if val > best_quality {
                    best_quality = val;
                    best_sol = Some(sol);
        ...
        for restart in 0..n_phase1 {
            let (sol, val) = run_one_instance(challenge, &hp, restart, None, &total_interactions, None);
        ...
        {
            let (sol, val) = run_one_instance_seeded(challenge, &hp, 99, None, &total_interactions, Some(&elite[0].0), None);
            elite.push((sol, val));
            elite.sort_by_key(|&(_, v)| std::cmp::Reverse(v));
            elite.truncate(8);
        }
        ...
        for lns_round in 0..7 {
            ...
            let mut candidates: Vec<(usize, i64)> = sel.iter().map(|&i| {
                let score = match lns_round % 10 {
                    ...
                    3 => { let w = (state.ch.weights[i] as i64).max(1); dw(state.contrib[i] as i64 * 1000, w) },
