...
mod inner {
    ...
    mod hpf {
        ...
        pub fn get_breakpoints(inst: &P1Instance, n_lambda_values: usize) -> BreakpointSets {
            ...
            let mut state = HpfState {
                num_nodes, source: 1, sink: 2, num_params, highest_strong_label: 1, hsl_hi: 1,
    ...
    fn dp_refinement_hp(state: &mut State, core_half: usize) {
        ...
        let h297_ok = weights.iter().all(|&w| w >= 1);
        for _iter in 0..4 {
            for i in 0..n { by_density[i] = i; }
            ...
            for (idx, &i) in by_density.iter().enumerate() {
                ...
                if w <= rem {
                    ...
                    if h297_ok && rem == 0 {
                        ...
                        // at the sentinel `n`.
                        if idx_first_rejected == n { idx_first_rejected = (idx + 1).min(n); }
                        break;
                    }
                }
    ...
    fn apply_pair_add(state: &mut State) -> bool {
        ...
        for ai in 0..m {
            ...
            for bi in (ai+1)..m {
                ...
                let delta = ca + state.contrib[b] as i64 + unsafe { *state.ch.interaction_values.get_unchecked(a).get_unchecked(b) } /* H259 */ as i64;
    ...
    fn apply_chain_move(state: &mut State) -> bool {
        ...
        for i_rm in 0..sel_len {
            ...
            for ui in 0..unsel_len {
                ...
                let c_a1 = state.contrib[a1] as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(rm) } /* H259 */ as i64;
                for uj in (ui+1)..unsel_len {
                    let a2 = unsel[uj].0; let w_a2 = state.ch.weights[a2] as i64;
                    if w_a1 + w_a2 > budget { continue; }
                    let c_a2 = state.contrib[a2] as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a2).get_unchecked(rm) } /* H259 */ as i64;
        ...
        for si in 0..sel_ranked.len() {
            ...
            for sj in (si+1)..sel_ranked.len() {
                ...
                for ui in 0..unsel_ranked.len() {
                    ...
                    if w_a1 > budget { continue; }
                    let c_a1 = state.contrib[a1] as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(r1) } /* H259 */ as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a1).get_unchecked(r2) } /* H259 */ as i64;
                    for uj in (ui+1)..unsel_ranked.len() {
                        let a2 = unsel_ranked[uj].0; let w_a2 = state.ch.weights[a2] as i64;
                        if w_a1 + w_a2 > budget { continue; }
                        let c_a2 = state.contrib[a2] as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a2).get_unchecked(r1) } /* H259 */ as i64 - unsafe { *state.ch.interaction_values.get_unchecked(a2).get_unchecked(r2) } /* H259 */ as i64;
    ...
    fn simulated_annealing(state: &mut State, rng: &mut Rng, n_rounds: usize, n_iter: usize) {
        ...
        for _ in 0..100 {
            ...
            let d = state.contrib[add] as f64 - state.contrib[rm] as f64 - unsafe { *state.ch.interaction_values.get_unchecked(add).get_unchecked(rm) } /* H259 */ as f64;
        ...
        for _ in 0..n_rounds {
            for _ in 0..n_iter {
                ...
                if coin < 8 {
                    ...
                    let delta = state.contrib[add] as i64 - state.contrib[rm] as i64 - unsafe { *state.ch.interaction_values.get_unchecked(add).get_unchecked(rm) } /* H259 */ as i64;
    ...
    fn crossover_frequency(population: &[SolState], ch: &Challenge, rng: &mut Rng) -> Vec<bool> {
        ...
        for sol in population { for i in 0..n { freq[i] += sol.bits[i] as usize; } }
        ...
        let mut consensus: Vec<usize> = Vec::with_capacity(n); let mut exploratory: Vec<usize> = Vec::with_capacity(n);
        for i in 0..n {
    ...
    fn local_search_vnd_windowed(state: &mut State, window_k: usize) {
        for _ in 0..80 {
            ...
            }
            {
                ...
                // cand-side dual of the cmax prune: acceptance needs c[cand] > c[rm], and feasibility
                // forces any feasible rm to have w_rm >= w_cand - slack. rmin is the weight-bucketed
                // SUFFIX min of c[rm]; a candidate at or below it cannot beat any feasible rm. The
                // test uses only `d > 0`, not the running bd, so it is a permanent per-pass filter.
                // Order-preserving: acceptance is a strict `>` so scan order decides ties.
                ...
                for &rm in &worst_used {
                    ...
                    if cmax[(max_w as usize).clamp(1, 10)] <= state.contrib[rm] { continue; }
                ...
                if m >= 2 {
                    ...
                    for ai in 0..m {
                        ...
                        for bi in (ai+1)..m {
                            let b = fits[bi];
                            if wa + state.ch.weights[b] > slack { continue; }
                            let d = ca + state.contrib[b] as i64 + unsafe { *state.ch.interaction_values.get_unchecked(a).get_unchecked(b) } /* H259 */ as i64;
            ...
            6 => {
                ...
                for i in 0..n {
                    if state.selected_bit[i] {
                        ...
                        else { removal_candidates.push((i, -(unsafe { *state.ch.interaction_values.get_unchecked(i).get_unchecked(seed) } /* H259 */ as i64))); }
                    }
                }
                removal_candidates.sort_unstable_by_key(|&(_, s)| s);
            },
            ...
            for i in 0..n {
                for j in (i+1)..n {
                    if challenge.weights[i] + challenge.weights[j] <= cap {
                        hub_pairs.push((unsafe { *challenge.interaction_values.get_unchecked(i).get_unchecked(j) } /* H259 */, i, j));
                    }
                }
            }
