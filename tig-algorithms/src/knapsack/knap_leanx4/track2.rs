...
mod inner {
    ...
    mod hpf {
        ...
        pub fn get_breakpoints(inst: &P1Instance,
            ...
            let mut state = HpfState {
                ...
                num_params, highest_strong_label: 1, hsl_hi: 1,
    ...
    impl P2Instance {
        #[inline] pub fn q_get(&self, i: usize, j: usize) -> i32 {
    ...
    impl P2State {
        pub fn new(ins: P2Instance) -> Self {
            ...
            P2State {
                ...
                best_value: NEG_INF, best_weight: 0, best_count: -1, ins,
            }
        }
            ...
            if limit >= 3 && used.len() >= 2 {
                ...
                for &(r1, r2, budget, lost) in &removed_pairs {
                    for x in 0..limit {
                        ...
                        if wa > budget { continue; }
                        let a_base = a * n;
                        let a_gain = contrib[a] as Ll - unsafe { *q.get_unchecked(a_base + r1) } /* H249 */ as Ll - unsafe { *q.get_unchecked(a_base + r2) } /* H249 */ as Ll;
                        for y in (x + 1)..limit {
                            let b = unused[y].id;
                            let wab = wa + w[b];
                            if wab > budget { continue; }
                            let b_base = b * n;
                            let b_gain = contrib[b] as Ll - unsafe { *q.get_unchecked(b_base + r1) } /* H249 */ as Ll - unsafe { *q.get_unchecked(b_base + r2) } /* H249 */ as Ll;
                            for z in (y + 1)..limit {
                                let c = unused[z].id;
    ...
    fn apply_best_swap12(s: &mut P2State, window_k: usize) -> bool {
        ...
        for si in 0..cs {
            ...
            for x in 0..cu {
                let id = unused[x].id;
                effective[x] = s.contrib[id] as Ll - unsafe { *s.ins.q.get_unchecked(id * n_items + rm) } /* H249 */ as Ll;
            }
            ...
            for x in 0..cu {
                ...
                let row_a = unsafe { /* H195 */ std::slice::from_raw_parts(s.ins.q.as_ptr().add(a * n_items), n_items) };
                for y in (x+1)..cu {
                    let b = unused[y].id;
                    if wa + s.ins.w[b] > budget { continue; }
    ...
    fn apply_best_swap21(s: &mut P2State, window_k: usize) -> bool {
        ...
        for ai in 0..cu {
            ...
            for &(r1, r2, budget, lost) in &used_pairs {
                ...
                let gained = cadd - unsafe { *q.get_unchecked(add_base + r1) } /* H249 */ as Ll
    ...
    fn apply_best_swap22(s: &mut P2State, window_k: usize) -> bool {
        ...
        for a in 0..cu {
            ...
            for b in (a+1)..cu {
                ...
                let gain = contrib[p] as Ll + contrib[q2] as Ll
                           + unsafe { *q.get_unchecked(p_base + q2) } /* H249 */ as Ll;
                unused_pairs.push((p, q2, p_base, q2_base, wp, pair_w, gain));
            }
        }

        // H244: scalar extrema, no vectors. `gain` depends only on the inner index, `lost`
        // only on the outer, and cross >= 0 (q >= 0), so d <= gmax - lost for every candidate.
        let mut s22_gmax: Ll = Ll::MIN;
        for p in unused_pairs.iter() { if p.6 > s22_gmax { s22_gmax = p.6; } }
        let mut s22_lmin: Ll = Ll::MAX;
        for p in used_pairs.iter() { if p.5 < s22_lmin { s22_lmin = p.5; } }
        // T1, one-shot: fires before the 31,200-trip cross_cache fill and its ~250 KB allocation.
        if !unused_pairs.is_empty() && !used_pairs.is_empty()
            && s22_gmax.saturating_sub(s22_lmin) <= 0 { return false; }
        ...
        for (pair_idx, &(_, _, p_base, q2_base, _, _, _)) in unused_pairs.iter().enumerate() {
            ...
            for (used_idx, rm_score) in used.iter().enumerate() {
                let rm = rm_score.id;
                cache_row[used_idx] = unsafe { *q.get_unchecked(p_base + rm) } /* H249 */ as Ll + unsafe { *q.get_unchecked(q2_base + rm) } /* H249 */ as Ll;
            }
        }
        ...
        for &(r1_idx, r2_idx, r1, r2, budget, lost) in &used_pairs {
            // T2, one per OUTER trip: kills this trip's whole 780-trip inner loop. Exact, and it
            // tightens as best_delta rises. Same shape as apply_best_swap21's guard at :1903.
            // ⚠ Deliberately a `continue` against a SCALAR, not a `break` against a suffix-min
            // vector -- H227's break depended on used_pairs' lexicographic order and cost +8.5%.
            if s22_gmax.saturating_sub(lost) <= best_delta { continue; }
