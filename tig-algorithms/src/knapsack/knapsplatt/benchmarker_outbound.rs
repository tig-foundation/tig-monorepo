...
#[inline]
fn weight_of(ch: &SubInstance, items: &[usize]) -> i64 {
    items.iter().map(|&i| ch.weights[i] as i64).sum()
}

fn round0_scores(ch: &SubInstance, out: &mut [i32]) {
...
impl<'a> State<'a> {
    ...
    fn restore_snapshot(&mut self, snapshot_sel: &[usize], snapshot_contrib: Vec<i32>, snap_value: i64) {
...
fn build_initial_solution(state: &mut State, order_scores: &[i32]) {
    ...
    for &i in &order {
        ...
        if state.total_weight + w <= state.capacity() { state.add_item(i); }
    }
}
...
fn polish_once(state: &mut State, params: &Params) {
    ...
    for &i in &idx { if !state.selected_bit[i] { unused_top.push(i); if unused_top.len()>=k { break; } } }
    let mut used_worst: Vec<usize> = Vec::new();
    for &i in idx.iter().rev() { if state.selected_bit[i] { used_worst.push(i); if used_worst.len()>=k { break; } } }
...
fn strategic_perturb_and_rebuild(state: &mut State, params: &Params) {
    ...
    for i in 0..rem {
        let r = bad[i];
        if state.selected_bit[r] { state.remove_item(r); }
    }
...
fn local_search_vnd(state: &mut State, params: &Params) {
    ...
    loop {
        if apply_best_add_windowed(state, &mut best_unused, &mut worst_used) { continue; }
...
fn solve_sub_instance(_sid: usize, ch: &SubInstance) -> (SubSolution, i32) {
    ...
    round0_scores(ch, &mut build_scores);
