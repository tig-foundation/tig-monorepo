...
mod inner_four {
    ...
    fn run_one_instance(challenge: &Challenge, hp: &Hparams, rng_offset: usize) -> Solution {
        ...
        let mut cnt = hp.ils_rounds;
            ...
            let strategy = if stall_count >= 9 { 6 } else { round % 8 };
            let strength = 4 + round / 5;
                    perturb_by_strategy(&mut state, strength, stall_count, strategy, &mut rng, hp);
                    greedy_reconstruct(&mut state, if stall_count >= 9 { 0 } else { strategy });
                    vnd_dispatch(&mut state, hp);
    ...
    impl Solver {
        pub fn solve(
            ...
            // === P9 contrib-guided escape post-solve (K=35, N=3 passes) ===
            {
                let p9_k: usize = 31;
                let mut rng_p9 = {
                ...
                for _pass in 0..3usize {
                    ...
                    // Sort selected by contrib ascending (remove least-contributing K)
                    ...
                    // Sort complement by contrib descending, take top 2K, shuffle, add up to K
