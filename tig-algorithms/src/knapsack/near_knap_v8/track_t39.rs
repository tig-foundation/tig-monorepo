...
impl<'a> State<'a> {
    ...
    #[inline(always)]
    pub fn sparse_inter(&self, i: usize, j: usize) -> i16 {
        if let Some(ref ng) = self.neigh {
            let j_u16 = j as u16;
...
impl Params {
    pub fn initialize(_h: &Option<Map<String, Value>>) -> Self {
        Self {
            n_perturbation_rounds: 8,
            perturbation_strength_base: 6,
            extra_starts: 0,
    ...
    if n >= 2500 {
        if let Some(neigh) = state.neigh {
            ...
            for &i in state.hubs_static.iter().take(lim) {
                if state.selected_bit[i] || Some(i) == state.banned_item {
                    continue;
                }
                ...
                if slack >= 2 && !hubs_g.is_empty() {
                    ...
                    for t in 0..lim {
                        ...
                        for u in 0..pref {
                            let (bb, vv) = row[u];
                            let b = bb as usize;
                            if a == b || state.selected_bit[b] || Some(b) == state.banned_item {
                                continue;
                            }
    ...
    if state.total_weight == 0 {
        ...
        if let Some(ref ng) = state.neigh {
            ...
            for _ in 0..samples {
                ...
                if i == j || Some(i) == state.banned_item || Some(j) == state.banned_item {
                    continue;
                }
...
pub fn construct_frontier_cluster_grow(state: &mut State, rng: &mut Rng) {
    ...
    if let Some(i) = best_seed {
        ...
        if let Some(ref ng) = state.neigh {
            ...
            for &(jj, vv) in row.iter() {
                let j = jj as usize;
                if j == i || state.selected_bit[j] || Some(j) == state.banned_item {
                    continue;
                }
    ...
    loop {
        ...
        for &u in &frontier {
            if state.selected_bit[u] || Some(u) == state.banned_item {
                continue;
            }
                ...
                if state.selected_bit[seed] && slack1 >= 1 {
                    ...
                    if let Some(ref ng) = state.neigh {
                        ...
                        for t in 0..pref {
                            let (jj, vv) = row[t];
                            let j = jj as usize;
                            if j == seed || state.selected_bit[j] || Some(j) == state.banned_item {
                                continue;
                            }
                            let wj = state.ch.weights[j];
                            if wj == 0 || wj > slack1 {
...
pub fn dp_refinement_x(state: &mut State, passes_multiplier: usize) {
    ...
    // DP Refinement applies exact micro-optimizations within the core window.
    // Adaptive noise introduces variance based on total_interactions to break cyclic local optima.
    let mut pseudo_state = (state.total_value as u64)
        .wrapping_add(state.total_weight as u64)
        ...
        if k != 0 {
            ...
            if let Some(ref ng) = state.neigh {
                for &it in &state.window_core {
                    ...
                    pseudo_state ^= pseudo_state << 13;
                    pseudo_state ^= pseudo_state >> 7;
                    pseudo_state ^= pseudo_state << 17;
                    
                    let interaction_factor = (state.total_interactions[it] / 2000).clamp(0, 127) as u32;
                    let noise_mask = 0x3F | interaction_factor;
                    let noise = (pseudo_state as u32 & noise_mask) as i32;
                    
                    pseudo_state ^= pseudo_state << 13;
                    pseudo_state ^= pseudo_state >> 7;
                    pseudo_state ^= pseudo_state << 17;
                    let usage_penalty = if state.usage[it] > 2 { 
                        ((pseudo_state as u32 & 0x1F) as i32) * (state.usage[it] as i32) / 2 
                    } else { 
                        0 
                    };
                    v0 += noise - usage_penalty;
...
pub fn micro_qkp_refinement(state: &mut State) {
    ...
    if let Some(ref ng) = state.neigh {
        ...
        let g = guides.len().min(if big_team { 6 } else { 10 });
        for t in 0..g {
            let vtx = guides[t];
            let row = unsafe { ng.get_unchecked(vtx) };
            let pref = row.len().min(if big_team { 24 } else { 48 });
            for u in 0..pref {
    ...
    let extra_r = state.window_rejected.len().min(if big_team { 24 } else { 48 });
    ...
    let extra_l = state.window_locked.len().min(48);
    ...
    let mut stack: Vec<(usize, u32, i64, usize)> = Vec::with_capacity(k * 2);
    stack.push((0, 0, 0, 0));
...
fn two_two_intensify(state: &mut State, window_k: usize, max_fuel: usize, max_accepts: usize) -> bool {
    let n = state.ch.num_items;
    let mut improved = false;
    let mut fuel: usize = 0;

    for _ in 0..max_accepts {
        if fuel >= max_fuel { break; }

        let mut u_list: Vec<(usize, f64)> = Vec::new();
        let mut s_list: Vec<(usize, f64)> = Vec::new();
        for i in 0..n {
            let r = state.contrib[i] as f64 / (state.ch.weights[i] as f64).max(1.0);
            if state.selected_bit[i] { s_list.push((i, r)); }
            else { u_list.push((i, r)); }
        }
        let ku = window_k.min(u_list.len());
        if ku > 0 { u_list.select_nth_unstable_by(ku - 1, |a, b| b.1.partial_cmp(&a.1).unwrap()); }
        let ks = window_k.min(s_list.len());
        if ks > 0 { s_list.select_nth_unstable_by(ks - 1, |a, b| a.1.partial_cmp(&b.1).unwrap()); }
        let unsel: Vec<usize> = u_list.into_iter().take(ku).map(|(i,_)| i).collect();
        let sel: Vec<usize> = s_list.into_iter().take(ks).map(|(i,_)| i).collect();

        if sel.len() < 2 || unsel.len() < 2 { return improved; }

        let mut add_pairs: Vec<(usize, usize, i64, u32)> = Vec::new();
        for ai in 0..unsel.len() {
            for aj in (ai+1)..unsel.len() {
                let a = unsel[ai]; let b = unsel[aj];
                let ib = state.sparse_inter(a, b);
                let gain = state.contrib[a] as i64 + state.contrib[b] as i64 + ib as i64;
                let w = state.ch.weights[a] + state.ch.weights[b];
                add_pairs.push((a, b, gain, w));
            }
        }
        add_pairs.sort_unstable_by_key(|p| std::cmp::Reverse(p.2));

        let mut rm_pairs: Vec<(usize, usize, i64, u32)> = Vec::new();
        for ri in 0..sel.len() {
            for rj in (ri+1)..sel.len() {
                let r1 = sel[ri]; let r2 = sel[rj];
                let ir = state.sparse_inter(r1, r2);
                let loss = state.contrib[r1] as i64 + state.contrib[r2] as i64 - ir as i64;
                let w = state.ch.weights[r1] + state.ch.weights[r2];
                rm_pairs.push((r1, r2, loss, w));
            }
        }
        rm_pairs.sort_unstable_by_key(|p| p.2);

        let mut best_delta: i64 = 0;
        let mut best_move: Option<(usize, usize, usize, usize)> = None;

        for &(r1, r2, loss, w_rm) in &rm_pairs {
            if !state.selected_bit[r1] || !state.selected_bit[r2] { continue; }
            if add_pairs.is_empty() { break; }
            if add_pairs[0].2 - loss <= best_delta { break; }
            for &(a1, a2, gain, w_add) in &add_pairs {
                if state.selected_bit[a1] || state.selected_bit[a2] { continue; }
                if gain - loss <= best_delta { break; }
                if state.total_weight as i64 - w_rm as i64 + w_add as i64 > state.ch.max_weight as i64 { continue; }
                let cross = (state.sparse_inter(r1, a1) + state.sparse_inter(r1, a2)
                           + state.sparse_inter(r2, a1) + state.sparse_inter(r2, a2)) as i64;
                let delta = gain - loss - cross;
                fuel += 1;
                if fuel >= max_fuel { break; }
                if delta > best_delta { best_delta = delta; best_move = Some((r1, r2, a1, a2)); }
            }
            if fuel >= max_fuel { break; }
        }
        ...
        if let Some((r1, r2, a1, a2)) = best_move {
            ...
            frontier_push(a1); frontier_push(a2); frontier_push(r1); frontier_push(r2);
            improved = true;
        } else { break; }
    }
    improved
...
#[allow(dead_code)]
fn two_two_exchange_phase(state: &mut State) {
    two_two_intensify(state, 40, 8000, 3);
}
...
pub fn run_one_instance(challenge: &Challenge, params: &Params) -> Solution {
    ...
    for sid in 0..n_starts {
        ...
        if sid > 0 && !hubs_static.is_empty() {
            let ban_idx = (rng.next_u32() as usize) % hubs_static.len().min(16);
            st.banned_item = Some(hubs_static[ban_idx]);
        }
        ...
        rebuild_windows(&mut st);
        micro_qkp_refinement(&mut st);
        two_two_intensify(&mut st, 40, 8000, 3);
    ...
    if best.is_some() && second.is_some() {
        ...
        if union_cnt > 0 && (inter_cnt * 100) / union_cnt <= 90 {
            let mut hyb = State::new_empty(challenge, &total_pre, &hubs_static, neigh_pre.as_ref());
            {
                let b1 = best.as_ref().unwrap();
                let b2 = second.as_ref().unwrap();
            ...
            if hyb.total_weight > challenge.max_weight {
                if n >= 2500 {
                    // Information Sharing: penalize removing highly connected backbone items during union path relinking
                    ...
                    for i in 0..n {
                        ...
                        let w = challenge.weights[i] as i64;
                        let support = hyb.support[i] as i64;
                        let s = if w > 0 { (c * 1000) / w } else { c * 1000 } + support * 50;
                        sel.push((s, i));
                    }
    ...
    for perturbation_round in 0..max_rounds {
...

        {
            ...
            two_two_intensify(&mut state_int, 40, 8000, 3);
        ...
        let freeze_div = !is_last_round
            && (stall_div > stall_int + 1 || (state_int.total_value >= state_div.total_value && stall_div > 0));

        if freeze_div {
            // INFORMATION SHARING / PATH RELINKING (IMPROVED)
            // Inherit common features strictly, but drop a tiny fraction to shatter deep local optima.
            // For items unique to one state, use interaction support to bias the probability, 
            // aggressively pulling the best structural items from state_int into state_div.
            let mut new_div = State::new_empty(challenge, &total_pre, &hubs_static, neigh_pre.as_ref());
            new_div.usage.clone_from(&state_div.usage);

            // Information Sharing / Path Relinking (Union-Trim Strategy)
            // Combine both state_int and state_div into a single overloaded state,
            // then carefully trim the worst items until valid.
            for i in 0..n {
                let in_int = state_int.selected_bit[i];
                let in_div = state_div.selected_bit[i];
                // Introduce small noise to drop common items and break locked cores
                if in_int && in_div && (rng.next_u32() % 64) == 0 {
                    continue;
                }
                if in_int || in_div {
                    new_div.add_item(i);
            ...
            if new_div.total_weight > challenge.max_weight {
                if n >= 2500 {
                    let mut sel: Vec<(i64, usize)> = Vec::with_capacity(n);
                    for i in 0..n {
                        if !new_div.selected_bit[i] {
                            continue;
                        }
                        let c = new_div.contrib[i] as i64;
                        let w = challenge.weights[i] as i64;
                        let support = new_div.support[i] as i64;
                        let s = if w > 0 { (c * 1000) / w } else { c * 1000 } + support * 50;
                        sel.push((s, i));
                    }
                    sel.sort_unstable_by(|a, b| a.0.cmp(&b.0));
                    for &(_s, i) in &sel {
                        ...
                        if new_div.selected_bit[i] {
                            new_div.remove_item(i);
                        }
                    }
                } else {
                    while new_div.total_weight > challenge.max_weight {
                        let mut worst_item: Option<usize> = None;
                        let mut worst_score: i64 = i64::MAX;
                        for i in 0..n {
                            if !new_div.selected_bit[i] {
                            ...
                            let c = new_div.contrib[i] as i64;
                            let w = new_div.ch.weights[i] as i64;
                            let support = new_div.support[i] as i64;
                            let s = if w > 0 { (c * 1000) / w } else { c * 1000 } + support * 50;
                            if s < worst_score {
                                worst_score = s;
            ...
            greedy_fill_with_beta(&mut new_div, &mut rng, 0x1F, true);
            rebuild_windows(&mut new_div);
            dp_refinement_x(&mut new_div, params.dp_passes_multiplier);
            rebuild_windows(&mut new_div);
            micro_qkp_refinement(&mut new_div);
            one_two_exchange_phase(&mut new_div);
            two_two_intensify(&mut new_div, 40, 8000, 3);
            local_search_vnd(&mut new_div, params);

            if new_div.total_value > state_div.total_value {
                state_div = new_div;
                stall_div = 0;
            ...
            two_two_intensify(&mut state_div, 40, 8000, 3);
        ...
        if state_div.total_value > state_int.total_value {
            ...
            state_int = state_div;
            state_div = tmp_state;
            let tmp_stall = stall_int;
            stall_int = stall_div;
            stall_div = tmp_stall;
            let tmp_tabu = tabu_int;
            tabu_int = tabu_div;
            tabu_div = tmp_tabu;
            seed_frontier_from_state(&state_int);
            dp_next_int = true;
