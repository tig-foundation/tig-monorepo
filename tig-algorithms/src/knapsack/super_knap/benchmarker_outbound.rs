...
fn centrality_boosted_scores(ch: &SubInstance, out: &mut [i32]) {
...
impl<'a> State<'a> {
    fn new_empty(ch: &'a SubInstance) -> Self {
        let n = ch.difficulty.num_items;
    ...
    fn selected_items(&self) -> Vec<usize> {
        (0..self.ch.difficulty.num_items).filter(|&i| self.selected_bit[i]).collect()
    }
    #[inline] fn capacity(&self) -> i64 { self.ch.max_weight as i64 }
    #[inline] fn slack(&self) -> i64 { self.capacity() - self.total_weight }
    fn add_item(&mut self, i: usize) {
        ...
        for k in 0..n { self.contrib[k] += self.ch.interaction_values[k][i] as i32; }
    }
    fn remove_item(&mut self, j: usize) {
        self.total_value -= self.contrib[j] as i64;
        self.total_weight -= self.ch.weights[j] as i64;
        let n = self.ch.difficulty.num_items;
        for k in 0..n { self.contrib[k] -= self.ch.interaction_values[k][j] as i32; }
        self.selected_bit[j] = false;
    }
    fn replace_item(&mut self, rm: usize, cand: usize) {
        let w_c = self.ch.weights[cand] as i64;
        if self.slack() >= w_c { self.add_item(cand); self.remove_item(rm); }
        else { self.remove_item(rm); self.add_item(cand); }
    }
    fn restore_snapshot(&mut self, snapshot_sel: &[usize], snapshot_contrib: &[i32], snap_value: i64, snap_weight: i64) {
    ...
    fn remove_from_vec(v: &mut Vec<usize>, x: usize) {
        if let Some(pos) = v.iter().position(|&y| y == x) { v.swap_remove(pos); }
    }
}
...
fn savings_construction(state: &mut State, order_scores: &[i32]) {
    ...
    let top_k = if n <= 600 { 64 } else if n <= 1200 { 96 } else if n <= 2000 { 112 } else { 128 };
...
fn integer_core_target(ch: &SubInstance, contrib: &[i32], core_half_dp: usize) -> Vec<usize> {
    let n = ch.difficulty.num_items;
    let mut order: Vec<usize> = (0..n).collect();
    order.sort_unstable_by(|&a, &b| {
    ...
    for (pos, &i) in order.iter().enumerate() {
        ...
        if pref_w + w > ch.max_weight as i64 { break_idx = pos; break; }
        pref_w += w;
    }
...
fn apply_dp_target_via_ops(state: &mut State, target_sel: &[usize]) {
    ...
    for &i in target_sel {
        if !state.selected_bit[i] { to_add.push(i); }
    }
...
fn build_ls_windows(state: &State, core_half_ls: usize) -> (Vec<usize>, Vec<usize>) {
    ...
    for &i in &order {
        if !state.selected_bit[i] {
            best_unused.push(i);
            if best_unused.len() >= core_half_ls { break; }
        }
    }
    ...
    for &i in order.iter().rev() {
        if state.selected_bit[i] {
            worst_used.push(i);
            if worst_used.len() >= core_half_ls { break; }
        }
    }
...
fn apply_best_add_windowed(
    ...
    for &cand in &*best_unused {
        ...
        if delta > 0 && best.map_or(true, |(_,bd)| delta > bd) { best = Some((cand, delta)); }
    }
...
fn polish_once(state: &mut State, params: &Params) {
    ...
    if !unsel.is_empty() {
        ...
        if unused_top.len() > kk { unused_top.truncate(kk); }
    }
    ...
    if !sel.is_empty() {
        ...
        if used_worst.len() > kk { used_worst.truncate(kk); }
    }
    ...
    if slack0 > 0 {
        for &i in &unused_top {
            ...
            if w <= slack0 {
                ...
                if d > 0 && best_add.map_or(true, |(_,bd)| d > bd) { best_add = Some((i,d)); }
            }
        }
    }
    ...
    for &rm in &used_worst {
        for &cand in &unused_top {
            if state.ch.weights[cand] != state.ch.weights[rm] { continue; }
            let d = (state.contrib[cand] as i64)
                - (state.contrib[rm] as i64)
                - (state.ch.interaction_values[cand][rm] as i64);
            if d > 0 && best_swap.map_or(true, |(_,_,bd)| d > bd) { best_swap = Some((cand,rm,d)); }
        }
    }
    ...
    for &rm in &used_worst {
        ...
        for &cand in &unused_top {
            ...
            if dw == 0 || dw > params.diff_lim { continue; }
            let d = (state.contrib[cand] as i64)
                - (state.contrib[rm] as i64)
                - (state.ch.interaction_values[cand][rm] as i64);
            if d > 0 && best_swap_red.map_or(true, |(_,_,bd)| d > bd) { best_swap_red = Some((cand,rm,d)); }
        }
    }
    ...
    if state.slack() > 0 {
        ...
        for &rm in &used_worst {
            ...
            for &cand in &unused_top {
                ...
                if w_c <= w_rm { continue; }
                let dw = w_c - w_rm;
                if dw as usize > params.diff_lim { continue; }
                if state.slack() < dw { continue; }
                let d = (state.contrib[cand] as i64)
                    - (state.contrib[rm] as i64)
                    - (state.ch.interaction_values[cand][rm] as i64);
...
fn large_neighborhood_destroy_repair(state: &mut State, params: &Params) {
    ...
    for &r in to_remove.clone().iter() {
        ...
        for &p in &sel {
            if p == r { continue; }
            if to_remove.contains(&p) { continue; }
...
fn ejection_chain_vnd(state: &mut State, params: &Params) {
    ...
    loop {
        ...
        if apply_best_swap_diff_reduce_windowed(state, params, &mut best_unused, &mut worst_used) { continue; }
        if apply_best_swap11_equal_windowed(state, &mut best_unused, &mut worst_used) { continue; }
        if apply_best_swap_diff_increase_windowed(state, params, &mut best_unused, &mut worst_used) { continue; }

        break;
    }
}

fn solve_sub_instance(_sid: usize, ch: &SubInstance) -> (SubSolution, i32) {
    let n = ch.difficulty.num_items;
    let params = Params::for_problem_size(n);
    let mut build_scores = vec![0i32; n];
    centrality_boosted_scores(ch, &mut build_scores);
    ...
    for _it in 0..params.n_maxils {
        ...
        let target = integer_core_target(ch, &state.contrib, params.core_half_dp);
    ...
    items.sort_unstable();
    (SubSolution { items }, state.total_value as i32)
}

pub fn solve_challenge(challenge: &Challenge) -> Result<Option<Solution>> {
    let mut solution = Solution {
        sub_solutions: vec![SubSolution { items: Vec::new() }; challenge.sub_instances.len()],
    };
    for (index, sub_instance) in challenge.sub_instances.iter().enumerate() {
        let (sub_solution, _best_value) = solve_sub_instance(index, sub_instance);
