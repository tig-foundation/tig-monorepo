...
impl Hparams {
        ...
        if n <= 1200 {
            if budget <= 5 {
                Self {
                    ...
                    use_heavy_polish: false, window_k: 300, core_half_dp: 60, restart_base: 0,
                }
            } else if budget <= 10 {
                Self {
                    n_random_starts: 4, n_crossover_gen: 12, sa_rounds: 0,
            ...
            Self {
                ...
                use_heavy_polish: false, window_k: 200, core_half_dp: 50, restart_base: 0,
            }
        }
    }
    ...
    fn from_map(h: &Option<Map<String, Value>>, n: usize, budget: u32) -> Self {
        ...
        if let Some(m) = h {
            ...
            if let Some(v) = m.get("restart_base").and_then(|v| v.as_u64()) { p.restart_base = v as usize; }
        }
        p
    }
}
...
fn run_one_instance(challenge: &Challenge, hp: &Hparams, rng_offset: usize, shared_tsn: Option<&TopNeighbors>, total_interactions: &[i64]) -> (Solution, i64) {
    ...
    let max_stall = if n > 1200 { (hp.ils_rounds / 3).max(10) } else { hp.ils_rounds };
    ...
    pub fn solve(
        ...
        if n > 1200 {
            ...
            for restart in 0..n_restarts {
                ...
                let (sol, val) = run_one_instance(challenge, &hp, actual, Some(&tsn), &total_interactions);
