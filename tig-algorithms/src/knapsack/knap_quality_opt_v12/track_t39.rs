...
// Challenge to phase-1 conversion
...
// Parametric max-flow min-cut
    ...
    mod hpf {
        ...
        pub fn get_breakpoints(inst: &Phase1Problem,
...
// Greedy constructors
    ...
    fn get_initial_node(right_nodes: &IntArray, um: &PairUtil,
    ...
    fn run_greedy_left(um:          &PairUtil,
                       n_nodes:     usize,
                       mut left:    IntArray,
    ...
    fn run_greedy_right(um:              &PairUtil,
                        n_nodes:         usize,
...
// Greedy dispatch
    ...
    fn run_greedy(inst: &Phase1Problem, beta: f64,
...
// Phase-2 problem instance
    ...
    pub fn bridge_p1_to_p2(inst: &Phase1Problem, budget: i32) -> P2Instance {
        let n = inst.n_items;
...
// Handoff from phase 1 to phase 2
    ...
    pub fn bridge_load_solution(s: &mut P2State, br: &BudgetOutcome) {
        s.clear();
...
// Local-search neighbourhood moves
...
// Iterated local search driver
    ...
    fn solve_hybrid(s:          &mut P2State,
        ...
        while round < ils_rounds {
            ...
            let mut iter = 0;
            // escalate perturbation after a long stall
            if stall >= 40{
                s.restore_best();
