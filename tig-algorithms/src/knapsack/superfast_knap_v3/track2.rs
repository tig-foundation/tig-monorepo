...
mod inner {
    ...
    mod hpf {
        ...
        #[inline(always)]
        unsafe fn capacity_at_param(arc: *mut HpfArc, param: f32) -> f32 {
            match (*arc).kind {
                ARC_EDGE => (*arc).base_capacity,
                ARC_SRC => {
                    let v = (*arc).wt - param * (*arc).cst;
                    if v > 0.0 { v } else { 0.0 }
                }
                ARC_SNK => {
                    let v = param * (*arc).cst - (*arc).wt;
                    if v > 0.0 { v } else { 0.0 }
                }
                _ => 0.0,
            }
        ...
        unsafe fn get_highest_strong_root(
            s: *mut HpfState,
            theparam: i32,
        ) -> *mut HpfNode {
        ...
        unsafe fn update_capacities(s: *mut HpfState, theparam: i32) {
            let param = (*s).params[theparam as usize];
            ...
            for i in 0..size {
                ...
                let new_capacity = capacity_at_param(arc, param);
        ...
        pub fn get_breakpoints(
            ...
            for nd in adjacency_list.iter_mut() {
                if nd.num_adjacent > 0 {
                    nd.out_of_tree.reserve_exact(nd.num_adjacent as usize);
                }
            }
            ...
            unsafe {
                ...
                for k in 0..n_edges {
                    (*s).arc_list[first].from =
                        &mut (*s).adjacency_list[inst.edges[k].i + 2];
                    (*s).arc_list[first].to =
                        &mut (*s).adjacency_list[inst.edges[k].j + 2];
    ...
    fn seed_empty_left(
        ...
        for nd in 0..interaction_totals.len() {
            ...
            let util = interaction_totals[nd] / weights[nd].max(1) as f64;
    ...
    fn run_greedy_left(
        ...
        if ws_valid && ws_cands_nonempty {
            ...
            if beta == 0.0 {
                ...
                for &nd in right_nodes {
                    ...
                    cand_nodes.push(nd);
                    cand_contribs.push(marginals[nd] / weights[nd].max(1) as f64);
                }
            } else {
                for &nd in right_nodes {
                    if left_mark[nd] {
                ...
                for &nd in &cand_nodes {
                    ...
                    contrib /= weights[nd].max(1) as f64;
        ...
        loop {
            ...
            for (k, &nd) in new_cands.iter().enumerate() {
                new_contribs[k] +=
                    (1.0 + 2.0 * beta) * um.get(nd, best_node) / weights[nd].max(1) as f64;
            }
    ...
    fn run_greedy(
        ...
        for bi in (0..n_budgets).rev() {
            ...
            let mut after_right = run_greedy_right(
                &um,
                n_nodes,
                right_init,
                budget,
    ...
    impl P2State {
        ...
        pub fn add_item(&mut self, i: usize) {
            ...
            self.value += self.ins.q_get(i, i) as Ll + self.contrib[i] as Ll;
            ...
            let n = self.ins.n;
            let start = i * n;
            let row = &self.ins.q[start..start + n];

            for j in 0..n {
                if j != i {
                    self.contrib[j] += row[j];
                }
            }
        }
    ...
    fn build_best_unused(s: &P2State, max_k: usize) -> Vec<ItemScore> {
        ...
        let mut density: Vec<ItemScore> = (0..n)
            ...
            .map(|i| {
                ...
                let gain = s.ins.q_get(i, i) as Ll + s.contrib[i] as Ll;
        ...
        let mut marginal: Vec<ItemScore> = (0..n)
            ...
            .map(|i| ItemScore {
                id: i,
                score: s.ins.q_get(i, i) as Ll + s.contrib[i] as Ll,
            })
            .collect();
        ...
        for is in marginal {
            if !present[is.id] {
                ...
                let gain = s.ins.q_get(is.id, is.id) as Ll + s.contrib[is.id] as Ll;
    ...
    fn build_worst_used(s: &P2State, max_k: usize) -> Vec<ItemScore> {
        ...
        let mut density: Vec<ItemScore> = (0..n)
            ...
            .map(|i| {
                let w = s.ins.w[i].max(1) as Ll;
                let loss = s.ins.q_get(i, i) as Ll + s.contrib[i] as Ll;

                ItemScore {
                    id: i,
                    score: loss * 1_000_000 / w,
                }
            })
            .collect();
        ...
        for is in marginal {
            if !present[is.id] {
                let w = s.ins.w[is.id].max(1) as Ll;
                let loss = s.ins.q_get(is.id, is.id) as Ll + s.contrib[is.id] as Ll;
    ...
    fn apply_best_add1(s: &mut P2State) -> bool {
        ...
        for i in 0..n {
            ...
            let d = s.ins.q_get(i, i) as Ll + s.contrib[i] as Ll;
    ...
    fn apply_best_add2(s: &mut P2State, window_k: usize) -> bool {
        ...
        for x in 0..cu {
            ...
            let ca = s.ins.q_get(a, a) as Ll + s.contrib[a] as Ll;
    ...
    fn build_synergy_unused(s: &P2State) -> Vec<ItemScore> {
        ...
        for x in 0..ranked.len() {
            ...
            for y in (x + 1)..ranked.len() {
                ...
                let score = s.ins.q_get(a, a) as Ll
                    + s.contrib[a] as Ll
                    + s.ins.q_get(b, b) as Ll
                    + s.contrib[b] as Ll
                    + s.ins.q_get(a, b) as Ll;
        ...
        for (_, a, b) in pairs {
            for id in [a, b] {
                if !present[id] && pool.len() < POOL_K {
                    ...
                    let gain = s.ins.q_get(id, id) as Ll + s.contrib[id] as Ll;
        ...
        if pool.len() < POOL_K {
            let mut extra: Vec<ItemScore> = (0..s.ins.n)
                ...
                .map(|i| {
                    ...
                    ItemScore {
                        id: i,
                        score: s.ins.q_get(i, i) as Ll + s.contrib[i] as Ll + best_pair,
                    }
                })
                .collect();
...

        {
            ...
            if unused.len() >= 2 && !used.is_empty() {
                ...
                for rm_score in &used {
                    ...
                    let lost = s.ins.q_get(rm, rm) as Ll + s.contrib[rm] as Ll;
                    ...
                    for x in 0..unused.len() {
                        ...
                        let a_gain = s.ins.q_get(a, a) as Ll
                            + s.contrib[a] as Ll
                            - s.ins.q_get(a, rm) as Ll;
                ...
                for rm_score in &used {
                    ...
                    for x in 0..limit {
                        ...
                        for y in (x + 1)..limit {
                            ...
                            let b_gain = s.ins.q_get(b, b) as Ll
                                + s.contrib[b] as Ll
                                - s.ins.q_get(b, rm) as Ll;
            ...
            if limit >= 3 && used.len() >= 2 {
                ...
                for x in 0..used.len() {
                    ...
                    for y in (x + 1)..used.len() {
                        ...
                        let lost = s.ins.q_get(r1, r1) as Ll
                            + contrib[r1] as Ll
                            + s.ins.q_get(r2, r2) as Ll
                            + contrib[r2] as Ll
                            - q[r1_base + r2] as Ll;
        ...
        for x in 0..unused.len() {
            ...
            for y in (x + 1)..unused.len() {
                ...
                for z in (y + 1)..unused.len() {
                    ...
                    let delta = s.ins.q_get(a, a) as Ll
                        + s.contrib[a] as Ll
                        + s.ins.q_get(b, b) as Ll
                        + s.contrib[b] as Ll
                        + s.ins.q_get(c, c) as Ll
    ...
    fn apply_best_swap11(s: &mut P2State, window_k: usize) -> bool {
        ...
        for y in 0..cu {
            ...
            let add_contrib = s.ins.q_get(add, add) as Ll + contrib[add] as Ll;
            let row_base = add * n;
            ...
            for x in 0..cs {
                ...
                let lost = s.ins.q_get(rm, rm) as Ll + contrib[rm] as Ll;
    ...
    fn apply_best_swap12(s: &mut P2State, window_k: usize) -> bool {
        ...
        for si in 0..cs {
            ...
            for x in 0..cu {
                ...
                let ca_eff = s.ins.q_get(a, a) as Ll
                    + s.contrib[a] as Ll
                    - s.ins.q_get(a, rm) as Ll;
                ...
                for y in (x + 1)..cu {
                    ...
                    let cb_eff = s.ins.q_get(b, b) as Ll
                        + s.contrib[b] as Ll
                        - s.ins.q_get(b, rm) as Ll;
    ...
    fn apply_best_swap21(s: &mut P2State, window_k: usize) -> bool {
        ...
        for ai in 0..cu {
            ...
            let add_w = w[add];
            let cadd = s.ins.q_get(add, add) as Ll + contrib[add] as Ll;
            let add_base = add * n;
    ...
    fn apply_best_swap22(s: &mut P2State, window_k: usize) -> bool {
        ...
        for a in 0..cu {
            ...
            for b in (a + 1)..cu {
                ...
                let gain = s.ins.q_get(p, p) as Ll
                    + contrib[p] as Ll
                    + s.ins.q_get(q2, q2) as Ll
                    + contrib[q2] as Ll
                    + q[p_base + q2] as Ll;
        ...
        for &(r1_idx, r2_idx, r1, r2, budget, lost) in &used_pairs {
            for (pair_idx, &(p, q2, _, _, wp, pair_w, gain)) in unused_pairs.iter().enumerate() {
                ...
                let cross = cross_cache[cache_row + r1_idx] + cross_cache[cache_row + r2_idx];
        ...
        if rem_cap > 0 {
            ...
            if max_w > 0 && k > 0 && max_w <= 2_000_000 {
                ...
                for t in 0..k {
                    ...
                    let val = s.ins.q_get(item, item) as i64 + s.contrib[item] as i64;
                ...
                let (best_w, _) = if let Some(neg_inf_dp) = sentinel32 {
                    ...
                    for t in 0..k {
                        ...
                        let val = s.ins.q_get(item, item) + s.contrib[item];
                    ...
                    for t in 0..k {
                        let item = ord[left + t].id;
                        let wt = s.ins.w[item] as usize;
                        let val = s.ins.q_get(item, item) as Ll + s.contrib[item] as Ll;
        ...
        let mut cand: Vec<ItemScore> = (0..n)
            ...
            .map(|i| {
                ...
                let score = match strategy {
                    ...
                    3 => loss * 100 - total_interactions[i],
                    4 => total_interactions[i] - 2 * loss,
    ...
    fn reconstruction_score(
        ...
        match strategy {
            0 => gain,
            1 => gain * 1_000_000 / w,
            2 => total_interactions[id] + gain,
            3 => gain * 1_000_000 / w + total_interactions[id] / 10,
            _ => gain + total_interactions[id] / 20 - s.ins.w[id] as Ll,
        }
    }
