...
pub fn solve_challenge(
    ...
    for generation in 0..max_generations {
        ...
        if current_value as u32 > (0 * 9 / 10) {
            let high_potential_items: Vec<usize> = unselected_items
                .iter()
                .filter(|&&i| challenge.values[i] as i32 > (0 as i32 / 4))
                .copied()
                .collect();
    ...
    if current_value as u32 >= 0 && current_weight <= challenge.max_weight {
        let _ = save_solution(&Solution {
            items: selected_items,
        });
        return Ok(());
