// Copyright (c) 2026 NVX — v13 optimized brute-force
// Optimizations vs v12:
// 1. Early exit every 5 dims (was 10) — catches non-NN faster
// 2. Reduced unroll (#pragma unroll 10) — less I-cache pressure, better occupancy
// 3. Ballot-based warp skip — skip reduction when all lanes early-exited
// 4. Fused distance blocks — 5-dim blocks with FMA-style accumulation
...
__global__ void search_v13(
    ...
    for (int chunk = 0; chunk < full_chunks; chunk++) {
        ...
        if (valid_query) {
            ...
            // 50 blocks of 5 dims — check every 5 dims for faster early exit
            #pragma unroll 10
            for (int b = 0; b < 50; b++) {
                const int d = b * 5;
            ...
            // Ballot: skip warp reduce if ALL lanes early-exited
