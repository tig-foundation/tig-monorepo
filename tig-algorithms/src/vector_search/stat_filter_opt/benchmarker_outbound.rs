...
// Returns the value at position n after partial sorting
fn quickselect_nth(arr: &mut [f32], n: usize) -> f32 {
    if arr.len() == 1 {
        return arr[0];
    }
...
pub fn solve_challenge(
    ...
    if scale > 0.0 && scale < 5.0 {
        ...
        // MAD threshold on DB norms using quickselect for median
        let mut h_norms = stream.memcpy_dtov(&d_db_norm_l2)?;
        let len = h_norms.len();
        let mid = len / 2;

        // Use quickselect to find approximate median
        // For even length, just use the n/2 element instead of averaging two middle values
        // This is slightly less accurate but faster and sufficient for MAD filtering
        let median = quickselect_nth(&mut h_norms, mid);

        // Calculate deviations and find MAD using quickselect
        let mut deviations: Vec<f32> = h_norms.iter().map(|&x| (x - median).abs()).collect();
        // Similarly, for MAD just use the single middle element
        let mad = quickselect_nth(&mut deviations, mid);
    ...
    let per_thread_bytes = topk * 8; // (std::mem::size_of::<i32>() + std::mem::size_of::<f32>());
    // 4 planes * W words * 8B per word
    //let base_query_bytes = 4 * words_per_plane * std::mem::size_of::<u64>();
    let base_query_bytes = mode.planes() * words_per_plane * 8; // * std::mem::size_of::<u64>();
    ...
    let shared_refine = (challenge.vector_dims as usize * 4 // std::mem::size_of::<f32>()
                        + threads_refine as usize * 4) as u32; // std::mem::size_of::<f32>()) as u32;
