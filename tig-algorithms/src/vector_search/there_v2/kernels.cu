...
extern "C" __global__ __launch_bounds__(256) void batched_search(
    ...
    int query_idx = (int)(blockIdx.x * 16 + warp_id * 2 + half);
    ...
    if (is_first_batch) {
        ...
        lane_min_dist = MAX_FLOAT;
        lane_best_idx = batch_start;
    } else {
        lane_min_dist = best_dists[query_idx];
        lane_best_idx = results[query_idx];
    }

    for (int i = lane16; i < batch_count; i += 16) {
        int db_idx = batch_start + i;
        const float* db_vec = database_vectors + (size_t)db_idx * stride;
        float dist = euclidean_distance_high_bounded(query, db_vec, vector_dims, lane_min_dist);
