...
extern "C" __global__ __launch_bounds__(256, 2) void batched_search(
    ...
    for (int i = lane16; i < batch_count; i += 16) {
        ...
        const float* db_block_ptr = database_vectors + (size_t)db_block * db_block_stride;

        float dist = euclidean_distance_high_bounded_chunked4(query, db_block_ptr, db_lane, vector_dims, group_bound);
