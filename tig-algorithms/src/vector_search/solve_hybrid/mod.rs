...
pub fn solve_challenge(
    ...
    if challenge.vector_dims as usize != DIMS {
        return Err(anyhow!(
            "hybrid_vector_search expects {} dimensions, got {}",
            DIMS,
    ...
    match num_queries {
        7_000 => solve_wmma_chunked_first(
            challenge,
            &module,
            &stream,
            &mut d_query_norms,
        ...
        9_000 => solve_wmma_chunked_first(
            challenge,
            &module,
            &stream,
            &mut d_query_norms,
        ...
        11_000 => solve_wmma_chunked_second(
            challenge,
            &module,
            &stream,
            &mut d_query_norms,
        ...
        13_000 => solve_wmma_chunked_second(
            challenge,
            &module,
            &stream,
            &mut d_query_norms,
        ...
        15_000 => solve_wmma_chunked_second(
            challenge,
            &module,
            &stream,
            &mut d_query_norms,
...
// ============================================================================
// FIRST solution L2 persisting-cache helpers.
// ============================================================================
...
unsafe fn set_db_l2_persisting(stream: &Arc<CudaStream>, d_db_half: &CudaSlice<u16>) -> bool {
    ...
    let _ = rt_sys::cudaDeviceSetLimit(
        rt_sys::cudaLimit::cudaLimitPersistingL2CacheSize,
        max_persist,
    );
    ...
    let value = rt_sys::cudaLaunchAttributeValue {
...
// ============================================================================
// FIRST solution path.
// Used for 7000 and 9000.
// Uses L2 persisting cache.
// ============================================================================

fn solve_wmma_chunked_first(
    challenge: &Challenge,
    module: &Arc<CudaModule>,
    stream: &Arc<CudaStream>,
    d_query_norms: &mut CudaSlice<f32>,
...
// Used for 11000, 13000, 15000.
// Does NOT use L2 persisting cache.
// ============================================================================

fn solve_wmma_chunked_second(
    challenge: &Challenge,
    module: &Arc<CudaModule>,
    stream: &Arc<CudaStream>,
    d_query_norms: &mut CudaSlice<f32>,
...
fn solve_wmma_chunked_impl(
    ...
    unsafe {
        let l2_pinned = if use_l2_persisting {
            set_db_l2_persisting(stream, &d_db_half)
        } else {
            false
        };

        struct L2ResetGuard<'a> {
            stream: &'a Arc<CudaStream>,
            active: bool,
        }

        impl<'a> Drop for L2ResetGuard<'a> {
            fn drop(&mut self) {
                if self.active {
                    unsafe {
                        reset_l2_persisting(self.stream);
                    }
                }
            }
        }

        let _l2_guard = L2ResetGuard {
            stream,
            active: l2_pinned,
        };
        ...
        stream
            ...
            .launch(LaunchConfig {
                grid_dim: (
                    (nq_pad as u32 + WMMA_QUERIES_PER_BLOCK - 1)
                        / WMMA_QUERIES_PER_BLOCK,
                    num_chunks as u32,
                    1,
                ),
...
fn launch_reduce_f32(
    ...
    reduce_full_first_kernel: &CudaFunction,
    reduce_full_next_kernel: &CudaFunction,
    reduce_tail_kernel: &CudaFunction,
...
fn reduce_config(num_queries: usize) -> LaunchConfig {
    LaunchConfig {
        grid_dim: (
            1,
            (num_queries as u32 + WARPS_PER_BLOCK - 1) / WARPS_PER_BLOCK,
            1,
        ),
...
pub fn help() {
    println!("hybrid_vector_search - GPU exhaustive vector search");
    println!("Tig is my favorite challenge")
}
