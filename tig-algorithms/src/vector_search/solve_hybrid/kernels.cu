...
// ============================================================================
// Pack float32 D=250 into fp16 tiled D=256 layout and compute norms.
// Layout per 16-row tile:
//   tile_base + row_in_tile * 16 + k_tile * 256 + col_in_k_tile
// This matches WMMA row_major A and col_major B tile loads.
// ============================================================================
...
// Kept for compatibility if your module/build references it elsewhere.
...
__device__ __forceinline__ void av_update_best_f32(
    ...
    if (idx >= 0 && (dist < best || (dist == best && (best_idx < 0 || idx < best_idx)))) {
        best = dist;
        best_idx = idx;
    }
}
...
__device__ __forceinline__ void av_warp_reduce_best_f32(
    ...
    for (int offset = 16; offset > 0; offset >>= 1) {
        ...
        if (
            ...
            (
                other_dist < best ||
                (other_dist == best && (best_idx < 0 || other_idx < best_idx))
            )
        ) {
            best = other_dist;
            best_idx = other_idx;
...
// ============================================================================
// FP32 GEMM reduction path
// dot_tile contains -2 * dot(query, database)
// distance = query_norm + database_norm + dot_tile
// ============================================================================
...
extern "C" __global__ void av_reduce_gemm_tile_f32(
    ...
    if (vec_path) {
        ...
        for (int j = vec_end + lane; j < tile_len; j += 32) {
            int idx = db_start + j;
            av_update_best_f32(query_norm + db_norm_row[j] + row[j], idx, local_best, local_idx);
        }
    } else {
...
extern "C" __global__ void av_wmma_chunk_best(
    ...
    if (warp_active) {
        const half* __restrict__ q_tile_base =
            q_half + (long long)q_tile_id * PACKED_TILE_STRIDE;

        #pragma unroll
        for (int k = 0; k < KTILES; ++k) {
            wmma::load_matrix_sync(
                a[k],
                q_tile_base + (long long)k * TILE_ELEMS,
                WN
            );
        }
    }
    ...
    float q_norm =
        (warp_active && lane < WM && q < num_queries)
            ? query_norms[q]
            : 0.0f;
    ...
    for (int tile = 0; tile < db_tiles; ++tile) {
        ...
        if (tid < WN) {
            db_norm_tile[tid] =
                (tid < valid)
                    ? database_norms[chunk_start + db_local + tid]
                    : 0.0f;
        }
        ...
        if (warp_active) {
            ...
            for (int k = 0; k < KTILES; ++k) {
                wmma::load_matrix_sync(
                    b,
                    db_tile_shared + (long long)k * TILE_ELEMS,
                    WN
                );
        ...
        if (warp_active && lane < WM && q < num_queries) {
            ...
            for (int j = 0; j < WN; ++j) {
                if (j < valid) {
                    ...
                    av_update_best_f32(dist, db_idx, best_dist, best_idx);
                }
            }
        }

        // Required before next iteration overwrites shared db_tile_vec/dot_tiles.
        // Safely skipped after final tile.
        if (tile + 1 < db_tiles) {
            __syncthreads();
        }
    }
