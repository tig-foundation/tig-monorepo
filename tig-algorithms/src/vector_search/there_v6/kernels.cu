...
__device__ __forceinline__ float4 load_query_chunk4_broadcast(
    const float4* __restrict__ q_chunks,
    int chunk,
    int q_lane,
    unsigned mask,
    int src_lane
    ...
    q.z = 0.0f;
    q.w = 0.0f;
    if (((int)threadIdx.x & 31) == src_lane) {
        q = q_chunks[(size_t)chunk * 16ull + (size_t)q_lane];
    }
    q.x = __shfl_sync(mask, q.x, src_lane);
    q.y = __shfl_sync(mask, q.y, src_lane);
    q.z = __shfl_sync(mask, q.z, src_lane);
    q.w = __shfl_sync(mask, q.w, src_lane);
    return q;
}

__device__ __forceinline__ float euclidean_distance_high_bounded_chunked4_blocked(
    ...
    const float4* __restrict__ q_chunks = reinterpret_cast<const float4*>(q_block);
    const float4* __restrict__ b_chunks = reinterpret_cast<const float4*>(b_block);

    if (dims >= 4) {
        float4 q0 = load_db_chunk4(q_chunks, chunk, query_lane);
    ...
    for (; i < dims - 31; i += 32, chunk += 8) {
        float4 q0 = load_db_chunk4(q_chunks, chunk + 0, query_lane);
        float4 q1 = load_db_chunk4(q_chunks, chunk + 1, query_lane);
        float4 q2 = load_db_chunk4(q_chunks, chunk + 2, query_lane);
        float4 q3 = load_db_chunk4(q_chunks, chunk + 3, query_lane);
        float4 q4 = load_db_chunk4(q_chunks, chunk + 4, query_lane);
        float4 q5 = load_db_chunk4(q_chunks, chunk + 5, query_lane);
        float4 q6 = load_db_chunk4(q_chunks, chunk + 6, query_lane);
        float4 q7 = load_db_chunk4(q_chunks, chunk + 7, query_lane);
    ...
    if (i < dims) {
        const float* __restrict__ q_tail = q_block + (size_t)full_chunks * 64ull;
        const float* __restrict__ b_tail = b_block + (size_t)full_chunks * 64ull;
        for (int tail_idx = i - tail_base_dim; i < dims; ++i, ++tail_idx) {
            float diff =
                q_tail[(size_t)tail_idx * 16ull + (size_t)query_lane] -
                b_tail[(size_t)tail_idx * 16ull + (size_t)db_lane];
            sum = fmaf(diff, diff, sum);
            if (sum > limit) return sum;
        }
    }
...
__device__ __forceinline__ float euclidean_distance_high_bounded_blocked_pair(
    const float* __restrict__ q_block,
    int q_lane,
    const float* __restrict__ b_block,
    int db_lane,
    ...
    if (dims >= 4) {
        float4 q0 = load_query_chunk4_broadcast(q_chunks, chunk, q_lane, mask, src_lane);
        float4 b0 = load_db_chunk4(b_chunks, chunk, db_lane);
    ...
    for (; i < dims - 31; i += 32, chunk += 8) {
        float4 q0 = load_query_chunk4_broadcast(q_chunks, chunk + 0, q_lane, mask, src_lane);
        float4 q1 = load_query_chunk4_broadcast(q_chunks, chunk + 1, q_lane, mask, src_lane);
        float4 q2 = load_query_chunk4_broadcast(q_chunks, chunk + 2, q_lane, mask, src_lane);
        float4 q3 = load_query_chunk4_broadcast(q_chunks, chunk + 3, q_lane, mask, src_lane);
        float4 q4 = load_query_chunk4_broadcast(q_chunks, chunk + 4, q_lane, mask, src_lane);
        float4 q5 = load_query_chunk4_broadcast(q_chunks, chunk + 5, q_lane, mask, src_lane);
        float4 q6 = load_query_chunk4_broadcast(q_chunks, chunk + 6, q_lane, mask, src_lane);
        float4 q7 = load_query_chunk4_broadcast(q_chunks, chunk + 7, q_lane, mask, src_lane);
    ...
    if (i < dims) {
        ...
        for (int tail_idx = i - tail_base_dim; i < dims; ++i, ++tail_idx) {
            float q = 0.0f;
            if (lane == src_lane) {
                q = q_tail[(size_t)tail_idx * 16ull + (size_t)q_lane];
            }
            q = __shfl_sync(mask, q, src_lane);
            float diff = q - b_tail[(size_t)tail_idx * 16ull + (size_t)db_lane];
            sum = fmaf(diff, diff, sum);
            if (sum > limit) return sum;
        }
    }
...
extern "C" __global__ __launch_bounds__(256, 2) void batched_search(
    ...
    extern __shared__ float4 s_query4[];
    ...
    const size_t query_block_elems = q_stride * 16ull;
    const bool use_smem = (query_block_elems > 0 && query_block_elems <= 12288ull);

    if (use_smem) {
        int block_query_base = (int)(blockIdx.x * 16);
        int src_idx = block_query_base + lane16;
        const bool valid = (src_idx < num_queries);

        const float* __restrict__ src_base = valid
            ? (query_vectors + (size_t)src_idx * q_stride)
            : (const float*)0;
        ...
        float* __restrict__ s_query_block = reinterpret_cast<float*>(s_query4);
        ...
        for (int c = group; c < full_chunks; c += 16) {
            ...
            s_query4[(size_t)c * 16ull + (size_t)lane16] = v;
        }

        float* __restrict__ s_query_tail = s_query_block + (size_t)full_chunks * 64ull;
        if (valid) {
            for (int t = group; t < tail_dims; t += 16) {
                s_query_tail[(size_t)t * 16ull + (size_t)lane16] = src_base[tail_base_dim + t];
            }
        } else {
            for (int t = group; t < tail_dims; t += 16) {
                s_query_tail[(size_t)t * 16ull + (size_t)lane16] = 0.0f;
            }
        }
        ...
        const float* __restrict__ query_block = reinterpret_cast<const float*>(s_query4);

        for (int i = lane16; i < batch_count; i += 16, db_idx += 16, db_block_ptr += db_block_stride) {
            float dist = euclidean_distance_high_bounded_chunked4_blocked(
                query_block,
                local_query,
                db_block_ptr,
                db_lane,
