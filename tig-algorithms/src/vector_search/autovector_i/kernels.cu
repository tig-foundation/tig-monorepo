...
extern "C" __global__ void av_compute_norms_250(
...
extern "C" __global__ void av_pack_norm_fp16_250_to_256(
...
extern "C" __global__ void av_pack_fp16_linear(
    const float* __restrict__ src,
    unsigned short* __restrict__ dst,
    int n
) {
...
__device__ __forceinline__ void av_update_best_f32(
    float dist,
    int idx,
    float& best,
    int& best_idx
...
__device__ __forceinline__ void av_warp_reduce_best_f32(
    float& best,
    int& best_idx
) {
    #pragma unroll
...
__device__ __forceinline__ void av_reduce_gemm_tile_f32_full_impl(
    ...
    for (int it = 0, v = lane; it < VEC_GROUPS / 32; ++it, v += 32) {
        ...
        int idx = db_start + j;
        av_update_best_f32(query_norm + norms.x + dots.x, idx, local_best, local_idx);

        ++idx;
        av_update_best_f32(query_norm + norms.y + dots.y, idx, local_best, local_idx);

        ++idx;
        av_update_best_f32(query_norm + norms.z + dots.z, idx, local_best, local_idx);

        ++idx;
        av_update_best_f32(query_norm + norms.w + dots.w, idx, local_best, local_idx);
    }

    av_warp_reduce_best_f32(local_best, local_idx);
...
extern "C" __global__ void av_reduce_gemm_tile_f32_full_first(
    const float* __restrict__ query_norms,
    const float* __restrict__ database_norms,
    const float* __restrict__ dot_tile,
    int* __restrict__ best_idx,
    ...
    av_reduce_gemm_tile_f32_full_impl<false>(
        query_norms,
        database_norms,
        dot_tile,
        best_idx,
...
extern "C" __global__ void av_reduce_gemm_tile_f32_full_next(
    const float* __restrict__ query_norms,
    const float* __restrict__ database_norms,
    const float* __restrict__ dot_tile,
    int* __restrict__ best_idx,
    ...
    av_reduce_gemm_tile_f32_full_impl<true>(
        query_norms,
        database_norms,
        dot_tile,
        best_idx,
...
extern "C" __global__ void av_reduce_gemm_tile_f32(
    const float* __restrict__ query_norms,
    const float* __restrict__ database_norms,
    const float* __restrict__ dot_tile,
    int* __restrict__ best_idx,
...
extern "C" __global__ void av_reduce_gemm_tile_fp16(
    const float* __restrict__ query_norms,
    const float* __restrict__ database_norms,
...
// Le tie-break d'origine  ( dist < best || (dist == best && idx < best_idx) )  est une
// chaine de 2 comparaisons donc jusqu'a 2 branchements, executee 16 fois par tuile.
// Il se reecrit en UN SEUL min.u64 sur la cle  (sortable(dist) << 32) | idx  :
//   * sortable() est la transformation monotone standard float -> uint32 (l'ordre des
//     floats est preserve, negatifs inclus) ; elle est purement binaire donc EXACTE et
//     son inverse restitue le float bit pour bit ;
//   * les indices etant dans [0, 2^31), leur ordre non signe == leur ordre signe, donc
//     a distance egale min() choisit le plus petit indice : MEME tie-break.
// => Q bit-identique par construction, et 0 branchement dans la boucle d'argmin.
__device__ __forceinline__ unsigned long long av_pack_key(float dist, int idx) {
...
__device__ __forceinline__ float av_key_dist(unsigned long long key) {
    unsigned int b = (unsigned int)(key >> 32);
    b ^= ((b >> 31) - 1u) | 0x80000000u;
    return __uint_as_float(b);
}

extern "C" __global__ void av_wmma_chunk_best(
    ...
    // Init = pack(FLT_MAX, -1) : -1 devient 0xFFFFFFFF, donc le plus grand indice non
    // signe, et tout candidat reel le bat -- exactement comme le code d'origine.
    unsigned long long best_key = av_pack_key(FLT_MAX, -1);
    ...
    // valid = min(chunk_len - tile*WN, WN) vaut WN pour TOUTES les tuiles sauf, au plus, la
    // derniere. Les gardes (tid < valid) et (j < valid) sont donc toujours vraies sur le chemin
    // chaud, mais chacune coute un branchement -- donc un bloc de base, facture ~15.6 ms par
    // bundle (mesure V3 PHASE_C4 §6, 48 sigma). La boucle argmin en porte 16 par tuile.
    // Q BIT-IDENTIQUE par construction : meme arithmetique, memes indices, meme ordre d'argmin.
    // Contrairement au deroulage (hunroll/hunroll2, re-roules par nvcc), il s'agit ici d'une
    // specialisation de chemin, que le compilateur n'a pas effectuee de lui-meme.
    ...
    for (int tile = 0; tile < full_tiles; ++tile) {
        ...
        if (tid < WN) {
            db_norm_tile[tid] = database_norms[chunk_start + db_local + tid];   // valid == WN
        }
        __syncthreads();
        ...
        // `dots = dot_tiles[warp_id]` : chaque warp ecrit et relit SA PROPRE tranche.
        // La dependance store->read est donc INTRA-WARP, et `store_matrix_sync` est une
        // operation warp-synchrone => une barriere de BLOC est une sur-synchronisation.
        // Les deux autres barrieres restent : elles protegent `db_tile_vec`, partage
        // entre les 4 warps du bloc (staging avant, ecrasement apres).
        // Q BIT-IDENTIQUE par construction : memes valeurs, meme ordre, seule la portee
        // de la synchronisation change.
        __syncwarp();

        // WM = 16 : dans l'epilogue d'origine, seuls les lanes 0-15 travaillent et la
        // MOITIE DU WARP est oisive. Ici chaque ligne est traitee par DEUX lanes (L et
        // L+16), chacune prenant 8 candidats sur 16, puis on recombine par un shfl.
        // Q BIT-IDENTIQUE : `min` sur l'ordre total des cles est associatif et commutatif,
        // donc scinder puis recombiner donne le MEME minimum ; et `dist` est calculee par
        // la meme expression avec le meme q_norm (diffuse exactement par shfl).
        if (warp_active) {                       // uniforme par warp : les 32 lanes entrent
            const int row  = lane & (WM - 1);    // ligne de requete traitee
            const int half = lane >> 4;          // 0 => j 0..7, 1 => j 8..15
            ...
            unsigned long long k = av_pack_key(FLT_MAX, -1);
            #pragma unroll
            for (int t = 0; t < WN / 2; ++t) {
                ...
                k = min(k, av_pack_key(dist, db_idx));
            }
    ...
    // ── chemin general : tuile de queue eventuelle (valid < WN), code d'origine inchange ──
        ...
        if (warp_active && lane < WM && q < num_queries) {
            ...
            for (int j = 0; j < WN; ++j) {
                if (j < valid) {
                    ...
                    best_key = min(best_key, av_pack_key(dist, db_idx));   // i8
                }
            }
        }
        __syncthreads();
    ...
    if (warp_active && lane < WM && q < num_queries) {
        ...
        chunk_best_dist[out] = av_key_dist(best_key);                       // i8 : decodage exact
        chunk_best_idx[out] = (int)(unsigned int)(best_key & 0xFFFFFFFFull);
    }
}

extern "C" __global__ void av_reduce_chunk_bests(
