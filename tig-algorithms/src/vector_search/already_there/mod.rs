...
pub fn solve_challenge(
    ...
    // Compute early exit statistics on CPU for first 100 database vectors
    {
        let stats_db_count = 100.min(database_size as usize);
        let stats_query_count = num_queries as usize;
        
        // Copy query and database vectors from GPU to CPU for stats
        let query_vectors: Vec<f32> = stream.memcpy_dtov(&challenge.d_query_vectors)?;
        let db_vectors: Vec<f32> = stream.memcpy_dtov(&challenge.d_database_vectors)?;
        
        // Only use first stats_db_count vectors
        let db_subset = &db_vectors[..stats_db_count * vector_dims as usize];
        
        // compute_early_exit_stats(
        //     &query_vectors,
        //     db_subset,
        //     stats_query_count,
        //     vector_dims as usize,
        //     stats_db_count,
        // );
    }
    ...
    let config = LaunchConfig {
        ...
        block_dim: (128, 1, 1),
        shared_mem_bytes: (vector_dims * 4) as u32,  // 4 bytes per float
    };
