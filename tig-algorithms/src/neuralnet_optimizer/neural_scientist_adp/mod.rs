...
pub fn solve_challenge(
    ...
    let (solution, _train_losses, _val_losses) = training_loop(
        challenge,
        module,
        stream,
        prop,
...
fn optimizer_step(
    ...
    if let Some(loss) = val_loss {
        if s.step_count > s.warmup_steps {
            ...
            if s.val_loss_history.len() > 50 {
                s.val_loss_history.remove(0);
            }
            
            if s.val_loss_history.len() >= 10 {
    ...
    if s.step_count > s.warmup_steps && s.val_loss_history.len() >= 15 {
        let recent_losses: Vec<f32> = s.val_loss_history.iter().rev().take(15).copied().collect();
        let mean = recent_losses.iter().sum::<f32>() / recent_losses.len() as f32;
        let variance = recent_losses.iter().map(|x| {
            let diff = x - mean;
            diff * diff
        }).sum::<f32>() / recent_losses.len() as f32;
        let std_dev = variance.sqrt();
        let volatility = std_dev / mean.abs().max(1e-8);
        
        let target_beta1 = if volatility > 0.15 {
            0.88
        ...
        s.beta1 = 0.90 * s.beta1 + 0.10 * target_beta1;
        s.beta1 = s.beta1.clamp(0.85, 0.96);
    }
