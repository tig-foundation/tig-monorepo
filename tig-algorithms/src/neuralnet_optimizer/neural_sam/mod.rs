// TIG's UI uses the pattern `tig_challenges::neuralnet_optimizer` to automatically detect your algorithm's challenge
use anyhow::Result;
use cudarc::{
...
fn optimizer_init_state(
    ...
    let state = DualPhaseConsensusState {
        m, v, prev_g, prev_u, slow_u, f, ef, upd, cfgs, layer_lrs, spectral_boost,
        step_count: 0, warmup_steps, total_steps, noise_variance,
        val_loss_history: Vec::new(),
        beta1, beta2, eps, weight_decay,
        bn_layer_boost, output_layer_damping,
...
fn compute_blends(s: &DualPhaseConsensusState, val_loss: Option<f32>) -> (f32, f32, f32, f32, f32, f32, f32) {
    ...
    (blend_adam / sum, blend_norm / sum, blend_sign / sum, gamma, bb_blend, lookahead_alpha, lookahead_tau)
}
...
fn optimizer_step(
    ...
    if let Some(loss) = val_loss {
        if s.step_count > s.warmup_steps {
            ...
            if s.val_loss_history.len() >= 6 {
                ...
                let nv_alpha = if s.total_steps <= 1600 && s.val_loss_history.len() >= 10 { 0.20 } else { 0.15 };
                s.noise_variance = (1.0 - nv_alpha) * s.noise_variance + nv_alpha * target_nv;
    ...
    if let (Some(prev), Some(curr)) = (s.prev_val_loss, val_loss) {
        if s.step_count > s.warmup_steps && s.step_count > 20 {
            let relative_improvement = (prev - curr) / prev.abs().max(1e-8);
    ...
    if let (Some(prev), Some(curr)) = (s.prev_val_loss, val_loss) {
        ...
        if s.step_count > s.warmup_steps {
            ...
            if s.plateau_count >= 25 && curr > s.noise_variance * 4.0 {
                ...
                let rel_imp = improvement / curr.abs().max(1e-8);
                let decay = if rel_imp > 0.01 { 0.75 } else if rel_imp > 0.001 { 0.85 }
    ...
    if let Some(loss) = val_loss {
        ...
        if loss <= s.noise_variance * 5.0 {
            ...
            global_damp *= 0.70 + 0.30 * noise_proximity.powf(steepness);
        }
    }
    ...
    let loss_converged = val_loss.map_or(false, |loss| loss <= s.noise_variance * 8.0);
    let use_robust = s.step_count > s.warmup_steps && (near_floor || (late_phase && loss_converged));
    ...
    let (in_precision_zone, precision_gain, gate_lo, gate_hi, forward_gain): (bool, f32, f32, f32, f32) = if let Some(loss) = val_loss {
        if s.step_count > s.warmup_steps {
            ...
            if loss >= z_lo && loss <= z_hi {
                ...
                let gl = 0.70 + 0.02 * pos;
                let gh = 1.50 + 0.05 * pos;
    ...
    let beta2_eff: f32 = if in_precision_zone && s.total_steps <= 1600 { (s.beta2 + 0.001).min(0.9995) } else { s.beta2 };
...
pub fn help() {
    println!("neural_sam: Dual-phase consensus optimizer with Fisher normalization");
    println!("Based on neural_advanced with targeted improvements");
}
