...
// Extern host-side helper implemented in kernel.cu: fused Nova Prime step launcher
extern "C" {
    pub fn nova_prime_step(
        stream: cudarc::runtime::sys::cudaStream_t,
        updates: *mut f32,
        gradients: *const f32,
        prev_gradients: *const f32,
        loss_hist: *const f32,
        grad_norm_hist: *const f32,
        m: *mut f32,
        v_fp16: *mut u16,
        base_lr: f32,
        weight_decay: f32,
        size: i32,
        history_len: i32,
        landscape_class_out: *mut u8,
        verification_result: *mut i32,
        optimal_lr_out: *mut f32,
    );
}
...
impl Profile {
    fn from_HyperParams(_HyperParams: &Option<Map<String, Value>>) -> Self { Profile::Compute }
...
/// Minimal HyperParams struct (used by other helpers)
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
pub struct HyperParams { pub base_lr: Option<f32>, pub beta1: Option<f32>, pub beta2: Option<f32>, pub weight_decay: Option<f32>, pub profile: Option<String> }

/// Minimal ModelSpec and OptConfig placeholders
#[derive(Debug, Clone, Serialize, Deserialize)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OptConfig { pub apbi_threshold: f32, pub dca_decay_rate: f32, pub hakd_aggressiveness: u8, pub enable_curvature: bool, pub sparse_pruning: bool }

impl OptConfig {
    /// Construct a lightweight OptConfig from an architecture embedding.
    ...
    pub fn from_embedding(emb: &[f32]) -> Self {
        // Use a few embedding slots (if present) to bias the HyperParams.
...
// and a HZS orchestrator). These implementations are intentionally
...
mod the_impl {
    ...
    /// HZS: high-level orchestrator that combines the components
...
// ============================================================================
// HyperParams (kept for compatibility)
// ============================================================================
...
/// TPF - Forecasted HyperParams for next 3 steps
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct OptimizerState {
    ...
    // HyperParams (immutable after init)
    base_lr: f32,
    beta1: f32,
    beta2: f32,
    eps: f32,
    ...
    // Validation loss history (for trend-based heuristics)
    val_loss_history: Vec<f32>,
    // Plateau counter for adaptive kernel triggering
    plateau_count: usize,
...
pub struct AutoTuneController {
    ...
    pub hyperparameter_space: HyperParamspace,
    pub performance_history: Vec<PerformanceSample>,
    pub current_hyperparams: HyperParams,
...
impl AutoTuneController {
    pub fn new() -> Self {
        let hyperparameter_space = HyperParamspace {
            lr_bounds: (1e-5, 1e-1),
    ...
    pub fn suggest_HyperParams(&mut self) -> HyperParams {
        if self.performance_history.len() < 5 {
            // Return default HyperParams for initial exploration
            return self.current_hyperparams.clone();
        }
        
        // Use Bayesian optimization to suggest next HyperParams
...
impl BayesianOptimizer {
    pub fn optimize(&self, space: &HyperParamspace) -> Vec<f32> {
...
_model_params: &[f32]) -> Result<EclipseOptimizationResult> {
        ...
        // AutoTune-X: Update performance and suggest new HyperParams
        if step_count % self.adaptation_interval == 0 {
            ...
            let new_hyperparams = self.autotune_controller.suggest_HyperParams();
...
pub struct EclipseOptimizationResult {
    ...
    pub suggested_hyperparams: HyperParams,
}
...
pub fn solve_challenge(
    ...
    let profile = Profile::from_HyperParams(HyperParams);
    set_profile(profile);
    ...
    let _gas_config = OptConfig::from_embedding(&embedding);
...
fn new_optimizer_state(
    ...
    // Per-layer learning rate mapping — more aggressive for small layers
    // We store relative multipliers in `layer_lrs` so later per-step code
    // can multiply by the global `base_lr`.
    let mut layer_lrs = Vec::with_capacity(param_sizes.len());
    // Choose a conservative base but map desired per-layer absolute LR
    let base_lr = 0.00195f32;
    for (&param_size) in param_sizes.iter() {
        let desired_lr = if param_size > 50_000 {
            0.00145f32
        } else if param_size > 10_000 {
            0.00165f32
        ...
        // Store multiplier relative to base_lr
        layer_lrs.push(desired_lr / base_lr);
    }
    ...
    for (idx, &param_size) in param_sizes.iter().enumerate() {
        // Small boost for BN-like layers; output layer damping applied at step-time
...
fn optimizer_step_compute_only(
    ...
    // Predictive trend factor based on recent validation loss velocity
    let trend_factor: f32 = if state.val_loss_history.len() >= 3 {
        let h = &state.val_loss_history;
        let n = h.len();
    ...
    // Trust backoff (multi-tiered)
    let trust_backoff: f32 = if let (Some(p), Some(c)) = (state.prev_loss, val_loss) {
        let rel_up = (c - p) / (p.abs() + 1e-6);
        if rel_up > 0.020 {
            0.70f32
        } else if rel_up > 0.012 {
            0.80f32
        } else if rel_up > 0.005 {
            0.90f32
        } else {
            1.0f32
        }
    ...
    let last = gradients.len().saturating_sub(1);
    for (i, g) in gradients.iter().enumerate() {
        ...
        // Dynamic output-layer damping and BN boost
        let layer_multiplier = if i + 2 >= last {
            if let Some(loss) = val_loss {
                if state.step_count > state.phase_controller.warmup_total + 30 {
                    let noise_var = state.autotune_controller.bayesian_optimizer.gaussian_process.noise_variance;
                    let loss_ratio = (loss / (noise_var * 6.0)).min(1.0);
                    (state.output_layer_damping * (0.7f32 + 0.3f32 * loss_ratio)).max(state.output_layer_damping)
                } else {
                    state.output_layer_damping
                }
            } else {
                state.output_layer_damping
            }
        } else if n <= 512 {
            state.bn_layer_boost
        } else {
            1.0
        };

        // Final per-layer LR: base * layer multiplier * predictive/trust factors * layer_multiplier
        let lr = effective_lr * state.layer_lrs[i] * trend_factor * trust_backoff * layer_multiplier;
        state.h_layer_lrs_cache[i] = lr;
    }
    ...
    // Update validation-loss tracking and plateau counter
    if let Some(l) = val_loss {
        state.val_loss_history.push(l);
        if let Some(prev) = state.prev_loss {
            if (l - prev).abs() < 1e-4 {
                state.plateau_count = state.plateau_count.saturating_add(1);
            } else {
    ...
    // Kernel trigger refinement: decide if robust kernel should be preferred
    let near_floor = val_loss.map(|l| {
        l < (state.autotune_controller.bayesian_optimizer.gaussian_process.noise_variance * 2.0)
    }).unwrap_or(false);
    let late_phase = progress > 0.90f32;
    let in_zone = state.gradient_history.buffer.len() > 8;
    let use_robust = state.step_count > state.phase_controller.warmup_total + 120
        && (
            near_floor
            || late_phase
            || (in_zone && state.plateau_count >= 12)
            || (state.plateau_count >= 8 && trend_factor < 0.95)
        );
    if use_robust {
        // Signal adaptive dispatcher to select robust kernel
        let _ = state.adaptive_dispatcher.select_kernel(LandscapeClass::Flat);
    }
...
// ============================================================================
// Fractal Gradient Descent (FGD) - inert, compile-only helper
// This module provides a lightweight, non-invasive Rust-side implementation
// of the Local Roughness Estimator (LRE) and helper utilities for Fractal
// Gradient Descent. It is intentionally inert (not wired into the step path)
// and marked with allow(dead_code) so adding it does not change runtime
// behaviour or compilation assumptions.
// ============================================================================
...
pub mod fgd {
    ...
    /// Small deterministic XORShift PRNG for optional noise injection.
    #[derive(Clone)]
    ...
    /// Per-layer / per-tensor LRE state kept on host. Small and optional.
    #[derive(Clone)]
    pub struct LreState {
        pub exp_avg_sq: Vec<f32>, // EMA of squared gradients (roughness proxy)
        pub momentum_buf: Vec<f32>,
        pub alpha: f32,
        pub seed: u32, // deterministic RNG seed when noise is used
    }
    ...
    impl LreState {
        pub fn new(len: usize) -> Self {
            Self {
                ...
                momentum_buf: vec![0.0f32; len],
                alpha: 0.9f32,
                seed: 0xC0FFEEu32.wrapping_add(len as u32),
            }
        }
    }

    /// FGD configuration container. Kept tiny and safe for compile-time presence.
    #[derive(Clone, Copy, Debug)]
    ...
    impl Default for FractalConfig {
        fn default() -> Self {
            Self { fractal_scale: 0.1, momentum: 0.9, eps: 1e-8, noise_enabled: false }
        }
    }

    /// High-level helper implementing the core math of FGD (host-side).
    /// This is intentionally not integrated into optimizer_step_*.
    pub struct FractalGradientDescent {
        pub cfg: FractalConfig,
    }

    impl FractalGradientDescent {
        pub fn new(cfg: FractalConfig) -> Self { Self { cfg } }

        /// Update EMA of squared gradients in-place and compute a scalar
        /// roughness index for the provided slice. Returns RI >= 0.
        pub fn update_lre_and_compute_ri(&self, state: &mut LreState, grads: &[f32]) -> f32 {
            let alpha = state.alpha;
            let mut sum_mean_abs = 0.0f32;
            let mut sum_exp_avg = 0.0f32;
            let n = grads.len().max(1) as f32;

            for (i, &g) in grads.iter().enumerate() {
                let g2 = g * g;
                let old = state.exp_avg_sq.get(i).copied().unwrap_or(0.0);
                let updated = alpha * old + (1.0 - alpha) * g2;
                if i < state.exp_avg_sq.len() {
                    state.exp_avg_sq[i] = updated;
                }
                sum_exp_avg += updated;
                sum_mean_abs += g.abs();
            }

            let mean_abs = sum_mean_abs / n;
            let mean_exp = sum_exp_avg / n;

            // variance proxy: E[g^2] - (E|g|)^2; ensure non-negative
            let var = (mean_exp - mean_abs * mean_abs).max(0.0f32);

            // roughness index normalized
            let ri = var / (mean_abs * mean_abs + self.cfg.eps);
            ri
        }

        /// Compute an adaptive learning rate multiplier given base lr and RI.
        pub fn adaptive_lr(&self, base_lr: f32, roughness_index: f32) -> f32 {
            base_lr / (1.0 + self.cfg.fractal_scale * roughness_index)
        }

        /// Inject deterministic noise into gradient buffer in-place if enabled.
        /// Noise magnitude scales with roughness_index and eps for numerical stability.
        pub fn inject_noise(&self, state: &mut LreState, grads: &mut [f32], roughness_index: f32) {
            if !self.cfg.noise_enabled { return; }
            let scale = roughness_index * self.cfg.eps.max(1e-12f32) * 1e3f32;
            let mut rng = XorShift32::new(state.seed);
            for (i, g) in grads.iter_mut().enumerate() {
                let r = (rng.next_f32() - 0.5) * 2.0; // [-1,1]
                *g += r * scale;
                // Advance seed slightly to avoid identical sequences across slices
                state.seed = state.seed.wrapping_add((i as u32).wrapping_add(1));
            }
        }

        /// Apply a momentum update (host-side) using adaptive LR and existing
        /// momentum buffer. The operation is identical in effect to the Python
        /// reference but done on CPU and meant only for prototyping / compile-time
        /// presence.
        pub fn apply_momentum_update(&self, state: &mut LreState, grads: &[f32], out_updates: &mut [f32]) {
            let lr = 1.0f32; // caller supplies adaptive scaling externally
            let beta = self.cfg.momentum;
            let n = grads.len().min(state.momentum_buf.len()).min(out_updates.len());
            for i in 0..n {
                let m = state.momentum_buf[i] * beta + grads[i] * lr;
                state.momentum_buf[i] = m;
                out_updates[i] = -m;
            }
        }
    }
...
fn optimizer_step_full(
    ...
    // Predictive trend factor from validation loss history
    let trend_factor: f32 = if state.val_loss_history.len() >= 3 {
        let h = &state.val_loss_history;
        let n = h.len();
        let recent_slope = h[n - 1] - h[n - 3];
    ...
    for (i, g) in gradients.iter().enumerate() {
        ...
        let layer_multiplier = if i + 2 >= last {
            if let Some(loss) = val_loss {
                if state.step_count > state.phase_controller.warmup_total + 30 {
                    let noise_var = state.autotune_controller.bayesian_optimizer.gaussian_process.noise_variance;
                    let loss_ratio = (loss / (noise_var * 6.0)).min(1.0f32);
                    (state.output_layer_damping * (0.7f32 + 0.3f32 * loss_ratio)).max(state.output_layer_damping)
                } else {
                    state.output_layer_damping
                }
