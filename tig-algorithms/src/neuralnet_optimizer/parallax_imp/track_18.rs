...
const K_ADAN: &str = "sk_adan_18";
const K_ADAN_NO_ROLLBACK: &str = "sk_adan_no_rollback_18";
const K_ADAN_MATRIX: &str = "sk_adan_matrix_18";
const K_KINK: &str = "sk_kink_18";
const K_KINK2: &str = "sk_kink2_18";
const K_OFFSET: &str = "sk_offset_18";
const K_SCALE: &str = "sk_scale_18";
const K_AXIS_RMS: &str = "sk_axis_rms_18";
const K_MATRIX_PRECOND: &str = "sk_matrix_precond_18";
const K_ROLLBACK: &str = "sk_rollback_18";
...
struct State {
    ...
    matrix_scratch: Vec<Option<MatrixScratch>>,
    roles: Vec<Role>,
    active: Vec<usize>,
    depth_mult: Vec<f32>,
    kink_eff: f32,
...
pub fn optimizer_init_state(
    ...
    let mut matrix_scratch = Vec::with_capacity(param_sizes.len());
    for i in 0..param_sizes.len() {
        ...
        matrix_scratch.push(if let Some((rows, cols)) = shape {
            Some(MatrixScratch {
...
pub fn optimizer_step(
    ...
    let k_adan_no_rollback = module.load_function(K_ADAN_NO_ROLLBACK)?;
    let k_adan_matrix = module.load_function(K_ADAN_MATRIX)?;
    ...
    for &i in s.active.iter() {
        ...
        unsafe {
            if let Some(scratch) = s.matrix_scratch[i].as_mut() {
                stream
                    .launch_builder(&k_axis_rms)
                    .arg(&gradients[i])
                    .arg(&mut scratch.row_rms)
                    .arg(&mut scratch.col_rms)
                    .arg(&scratch.rows)
                    .arg(&scratch.cols)
                    .launch(LaunchConfig {
                        grid_dim: (scratch.rows.max(scratch.cols) as u32, 2, 1),
                        block_dim: (256, 1, 1),
                        shared_mem_bytes: 0,
                    })?;
            }

            if let Some(scratch) = s.matrix_scratch[i].as_ref() {
                stream
                    .launch_builder(&k_adan_matrix)
                    .arg(&gradients[i])
                    .arg(&model_params[i])
                    .arg(&mut s.m[i])
                    .arg(&mut s.v[i])
                    ...
                    .arg(&n_i)
                    .launch(LaunchConfig::for_num_elems(n as u32))?;
            } else if i == s.shift_idx {
                stream
                    .launch_builder(&k_adan_no_rollback)
                    .arg(&gradients[i])
                    .arg(&model_params[i])
                    .arg(&mut s.m[i])
                    .arg(&mut s.v[i])
    ...
    let mut shift_rolled_back = false;
        ...
        && s.batches_per_epoch > 0
    {
        ...
        if amount != 0.0 {
            ...
            shift_rolled_back = true;
        }
    }

    unsafe {
        for &i in s.active.iter() {
            if i == 1 || (i == s.shift_idx && !shift_rolled_back) {
                let n = model_params[i].len();
                stream
