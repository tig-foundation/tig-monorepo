// n_hidden=4 path: adapted from neural_extrem_v3's t29 track (branch
// neuralnet_optimizer/neural_extrem_v3), used under the TIG Inbound Game
// License (see README).
use anyhow::Result;
use cudarc::{
...
fn parse_config(hyperparameters: &Option<Map<String, Value>>) -> TrackConfig {
    ...
    TrackConfig {
        total_steps: hp.and_then(|h| h.get("total_steps").and_then(|v| v.as_u64())).unwrap_or(5500) as usize,
        warmup_steps: hp.and_then(|h| h.get("warmup_steps").and_then(|v| v.as_u64())).unwrap_or(16) as usize,
        ...
        beta2: hp.and_then(|h| h.get("beta2").and_then(|v| v.as_f64())).unwrap_or(0.96) as f32,
        eps: hp.and_then(|h| h.get("eps").and_then(|v| v.as_f64())).unwrap_or(1e-8) as f32,
        weight_decay: hp.and_then(|h| h.get("weight_decay").and_then(|v| v.as_f64())).unwrap_or(0.015) as f32,
        bn_layer_boost: hp.and_then(|h| h.get("bn_layer_boost").and_then(|v| v.as_f64())).unwrap_or(1.0) as f32,
