...
//   keep     = 1.0 if dir * g > 0 else keep_damp   (cautious sign mask)
//   delta    = -lr * keep * dir
    ...
    int n)
{
    ...
    if (idx < n) {
        ...
        float m_new = beta_m * m[idx] + (1.0f - beta_m) * g;
        float v_new = beta_v * v[idx] + (1.0f - beta_v) * g_diff;
        float g_n   = g + mix * g_diff;
        float n_new = beta_n * s[idx] + (1.0f - beta_n) * g_n * g_n;
        ...
        float adan_dir = (m_new + mix * v_new) / (sqrtf(n_new) + eps);
        ...
        float keep = (dir * g > 0.0f) ? 1.0f : keep_damp;
