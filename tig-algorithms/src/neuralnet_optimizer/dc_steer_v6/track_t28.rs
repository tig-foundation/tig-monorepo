...
struct PrometheusState {
    ...
    // DC steering: persistent H2D staging buffer (allocated lazily on the first
    // pulse) plus the intra-epoch step counter this solver does not otherwise
    // track. `upd` (the t26/t29 scratch slot) does not exist here.
...
// Ported from t27/i9 (gave -13.7% time). LR_MAX corrected to 2.8e-3 (vs dead-listed 3.4e-3).
fn optimizer_step(
    ...
    state.step += 1;
    // Intra-epoch step index (1-based), mirroring `helpers::update_state_from_val_loss`.
    // Kept separate from `last_sched_epoch` so the LR schedule is untouched.
    ...
    // add is ordered behind it. At most two extra launches per epoch, and none
...
pub fn solve(
    ...
    // Runtime HP overrides for bench-binary variant sweeps
