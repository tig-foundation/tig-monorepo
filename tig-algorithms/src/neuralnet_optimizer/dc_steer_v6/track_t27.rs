...
fn optimizer_step(
    ...
    // Runs after the fused kernel (and the Muon post-pass) has written that
    // update; same stream, so the add is ordered behind it. At most two extra
    // launches per epoch, and none at all while `dc_enable == 0`.
    {
