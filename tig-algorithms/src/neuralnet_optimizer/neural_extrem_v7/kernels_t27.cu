...
// Ported from prometheus_aidda/code_raw/kernels.cu for track t27 combine.
//   g_diff = first_step ? 0 : (g - g_prev)
//   m_t = 0.98*m + 0.02*g   v_t = 0.92*v + 0.08*g_diff
//   n_t = 0.99*n + 0.01*(g + 0.92*g_diff)^2
//   adan_dir = (m_t + 0.92*v_t) / (sqrt(n_t) + eps)
//   wd_gate = 1 if adan_dir*theta >= 0 else 0    (cautious weight decay)
//   dir = adan_dir + wd_gate * wd * theta
//   keep = 1 if dir*g > 0 else 0.25              (cautious sign mask, damped not zeroed)
//   delta = -lr * keep * dir
...
// Fused multi-tensor AdaBelief update + EMA-Nesterov acceleration wrapper.
// Replaces n_fused separate adabelief_update_kernel_t27 launches with one kernel.
//
// all_ptrs layout (role-segmented, n_fused entries per role):
//   [grad0..N, param0..N, m0..N, v0..N, s0..N, pg0..N, ema0..N, delta0..N]   (8 roles)
// lr_wd layout (interleaved): [lr0, wd0, lr1, wd1, ..., lrN, wdN]
// offsets: cumulative element counts, length n_fused+1 (offsets[n_fused] = total_elems)
//
// EMA-Nesterov graft (db1e4b89, generic acceleration wrapper):
//   Δ_adan = AdanW delta (unchanged math).  b = ema-of-updates buffer (role 6).
//   b_new = ema_gamma*b_old + (1-ema_gamma)*Δ_adan
//   delta = Δ_adan + ema_beta*(b_new - b_old)
// ema_beta=0 ⇒ delta = Δ_adan byte-identical to baseline (parity fallback).
//
// QHAdam graft (CASSURE swap#3, i27, DEAD — qh_nu locked to 1.0 = parity only).
//
// AdaBelief 2nd-moment graft (CASSURE swap#4, i28, refs 739cd4c8 / e7e2a937):
//   Replace Adan's variance proxy  (g + 0.92·g_diff)²  with AdaBelief's
//   gradient-belief error          (g − m_new)²
//   via blend knob `ab_blend` (float, default 1.0 = parity / pure Adan):
//     g_adan  = g + 0.92·g_diff                  (Adan 2nd-moment target)
//     n_ab    = (g − m_new)² + 1e-16             (AdaBelief error + ε² guard)
//     n_tgt   = ab_blend·g_adan² + (1−ab_blend)·n_ab
//     n_new   = 0.99·s + 0.01·n_tgt
//   ab_blend=1.0 ⇒ n_tgt = g_adan² ⇒ byte-identical Adan baseline (parity fallback).
//   ab_blend=0.0 ⇒ pure AdaBelief: small denominator when g≈m (large steps) vs
//     large when g misaligned (conservative). ε²=1e-16 prevents collapse.
//
// Each thread maps global_idx → tensor t + local element via binary search
// on the shared-memory copy of offsets. AdanW math is byte-identical to adabelief_update_kernel_t27.
extern "C" __global__ void adabelief_fused_kernel_t27(
    ...
    float sophia_rho)
{
    // Load offset table into shared memory for fast binary search
    ...
    // Binary search: find tensor t such that s_off[t] <= gidx < s_off[t+1]
    ...
    // Dereference role-segmented pointer arrays
    ...
    // Identical math to adabelief_update_kernel_t27
    float g     = grad[lidx];
    ...
    // AdaBelief 2nd-moment blend (ab_blend=1.0 → byte-exact Adan parity via branch).
    // ab_blend is uniform across all threads (kernel arg) → no warp divergence.
    float g_adan  = g + 0.92f * g_diff;                    // Adan variance target
    float n_tgt;
    if (ab_blend < 0.9999f) {
        float g_ab_e = g - m_new;                           // AdaBelief belief-error
        float n_ab   = g_ab_e * g_ab_e + 1e-16f;           // ε² guard
        n_tgt = ab_blend * g_adan * g_adan + (1.0f - ab_blend) * n_ab;
    } else {
        n_tgt = g_adan * g_adan;                            // byte-exact parity path
    }
    ...
    // QHAdam locked to qh_nu=1.0 (parity, swap#3 DEAD).
    float m_qh = qh_nu * m_new + (1.0f - qh_nu) * g;
    // Sophia CASSURE#3: replace sqrt denominator with linear h=n_new + clip at 1/rho.
    // sophia_rho=0 → standard AdanW sqrt path (near-parity, PTX-drift only).
    // sophia_rho>0 → Sophia step: clip(m/h, 1/sophia_rho) without sqrt.
    float adan_dir;
    if (sophia_rho > 1e-7f) {
        float raw_ratio = (m_qh + 0.92f * v_new) / fmaxf(n_new, eps);
        float clip_b    = 1.0f / sophia_rho;
        adan_dir = copysignf(fminf(fabsf(raw_ratio), clip_b), raw_ratio);
    } else {
        adan_dir = (m_qh + 0.92f * v_new) / (sqrtf(n_new) + eps);
    }
    float wd_gate  = (adan_dir * theta >= 0.0f) ? 1.0f : 0.0f;
    ...
    // EMA-Nesterov acceleration wrapper (b = EMA of base updates).
    // ema_beta=0 ⇒ delta = d_adan byte-identical to baseline.
...
// ─── L5 Muon Newton-Schulz kernels (i12) ─────────────────────────────────────
// All operate on 256×256 = 65536-element matrices (hidden weight momentum m).
// Kernels use unsigned long long device pointers to avoid cudarc type gymnastics.

#define NS_DIM  256
#define NS_TILE 16

// Frobenius norm² via blockwise reduction + atomicAdd to sq_sum.
// Launch: 256 blocks × 256 threads covers exactly 65536 elements.
    ...
    int n)
{
    ...
    float* sq_sum = (float*)sq_ptr;
    __shared__ float smem[256];
    int gid = (int)(blockIdx.x * blockDim.x + threadIdx.x);
    smem[threadIdx.x] = (gid < n) ? m[gid] * m[gid] : 0.0f;
    __syncthreads();
    for (int s = 128; s > 0; s >>= 1) {
        if ((int)threadIdx.x < s) smem[threadIdx.x] += smem[threadIdx.x + s];
        __syncthreads();
    }
    if (threadIdx.x == 0) atomicAdd(sq_sum, smem[0]);
}

// Normalize m into dst: dst[i] = m[i] / sqrt(*sq_sum + eps).
...
// C = A^T @ B (all NS_DIM×NS_DIM row-major). Used as A^T@A for Gram matrix.
    ...
    unsigned long long C_ptr)
{
    ...
    int row = (int)(blockIdx.y * NS_TILE + threadIdx.y); // col of A = row of A^T
    int col = (int)(blockIdx.x * NS_TILE + threadIdx.x);
    float sum = 0.0f;
    #pragma unroll
    for (int t = 0; t < NS_DIM / NS_TILE; t++) {
        // A^T tile: sA[ty][tx] = A[k][row] where k = t*NS_TILE + tx
        sA[threadIdx.y][threadIdx.x] = A[(t * NS_TILE + (int)threadIdx.x) * NS_DIM + row];
        // B tile: sB[ty][tx] = B[k][col] where k = t*NS_TILE + ty
...
// C = A @ B (all NS_DIM×NS_DIM row-major).
// block (16,16,1), grid (16,16,1).
    ...
    unsigned long long C_ptr)
{
    ...
    #pragma unroll
    for (int t = 0; t < NS_DIM / NS_TILE; t++) {
        // A tile: sA[ty][tx] = A[row][k] where k = t*NS_TILE + tx
...
// poly[gid] = (row==col ? a : 0) + b*X[gid] + c*X2[gid].
// Coefficients: a=3.4445, b=-4.7750, c=2.0315 (NS quintique, dc7f3eab).
...
// EMA-Nesterov wrapper + write Muon delta (overrides fused AdanW delta for HW tensors).
// d_muon = -muon_lr * ns_dir[i]
// b_new = ema_gamma*b_old + (1-ema_gamma)*d_muon
// delta[i] = d_muon + ema_beta*(b_new - b_old)
...
extern "C" __global__ __launch_bounds__(128, 6) void dual_consensus_fisher_kernel_14(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        // Shift toward fisher-normalized track (more stable for deep nets)
        ...
        // Enhanced trust with alignment boost (like t26)
...
// i54: GrokFast-EMA — amplify slow-frequency gradient components before optimizer.
// Computes: mu[i] = alpha*mu[i] + (1-alpha)*g[i]; g_aug[i] = g[i] + lambda*mu[i]
// lambda=0 → never launched (sentinel skips this kernel entirely in Rust).
// Uses unsigned long long pointers consistent with NS kernels.
...
// i57: MARS variance-reduction — DRIFT-FREE pre-kernel elementwise channel (calqué GrokFast i54).
// The i42 canonical fused kernel stays BYTE-UNTOUCHED; MARS runs HERE as a separate pass and only
// the augmented gradient c is fed downstream. This is the fix for the i49/i56 in-kernel drift
// (adding any arithmetic term to adabelief_fused_kernel_t27 changed FMA/reduction order → −4.27%).
//   c_i = g_i + clamp( gamma·49·(g_i − g_prev_i), ±(κ·|g_i|+eps) )   (β₁=0.98 ⇒ β₁/(1−β₁)=49)
//   κ = mars_clip_scale HP (1.0 → parity with i59; <1.0 → tighter cap → allows γ↑ without spikes).
//   g_diff = 0 on first_step (g_prev buffer is zero-init ⇒ spurious huge corr suppressed).
//   PURE elementwise, NO reduction (per-coordinate clip, not clip-by-norm) → byte-exact when skipped.
//   gamma=0 → never launched (sentinel skips this kernel in Rust) → same path as i42 → 601752 exact.
//   g_prev buffer stores RAW g for next step's diff (independent of the fused kernel's own prev_grad).
    ...
    int n)
{
    ...
    float cap  = mars_clip_scale * fabsf(gi) + eps;  // i61: κ-scaled per-coord clip (HP mars_clip_scale)
    corr = fminf(fmaxf(corr, -cap), cap);
    c[i]      = gi + corr;                            // variance-reduced gradient (= gi at gamma=0)
    g_prev[i] = gi;                                   // store RAW g for next step
}
