...
fn identify_bn_affine_params(param_sizes: &[usize]) -> Vec<bool> {
    let mut is_bn_affine = vec![false; param_sizes.len()];

    let mut i = 0_usize;
    while i + 5 < param_sizes.len() {
        if param_sizes[i + 2] == 256 && param_sizes[i + 3] == 256 &&
           param_sizes[i + 4] == 256 && param_sizes[i + 5] == 256 {
            is_bn_affine[i + 2] = true;
            is_bn_affine[i + 3] = true;
        }
        i += 6;
    }
...
thread_local! {
    ...
    static BN_AFFINE_MASK: std::cell::RefCell<Vec<bool>> = std::cell::RefCell::new(Vec::new());
}
...
fn optimizer_init(
    ...
    let bn_affine_mask = identify_bn_affine_params(param_sizes);
    BN_AFFINE_MASK.with(|m| *m.borrow_mut() = bn_affine_mask);
...
fn optimizer_step(
    ...
    let bn_affine = BN_AFFINE_MASK.with(|m| m.borrow().clone());
...
;
        ...
        let is_bn_affine = i < bn_affine.len() && bn_affine[i];
        ...
        let mut effective_lr = lr * layer_multiplier * precision_gain * forward_gain * trust_backoff * trend_trust;

        if is_bn_affine {
            effective_lr = effective_lr * s.bn_layer_boost;
            if s.step_count > s.warmup_steps {
                let bn_lr_floor = 0.00075f32;
                effective_lr = effective_lr.max(bn_lr_floor);
            }
        }
