extern "C" __global__ __launch_bounds__(512, 4) void sign_ef_consensus_kernel_18(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float max_step = fmaxf(rel_update_cap * (wabs + abs_floor), min_step);
...
extern "C" __global__ __launch_bounds__(512, 4) void dual_consensus_fisher_kernel_18(
    const float* __restrict__ gradients,
    const float* __restrict__ params,
    ...
    const float fisher_beta = 0.98f;
    const float one_minus_fisher_beta = 1.0f - fisher_beta;
    const float ortho_mix = fminf(fmaxf(0.22f + 0.4f * blend_sign, 0.0f), 0.62f);
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float gamma_local = (g * pg >= 0.0f) ? nesterov_gamma : (0.15f * nesterov_gamma);
        ...
        float consensus_mix = 0.32f * (1.0f - agree) + 0.25f * vol + 0.28f * blend_sign + 0.15f * (1.0f - stability);
