...
fn optimizer_step(
    ...
    let base_prefer_robust = s.step_count > s.warmup_steps && (near_floor || late_phase);
    ...
    let k_stability = module.load_function("layer_stability_reduction_kernel_7")?;
    let mut updates = Vec::with_capacity(gradients.len());
    let num_layers = gradients.len();
    let do_layer_trust = s.step_count > s.warmup_steps;
    let mut stat_bufs: Vec<CudaSlice<f32>> = Vec::with_capacity(num_layers);
    let mut stability_bufs: Vec<CudaSlice<f32>> = Vec::with_capacity(num_layers);
    for i in 0..num_layers {
        let num_blocks = s.cfgs[i].grid_dim.0 as usize;
        stat_bufs.push(stream.alloc_zeros::<f32>(2 * num_blocks)?);
        stability_bufs.push(stream.alloc_zeros::<f32>(3 * num_blocks)?);
    }
    for i in 0..num_layers {
        ...
        unsafe {
            stream.launch_builder(&k_norm)
                ...
                .arg(&(n as u32))
                .launch(cfg)?;
            stream.launch_builder(&k_stability)
                .arg(g)
                .arg(pg)
                .arg(m_ref)
                .arg(v_ref)
                .arg(f_ref)
                .arg(&mut stability_bufs[i])
                .arg(&(n as u32))
                .arg(&beta1_eff)
                .arg(&beta2_eff)
    ...
    let mut layer_trusts = vec![1.0f32; num_layers];
    let mut layer_prev_agrees = vec![1.0f32; num_layers];
    let mut layer_path_agrees = vec![1.0f32; num_layers];
    let mut layer_instabilities = vec![0.0f32; num_layers];
    let mut grad_norm_sum = 0.0f32;
    let mut instability_sum = 0.0f32;
    let mut non_empty_layers = 0usize;
    for i in 0..num_layers {
        let n = gradients[i].len();
        if n == 0 { continue; }
        let num_blocks = s.cfgs[i].grid_dim.0 as usize;
        let host_vals: Vec<f32> = stream.memcpy_dtov(&stat_bufs[i])?;
        let mut param_sq = 0.0f32;
        let mut grad_sq = 0.0f32;
        for b in 0..num_blocks {
            param_sq += host_vals[2 * b];
            grad_sq  += host_vals[2 * b + 1];
        }
        let param_norm = param_sq.max(0.0f32).sqrt();
        let grad_norm = grad_sq.max(0.0f32).sqrt();
        grad_norm_sum += grad_norm;
        if do_layer_trust && param_norm > 0.0f32 && grad_norm > 0.0f32 {
            let trust_ratio = ((param_norm + eps_eff) / (grad_norm + eps_eff)).sqrt();
            layer_trusts[i] = trust_ratio.clamp(gate_lo, gate_hi);
        }
        let host_stability: Vec<f32> = stream.memcpy_dtov(&stability_bufs[i])?;
        let mut flip_total = 0.0f32;
        let mut prev_total = 0.0f32;
        let mut path_total = 0.0f32;
        for b in 0..num_blocks {
            flip_total += host_stability[3 * b];
            prev_total += host_stability[3 * b + 1];
            path_total += host_stability[3 * b + 2];
        }
        let inv_n = 1.0f32 / (n as f32);
        let flip_rate = (flip_total * inv_n).clamp(0.0f32, 1.0f32);
        let prev_agree = (prev_total * inv_n).clamp(0.0f32, 1.0f32);
        let path_agree = (path_total * inv_n).clamp(0.0f32, 1.0f32);
        layer_prev_agrees[i] = prev_agree;
        layer_path_agrees[i] = path_agree;
        let instability = flip_rate + (1.0f32 - prev_agree) + (1.0f32 - path_agree);
        layer_instabilities[i] = instability;
        instability_sum += instability;
        non_empty_layers += 1;
    }
    let mean_instability = if non_empty_layers > 0 {
        instability_sum / non_empty_layers as f32
    } else {
        0.0f32
    };
    let mut layer_use_robust = vec![false; num_layers];
    for i in 0..num_layers {
        if gradients[i].len() == 0 { continue; }
        layer_use_robust[i] = if s.step_count <= s.warmup_steps {
            false
        } else if base_prefer_robust {
            layer_instabilities[i] >= mean_instability
        } else {
            layer_instabilities[i] > mean_instability && layer_path_agrees[i] < layer_prev_agrees[i]
        };
    }
    s.bpe_ema = grad_norm_sum;

    let do_svrg = s.step_count >= s.warmup_steps && layer_use_robust.iter().any(|&use_robust| !use_robust);
    ...
    if do_svrg {
        for i in 0..num_layers {
            let n = gradients[i].len();
            if n == 0 || layer_use_robust[i] { continue; }
    ...
    for (i, g) in gradients.iter().enumerate() {
        ...
        let effective_lr = lr * layer_multiplier * precision_gain * forward_gain * trust_backoff * trend_factor * layer_trusts[i];
        let cfg = s.cfgs[i];
        let update_buf_ref = &mut s.upd[i];
        let use_robust = layer_use_robust[i];
        let g_input: &CudaSlice<f32> = if do_svrg && !use_robust {
