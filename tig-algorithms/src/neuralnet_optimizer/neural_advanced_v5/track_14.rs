...
fn optimizer_step(state: &mut dyn OptimizerStateTrait, model_params: &[CudaSlice<f32>], gradients: &[CudaSlice<f32>], epoch: usize, _train_loss: Option<f32>, val_loss: Option<f32>, stream: Arc<CudaStream>, module: Arc<CudaModule>, _prop: &cudaDeviceProp) -> Result<Vec<CudaSlice<f32>>> {
    ...
    let k_proj_stats = module.load_function("layer_proj_stats_kernel_14")?;
    ...
    for (i, g) in gradients.iter().enumerate() {
        ...
        let cfg = s.cfgs[i];
        let mut d_proj_stats = stream.alloc_zeros::<f32>(2)?;
        unsafe {
            stream.launch_builder(&k_proj_stats)
                .arg(&model_params[i])
                .arg(g)
                .arg(&mut d_proj_stats)
                .arg(&(n as u32))
                .launch(cfg)?;
        }
        let proj_stats: Vec<f32> = stream.memcpy_dtov(&d_proj_stats)?;
        let proj_coeff: f32 = if proj_stats[0] > 0.0 {
            proj_stats[1] / (proj_stats[0] + eps_eff)
        } else {
            0.0
        };
