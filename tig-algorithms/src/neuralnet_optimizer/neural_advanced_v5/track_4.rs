...
type LayerGainStore = HashMap<usize, (Vec<f32>, Vec<f32>)>;

fn layer_gain_store() -> &'static Mutex<LayerGainStore> {
    static STORE: OnceLock<Mutex<LayerGainStore>> = OnceLock::new();
...
fn optimizer_step(
    ...
    let state_key = s as *mut OptimizerState as usize;
    let (mut layer_gains, mut ratio_emas) = {
        let mut store = layer_gain_store().lock().unwrap();
        let entry = store
            .entry(state_key)
            .or_insert_with(|| (vec![1.0; last], vec![0.0; last]));
        if entry.0.len() != last || entry.1.len() != last {
            *entry = (vec![1.0; last], vec![0.0; last]);
        }
    ...
    let k_norms = module.load_function("tensor_sqnorms_block_kernel")?;
    let k_scale = module.load_function("scale_updates_kernel")?;
    ...
    for (i, g) in gradients.iter().enumerate() {
        ...
        let mut scheduled_lr =
            s.layer_lrs[i] * lr_factor * layer_mul * precision_gain * forward_gain;
        if use_robust {
            scheduled_lr *= 0.72;
        }
        let target_ratio = scheduled_lr.clamp(eps_eff, 0.0130);

        let mut effective_lr = scheduled_lr * layer_gains[i];
        ...
        let mut d_block_sqnorms = stream.alloc_zeros::<f32>(2 * num_blocks)?;

        unsafe {
            stream
                .launch_builder(&k_norms)
                .arg(p)
                .arg(&s.upd[i])
                .arg(&mut d_block_sqnorms)
                .arg(&(n as u32))
                .launch(cfg)?;
        }

        let block_sqnorms_h: Vec<f32> = stream.memcpy_dtov(&d_block_sqnorms)?;
        let mut param_sq = 0.0f32;
        let mut update_sq = 0.0f32;
        for b in 0..num_blocks {
            param_sq  += block_sqnorms_h[2 * b];
            update_sq += block_sqnorms_h[2 * b + 1];
        }
        let param_norm = param_sq.max(0.0).sqrt();
        let update_norm = update_sq.max(0.0).sqrt();

        let trust_ratio = if param_norm.is_finite()
            && update_norm.is_finite()
            && param_norm > eps_eff
            && update_norm > param_norm
        {
            param_norm / (update_norm + eps_eff)
        } else {
            1.0
        };

        let realized_update_norm = update_norm * trust_ratio;

        if trust_ratio < 1.0 {
            unsafe {
                stream
                    .launch_builder(&k_scale)
                    .arg(&mut s.upd[i])
                    .arg(&(n as u32))
                    .arg(&trust_ratio)
                    .launch(cfg)?;
        ...
        if param_norm.is_finite() && realized_update_norm.is_finite() && param_norm > eps_eff {
            let realized_ratio = realized_update_norm / (param_norm + eps_eff);
            let ema_prev = ratio_emas[i];
            let ema = if ema_prev > 0.0 && ema_prev.is_finite() {
                0.9 * ema_prev + 0.1 * realized_ratio
            } else {
                realized_ratio
            };
            ratio_emas[i] = ema;

            let correction = (target_ratio / (ema + eps_eff)).sqrt();
            if correction.is_finite() {
                layer_gains[i] = (layer_gains[i] * correction).clamp(0.25, 4.0);
            }
        }
    ...
    {
        let mut store = layer_gain_store().lock().unwrap();
        store.insert(state_key, (layer_gains, ratio_emas));
    }
