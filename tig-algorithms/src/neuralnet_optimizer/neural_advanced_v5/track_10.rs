...
fn optimizer_init(
    ...
    for &n in param_sizes {
        ...
        let mut slow_fisher_init = stream.alloc_zeros::<f32>(n)?;
        stream.memcpy_htod(&vec![1e-4f32; n], &mut slow_fisher_init)?;
        v.push(slow_fisher_init);
...
fn compute_trend_factor(history: &[f32], best: f32) -> f32 {
    ...
    let inv_best = 1.0f32 / (best.abs() + 1.0e-8f32);
    let mut ema_gap = 0.0f32;
    let mut damage = 0.0f32;

    for &loss in history.iter() {
        let gap = (loss - best).max(0.0f32) * inv_best;
        let excess = (gap - 0.5f32 * ema_gap).max(0.0f32);
        let recovery = (ema_gap - gap).max(0.0f32);

        damage = (0.85f32 * damage + excess - recovery).max(0.0f32);
        ema_gap = 0.75f32 * ema_gap + 0.25f32 * gap;
    ...
    let recent_gap = (history[n - 1] - best).max(0.0f32) * inv_best;
    let r = 1.0f32 + recent_gap.max(damage);
