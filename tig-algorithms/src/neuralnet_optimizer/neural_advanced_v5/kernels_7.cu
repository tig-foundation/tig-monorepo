extern "C" __global__ __launch_bounds__(256, 4) void grad_norm_reduction_kernel_7(
    ...
    __shared__ float smem_p[8];
    __shared__ float smem_g[8];
    const unsigned int tid = blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned int stride = blockDim.x * gridDim.x;
    const unsigned int lane = threadIdx.x & 31;
    const unsigned int warp_id = threadIdx.x >> 5;

    float local_p_sq = 0.0f;
    ...
    for (int offset = 16; offset > 0; offset >>= 1) {
        local_p_sq += __shfl_down_sync(0xffffffff, local_p_sq, offset);
        local_g_sq += __shfl_down_sync(0xffffffff, local_g_sq, offset);
    }
    if (lane == 0) {
        smem_p[warp_id] = local_p_sq;
        smem_g[warp_id] = local_g_sq;
    }
    __syncthreads();

    if (warp_id == 0) {
        float p_val = (lane < 8) ? smem_p[lane] : 0.0f;
        float g_val = (lane < 8) ? smem_g[lane] : 0.0f;
        #pragma unroll
        for (int offset = 4; offset > 0; offset >>= 1) {
            p_val += __shfl_down_sync(0xffffffff, p_val, offset);
            g_val += __shfl_down_sync(0xffffffff, g_val, offset);
        }
        if (lane == 0) {
            stats_out[2u * blockIdx.x]      = p_val;
            stats_out[2u * blockIdx.x + 1u] = g_val;
        }
    }
}

extern "C" __global__ __launch_bounds__(256, 4) void layer_stability_reduction_kernel_7(
    const float* __restrict__ gradients,
    const float* __restrict__ prev_grad,
    ...
    __shared__ float smem_flip[8];
    __shared__ float smem_prev[8];
    __shared__ float smem_path[8];
    ...
    float local_flip = 0.0f;
    float local_prev = 0.0f;
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float denom = sqrt_v + eps;
        const float inv_denom = 1.0f / fmaxf(denom, 1.0e-12f);

        float base_dir = blend_adam * (m_hat * inv_denom) + blend_norm * (g_pred * inv_denom) + blend_sign * copysignf(1.0f, m_hat);
        const float overlap = copysignf(fminf(fabsf(g_pred), fabsf(m_hat)), m_hat);
        const float g_ortho = g_pred - overlap;
        const float ortho_dir = g_ortho * inv_denom;
        base_dir = (1.0f - ortho_mix) * base_dir + ortho_mix * ortho_dir;

        const float fisher_rms = sqrtf(fmaxf(fisher_diag[idx], 1.0e-8f)) + eps;
        const float robust_dir = 0.45f * copysignf(1.0f, m_hat) + 0.55f * (g_pred / fisher_rms);

        const float prev_metric = 0.5f + 0.5f * ((g * pg) / (fabsf(g) * fabsf(pg) + eps));
        const float path_metric = 0.5f + 0.5f * ((base_dir * robust_dir) / (fabsf(base_dir) * fabsf(robust_dir) + eps));

        local_flip += (g * pg < 0.0f) ? 1.0f : 0.0f;
        local_prev += fminf(fmaxf(prev_metric, 0.0f), 1.0f);
        local_path += fminf(fmaxf(path_metric, 0.0f), 1.0f);
    }
    #pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
        local_flip += __shfl_down_sync(0xffffffff, local_flip, offset);
        local_prev += __shfl_down_sync(0xffffffff, local_prev, offset);
        local_path += __shfl_down_sync(0xffffffff, local_path, offset);
    }
    if (lane == 0) {
        smem_flip[warp_id] = local_flip;
        smem_prev[warp_id] = local_prev;
        smem_path[warp_id] = local_path;
    }
    __syncthreads();

    if (warp_id == 0) {
        float flip_val = (lane < 8) ? smem_flip[lane] : 0.0f;
        float prev_val = (lane < 8) ? smem_prev[lane] : 0.0f;
        float path_val = (lane < 8) ? smem_path[lane] : 0.0f;
        #pragma unroll
        for (int offset = 4; offset > 0; offset >>= 1) {
            flip_val += __shfl_down_sync(0xffffffff, flip_val, offset);
            prev_val += __shfl_down_sync(0xffffffff, prev_val, offset);
            path_val += __shfl_down_sync(0xffffffff, path_val, offset);
        }
        if (lane == 0) {
            stats_out[3u * blockIdx.x]      = flip_val;
            stats_out[3u * blockIdx.x + 1u] = prev_val;
            stats_out[3u * blockIdx.x + 2u] = path_val;
        }
    }
}
...
extern "C" __global__ __launch_bounds__(256, 3) void dual_consensus_fisher_kernel_7(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float trust_radius = fminf(fmaxf(param_mag / mom_mag, 0.5f), 2.0f);
        ...
        const float block_norm_scale = 1.0f / fmaxf(0.5f + 0.5f * elem_rms, 1.0e-8f);
