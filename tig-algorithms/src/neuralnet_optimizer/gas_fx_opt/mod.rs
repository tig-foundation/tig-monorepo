...
fn new_optimizer_state(
    ...
    let mut d_layer_grads_ptrs = unsafe { stream.alloc::<u64>(param_sizes.len())? };
...
fn optimizer_step_compute_only(
    ...
    // ========================================================================
    // STEP 0a: Validate weights match gradients (v0.0.5 requirement)
    // ========================================================================
    let has_weights = !weights.is_empty() && weights.len() == gradients.len();
...
fn optimizer_step_full(
    ...
    // v0.0.5: If weights available, compute weight-based CONE scaling
    let mut cone_group_weights = vec![1.0f32; gradients.len()];
    if has_weights {
        // CONE multi-objective: balance speed/accuracy/memory via weight norms
        for (i, (weight, grad)) in weights.iter().zip(gradients.iter()).enumerate() {
            // Placeholder: weight magnitude could inform CONE allocation
            // In full implementation: compute norm, derive Pareto-optimal weights
            cone_group_weights[i] = 1.0f32;
        }
    }
    ...
    // v0.0.5: Choose kernel based on weight availability
    if has_weights {
        // Use weight-aware kernel for better adaptive learning rates
        for (layer_idx, (grad, weight)) in gradients.iter().zip(weights.iter()).enumerate() {
            let cfg = state.launch_cache.get_config(grad.len());
            let weight_aware_kernel = module.load_function("gas_fx_weight_aware_kernel")?;
            
            unsafe {
                stream
                    .launch_builder(&weight_aware_kernel)
                    .arg(state.updates[layer_idx].device_ptr(&stream).0 as *mut f32)
                    .arg(grad)
                    .arg(weight)
                    .arg(state.m[layer_idx].device_ptr(&stream).0 as *mut f32)
                    .arg(state.v_fp16[layer_idx].device_ptr(&stream).0 as *mut u16)
                    .arg(&effective_lr)
                    .arg(&state.beta1)
                    .arg(&state.beta2)
                    .arg(&state.eps)
        ...
        // Fallback to mega-fused kernel (traditional path without weights)
        let mega_kernel = module.load_function("gas_fx_mega_fused_kernel")?;
        let num_layers = gradients.len() as i32;
    ...
    if cone_trigger && has_weights {
        // v0.0.5: CONE negotiator now has weight information for better allocation
        // Example: update utility vectors based on weight magnitudes
        // This informs Pareto frontier construction in cone_group_weights
    }
