extern "C" __global__ __launch_bounds__(512, 4) void sign_ef_consensus_kernel_original(
...
extern "C" __global__ __launch_bounds__(512, 4) void dual_consensus_fisher_kernel_original(
    const float* __restrict__ gradients,
    const float* __restrict__ params,    
...
extern "C" __global__ __launch_bounds__(256, 3) void sign_ef_consensus_kernel(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        slow_update[idx] = su;
        updates[idx] = adj_update - (lr * weight_decay) * params[idx];
    }
}
