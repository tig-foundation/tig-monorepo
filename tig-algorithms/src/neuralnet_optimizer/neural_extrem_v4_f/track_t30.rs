...
impl Default for TrackHP {
    fn default() -> Self {
        TrackHP { total_steps: 1800, warmup_steps: 60,
            beta1: 0.92, beta2: 0.997, weight_decay: 0.0022,
            spectral_boost: 1.1, bn_layer_boost: 2.00 }
    }
}
thread_local! {
...
fn optimizer_init(
    ...
    let state = OptimizerState {
        ...
        spectral_boost: { let c = TRACK_HP.with(|c| c.borrow().clone()); c.spectral_boost }, step_count: 0,
        warmup_steps: { TRACK_HP.with(|c| c.borrow().warmup_steps) },
        total_steps: { TRACK_HP.with(|c| c.borrow().total_steps) },
        noise_variance: 0.040, val_loss_history: Vec::new(),
        beta1: { TRACK_HP.with(|c| c.borrow().beta1) },
        beta2: { TRACK_HP.with(|c| c.borrow().beta2) },
        eps: 1e-8,
        weight_decay: { TRACK_HP.with(|c| c.borrow().weight_decay) },
        bn_layer_boost: { TRACK_HP.with(|c| c.borrow().bn_layer_boost) },
        output_layer_damping: 0.80,
        prev_val_loss: None, best_val_loss: None, plateau_count: 0, slope_ema: 0.0, lr_boost: 1.0,
        last_pulse_step: 0, last_epoch: 0, steps_in_epoch: 0, bpe_ema: 1.0, phase_tempo: 1.0, spectral_decay: 0.4, best_w: None, tt_best_loss: f32::INFINITY, tt_cooldown: 0, global_lr_mult: 1.0, tt_prev_epoch: 0,
    };
