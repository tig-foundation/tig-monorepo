extern "C" __global__ __launch_bounds__(128, 6) void sign_ef_consensus_kernel_14(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        ef_residual[idx] = ef_old - u_quant;
...
extern "C" __global__ __launch_bounds__(128, 6) void dual_consensus_fisher_kernel_14(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float trust = 1.0f / (1.0f + 0.5f * vol + 0.5f * flip);
        chosen_update *= trust;
...
extern "C" __global__ __launch_bounds__(256) void tt_save_kernel(
    const float* __restrict__ params,
    float* __restrict__ best_w,
    const unsigned int n
) {
    const unsigned int tid = blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned int stride = blockDim.x * gridDim.x;
    for (unsigned int idx = tid; idx < n; idx += stride) {
        best_w[idx] = params[idx];
    }
}

extern "C" __global__ __launch_bounds__(256) void tt_restore_kernel(
    const float* __restrict__ params,
    const float* __restrict__ best_w,
    float* __restrict__ updates,
    float* __restrict__ m,
    float* __restrict__ v,
    float* __restrict__ prev_g,
    float* __restrict__ prev_u,
    float* __restrict__ slow_u,
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        m[idx] = 0.0f; v[idx] = 0.0f;
        prev_g[idx] = 0.0f; prev_u[idx] = 0.0f;
        slow_u[idx] = 0.0f; f[idx] = 1.0e-4f; ef[idx] = 0.0f;
    }
}

extern "C" __global__ __launch_bounds__(256) void asam_perturb_kernel(
    const float* __restrict__ params,
    ...
    const unsigned int tid = blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned int stride = blockDim.x * gridDim.x;
    if (rho <= 0.0f) { for (unsigned int idx = tid; idx < n; idx += stride) p_out[idx] = params[idx]; return; }
    const float inv_bc1 = 1.0f / fmaxf(bc1, 1.0e-12f);
    const float inv_bc2 = 1.0f / fmaxf(bc2, 1.0e-12f);
    for (unsigned int idx = tid; idx < n; idx += stride) {
        const float p = params[idx];
        const float m_hat = m[idx] * inv_bc1;
        const float v_hat = v[idx] * inv_bc2;
        const float denom = sqrtf(fmaxf(v_hat, 0.0f)) + eps;
        const float dir = m_hat / denom;
        const float scale = fmaxf(fabsf(p), 0.05f);
        p_out[idx] = p + rho * scale * dir;
    }
}
