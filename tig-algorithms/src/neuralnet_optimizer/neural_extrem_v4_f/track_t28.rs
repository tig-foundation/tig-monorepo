...
fn parse_config(hyperparameters: &Option<Map<String, Value>>) -> TrackConfig {
    ...
    TrackConfig {
        total_steps: hp.and_then(|h| h.get("total_steps").and_then(|v| v.as_u64())).unwrap_or(660) as usize,
        warmup_steps: hp.and_then(|h| h.get("warmup_steps").and_then(|v| v.as_u64())).unwrap_or(50) as usize,
        noise_variance: hp.and_then(|h| h.get("noise_variance").and_then(|v| v.as_f64())).unwrap_or(0.020) as f32,
        spectral_boost: hp.and_then(|h| h.get("spectral_boost").and_then(|v| v.as_f64())).unwrap_or(0.95) as f32,
...
fn optimizer_init(_seed: [u8; 32], param_sizes: &[usize], stream: Arc<CudaStream>, _module: Arc<CudaModule>, prop: &cudaDeviceProp) -> Result<Box<dyn OptimizerStateTrait>> {
    TRACK_CONFIG.with(|c| {
        ...
        let fisher_host = vec![1e-4f32; max_n];
        let mut m = Vec::with_capacity(param_sizes.len()); let mut v = Vec::with_capacity(param_sizes.len());
        let mut prev_g = Vec::with_capacity(param_sizes.len()); let mut prev_u = Vec::with_capacity(param_sizes.len());
        let mut slow_u = Vec::with_capacity(param_sizes.len()); let mut f = Vec::with_capacity(param_sizes.len());
        let mut ef = Vec::with_capacity(param_sizes.len()); let mut upd = Vec::with_capacity(param_sizes.len());
        for &n in param_sizes {
            ...
            if n > 0 { stream.memcpy_htod(&fisher_host[..n], &mut fisher_init)?; }
            f.push(fisher_init); ef.push(stream.alloc_zeros::<f32>(n)?);
            upd.push(unsafe { stream.alloc::<f32>(n)? });
        }
        // NITT: pre-allocate best_w buffers for soft rollback
        let mut best_w_bufs = Vec::with_capacity(param_sizes.len());
        for &n in param_sizes {
        ...
        for &n in param_sizes { let calc_blocks = (n as u32 + cfg.threads_per_block - 1) / cfg.threads_per_block; cfgs.push(LaunchConfig { grid_dim: (calc_blocks.min(sm_blocks).max(1), 1, 1), block_dim: (cfg.threads_per_block, 1, 1), shared_mem_bytes: 0 }); }
        let mut layer_lrs = Vec::with_capacity(param_sizes.len());
        for (i, &ps) in param_sizes.iter().enumerate() { let mut lr = 0.0012f32; if i == 0 { lr = 0.0011; } if ps <= 512 { lr = 0.0018; } if ps > 50000 { lr = 0.0009; } if i == param_sizes.len() - 1 { lr = 0.0007; } layer_lrs.push(lr); }
        Ok(Box::new(OptimizerState { m, v, prev_g, prev_u, slow_u, f, ef, upd, cfgs, layer_lrs, spectral_boost: cfg.spectral_boost, step_count: 0, warmup_steps: cfg.warmup_steps, total_steps: cfg.total_steps, noise_variance: cfg.noise_variance, val_loss_history: Vec::new(), beta1: cfg.beta1, beta2: cfg.beta2, eps: cfg.eps, weight_decay: cfg.weight_decay, bn_layer_boost: cfg.bn_layer_boost, output_layer_damping: cfg.output_layer_damping, prev_val_loss: None, best_val_loss: None, plateau_count: 0, slope_ema: 0.0, lr_boost: 1.0, last_pulse_step: 0, last_epoch: 0, steps_in_epoch: 0, bpe_ema: 1.0, phase_tempo: 1.0, spectral_decay: 0.3, best_w: Some(best_w_bufs), tt_best_loss: f32::INFINITY, tt_cooldown: 0, global_lr_mult: 1.0, tt_prev_epoch: 0 }) as Box<dyn OptimizerStateTrait>)
    })
}
...
fn optimizer_step(state: &mut dyn OptimizerStateTrait, model_params: &[CudaSlice<f32>], gradients: &[CudaSlice<f32>], epoch: usize, _train_loss: Option<f32>, val_loss: Option<f32>, stream: Arc<CudaStream>, module: Arc<CudaModule>, _prop: &cudaDeviceProp) -> Result<Vec<CudaSlice<f32>>> {
    ...
    update_state_from_val_loss(s, epoch, val_loss); let global_damp = compute_global_damp(s, val_loss);

    // NITT: Soft rollback logic
    ...
    let t = s.step_count as i32; let bias_correction1 = 1.0 - s.beta1.powi(t); let bias_correction2 = 1.0 - s.beta2.powi(t);
    let (blend_adam, blend_norm, blend_sign, nesterov_gamma, bb_blend, lookahead_alpha, lookahead_tau) = compute_blends(s, val_loss);
    let near_floor = val_loss.map_or(false, |loss| loss <= s.noise_variance * 3.0); let late_phase = s.step_count > s.total_steps * 3 / 4;
    ...
    let beta1_eff: f32 = if in_precision_zone { (s.beta1 + 0.02).min(0.995) } else { s.beta1 }; let beta2_eff: f32 = s.beta2;
    ...
    if s.step_count > s.warmup_steps { if near_floor { wd_eff *= 1.10; } else if s.plateau_count >= 20 { wd_eff *= 0.50; } }
    wd_eff *= (1.0 / s.phase_tempo).clamp(0.6, 1.0);
    let trust_backoff: f32 = if let (Some(prev), Some(curr)) = (s.prev_val_loss, val_loss) { let delta = curr - prev; if delta > 2e-4 { 1.0 / (1.0 + 1.5 * (delta / (prev.abs() + 1e-8)).min(0.02)) } else { 1.0 } } else { 1.0 };
    ...
    let k_nitt = module.load_function("nitt_restore_kernel_18")?;
    ...
    for (i, g) in gradients.iter().enumerate() {
        ...
        let cfg_nitt = LaunchConfig { grid_dim: (((n as u32 + 255) / 256).max(1), 1, 1), block_dim: (256, 1, 1), shared_mem_bytes: 0 };

        // NITT save: copy current best weights
        if nitt_save {
            if let Some(ref mut bw) = s.best_w {
                stream.memcpy_dtod(p, &mut bw[i])?;
            }
        }

        // NITT rollback: soft restore (25% momentum retention)
        if nitt_rollback {
            if let Some(ref bw) = s.best_w {
                let update_buf = &mut s.upd[i];
                unsafe {
                    stream.launch_builder(&k_nitt)
                        .arg(p).arg(&bw[i]).arg(update_buf)
                        .arg(&mut s.m[i]).arg(&mut s.v[i]).arg(&mut s.slow_u[i])
                        .arg(&(n as u32)).launch(cfg_nitt)?;
                }
        ...
        let base_lr = s.layer_lrs[i]; let tempo_lr = (1.0 / s.phase_tempo.powf(0.35)).max(0.6);
        let lr = spectral_phase_lr(s, base_lr) * global_damp * s.lr_boost * tempo_lr * s.global_lr_mult;
        let layer_multiplier = if i == gradients.len() - 1 { if let Some(loss) = val_loss { if s.step_count > s.warmup_steps + 30 { (s.output_layer_damping * (0.7 + 0.3 * (loss / (s.noise_variance * 6.0)).min(1.0))).max(s.output_layer_damping) } else { s.output_layer_damping } } else { s.output_layer_damping } } else if n <= 512 { s.bn_layer_boost } else { 1.0 };
        ...
        let cfg = s.cfgs[i]; let update_buf_ref = &mut s.upd[i];
        unsafe { 
            if use_robust { 
