extern "C" __global__ __launch_bounds__(256, 3) void sign_ef_consensus_kernel(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        ef_delta = __fdividef(ef_delta, (1.0f + __fdividef(fabsf(ef_delta), fmaxf(ef_cap, 1.0e-12f))));
...
extern "C" __global__ __launch_bounds__(256) void asam_perturb_kernel_29(
    const float* __restrict__ params, const float* __restrict__ m, const float* __restrict__ v,
    float* __restrict__ p_out, const unsigned int n, const float rho, const float eps, const float bc1, const float bc2
) {
