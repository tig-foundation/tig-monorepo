...
impl Default for TrackHP {
    fn default() -> Self {
        TrackHP { total_steps: TOTAL_STEPS, warmup_steps: WARMUP_STEPS,
            beta1: 0.90, beta2: 0.995, weight_decay: 0.005,
            spectral_boost: 1.12, bn_layer_boost: 1.50 }
    }
}
...
fn optimizer_init(
    ...
    let state = OptimizerState {
        ...
        bpe_ema: 1.0,
        phase_tempo: 1.0,
        spectral_decay: 0.3, best_w: None, tt_best_loss: f32::INFINITY, tt_cooldown: 0, global_lr_mult: 1.0, tt_prev_epoch: 0,
    };
...
fn optimizer_query(
    ...
    let s = state.as_any().downcast_ref::<OptimizerState>().unwrap();
    let mut rho = 0.0f32;
    if s.step_count > s.warmup_steps + 15 {
        if let Some(vl) = val_loss {
            let threshold = s.noise_variance * 6.0;
            if vl < threshold {
                let proximity = (vl / threshold).clamp(0.2, 1.0);
                rho = 0.030f32 * proximity;
            }
        }
    }
    if rho <= 1e-4 { return Ok(None); }

    let k_asam = module.load_function("asam_perturb_kernel_29")?;
    let mut modified = Vec::with_capacity(params.len());
    ...
    for i in 0..params.len() {
        ...
        let layer_rho = if n > 256 { rho } else { 0.0f32 };
        let cfg = LaunchConfig {
            grid_dim: (((n as u32 + 255) / 256).max(1), 1, 1),
            block_dim: (256, 1, 1),
            shared_mem_bytes: 0,
        };
        unsafe {
            stream.launch_builder(&k_asam)
                .arg(&params[i]).arg(&s.m[i]).arg(&s.v[i]).arg(&mut p_out)
                .arg(&(n as u32)).arg(&layer_rho).arg(&s.eps).arg(&bc1).arg(&bc2)
                .launch(cfg)?;
        }
        modified.push(p_out);
    }
