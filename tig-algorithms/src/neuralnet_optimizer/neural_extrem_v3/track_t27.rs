...
impl Default for TrackConfig {
    fn default() -> Self {
        TrackConfig {
            total_steps: 1000, warmup_steps: 40, noise_variance: 0.040, spectral_boost: 1.1,
            beta1: 0.92, beta2: 0.997, eps: 1e-8, weight_decay: 0.0025,
            bn_layer_boost: 1.47, output_layer_damping: 0.80, threads_per_block: 128, blocks_per_sm: 4,
        }
    }
}
...
fn parse_config(hyperparameters: &Option<Map<String, Value>>) -> TrackConfig {
    ...
    TrackConfig {
        ...
        bn_layer_boost: hp.and_then(|h| h.get("bn_layer_boost").and_then(|v| v.as_f64())).unwrap_or(1.47) as f32,
...
fn optimizer_init(_seed: [u8; 32], param_sizes: &[usize], stream: Arc<CudaStream>, _module: Arc<CudaModule>, prop: &cudaDeviceProp) -> Result<Box<dyn OptimizerStateTrait>> {
    TRACK_CONFIG.with(|c| {
        ...
        for &n in param_sizes {
            let calc_blocks = ((n as u32 + cfg.threads_per_block - 1) / cfg.threads_per_block).min(sm_blocks).max(1);
            cfgs.push(LaunchConfig { grid_dim: (calc_blocks, 1, 1), block_dim: (cfg.threads_per_block, 1, 1), shared_mem_bytes: 0 });
        }
