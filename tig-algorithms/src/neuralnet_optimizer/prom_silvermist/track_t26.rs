...
fn optimizer_step(
    ...
    for (i, g) in gradients.iter().enumerate() {
        ...
        let mut lr = spectral_phase_lr(s, base_lr);
        // Parameter order is [linear w,b] + repeated [bn w,b,mean,var] blocks + output linear.
        // Keep the late-stage floor on the BN affine tensors only; the running stats stay frozen.
        let is_hidden_bn_affine = (i + 2 < num_layers) && (i % 6 == 4 || i % 6 == 5);
        if is_hidden_bn_affine && s.step_count > s.warmup_steps {
            let progress = (s.step_count as f32 / s.total_steps.max(1) as f32).clamp(0.0, 1.0);
            if progress > 0.50f32 {
                let tail = ((progress - 0.50f32) / 0.50f32).clamp(0.0, 1.0);
                let bn_floor = base_lr * (0.24f32 + 0.28f32 * tail);
                lr = lr.max(bn_floor);
            }
        }
        let lr = lr * global_damp * s.lr_boost * tempo_lr;
