...
use super::helpers::{
    OptimizerState,
    compute_blends, update_state_from_val_loss,
    compute_global_damp, compute_precision_params, finalize_state,
    intra_epoch_factor, loss_curvature_factor,
};

#[inline]
fn spectral_phase_lr_floored(s: &OptimizerState, base_lr: f32, lr_min_frac: f32) -> f32 {
    ...
    let cosine_factor = lr_min_frac + (1.0 - lr_min_frac) * 0.5 * (1.0 + (std::f32::consts::PI * progress).cos());
...
struct TrackConfig {
    ...
    init_scale: f32, lr_min_frac: f32,
}
...
impl Default for TrackConfig {
    fn default() -> Self {
        TrackConfig {
            ...
            init_scale: 1.0, lr_min_frac: 0.0,
        }
    }
}
...
fn parse_config(hyperparameters: &Option<Map<String, Value>>) -> TrackConfig {
    ...
    TrackConfig {
        ...
        lr_min_frac: hp.and_then(|h| h.get("lr_min_frac").and_then(|v| v.as_f64())).unwrap_or(0.0) as f32,
    }
}
...
fn optimizer_step(
    ...
    // Must run BEFORE updates.push(s.upd[i].clone()) since clone() = deep copy.
    ...
    for (i, g) in gradients.iter().enumerate() {
        ...
        let lr_min_frac = TRACK_CONFIG.with(|c| c.borrow().lr_min_frac);
        let lr = spectral_phase_lr_floored(s, base_lr, lr_min_frac) * global_damp * s.lr_boost * tempo_lr;
        ...
        // At step 0: add (init_scale-1)*W_0 to upd BEFORE clone (clone = deep copy).
