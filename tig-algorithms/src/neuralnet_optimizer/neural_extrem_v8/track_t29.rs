...
fn parse_config(hyperparameters: &Option<Map<String, Value>>) -> TrackConfig {
    ...
    TrackConfig {
        ...
        pg_base: hp.and_then(|h| h.get("pg_base").and_then(|v| v.as_f64())).unwrap_or(1.02) as f32,
    }
}
...
fn optimizer_step(
    ...
    let (pg_slope, pg_base) = TRACK_CONFIG.with(|c| { let c = c.borrow(); (c.pg_slope, c.pg_base) });
