...
pub struct OptimizerState {
    ...
    /// EXP-1: anneal horizon, DECOUPLED from the step budget. 0 => use total_steps (shipped path,
    /// arithmetically identical). On Family B (t27/t28) `t_max_epochs` already does this and is
    /// worth +4,427 / +3,300 quality; on Family A the two roles share one number, so the same
    /// intervention is unreachable without this field.
...
pub mod dc {
    ...
    pub struct DcConfig {
        ...
        /// 73B-H1 `dc_stop_epoch`: last epoch at which the pulse may fire (0 = never stop, the
        /// shipped path). The pulse writes into `updates` INDEPENDENTLY of lr, so once
        /// spectral_phase_lr returns exactly 0.0 at progress=1 it is the only thing still moving
        /// val_loss -- and every improvement it manufactures resets the harness's 50-epoch
        /// patience. On nh10/an600 that is 80 of 156 epochs (51% of wall) bought at +2 +- 21
        /// quality/epoch. Gating it at the horizon deletes that tail.
    ...
    pub fn config_from_hp(hyperparameters: &Option<Map<String, Value>>, enable_default: u64) -> DcConfig {
        ...
        DcConfig {
            ...
            stop_epoch: hp
                .and_then(|h| h.get("dc_stop_epoch").and_then(|v| v.as_u64()))
    ...
    impl DcState {
        pub fn arm(&mut self, cfg: &DcConfig, m: [f64; 2], nb: usize) {
            ...
            self.stop_epoch = cfg.stop_epoch;
    ...
    pub fn step(
        ...
        if sie >= nb && epoch >= dc.start_epoch && (dc.stop_epoch == 0 || epoch < dc.stop_epoch) {
            let l = dc.gains.len();
