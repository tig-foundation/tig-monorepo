...
// track_t30 (n_hidden=7): self-contained solver.
// solve() dispatches to the inner track_7 implementation.
...
fn dead_update_mask_t30(count: usize) -> Vec<bool> {
    ...
    mod helpers {
        ...
        /// Mirrors `helpers::FusedMeta` used by the t26/t29 solvers; kept local so the
        /// shared `helpers.rs` (t26/t27/t28/t29) is untouched.
        #[derive(Clone)]
        ...
        /// T30-NORMFUSE gate. `false` == the shipped path, byte-for-byte: the struct below is
        /// never built and the fused kernel is never loaded or launched. Flip only after the
        /// bit-identity gate passes 15/15 on quality AND fuel.
        pub const T30_NORM_FUSE: bool = true;

        /// Static side-table for the single-block grad-norm tensors. Built once in init.
        /// `idx` are indices into the per-tensor arrays; every one of them has
        /// `cfgs[i].grid_dim.0 == 1`, which is what makes the fusion bit-identical.
        #[derive(Clone)]
        ...
        pub struct OptimizerState {
            ...
            /// P6 tables; `None` on the (unreachable) track_14 / track_18 inner solvers.
            pub p6: Option<FusedMeta>,
            pub nrm1: Option<NormFuse1>,
            /// DC steering: persistent H2D staging buffer, allocated lazily on the
            /// first pulse. `upd` cannot double as scratch here — unlike t26/t29 it
            /// holds the fused kernel's live output buffers.
        ...
        pub fn spectral_phase_lr(s: &OptimizerState, base_lr: f32) -> f32 {
            ...
            let horizon = if s.anneal_steps == 0 { total } else { s.anneal_steps as f32 };
            let progress = ((t - warm) / (horizon - warm).max(1.0)).min(1.0);
    ...
    pub mod track_7 {
        ...
        // The DC-steering implementation lives in the algorithm-root `helpers`
        // module (shared with t26/t27/t28), NOT in this file's local `helpers`;
        // hence the three `super`s (track_7 -> inner -> track_t30 -> root).
        use super::super::super::helpers::dc;

        /// DC steering default for this track (see RESULTS.md); `{"dc_enable":0}`
        /// restores the pre-port champion bit-for-bit.
        const DC_ENABLE_DEFAULT_T30: u64 = 1;
        const DC_AXPY_T30: &str = "dc_axpy_t30";

        thread_local! {
            /// DC steering state. Inert unless `dc_enable != 0`.
            ...
            // P-TR probe (40B): upper clamp of the LARS trust ratio (kernels_t30.cu:393).
            static TR_HI_T7: std::cell::Cell<f32> = std::cell::Cell::new(2.0f32);
            static ANNEAL_STEPS_T7: std::cell::Cell<usize> = std::cell::Cell::new(0);
            // 41A-H1: block_norm_scale adaptivity mix. 1.0 == shipped.
        ...
        pub fn solve(
            ...
            // EXP-5 fix: nh7's OptimizerState literal hardcodes its whole config, so the
            // anneal_steps field was initialised to 0 ("use total_steps") and the HP could never
            // reach it -- three ladder arms returned EXACT zeros, the signature of an unreachable
            // path rather than a null. Read it here, next to the HPs track_7 already reads.
            let anneal_hp: usize = hyperparameters.as_ref()
                .and_then(|h| h.get("anneal_steps").and_then(|v| v.as_u64()))
                .map(|u| u as usize).unwrap_or(0);
            ANNEAL_STEPS_T7.with(|c| c.set(anneal_hp));
            ...
            // unwrap_or(2.0) reproduces the shipped literal exactly, so hp={} is unchanged.
        ...
        fn optimizer_init(
            ...
            // (exactly the ones `optimizer_step`'s update loop does not `continue` on),
            ...
            // T30-NORMFUSE: collect the live tensors whose grad-norm launch is a SINGLE block.
            // Those are exactly the ones for which one-block-per-tensor reproduces the shipped
            // (tid, stride) pair, and whose slot takes exactly one atomicAdd.
            ...
            let state = OptimizerState {
                ...
                anneal_steps: ANNEAL_STEPS_T7.with(|c| c.get()),
        ...
        fn optimizer_step(
            ...
            // P6: the fused entry points. Both inline the same `*_body_7` device helpers as the
            // per-tensor kernels, so `--use_fast_math` cannot contract FMAs differently.
            ...
            // Loaded only when the gate is on, so the default path issues no extra
            // cuModuleGetFunction and remains byte-for-byte what ships today.
            ...
            // T30-NBUF: one contiguous f32 buffer for all norms instead of num_layers 1-element
            // buffers. Each tensor's launch still writes ONLY its own slot (base + 4*i), with the
            // same grid, so every block computes the identical value and the atomicAdd arrival order
            // per slot is unchanged (the runtime's per-launch fuel check blocks the host after every
            // launch, so no two launches are ever in flight). Lets the 24 blocking 1-float D2H
            // readbacks below collapse into one.
            ...
            // T30-NORMFUSE: when the gate is on, every SINGLE-BLOCK tensor is handled by one
            // grouped launch and is skipped in the per-tensor loop below. Multi-block tensors
            // keep their own launch -- fusing them would expose atomic arrival order.
            ...
            for i in 0..num_layers {
                ...
                // T30-LEAN: dead tensors carry exactly-zero gradients, so their grad-norm
                // contribution is atomicAdd(x, 0.0) and their layer_lrs entry is never read.
                // Skipping them is bit-identical and removes ~20 launches + blocking D2H per step.
            ...
            if do_svrg {
                for i in 0..num_layers {
                    let n = gradients[i].len();
                    // T30-LEAN: same argument as the grad-norm loop above.
                    if n == 0 || dead_t30[i] { continue; }
                ...
                // ── H1: ONE grouped launch instead of one launch per live tensor. ──
                // `p6.live` is EXACTLY this loop's live set: it filters
                // `param_sizes[i] > 0 && !dead_update_mask_t30(..)[i]` over `0..len-6`, and for
                // this architecture every index in `len-6..len` is already dead (the last BN
                // group is fully frozen and the preceding group's running_mean/var are dead), so
                // the two agree element-for-element AND in order.  `p6.fused_cfg` gives each
                // tensor exactly `cfgs[i].grid_dim.0` blocks -- the same grid the per-tensor
                // launch used -- which is what makes the fused kernel bit-identical.
            ...
            // P6: T = number of tensors that actually get computed.
            ...
            for (i, g) in gradients.iter().take(n_ret_t30).enumerate() {
                ...
                // The robust kernel always reads the raw gradient; the fast kernel reads the
                // SVRG-reduced one when it exists (`do_svrg == !use_robust && ...`, so the two
                // are mutually exclusive and one pointer slot suffices).
                ...
                // Pointers are re-read every step: `OptimizerState::box_clone` + `CudaSlice::clone`
                // (alloc + D2D) mean a cached pointer could aim at a stale buffer.
            ...
            if k != t_count {
                // The live set is derived from `dead_update_mask_t30(param_sizes.len())` at init
                // and must reproduce exactly here; a mismatch means the harness changed the layout.
            ...
            if t_count > 0 {
                ...
                // drain the pipeline (unlike the per-launch fuel check's blocking D2H).
            ...
            // Outputs, in harness index order. Same `s.upd[i]` buffers the fused kernel just
            // wrote, cloned exactly as before (single stream => the D2D copies are ordered
            // after the launch).
            for (i, g) in gradients.iter().take(n_ret_t30).enumerate() {
                let n = g.len();
            ...
            // Applied to the RETURNED clone, after the D2D copy above, so the
            // persistent `s.upd` fused-kernel buffers stay untouched. At most two
            // extra launches per epoch, none at all while `dc_enable == 0`.
            {
            ...
            if do_adaptive_lr {
                ...
                // T30-NBUF: one blocking D2H of num_layers floats replaces the per-tensor loop.
                // Dead slots are still exactly the 0.0 that alloc_zeros wrote (their launch is
                // skipped above), which is what curr_norms already held.
    ...
    pub mod track_14 {
        ...
        fn optimizer_init(_seed: [u8; 32], param_sizes: &[usize], stream: Arc<CudaStream>, _module: Arc<CudaModule>, prop: &cudaDeviceProp) -> Result<Box<dyn OptimizerStateTrait>> {
            ...
            Ok(Box::new(OptimizerState {
                ...
                step_count: 0,
                warmup_steps: 40,
                total_steps: 1000, anneal_steps: 0,
    ...
    pub mod track_18 {
        ...
        impl Default for TrackConfig {
            fn default() -> Self {
                TrackConfig {
                    total_steps: 1200, anneal_steps_cfg: 0,
                    warmup_steps: 40,
                    noise_variance: 0.040,
                    spectral_boost: 1.1,
                    beta1: 0.92,
        ...
        fn parse_config(hyperparameters: &Option<Map<String, Value>>) -> TrackConfig {
            ...
            TrackConfig {
                ...
                anneal_steps_cfg: hp.and_then(|h| h.get("anneal_steps").and_then(|v| v.as_u64())).unwrap_or(0) as usize,
