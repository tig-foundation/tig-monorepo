...
// Ported from prometheus_aidda/code_raw/kernels.cu for track t28 combine (P1 cross-poll).
...
//   delta = -lr * keep * dir
extern "C" __global__ void adabelief_update_kernel_t28(
...
// Fused multi-tensor AdaBelief update (P3 from t27/i9: 1 launch/step).
// Replaces n_fused separate adabelief_update_kernel_t28 launches with one kernel.
//
// all_ptrs layout (role-segmented, n_fused entries per role):
//   [grad0..gradN, param0..paramN, m0..mN, v0..vN, s0..sN, pg0..pgN, delta0..deltaN]
// lr_wd layout (interleaved): [lr0, wd0, lr1, wd1, ..., lrN, wdN]
// offsets: cumulative element counts, length n_fused+1
//
// Each thread maps global_idx -> tensor t + local element via binary search on shared offsets.
// Math byte-identical to adabelief_update_kernel_t28.
extern "C" __global__ void adabelief_fused_kernel_t28(
...
extern "C" __global__ void dc_axpy_t28(
