...
extern "C" __global__ __launch_bounds__(256, 4) void grad_norm_reduction_kernel_t30(
    ...
    if (warp_id == 0) {
        ...
        if (lane == 0) norm_out[blockIdx.x] = val;
    }
}

extern "C" __global__ __launch_bounds__(256, 4) void svrg_variance_reduce_kernel_t30(
    const float* __restrict__ gradients,
...
extern "C" __global__ __launch_bounds__(256, 3) void sign_ef_consensus_kernel_t30(
    const float* __restrict__ gradients,
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        slow_update[idx] = su;
        float base_upd = final_update * scale;
        const float p_val = params[idx];
        const float upd_align = base_upd * p_val;
        const float dir_modifier = (upd_align > 0.0f) ? 0.9f : 1.1f;
        base_upd *= dir_modifier;
        updates[idx] = base_upd - (lr * weight_decay) * p_val;
    }
}

extern "C" __global__ __launch_bounds__(256, 3) void dual_consensus_fisher_kernel_t30(
    const float* __restrict__ gradients,
    const float* __restrict__ params,
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float trust_radius = fminf(fmaxf(param_mag / mom_mag, 0.5f), 2.5f);
        const float trust_centered = trust_radius / 1.0f;
        const float trust = trust_centered / (1.0f + 0.5f * vol + 0.5f * flip);
        chosen_update *= trust;
        // Directional penalty: penalize weight growth, reward shrinkage
        const float upd_align = chosen_update * params[idx];
        const float dir_modifier = (upd_align > 0.0f) ? 0.9f : 1.1f;
        chosen_update *= dir_modifier;
        // Continuous block normalization (neural_advanced_v4 style)
