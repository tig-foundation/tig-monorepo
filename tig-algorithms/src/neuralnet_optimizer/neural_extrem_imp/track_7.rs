...
thread_local! {
    ...
    static NORM_EMA: std::cell::RefCell<f32> = std::cell::RefCell::new(0.0);
}
...
impl Default for TrackConfig {
    fn default() -> Self {
        TrackConfig {
            total_steps: 1000, warmup_steps: 350, noise_variance: 0.040, spectral_boost: 1.1,
            beta1: 0.92, beta2: 0.997, eps: 1e-8, weight_decay: 0.0022,
            bn_layer_boost: 1.12, output_layer_damping: 0.80, threads_per_block: 256, blocks_per_sm: 3,
        }
    }
}
...
fn parse_config(hyperparameters: &Option<Map<String, Value>>) -> TrackConfig {
    ...
    TrackConfig {
        ...
        warmup_steps: hp.and_then(|h| h.get("warmup_steps").and_then(|v| v.as_u64())).unwrap_or(350) as usize,
        ...
        weight_decay: hp.and_then(|h| h.get("weight_decay").and_then(|v| v.as_f64())).unwrap_or(0.0022) as f32,
        bn_layer_boost: hp.and_then(|h| h.get("bn_layer_boost").and_then(|v| v.as_f64())).unwrap_or(1.12) as f32,
        output_layer_damping: hp.and_then(|h| h.get("output_layer_damping").and_then(|v| v.as_f64())).unwrap_or(0.80) as f32,
...
fn optimizer_init(
    ...
    NORM_EMA.with(|n| *n.borrow_mut() = 0.0);
    TRACK_CONFIG.with(|c| {
        let cfg = c.borrow();
        ...
        for (i, &param_size) in param_sizes.iter().enumerate() {
            let lr = if i == num_layers - 1 { 0.00065f32 }
            else if i == 0 { 0.00110f32 }
            else if param_size <= 256 { 0.00240f32 }
            else if param_size <= 512 { 0.00200f32 }
            else if param_size > 50000 { 0.00095f32 }
            else { 0.00130f32 };
            layer_lrs.push(lr);
        }
...
fn optimizer_step(
    ...
    let k_fast = module.load_function("dual_consensus_fisher_kernel_t30")?;
    let k_robust = module.load_function("sign_ef_consensus_kernel_t30")?;
    let k_svrg = module.load_function("svrg_variance_reduce_kernel_t30")?;
    let k_norm = module.load_function("grad_norm_reduction_kernel_t30")?;
    ...
    for i in 0..num_layers {
        ...
        if n == 0 {
            norm_bufs.push(stream.alloc_zeros::<f32>(1)?);
            continue;
        }
        let grid_x = s.cfgs[i].grid_dim.0;
        norm_bufs.push(stream.alloc_zeros::<f32>(grid_x as usize)?);
    }
    for i in 0..num_layers {
        let n = gradients[i].len();
        if n == 0 { continue; }
    ...
    if do_svrg {
        ...
        for i in 0..num_layers {
            ...
            let g = &gradients[i];
            let ef_ref = &s.ef[i];
            if let Some(vr_buf) = vr_grad_bufs[i].as_mut() {
                unsafe {
    ...
    if do_adaptive_lr {
        ...
        for i in 0..num_layers {
            ...
            let host_val: Vec<f32> = stream.memcpy_dtov(&norm_bufs[i])?;
            let sum_sq: f32 = host_val.iter().sum();
            curr_norms[i] = sum_sq.max(0.0).sqrt();
        }
        let curr_norm_sum: f32 = curr_norms.iter().sum::<f32>();
        let prev_norm_sum = NORM_EMA.with(|n| *n.borrow());
        ...
        NORM_EMA.with(|n| *n.borrow_mut() = curr_norm_sum);
    }
