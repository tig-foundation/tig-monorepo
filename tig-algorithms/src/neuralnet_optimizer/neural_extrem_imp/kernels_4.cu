...
extern "C" __global__ __launch_bounds__(256, 3) void sign_ef_consensus_kernel(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float local_tau = lookahead_tau * expf(-rms);
        const float bounded_tau = fmaxf(local_tau, lookahead_tau * 0.05f);

        float su = slow_update[idx];
        su = (1.0f - bounded_tau) * su + bounded_tau * u_quant;
...
extern "C" __global__ __launch_bounds__(256, 3) void dual_consensus_fisher_kernel(
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        float base_u = blend_adam * adam_u + blend_norm * norm_u + blend_sign * sign_u;
        ...
        float robust_u = 0.55f * sign_u + 0.45f * fisher_u;

        const float rprop_scale = 1.0f + 0.2f * tanhf((g * pg) / (v_hat + eps));
        base_u *= rprop_scale;
        robust_u *= rprop_scale;
        ...
        const float local_tau = lookahead_tau * expf(-sqrtf(fmaxf(v_hat, 0.0f)));
        const float bounded_tau = fmaxf(local_tau, lookahead_tau * 0.05f);

        float su = slow_update[idx];
        su = (1.0f - bounded_tau) * su + bounded_tau * u;
