...
extern "C" __global__ __launch_bounds__(128, 6) void dual_consensus_fisher_kernel_10(
    ...
    __shared__ float smem_sq[4];
    __shared__ float smem_gw[4];
    __shared__ float smem_ww[4];
    ...
    // Pass 1: per-block gradient RMS and projection
    float local_sq_sum = 0.0f;
    float local_gw_sum = 0.0f;
    float local_ww_sum = 0.0f;
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float g_pred_raw = g + gamma_local * (g - pg);

        local_sq_sum += g * g;
        local_gw_sum += g_pred_raw * w;
        local_ww_sum += w * w;
        local_count++;
    }
    #pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
        ...
        local_gw_sum += __shfl_down_sync(0xffffffff, local_gw_sum, offset);
        local_ww_sum += __shfl_down_sync(0xffffffff, local_ww_sum, offset);
        local_count  += __shfl_down_sync(0xffffffff, local_count,  offset);
    }
    if (lane == 0) {
        smem_sq[warp_id] = (local_count > 0) ? (local_sq_sum / (float)local_count) : 0.0f;
        smem_gw[warp_id] = local_gw_sum;
        smem_ww[warp_id] = local_ww_sum;
    }
    __syncthreads();

    float block_grad_rms = 0.0f;
    float block_proj = 0.0f;
    if (warp_id == 0) {
        float val_sq = (lane < n_warps) ? smem_sq[lane] : 0.0f;
        float val_gw = (lane < n_warps) ? smem_gw[lane] : 0.0f;
        float val_ww = (lane < n_warps) ? smem_ww[lane] : 0.0f;
        #pragma unroll
        for (int offset = 2; offset > 0; offset >>= 1) {
            val_sq += __shfl_down_sync(0xffffffff, val_sq, offset);
            val_gw += __shfl_down_sync(0xffffffff, val_gw, offset);
            val_ww += __shfl_down_sync(0xffffffff, val_ww, offset);
        }
        if (lane == 0) {
            smem_sq[0] = sqrtf(fmaxf(val_sq / fmaxf((float)n_warps, 1.0f), 0.0f)) + eps;
            smem_gw[0] = val_gw / (val_ww + 1.0e-8f);
        }
    }
    __syncthreads();
    block_grad_rms = smem_sq[0];
    block_proj = smem_gw[0];
    ...
    for (unsigned int idx = tid; idx < n; idx += stride) {
        ...
        const float g_pred = g_pred_raw - block_proj * w;
        ...
        const float g_clipped = fminf(fmaxf(g_pred_raw, -5.0f * grad_std), 5.0f * grad_std);
        fd = fisher_beta * fd + one_minus_fisher_beta * g_clipped * g_clipped;
        const float fisher_rms = sqrtf(fmaxf(fd, 0.0f)) + eps;

        const float fisher_norm_update = -lr * (g_pred_raw / fisher_rms);
        ...
        const float grad_mom_align = (g_pred_raw * m_hat >= 0.0f) ? 1.0f : 0.0f;
