...
pub fn solve(
    ...
    let hyperedge_majority_refine_kernel = module.load_function("hyperedge_majority_refine_100k")?;
    ...
    for _majority_pass in 0..4 {
        let mut d_majority_moves = stream.alloc_zeros::<i32>(1)?;
        unsafe {
            stream
                .launch_builder(&hyperedge_majority_refine_kernel)
                .arg(&mut d_partition)
                .arg(&challenge.d_hyperedge_offsets)
                .arg(&challenge.d_hyperedge_nodes)
                .arg(&(challenge.num_hyperedges as i32))
                ...
                .arg(&mut d_nodes_in_part)
                .arg(&(challenge.max_part_size as i32))
                .arg(&mut d_majority_moves)
                .launch(hedge_cfg.clone())?;
        }
        let moves: Vec<i32> = stream.memcpy_dtov(&d_majority_moves)?;
        if moves[0] == 0 {
            break;
        }
    }
    ...
    {
        let mut all_hedges_by_size: Vec<i32> = (0..challenge.num_hyperedges as i32).collect();
        all_hedges_by_size.sort_unstable_by(|&a, &b| {
            let sz_a = hedge_offsets_host[(a + 1) as usize] - hedge_offsets_host[a as usize];
            let sz_b = hedge_offsets_host[(b + 1) as usize] - hedge_offsets_host[b as usize];
            sz_a.cmp(&sz_b).then_with(|| a.cmp(&b))
        });
        for _ in 0..5 {
            let prev = partition_host_refine.clone();
            perturb_guided_on_host(
                &mut partition_host_refine,
                &mut nodes_in_part_host,
                &all_hedges_by_size,
            );
            if partition_host_refine == prev { break; }
        }
