...
extern "C" __global__ void hyperedge_majority_refine_100k(
    int* __restrict__ partition,
    const int* __restrict__ hyperedge_offsets,
    const int* __restrict__ hyperedge_nodes,
    const int num_hyperedges,
    const int num_parts,
    int* __restrict__ part_sizes,
    const int max_part_size,
    int* __restrict__ moves_made
) {
    const int e = blockIdx.x * blockDim.x + threadIdx.x;
    if (e >= num_hyperedges) return;

    const int start = hyperedge_offsets[e];
    const int end   = hyperedge_offsets[e + 1];
    if (start >= end) return;

    int local_counts[64];
    for (int p = 0; p < num_parts; ++p) local_counts[p] = 0;

    for (int idx = start; idx < end; ++idx) {
        int node = hyperedge_nodes[idx];
    ...
    int dominant = 0;
    int dom_count = local_counts[0];
    for (int p = 1; p < num_parts; ++p) {
        if (local_counts[p] > dom_count) {
            dom_count = local_counts[p];
            dominant = p;
        }
    }

    if (dom_count == (end - start)) return;

    for (int idx = start; idx < end; ++idx) {
        int node = hyperedge_nodes[idx];
        int old = partition[node];
        if (old == dominant) continue;

        if (atomicAdd(&part_sizes[old], 0) <= 1) continue;

        int new_size = atomicAdd(&part_sizes[dominant], 1);
        if (new_size > max_part_size) {
            atomicSub(&part_sizes[dominant], 1);
            continue;
        }

        int res = atomicCAS(&partition[node], old, dominant);
        if (res == old) {
            atomicSub(&part_sizes[old], 1);
            atomicAdd(moves_made, 1);
        } else {
            atomicSub(&part_sizes[dominant], 1);
        }
    }
