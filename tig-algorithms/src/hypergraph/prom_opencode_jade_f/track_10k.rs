...
pub fn solve(
    ...
    let mut d_hedge_moves = stream.alloc_zeros::<i32>(challenge.num_hyperedges as usize * 8)?;
    ...
    macro_rules! do_hyperedge_centric_phase {
        () => {{
            ...
            valid_moves.clear();
            for (h_idx, chunk) in hedge_moves_host.chunks_exact(8).enumerate() {
