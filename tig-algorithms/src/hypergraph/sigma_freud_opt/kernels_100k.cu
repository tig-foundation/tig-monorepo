...
extern "C" __global__ void assign_from_elite_votes_100k(
    ...
    const unsigned long long* __restrict__ edge_flags,
    int *partition_out                      
) {
    (void)num_elites;
    ...
    if (cand_mask == 0ULL) {
        ...
        while (tmp) {
            ...
            if (used_degree > 0) {
                for (int j = 0; j < used_degree; j++) {
                    int hedge = node_hyperedges[start + j];
                    score += ((__ldg(edge_flags + 2 * hedge) & bit) != 0ULL);
                }
            }
...
extern "C" __global__ void precompute_edge_flags_100k(
    ...
    unsigned long long * __restrict__ edge_flags
) {
    int hedge = blockIdx.x * blockDim.x + threadIdx.x;
    if (hedge < num_hyperedges) {
        int start = hyperedge_offsets[hedge];
        ...
        edge_flags[2 * hedge] = flags_all;
        edge_flags[2 * hedge + 1] = flags_double;
    }
}
...
extern "C" __global__ void compute_refinement_moves_optimized_100k(
    ...
    for (int j = 0; j < used_degree; j++) {
        ...
        unsigned long long flags_all, flags_double;
        {
            const ulonglong2* ptr = (const ulonglong2*)(edge_flags + 2 * hyperedge);
