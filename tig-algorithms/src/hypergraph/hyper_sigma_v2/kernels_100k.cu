...
extern "C" __global__ void compute_refinement_moves_optimized_100k(
    ...
    int best_tiebreak = -2147483647;   // secondary key; only consulted when zgm_mode != 0
    ...
    while (cand_mask) {
        ...
        if (zgm_mode == 0) {
            // base: equal-gain ties broken by lowest part index (bit-exact sentinel path)
            if (total_gain > best_gain || (total_gain == best_gain && target_part < best_target)) {
                best_gain = total_gain;
                best_target = target_part;
            }
        } else {
            // i11 zero-gain-move tie-break: a strictly better gain always wins (the
            // connectivity objective is untouched); EQUAL-gain ties are resolved by a
            // structural secondary key instead of arbitrary lowest index.
            //   zgm_mode==1: densest connection  = max part_counts[target] (consolidate)
            //   zgm_mode==2: emptiest target      = max free capacity      (spread)
            int tb = (zgm_mode == 1)
                        ? (int)part_counts[target_part]
            ...
            if (total_gain > best_gain) {
                ...
                best_target = target_part;
                       ...
                       (tb > best_tiebreak ||
                        (tb == best_tiebreak && target_part < best_target))) {
                best_tiebreak = tb;
                best_target = target_part;
            }
        }
