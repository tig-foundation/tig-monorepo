...
pub fn solve(
    ...
    // accept_mode: 0 = greedy restart-from-best (bit-exact base sentinel — unchanged
    //              100k behaviour); 1 = LAHC non-greedy walk (restart-from-current,
    //              accept if no worse than current OR the cost held lahc_len iters ago).
    // Ported from track_10k.rs (P proven KEPT on t23). At 100k the greedy ILS is
    // Q-neutral (single basin: it re-refines the deep local opt back to best every
    // iter); the walk converts wasted ILS iterations into bounded basin-hopping.
    // best is still tracked/returned separately, so accept_mode!=0 never regresses.
    let accept_mode = hyperparameters
        .as_ref()
        .and_then(|p| p.get("accept_mode").and_then(|v| v.as_i64()))
        .unwrap_or(0);
    let lahc_len = hyperparameters
        .as_ref()
        .and_then(|p| p.get("lahc_len").and_then(|v| v.as_i64()))
        .map(|v| v.clamp(1, 200) as usize)
        .unwrap_or(20);

    // --- i10 structural levers: break the SATURATED elite pool at high refinement.
    // At refinement=9000 every ILS trajectory descends into one deep basin and the
    // elite pool saturates fast, making ils/acceptance/guided-perturb Q-neutral
    // (Hindsight 552d5284 pool-saturates-with-depth, 0835d5cc guided-perturb can't
    // generate new basins beyond the pool). Two knobs attack the pool itself:
    //  * num_high_hedges: breadth of the guided-perturbation front (hardcoded 500 =
    //    0.5% coverage on 100k). Wider front -> more diverse basins (86bcb104: 1000
    //    optimal on t23; 6d4dd5ec: K=5 lost 2046). Default 500 = sentinel (bit-exact).
    //  * perturb_escalate: adaptive random-perturb strength that grows during ILS
    //    stagnation to escape the saturated basin (feccb6a8: too-weak stays in-basin,
    //    too-strong = random restart -> capped). Default 0 = off = sentinel.
    let num_high_hedges_hp = hyperparameters
        .as_ref()
        .and_then(|p| p.get("num_high_hedges").and_then(|v| v.as_i64()))
        .map(|v| v.clamp(50, 4000) as usize)
        .unwrap_or(500);
    let perturb_escalate = hyperparameters
        .as_ref()
        .and_then(|p| p.get("perturb_escalate").and_then(|v| v.as_i64()))
        .unwrap_or(0);
    let perturb_escalate_cap = hyperparameters
        .as_ref()
        .and_then(|p| p.get("perturb_escalate_cap").and_then(|v| v.as_i64()))
        .map(|v| v.clamp(3, 30) as i32)
        .unwrap_or(8);

    // --- i11 structural lever: zero-gain-move tie-break in the FM move kernel.
    // Grounding (Hindsight fdf6eccb / 9a22d657 / 2d63794e[rel5] / 76824472): move-
    // based FM refinement stalls on large hyperedges because single-vertex moves
    // often have NO immediate cut effect — a large edge's vertices are already
    // scattered over many blocks, so many candidate targets tie at gain 0. The base
    // move kernel breaks such ties by lowest part index (arbitrary), so the search
    // cannot steer zero-gain moves productively -> pool/basin saturation. zgm_tiebreak
    // selects, among EQUAL-gain targets, a structurally better one (never overrides a
    // strictly-better gain, so the connectivity objective itself is untouched):
    //   0 = base (lowest index)         -> bit-exact 269300 sentinel (build-fidelity).
    //   1 = densest connection (max part_counts[target]): consolidate the vertex into
    //       the block it is most connected to, shrinking its block-spread (attacks the
    //       2d63794e root cause directly).
    //   2 = emptiest target (max free capacity): steer toward under-full blocks to
    //       open new productive moves (orthogonal escape direction).
    let zgm_tiebreak: i32 = hyperparameters
        .as_ref()
        .and_then(|p| p.get("zgm_tiebreak").and_then(|v| v.as_i64()))
    ...
    let num_high_hedges =
        std::cmp::min(num_high_hedges_hp, challenge.num_hyperedges as usize);
    let mut high_hedge_ids_host: Vec<i32> = build_high_hedge_ids(&conn_vec, num_high_hedges);

    // Adaptive perturbation strength state (perturb_escalate!=0). Counts consecutive
    // ILS iters without a strict best improvement; the random perturb uses
    // base+stagnation (capped) so plateaus trigger stronger, basin-escaping kicks.
    let mut ils_stagnation: i32 = 0;
    ...
    // LAHC walk state (accept_mode==1). Inert when accept_mode==0: current_* is never
    // read (restore uses best_*), so the sentinel path stays byte-exact vs base.
    ...
    let mut lahc_hist: Vec<i32> = vec![best_connectivity; lahc_len];

    for ils_iter in 0..ils_iterations {
        // accept_mode==0: restart from best (base). accept_mode!=0: restart from the
        // walk's current state so bounded-uphill acceptances persist across iters.
        let (restore_part, restore_nodes): (&[i32], &[i32]) = if accept_mode != 0 {
            (&current_partition_host, &current_nodes_in_part_host)
        } else {
            (&best_partition_host, &best_nodes_in_part_host)
        };
        let d_partition_restored = stream.memcpy_stod(restore_part)?;
        let d_nodes_in_part_restored = stream.memcpy_stod(restore_nodes)?;
        d_partition = d_partition_restored;
        d_nodes_in_part = d_nodes_in_part_restored;
        partition_host_refine.copy_from_slice(restore_part);
        nodes_in_part_host.copy_from_slice(restore_nodes);
        ...
        let refreshed_host_from_device = if use_consensus_next && elite_count > 1 && accept_mode == 0 {
            ...
            if ils_iter % 2 == 0 {
                ...
                // Escalate random-perturb strength with ILS stagnation (capped) so
                // plateaus get progressively harder kicks; perturb_escalate==0 keeps
                // the fixed base strength (bit-exact sentinel).
                let eff_perturb_strength = if perturb_escalate != 0 {
                    (perturb_strength + ils_stagnation).min(perturb_escalate_cap)
                } else {
                    perturb_strength
                };
                perturb_on_host(
        ...
        // LAHC acceptance for the walk (accept_mode==1). d_partition still holds the
        // just-evaluated candidate (cost == new_connectivity). Accept if no worse than
        // the current cost OR the cost the walk held `lahc_len` iterations ago; on
        // accept the walk moves to the candidate. best_* is untouched here (already
        // updated above on strict improvement) so the returned solution never worsens.
        if accept_mode != 0 {
            let v = ils_iter % lahc_len;
            let accept =
                new_connectivity <= current_connectivity || new_connectivity <= lahc_hist[v];
            if accept {
            ...
            lahc_hist[v] = current_connectivity;
        }

        use_consensus_next = !improved;

        // Track ILS stagnation for adaptive perturbation escalation. Reset on any
        // strict best improvement; grow otherwise. Inert when perturb_escalate==0.
        if improved {
            ils_stagnation = 0;
        } else {
            ils_stagnation += 1;
