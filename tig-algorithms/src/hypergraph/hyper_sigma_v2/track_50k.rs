...
pub fn solve(
    ...
    // (track-isolation: mod.rs untouched). Reproduces id124's 271,242 config (hub_mode=1).
    // user hp wins (or_insert). Reads flow unchanged: shadow `hyperparameters`. ===
    let __baked_hp: Option<Map<String, Value>> = {
        use serde_json::Number;
        let mut m = hyperparameters.clone().unwrap_or_default();
        let n = |v: u64| Value::Number(Number::from(v));
        let mut ins = |k: &str, v: Value| { m.entry(k.to_string()).or_insert(v); };
        ins("effort", n(5));
        ins("clusters", n(64));
        ins("move_limit", n(880000));
        ins("refinement", n(10800));
        ins("tabu_tenure", n(8));
        ins("ils_iterations", n(10));
        ins("post_ils_polish", n(100));
        ins("post_refinement", n(64));
        ins("ils_quick_refine", n(50));
        ins("perturb_strength", n(5));
        ins("pr_rebal_hoist", Value::Bool(true));
        ins("hub_mode", n(1));
        Some(m)
    };
    let hyperparameters = &__baked_hp;
    ...
    // i39 fast_moves: memory-coalesced packed variants of the dominant refinement pair
    let precompute_edge_flags_packed_kernel = module.load_function("precompute_edge_flags_packed_50k")?;
    let compute_moves_fast_kernel = module.load_function("compute_refinement_moves_fast_50k")?;
    // i40 boundary-localized refinement: restrict the dominant compute-moves kernel to
    // the boundary set (nodes incident to >=1 cut hyperedge). Iso-Q (interior = 0x80000000).
    let init_round_boundary_kernel = module.load_function("init_round_boundary_50k")?;
    let mark_boundary_kernel = module.load_function("mark_boundary_nodes_50k")?;
    let compact_boundary_kernel = module.load_function("compact_boundary_nodes_50k")?;
    let compute_moves_boundary_kernel = module.load_function("compute_refinement_moves_boundary_50k")?;
    ...
    // i39 fast_moves: packed [flags_all, flags_double] interleaved (2 u64 per hedge)
    let mut d_edge_flags_packed = stream.alloc_zeros::<u64>(2 * challenge.num_hyperedges as usize)?;
    // i40 boundary-localized refinement scratch (allocated always; used only when boundary_only=1)
    let mut d_is_boundary = stream.alloc_zeros::<i32>(challenge.num_nodes as usize)?;
    let mut d_boundary_nodes = stream.alloc_zeros::<i32>(challenge.num_nodes as usize)?;
    ...
    // hub_mode: gain-sampling precision for high-degree (hub) nodes in the two
    // refinement-gain kernels. 0=sentinel (min(deg,256) strided, bit-exact baseline),
    // 1=uncap-exact (used_degree=deg), 2=dense (min(deg,1024) strided),
    // 3=contiguous-window (first min(deg,256) incident hyperedges). See strategy.md.
    let hub_mode: i32 = hyperparameters
        .as_ref()
        .and_then(|p| p.get("hub_mode").and_then(|v| v.as_i64()))
        .unwrap_or(0) as i32;

    // i39 fast_moves sentinel: 0=original scattered-2xu64 path (byte-exact drift-check),
    // 1=packed ulonglong2 __ldg path for the DOMINANT `for round in 0..refinement_rounds`
    // loop (iso-Q: identical gain math, packed flags carry the same values).
    let fast_moves: i32 = hyperparameters
        .as_ref()
        .and_then(|p| p.get("fast_moves").and_then(|v| v.as_i64()))
        .unwrap_or(0) as i32;

    // i40 boundary_only sentinel: 0=full-graph compute-moves over all N nodes each round
    // (byte-exact drift-check path), 1=restrict the dominant `for round in
    // 0..refinement_rounds` compute-moves kernel to the boundary set (nodes incident to
    // >=1 cut hyperedge). Iso-Q by construction: interior nodes always yield the
    // 0x80000000 no-move sentinel, so restricting to the boundary superset produces
    // identical move_priorities while cutting the per-round O(N) cost to O(|boundary|).
    let boundary_only: i32 = hyperparameters
        .as_ref()
        .and_then(|p| p.get("boundary_only").and_then(|v| v.as_i64()))
        .unwrap_or(0) as i32;
    ...
    for round in 0..refinement_rounds {
        if boundary_only == 1 {
            // i40: full precompute (scattered 2xu64, byte-exact flags), then restrict the
            // heavy compute-moves kernel to the boundary set only. move_priorities is
            // re-initialised to 0x80000000 for ALL nodes each round so interior slots
            // (never written by the boundary kernel) stay no-move -> byte-exact host scan.
            unsafe {
                stream
            ...
            unsafe {
                stream
                    .launch_builder(&init_round_boundary_kernel)
                    .arg(&(challenge.num_nodes as i32))
                    .arg(&mut d_move_priorities)
                    .arg(&mut d_is_boundary)
                    .arg(&mut d_boundary_count)
                    .launch(cfg.clone())?;
            }
            unsafe {
                stream
                    .launch_builder(&mark_boundary_kernel)
                    .arg(&(challenge.num_hyperedges as i32))
                    .arg(&(challenge.num_nodes as i32))
                    .arg(&challenge.d_hyperedge_nodes)
                    .arg(&challenge.d_hyperedge_offsets)
            ...
            unsafe {
                stream
                    .launch_builder(&compact_boundary_kernel)
                    .arg(&(challenge.num_nodes as i32))
                    .arg(&d_is_boundary)
                    .arg(&mut d_boundary_nodes)
                    .arg(&mut d_boundary_count)
                    .launch(cfg.clone())?;
            }
            unsafe {
                stream
                    .launch_builder(&compute_moves_boundary_kernel)
                    .arg(&(challenge.num_nodes as i32))
                    .arg(&(challenge.num_parts as i32))
                    .arg(&(challenge.max_part_size as i32))
                    .arg(&challenge.d_node_hyperedges)
                    ...
                    .arg(&mut d_move_priorities)
                    .arg(&hub_mode)
                    .arg(&d_boundary_nodes)
                    .arg(&d_boundary_count)
                    .launch(cfg.clone())?;
            }
        } else if fast_moves == 1 {
            // i39: packed precompute + coalesced ulonglong2 __ldg move kernel (iso-Q)
            unsafe {
                stream
                    .launch_builder(&precompute_edge_flags_packed_kernel)
                    .arg(&(challenge.num_hyperedges as i32))
                    .arg(&(challenge.num_nodes as i32))
                    .arg(&challenge.d_hyperedge_nodes)
                    .arg(&challenge.d_hyperedge_offsets)
                    .arg(&d_partition)
                    .arg(&mut d_edge_flags_packed)
                    .launch(hedge_cfg.clone())?;
            }
            unsafe {
                stream
                    .launch_builder(&compute_moves_fast_kernel)
                    .arg(&(challenge.num_nodes as i32))
                    .arg(&(challenge.num_parts as i32))
                    .arg(&(challenge.max_part_size as i32))
                    .arg(&challenge.d_node_hyperedges)
