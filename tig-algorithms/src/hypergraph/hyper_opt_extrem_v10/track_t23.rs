...
pub fn solve(
    ...
    let mut d_total_connectivity = stream.alloc_zeros::<i32>(1)?;
    ...
    let build_high_hedge_ids_pool_guided =
        |connectivity: &[i32], num_high_hedges: usize,
         elite_flat: &[i32], elite_cnt: usize| -> Vec<i32> {
            let n = challenge.num_nodes as usize;
            let m = challenge.num_hyperedges as usize;
            let mut cut_count: Vec<u32> = vec![0u32; m];
            for sol_idx in 0..elite_cnt {
                let sol = &elite_flat[sol_idx * n..(sol_idx + 1) * n];
                for h in 0..m {
                    if connectivity[h] == 0 { continue; }
                    let start = hedge_offsets_host[h] as usize;
                    let end = hedge_offsets_host[h + 1] as usize;
                    if end <= start + 1 { continue; }
                    let first_part = sol[hyperedge_nodes_host[start] as usize];
                    let is_cut = (start + 1..end).any(|k| {
                        sol[hyperedge_nodes_host[k] as usize] != first_part
                    });
                    if is_cut { cut_count[h] += 1; }
                }
            }
            let mut impact: Vec<(i32, i32, i32)> = connectivity
                .iter()
                ...
                .filter(|(_, &c)| c > 0)
                .map(|(h, &c)| {
                    let stability = cut_count[h] as f32 / elite_cnt.max(1) as f32;
                    let borderline = 1.0_f32 - (2.0 * stability - 1.0).abs();
                    let score = (c as f32 * (0.5 + 0.5 * borderline)) as i32;
                    (score, c, h as i32)
                })
                .collect();
            impact.sort_unstable_by(|a, b| {
            ...
            impact.into_iter().take(num_high_hedges).map(|t| t.2).collect()
        };
    ...
    let hedge_clusters_early: Vec<i32> = stream.memcpy_dtov(&d_hyperedge_clusters)?;
    ...
    let num_high_hedges = std::cmp::min(1100usize, challenge.num_hyperedges as usize);
    ...
    for ils_iter in 0..ils_iterations {
        ...
        stream.memset_zeros(&mut d_total_connectivity)?;
        unsafe {
            stream
        ...
        let new_connectivity = stream.memcpy_dtov(&d_total_connectivity)?[0];
        ...
        if new_connectivity < best_connectivity {
            ...
            if elite_count >= 2 {
                high_hedge_ids_host = build_high_hedge_ids_pool_guided(
                    &connectivity_vec, num_high_hedges, &elite_flat_host, elite_count);
            } else {
    ...
    if elite_count >= 2 {
        ...
        let max_part = challenge.max_part_size as i32;

        let mut node_hedge_list_rr: Vec<Vec<u32>> = vec![Vec::new(); n];
        for h in 0..m {
            let start = hedge_offsets_host[h] as usize;
            let end = hedge_offsets_host[h + 1] as usize;
            for idx in start..end {
                let v = hyperedge_nodes_host[idx] as usize;
                if v < n {
                    node_hedge_list_rr[v].push(h as u32);
                }
            }
        }

        let mut hpc: Vec<[u8; 64]> = vec![[0u8; 64]; m];
        for v in 0..n {
            let p = best_partition_host[v] as usize;
            if p < kc {
                for &h in &node_hedge_list_rr[v] {
                    hpc[h as usize][p] = hpc[h as usize][p].saturating_add(1);
                }
            }
        }
        ...
        for v in 0..n {
            let cur = best_partition_host[v] as usize;
            if cur >= kc { continue; }
            let is_boundary = node_hedge_list_rr[v].iter().any(|&h| hpc[h as usize][cur] == 1);
            if !is_boundary { continue; }
            let mut votes = [0u32; 64];
            let mut disagreement = 0usize;
            for slot in 0..elite_count {
                let ep = elite_flat_host[slot * n + v] as usize;
                if ep < kc {
                    votes[ep] += 1;
                    if ep != cur { disagreement += 1; }
                }
            }
            if disagreement < 1 { continue; }
            let consensus_part = (0..kc)
                .filter(|&p| p != cur)
                .max_by_key(|&p| votes[p])
                .unwrap_or(cur);
            if votes[consensus_part] == 0 { continue; }
            candidates.push((disagreement, v, consensus_part));
        }

        candidates.sort_unstable_by(|a, b| b.0.cmp(&a.0));
        let top_n = (n / 20).min(candidates.len());

        let mut nip_host = best_nodes_in_part_host.clone();
        for &(_, v, tgt) in &candidates[..top_n] {
            let cur = best_partition_host[v] as usize;
            if cur >= kc || tgt >= kc || cur == tgt { continue; }
            if nip_host[tgt] >= max_part { continue; }
            for &h in &node_hedge_list_rr[v] {
                let hc = &mut hpc[h as usize];
                hc[cur] = hc[cur].saturating_sub(1);
                hc[tgt] = hc[tgt].saturating_add(1);
            }
            nip_host[cur] -= 1;
            nip_host[tgt] += 1;
            best_partition_host[v] = tgt as i32;
        }
        best_nodes_in_part_host = nip_host;
    }
...

    {
        ...
        let num_donors = (elite_count.saturating_sub(1)).min(4);
        if elite_count >= 2 {
            ...
            let mut rc_part = best_partition_host.clone();
            let mut rc_nip = best_nodes_in_part_host.clone();
            ...
            'sweeps: for _sweep in 0..3usize {
                ...
                for dk in 0..num_donors {
                    ...
                    for &(_, v, p_tgt) in disagree_rc.iter() {
                        ...
                        if rc_nip[p_tgt as usize] >= max_ps_rc { continue; }
            ...
            if final_conn_rc < best_connectivity {
...

    {
        ...
        let hedge_clusters_host = &hedge_clusters_early;
        let nc_vc = num_hedge_clusters as usize;
        let mut node_comm: Vec<usize> = (0..n_vc).collect();
        {
            let mut clust_cnt: Vec<i32> = vec![0i32; nc_vc];
            let mut clust_seen: Vec<u32> = vec![0u32; nc_vc];
            let mut clust_list: Vec<usize> = Vec::new();
            let mut cstamp = 0u32;
            for u in 0..n_vc {
                ...
                for hi in nd_off_vc[u]..nd_off_vc[u + 1] {
                    ...
                    let c = hedge_clusters_host[h] as usize;
                    if c >= nc_vc { continue; }
                    if clust_seen[c] != cstamp {
                        clust_seen[c] = cstamp;
                        clust_cnt[c] = 1;
                        clust_list.push(c);
                    } else {
                ...
                if best_c < nc_vc { node_comm[u] = best_c; }
            }
        }
        ...
        for mode in 0..7_usize {

            let mut rep_vc: Vec<usize> = (0..n_vc).collect();
            let mut matched_vc: Vec<bool> = vec![false; n_vc];
            let mut shared_vc: Vec<i64> = vec![-1i64; n_vc];
            let mut clq_vc: Vec<i64> = vec![0i64; n_vc];
            ...
            for u in 0..n_vc {
                ...
                for hi in nd_off_vc[u]..nd_off_vc[u + 1] {
                    ...
                    for k in hs..he {
                        ...
                        if mode == 6 && node_comm[v] != node_comm[u] { continue; }
...

    {
        ...
        let kc = num_parts_usize.min(64);
        let max_part = challenge.max_part_size as i32;
        let max_pq_moves = 2000usize;

        stream.memcpy_dtoh(&d_partition, &mut partition_host_refine)?;
        stream.memcpy_dtoh(&d_nodes_in_part, &mut nodes_in_part_host)?;

        let mut node_hedge_list: Vec<Vec<u32>> = vec![Vec::new(); n];
        for h in 0..m {
            let start = hedge_offsets_host[h] as usize;
            let end = hedge_offsets_host[h + 1] as usize;
            for idx in start..end {
                let v = hyperedge_nodes_host[idx] as usize;
                if v < n {
                    node_hedge_list[v].push(h as u32);
                }
            }
        }

        let mut hedge_part_cnt: Vec<[u8; 64]> = vec![[0u8; 64]; m];
        for v in 0..n {
        ...
        for _pass in 0..3usize {
            let mut pq: std::collections::BinaryHeap<(i32, usize)> =
            ...
            for v in 0..n {
                ...
                for tgt in 0..kc {
                    if tgt == cur || nodes_in_part_host[tgt] >= max_part { continue; }
                    let mut g: i32 = 0;
                    for &h in &node_hedge_list[v] {
                ...
                if best_gain > 0 { pq.push((best_gain, v)); }
            }
            ...
            while let Some((gain_stored, v)) = pq.pop() {
                if moves_this_pass >= max_pq_moves { break; }
                ...
                for tgt in 0..kc {
                    ...
                    if g > best_gain { best_gain = g; best_tgt = tgt; }
                }

                if best_gain <= 0 { continue; }

                if best_gain != gain_stored {
                    pq.push((best_gain, v));
                    continue;
                }
                ...
                for &h in &node_hedge_list[v] {
                    hedge_part_cnt[h as usize][cur] -= 1;
                    hedge_part_cnt[h as usize][best_tgt] =
                        hedge_part_cnt[h as usize][best_tgt].saturating_add(1);
                }
                moves_this_pass += 1;
                pass_improved = true;

                for &h in &node_hedge_list[v] {
                    let hstart = hedge_offsets_host[h as usize] as usize;
                    let hend = hedge_offsets_host[h as usize + 1] as usize;
                    for idx in hstart..hend {
                        let w = hyperedge_nodes_host[idx] as usize;
                        if w == v || w >= n { continue; }
                        let wc = partition_host_refine[w] as usize;
                        if wc >= kc || nodes_in_part_host[wc] <= 1 { continue; }
                        let mut g_w: i32 = 0;
                        for tgt in 0..kc {
                            if tgt == wc || nodes_in_part_host[tgt] >= max_part { continue; }
                            let mut gg: i32 = 0;
                            for &hh in &node_hedge_list[w] {
                                let cnt = &hedge_part_cnt[hh as usize];
                                if cnt[wc] == 1 { gg += 1; }
                                if cnt[tgt] == 0 { gg -= 1; }
                            }
                            if gg > g_w { g_w = gg; }
                        }
                        if g_w > 0 { pq.push((g_w, w)); }
                    }
                }
            }
        ...
        'hfc_piercing: for _pierce_pass in 0..2usize {
            let mut cut_nets_3p: Vec<(i32, usize)> = Vec::new();
            let mut cut_nets_2: Vec<(i32, usize)> = Vec::new();
            for h in 0..m {
                let hstart = hedge_offsets_host[h] as usize;
                let hend = hedge_offsets_host[h + 1] as usize;
                let sz = (hend - hstart) as i32;
                if sz <= 1 { continue; }
                let parts: usize = hedge_part_cnt[h][..kc].iter().filter(|&&c| c > 0).count();
                if parts >= 3 {
                    cut_nets_3p.push((sz, h));
                } else if parts == 2 {
                    cut_nets_2.push((sz, h));
                }
            }
            cut_nets_3p.sort_unstable_by(|a, b| b.0.cmp(&a.0).then_with(|| a.1.cmp(&b.1)));
            cut_nets_2.sort_unstable_by(|a, b| b.0.cmp(&a.0).then_with(|| a.1.cmp(&b.1)));

            let mut pierce_ops = 0usize;
            let max_pierce = 300usize;

            for &(_, h) in &cut_nets_3p {
                if pierce_ops >= max_pierce { break; }
                let mut maj = kc;
                let mut maj_cnt: u8 = 0;
                for p in 0..kc {
                    if hedge_part_cnt[h][p] > maj_cnt {
                        maj_cnt = hedge_part_cnt[h][p];
                        maj = p;
                    }
                }
                if maj >= kc { continue; }
                let hstart = hedge_offsets_host[h] as usize;
                let hend = hedge_offsets_host[h + 1] as usize;
                let mut node_ids: Vec<usize> = (hstart..hend)
                    .map(|idx| hyperedge_nodes_host[idx] as usize)
                    .filter(|&v| {
                        if v >= n { return false; }
                        let p = partition_host_refine[v] as usize;
                        p < kc && p != maj
                ...
                for &v in &node_ids {
                    let cur = partition_host_refine[v] as usize;
                    if cur == maj || cur >= kc { continue; }
                    if nodes_in_part_host[cur] <= 1 { continue; }
                    if nodes_in_part_host[maj] >= max_part { continue; }
                    let mut gain: i32 = 0;
                    for &hh in &node_hedge_list[v] {
                        let cnt = &hedge_part_cnt[hh as usize];
                        if cnt[cur] == 1 { gain += 1; }
                        if cnt[maj] == 0 { gain -= 1; }
                    }
                    if gain >= 0 {
                        for &hh in &node_hedge_list[v] {
                            let cnt = &mut hedge_part_cnt[hh as usize];
                            cnt[cur] = cnt[cur].saturating_sub(1);
                            cnt[maj] = cnt[maj].saturating_add(1);
                        }
                        nodes_in_part_host[cur] -= 1;
                        nodes_in_part_host[maj] += 1;
                        partition_host_refine[v] = maj as i32;
                        pierce_ops += 1;
                        break;
                    }
                }
            ...
            if pierce_ops == 0 { break 'hfc_piercing; }
        }

        {
            ...
            let boundary_verts: Vec<usize> = (0..n)
                .filter(|&v| {
            ...
            if n_boundary > 0 {
                ...
                let lahc_cap: usize = hyperparameters
                    ...
                    .unwrap_or_else(|| {
                        let mult = hyperparameters
                            .as_ref()
                            .and_then(|p| p.get("lahc_iters_mult").and_then(|v| v.as_i64()))
                            .map(|v| v.clamp(1, 10) as usize)
                            .unwrap_or(1);
                        (mult * n_boundary).max(100)
                    });
