...
extern "C" __global__ void mark_boundary_nodes_10k(
...
extern "C" __global__ void update_dirty_edge_flags_10k(
...
extern "C" __global__ void mark_dirty_edges_10k(
...
extern "C" __global__ void compute_moves_boundary_10k(
    const int num_nodes, const int num_parts, const int max_part_size,
...
extern "C" __global__ void compute_moves_flags_10k(
    const int num_nodes, const int num_parts, const int max_part_size,
    const int balance_weight, const int max_used_degree,
    const int neg_gain_threshold,
...
extern "C" __global__ void execute_selected_moves_10k(
    const int num_moves,
...
extern "C" __global__ void reset_counters_10k(int *num_valid_moves) {
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        *num_valid_moves = 0;
    }
}
...
extern "C" __global__ void compute_connectivity_per_hedge_10k(
...
extern "C" __global__ void label_propagation_step_10k(
    const int num_nodes, const int num_parts,
    const int *node_hyperedges, const int *node_offsets,
    const int *partition_in,
    const unsigned long long *edge_flags_all,
    const unsigned long long *edge_flags_double,
    int *partition_out,
    int *move_count
...
extern "C" __global__ void rebalance_greedy_10k(
    const int num_nodes, const int num_parts, const int max_part_size,
    const int *node_hyperedges, const int *node_offsets,
    const unsigned long long *edge_flags_all,
    int *partition, int *nodes_in_part
...
extern "C" __global__ void perturb_targeted_10k(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int perturb_strength,
...
extern "C" __global__ void my_calc_connectivity_10k(
...
extern "C" __global__ void compute_swap_gains_top3_10k(
    const int num_nodes,
    const int num_parts,
