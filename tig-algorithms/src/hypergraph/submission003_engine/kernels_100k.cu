...
extern "C" __global__ void precompute_edge_flags_100k(
    ...
    unsigned long long * __restrict__ edge_flags_all,
    unsigned long long * __restrict__ edge_flags_double
) {
    int hedge = blockIdx.x * blockDim.x + threadIdx.x;
    if (hedge < num_hyperedges) {
        int start = hyperedge_offsets[hedge];
...
extern "C" __global__ void compute_refinement_moves_optimized_100k(
    ...
    const unsigned long long* __restrict__ edge_flags_all,
    const unsigned long long* __restrict__ edge_flags_double,
    int *move_priorities
) {
...
// to the 100k track.  Everything ABOVE this line is the ORIGINAL
// kernels_100k.cu, byte for byte.  Every kernel below carries the _100k suffix
// so one PTX can hold all five tracks.
// ===========================================================================


// ---- exp6: flags + moves in ONE launch. Phase 1 computes the per-hyperedge flags exactly as
// precompute_edge_flags_100k (once per hyperedge); a software grid barrier (monotonic counter,
// target = calls * gridDim.x, passed by the host) orders it before phase 2, which is
// compute_refinement_moves_optimized_100k verbatim in a grid-stride loop. Requires all blocks to be
// co-resident: host launches at most 2 blocks per SM (128 threads each).
extern "C" __global__ void fused_flags_moves_100k(
    const int num_hyperedges,
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    ...
    // ---- phase 1a: small hyperedges, one thread each (identical to precompute_edge_flags_100k) ----
    ...
    // ---- phase 2: moves (identical to compute_refinement_moves_optimized_100k; flags read via L2) ----
    for (int node = blockIdx.x * blockDim.x + threadIdx.x; node < num_nodes; node += stride) {
        ...
        for (int j = 0; j < used_degree; j++) {
            // 100k semantics: PREFIX sampling (the first used_degree pins),
            // never the 100k's strided rel = j*node_degree/used_degree.
            int hyperedge = node_hyperedges[start + j];
...
// fused_flags_moves_filter_100k. The description above is kept because the
...
#define FF100K_SENT    ((int)0x80000000)
#define FF100K_MG_INIT (-1048576)
...
static __device__ __forceinline__ unsigned long long ff100k_lcg_advance(
...
static __device__ __forceinline__ unsigned int ff100k_ld_vol_u32(const unsigned int *p)
{
    unsigned int r;
...
#define FF100K_GRID_BARRIER(gb, target)                        \
    do {                                                   \
        __threadfence();                                   \
        __syncthreads();                                   \
        if (threadIdx.x == 0) {                            \
            atomicAdd((gb), 1u);                           \
            while (ff100k_ld_vol_u32(gb) < (target)) {         \
                __nanosleep(20);                          \
            }                                              \
        }                                                  \
        __syncthreads();                                   \
...
static __device__ __forceinline__ int ff100k_block_sum(int v, int *s_warp)
{
...
static __device__ __forceinline__ int ff100k_block_max(int v, int *s_warp)
{
    for (int off = 16; off > 0; off >>= 1) {
...
static __device__ __forceinline__ int ff100k_block_scan(int pred, int *s_warp, int *total_out)
{
    const unsigned lane = threadIdx.x & 31u;
    const unsigned warp = threadIdx.x >> 5;
...
static __device__ __forceinline__ ulonglong2 ff100k_ldcg_u2(const ulonglong2 *p)
{
    ulonglong2 r;
...
#define FF100K_PC_INC4(flags) do { \
    unsigned long long c = (flags), t; \
    t = pc0 & c; pc0 ^= c; c = t; \
    t = pc1 & c; pc1 ^= c; c = t; \
    t = pc2 & c; pc2 ^= c; c = t; \
...
#define FF100K_PC_ADD_SHFL4(off) do { \
...
#define FF100K_PC_ADD_SHFL5(off) do { \
    unsigned long long b_, cy_, t1_, t2_; \
    b_ = __shfl_xor_sync(0xffffffffu, pc0, (off)); \
    t1_ = pc0 ^ b_; cy_ = pc0 & b_; pc0 = t1_; \
    b_ = __shfl_xor_sync(0xffffffffu, pc1, (off)); \
...
#define FF100K_PC_ADD_SHFL6(off) do { \
    unsigned long long b_, cy_, t1_, t2_; \
    b_ = __shfl_xor_sync(0xffffffffu, pc0, (off)); \
    t1_ = pc0 ^ b_; cy_ = pc0 & b_; pc0 = t1_; \
    b_ = __shfl_xor_sync(0xffffffffu, pc1, (off)); \
...
#define FF100K_PC_ADD_SHFL7(off) do { \
    unsigned long long b_, cy_, t1_, t2_; \
    b_ = __shfl_xor_sync(0xffffffffu, pc0, (off)); \
    t1_ = pc0 ^ b_; cy_ = pc0 & b_; pc0 = t1_; \
    b_ = __shfl_xor_sync(0xffffffffu, pc1, (off)); \
...
#define FF100K_PC_ADD_SHFL9(off) do { \
    unsigned long long b_, cy_, t1_, t2_; \
    b_ = __shfl_xor_sync(0xffffffffu, pc0, (off)); \
    t1_ = pc0 ^ b_; cy_ = pc0 & b_; pc0 = t1_; \
    b_ = __shfl_xor_sync(0xffffffffu, pc1, (off)); \
...
#define FF100K_PC_GET3(tp) ( \
    ((int)((pc0 >> (tp)) & 1ULL) << 0) | \
    ((int)((pc1 >> (tp)) & 1ULL) << 1) | \
    ((int)((pc2 >> (tp)) & 1ULL) << 2) )

#define FF100K_PC_GET4(tp) ( \
    ((int)((pc0 >> (tp)) & 1ULL) << 0) | \
    ((int)((pc1 >> (tp)) & 1ULL) << 1) | \
    ((int)((pc2 >> (tp)) & 1ULL) << 2) | \
    ((int)((pc3 >> (tp)) & 1ULL) << 3) )

#define FF100K_PC_GET5(tp) ( \
    ((int)((pc0 >> (tp)) & 1ULL) << 0) | \
    ((int)((pc1 >> (tp)) & 1ULL) << 1) | \
    ((int)((pc2 >> (tp)) & 1ULL) << 2) | \
    ((int)((pc3 >> (tp)) & 1ULL) << 3) | \
...
#define FF100K_FE_RED_1
#define FF100K_FE_RED_2   FF100K_FE_STEP(1);
#define FF100K_FE_RED_4   FF100K_FE_STEP(2); FF100K_FE_STEP(1);
#define FF100K_FE_RED_8   FF100K_FE_STEP(4); FF100K_FE_STEP(2); FF100K_FE_STEP(1);
#define FF100K_FE_RED_16  FF100K_FE_STEP(8); FF100K_FE_STEP(4); FF100K_FE_STEP(2); FF100K_FE_STEP(1);
#define FF100K_FE_RED_32  FF100K_FE_STEP(16); FF100K_FE_STEP(8); FF100K_FE_STEP(4); FF100K_FE_STEP(2); FF100K_FE_STEP(1);

// moves phase: OR (cand_mask), + (count_current_present, crit), bit-sliced +.
#define FF100K_MV_STEP(ADDM, off) do {                                             \
...
#define FF100K_MV_RED_1
#define FF100K_MV_RED_2   FF100K_MV_STEP(FF100K_PC_ADD_SHFL4, 1);
#define FF100K_MV_RED_4   FF100K_MV_STEP(FF100K_PC_ADD_SHFL5, 2); FF100K_MV_STEP(FF100K_PC_ADD_SHFL5, 1);
#define FF100K_MV_RED_8   FF100K_MV_STEP(FF100K_PC_ADD_SHFL6, 4); FF100K_MV_STEP(FF100K_PC_ADD_SHFL6, 2); \
                      FF100K_MV_STEP(FF100K_PC_ADD_SHFL6, 1);
#define FF100K_MV_RED_16  FF100K_MV_STEP(FF100K_PC_ADD_SHFL7, 8); FF100K_MV_STEP(FF100K_PC_ADD_SHFL7, 4); \
                      FF100K_MV_STEP(FF100K_PC_ADD_SHFL7, 2); FF100K_MV_STEP(FF100K_PC_ADD_SHFL7, 1);
#define FF100K_MV_RED_32  FF100K_MV_STEP(FF100K_PC_ADD_SHFL9, 16); FF100K_MV_STEP(FF100K_PC_ADD_SHFL9, 8); \
                      FF100K_MV_STEP(FF100K_PC_ADD_SHFL9, 4); FF100K_MV_STEP(FF100K_PC_ADD_SHFL9, 2); \
                      FF100K_MV_STEP(FF100K_PC_ADD_SHFL9, 1);

// ---- per-pin bodies, factored out so the batched gather can hoist the loads --
// FF100K_FE_ACC is the champion's flags accumulation for ONE pin, taking the pin's
...
// range tests did).
#define FF100K_GIANT_BATCH 16

#define FF100K_FE_ACC(P) do {                                                      \
    const int part_ = (P);                                                     \
...
// FF100K_MV_PIN is the champion's moves accumulation for ONE pin, taking the pin's
...
#define FF100K_MV_PIN(FP, INCM) do {                                               \
...
#define FF100K_FE_BODY(G, SH, RED)                                                 \
{                                                                              \
...
// come from the enclosing FF100K_MV_BODY scope.
//
...
#define FF100K_MV_CSR(G) do {                                                      \
    const int j1_ = j_ + (G), j2_ = j_ + 2 * (G), j3_ = j_ + 3 * (G);          \
    v1_ = (j1_ < used_degree);                                                 \
    v2_ = (j2_ < used_degree);                                                 \
    v3_ = (j3_ < used_degree);                                                 \
    /* 100k: PREFIX sampling -- pin slot j IS CSR slot j (see NOTES S2). */   \
    h0_ = __ldg(&node_hyperedges[nstart + j_]);                                \
    h1_ = v1_ ? __ldg(&node_hyperedges[nstart + j1_]) : 0;                     \
    h2_ = v2_ ? __ldg(&node_hyperedges[nstart + j2_]) : 0;                     \
    h3_ = v3_ ? __ldg(&node_hyperedges[nstart + j3_]) : 0;                     \
} while (0)
    ...
    if ((unsigned)current_part < (unsigned)num_parts &&                        \
        ...
        if (go_) {                                                             \
            for (;;) {                                                         \
                ...
                const ulonglong2 f0_ = ff100k_ldcg_u2(&edge_flags_pair[h0_]);      \
                if (v1_) f1_ = ff100k_ldcg_u2(&edge_flags_pair[h1_]);              \
                if (v2_) f2_ = ff100k_ldcg_u2(&edge_flags_pair[h2_]);              \
                if (v3_) f3_ = ff100k_ldcg_u2(&edge_flags_pair[h3_]);              \
                FF100K_MV_PIN(f0_, INCM);                                          \
                if (v1_) { FF100K_MV_PIN(f1_, INCM); }                             \
                if (v2_) { FF100K_MV_PIN(f2_, INCM); }                             \
                if (v3_) { FF100K_MV_PIN(f3_, INCM); }                             \
                j_ += 4 * (G);                                                 \
...
extern "C" __global__ __launch_bounds__(128, 2) void fused_flags_moves_filter_100k(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *__restrict__ hyperedge_nodes,
    ...
    for (int i = (int)blockIdx.x; i < n_fe_giant; i += (int)gridDim.x) {
        ...
        for (int k0 = (int)threadIdx.x; k0 < hsz;
             k0 += (int)blockDim.x * FF100K_GIANT_BATCH) {
            int gn_[FF100K_GIANT_BATCH];
            int gp_[FF100K_GIANT_BATCH];
#pragma unroll
            for (int m = 0; m < FF100K_GIANT_BATCH; m++) {
...
#pragma unroll
    ...
    for (int t = warp_global; t < n_fe_tasks; t += warp_stride) {
        ...
        const int4 im_ = fim_;
        switch (im_.w) {
            case 0:  FF100K_FE_BODY(1,  0, FF100K_FE_RED_1)  break;
            case 1:  FF100K_FE_BODY(2,  1, FF100K_FE_RED_2)  break;
            case 2:  FF100K_FE_BODY(4,  2, FF100K_FE_RED_4)  break;
            case 3:  FF100K_FE_BODY(8,  3, FF100K_FE_RED_8)  break;
            case 4:  FF100K_FE_BODY(16, 4, FF100K_FE_RED_16) break;
            default: FF100K_FE_BODY(32, 5, FF100K_FE_RED_32) break;
        }
        fim_ = nx_;
    }

    // ---- barrier 1 ---------------------------------------------------------
    FF100K_GRID_BARRIER(grid_barrier, barrier_base + gridDim.x);
    ...
    for (int t = warp_global; t < n_mv_tasks; t += warp_stride) {
        ...
        const int4 im_ = mim_;
        switch (im_.w) {
            case 0:  FF100K_MV_BODY(1,  0, FF100K_PC_INC3, FF100K_PC_GET3, FF100K_MV_RED_1)  break;
            case 1:  FF100K_MV_BODY(2,  1, FF100K_PC_INC3, FF100K_PC_GET4, FF100K_MV_RED_2)  break;
            case 2:  FF100K_MV_BODY(4,  2, FF100K_PC_INC3, FF100K_PC_GET5, FF100K_MV_RED_4)  break;
            case 3:  FF100K_MV_BODY(8,  3, FF100K_PC_INC3, FF100K_PC_GET6, FF100K_MV_RED_8)  break;
            case 4:  FF100K_MV_BODY(16, 4, FF100K_PC_INC3, FF100K_PC_GET7, FF100K_MV_RED_16) break;
            default: FF100K_MV_BODY(32, 5, FF100K_PC_INC4, FF100K_PC_GET9, FF100K_MV_RED_32) break;
        }
        mim_ = nx_;
    }

    {
        const int m = ff100k_block_max(blk_max, s_warp);
        if (threadIdx.x == 0) atomicMax(max_gain_buf, m);
    }

    // ---- barrier 2 ---------------------------------------------------------
    FF100K_GRID_BARRIER(grid_barrier, barrier_base + 2u * gridDim.x);

    // max over non-sentinel keys of (key >> 16); FF100K_MG_INIT if there are none
    ...
    for (int it = 0; it < nrounds; it++) {
        ...
        // value is never used when key == FF100K_SENT and tabu_until[node] is in
        ...
        // ff100k_block_sum in pass A and the rank with this scan in pass B1; the
        // scan already returns both).
        int etot = 0;
        const int erank = ff100k_block_scan(elig, s_warp, &etot);
        if (act && threadIdx.x == 0) blk_elig[tile] = etot;

        // ---- barrier (was barrier 3) --------------------------------------
        btgt_ += (unsigned int)gridDim.x;
        FF100K_GRID_BARRIER(grid_barrier, btgt_);

        int partsum = 0;
        for (int i = (int)threadIdx.x; i < plim; i += (int)blockDim.x) {
            partsum += __ldcg(&blk_elig[i]);
        }
        const int baseE = ff100k_block_sum(partsum, s_warp);
        ...
        if (elig) {
            ...
            const unsigned long long x = ff100k_lcg_advance(lcg_x0, n);
        ...
        int vtot = 0;
        const int vrank = ff100k_block_scan(valid, s_warp, &vtot);
        ...
        const int base = ff100k_block_sum(partsum2, s_warp);
    ...
    if (blockIdx.x == 0) {
        ...
        const int t = ff100k_block_sum(s, s_warp);
...
//
// Body: `fused_flags_moves_filter_100k`'s prologue VERBATIM (phase 0, the
...
//
//   ..._flat_100k   __launch_bounds__(128, 2)   host `fused_mode = 1`
//   ..._flatw_100k  __launch_bounds__(256, 2)   host `fused_mode = 2`
//
...
extern "C" __global__ __launch_bounds__(128, 2) void fused_flags_moves_filter_flat_100k(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *__restrict__ hyperedge_nodes,
...
#pragma unroll
    ...
    // L2-resident i32 loads per node per pass and one extra ff100k_lcg_advance per
    ...
    for (int t = t0; t < t1; t++) {
        ...
        const int tu_ld = (node < num_nodes) ? __ldcg(&tabu_until[node]) : 0;
        int elig = 0;
        if (node < num_nodes && key_ld != FF100K_SENT) {
            const int gain = key_ld >> 16;
            const bool is_tabu = (tu_ld > round_idx) && (gain < aspiration);
            if (!is_tabu && gain <= 0 && gain >= -3) elig = 1;
        }
        blkE += ff100k_block_sum(elig, s_warp);
    }
    if (threadIdx.x == 0) blk_elig[blockIdx.x] = blkE;

    // ---- barrier 3 ---------------------------------------------------------
    FF100K_GRID_BARRIER(grid_barrier, barrier_base + 3u * gridDim.x);

    int psE_ = 0;
    for (int i = (int)threadIdx.x; i < (int)blockIdx.x; i += (int)blockDim.x) {
        psE_ += __ldcg(&blk_elig[i]);
    }
    const int baseE_block = ff100k_block_sum(psE_, s_warp);
    ...
    for (int t = t0; t < t1; t++) {
        ...
        const int valid = (posval | accept);
        int vtot = 0;
        (void)ff100k_block_scan(valid, s_warp, &vtot);
        runE += etot;
        blkV += vtot;
    }
    ...
    // ---- barrier 4 ---------------------------------------------------------
    FF100K_GRID_BARRIER(grid_barrier, barrier_base + 4u * gridDim.x);

    int psV_ = 0;
    for (int i = (int)threadIdx.x; i < (int)blockIdx.x; i += (int)blockDim.x) {
        psV_ += __ldcg(&tile_valid[i]);
    }
    const int baseV_block = ff100k_block_sum(psV_, s_warp);
    ...
    if (blockIdx.x == 0) {
        ...
        const int tot = ff100k_block_sum(s, s_warp);
        if (threadIdx.x == 0) cand[0] = tot;
    }
}

extern "C" __global__ __launch_bounds__(256, 2) void fused_flags_moves_filter_flatw_100k(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *__restrict__ hyperedge_nodes,
