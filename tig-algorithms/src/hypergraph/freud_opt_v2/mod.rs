...
pub fn help() {
    ...
    println!("  edge_flag_cap - Max sampled edges per node (default: 256, range: 1-2048)");
    println!();
    println!("Usage:");
...
pub fn solve_challenge(
    ...
    let edge_flag_cap = hyperparameters
        .as_ref()
        .and_then(|p| p.get("edge_flag_cap"))
        .and_then(|v| v.as_i64())
        .map(|v| v.clamp(1, 2048) as usize)
        .unwrap_or(256);

    let buffer_size = (challenge.num_nodes as usize) * edge_flag_cap;
    ...
    println!("edge_flag_cap: {:?}", edge_flag_cap);
