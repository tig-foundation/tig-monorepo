...
extern "C" __global__ void compute_refinement_moves_optimized_10k(
    ...
    int used_degree = node_degree > 512 ? 512 : node_degree;
    if (used_degree <= 0) return;
...
extern "C" __global__ void crossover_partitions_10k(
    ...
    if (part_a == part_b) {
        ...
        unsigned long long bit_a = 1ULL << part_a;
    ...
    atomicAdd(&child_nodes_in_part[chosen_part], 1);
}

extern "C" __global__ void perturb_solution_targeted_10k(
    const int num_nodes,
    const int num_hyperedges,
    const int num_parts,
    const int max_part_size,
...
extern "C" __global__ void perturb_path_relink_10k(
    ...
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        ...
        for (int node = 0; node < num_nodes && moves_made < target_moves; node++) {
            ...
            state = state * 6364136223846793005ULL + 1442695040888963407ULL;
            unsigned long long threshold = ((unsigned long long)fraction_percent * 0x100000000ULL) / 100ULL;
