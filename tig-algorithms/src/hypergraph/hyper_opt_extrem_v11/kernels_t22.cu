...
// __launch_bounds__ variants for compute_refinement_moves_optimized_100k.
// lb0 = original (no annotation) = control gate; lb2/lb3/lb4 = minBlocksPerSM constraint.
// Selected at runtime via hp.launch_bounds_min_blocks (0=control, 2/3/4=occupancy constrained).
// MAX_THREADS=128 matches block_size; MIN=minBlocksPerSM caps registers/thread → higher occupancy.

extern "C" __launch_bounds__(128, 2) __global__ void compute_refinement_moves_optimized_100k_lb2(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
...
extern "C" __launch_bounds__(128, 3) __global__ void compute_refinement_moves_optimized_100k_lb3(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
...
extern "C" __launch_bounds__(128, 4) __global__ void compute_refinement_moves_optimized_100k_lb4(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
...
// Computes gain_scores[v * np_sp + t] = gain of moving node v to partition t,
// using pin-count matrix hpc[h * np_sp + p] (Ronzani&Silvano ecdbd296 machinery).
// Non-boundary or same-partition entries set to INT_MIN.
// CPU does sequential greedy commit unchanged (3dad185e pattern, bit-exact).
extern "C" __global__ void compute_balanced_swap_scores_100k(
    int n_sp,
    int np_sp,
    const int* sp_part,
    const unsigned short* hpc,
    const unsigned int* nd_off,
    const unsigned int* nd_hedges,
    int* gain_scores
) {
    ...
    // Initialize output to INT_MIN (non-boundary or same-partition = skip)
    for (int t = 0; t < np_sp; t++) {
        gain_scores[v * np_sp + t] = INT_MIN;
    }
    if (p_v < 0 || p_v >= np_sp) return;

    int hs = (int)nd_off[v];
    int he = (int)nd_off[v + 1];

    int base_gain = 0;
    // tgt[t] accumulates -1 per hedge where partition t has 0 pins
    int tgt[64];
    for (int t = 0; t < np_sp; t++) tgt[t] = 0;
    int is_boundary = 0;

    for (int hi = hs; hi < he; hi++) {
        int h = (int)nd_hedges[hi];
        int base_h = h * np_sp;

        unsigned short cnt_pv = hpc[base_h + p_v];
        if (cnt_pv == 1) base_gain++;

        int parts_present = 0;
        for (int t = 0; t < np_sp; t++) {
            unsigned short cnt_t = hpc[base_h + t];
            if (cnt_t > 0) parts_present++;
            if (t != p_v && cnt_t == 0) tgt[t]--;
        }
        if (parts_present >= 2) is_boundary = 1;
    }
    ...
    for (int t = 0; t < np_sp; t++) {
        if (t != p_v) {
            gain_scores[v * np_sp + t] = base_gain + tgt[t];
        }
    }
}
