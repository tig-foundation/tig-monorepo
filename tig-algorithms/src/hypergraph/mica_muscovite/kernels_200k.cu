...
extern "C" __global__ void assign_from_elite_votes_200k(
    ...
    const ulonglong2 *edge_flags_best,
    int *partition_out                      
) {
    (void)num_elites;
...
extern "C" __global__ void precompute_edge_flags_200k(
    ...
    if (hedge < num_hyperedges) {
        ...
        edge_flags[hedge] = make_ulonglong2(flags_all, flags_double);
    }
}
...
extern "C" __global__ void compute_refinement_moves_optimized_200k(
    ...
    for (int j = 0; j < used_degree; j++) {
        ...
        const ulonglong2 __ef = __ldg(edge_flags + hyperedge);
        unsigned long long flags_all = __ef.x;
        unsigned long long flags_double = __ef.y;
        ...
        while (flags) {
            ...
            flags &= (flags - 1);
            unsigned long long __b = 1ULL << bit;
            if (cand_mask & __b) { part_counts[bit]++; }
            else { part_counts[bit] = 1; cand_mask |= __b; }
        }
    ...
    while (cand_mask) {
        ...
        if (!better && total_gain == best_gain) {
            if (zgm_mode == 1) {
                int cnt_t = (int)part_counts[target_part];
                int cnt_b = (int)part_counts[best_target];
                if (cnt_t > cnt_b) better = true;
                else if (cnt_t == cnt_b && target_part < best_target) better = true;
            } else {
...
__device__ __forceinline__ bool w8_mieux_200k(
...
extern "C" __global__ void compute_refinement_moves_warp8_200k(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
    ...
    (void)zgm_mode;  // this kernel hard-codes its tie-break
    const int gid  = blockIdx.x * blockDim.x + threadIdx.x;
    const int node = gid >> 3;
    const int voie = gid & 7;
    if (node >= num_nodes) return;
    ...
    if (voie == 0) move_priorities[node] = 0x80000000;
    ...
    const unsigned long long ma_tranche  = 0xFFULL << (voie * 8);
    ...
    for (int j = 0; j < used_degree; j++) {
        ...
        unsigned long long mask = (flags_all & ~current_bit) | (flags_double & current_bit);
        if (voie == 0 && (mask & current_bit)) count_current_present++;

        unsigned long long flags = mask & ~current_bit & ma_tranche;
        while (flags) {
            int bit = __ffsll(flags) - 1;
            flags &= (flags - 1);
            cand_mask |= 1ULL << bit;
    ...
    while (tmp) {
        ...
        if (w8_mieux_200k(total_gain, target_part, best_gain, best_target,
    ...
    for (int off = 4; off > 0; off >>= 1) {
        ...
        if ((voie < off) && ot >= 0 &&
            w8_mieux_200k(og, ot, best_gain, best_target, shared_nodes_in_part, node)) {
...
extern "C" __global__ void planted_ranges_200k(
    const unsigned char *seed,
    const int target_num_nodes,
    const int num_hyperedges,
    const int *hyperedge_sizes,
    const float *level_weights,
    const int nb_level_weights,
    int *range_start,
    int *range_end,
    int *range_level
) {
    for (int e = threadIdx.x + blockIdx.x * blockDim.x; e < num_hyperedges; e += blockDim.x * gridDim.x) {
        curandState state;
        curand_init(((unsigned long long *)(seed))[2], e, 0, &state);

        int sz = hyperedge_sizes[e];
        if (sz <= 0) { range_start[e] = 0; range_end[e] = target_num_nodes; range_level[e] = 0; continue; }
        int num_levels = (int)log2f((float)target_num_nodes / (float)sz) + 1;
        if (num_levels > nb_level_weights) num_levels = nb_level_weights;
        if (num_levels < 1) num_levels = 1;

        float total = 0.0f;
        for (int i = 0; i < num_levels; i++) total += level_weights[i];
        float rv = curand_uniform(&state) * total;
        float cum = 0.0f;
        int level = num_levels - 1;
        for (int i = 0; i < num_levels; i++) {
            cum += level_weights[i];
            if (rv <= cum) { level = i; break; }
        }

        int num_groups = 1 << level;
        int group = curand(&state) % num_groups;

        int s = target_num_nodes / num_groups;
        int r = target_num_nodes % num_groups;
        int st, en;
        if (group < r) { st = (s + 1) * group;              en = (s + 1) * (group + 1); }
        else           { st = (s + 1) * r + s * (group - r); en = (s + 1) * r + s * (group + 1 - r); }
