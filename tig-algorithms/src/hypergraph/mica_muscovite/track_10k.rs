...
pub fn solve(
    ...
    let planted_kernel = module.load_function("planted_ranges_10k")?;
    ...
    let refinement_rounds = {
        ...
        if budget == 0 {
            ...
            let fixed: u64 = 25_000_000_000;
            let per_round: u64 = 4000000;
    ...
    // sa_rng_fix: alternate random range for the annealing schedule.
    let sa_rng_fix: bool = hyperparameters
        .as_ref()
        .and_then(|p| p.get("sa_rng_fix").and_then(|v| v.as_u64()))
        .map(|v| v != 0)
        .unwrap_or(false);
    ...
    macro_rules! replay_execute_moves_host {
        ($sorted_nodes:expr, $sorted_parts:expr) => {{
            ...
            if host_moves_executed > 0 {
                ...
                stream.memcpy_htod(partition_host_mirror.as_slice(), &mut d_partition)?;
            }
    ...
    let planted_lvl: i32 = hyperparameters.as_ref()
        ...
        .map(|v| v.clamp(0, 19) as i32).unwrap_or(2);
    ...
    if planted && cible_plausible {
        ...
        for v in 0..n {
            ...
            partition_host_mirror[v] = ((base + (v as i64 % etendue)) % np) as i32;
        }
        if incoherents * 1000 > n {
            stream.memcpy_dtoh(&d_partition, &mut partition_host_mirror)?;
            stream.memcpy_dtoh(&d_nodes_in_part, &mut nodes_in_part_mirror)?;
        } else {
            for x in nodes_in_part_mirror.iter_mut() { *x = 0; }
            for v in 0..n { nodes_in_part_mirror[partition_host_mirror[v] as usize] += 1; }
            stream.memcpy_htod(&partition_host_mirror, &mut d_partition)?;
            stream.memcpy_htod(&nodes_in_part_mirror, &mut d_nodes_in_part)?;
        }
    }

    let stall_window: usize = hyperparameters
        .as_ref()
        .and_then(|p| p.get("stall_window").and_then(|v| v.as_i64()))
        .map(|v| v.clamp(0, 50_000) as usize)
        .unwrap_or(0);
    let mut cum_gain = 0i64;
    let mut best_cum = i64::MIN;
    ...
    for round in 0..refinement_rounds {
        ...
        if !move_keys_host.iter().any(|&k| k > 0) {
            break;
        }
        ...
        cum_gain += round_gain;
        if cum_gain > best_cum {
            best_cum = cum_gain;
            since_best = 0;
        } else {
            since_best += 1;
        }
        if stall_window > 0 && since_best >= stall_window {
            break;
        }
    ...
    for ils_iter in 0..ils_iterations {
        ...
        let accept = if delta <= 0 {
            ...
            let rng_val = ((rng_seed >> if sa_rng_fix { 32 } else { 33 }) as f64) / (u32::MAX as f64);
