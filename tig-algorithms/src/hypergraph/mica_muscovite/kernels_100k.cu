...
extern "C" __global__ void compute_refinement_moves_optimized_100k(
    ...
    while (cand_mask) {
        ...
        if (zgm_mode == 3) {
            ...
            int tb = (zgm_mode == 1) ? (int)part_counts[target_part]
...
__device__ __forceinline__ bool w8_mieux_100k(
...
extern "C" __global__ void compute_refinement_moves_warp8_100k(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
    ...
    while (tmp) {
        ...
        if (w8_mieux_100k(total_gain, target_part, best_gain, best_target,
    ...
    for (int off = 4; off > 0; off >>= 1) {
        ...
        if ((voie < off) && ot >= 0 &&
            w8_mieux_100k(og, ot, best_gain, best_target, shared_nodes_in_part, node)) {
...
extern "C" __global__ void planted_ranges_100k(
