...
__device__ void isocline_body_moves(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
...
__device__ void isocline_body_flags(
    const int num_hyperedges,
    const int num_nodes,
    const int *hyperedge_nodes,
    const int *hyperedge_offsets,
...
extern "C" __global__ void precompute_edge_flags_10k(
    ...
    isocline_body_flags(num_hyperedges, num_nodes, hyperedge_nodes, hyperedge_offsets,
                   partition, edge_flags_all, edge_flags_double);
}
...
extern "C" __global__ void compute_refinement_moves_optimized_10k(
    ...
    const unsigned long long *edge_flags_all, const unsigned long long *edge_flags_double,
    int *move_priorities, int *num_valid_moves
) {
    isocline_body_moves(num_nodes, num_parts, max_part_size, node_hyperedges, node_offsets,
                   partition, nodes_in_part, edge_flags_all, edge_flags_double,
                   move_priorities, num_valid_moves);
}

extern "C" __global__ void planted_ranges_10k(
