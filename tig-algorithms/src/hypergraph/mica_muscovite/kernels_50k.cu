...
__device__ __forceinline__ bool w8_mieux_50k(
...
extern "C" __global__ void compute_refinement_moves_warp8_50k(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
    ...
    while (tmp) {
        ...
        if (w8_mieux_50k(total_gain, target_part, best_gain, best_target,
    ...
    for (int off = 4; off > 0; off >>= 1) {
        ...
        if ((voie < off) && ot >= 0 &&
            w8_mieux_50k(og, ot, best_gain, best_target, shared_nodes_in_part, node)) {
...
extern "C" __global__ void planted_ranges_50k(
