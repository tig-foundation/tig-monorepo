...
pub fn solve_sub_instance(
    ...
    let density = instance.difficulty.num_hyperedges as f32 / instance.num_nodes as f32;
    let complexity_factor = if density > 4.0 { 8 } else if density > 2.0 { 6 } else { 4 };
    let base_clusters = instance.num_parts as usize * complexity_factor;
    let num_hedge_clusters = if instance.difficulty.num_hyperedges > 100000 {
    ...
    let batch_size = if instance.num_nodes < 30000 {
        std::cmp::min(std::cmp::max(32768, sm_count * warp_size * 16), instance.num_nodes as usize)
    } else if instance.num_nodes < 100000 {
    ...
    for round in 0..max_rounds {
        ...
        let termination_threshold = if instance.num_nodes < 50000 {
            (instance.num_nodes as i32 / 800).max(2)
        } else {
            (instance.num_nodes as i32 / 1200).max(3)
        };
