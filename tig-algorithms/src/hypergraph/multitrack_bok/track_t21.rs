...
// ============================================================================
// BEST-OF-K + CUT-OVERLAY HELPERS (ported verbatim from track_t23; host-side,
// size-agnostic, RNG-free => deterministic). Operate on hyperedge CSR arrays +
// a node partition; depend on NO 10k/20k-specific CUDA kernels.
// ============================================================================
...
pub(crate) fn solve_core(
    ...
    // ==== best-of-K + cut-overlay hyperparameters (defaults reproduce the
    // pre-port t21 exactly: runs=1 => one run, overlay=0 => no overlay). ====
    let bok_runs = hyperparameters
        .as_ref()
        .and_then(|p| p.get("runs").and_then(|v| v.as_i64()))
        .map(|v| v.clamp(1, 36) as usize)
        .unwrap_or(1);
    ...
    // Outer-scope sizes + host CSR (for km1 scoring / overlay; RNG-free reads).
    let n_outer = challenge.num_nodes as usize;
    let num_parts_outer = challenge.num_parts as usize;
    let max_ps_outer = challenge.max_part_size as usize;
    let co_hedge_offsets: Vec<i32> = stream.memcpy_dtov(&challenge.d_hyperedge_offsets)?;
    let co_hedge_nodes: Vec<i32> = stream.memcpy_dtov(&challenge.d_hyperedge_nodes)?;
    ...
    let mut bok_best_part: Vec<i32> = Vec::new();
    let mut bok_best_km1: i64 = i64::MAX;

    for bok_run in 0..bok_runs {
        // run 0 => offset 0 => byte-identical to the pre-port single run.
    ...
    let mut rng = SmallRng::from_seed({ let mut s = [0u8; 32]; s[..challenge.seed.len().min(32)].copy_from_slice(&challenge.seed[..challenge.seed.len().min(32)]); let ro = run_off.to_le_bytes(); for i in 0..8 { s[i] ^= ro[i]; } s });
    ...
    // ---- best-of-K: score this run's final partition, keep the global best,
    // and (when overlay is on) stash it into the elite pool. ----
    {
        let mut bok_seen = vec![false; num_parts_outer];
        ...
        if run_km1 < bok_best_km1 {
            ...
            bok_best_part = part_host.clone();
        }
        if overlay_enabled {
            co_elites.push(part_host.clone());
    ...
    // From the K raw elites, greedily pick an `overlay_select`-sized subset with
    // maximally DIFFERENT cut-sets (label-invariant Hamming distance over the
    // per-hyperedge cut boolean), anchored on the best-km1 elite. overlay_select
    // == 0 (default) or >= pool size => use all elites (unchanged).
    if overlay_enabled && overlay_select > 0 && overlay_select < co_elites.len() {
        let nh = co_hedge_offsets.len().saturating_sub(1);
        ...
        let cut_bits: Vec<Vec<u64>> = co_elites
            ...
            .map(|part| {
                ...
                for e in 0..nh {
                    let s = co_hedge_offsets[e] as usize;
                    let t = co_hedge_offsets[e + 1] as usize;
                    if t <= s {
                        continue;
                    }
                    let first = part[co_hedge_nodes[s] as usize];
                    let mut cut = false;
                    for &pin in &co_hedge_nodes[s + 1..t] {
    ...
    if overlay_enabled && !co_elites.is_empty() {
        if let Some(proj) = co_overlay_resolve(
            ...
            let mut co_sizes = vec![0i32; num_parts_outer];
            let mut feasible = true;
            for &p in proj.iter() {
                if p < 0 || (p as usize) >= num_parts_outer {
                    feasible = false;
                    break;
                }
            ...
            if feasible {
                for p in 0..num_parts_outer {
                    if co_sizes[p] < 1 || co_sizes[p] > max_ps_outer as i32 {
                        feasible = false;
                        break;
                    }
                }
            ...
            if feasible {
                // Strict-win adopt only (monotone): recompute true km1 and keep
                // the projection solely if it beats the incumbent.
                let mut co_seen = vec![false; num_parts_outer];
    ...
    save_solution(&Solution { partition: bok_best_part.iter().map(|&x| x as u32).collect() })?;
    Ok(())
}
