...
// ============================================================================
// HOST-SIDE HELPERS (ported from track_t23): exact km1 eval + cut-overlay.
// All size-agnostic and RNG-free => deterministic. No track-specific kernels.
// ============================================================================
...
// strict true-km1 improvement (monotone). All host-side, index-ordered and
...
pub fn solve(
    ...
    // runs=1 (default) + overlay=0 (default) => byte-identical to prior t25. ====
    let bok_runs = hyperparameters
        .as_ref()
        .and_then(|p| p.get("runs").and_then(|v| v.as_i64()))
        .map(|v| v.clamp(1, 31) as usize)
        .unwrap_or(1);
    ...
    
    {
        ...
        for _ in 0..kick_repolish {
            ...
            stream.memcpy_dtoh(&d_move_priorities, &mut move_keys_host)?;
            valid_moves.clear();
            let mut rng_state_kr = 11223344u64.wrapping_add(run_off);
            for (node, &key) in move_keys_host.iter().enumerate() {
                if key as u32 != 0x80000000 {
                    let gain = key >> 16;
                    if gain > 0 {
    ...
    // per-hyperedge cut boolean), anchored on the best-km1 elite. Diverse
    // restarts => richer overlay re-solve. overlay_select==0 (default) or
    // >= pool size => use all elites (unchanged).
    ...
    if overlay_enabled && !co_elites.is_empty() {
        if let Some(proj) = co_overlay_resolve(
            ...
            if feasible {
                // mem_eval_lambda cross-check: only a recomputed strict win over
                // the incumbent is ever adopted (monotone).
