...
// (not re-seed); seed still present for kernel signature compatibility.
...
// __launch_bounds__ variants — selected at runtime via hp.launch_bounds_min_blocks (0=control, 2/3/4).
// Caps registers/thread → higher SM occupancy → better latency hiding (memory-bound kernel).
extern "C" __launch_bounds__(128, 2) __global__ void compute_refinement_moves_optimized_200k_lb2(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
...
extern "C" __launch_bounds__(128, 3) __global__ void compute_refinement_moves_optimized_200k_lb3(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
...
extern "C" __launch_bounds__(128, 4) __global__ void compute_refinement_moves_optimized_200k_lb4(
    const int num_nodes,
    const int num_parts,
    const int max_part_size,
    const int *node_hyperedges,
...
// ==== score_pr_g3_warp_200k — GPU SCORING for g[3+] path-relink pass (i96, port from t25/i53) ====
...
extern "C" __global__ void score_pr_g3_warp_200k(
