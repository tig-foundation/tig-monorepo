...
extern "C" __global__ void compute_refinement_moves(
    ...
    if (node < num_nodes) {
        ...
        for (int j = 0; j < node_degree; j++) {
            unsigned long long flags_low = edge_flags_low[j];
            unsigned long long flags_high = use_dual_buffer ? edge_flags_high[j] : 0;
            
            while (flags_low) {
                int bit = __ffsll(flags_low) - 1;
                flags_low &= ~(1ULL << bit);
                if (bit != current_part && !seen[bit]) {
            ...
            if (use_dual_buffer) {
                while (flags_high) {
                    int bit = __ffsll(flags_high) - 1;
                    flags_high &= ~(1ULL << bit);
                    int part = 64 + bit;
                    if (part < num_parts && part != current_part && !seen[part]) {
                        candidates[num_candidates++] = part;
                        seen[part] = true;
                    }
                }
            }
        ...
        if (min_part != current_part && !seen[min_part] && num_candidates < 256) {
            candidates[num_candidates++] = min_part;
            seen[min_part] = true;
        }
