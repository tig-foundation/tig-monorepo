        ...
        "c009" => {
            ...
            #[cfg(feature = "c009")]
            {
                ...
                let solve_challenge_fn = unsafe {
                    library.get::<fn(
                        &c009::Challenge,
                        save_solution: &dyn Fn(&c009::Solution) -> anyhow::Result<()>,
                        Option<String>,
                        Arc<CudaModule>,
                        Arc<CudaStream>,
                        &cudaDeviceProp,
                ...
                let num_sub = c009::NUM_SUB_INSTANCES as u64;
                ...
                ctx.set_blocking_synchronize()?;
                let module = ctx.load_module(ptx)?;
                // Generation is not charged against innovator fuel.
                ...
                let solutions: RefCell<Vec<Option<c009::Solution>>> =
                ...
                for (sub_idx, challenge) in challenges.iter().enumerate() {
                    ...
                    let save_solution_fn = |solution: &c009::Solution| -> Result<()> {
                        solutions.borrow_mut()[sub_idx] = Some(solution.clone());
                        Ok(())
                    };

                    let _result = solve_challenge_fn(
                        challenge,
                        &save_solution_fn,
                ...
                let solutions_vec: Vec<c009::Solution> = solutions
                    .into_inner()
                    .into_iter()
                    .map(|s| s.unwrap_or_else(c009::Solution::new))
                    .collect();
